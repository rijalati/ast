...
#                 Copyright (c) 2006-2015 Roland Mainz                 #
...
#
# Copyright (c) 2006, 2015, Roland Mainz. All rights reserved.
#
...
# wget --http-user="I accept www.opensource.org/licenses/eclipse" --http-passwd="." 'http://www.research.att.com/sw/download/beta/INIT.2012-02-14.tgz'
...
[[ -d "$HOME/ekopath-5.0.1/bin/" ]] && PATH+=":$HOME/ekopath-5.0.1/bin/"
...
/*
 * Copyright (c) 2006, 2015, Roland Mainz <roland.mainz@nrubsig.org>
 */
...
function build_shell
{
	...
	case "${buildmode}" in
		*.linux.*)
			...
			#
			# Extra warnings:
			# -fstrict-aliasing -Wstrict-aliasing to catch issues with >= C99 aliasing rules
			# -fstrict-overflow -Wstrict-overflow to catch issues with >= C99 integer overflow rules
			# -Wsequence-point to find sequence point issues
			# [gcc] -Wclobbered to find variables which may get clobbered by |longjmp()| if not |volatile| or better scoped
			# -Waddress to catch pointer/address abuse
			# -Wno-parentheses to avoid being harassed by warnings which are about gcc's preferred coding-style and not issues (in most cases)
			# -Wno-unused to avoid too much useless warning spam by gcc
			# -Wno-trigraphs we don't use trigraphs
			# -Wuninitialized warn about uninitialised variables
			#
			# * ToDo:
			...
			case "${buildmode}" in
				...
				*.pcc.c1x*)	bgcc_frontend='pcc'    bgcc_stdlangmode='-std=gnu1x -D_GNU_SOURCE=1' bextra_warnings='-fstrict-aliasing -Wstrict-aliasing -fstrict-overflow -Wstrict-overflow -Wsequence-point -Wno-parentheses -Wno-unused -Wno-trigraphs -Wuninitialized -Wclobbered -Waddress' ;;
				*)		fatal_error "build_shell: Illegal frontend (clang/gcc/pathcc/pcc) \"${buildmode}\"." ;;
			esac

			# xipo/lto require optimiser flags near -flto
			if [[ ${buildmode} == *.xipo.*.gcc.* ]] ; then
				bgcc99="${bgcc_frontend} ${bgcc_stdlangmode} ${bgcc_optdebug_flags} -flto -fuse-linker-plugin"
				bgcc_ccflags="${bast_flags} ${bextra_warnings}"
			else
				bgcc99="${bgcc_frontend} ${bgcc_stdlangmode}"
				bgcc_ccflags="${bast_flags} ${bgcc_optdebug_flags} ${bextra_warnings}"
			fi
			...
			case "${buildmode}" in
				...
				*.i386.32bit*)
					...
					;;
				*.i386.64bit*)
					HOSTTYPE='linux.i386-64' CC="${bgcc99} -m64 -fPIC" cc_sharedlib='-shared' CCFLAGS="${bgcc_ccflags}"
					;;
				# Linux s390/s390x
				*.s390.32bit*)
					HOSTTYPE='s390'    CC="${bgcc99} -m32 -fPIC" cc_sharedlib='-shared' CCFLAGS="${bgcc_ccflags}"
					;;
				*.s390.64bit*)
					HOSTTYPE='s390-64' CC="${bgcc99} -m64 -fPIC" cc_sharedlib='-shared' CCFLAGS="${bgcc_ccflags}"
					;;
				*)
		...
		*.solaris.*)
			...
			case "${buildmode}" in
				*.xipo.*)
					bsuncc_optdebug_flags='-xO4 -xipo=2 -xipo_archive=readonly -xalias_level=std'
					bgcc_optdebug_flags='-O2'
					;;
				*.opt.*)
				...
				*.debug.*)
					...
					# we use -fno-builtin to prevent false reports from valgrind
					bgcc_optdebug_flags='-g -fno-builtin'
					;;
				*)
					fatal_error "build_shell: Illegal optimiser flag \"${buildmode}\"."
					;;
			...
			# Fixme: Add -Wclobbered -Waddress if Solaris /usr/bin/gcc starts to support it
			bgcc_warnflags='-Wall -Wextra -Wno-unknown-pragmas -Wno-missing-braces -Wno-sign-compare -Wno-parentheses -Wno-implicit-function-declaration -Wno-unused -Wno-trigraphs -Wno-char-subscripts -Wno-switch -Wsequence-point'
			...
			case "${buildmode}" in
				...
				*.i386.64bit.suncc*)  HOSTTYPE='sol11.i386-64' CC="${bsunc99} -m64 -KPIC"  	  cc_sharedlib='-G' CCFLAGS="${bsuncc_ccflags}"  ;;
				...
				*.sparc.64bit.suncc*) HOSTTYPE='sol11.sun4-64' CC="${bsunc99} -m64 -dalign -KPIC"    cc_sharedlib='-G' CCFLAGS="${bsuncc_ccflags}" bsuncc_app_ccflags="${bsuncc_app_ccflags_sparc}" ;;
			
				*.i386.32bit.gcc*)  HOSTTYPE='sol11.i386'    CC="${bgcc99} -fPIC"  					  cc_sharedlib='-shared' CCFLAGS="${bgcc_ccflags}"  ;;
				*.i386.64bit.gcc*)  HOSTTYPE='sol11.i386-64' CC="${bgcc99} -m64 -mtune=opteron -Ui386 -U__i386 -fPIC"	  cc_sharedlib='-shared' CCFLAGS="${bgcc_ccflags}"  ;;
				*.sparc.32bit.gcc*) HOSTTYPE='sol11.sun4'    CC="${bgcc99} -m32 -mcpu=v8 -fPIC"				  cc_sharedlib='-shared' CCFLAGS="${bgcc_ccflags}"  ;;
				*.sparc.64bit.gcc*) HOSTTYPE='sol11.sun4-64' CC="${bgcc99} -m64 -mcpu=v9 -fPIC"				  cc_sharedlib='-shared' CCFLAGS="${bgcc_ccflags}"  ;;
				*.s390.32bit.gcc*)  HOSTTYPE='sol11.s390'    CC="${bgcc99} -m32	      -fPIC"				  cc_sharedlib='-shared' CCFLAGS="${bgcc_ccflags}"  ;;
				*.s390.64bit.gcc*)  HOSTTYPE='sol11.s390-64' CC="${bgcc99} -m64	      -fPIC"				  cc_sharedlib='-shared' CCFLAGS="${bgcc_ccflags}"  ;;
	...
	CC="$CC -lm -ldl" # workaround for ast-ksh.2013-05-24 *ONLY*
	...
	if [[ "${buildmode}" != *.staticshell* ]] ; then
		...
		for lib in 'libast' 'libdll' 'libcmd' 'libcoshell' 'libsum' 'libshell' ; do
			case "${lib}" in
				'libshell')
					base='src/cmd/ksh93/'
					vers=1
					link="-L${root}/lib/ -l${link_libcmd} -l${link_libcoshell} -l${link_libdll} -l${link_libast} -lm"
					;;
				'libcmd')
					base="src/lib/${lib}"
					vers=1
					link="-L${root}/lib/ -l${link_libsum} -l${link_libast} -lm"
					;;
				'libdll')
					base="src/lib/${lib}"
					vers=1
					link="-L${root}/lib/ -l${link_libast} -ldl -lm"
					;;
				'libast')
					base="src/lib/${lib}"
					vers=1
			...
			(
				...
				if [[ "${buildmode}" == *solaris* ]] ; then
					...
					# "-Wl,-soname=${lib}.so.${vers}" is needed to set ELF SONAME
					${CC} ${cc_sharedlib} ${CCFLAGS} -Wl,--whole-archive -Wl,-zmuldefs "${lib_a}" -Wl,--no-whole-archive "-Wl,-soname=${lib}.so.${vers}" -o "${root}/lib/${lib}.so.${vers}" ${link}
				fi
		...
		(
			...
			cd "${root}/src/cmd/msggen/"
			${CC} ${CCFLAGS} ${bsuncc_app_ccflags} -L${root}/lib/ -o msggen msggen.o -last -lm
			${CC} ${CCFLAGS} ${bsuncc_app_ccflags} -L${root}/lib/ -o msgget msgget.o -last -lm
			${CC} ${CCFLAGS} ${bsuncc_app_ccflags} -L${root}/lib/ -o msgcvt msgcvt.o -last -lm

			cd "${root}/src/cmd/ksh93/"
			if [[ "${buildmode}" == *solaris* ]] ; then
				...
				${CC} ${CCFLAGS} ${bsuncc_app_ccflags} -L${root}/lib/ -Bdirect -o shcomp shcomp.o -lshell -Bstatic -l${link_libcmd} -Bdynamic -lsum -lcoshell -ldll -last -lm -lmd -lsecdb
			else
				${CC} ${CCFLAGS} ${bsuncc_app_ccflags} -L${root}/lib/ -o ksh pmain.o -lshell -lcmd -lcoshell -lsum -ldll -last -lm
				${CC} ${CCFLAGS} ${bsuncc_app_ccflags} -L${root}/lib/ -o shcomp shcomp.o -lshell -lcmd -lcoshell -lsum -ldll -last -lm
			fi
...
function test_shell
{
	...
	case "${buildmode}" in
		...
		'testshell.valgrind.memcheck')
			val_tools='memcheck'
			;&
		'testshell.valgrind.sgcheck'|'testshell.valgrind.exp-sgcheck')
			[[ -z "${val_tools}" ]] && val_tools='exp-sgcheck'
			;&
		'testshell.valgrind')
			[[ -z "${val_tools}" ]] && val_tools='memcheck exp-sgcheck'
			...
			for lang in ${TEST_LANG} ; do
				(
					...
					for i in ./src/cmd/ksh93/tests/*.sh ; do
						for val_tool in ${val_tools} ; do
							logdir="$PWD/valgrindlogs$RANDOM"
							mkdir "${logdir}"
							# we don't use VMALLOC_OPTIONS=abort for performance
							valgrind \
								"--tool=${val_tool}" \
								--trace-children=yes \
								--read-var-info=yes \
								--num-callers=50 \
								"--log-file=${logdir}/%p.log" \
								${SHELL} ./src/cmd/ksh93/tests/shtests \
									...
									VMALLOC_OPTIONS=' ' \
	    								SHCOMP=$PWD/arch/*/bin/shcomp \
									"$i"
							( # subshell to disable xtrace
								set +o xtrace
								printf '# valgrind logs begin for %s/%s/%s\n' "${lang}" "${i}" "${val_tool}"
								for logfile in "${logdir}"/*.log ; do
									if [[ "$(fgrep 'at 0x' <"$logfile")" != '' ]] ; then
										cat "${logfile}"
									fi
									rm "${logfile}"
								done
								printf '# valgrind logs end\n'
								rmdir "${logdir}"
							)
						done
					done
		...
		'testshell')
			for lang in ${TEST_LANG} ; do
				(
					export LC_ALL="${lang}" LANG="${lang}"
					for i in ./src/cmd/ksh93/tests/*.sh ; do 
						${SHELL} ./src/cmd/ksh93/tests/shtests --locale \
							LD_LIBRARY_PATH_64="$LD_LIBRARY_PATH_64" \
							LD_LIBRARY_PATH="$LD_LIBRARY_PATH" \
							LD_LIBRARY_PATH_32="$LD_LIBRARY_PATH_32" \
							LC_ALL="${lang}" LANG="${lang}" \
