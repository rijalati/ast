...
= "[-?\n@(#)$Id: probe (AT&T Research) 2007-02-22 $\n]" USAGE_LICENSE
  "[+NAME?probe - generate/install/display language processor probe "
  "information]"
  ...
  "[d:delete?Delete the current information. Information can only be deleted "
  "by"
  "	the generating user.]"
  "[f:force?Force information generation even if it is out of date. Only "
  "probe"
  "	information files with write mode disabled can be regenerated.]"
  "[g:generate?Generate the probe information if it is out of date. Only "
  "probe"
  "	information files with write mode disabled can be regenerated.]"
...
main(int argc, char **argv)
{
    ...
    if ((options & TEST) || !(ps.st_mode & S_IWUSR)) {
        ...
        *(script + n) = 0;
        if (suid >= 0)
            strcpy(base, key);
        else if (!(path = pathprobe(language,
                                    tool,
                                    processor,
                                    -1,
                                    NiL,
    ...
    if (options & DELETE) {
        ...
        if (!(options & (FORCE | TEST)) && ps.st_mode
            ...
            if (!(options & SILENT))
                error(0,
                      "probing %s language processor %s for %s information",
                      language,
                      processor,
                      tool);
            ...
            cmdenvv[n++] = sentinel;
            cmdenvv[n] = 0;
            if (!(pp = procopen(script,
                                cmdargv,
                                cmdenvv,
                                NiL,
                                PROC_UID | PROC_GID
                                | ((options & TEST) ? 0 : PROC_READ))))
                n = -1;
            else if (!(options & TEST)
                     && (!(pf = sfnew(NiL, NiL, SF_UNBOUND, pp->rfd, SF_READ))
                         || !(pp->rfd = -1)
                         || sfmove(pf, fp, SF_UNBOUND, -1) < 0 || sfclose(pf)
                         || sfclose(fp)))
                n = -2;
            else
                n = 0;
        ...
        if (!error_info.errors) {
            if (options & OVERRIDE) {
                ...
                while (base > path && (*--base != '/' || --n))
                    ;
                if (n || !strneq(base, "/lib/", 5)
                    || !strneq(base + 5, probe, strlen(probe)))
                ...
                if (!(options & KEY)) {
                            ...
                            *s = 0;
                            if (access(cmd, F_OK)
                                && mkdir(cmd,
                                         ...
                                         | S_IXOTH))
                                error(ERROR_SYSTEM | 3,
                                      "%s: cannot create override directory",
                                      cmd);
                        }
                        if (!(pf = sfopen(NiL, cmd, "w")))
                    ...
                    if (sfmove(fp, pf, SF_UNBOUND, -1) < 0 || sfclose(fp)
                        || sfclose(pf))
