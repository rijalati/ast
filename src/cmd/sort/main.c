...
#define pathstdin(s)                                                         \
    (!(s) || streq(s, "-") || streq(s, "/dev/stdin") || streq(s, "/dev/fd/0"))
#define pathstdout(s)                                                        \
    (!(s) || streq(s, "-") || streq(s, "/dev/stdout")                        \
     || streq(s, "/dev/fd/1"))
...
parse(Sort_t *sp, char **argv)
{
    ...
    for (;;)
    {
        ...
        case 'C':
            ...
            key->disc->eventf
            = opt_info.option[1] == 'C' ? verify_silent : verify;
            continue;
        case 'E':
        case 'J':
            sfsprintf(
            opt, sizeof(opt), "%c%s", opt_info.option[1], opt_info.arg);
                ...
                if (!*e || *e == '.' && e > (s + 1))
                {
                               ...
                               != REC_V_ATTRIBUTES(key->disc->data)))
                    {
                        error(2,
                              ...
                              fmtrec(r, 0),
                              key->input[i],
                              fmtrec(key->disc->data, 0));
                        return 1;
                    }
                    if (RECTYPE(r) != REC_variable
                        || RECTYPE(key->disc->data) != REC_variable
                        || REC_V_SIZE(key->disc->data) < REC_V_SIZE(r))
                        key->disc->data = r;
                    i = n;
                }
            }
            ...
            || n == REC_M_data))
    {
        ...
        if (z && key->disc->data == REC_N_TYPE())
            error(3,
                  "%s: record format cannot be determined from data sample",
                  key->input[0]);
    }
        ...
        && (stat(key->output, &st) || S_ISREG(st.st_mode)))
    {
        ...
        if (!strchr(s, '%'))
        {
            ...
            if (!(e = strrchr(s, '.')))
                e = s + strlen(s);
            if (RECTYPE(key->disc->data) == REC_variable
                && !REC_V_SIZE(key->disc->data))
                key->disc->data |= ((1 << 15) - 1);
            if (!(s = strdup(sfprints("%-*.*s%%%s%s",
                                      e - p,
                                      e - p,
                                      p,
                                      fmtrec(key->disc->data, 1),
                                      e))))
                error(ERROR_SYSTEM | 3, "out of space");
            key->output = s;
            if (sp->verbose)
    ...
    if (sp->verbose)
        error(
        0, "%s %s record format", error_info.id, fmtrec(key->disc->data, 0));
...
init(Sort_t *sp, Rskeydisc_t *dp, char **argv)
{
    ...
    dp->flags = 0;
    dp->errorf = errorf;
    if (!(sp->key = key = rskeyopen(dp, NiL))
        || !(sp->rec = rsnew(key->disc)))
        return -1;
    ...
    if (sp->single = !key->input[1])
    {
        ...
        if (!S_ISREG(is.st_mode)
            || sp->opened->disc) /* XXX: need sfio call to test if any disc
                                    pushed */
        {
            sp->total = 0;
            sp->test |= TEST_read;
    ...
    if (key->nproc > 1)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                = (key->procsize > chunk)
                  ? chunk
                  : chunk / ((chunk + key->procsize - 1) / key->procsize);
            }
        }
    }
    key->insize = sp->end = x;
                    ...
                    if (!pathstdin(s))
                    {
                                 ...
                                 && os.st_ino == is.st_ino)
                        {
                            ...
                            if (sp->overwrite
                                ...
                                sp->op
                                = sfnew(NiL, NiL, SF_UNBOUND, n, SF_WRITE);
                            if (t)
                                *t = '/';
                            if (!sp->op || fstat(n, &is))
                                error(ERROR_SYSTEM | 3,
                                      "%s: cannot create overwrite file %s",
                                      key->output,
                                      sp->overwrite);
                            if (os.st_uid != is.st_uid
                                || os.st_gid != is.st_gid)
                                sp->preserve = 1;
                            break;
                        }
        ...
        else
        {
            ...
            if (!sp->buf)
            {
                if (m < 0 && n < -m && z == sp->end)
                {
                    ...
                    if (sp->verbose && n)
                        error(0,
                              "%s buffer boundary expand to %I*d",
                              error_info.id,
                              sizeof(n),
                              n);
                }
                if (!sp->buf && n > 0
                    && !(sp->buf = sfreserve(ip, n, SF_LOCKR)))
                    n = -1;
            }
...
jobs(Sort_t *sp)
{
    ...
    for (jp = sp->jobs; jp < xp; jp++)
    {
        ...
        case 0:
            sp->child = 1;
            sfsprintf(
            id, sizeof(id), "%s#%d", error_info.id, jp - sp->jobs + 1);
            error_info.id = id;
            sp->end = jp->chunk;
...
done(Sort_t *sp)
{
    while (rsdisc(sp->rec, NiL, RS_POP))
        ;
    if ((sfsync(sp->op) || sp->op != sfstdout && rsfileclose(sp->rec, sp->op))
        && !error_info.errors)
        error(ERROR_SYSTEM | 2, "%s: write error", sp->key->output);
    if (rsclose(sp->rec))
        error(2, "sort error");
