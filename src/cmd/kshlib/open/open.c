...
static const Namdisc_t timedisc = {
    sizeof(struct dctime), put_time, get_time, 0, 0, create_time,
};


static Namval_t *
make_time(Namval_t *np)
{
 ...
 */
static Shfield_t *
sh_findfield(Shfield_t *ftable, int nelem, const char *name)
{
    Shfield_t *fp = ftable;
    int i, n;
    const char *cp;
...
static Namval_t *
sh_newnode(Shfield_t *fp, Namval_t *np)
{
...
static char *
walk_class(Namval_t *np, int dlete, struct dcclass *dcp)
{
    static Sfio_t *out;
    Sfio_t *outfile;
...
= { sizeof(struct dcclass), put_classval, get_classval, 0, 0, fieldcreate };

static int
mkclass(Namval_t *np, Shclass_t *sp)
{
    struct dcclass *tcp
...
static Shclass_t Fileclass = { sizeof(filefield) / sizeof(*filefield),
...
extern int
b_open(int argc, char *argv[], Shbltin_t *context)
{
    Namval_t *np;
    int n, oflag = 0;
