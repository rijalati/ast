...
= "[-?@(#)$Id: Dbm_t (AT&T Research) 2012-08-23 $\n]" USAGE_LICENSE
  "[+NAME?Dbm_t - create an associative array containing contents of a dbm "
  "file]"
  "[+DESCRIPTION?\bDbm_t\b is a declaration command that creates an "
  "associative "
  ...
  "[+?Unless hte \b-T\b option is specified, the keys in the file cannot "
  "contain "
  ...
  "[r:read?Open for read access only.  \avname\a becomes a readonly "
  "variable.]"
...
static const Namdisc_t *
array_disc(Namval_t *np)
{
    Namarr_t *ap;
    const Namdisc_t *dp;
...
static size_t
check_size(char **buff, size_t olds, size_t news)
{
    if (news >= olds) {
        if (olds) {
            while ((olds *= 2) <= news)
...
dbm_get(struct dbm_array *ap)
{
    ...
    if (data.dsize && (val = ( char * ) data.dptr)) {
        ...
        if (err = dbm_error(ap->dbm)) {
            dbm_clearerr(ap->dbm);
            error(ERROR_system(err),
                  "Unable to get key %.*s",
                  ap->key.dsize - ap->addzero,
                  ap->key.dptr);
        }
    }
}
...
dbm_associative(Namval_t *np, const char *sp, int mode)
{
    ...
    default:
        if (sp) {
            ...
            if (keylen != ap->key.dsize || !ap->key.dptr
                || strcmp(sp, ap->key.dptr)) {
                if (ap->modified)
                    dbm_put(ap);
                ap->key.dsize = keylen;
                ap->namlen = check_size(&ap->name, ap->namlen, ap->key.dsize);
            ...
            if (mode & NV_AADD) {
                if (ap->shp->subshell)
                    sfprintf(sfstderr,
                             "subshell=%d subscript=%s will be modified, "
                             "need to save \n",
                             ap->shp->subshell,
                             sp);
                ap->modified = 1;
...
static int
dbm_create(int argc, char **argv, Shbltin_t *context)
{
