...
static int
match(int, char *[], Shbltin_t *);
static Namval_t *
create_dss(Namval_t *, const char *, int, Namfun_t *);
static Namval_t *
create_type(Namval_t *, const char *, int, Namfun_t *);
 ...
 */
static int
query(int argc, char *argv[], Shbltin_t *bp)
{
    struct parent *dp
    = ( struct parent * )nv_hasdisc(bp->vnode, &parent_disc);
    Dss_t *dss = dp->dss;
    ...
    if (!pp || argv[1]) {
        if (!pp) {
            if (!(qp = cxquery(cx, dp->hdr.names[n], cx->disc)))
                errormsg(
                SH_DICT, ERROR_exit(1), "%s: load method %s", argv[0]);
...
static void
check_numeric(Namval_t *np, Cxtype_t *tp, Cxstate_t *sp)
{
    if (cxisnumber(tp)) {
...
static Namval_t *
typenode(const char *name, int flag)
{
...
Cxvalue_t *
get_child_common(Namval_t *np, Namfun_t *fp, Cxoperand_t *op)
{
    ...
    if (nv_isattr(pp->hdr.np, NV_INTEGER)) {
        ...
        if ((*tp->member->getf)(pp->cx, &in, op, NiL, NiL, NiL, pp->cx->disc)
            == 0)
            return (&op->value);
    }
    return (0);
...
static char *
get_child(Namval_t *np, Namfun_t *fp)
{
    Cxvariable_t *vp = vnode(np);
    Cxoperand_t op;
    Cxvalue_t *val;
...
static char *
get_mchild(Namval_t *np, Namfun_t *fp)
{
    Cxoperand_t ret;
    ...
    if (nv_isattr(np, NV_BINARY) && !nv_isattr(np, NV_RAW)) {
        if (cxcast(dp->cx, &ret, vp, dp->state->type_buffer, rp, ( char * )0)
            == 0) {
            nv_setsize(np, ret.value.buffer.size);
            value = ret.value.buffer.data;
        }
    } else if (cxcast(
               dp->cx, &ret, vp, dp->state->type_string, rp, ( char * )0)
               == 0)
...
static Sfdouble_t
nget_child(Namval_t *np, Namfun_t *fp)
{
    Cxvariable_t *vp = vnode(np);
    Cxoperand_t op;
    Cxvalue_t *val;
...
static Sfdouble_t
nget_mchild(Namval_t *np, Namfun_t *fp)
{
    Cxoperand_t ret;
    Namchld_t *pp = ( Namchld_t * )fp;
    struct parent *dp = ( struct parent * )pp->ptype;
    ...
    if (cxcast(dp->cx,
               &ret,
               vp,
               dp->state->type_number,
               dp->hdr.np->nvalue,
               ( char * )0)
        == 0)
#else
    if (cxcast(
        dp->cx, &ret, vp, dp->state->type_number, dp->hdr.data, ( char * )0)
        == 0)
#endif
...
static void
put_child(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    Namchld_t *cp = ( Namchld_t * )fp;
    struct parent *pp = ( struct parent * )cp->ptype;
    Cxtype_t *tp = pp->type;
    ...
    if ((*tp->member->setf)(pp->cx, &in, &ret, &op, NiL, NiL, pp->cx->disc)
        == 0)
        nv_putval(pp->hdr.np,
                  ( char * )&ret.value.number,
                  NV_INTEGER | NV_DOUBLE | NV_LONG | NV_NODISC);
}

static void
put_mchild(Namval_t *np, const char *val, int flag, Namfun_t *fp)
{}

static char *
name_child(Namval_t *np, Namfun_t *fp)
{
    Namchld_t *pp = ( Namchld_t * )fp;
    struct parent *dp = ( struct parent * )pp->ptype;
...
static Namval_t *
type_child(Namval_t *np, Namfun_t *fp)
{
    struct parent *pp = ( struct parent * )(( Namchld_t * )fp)->ptype;
    Cxstate_t *sp = pp->state;
    Cxvariable_t *vp = vnode(np);
    if (!pp->dss || vp->type == sp->type_number
        || vp->type == sp->type_string)
        return (0);
...
= { sizeof(Namchld_t), put_child, get_child, nget_child, 0, 0, 0,
    name_child,        0,         type_child };

/* for child variables of methods */
static const Namdisc_t mchild_disc
= { sizeof(Namchld_t), put_mchild, get_mchild, nget_mchild, 0, 0, 0,
    name_child,        0,          type_child };

static const Namdisc_t type_disc;

static Namval_t *
node(Cxvariable_t *vp, struct parent *dp)
{
    Namval_t *mp, *np = 0;
    Namfun_t *fp, *nfp = 0;
    if (!dp->hdr.nodes) {
        ...
        if (mp != dp->hdr.np && pp && pp->hdr.fun.disc == &parent_disc
            && pp->hdr.nodes)
    ...
    if (!np->nvname) {
        ...
        if (!dp->dss && vp->type->base && (mp = typenode(vp->type->name, 0))
            && (fp = nv_hasdisc(mp, &type_disc))) {
            int size = fp->dsize;
...
static Namfun_t *
clone_parent(Namval_t *np, Namval_t *mp, int flags, Namfun_t *fp)
{
    Namtype_t *dp;
    struct type *tp;
...
static void
pushtype(Namval_t *np, Cxtype_t *tt, Namfun_t *fp)
{
    Namval_t *mp;
...
static Namval_t *
create_parent(Namval_t *np, const char *name, int flag, Namfun_t *fp)
{
...
static void
dss_unset(Namval_t *np, struct parent *dp)
{
    if (dp->namelen)
        free(( void * )dp->name);
    if (!dp->hdr.np) {
...
static void
put_parent(Namval_t *np, const char *val, int flag, Namfun_t *fp)
{
    ...
    if (!val) {
        ...
        dss_unset(np, pp);
    } else if (nq = nv_open(val,
                            sh.var_tree,
                            NV_VARNAME | NV_ARRAY | NV_NOADD | NV_NOFAIL)) {
        Namfun_t *pp;
...
static int
dss_except(Sfio_t *iop, int event, void *data, Sfdisc_t *fp)
{
    if (event == SF_CLOSING) {
        struct dssfile *sp = ( struct dssfile * )fp;
        if (sp && sp->np) {
            Namfun_t *nfp;
            for (nfp = sp->np->nvfun; nfp && !nfp->disc && !nfp->disc->readf;
                 nfp = nfp->next)
                ;
            if (nfp)
...
static int
read_parent(Namval_t *np, Sfio_t *iop, int delim, Namfun_t *fp)
{
    Namval_t *mp;
    struct parent *dp = ( struct parent * )fp;
    struct dssfile *sp;
    if (!iop)
        ...
        sp->np = np;
        if (!((sp->fp
               = dssfopen(dp->dss, ( char * )0, iop, DSS_FILE_READ, 0)))) {
            errormsg(
            SH_DICT, ERROR_exit(1), "%d: invalid dss format", sffileno(iop));
            return (1);
        }
...
static int
write_parent(Namval_t *np, Sfio_t *iop, int delim, Namfun_t *fp)
{
    struct parent *dp = ( struct parent * )fp;
    struct dssfile *sp;
    if (sfraise(iop, DSS_EVENT, ( void * )&sp) == 0) {
        ...
        sp->np = np;
        if (!((sp->fp
               = dssfopen(dp->dss, ( char * )0, iop, DSS_FILE_WRITE, 0)))) {
            errormsg(SH_DICT,
                     ERROR_exit(1),
                     "%d: cannot open for dss write",
                     sffileno(iop));
            return (1);
        }
        sp->disc.exceptf = dss_except;
...
static Namval_t *
next_parent(Namval_t *np, Dt_t *root, Namfun_t *fp)
{
    struct parent *dp = ( struct parent * )fp;
    Cxvariable_t *vp = 0;
    if (root && (np != nv_namptr(dp->hdr.nodes, 0))) {
        if ((fp = nv_hasdisc(np, &mchild_disc))
            || (fp = nv_hasdisc(np, &child_disc)))
...
static const Namdisc_t parent_disc
= { sizeof(struct parent), put_parent,   0, 0,           0,
    create_parent,         clone_parent, 0, next_parent, 0,
    read_parent,           write_parent };
 ...
 */
static char *
get_parent(Namval_t *np, Namfun_t *fp)
{
#if 1
    char *data = np->nvalue;
...
#    endif
    ...
    if (size > 0) {
        ...
        m = base64encode(
        data, size, ( void ** )0, cp = getbuf(n), n, ( void ** )0);
#else
    if (rp && rp->size > 0) {
        size_t m, n = (4 * rp->size) / 3 + rp->size / 45 + 8;
        m = base64encode(
        rp->data, rp->size, ( void ** )0, cp = getbuf(n), n, ( void ** )0);
#endif
        nv_setsize(np, m);
        return (cp);
    }
...
static const Namdisc_t parent2_disc = { 0, 0, get_parent };

/*
 * add discipline builtin given by name to type
 */
static Namval_t *
add_discipline(const char *typename,
               const char *name,
               int (*fun)(int, char *[], Shbltin_t *),
               void *context)
{
    Namval_t *mp;
    int offset = stktell(stkstd);
...
static void
put_type(Namval_t *np, const char *val, int flag, Namfun_t *fp)
{
    ...
    if (val && !(flag & NV_INTEGER) && tp->type->internalf
        && !cxisstring(tp->type)) {
        size_t size = strlen(val);
        cop.type = tp->type;
        if ((*tp->type->internalf)(
            tp->cx, tp->type, NiL, NiL, &cop, val, size, Vmregion, &Dssdisc)
            < 0)
            errormsg(SH_DICT,
                     ERROR_exit(1),
                     "%s: cannot covert to type dss.%s",
                     val,
                     tp->type->name);
        if (cxisnumber(cop.type))
            ...
            nv_putv(
            np, ( char * )cop.value.buffer.data, NV_RAW | NV_BINARY, fp);
        }
    } else
        nv_putv(np, val, flag, fp);
    if (!val) {
...
static char *
get_type(Namval_t *np, Namfun_t *fp)
{
    struct type *tp = ( struct type * )fp;
    struct parent *dp;
    Cxvalue_t cval;
    ...
    for (i = 0; i < 2; i++) {
        n = (*tp->type->externalf)(
        tp->cx, tp->type, format, NiL, &cval, buf, buflen, &Dssdisc);
        if (n < buflen)
            break;
        buf = getbuf(n);
    }
...
static char *
setdisc_type(Namval_t *np, const char *event, Namval_t *action, Namfun_t *fp)
{
    struct type *tp = ( struct type * )fp;
    int n = -1;
...
create_type(Namval_t *np, const char *name, int flag, Namfun_t *fp)
{
    ...
    if (memcmp(name, NV_DATA, sizeof(NV_DATA) - 1) == 0
        && (name[sizeof(NV_DATA) - 1] == 0
            || strchr("+=[", name[sizeof(NV_DATA) - 1]))) {
...
static const Namdisc_t type_disc
= { 0,           put_type,   get_type, 0,        setdisc_type,
    create_type, clone_type, 0,        next_type };
...
static void
mktype(Namval_t *np, Cxtype_t *tp, Cx_t *cx)
{
    struct type *pp;
...
static Namval_t *
create_dss(Namval_t *np, const char *name, int flag, Namfun_t *fp)
{
    Cxvariable_t *vp;
    Cxstate_t *sp;
    Cxtype_t *tp = 0;
    ...
    for (n = 0, vp = ( Cxvariable_t * )dtfirst(dict); vp;
         vp = ( Cxvariable_t * )dtnext(dict, vp)) {
        Namval_t *qp;
...
#endif
        vp->data = ( void * )n++;
        if (vp->type == sp->type_number || vp->type == sp->type_string
            || vp->type == sp->type_buffer)
            continue;
        if (!(qp = typenode(vp->type->name, 0))) {
            if (!vp->type->member
                && (qp = typenode(vp->type->name, NV_ADD))) {
...
static char *
name_dss(Namval_t *np, Namfun_t *fp)
{
...
static const Namdisc_t dss_disc = {
    0, 0, 0, 0, 0, create_dss, 0, name_dss,
};
...
= "[-1c?\n@(#)$Id: dss.list (AT&T Research) 2007-05-09 $\n]" USAGE_LICENSE
  ...
  "[+?If multiple entities types are written, then the entity type is "
  "written "
...
static int
listdss(int argc, char *argv[], Shbltin_t *bp)
{
    ...
    while (name = *argv++) {
        if (fval('t') && *name == 't')
            ...
            continue;
        for (tp = ( Cxtype_t * )dtfirst(dict); tp;
             tp = ( Cxtype_t * )dtnext(dict, tp)) {
            if (n)
...
= "[-1c?\n@(#)$Id: dss.load (AT&T Research) 2003-01-10 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?\b\f?\f\b causes each of the specified dss libraries "
  "\alib\a "
...
static int
loadlib(int argc, char *argv[], Shbltin_t *bp)
{
...
static Cxvalue_t *
getvalue(Namval_t *np, Namfun_t *fp, Cxoperand_t *valp)
{
...
#if 1
        cxcast(
        dp->cx, valp, vp, vp->type, dp->hdr.parent->nvalue, ( char * )0);
#else
...
static int
match(int argc, char *argv[], Shbltin_t *bp)
{
    ...
    if (flag) {
        if (!(np = nv_open(*argv,
                           (( Shell_t * )bp->shp)->var_tree,
                           NV_NOADD | NV_VARNAME | NV_NOASSIGN)))
...
lib_init(int flag, void *context)
{
    ...
    for (level = 0; level < 2; level++) {
        char *cp;
        n = 0;
        for (lib = dsslib(NiL, DSS_VERBOSE, &Dssdisc); lib;
