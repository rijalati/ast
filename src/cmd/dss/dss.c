...
= "[-?\n@(#)$Id: dss (AT&T Research) 2012-08-15 $\n]" USAGE_LICENSE
  ...
  "[+SEE ALSO?\bvczip\b(1), \bgzip\b(1), \bbzip2\b(1), \bpzip\b(1), "
  "\bcql\b(1), \bdss::dss\b(5P)]";

static void
showplugin(Dsslib_t *lib,
           const char *style,
           Sfio_t *sp,
           int *sep,
           Dssdisc_t *disc)
...
main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        case 'f':
            if (!(action
                  = strdup(sfprints("{write %s}",
                                    fmtquote(opt_info.arg,
                                             "'",
                                             "'",
                                             strlen(opt_info.arg),
                                             FMT_ALWAYS | FMT_ESCAPED)))))
