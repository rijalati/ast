...
= "[-?\n@(#)$Id: proto (AT&T Research) 2012-02-20 $\n]" USAGE_LICENSE
  ...
  "[o:options?Additional space or \b,\b separated \aname=value\a "
  "\b--license\b"
  "	options.]:[name=value,...]"
...
main(int argc, char **argv)
{
    ...
    if (file)
        do
            flags = proto(
            file, license, options, package, copy, type(file, comment), flags);
        while (file = *++argv);
    else if (!list)
        flags = proto(
        file, license, options, package, copy, type(file, comment), flags);
