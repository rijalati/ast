...
int emacs_read(fd, buff, scend) char *buff;
int fd;
unsigned scend;
{
    int c;
...
static void putstring(s) char *s;
{
    int c;
    while (c = *s++)
        putchar(c);
...
static int escape(out, count) genchar *out;
int count;
{
    int i, value;
    int digit, ch;
...
static void xcommands(count) int count;
{
    int i = ed_getchar();
...
static void search(out, direction) genchar out[];
int direction;
{
...
static void draw(option) DRAWTYPE option;
{
#define NORMAL ' '
#define LOWER '<'
#define BOTH '*'
...
#ifdef MULTIBYTE
static int print(c) int c;
{
    return ((c & ~STRIP) == 0 && isprint(c));
}

static int isword(i) int i;
{
    int c = drawbuff[i];
