...
static void hist_trim(n) int n;
{
    ...
    fdo
    = io_renumber(hist_ptr->fixfd, fcntl(hist_ptr->fixfd, F_DUPFD, USERIO));
    hist_ptr->fixfd = fdo;
...
static int hist_nearend(fd, size) int fd;
off_t size;
{
    int n = 0;
    int state = 0;
...
void hist_list(offset, last, nl) off_t offset;
int last;
char *nl;
{
    int oldc = 0;
...
histloc hist_find(string, index1, flag, direction) char *string;
int index1;
int flag;
int direction;
{
...
int hist_match(offset, string, flag) off_t offset;
char *string;
int flag;
{
    char *cp;
...
int hist_copy(s1, command, line) char *s1;
int command, line;
{
    int c;
    struct history *fp = hist_ptr;
...
char *hist_word(s1, word) char *s1;
int word;
{
    int c;
    char *cp = s1;
...
histloc hist_locate(command, line, lines) int command;
int line;
int lines;
{
    histloc next;
 ...
 */
void hist_subst(command, fd, replace) const char *command;
int fd;
char *replace;
{
    char *new = replace;
...
static void io_init(fd, fp, buf) int fd;
struct fileblk *fp;
char *buf;
{
    if (!fp)
...
static off_t io_seek(fd, offset, ptrname) int fd;
off_t offset;
int ptrname;
{
    struct fileblk *fp;
...
static int io_readbuff(fp) struct fileblk *fp;
{
    int n;
...
static void io_fclose(fd) int fd;
{
    struct fileblk *fp = io_ftable[fd];
...
static int io_mktmp(fname, len) char *fname;
int len;
{
    int fd;
