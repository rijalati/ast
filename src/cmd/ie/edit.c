...
ed_expand(outbuff, cur, eol, mode) char outbuff[];
int *cur;
int *eol;
int mode;
{
...
#        ifdef tenex
static char *overlay(str, newstr) char *str, *newstr;
{
 ...
 */
int ed_virt_to_phys(virt, phys, cur, voff, poff) genchar *virt;
genchar *phys;
int cur, voff, poff;
{
    genchar *sp = virt;
...
int ed_internal(src, dest) unsigned char *src;
genchar *dest;
{
    int c;
    genchar *dp = dest;
...
int ed_external(src, dest) genchar *src;
char *dest;
{
    int c;
    char *dp = dest;
...
int ed_gencpy(dp, sp) genchar *dp;
genchar *sp;
{
    while (*dp++ = *sp++)
        ;
...
int ed_genncpy(dp, sp, n) genchar *dp;
genchar *sp;
int n;
{
    while (n-- > 0 && (*dp++ = *sp++))
...
ed_setwidth(string) char *string;
{
    int indx = 0;
    int state = 0;
    int c;
 ...
 */
static int compare(a, b, n) char *a;
char *b;
int n;
{
    while (n-- > 0) {
...
tcgetattr(fd, tt) struct termios *tt;
{
    int r;
    int i;
    tcgeta = 0;
