...
static struct fileblk outfile
= { ed_errbuf, ed_errbuf, ed_errbuf + IOBSIZE, 2, IOWRT };
static int editfd;
static int output = 0;
...
extern ssize_t read(fd, buf, n) int fd;
void *buf;
size_t n;
{
    int r, flag;
...
extern ssize_t _read(fd, buf, n) int fd;
void *buf;
size_t n;
{
...
extern ssize_t __read(fd, buf, n) int fd;
void *buf;
size_t n;
{
    return (read(fd, buf, n));
...
extern ssize_t _libc_read(fd, buf, n) int fd;
void *buf;
size_t n;
{
    return (read(fd, buf, n));
...
extern ssize_t __libc_read(fd, buf, n) int fd;
void *buf;
size_t n;
{
    return (read(fd, buf, n));
...
void p_str(string, c) char *string;
int c;
{
    struct fileblk *fp = io_ftable[output];
    int cc;
...
char *ed_movstr(a, b) const char *a;
char *b;
{
    while (*b++ = *a++)
        ;
...
void ed_failed(name, message) char *name, *message;
{
    p_setout(ERRIO);
    p_str(name, ' ');
    p_char(':');
 ...
 * print a prompt
 */
void pr_prompt(string) char *string;
{
    int c;
