...
#else
    if (is_option(VIRAW)
        || (last_virt >= 0 && virtual[last_virt] == term_char))
        getline(APPEND);
    else
        getline(ESC);
#endif /* RAWONLY */
...
static void append(c, mode) int c;
int mode;
{
    int i;
...
static void backword(nwords, cmd) int nwords;
int cmd;
{
...
static void delete (nchars, mode) int nchars;
char mode;
{
    int i;
    genchar *vp;
...
static void del_line(mode) int mode;
{
...
static int delmotion(motion, mode) int motion;
char mode;
{
    int begin;
    int end;
...
static void endword(nwords, cmd) int nwords;
int cmd;
{
    int tcur_virt = cur_virt;
    while (nwords--)
...
static void forward(nwords, cmd) int nwords;
char cmd;
{
    int tcur_virt = cur_virt;
    while (nwords--)
...
static void getline(mode) int mode;
{
    int c;
    int tmp;
...
static int mvcursor(motion) int motion;
{
    int count;
    int tcur_virt;
    int incr = -1;
...
static void pr_string(s) char *s;
{
    /*** copy string s ***/
...
static void putstring(col, nchars) int col;
int nchars;
{
    while (nchars--)
...
static void refresh(mode) int mode;
{
    int p;
    int regb;
...
static void replace(c, increment) int c;
int increment;
{
    int cur_window;
    ...
    cur_window = cur_phys - first_wind;
    if (ocur_virt == INVALID || !is_print(c) || !is_print(virtual[cur_virt])
        || !is_print(o_v_char)
#ifdef MULTIBYTE
...
static int search(mode) char mode;
{
    int new_direction;
    int oldcurhline;
...
static int textmod(c, mode) int c;
int mode;
{
    int i;
    genchar *p = lastline;
