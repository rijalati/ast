...
= "[-?\n@(#)$Id: rectify (AT&T Research) 1999-03-22 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?\brectify\b induces fixed length record groups from input "
  "data"
    ...
    while (s = sfgetr(dp, '\n', 0))
    {
            ...
            if (offset < state.offset)
            {
                ...
                if ((offset + skip) > state.offset)
                {
                    ...
                    if (sfseek(fp, skip, SEEK_CUR) < 0)
                        error(ERROR_SYSTEM | 3,
                              "%s: cannot seek %I*d bytes at %I*d",
                              file,
                              sizeof(skip),
                              skip,
                              sizeof(offset),
        ...
        case 't':
            ...
            do
            {
                    ...
                    != id[state.type[state.typelast].len])
                {
                    ...
                    if (verbose && offset >= state.offset)
                        sfprintf(sfstdout,
                                 ...
                                 sizeof(offset),
                                 offset,
                                 2 * state.type[state.typelast].len,
                                 id[state.type[state.typelast].len]);
                    context = 0;
                }
                ...
                if (!(p = sfreserve(fp,
                                    size,
                                    state.type[state.typelast].unit ? SF_LOCKR
                                                                    : 0)))
                    error(ERROR_SYSTEM | 3,
                          "cannot read %ld bytes at %I*d",
                          size,
...
main(int argc, char **argv)
{
    ...
    for (;;)
    {
        ...
        case 'd':
            if (desc)
                error(
                2, "%s: only one description file allowed", opt_info.arg);
            else
                desc = opt_info.arg;
            continue;
        case 'f':
    ...
    sfsprintf(
    state.format4, n, "%%%s%%%s%%%s%%%s", format, format, format, format);
