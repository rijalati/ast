...
= "[-?\n@(#)$Id: pop (AT&T Research) 2003-04-05 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?\bpop\b operates on partitioned fixed row and column data "
  "files."
...
label(Pz_t *pz, Pzpart_t *pp, int op, size_t *map, size_t m, char *format)
{
    ...
    while (s = sfgetr(sp, '\n', 1))
    {
        ...
        if (lp->end >= pp->row)
            error(3,
                  "format entry extends beyond %I*d row size",
                  sizeof(pp->row),
                  pp->row);
    ...
    if (lp->end != (pp->row - 1))
        error(3,
              "format file row size %d does not match expected %I*d",
              lp->end + 1,
              sizeof(pp->row),
              pp->row);
    xv[lp->beg] = lp;
    ...
    if (op & OP_INFO)
    {
        ...
        if ((rows = gather(pz, pp, pz->io, ip, map, m)) < 0)
            return 1;
        sfprintf(sfstdout,
                 "%s frequency info over %I*d rows\n\n",
                 (op & OP_LO) ? "low" : "high",
                 sizeof(rows),
                 rows);
        sfprintf(
        sfstdout, "%33s %3s %6s %3s\n\n", "FIELD", "COL", "FREQ", "VAL");
        if (op & OP_LO)
            g = map[0];
        else
            g = 0;
