...
= "[-?\n@(#)$Id: pin (AT&T Research) 2003-07-17 $\n]" USAGE_LICENSE
  ...
  "[S:size?Ignore \b--row\b, determine the fixed record size based on a "
  "window"
  ...
  "}"
  "[V:verify?Verify \b--size\b by reading all data instead of the window "
  "sample.]"
  ...
  "\nfile ...\n"
  "\n"
  "[+SEE ALSO?\bbzip\b(1), \bgzip\b(1), \bpop\b(1), \bpzip\b(1), "
  "\bpzip\b(3)]";
...
typedef void (*Optimize_f)(struct Optimize_method_s *,
                           unsigned char *,
                           unsigned char *,
                           size_t **,
                           size_t *,
...
typedef void (*Reorder_f)(struct Reorder_method_s *,
                          unsigned char *,
                          unsigned char *,
                          int *,
                          size_t,
    ...
    if (pz || high)
    {
        ...
        else
        {
            ...
            if (state.cache && (sp = sfopen(NiL, state.cachefile, "r")))
            {
                ...
                if (state.verbose)
                    sfprintf(sfstderr,
                             "filter using %s for frequencies\n",
                             state.cachefile);
                ...
                for (j = 0; j < row; j++)
                    if (!state.stats[j].prev)
                        error(
                        3, "invalid cache -- column %d frequency omitted", j);
                sfclose(sp);
                error_info.file = 0;
                error_info.line = 0;
            }
            ...
            if (high < 0)
            {
                ...
                if (state.verbose)
                    sfprintf(sfstderr,
                             "%d high frequency column%s out of %d\n",
                             high,
                             high == 1 ? "" : "s",
                             row);
            }
            if (maxhigh < 0)
            {
                ...
                if (high > (row * maxhigh / 100))
                    error(6,
                          "high frequency count %d exceeds %d%% of %d",
                          high,
                          maxhigh,
                          row);
            }
...
static void
reorder_heuristic(Reorder_method_t *method,
                  unsigned char *buf,
                  unsigned char *dat,
                  int *lab,
                  size_t row,
        ...
        if (k == 2)
        {
            ...
            for (tp = cur; tp < cp; tp++)
                for (j = 0; j < tp->elements; j++)
                {
                    ...
                    np->elements = ii;
                    cst[tp->member[j]]
                    = tp->size - part(buf, dat, np, row, tot);
...
static void
reorder_tsp(Reorder_method_t *method,
            unsigned char *buf,
            unsigned char *dat,
            int *lab,
            size_t row,
            size_t tot)
{
    ...
    disc.errorf = errorf;
    if (!(tsp = tspopen(
          &disc, cost, row, TSP_DFS | (state.verbose ? TSP_VERBOSE : 0))))
...
static void
optimize_dynamic(Optimize_method_t *method,
                 unsigned char *buf,
                 unsigned char *dat,
                 size_t **val,
                 size_t *cst,
...
static void
optimize_greedy(Optimize_method_t *method,
                unsigned char *buf,
                unsigned char *dat,
                size_t **val,
                size_t *cst,
...
static void
optimize_transitive(Optimize_method_t *method,
                    unsigned char *buf,
                    unsigned char *dat,
                    size_t **val,
                    size_t *cst,
...
static Optimize_method_t optimize_methods[] = {
    { "dynamic", "dynamic program optimal partition", optimize_dynamic },
    { "greedy", "greedy approximation partition", optimize_greedy },
    { "none", "no partition", 0 },
    { "transitive",
...
static Reorder_method_t reorder_methods[] = {
    { "heuristic", "heuristic reorder", reorder_heuristic },
    { "none", "no reordering", 0 },
    { "tsp", "tsp reordering", reorder_tsp },
};
...
optinfo(Opt_t *op, Sfio_t *sp, const char *s, Optdisc_t *dp)
{
    ...
    if (streq(s, "optimize_default"))
        ...
        for (i = 0; i < elementsof(optimize_methods); i++)
            n += sfprintf(sp,
                          "[+%s?%s]",
                          optimize_methods[i].name,
                          optimize_methods[i].description);
        ...
        for (i = 0; i < elementsof(reorder_methods); i++)
            n += sfprintf(sp,
                          "[+%s?%s]",
                          reorder_methods[i].name,
                          reorder_methods[i].description);
    return n;
}
...
main(int argc, char **argv)
{
    ...
    for (;;)
    {
        ...
        case 'h':
            ...
            if (*s)
                error(
                3, "%s: %s: invalid number", opt_info.name, opt_info.arg);
            continue;
        case 'l':
    ...
    argv += opt_info.index;
    if (error_info.errors || !(state.input = *argv++)
        || (n = *argv != 0) && !(op & OP_size))
    ...
    for (;;)
    {
        ...
        if (row && (op & OP_verify))
        {
            ...
            if ((m = sfread(ip, dat, win)) < 0)
                m = 0;
            rows = m / row;
            if (filter(ip, &buf, &dat, pz, high, maxhigh, row, row * rows)
                == row)
                row = 0;
        }
        if (!n)
    ...
    if (state.cache)
    {
        ...
        if (!stat(state.cachefile, &cs) && cs.st_mtime < is.st_mtime
            && remove(state.cachefile))
            error(ERROR_SYSTEM | 3,
                  "%s: cannot update intermediate data cache",
                  state.cachefile);
    }
    ...
    if (op & OP_optimize)
    {
        ...
        (*optimize_method->fun)(
        optimize_method, buf, dat, val, cst, sol, row, tot, maxgrp);
    ...
    if (!(op & OP_reorder))
        sfprintf(sfstdout,
                 "# group coalescing limited to adjacent columns\n");
    if (maxgrp)
        sfprintf(sfstdout, "# group size limited to %d columns\n", maxgrp);
    sfprintf(sfstdout,
             "# row %d window %d compression level %d\n",
             rec,
             state.window,
             state.level);
    if (state.sort)
