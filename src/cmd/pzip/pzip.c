...
= "[-?\n@(#)$Id: pzip (AT&T Research) 2003-07-17 $\n]" USAGE_LICENSE
  ...
  "[a:append?Sets the \bPZ_APPEND\b flag that may be used by some "
  "disciplines.]"
  ...
  "[B:bufsize?Set the output buffer size to \asize\a -- for "
  "debugging.]#[size]"
  ...
  "	file.]:[library]"
  "[z:lzw?Use \bcompress\b(1) lzw compression instead of the default "
  "\bgzip\b(1)."
  ...
  "[+BUGS?\bpzip\b decompress currently fails if the standard input is a "
  "pipe."
...
main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        case ':':
            if (!opt_info.option[0] && opt_info.name[0] == opt_info.name[1]
                || opt_info.option[0] == '-' && opt_info.option[1] == '?')
    ...
    if (push) {
        if (*argv)
            error(
            3,
            "%s: file argument not expected for sfdcpzip discipline test",
            *argv);
        if (sfdcpzip(sfstdin, NiL, flags, &disc) < 0)
            error(3, "sfdcpzip discipline push error");
        if (sfmove(sfstdin, sfstdout, SF_UNBOUND, -1) < 0
            || sfclose(sfstdout))
    ...
    if (pz = pzopen(&disc, *argv, flags)) {
        ...
        return ((flags & PZ_WRITE) && pzdeflate(pz, sfstdout)
                || (flags & PZ_READ) && pzinflate(pz, sfstdout))
               || pzclose(pz) || error_info.errors;
    }
    return 1;
}
