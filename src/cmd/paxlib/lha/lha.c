...
{
    return (s[0] == '-' && isalnum(s[1]) && isalnum(s[2]) && isalnum(s[3])
            && s[4] == '-')
           ? 0
           : -1;
}
...
lha_getheader(Pax_t *pax, Paxarchive_t *ap, Paxfile_t *f)
{
    ...
    if (paxread(pax, ap, ss, sizeof(ss), 0, 0) != sizeof(ss)
        || !(hdr = paxget(pax, ap, n = ss[0], NiL)) || lha_method(hdr))
        return 0;
    for (h = 0, i = 0; i < n; i++)
        h += hdr[i];
    if ((h & 0xff) != ss[1])
                    ...
                    if (ar->check) {
                        if (codexdata(sp, &sum) <= 0)
                            ...
                            f->name);
                                 ...
                                 pax, ap, f, ar->checksum, sum.num & 0xffff))
                            r = 1;
                    }
                    break;
                }
