    ...
    if (paxseek(pax, ap, -( off_t ) ZIP_END_HEADER, SEEK_END, 1) > 0
        ...
        while ((hdr = paxget(pax, ap, -ZIP_CEN_HEADER, NiL))
            ...
            if (paxread(pax, ap, mem->name, ( off_t ) n, ( off_t ) 0, 0)
                ...
                pax,
                2,
                "%s: invalid %s format verification header name [size=%I*u]",
                ap->name,
                fp->name,
                sizeof(n),
                n);
            ...
            if (ext
                ...
                (*pax->errorf)(NiL,
                               pax,
                               2,
                               "%s: %s: invalid %s format verification "
                               "header extended data [size=%I*u]",
                               ap->name,
                               mem->name,
                               fp->name,
                               sizeof(ext),
        ...
        if (paxseek(pax, ap, ( off_t ) 0, SEEK_SET, 1)) {
            (*pax->errorf)(NiL,
                           pax,
                           2,
                           "%s: %s central header reposition error",
                           ap->name,
                           fp->name);
            zip_done(pax, ap);
            return -1;
    ...
    while (hdr = paxget(pax, ap, -(num = ZIP_LOC_HEADER), NiL)) {
        ...
        case ZIP_LOC_MAGIC:
            ...
            if (swapget(3, &hdr[ZIP_LOC_FLG], 2) & 0x0001)
                i += sfsprintf(ar->method,
                               sizeof(ar->method),
                               "crypt-zip+SIZE=%I*u|",
                               sizeof(f->st->st_size),
                               f->st->st_size);
            if (!f->st->st_size
                || (m = swapget(3, &hdr[ZIP_LOC_METHOD], 2)) == ZIP_COPY
                || (pax->test & 2))
                i
                += sfsprintf(ar->method + i, sizeof(ar->method) - i, "copy");
            else
                i += sfsprintf(ar->method + i,
                               sizeof(ar->method) - i,
                               "zip-%u+SIZE=%I*u",
                               m,
        ...
        case ZIP_EXT_MAGIC:
            ...
            if (paxread(pax, ap, NiL, ( off_t ) ZIP_EXT_HEADER, ( off_t ) 0, 0)
                ...
                (*pax->errorf)(NiL,
                               pax,
                               2,
                               "%s: invalid %s format verification header",
                               ap->name,
                               ap->format->name);
                return 0;
            }
...
zip_getdata(Pax_t *pax, Paxarchive_t *ap, Paxfile_t *f, int fd)
{
    ...
    if (paxseek(pax, ap, pos, SEEK_SET, 0) != pos) {
        (*pax->errorf)(NiL,
                       pax,
                       2,
                       "%s: %s: cannot seek past %s format data",
                       ap->name,
                       f->name,
                       ap->format->name);
        r = -1;
