...
#if _hdr_nlist && _lib_nlist && _mem_n_name_nlist && _mem_n_type_nlist       \
&& _mem_n_value_nlist
#    define NAMELIST 1
#    include <nlist.h>
#endif
...
#endif
#if _hdr_utmpx
    "/etc/utmpx",  "/var/adm/utmpx", "/var/run/utmpx",
#endif
...
= { "/dev/mouse", "/dev/kbd", "/dev/keybd", "/dev/keyboard" };
...
main(int argc, char **argv)
{
    ...
    if (argv[1] && strcmp(argv[1], data))
    {
        ...
        for (;;)
        {
            ...
            while (!kill(remote->pid, 0))
                update(data,
                       0,
                       CS_STAT_FREQ + (CS_STAT_DOWN - CS_STAT_FREQ) / 2,
                       &ss);
            procclose(remote);
            remote = 0;
            if (ss.up > 0)
        ...
        for (;;)
            update(
            data, 0, CS_STAT_FREQ + (CS_STAT_DOWN - CS_STAT_FREQ) / 2, &ss);
    }
    remove(data);
    ...
    for (i = 0; i < elementsof(usrfiles); i++)
        if ((uf = open(usrfile = usrfiles[i], O_RDONLY)) >= 0)
            break;
    if (uf < 0)
                ...
                if (symbols[n].once >= 0)
                {
                    if (lseek(kf, ( long )names[n].n_value, 0)
                        ...
                        error(ERROR_SYSTEM | 3,
                              ...
                              memfile,
                              names[n].n_name);
                    if (read(kf, symbols[n].addr, symbols[n].size)
                        != symbols[n].size)
                        error(ERROR_SYSTEM | 3,
                              "%s: %s read error",
                              memfile,
...
#    endif
        ...
        update(data,
               now,
               (4 * CS_STAT_FREQ + 2 * (CSTOSS(toss, 0) % (CS_STAT_FREQ + 1)))
               / 5,
               &ss);
    }
}
