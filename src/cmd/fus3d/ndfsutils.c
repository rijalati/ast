...
int
utilgetrealfile(const char *ipath, char *opath)
{
    struct stat st;
...
int
utilgetbottomfile(const char *ipath, char *opath)
{
    struct stat st;
...
static int
filecopy(char *ipath, const char *opath)
{
    char p1[PATH_MAX];
    Sfio_t *ifp, *ofp;
    struct stat st1;
...
int
utilensuretopfile(const char *path)
{
    char p1[PATH_MAX];
...
int
utilensuretopfileparent(const char *path)
{
    char p1[PATH_MAX], *s;
...
int
utilinitdir(Ndfsfile_t *fp, char *tpath, char *bpath)
{
    Dt_t *didict;
    Ndfsdirinfo_t *dip;
    int dipi;
...
int
utilmkspecialpath(const char *ipath, int type, char *opath)
{
    char *pp, *s;
    int l;
...
int
utilisspecialpath(const char *path)
{
    char *pp;
    size_t n;
    int pt;
    ...
    if ((n = strlen(path)) >= NDFS_SPECIALPATH_len
        ...
        && (n == NDFS_SPECIALPATH_len
            || path[n - NDFS_SPECIALPATH_len - 1] == '/')
...
int
utilisspecialname(const char *name, char **rnamep)
{
    int pt;
...
int
utilmkspecialpid(pid_t pid, int insflags, int remflags)
{
    Ndfspid_t *pidp;
...
int
utilisspecialpid(pid_t pid, int *flagsp)
{
    Ndfspid_t *pidp;
