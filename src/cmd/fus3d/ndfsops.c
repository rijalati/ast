...
int
ndfsstatfs(const char *path, struct statvfs *svfsp)
{
...
int
ndfsmkdir(const char *path, mode_t mode)
{
    int pf;
...
int
ndfslink(const char *fpath, const char *tpath)
{
    int pf;
...
int
ndfssymlink(const char *fpath, const char *tpath)
{
    int pf;
...
int
ndfsreadlink(const char *path, char *buf, size_t bufsiz)
{
...
int
ndfschmod(const char *path, mode_t mode)
{
    int pf;
...
int
ndfsutimens(const char *path, const struct timespec ts[2])
{
    int pf;

    log(LOG(2, "ndfsutimens"),
        "utimens %s %ld.%ld %ld.%ld",
        path,
        ts[0].tv_sec,
        ts[0].tv_nsec,
        ts[1].tv_sec,
...
int
ndfsrename(const char *fpath, const char *tpath)
{
    int pf;
...
int
ndfsaccess(const char *path, int mask)
{
    char p1[PATH_MAX], *pp;
    int pf;
...
int
ndfsgetattr(const char *path, struct stat *stp)
{
    char p1[PATH_MAX], *pp;
    char *s;
    size_t n;
...
int
ndfsfgetattr(const char *path, struct stat *stp, struct fuse_file_info *fip)
{
    Ndfsfile_t *fp;
...
int
ndfsopendir(const char *path, struct fuse_file_info *fip)
{
    Ndfsfile_t *fp;
    char p1[PATH_MAX], *pp;
    int pf;
...
int
ndfsreaddir(const char *path,
            void *buf,
            fuse_fill_dir_t fillfunc,
            off_t off,
...
int
ndfsreleasedir(const char *path, struct fuse_file_info *fip)
{
    Ndfsfile_t *fp;
...
int
ndfscreate(const char *path, mode_t mode, struct fuse_file_info *fip)
{
    Ndfsfile_t *fp;
    int pf;
...
int
ndfsopen(const char *path, struct fuse_file_info *fip)
{
    Ndfsfile_t *fp;
    char p1[PATH_MAX], *pp;
    int pf, pt;
...
int
ndfsflush(const char *path, struct fuse_file_info *fip)
{
    Ndfsfile_t *fp;
...
int
ndfsrelease(const char *path, struct fuse_file_info *fip)
{
    Ndfsfile_t *fp;
...
int
ndfstruncate(const char *path, off_t off)
{
    int pf;
...
int
ndfsftruncate(const char *path, off_t off, struct fuse_file_info *fip)
{
    Ndfsfile_t *fp;
...
int
ndfsfsync(const char *path, int flag, struct fuse_file_info *fip)
{
    Ndfsfile_t *fp;
