...
#define SNAPSHOT_EASY                                                        \
    "%(ctime)..64u,%(perm)..64u,%(size)..64u,%(uid)..64u,%(gid)..64u"
...
= "[-?\n@(#)$Id: tw (AT&T Research) 2012-04-11 $\n]" USAGE_LICENSE
  ...
  "\bprintf\b(3) format specification. A default delimiter (\"" SNAPSHOT_DELIM
  "\") and field values are assumed for an empty "
  "snapshot input file. The format fields are:]"
  "{"
  "[+" SNAPSHOT_ID "?The literal string \b" SNAPSHOT_ID
  "\b.]"
  ...
  "\"" SNAPSHOT_HARD
  "\". This part is computed only if the easy part "
  "has changed.]"
  ...
  "[+tw -m -d / -e \"fstype == '/'.fstype && mtime > "
  "'/etc/backup.time'.mtime\"?"
...
act(Ftw_t *ftw, int op)
{
    ...
    case ACT_CODETYPE:
        ...
        if (findwrite(state.find,
                      ftw->path,
                      ftw->pathlen,
                      magictype(state.magic, fp, PATH(ftw), &ftw->statb)))
            state.finderror = 1;
        if (fp)
            sfclose(fp);
        break;
...
tw(Ftw_t *ftw)
{
    ...
    case FTW_DNX:
        ...
        break;
    case FTW_DP:
        if (!(state.ftwflags & FTW_TWICE)
            || (state.ftwflags & FTW_DOT) && stat(PATH(ftw), &ftw->statb))
            goto pop;
        break;
    case FTW_D:
        ftw->ignorecase
        = (state.icase
           || (!ftw->level || !ftw->parent->ignorecase)
              && strchr(astconf("PATH_ATTRIBUTES", ftw->name, NiL), 'c'))
          ? STR_ICASE
          : 0;
        break;
    default:
...
order(Ftw_t *f1, Ftw_t *f2)
{
    ...
    for (;;) {
        ...
        case ID:
            ...
            if (x->data.variable.symbol->index == F_name
    ...
    message(
    (-2, "order(%s,%s) = %d [n1=%ld n2=%ld]", f1->name, f2->name, v, n1, n2));
    return v;
}
...
main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        case 'G':
            ...
            if (streq(opt_info.arg, "old"))
                disc.flags |= FIND_OLD;
            else if (streq(opt_info.arg, "gnu")
                     || streq(opt_info.arg, "locate"))
                disc.flags |= FIND_GNU;
    ...
    if (state.snapshot.tmp)
        sfprintf(sfstdout,
                 ...
                 SNAPSHOT_ID,
                 state.snapshot.format.delim,
                 state.snapshot.format.path,
                 state.snapshot.format.delim,
                 state.snapshot.format.easy,
                 state.snapshot.format.delim,
                 state.snapshot.format.hard ? state.snapshot.format.hard : "",
                 state.snapshot.format.delim);
    if (x = exexpr(state.program, "begin", NiL, 0))
        eval(x, NiL);
    if ((x = exexpr(state.program, "select", NiL, INTEGER))
        || (x = exexpr(state.program, NiL, NiL, INTEGER)))
        state.select = x;
