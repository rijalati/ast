...
= "[-1p1?@(#)$Id: find (AT&T Research) 2013-08-02 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?\bfind\b recursively descends the directory hierarchy for "
  "each"
        ...
        if (s) {
            ...
            case 'l':
                ...
                = S_ISLNK(ent->fts_statp->st_mode)
                  && pathgetlink(PATH(ent), fp->tmp, sizeof(fp->tmp)) > 0
                  ? fp->tmp
                  : "";
                break;
            default:
        ...
        if (argp->type & Op) {
            ...
            if (!(argp->type & Unary)) {
                ...
                case Num:
                    ...
                    if (*e)
                        error(1,
                              "%s: invalid character%s after number",
                              e,
                              *(e + 1) ? "s" : "");
                    break;
                default:
...
execute(State_t *state, FTSENT *ent)
{
    ...
    case FTS_D:
        ...
        ent->ignorecase
        = (state->icase
           || (!ent->fts_level || !ent->fts_parent->ignorecase)
              && strchr(astconf("PATH_ATTRIBUTES", ent->fts_name, NiL), 'c'))
          ? STR_ICASE
          : 0;
        break;
    default:
    ...
    while (np) {
        ...
        case LOCAL:
            ...
            goto num;
        case XTYPE:
            val = ((state->walkflags & FTS_PHYSICAL) ? stat(PATH(ent), &st)
                                                     : lstat(PATH(ent), &st))
                  ? 0
                  : st.st_mode;
            goto type;
        ...
        case INAME:
            ...
            val = strgrpmatch(
                  ...
                  | (np->action == INAME ? STR_ICASE : ent->ignorecase))
                  != 0;
            if (bp)
                *bp = '/';
            break;
        case LNAME:
            val
            = S_ISLNK(ent->fts_statp->st_mode)
              && pathgetlink(PATH(ent), state->txt, sizeof(state->txt)) > 0
              && strgrpmatch(state->txt,
                             np->first.cp,
                             NiL,
                             0,
                             STR_MAXIMAL | STR_LEFT | STR_RIGHT
        ...
        case ILNAME:
            ...
            = S_ISLNK(ent->fts_statp->st_mode)
              ...
              && strgrpmatch(state->txt,
                             ...
                             NiL,
                             0,
                             STR_MAXIMAL | STR_LEFT | STR_RIGHT | STR_ICASE);
            break;
        case PATH:
            val = strgrpmatch(ent->fts_path,
                              np->first.cp,
                              NiL,
                              0,
                              STR_MAXIMAL | STR_LEFT | STR_RIGHT
        ...
        case MAGIC:
            ...
            val
            = strmatch(magictype(state->magic, fp, PATH(ent), ent->fts_statp),
                       np->first.cp)
              != 0;
            if (fp)
                sfclose(fp);
        ...
        case REGEX:
            if (!(val = regnexec(
                  np->second.re, ent->fts_path, ent->fts_pathlen, NiL, 0, 0)))
                val = 1;
        ...
        case LS:
            fmtls(state->buf,
                  ...
                  NiL,
                  S_ISLNK(ent->fts_statp->st_mode)
                  && pathgetlink(PATH(ent), state->txt, sizeof(state->txt))
                     > 0
                  ? state->txt
                  : NiL,
                  LS_LONG | LS_INUMBER | LS_BLOCKS);
...
order(FTSENT *const *p1, FTSENT *const *p2)
{
    ...
    default:
        ...
        /*FALLTHROUGH*/
    case NAME:
        n = state->icase ? strcasecmp(f1->fts_name, f2->fts_name)
                         : strcoll(f1->fts_name, f2->fts_name);
        goto done;
    }
    if (n1 < n2)
        n = -1;
...
main(int argc, char **argv)
{
    ...
    state.maxlevel = ~0;
    state.walkflags
    = FTS_PHYSICAL | FTS_NOSTAT | FTS_NOPOSTORDER | FTS_SEEDOTDIR;
    ...
    for (ap = commands; ap->name; ap++) {
        ...
        if (ap->arg)
            sfprintf(
            state.str, "%c[%s]", (ap->type & Num) ? '#' : ':', ap->arg);
        if (ap->values)
