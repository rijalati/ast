...
#define STAT(f, b)                                                           \
    ((state.ftwflags & FTW_PHYSICAL) ? lstat(f, b) : pathstat(f, b))
       ...
       Exdisc_t *disc)
{
    ...
    case F_symlink:
        if (!S_ISLNK(st->st_mode)
            || pathgetlink(PATH(ftw), text, sizeof(text)) < 0)
            text[0] = 0;
        v.string = text;
        goto string;
    case F_type:
    ...
    default:
        ...
        default:
            error(3,
                  "%s: invalid identifier reference [ index=%d memindex=%d "
                  "memoffset=%d ]",
                  sym->name,
                  sym->index,
                  MEMINDEX(sym->index),
...
initvisit(Exid_t *sym)
{
    ...
    if (!state.vistab) {
        ...
        if (!(state.vistab = dtopen(disc, Dtset)))
            error(
            3, "%s: cannot allocate file identifier hash table", sym->name);
    }
}
            ...
            case F_visit:
                if (!ref->next) {
                    if (!state.vistab)
                        initvisit(sym);
                    sym->index
                    = MEMMAKE(ref->symbol->index, state.visitmem++);
                    break;
                }
                /*FALLTHROUGH*/
            default:
                error(
                3, "invalid %s.%s reference", ref->symbol->name, sym->name);
                break;
            }
            ref = ref->next;
        }
