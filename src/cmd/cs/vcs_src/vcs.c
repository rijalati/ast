...
void *svc_init(handle, fdmax) void *handle;
int fdmax;
{
    char pidfile[80];
    char pid[80];
...
static int svc_connect(handle, fd, id, clone, args) void *handle;
int fd;
Cs_id_t *id;
int clone;
char **args;
...
static int version_info(buf, size, uid, own, uptm) char *buf;
int size;
int uid;
int own;
time_t uptm;
{
    return sfsprintf(buf,
                     size,
                     "I Hi! %s, vcs[%d] by %s@%s at %s [%s]\n",
                     fmtuid(uid),
                     getpid(),
                     fmtuid(own),
                     csname(0),
...
static int svc_read(handle, fd) void *handle;
int fd;
{
...
static int svc_timeout(handle) void *handle;
{
    return (0);
}
