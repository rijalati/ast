 ...
 */
int NetFillCache(nFile, timeout) NetFile *nFile;
int timeout;
{
    int rtn, num;
    Cs_poll_t fds;
 ...
 */
int NetDataReady(nFile) NetFile *nFile;
{
    if (nFile->size == 0)
 ...
 */
int NetRead(nFile, buf, bufsize) NetFile *nFile;
char *buf;
int bufsize;
{
    int readlen;
 ...
 */
char *NetGets(nFile, buf, bufsize) NetFile *nFile;
char *buf;
int bufsize;
{
    char *ptr;
 ...
 */
int NetWrite(nFile, buf, bufsize) NetFile *nFile;
char *buf;
int bufsize;
{
    int fd, writelen, len;
 ...
 */
int NetClose(nFile) NetFile *nFile;
{
    int rtn;
 ...
 */
NetFile *NetConnect(srv, host, port) struct server_info *srv;
char *host;
int port;
{
