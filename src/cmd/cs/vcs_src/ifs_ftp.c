 ...
 */
int ftp_addentry(fname, ftime) char *fname;
time_t ftime;
{
    FILE *flog, *fent;
 ...
 */
int ftp_doentry(filepath) char *filepath;
{
    struct stat st;
    char *ptr;
 ...
 */
int ftp_parselist(fi, buf) struct fileinfo *fi;
char *buf;
{
    char *ptr;
    ...
    *ptr++ = '\0';
    ...
    case 'l':
        fi->ftype = 2; /* unix link file */
        fi->info = ptr;
        if ((ptr = strrchr(buf, ' ')) == NULL || strcmp(ptr, " ->") != 0)
            return 0;
        *ptr = '\0';
 ...
 */
int ftp_makedents(tmpfile) char *tmpfile;
{
 ...
 *name: FtpCommand
 */
int FtpCommand(nFile, cmd, arg) NetFile *nFile;
char *cmd, *arg;
{
    char buf[STRLEN];
 ...
 *name: FtpReply
 */
int FtpReply(nFile, buf, bsize) NetFile *nFile;
char *buf;
int bsize;
{
    char tmpbuf[STRLEN];
 ...
 *name: FtpTalk
 */
int FtpTalk(nFile, cmd, arg) NetFile *nFile;
char *cmd, *arg;
{
 ...
 *name: FtpDataConnect
 */
NetFile *FtpDataConnect(srv, nFile) struct server_info *srv;
NetFile *nFile;
{
    char reply[STRLEN];
    char host[STRLEN];
 ...
 *name: FtpXfer
 */
int FtpXfer(srv, cmd, rpath, tmpfile) struct server_info *srv;
char *cmd;
char *rpath;
char *tmpfile;
{
 ...
 *name: FtpXferFile
 */
int FtpXferFile(srv, tmpfile) struct server_info *srv;
char *tmpfile;
{
 ...
 *name: FtpXferDir
 */
int FtpXferDir(srv, tmpfile) struct server_info *srv;
char *tmpfile;
{
 ...
 */
int FtpConnect(srv) struct server_info *srv;
{
 ...
 */
int FtpDisconnect(srv) struct server_info *srv;
{
    struct mount_item *mitem = srv->mitem;
    NetFile *nFile = mitem->nFile;
 ...
 */
int FtpGetFile(srv) struct server_info *srv;
{
    struct mount_item *mitem = srv->mitem;
    char tmpfile[STRLEN];
    char *lpath = srv->lpath;
 ...
 */
int FtpPutFile(srv) struct server_info *srv;
{
    struct mount_item *mitem = srv->mitem;
    NetFile *nFile, *nData;
    char buf[STRLEN];
