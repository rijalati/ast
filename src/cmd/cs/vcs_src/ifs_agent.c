 ...
 * rtn: NULL if error
 */
struct agent_item *AgentRegister(name, initfunc) char *name;
FUNC *initfunc;
{
 ...
 */
struct agent_item *AgentSearch(name) char *name;
{
 ...
 *name: IfsAbortSignal
 */
void IfsAbortSignal(sig) int sig;
{
    IfsAbortFlag = 1;
}
 ...
 */
struct mount_list *MountListSearch(uid) int uid;
{
...
void MountItemDump(mi) struct mount_item *mi;
{
    char buf[1024], *ptr;
    int n;
 ...
 */
struct mount_item *
AgentMount(ml, url, localpath, option) struct mount_list *ml;
char *url;
char *localpath;
char *option;
{
 ...
 * rtn: -1 if error
 */
int AgentUnmount(ml, lpath) struct mount_list *ml;
char *lpath;
{
 ...
 * rtn: -1 if error
 */
int AgentAutoMount(ml, lpath) struct mount_list *ml;
char *lpath;
{
    char *ptr, *url, *urlend;
...
struct mount_item *ag_find_mount(ml, lpath, rpath) struct mount_list *ml;
char *lpath, *rpath;
{
    struct mount_item *mi;
    char pathbuf[STRLEN];
...
int ag_make_remote(mi, rpath, buf, size) struct mount_item *mi;
char *rpath, *buf;
int size;
{
    char *end = buf + size;
...
int ag_showmount(ml, lpath) struct mount_list *ml;
char *lpath;
{
...
int ag_parsepath(srv, uid, argc, argv, proxy) struct server_info *srv;
int uid, argc;
char *argv[];
char *proxy;
{
...
cserrmsg()
{
    ...
    case E_PROTOCOL:
        ...
        break;
    case E_OPENDEST:
        msg = "dest-file open error";
        break;
    case E_GETHOST:
        msg = "unknown hostname";
        break;
    case E_SOCKET:
        msg = "can't open stream socket";
        break;
    case E_CONNECT:
    ...
    case E_DATAXFER:
        ...
        break;
    default:
        msg = "undefined error number";
        break;
    }
    return msg;
}

int csputmsg(buf, bsize, rcode) char *buf;
int bsize;
int rcode;
{
    int len;
 ...
 *name: IfsCacheValid
 */
int IfsCacheValid(fpath, expire) char *fpath;
 ...
 */
int IfsMount(ret, retsize, uid, argc, argv) char *ret;
int uid, argc;
char *argv[];
{
    struct mount_list *ml;
 ...
 */
int IfsConnect(ret, retsize, uid, argc, argv) char *ret;
int argc;
char *argv[];
{
    struct server_info srv;
 ...
 */
int IfsDisconnect(ret, retsize, uid, argc, argv) char *ret;
int argc;
char *argv[];
{
    struct server_info srv;
 ...
 */
int IfsListDEnts(ret, retsize, uid, argc, argv) char *ret;
int argc;
char *argv[];
{
    struct server_info srv;
 ...
 */
int IfsGetFile(ret, retsize, uid, argc, argv) char *ret;
int argc;
char *argv[];
{
    struct server_info srv;
 ...
 */
int IfsReal(ret, retsize, uid, argc, argv) char *ret;
int retsize;
int uid;
int argc;
char *argv[];
 ...
 */
int IfsPutFile(ret, retsize, uid, argc, argv) char *ret;
int argc;
char *argv[];
{
    struct server_info srv;
 ...
 */
int IfsUserDef(ret, retsize, uid, argc, argv) char *ret;
int argc;
char *argv[];
{
    struct server_info srv;
