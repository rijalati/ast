...
char *stamp2version(list, t) rdirent_t *list;
time_t t;
{
    char buf[15];
    char *s;
...
int locking(fd) Sfio_t *fd;
{
    return (0);
}


int unlocking(fd) Sfio_t *fd;
{
    return (0);
}
 ...
 *  s is modified in place with '\0' replacing occurrences of separator char
 *c. w is an array of n char pointers to receive addresses of the fields.  The
 *  return value gives the number of fields actually found; additional entries
 *  in w are given the address of a null string.
 */

int chop(s, w, n, c) char *s;
char *w[];
int n;
char c;
{
...
int permission(st, uid, gid) struct stat *st;
uid_t uid;
uid_t gid;
{
    if (uid == st->st_uid)
        return (st->st_mode & S_IWUSR ? 0 : st->st_mode & S_IRUSR ? 1 : 2);
    else if (gid == st->st_gid)
        return (st->st_mode & S_IWGRP ? 0 : st->st_mode & S_IRGRP ? 1 : 2);
    else
        return (st->st_mode & S_IWOTH ? 0 : st->st_mode & S_IROTH ? 1 : 2);
}
