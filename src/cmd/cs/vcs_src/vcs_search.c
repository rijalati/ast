 ...
 *
 */
int search_tag(rf, ap, versions, domain, tpp, mode, dir) Sfio_t *rf;
attr_t *ap;
char *versions;
int domain;
tag_t **tpp;
...
int gettagbyspec(list, versions, domain, tpp) rdirent_t *list;
char *versions;
int domain;
tag_t **tpp;
{
...
int lookup_tag_time(tbl, total, to, domain, first) rdirent_t **tbl;
int total;
time_t to;
int domain;
int first;
...
int lookup_tag(entries, rf, ap, version, domain, tpp, mode) rdirent_t *entries;
Sfio_t *rf;
attr_t *ap;
char *version;
int domain;
...
int lookup_tag_t(tbl, total, version, domain, mode) rdirent_t **tbl;
int total;
char *version;
int domain;
int mode;
...
int pattern2time(pattern, to) char *pattern;
time_t *to;
{
    char buf[1024];
    char *s;
...
#define LINIT() (yy = 0, ss = 59, mm = 59, hh = 23)
    ...
    if (yy)
        sfsprintf(
        tbuf, sizeof(tbuf), "%s %d %d %d:%d:%d", month, dd, yy, hh, mm, ss);
    else
        sfsprintf(
...
tag_t *gettagbytime(list, mtime) rdirent_t *list;
time_t mtime;
{
    rdirent_t *ep;
...
tag_t *gettagbyname(list, name, mode, level) rdirent_t *list;
char *name;
int mode;
int level;
{
