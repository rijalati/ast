 ...
 *name: NewsCommand
 */
int NewsCommand(nFile, cmd, arg) NetFile *nFile;
char *cmd, *arg;
{
    char buf[STRLEN];
 ...
 *name: NewsReply
 */
int NewsReply(nFile, buf, bsize) NetFile *nFile;
char *buf;
int bsize;
{
    char tmpbuf[STRLEN];
 ...
 *name: NewsXferFile
 */
int NewsXferFile(nFile, destfile) NetFile *nFile;
char *destfile;
{
    FILE *fp;
    char buf[STRLEN], *ptr;
 ...
 *name: NewsListGroups
 */
int NewsListGroups(srv) struct server_info *srv;
{
 ...
 *name: NewsEnterGroup
 */
int NewsEnterGroup(srv, group, lpath) struct server_info *srv;
char *group;
char *lpath;
{
    struct mount_item *mitem = srv->mitem;
 ...
 *name: NewsGetArticle
 */
int NewsGetArticle(srv, group, article) struct server_info *srv;
char *group;
char *article;
{
    struct mount_item *mitem = srv->mitem;
 ...
 *name: NewsConnect
 */
int NewsConnect(srv) struct server_info *srv;
{
    struct mount_item *mitem = srv->mitem;
    NetFile *nFile = mitem->nFile;
    int port;
 ...
 *name: NewsDisconnect
 */
int NewsDisconnect(srv) struct server_info *srv;
{
    struct mount_item *mitem = srv->mitem;
    NetFile *nFile = mitem->nFile;
 ...
 *name: NewsGetFile
 */
int NewsGetFile(srv) struct server_info *srv;
{
    struct mount_item *mitem = srv->mitem;
    char group[STRLEN];
    char *rpath, *article;
 ...
 *name: NewsPutFile
 */
int NewsPutFile(srv) struct server_info *srv;
{
    return 0;
}
 ...
 *name: NewsNop
 */
int NewsNop(srv) struct server_info *srv;
{
    return 0;
}
