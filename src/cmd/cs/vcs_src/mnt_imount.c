...
#define cmdeq(s, i, l)                                                       \
    (strncmp(s, COMMAND[i].name, (l < COMMAND[i].len ? l : COMMAND[i].len))  \
     == 0)
...
#define PROMPT()                                                             \
    {                                                                        \
        printf("[%s]> ", (istate.fd > 0 ? istate.cs_svc : "IM"));            \
        if (xjade)                                                           \
            printf("\n");                                                    \
        fflush(stdout);                                                      \
    }
...
int im_help_help(cmd) char *cmd;
{
    printf("\t%s command\n", cmd);
    return (0);
}

int im_help(argc, argv) int argc;
char **argv;
{
    int i;
    char *s;
...
int im_system(argc, argv) int argc;
char **argv;
{
    char *s;
    char buf[2048];
