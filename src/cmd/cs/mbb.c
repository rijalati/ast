...
= "[-?\n@(#)$Id: mbb (AT&T Research) 2000-05-09 $\n]" USAGE_LICENSE
  ...
  "[+PROTOCOL?Channel 0 is for service control messages. The server and "
  "clients"
    ...
    if ((force || to->blocked[log] < 0)
        ...
        if (!force && !state->logged) {
            ...
            if (!state->logs[log].sp) {
                ...
                if (!(state->logs[log].sp
                    ...
                    error(ERROR_SYSTEM | 3,
                          "%s: cannot create message log",
                          state->logs[log].name);
                message(
                (-1, "[%d] %s: create log", __LINE__, state->logs[log].name));
            }
            message((-1,
                     "[%d] %s: %d log",
                     __LINE__,
            ...
            if (sfwrite(state->logs[log].sp, s, n) != n)
                error(ERROR_SYSTEM | 3,
                      "%s: log file write error",
                      state->logs[log].name);
            if ((state->logs[log].offset += n) >= HOG
                && !state->logs[!log].sp)
                state->log = !log;
        }
        if (to->blocked[log] < 0) {
...
dump(Css_t *css, Connection_t *con, int log, Cssdisc_t *disc)
{
    ...
    if (n > CHUNK)
        n = CHUNK;
    if (sfseek(state->logs[log].sp, con->blocked[log], SEEK_SET)
        != con->blocked[log])
        error(ERROR_SYSTEM | 3,
              "%s: cannot seek to %I*d",
              state->logs[log].name,
              sizeof(con->blocked[log]),
              con->blocked[log]);
    message((-1,
             "[%d] %s reserve n %I*d offset %I*d",
             __LINE__,
             state->logs[log].name,
             sizeof(n),
             n,
    ...
    if (state->logs[log].sp
        && sfseek(state->logs[log].sp, state->logs[log].offset, SEEK_SET)
           != state->logs[log].offset)
        error(ERROR_SYSTEM | 3,
              "%s: cannot seek to %I*d",
              state->logs[log].name,
              sizeof(state->logs[log].offset),
              state->logs[log].offset);
    return r;
}
