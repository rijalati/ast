...
= "[-?\n@(#)$Id: cs (AT&T Research) 2006-06-11 $\n]" USAGE_LICENSE
  ...
  "[a:attribute?List the attribute \aname\a for each host. If \aname\a is "
  "\b-\b"
  ...
  "	more output.]#[level]"
  "[f:continuous?Used with \b--cat\b to list messages on a \apath\a or "
  "standard"
  "	input that is continuously updated.]"
  "[h:hostenvironment?Lists \bHOSTNAME\b=\aname\a \bHOSTTYPE\b=\atype\a on "
  "the"
  ...
  "	output.]"
  "[p:process?List the active connect stream process ids on the standard "
  "output.]"
  ...
  "[t:translate?\ahost\a name operands are translated to IP address dot "
  "notation"
  ...
  "[O:open-flags?Set optional \bcsopen\b(3) flags. Used by the \bcs\b(3) "
  "library"
  ...
  "[+SEE ALSO?\bcoshell\b(1), \bcss\b(1), \bpackage\b(1), \bss\b(1), "
  "\bcs\b(3)]";
...
address(const char *name)
{
    ...
    if (addr = csaddr(name))
        sfprintf(
        sfstdout,
        "name=%s addr=%s host=%s user=%s flags=:%s%s%s%s%s\n",
        csfull(addr),
        csntoa(addr),
        cs.host,
        cs.user,
        ...
        (cs.flags
         & (CS_ADDR_LOCAL | CS_ADDR_NUMERIC | CS_ADDR_SHARE | CS_ADDR_REMOTE))
        ? ""
        : ":");
    else
        ...
        if (ftw->level > elementsof(label)) {
            ...
            *s = CS_MNT_PROCESS;
            ...
            if (state.list & LIST_SERVICE) {
                ...
                *(s - 1) = 0;
                sfprintf(sfstdout,
                         "%c%s  1 %-8s %-8s %7s %s %s%s%s\n",
                         label[0][0],
                         fmtmode(mode, 0) + 1,
                         fmtuid(st.st_uid),
                         (mode & S_IROTH) ? "other"
                                          : fmtgid(ftw->statb.st_gid),
                         port,
                         time_buf,
                         label[2],
                         qual_buf,
                ...
                if (!(mode & S_IROTH)) {
                    if (!(mode & S_IRGRP)) {
                        ...
                        if (ftw->statb.st_gid != getegid())
                            n += sfprintf(
                            sfstdout, "=%s", fmtgid(ftw->statb.st_gid));
                    }
                }
                if (*ftw->name == '-')
                    ...
                    if (!pathpath(port_buf,
                                  "",
                                  PATH_ABSOLUTE | PATH_EXECUTE,
                                  serv_buf,
                                  sizeof(serv_buf))
                        || stat(serv_buf, &st) || st.st_uid != uid)
            ...
            if (*argv) {
                ...
                if (dup(fd) != 0 || dup(fd) != 1)
                    error(ERROR_SYSTEM | 3,
                          "%s: cannot redirect connect stream",
                          path);
                close(fd);
                if (!(initiate & CS_OPEN_TEST)
                    && csdaemon((1 << 0) | (1 << 1)))
                    error(ERROR_SYSTEM | 3,
                          "%s: cannot dive into background",
                          path);
                procopen(
                *argv, argv, NiL, NiL, PROC_OVERLAY | PROC_UID | PROC_GID);
                error(
                ERROR_SYSTEM | 4, "%s: %s: cannot execute", path, *argv);
            }
            sfprintf(sfstdout, "%s\n", cspath(fd, 0));
        }
    } else if (interactive)
