...
= "@(#)$Id: cs.tst-old (AT&T Research) 1997-11-11 $\0\n";
...
svc_read(void *handle, int fd)
{
    ...
    if (isalpha(dat[0]) && (dat[1] == 0 || isdigit(dat[1])))
        ...
        n = sfsprintf(buf,
                      sizeof(buf),
                      "I [%s] server=%s version=%s %s=%s server.pid=%d "
                      "pid=%d uid=%d gid=%d clone=%d args=`%s'\n",
                      fmttime(*dat ? dat : "%K", time(NiL)),
                      csname(0),
                      TST_VERSION,
                      CS_HOST_LOCAL,
                      csntoa(state->id[fd].id.hid),
                      getpid(),
                      state->id[fd].id.pid,
                      state->id[fd].id.uid,
                      state->id[fd].id.gid,
...
main(int argc, char **argv)
{
    NoP(argc);
    csserve(
    NiL, argv[1], svc_init, NiL, svc_connect, svc_read, NiL, svc_timeout);
    exit(1);
}
