...
actionf(Css_t *css, Cssfd_t *fp, Cssdisc_t *disc)
{
    ...
    case CS_POLL_READ:
        ...
        if (isalpha(dat[0]) && (dat[1] == 0 || isdigit(dat[1])))
            ...
            default:
                n
                = sfsprintf(buf, sizeof(buf), "E %s: unknown command\n", dat);
                break;
            }
        else
            n = sfsprintf(buf,
                          ...
                          "pid=%d uid=%d gid=%d args=`%s'\n",
                          fmttime(*dat ? dat : "%K", time(NiL)),
                          csname(css->state, 0),
                          TST_VERSION,
                          CS_HOST_LOCAL,
                          csntoa(css->state, con->id.hid),
                          getpid(),
                          con->id.pid,
                          con->id.uid,
                          con->id.gid,
...
main(int argc, char **argv)
{
    ...
    disc.flags = CSS_DAEMON | CSS_LOG | CSS_ERROR | CSS_DORMANT
                 | CSS_INTERRUPT | CSS_WAKEUP;
