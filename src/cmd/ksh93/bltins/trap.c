...
static int
sig_number(Shell_t *, const char *);

int
b_trap(int argc, char *argv[], Shbltin_t *context)
{
    char *arg = argv[1];
    int sig, clear;
    ...
    if (arg = *argv) {
        ...
        while (arg = *argv++) {
            ...
            if (sig >= shp->gd->sigmax) {
                ...
                char **trapcom
                = (shp->st.otrapcom ? shp->st.otrapcom : shp->st.trapcom);
                if (arg = trapcom[sig])
                    sfputr(sfstdout, arg, '\n');
            } else if (clear) {
                sh_sigclear(shp, sig);
...
int
b_kill(int argc, char *argv[], Shbltin_t *context)
{
    char *signame;
...
endopts:
    ...
    if (job_walk(
        shp, sfstdout, job_kill, sig | (flag & (Q_FLAG | QQ_FLAG)), argv))
        shp->exitval = 1;
    shp->sigval = 0;
    return (shp->exitval);
}
...
static int
sig_number(Shell_t *shp, const char *string)
{
    const Shtable_t *tp;
    int n, o, sig = 0;
    char *last, *name;
    if (isdigit(*string)) {
        ...
        if (n < 0 && shp->gd->sigruntime[1] && (name = stkptr(shp->stk, o))
            && *name++ == 'R' && *name++ == 'T') {
