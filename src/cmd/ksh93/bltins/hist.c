...
static void
hist_subst(Shell_t *shp, const char *, int fd, char *);
...
#endif
int
b_hist(int argc, char *argv[], Shbltin_t *context)
{
    History_t *hp;
    char *arg;
    int flag, fdo;
    ...
    if (lflag) {
        ...
        if ((fdo
             = open(fname, O_CREAT | O_RDWR, S_IRUSR | S_IWUSR | O_cloexec))
            < 0)
    ...
    while (1) {
        if (nflag == 0)
            ...
            sfputc(outfile, '\t');
        hist_list(shp->gd->hist_ptr,
                  outfile,
                  hist_tell(shp->gd->hist_ptr, range[flag]),
                  0,
                  arg);
        if (lflag)
            sh_sigcheck(shp);
...
static void
hist_subst(Shell_t *shp, const char *command, int fd, char *replace)
{
    char *newp = replace;
    char *sp;
    int c;
