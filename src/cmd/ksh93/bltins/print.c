...
static int
extend(Sfio_t *, void *, Sffmt_t *);
static const char preformat[] = "";
static char *
genformat(Shell_t *, char *);
static int
fmtvecho(Shell_t *, const char *, struct printf *);
static ssize_t
fmtbase64(Shell_t *, Sfio_t *, char *, const char *, int);
...
int
B_echo(int argc, char *argv[], Shbltin_t *context)
{
...
int
b_printf(int argc, char *argv[], Shbltin_t *context)
{
    struct print prdata;
    NOT_USED(argc);
...
int
b_print(int argc, char *argv[], Shbltin_t *context)
{
    Sfio_t *outfile;
    ...
    while ((n = optget(argv, options)))
        ...
        case 'v':
            if (argc < 0) {
                if (!(vname = nv_open(
                      opt_info.arg, shp->var_tree, NV_VARNAME | NV_NOARRAY)))
                    errormsg(
                    SH_DICT, 2, "Cannot create variable %s", opt_info.arg);
            } else
                vflag = 'v';
            break;
        case 'C':
    ...
    if (vflag && format)
        errormsg(
        SH_DICT, ERROR_usage(2), "-%c and -f are mutually exclusive", vflag);
skip:
    if (format)
        format = genformat(shp, format);
    /* handle special case of '-' operand for print */
    if (argc > 0 && *argv && strcmp(*argv, "-") == 0
        && strcmp(argv[-1], "--"))
        argv++;
    if (vname) {
        if (!shp->strbuf2)
...
skip2:
    ...
    if (!sflag && !(outfile = shp->sftable[fd])) {
        ...
        shp->sftable[fd] = outfile
        = sfnew(NIL(Sfio_t *), shp->outbuff, IOBSIZE, fd, n);
...
printv:
    if (format) {
        ...
        if (pdata.nextarg == nullarg && pdata.argsize > 0)
            sfwrite(
            outfile, stkptr(shp->stk, stktell(shp->stk)), pdata.argsize);
...
int
sh_echolist(Shell_t *shp, Sfio_t *outfile, int raw, char *argv[])
{
    char *cp;
    int n;
    struct printf pdata;
...
#if SHOPT_MULTIBYTE && defined(FMT_EXP_WIDE)
            c = chrexp(
            s - 1, &p, &w, FMT_EXP_CHAR | FMT_EXP_LINE | FMT_EXP_WIDE);
#else
            c = chresc(s - 1, &p);
#endif
            s = p;
...
static char *
genformat(Shell_t *shp, char *format)
{
    char *fp;
    stkseek(shp->stk, 0);
...
static char *
fmthtml(Shell_t *shp, const char *string, int flags)
{
...
static ssize_t
fmtbase64(Shell_t *shp, Sfio_t *iop, char *string, const char *fmt, int alt)
{
    char *cp;
    Sfdouble_t d;
    ssize_t size;
    Namval_t *np
    = nv_open(string, shp->var_tree, NV_VARNAME | NV_NOASSIGN | NV_NOADD);
    Namarr_t *ap;
...
static int
varname(const char *str, ssize_t n)
{
    int c, dot = 1, len = 1;
    if (n < 0) {
        if (*str == '.')
...
static const char *
mapformat(Sffmt_t *fe)
{
    const struct printmap *pm = Pmap;
    while (pm->size > 0) {
        if (pm->size == fe->n_str
            && memcmp(pm->name, fe->t_str, fe->n_str) == 0)
            return (pm->map);
        pm++;
    }
    return (0);
...
static int
extend(Sfio_t *sp, void *v, Sffmt_t *fe)
{
    char *lastchar = "";
    int neg = 0;
    Sfdouble_t d;
    ...
    if (fe->n_str > 0 && (format == 'T' || format == 'Q')
        && varname(fe->t_str, fe->n_str) && (!argp || varname(argp, -1))) {
        if (argp)
            pp->lastarg = argp;
        else
            argp = pp->lastarg;
        if (argp) {
            sfprintf(
            pp->sh->strbuf, "%s.%.*s%c", argp, fe->n_str, fe->t_str, 0);
    ...
    if (!argp || format == 'Z') {
        ...
        case 'n': {
            Namval_t *np;
            np = nv_open(
            argp, shp->var_tree, NV_VARNAME | NV_NOASSIGN | NV_NOARRAY);
            _nv_unset(np, 0);
        ...
        case 'T':
            value->ll
            = ( Sflong_t ) tmxdate(*pp->nextarg, &lastchar, TMX_NOW);
            break;
        default:
            value->ll = 0;
            fe->fmt = 'd';
    ...
    case 'T':
        if (fe->n_str > 0) {
            ...
            value->s = fmttmx(NIL(char *), value->ll);
        fe->fmt = 's';
        fe->size = -1;
        break;
    }
...
static int
fmtvecho(Shell_t *shp, const char *string, struct printf *pp)
{
