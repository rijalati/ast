 ...
 */
int
b_command(int argc, char *argv[], Shbltin_t *context)
{
    int n, flags = 0;
 ...
 */
int
b_whence(int argc, char *argv[], Shbltin_t *context)
{
    int flags = 0, n;
    Shell_t *shp = context->shp;
    NOT_USED(argc);
...
static int
whence(Shell_t *shp, char **argv, int flags)
{
    const char *name;
    Namval_t *np;
    const char *cp;
        ...
        do
        {
            ...
            else if (cp)
            {
                if (flags & V_FLAG)
                {
                    ...
                    if (*cp == '/'
                        && (np = nv_search(cp, shp->bltin_tree, 0)))
