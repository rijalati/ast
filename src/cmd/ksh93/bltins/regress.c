...
#    define TRACE(r, i, f)                                                   \
        sh_regress(REGRESS_##r, i, sfprints f, __LINE__, __FILE__)
...
= "[-1p0?\n@(#)$Id: __regress__ (AT&T Research) 2009-03-29 $\n]" USAGE_LICENSE
  ...
  "[+?Each enabled intercept may result in trace lines of the form "
  "\b" REGRESS_HEADER
...
static const char *regress_options[] = {
    "ERROR", "egid", "euid", "p_suid", "source", "etc",
};

void
sh_regress_init(Shell_t *shp)
{
...
uid_t
sh_regress_p_suid(unsigned int line, const char *file)
{
...
char *
sh_regress_etc(const char *path, unsigned int line, const char *file)
{
...
int
b___regress__(int argc, char **argv, Shbltin_t *context)
{
