 ...
 */
static void
invalidate(Namval_t *np, void *data)
{
 ...
 */
int
sh_diropenat(Shell_t *shp, int dir, const char *path)
{
    int fd, shfd;
    ...
    if ((fd = openat(dir, path, O_DIRECTORY | O_NONBLOCK | O_CLOEXEC)) < 0)
        ...
        if (errno != EACCES
            || (fd = openat(
                dir, path, O_SEARCH | O_DIRECTORY | O_NONBLOCK | O_CLOEXEC))
               < 0)
#endif
            return fd;

    /* Move fd to a number > 10 and the fd number with the shell */
...
int
b_cd(int argc, char *argv[], Shbltin_t *context)
{
    char *dir;
    Pathcomp_t *cdpath = 0;
    const char *dp;
...
#if SHOPT_FS_3D
            if (!(cp = pathcanon(stakptr(PATH_OFFSET),
                                 PATH_MAX,
                                 PATH_ABSOLUTE | PATH_DOTDOT
                                 | PATH_DROP_TAIL_SLASH)))
                continue;
#else
...
int
b_pwd(int argc, char *argv[], Shbltin_t *context)
{
    char *cp, *dir;
    Shell_t *shp = context->shp;
    bool pflag = false;
