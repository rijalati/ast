...
#            define isapipe(f, p)                                            \
                (test_stat(f, p) >= 0                                        \
                 && (S_ISFIFO((p)->st_mode)                                  \
                     || S_ISSOCK((p)->st_mode) && (p)->st_ino                \
                        && ((p)->st_mode & (S_IRUSR | S_IWUSR))              \
                           != (S_IRUSR | S_IWUSR)))
#        else
#            define isapipe(f, p)                                            \
                (test_stat(f, p) >= 0                                        \
...
static time_t
test_time(const char *, const char *);
static int
test_stat(const char *, struct stat *);
static int
test_mode(const char *);
...
static char *
nxtarg(struct test *, int);
static int
expr(struct test *, int);
static int
e3(struct test *);

static int
test_strmatch(Shell_t *shp, const char *str, const char *pat)
{
...
int
b_test(int argc, char *argv[], Shbltin_t *context)
{
    struct test tdata;
    char *cp = argv[0];
    int not;
...
int
test_unop(Shell_t *shp, int op, const char *arg)
{
    struct stat statb;
    int f;
    switch (op) {
    ...
    case 'N':
        return (test_stat(arg, &statb) >= 0
                && tmxgetmtime(&statb) > tmxgetatime(&statb));
    case 'p':
    ...
    case 'R': {
        ...
        if (!(np = nv_open(arg,
                           shp->var_tree,
                           NV_VARNAME | NV_NOFAIL | NV_NOADD | NV_NOREF)))
            return (0);
        isref = nv_isref(np);
        if (op == 'R')
            return (isref);
...
static time_t
test_time(const char *file1, const char *file2)
{
    Time_t t1, t2;
...
int
sh_access(const char *name, int mode)
{
