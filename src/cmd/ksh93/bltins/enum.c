...
= "[-?@(#)$Id: enum (AT&T Research) 2013-04-29 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?\benum\b is a declaration command that creates an "
  "enumeration "
  ...
  "[+?When an enumeration variable is used in arithmetic expression, its "
  "value "
  ...
  "[+?The enum \b_Bool\b exists by default with values \btrue\b and "
  "\bfalse\b. "
  "The predefined alias \bbool\b is defined as \b_Bool\b.]"
  "[i:ignorecase?The values are case insensitive.]"
  "[p?Writes the enums to standard output.  If \atypename\a is omitted then "
  "all "
...
static int
enuminfo(Opt_t *op, Sfio_t *out, const char *str, Optdisc_t *fp)
{
    Namval_t *np;
    struct Enum *ep;
    int n = 0;
...
static Sfdouble_t
get_nenum(Namval_t *np, Namfun_t *fp)
{
...
create_enum(Namval_t *np, const char *name, int flags, Namfun_t *fp)
{
    ...
    error(
    ERROR_exit(1), "%s:  invalid enum constant for %s", name, nv_name(np));
    return (mp);
}

const Namdisc_t ENUM_disc
= { 0, put_enum, get_enum, get_nenum, 0, create_enum, clone_enum };

static int
sh_outenum(Shell_t *shp, Sfio_t *iop, Namval_t *tp)
{
    Namval_t *mp;
    Dt_t *dp = 0;
...
#ifdef STANDALONE
static int
enum_create(int argc, char **argv, Shbltin_t *context)
#else
int
b_enum(int argc, char **argv, Shbltin_t *context)
#endif
{
