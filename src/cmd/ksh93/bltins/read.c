...
#pragma prototyped
/*
 * read [-ACprs] [-q format] [-d delim] [-u filenum] [-t timeout] [-n n] [-N
 * n] [name...]
 *
 *   David Korn
 *   AT&T Labs
...
static int
json2sh(Shell_t *shp, Sfio_t *in, Sfio_t *out)
{
...
static struct Method methods[] = { "json", json2sh, "ksh", 0, 0, 0 };

int
b_read(int argc, char *argv[], Shbltin_t *context)
{
    Sfdouble_t sec;
    char *name = 0;
    int r, flags = 0, fd = 0;
    ...
    if (method) {
        ...
        if (!methods[mindex].name)
            errormsg(
            SH_DICT, ERROR_system(1), "%s method not supported", method);
    }
            ...
            long timeout)
{
    ...
    if (size
        || (flags >> D_FLAG)) /* delimiter not new-line or fixed size read */
    {
    ...
    while (1) {
        ...
        case 0:
            ...
            while (1) {
                ...
                if (inquote) {
                    ...
                    if (c && c != S_EOF) {
                        if (c == S_NL) {
                            ...
                            if (cp
                                = ( unsigned char * ) sfgetr(iop, delim, 0))
                                c = sfvalue(iop);
                            else if (cp = ( unsigned char * ) sfgetr(
                                     iop, delim, -1))
                                c = sfvalue(iop) + 1;
                            val = ( char * ) cp;
                        }
                ...
                if (!del && c == S_DELIM)
                    del = cp - 1;
                if (name || c == S_NL || c == S_ESC || c == S_EOF
                    || c == S_MBYTE)
                    break;
                if (wrd < 0)
                    wrd = 0;
