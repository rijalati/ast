...
= "[-?\n@(#)$Id: mkservice (AT&T Research) 2001-06-13 $\n]" USAGE_LICENSE
  ...
  "[+?The \aservice_path\a must be of the form "
  "\b/dev/tcp/localhost/\b\aportno\a "
...
static const char *disctab[] = { "accept", "action", "close", 0 };
...
static int
waitnotify(int fd, long timeout, int rw)
{
...
static int
Accept(Service_t *sp, int accept_fd)
{
    Shell_t *shp = sp->sh;
...
static int
Action(Service_t *sp, int fd, int close)
{
    Namval_t *nq;
    int r = 0;
...
static int
Error(Service_t *sp, int level, const char *arg, ...)
{
    va_list ap;
...
= { sizeof(Service_t), putval, 0, 0, setdisc };

int
b_mkservice(int argc, char **argv, Shbltin_t *context)
{
    char *var;
    char *path;
    Namval_t *np;
...
int
b_eloop(int argc, char **argv, Shbltin_t *context)
{
    Shell_t *shp = context->shp;
    long timeout = -1;
    NOT_USED(argc);
