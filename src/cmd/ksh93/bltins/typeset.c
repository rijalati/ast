...
static int
print_namval(Sfio_t *, Namval_t *, int, struct tdata *);
static void
print_attribute(Namval_t *, void *);
static void
print_all(Sfio_t *, Dt_t *, struct tdata *);
static void
print_scan(Sfio_t *, int, Dt_t *, int, struct tdata *);
static int
unall(int, char **, Dt_t *, Shell_t *);
static int
setall(char **, int, Dt_t *, struct tdata *);
static void
pushname(Namval_t *, void *);
static void (*nullscan)(Namval_t *, void *);

static Namval_t *
load_class(const char *name)
{
...
#endif
int
b_readonly(int argc, char *argv[], Shbltin_t *context)
{
    int flag;
    char *command = argv[0];
    struct tdata tdata;
    ...
    tdata.aflag = '-';
    while (
    (flag = optget(argv, *command == 'e' ? sh_optexport : sh_optreadonly)))
        switch (flag) {
        case 'p':
...
int
b_alias(int argc, char *argv[], Shbltin_t *context)
{
        ...
        if (flag & NV_TAGGED) {
            ...
            if (argv[1] && argv[1][0] == '-') {
                if (argv[1][1] == 'r' && argv[1][2] == 0) {
                    Namval_t *np = nv_search(
                    ( char * ) PATHNOD, tdata.sh->var_tree, HASH_BUCKET);
...
#endif
int
b_typeset(int argc, char *argv[], Shbltin_t *context)
{
    ...
    while ((n = optget(argv, optstring))) {
        ...
        case 'R':
            ...
            break;
        case 'M':
            if ((tdata.wctname = opt_info.arg)
                && !nv_mapchar(( Namval_t * ) 0, tdata.wctname))
...
endargs:
    ...
    if (local && context->shp->var_base == context->shp->var_tree)
        errormsg(
        SH_DICT, ERROR_exit(1), "local can only be used in a function");
#endif
    opt_info.disc = 0;
    ...
    if (sizeof(char *) < 8 && tdata.argnum > SHRT_MAX)
        errormsg(SH_DICT,
                 ERROR_exit(2),
                 "option argument cannot be greater than %d",
                 SHRT_MAX);
    if (isfloat)
        flag |= NV_DOUBLE;
    if (isshort) {
    ...
    if (flag & NV_TYPE) {
        ...
        tdata.tp = nv_open(stkptr(stkp, offset),
                           tdata.sh->var_tree,
        ...
        if (!tdata.tp)
            errormsg(
            SH_DICT, ERROR_exit(1), "%s: unknown type", tdata.prefix);
...
static void
print_value(Sfio_t *iop, Namval_t *np, struct tdata *tp)
{
    char *name;
    int aflag = tp->aflag;
    Namval_t *table;
    if (nv_isnull(np)) {
        ...
        print_scan(
        iop, NV_FUNCTION | NV_NOSCOPE, tp->sh->fun_tree, aflag == '+', tp);
        tp->wctname = 0;
        tp->sh->namespace = nsp;
        if (--tp->indent)
            sfnputc(iop, '\t', tp->indent);
...
static int
setall(char **argv, int flag, Dt_t *troot, struct tdata *tp)
{
    char *name;
    char *last = 0;
    int nvflags = (flag
                   & (NV_ARRAY | NV_NOARRAY | NV_VARNAME | NV_IDENT
                      | NV_ASSIGN | NV_STATIC | NV_MOVE));
    int r = 0, ref = 0, comvar = (flag & NV_COMVAR),
        iarray = (flag & NV_IARRAY);
    size_t len;
    Shell_t *shp = tp->sh;
    if (!shp->prefix) {
        if (!tp->pflag)
    ...
    if (argv[1]) {
        ...
        while (name = *++argv) {
            ...
            if (troot == shp->fun_tree) {
                ...
                if (flag & NV_LTOU) {
                    ...
                    if (shp->prefix) {
                        sfprintf(
                        shp->strbuf, "%s.%s%c", shp->prefix, name, 0);
                    ...
                    if (!np)
                        ...
                        if (np = nv_search(name, troot, 0)) {
                            if (!is_afunction(np))
                                np = 0;
                        } else if (memcmp(name, ".sh.math.", 9) == 0
                                   && sh_mathstd(name + 9))
                            continue;
                }
                if (np
                    && ((flag & NV_LTOU) || !nv_isnull(np)
                        || nv_isattr(np, NV_LTOU))) {
                    if (flag == 0 && !tp->help) {
            ...
            if (troot == shp->track_tree && tp->aflag == '-') {
                ...
                path_alias(
                np, path_absolute(shp, nv_name(np), NIL(Pathcomp_t *)));
                continue;
            }
            if (shp->nodelist && (len = strlen(name)) && name[len - 1] == '@')
                ...
                | ((iarray | (nvflags & (NV_REF | NV_NOADD) == NV_REF))
                   ? NV_FARRAY
                   : 0));
            if (!np)
                continue;
            if (nv_isnull(np) && !nv_isarray(np) && nv_isattr(np, NV_NOFREE))
            ...
            else if ((ap = nv_arrayptr(np)) && nv_aindex(np) > 0
                     && ap->nelem == 1 && nv_getval(np) == Empty) {
                ap->nelem++;
                _nv_unset(np, 0);
                ap->nelem--;
                ...
                errormsg(SH_DICT,
                         ERROR_exit(1),
                         "cannot change associative array %s to index array",
                         nv_name(np));
            else if ((iarray || (flag & NV_ARRAY)) && nv_isvtree(np)
                     && !nv_type(np))
...
#else
            if (flag == NV_ASSIGN && !ref && tp->aflag != '-'
                && !strchr(name, '='))
#endif
            {
                    ...
                    || (shp->last_root == shp->var_tree
                        && ((tp->tp && tp->tp != nv_type(np))
                            ...
                            || (!(flag & (NV_EXPORT | NV_RDONLY))
                                && nv_isattr(np, (NV_EXPORT | NV_IMPORT))
                                   == (NV_EXPORT | NV_IMPORT))))) {
                    {
            ...
            if (!(flag & NV_INTEGER) && (flag & (NV_LTOU | NV_UTOL))) {
                ...
                if (!tp->wctname)
                    errormsg(
                    SH_DICT, ERROR_exit(1), e_mapchararg, nv_name(np));
        ...
        if (tp->aflag) {
            ...
            if ((flag & (NV_UTOL | NV_LTOU)) == (NV_UTOL | NV_LTOU)) {
                print_scan(
                sfstdout, flag & ~NV_UTOL, troot, tp->aflag == '+', tp);
                flag &= ~NV_LTOU;
            }
            ...
            if (tp->noref) {
                tp->noref = 0;
                print_scan(
                sfstdout, flag | NV_REF, troot, tp->aflag == '+', tp);
            }
...
int
sh_addlib(Shell_t *shp, void *dll, char *name, Pathcomp_t *pp)
{
    int n;
    int r;
    Libinit_f initfn;
...
Shbltin_f
sh_getlib(Shell_t *shp, char *sym, Pathcomp_t *pp)
{
    int n;
...
int
sh_addlib(Shell_t *shp, void *library, char *name, Pathcomp_t *pp)
{
    return 0;
}

Shbltin_f
sh_getlib(Shell_t *shp, char *name, Pathcomp_t *pp)
{
    return 0;
}
 ...
 */
int
b_builtin(int argc, char *argv[], Shbltin_t *context)
{
    char *arg = 0, *name;
    int n, r = 0, flag = 0;
    Namval_t *np;
    ...
    if (arg || *argv) {
        if (sh_isoption(tdata.sh, SH_RESTRICTED))
            errormsg(
            SH_DICT, ERROR_exit(1), e_restricted, argv[-opt_info.index]);
...
#        if (_AST_VERSION >= 20040404)
        if (!(library
              = dllplug(SH_ID, arg, NIL(char *), RTLD_LAZY, NIL(char *), 0)))
#        else
...
#    endif
    ...
    while (arg = *argv) {
        ...
        if (dlete || liblist)
            for (n = (nlib ? nlib : dlete); --n >= 0;) {
                ...
                if (!dlete && !liblist[n].dll)
                    continue;
                if (dlete
                    || (addr = ( Shbltin_f ) dlllook(liblist[n].dll,
                                                     stkptr(stkp, flag))))
#else
                if (dlete)
#endif /* SHOPT_DYNAMIC */
                {
                    if (np = sh_addbuiltin(
                        tdata.sh, arg, addr, pointerof(dlete))) {
        ...
        if (!dlete && !addr
            && !(np = sh_addbuiltin(tdata.sh, arg, ( Shbltin_f ) 0, 0)))
            errmsg = "not found";
        if (errmsg) {
...
int
b_set(int argc, char *argv[], Shbltin_t *context)
{
...
int
b_unalias(int argc, char *argv[], Shbltin_t *context)
{
    Shell_t *shp = context->shp;
...
int
b_unset(int argc, char *argv[], Shbltin_t *context)
{
    Shell_t *shp = context->shp;
...
static int
unall(int argc, char **argv, Dt_t *troot, Shell_t *shp)
{
    Namval_t *np;
    const char *name;
    volatile int r;
        ...
        if (np) {
            ...
            if (troot == shp->var_tree) {
                ...
                if ((ap = nv_arrayptr(np)) && !ap->fixed
                    && name[strlen(name) - 1] == ']' && !nv_getsub(np))
#else
                if (nv_isarray(np) && name[strlen(name) - 1] == ']'
                    && !nv_getsub(np))
...
static int
print_namval(Sfio_t *file, Namval_t *np, int flag, struct tdata *tp)
{
    char *cp;
    ...
    if (isfun) {
        ...
        if (flag) {
            ...
            if (iop
                && sfseek(iop, ( Sfoff_t ) np->nvalue.rp->hoffset, SEEK_SET)
                   >= 0)
 ...
 */
static void
print_all(Sfio_t *file, Dt_t *root, struct tdata *tp)
{
    tp->outfile = file;
 ...
 */
static void
print_attribute(Namval_t *np, void *data)
{
...
static void
print_scan(Sfio_t *file, int flag, Dt_t *root, int option, struct tdata *tp)
{
    char **argv;
    Namval_t *np;
    int namec;
    ...
    namec
    = nv_scan(root, nullscan, ( void * ) tp, tp->scanmask, flag & ~NV_IARRAY);
    argv = tp->argnam
    = ( char ** ) stkalloc(tp->sh->stk, (namec + 1) * sizeof(char *));
    namec
    = nv_scan(root, pushname, ( void * ) tp, tp->scanmask, flag & ~NV_IARRAY);
    if (mbcoll())
        strsort(argv, namec, strcoll);
    if (namec == 0 && tp->sh->namespace
        && nv_dict(tp->sh->namespace) == root) {
        sfnputc(file, '\t', tp->indent);
        sfwrite(file, ":\n", 2);
    } else
        while (namec--) {
            if ((np = nv_search(*argv++, root, 0)) && np != onp
                && (!nv_isnull(np) || np->nvfun
                    || nv_isattr(np, ~NV_NOFREE))) {
                onp = np;
                if (name) {
                    char *newname = nv_name(np);
                    if (memcmp(name, newname, len) == 0
                        && newname[len] == '.')
                        continue;
                    name = 0;
                }
...
static void
pushname(Namval_t *np, void *data)
{
