...
#if KSHELL
static int
keytrap(Edit_t *, char *, int, int, int);
#else
Edit_t editb;
#endif /* KSHELL */
...
void
ed_setup(Edit_t *ep, int fd, int reedit)
{
    Shell_t *shp = ep->sh;
    char *pp;
    char *last, *prev;
...
static void
ed_putstring(Edit_t *ep, const char *str)
{
    int c;
    while (c = *str++)
        ed_putchar(ep, c);
...
static void
ed_nputchar(Edit_t *ep, int n, int c)
{
    while (n-- > 0)
        ed_putchar(ep, c);
}
 ...
 */
static int
putstack(Edit_t *ep, char string[], int nbyte, int type)
{
    int c;
#if SHOPT_MULTIBYTE
    char *endp, *p = string;
...
/*
 * returns the line and column corresponding to offset <off> in the physical
 * buffer if <cur> is non-zero and <= <off>, then correspodning <curpos> will
 * start the search
 */
Edpos_t
ed_curpos(Edit_t *ep, genchar *phys, int off, int cur, Edpos_t curpos)
{
    genchar *sp = phys;
...
int
ed_setcursor(Edit_t *ep, genchar *physical, int old, int new, int first)
{
    static int oldline;
    int delta;
    int clear = 0;
    ...
    if (ep->e_multiline) {
        ...
        if (ep->e_curpos.col == 0 && ep->e_curpos.line > 0
...
int
ed_external(const genchar *src, char *dest)
{
    genchar wc;
    int c, size;
    char *dp = dest;
 ...
 */
static int
keytrap(Edit_t *ep, char *inbuff, int insize, int bufsize, int mode)
{
    char *cp;
    int savexit;
    Shell_t *shp = ep->sh;
...
static int
ed_histlencopy(const char *cp, char *dp)
{
    int c, n = 1, col = 1;
    const char *oldcp = cp;
...
int
ed_histgen(Edit_t *ep, const char *pattern)
{
    ...
    if (!(hp = ep->sh->gd->hist_ptr)
        && (!nv_getval(HISTFILE) || !sh_histinit(ep->sh)))
        return (0);
    if (ep->e_cur <= 2)
        maxmatch = 0;
