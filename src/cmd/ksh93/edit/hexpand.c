...
static int mod_flags[]
= { 0, 0, 0, 0, HIST_PRINT, HIST_QUOTE, HIST_QUOTE | HIST_QUOTE_BR, 0, 0 };
...
static char *
parse_subst(Shell_t *shp, const char *s, struct subst *sb)
{
    char *cp, del;
    int off, n = 0;
...
hist_expand(Shell_t *shp, const char *ln, char **xp)
{
    ...
    *tmp2 = 0;       /* temporary line buffer */
    ...
    while (cp && *cp) {
        ...
        if (hc[2] && *cp == hc[2]) /* history comment designator, skip rest of
                                      line */
        {
            sfputc(shp->stk, *cp++);
            sfputr(shp->stk, cp, -1);
