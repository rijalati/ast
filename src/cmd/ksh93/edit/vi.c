...
mvcursor(Vi_t *, int);
static void
pr_string(Vi_t *, const char *);
static void
putstring(Vi_t *, int, int);
static void
refresh(Vi_t *, int);
static void
replace(Vi_t *, int, int);
static void
restore_v(Vi_t *);
static void
save_last(Vi_t *);
...
#else
    int viraw
    = (sh_isoption(ed->sh, SH_VIRAW) || ed->sh->st.trap[SH_KEYTRAP]);
#    ifndef FIORDCHK
...
#    endif
    ...
    if (!viraw) {
        ...
        if (last_virt > 0)
            last_phys
            = ed_virt_to_phys(vp->ed, virtual, physical, last_virt, 0, 0);
        if (last_phys >= w_size) {
        ...
        if (crallowed) {
            ...
            pr_string(vp, Prompt);
            if (term_char == ESC
                && (last_virt < 0 || virtual[last_virt] != ESC))
                refresh(vp, CONTROL);
            else
                refresh(vp, INPUT);
        } else {
...
static void
backword(Vi_t *vp, int nwords, int cmd)
{
        ...
        case '\t': /** bring choice to edit **/
            if (vp->ed->hlist) {
                if (vp->repeat > vp->ed->nhlist - vp->ed->hoff)
                    goto ringbell;
                curhline
                = vp->ed->hlist[vp->repeat + vp->ed->hoff - 1]->index;
                goto newhist;
            }
            goto ringbell;
#else
...
static void
cdelete(Vi_t *vp, int nchars, int mode)
{
    int i;
    genchar *cp;
...
static int
delmotion(Vi_t *vp, int motion, int mode)
{
    int begin, end, delta;
...
static void
endword(Vi_t *vp, int nwords, int cmd)
{
    int tcur_virt = cur_virt;
    while (nwords--) {
...
static void
forward(Vi_t *vp, int nwords, int cmd)
{
    int tcur_virt = cur_virt;
    while (nwords--) {
        if (cmd == 'W') {
    ...
    for (;;) {
        ...
        case '\b': /** backspace **/
            if (cur_virt >= 0 && virtual[cur_virt] == '\\') {
                ...
                if (mode == REPLACE
                    || (last_save > 0 && last_virt <= last_save)) {
                    if (cur_virt <= first_virt)
                        ed_ringbell();
        ...
        case UWERASE: /** delete back word **/
            if (cur_virt > first_virt && !isblank(cur_virt)
                && !ispunct(virtual[cur_virt]) && isblank(cur_virt - 1)) {
                cdelete(vp, 1, BAD);
            } else {
                tmp = cur_virt;
                backword(vp, 1, 'W');
    ...
    case '[':
        ...
        case 'A':
            ...
            if (!vp->ed->hlist && cur_virt >= 0 && cur_virt < (SEARCHSIZE - 2)
                && cur_virt == last_virt)
#else
            if (cur_virt >= 0 && cur_virt < (SEARCHSIZE - 2)
                && cur_virt == last_virt)
#endif /* SHOPT_EDPREDICT */
            {
                virtual[last_virt + 1] = '\0';
...
static void
pr_string(Vi_t *vp, const char *sp)
{
    /*** copy string sp ***/
...
static void
putstring(Vi_t *vp, int col, int nchars)
{
    while (nchars--)
...
#define w v
    ...
    v = cur_virt;
#if SHOPT_EDPREDICT
    if (mode == INPUT && v > 0 && virtual[0] == '#' && v == last_virt
        && virtual[v] != '*' && sh_isoption(vp->ed->sh, SH_VI)) {
        int n;
        virtual[last_virt + 1] = 0;
#    if SHOPT_MULTIBYTE
...
#    endif /* SHOPT_MULTIBYTE */
    ...
    if (first_w == vp->ofirst_wind && ncur_phys >= vp->ocur_phys
        && opflag == 1) {
        p = vp->ocur_phys;
        w = p - first_w;
    } else {
...
static void
replace(Vi_t *vp, int c, int increment)
{
    int cur_window;
    ...
    if (vp->ocur_virt == INVALID || !is_print(c)
        || !is_print(virtual[cur_virt]) || !is_print(vp->o_v_char)
#if SHOPT_MULTIBYTE
 ...
 */
static int
curline_search(Vi_t *vp, const char *string)
{
...
sync_cursor(Vi_t *vp)
{
    ...
    new_phys = 0;
    if (vp->first_wind == vp->ofirst_wind && cur_virt > vp->ocur_virt
        && vp->ocur_virt != INVALID) {
...
static int
textmod(Vi_t *vp, int c, int mode)
{
    int i;
