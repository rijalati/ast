...
static char *
fmtx(Shell_t *shp, const char *string)
{
...
static int
charcmp(int a, int b, int nocase)
{
    if (nocase)
    {
        if (isupper(a))
 ...
 */
static char *
find_begin(char outbuff[], char *last, int endchar, int *type)
{
...
#if SHOPT_COMPLETE
static char **
prog_complete(Dt_t *dict, char *line, char *word, int cur)
{
...
int
ed_expand(Edit_t *ep, char outbuff[], int *cur, int *eol, int mode, int count)
{
        ...
        if (mode == '\\')
        {
            ...
            if (mode && out[-1] != '/')
            {
                if (cmd_completion)
                {
                    ...
                    Pathcomp_t *pp;
                    if (*cp == '/'
                        && (pp = path_dirfind(shp->pathlist, cp, '/'))
...
ed_macro(Edit_t *ep, int i)
{
    ...
    if (i == '_')
        ...
        ep->e_macro[2] = 0;
    if (isalnum(i)
        && (np = nv_search(ep->e_macro, ep->sh->alias_tree, HASH_SCOPE))
        && (out = nv_getval(np)))
    {
#if SHOPT_MULTIBYTE
