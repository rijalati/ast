...
int _Hist = 0;
static void
hist_marker(char *, long);
static History_t *
hist_trim(History_t *, int);
static int
hist_nearend(History_t *, Sfio_t *, off_t);
static int
hist_check(int);
static int
hist_clean(int);
#ifdef SF_BUFCONST
static ssize_t
hist_write(Sfio_t *, const void *, size_t, Sfdisc_t *);
static int
hist_exceptf(Sfio_t *, int, void *, Sfdisc_t *);
#else
static int
hist_write(Sfio_t *, const void *, int, Sfdisc_t *);
static int
hist_exceptf(Sfio_t *, int, Sfdisc_t *);
#endif
...
static int
acctinit(History_t *hp)
{
    char *cp, *acctfile;
    Namval_t *np
    = nv_search("ACCTFILE", (( Shell_t * )hp->histshell)->var_tree, 0);
...
#if SHOPT_AUDIT
static int
sh_checkaudit(History_t *hp, const char *name, char *logbuf, size_t len)
{
    char *cp, *last;
...
static const Sfdisc_t hist_disc
= { NULL, hist_write, NULL, hist_exceptf, NULL };

static void
hist_touch(void *handle)
{
...
retry:
    ...
    if ((fd = open(
         cp, O_BINARY | O_APPEND | O_RDWR | O_CREAT | O_cloexec, histmode))
        >= 0)
    {
    ...
    if (fd < 0)
    {
...
#endif /* KSHELL */
        {
            if (!(fname = pathtmp(NIL(char *), 0, 0, NIL(int *))))
                return (0);
            fd = open(fname,
                      O_BINARY | O_APPEND | O_CREAT | O_RDWR,
    ...
    hp->histfp = sfnew(NIL(Sfio_t *),
                       hp->histbuff,
                       HIST_BSIZE,
                       fd,
                       SF_READ | SF_WRITE | SF_APPENDWR | SF_SHARE);
...
#endif /* KSHELL */
    sh_timeradd(
    1000L * (HIST_RECENT - 30), 1, hist_touch, ( void * )hp->histname);
#if SHOPT_ACCTFILE
...
#if SHOPT_AUDIT
    {
        ...
        if (sh_isstate(shp, SH_INTERACTIVE)
            && (hp->auditmask
                = sh_checkaudit(hp, SHOPT_AUDITFILE, buff, sizeof(buff))))
        {
            if ((fd = sh_open(buff,
                              O_BINARY | O_WRONLY | O_APPEND | O_CREAT
                              | O_cloexec,
...
hist_clean(int fd)
{
    ...
    return (fstat(fd, &statb) >= 0
            && (time(( time_t * )0) - statb.st_mtime) >= HIST_RECENT);
}
    ...
    while (1)
    {
        if (!incmd)
        {
            ...
            if (hist_new->histcnt > hist_new->histmarker + HIST_BSIZE / 2)
            {
                ...
                hist_new->histmarker
                = hist_new->histcmds[hist_ind(hist_new, c)]
                = hist_new->histcnt;
            }
            oldp = newp;
            newp = hist_seek(hist_old, ++n);
 ...
 */
static int
hist_nearend(History_t *hp, Sfio_t *iop, off_t size)
{
    ...
    while (cp = buff
           = ( unsigned char * )sfreserve(iop, SF_UNBOUND, SF_LOCKR))
    {
        n = sfvalue(iop);
        *(endbuff = cp + n) = 0;
        while (1)
...
#ifdef SF_BUFCONST
static ssize_t
hist_write(Sfio_t *iop, const void *buff, size_t insize, Sfdisc_t *handle)
#else
static int
hist_write(Sfio_t *iop, const void *buff, int insize, Sfdisc_t *handle)
#endif
{
    ...
    *bufptr++ = 0;
    ...
    if (hp->auditfp)
    {
        ...
        sfprintf(hp->auditfp,
                 "%u;%u;%s;%*s%c",
                 sh_isoption(shp, SH_PRIVILEGED) ? shgd->euserid
                                                 : shgd->userid,
                 t,
                 hp->tty,
                 size,
    ...
    c = hist_ind(hp, ++hp->histind);
    hp->histcmds[c] = hp->histcnt;
    if (hp->histflush > HIST_MARKSZ
        && hp->histcnt > hp->histmarker + HIST_BSIZE / 2)
    {
...
static void
hist_marker(char *buff, long cmdno)
{
    *buff++ = HIST_CMDNO;
    *buff++ = 0;
    *buff++ = (cmdno >> 16);
...
void
hist_list(History_t *hp, Sfio_t *outfile, off_t offset, int last, char *nl)
{
    int oldc = 0;
    int c;
    if (offset < 0 || !hp)
...
Histloc_t
hist_find(History_t *hp, char *string, int index1, int flag, int direction)
{
    int index2;
    off_t offset;
    int *coffset = 0;
    ...
    while (index1 != index2)
    {
        ...
        if ((location.hist_line = hist_match(hp, offset, string, coffset))
            >= 0)
        {
...
int
hist_match(History_t *hp, off_t offset, char *string, int *coffset)
{
 ...
 * line=linenumber  for emacs copy and only this line of command will be
 * copied line < 0 for full command copy -1 returned if there is no history
 * file
 */

int
hist_copy(char *s1, int size, int command, int line)
{
    int c;
...
char *
hist_word(char *string, int size, int word)
{
    int c;
    char *s1 = string;
...
Histloc_t
hist_locate(History_t *hp, int command, int line, int lines)
{
    Histloc_t next;
    line += lines;
    if (!hp)
...
#ifdef SF_BUFCONST
static int
hist_exceptf(Sfio_t *fp, int type, void *data, Sfdisc_t *handle)
#else
static int
hist_exceptf(Sfio_t *fp, int type, Sfdisc_t *handle)
#endif
{
    int newfd, oldfd;
    History_t *hp = ( History_t * )handle;
    if (type == SF_WRITE)
    {
        ...
        if ((newfd = open(hp->histname,
                          O_BINARY | O_APPEND | O_CREAT | O_RDWR | O_cloexec,
                          S_IRUSR | S_IWUSR))
            >= 0)
        {
