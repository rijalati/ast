...
= { "bbashdefault", "ddefault", "Idirnames", "ffilenames", "nnospace",
    "pplusdirs",    0 };

static const char Actions[] = "aABkbcfdDEeFgHhjrsoOZSuv";

static const char *Action_names[]
= { "aalias",   "Aarrayvar", "Bbinding",   "kkeyword",   "bbuiltin",
    "ccommand", "ffile",     "ddirectory", "Idisabled",  "Jenabled",
    "eexport",  "Ffunction", "ggroup",     "Hhelptopic", "hhostname",
    "jjob",     "rrunning",  "sservice",   "osetopt",    "Oshopt",
    "Zsignal",  "Sstopped",  "uuser",      "vvariable",  0 };
...
= { "yalias | sed -e 's/=.*//'",
    ...
    "k",
    "x'builtin'",
    "x(IFS=:;for i in $PATH;do cd \"$i\";for f in *;do [[ -x $f && ! -d $f "
    "]] && print -r -- \"$f\";done;cd ~-;done)",
...
static char
action(const char *list[], const char *str)
{
    const char *cp;
    int n = 0;
...
static int
compgen_info(Opt_t *op, Sfio_t *out, const char *str, Optdisc_t *od)
{
...
static int
complete_info(Opt_t *op, Sfio_t *out, const char *str, Optdisc_t *od)
{
    if (strcmp(str, "description") == 0)
...
char **
ed_pcomplete(struct Complete *comp,
             const char *line,
             const char *prefix,
             int index)
{
    ...
    if (comp->command || comp->fun)
    {
        ...
        if (comp->command)
        {
            if (!complete)
                errormsg(SH_DICT,
                         ERROR_warn(0),
                         "-C option may not work as you expect");
            stkseek(shp->stk, 0);
            sfsync(tmp);
            *cpsave = 0;
            sfprintf(shp->stk,
...
static const char *
lquote(struct Complete *cp, const char *str)
{
    int c;
    char *sp;
    Sfio_t *stakp;
...
static void
print_out(struct Complete *cp, Sfio_t *out)
{
    int c, i = 0, a;
    char *sp;
    sfputr(out, "complete", ' ');
...
int
b_complete(int argc, char *argv[], Shbltin_t *context)
{
    int n, r = 0;
