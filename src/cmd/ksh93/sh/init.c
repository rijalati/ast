...
nv_init(Shell_t *);
static Dt_t *
inittree(Shell_t *, const struct shtable2 *);
static int shlvl;
...
static void
put_ed(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    ...
    if (*name == 'E' && nv_getval(sh_scoped(shp, VISINOD)))
        goto done;
    if (!(cp = val)
        && (*name == 'E' || !(cp = nv_getval(sh_scoped(shp, EDITNOD)))))
        goto done;
...
static void
put_history(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
...
/* Trap for OPTINDEX */
static void
put_optindex(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
...
static Sfdouble_t
nget_optindex(Namval_t *np, Namfun_t *fp)
{
    return (( Sfdouble_t )*np->nvalue.lp);
}

static Namfun_t *
clone_optindex(Namval_t *np, Namval_t *mp, int flags, Namfun_t *fp)
{
...
static void
put_restricted(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
    ...
    if (np == PATHNOD
        || (path_scoped = (strcmp(name, PATHNOD->nvname) == 0))) {
    ...
    if (val && !(flags & NV_RDONLY) && np->nvalue.cp
        ...
        return;
    if (np == FPATHNOD
        || (fpath_scoped = (strcmp(name, FPATHNOD->nvname) == 0)))
...
static void
put_cdpath(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    Pathcomp_t *pp;
    Shell_t *shp = sh_ptr(np);
    nv_putv(np, val, flags, fp);
    ...
    val = np->nvalue.cp;
    pp = ( void * )path_addpath(
    shp, ( Pathcomp_t * )shp->cdpathlist, val, PATH_CDPATH);
...
static char *
msg_translate(const char *catalog, const char *message)
{
    NOT_USED(catalog);
    return (( char * )message);
}
#    else
static char *
msg_translate(const char *message, int type)
{
    NOT_USED(type);
    return (( char * )message);
}
...
static void
put_lang(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
    int type;
    char *name = nv_name(np);
...
#        define LC_LANG LC_ALL
             ...
             && (!(name = nv_getval(LCALLNOD)) || !*name))
        type = LC_LANG;
#    endif
    else
        type = -1;
    if (!sh_isstate(shp, SH_INIT)
        && (type >= 0 || type == LC_ALL || type == LC_LANG)) {
        char *r;
#    ifdef AST_LC_setenv
...
#    endif
    ...
    if (CC_NATIVE != CC_ASCII
        && (type == LC_ALL || type == LC_LANG || type == LC_CTYPE)) {
        ...
        if (ast.locale.set & (1 << AST_LC_CTYPE)) {
            int c;
            char *state[4];
            sh_lexstates[ST_BEGIN] = state[0]
            = ( char * )malloc(4 * (1 << CHAR_BIT));
...
static void
put_ifs(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
 ...
 */
static char *
get_ifs(Namval_t *np, Namfun_t *fp)
{
    struct ifs *ifsp = ( struct ifs * )fp;
    char *cp, *value;
    int c, n;
...
#ifdef timeofday
#    define dtime(tp)                                                        \
        (( double )((tp)->tv_sec) + 1e-6 * (( double )((tp)->tv_usec)))
#    define tms timeval
#else
...
static void
put_seconds(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    double d;
    struct tms tp;
    if (!val) {
...
static char *
get_seconds(Namval_t *np, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
...
static Sfdouble_t
nget_seconds(Namval_t *np, Namfun_t *fp)
{
    struct tms tp;
 ...
 */
static void
put_rand(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
 ...
 */
static Sfdouble_t
nget_rand(Namval_t *np, Namfun_t *fp)
{
...
static char *
get_rand(Namval_t *np, Namfun_t *fp)
{
 ...
 */
static Sfdouble_t
nget_lineno(Namval_t *np, Namfun_t *fp)
{
    double d = 1;
...
static void
put_lineno(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    long n;
    Shell_t *shp = sh_ptr(np);
    if (!val) {
...
static char *
get_lineno(Namval_t *np, Namfun_t *fp)
{
...
static char *
get_lastarg(Namval_t *np, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
    char *cp;
    int pid;
...
static void
put_lastarg(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
    if (flags & NV_INTEGER) {
...
static void
astbin_update(Shell_t *shp, const char *from, const char *to)
{
    Namval_t *mp, *np;
    const char *path;
...
static void
put_astbin(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    if (!val || *val == 0)
        val = ( char * )e_astbin;
...
static void
put_options(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    ...
    while (c = *cp++) {
        if (c == '=') {
            sfputc(shp->stk, 0);
            mp = nv_open(sp = stkptr(shp->stk, offset),
                         shp->var_base,
                         NV_VARNAME | NV_NOADD);
            if (mp)
                nv_putval(mp, cp, 0);
...
static char *
get_options(Namval_t *np, Namfun_t *fp)
{
    Shell_t *shp = np->nvshell;
    int offset = stktell(shp->stk);
    char const *cp;
...
static void
match2d(Shell_t *shp, struct match *mp)
{
    Namval_t *np;
    int i;
    Namarr_t *ap;
            ...
            int index)
{
    ...
    if (index == 0) {
        ...
        mp->nodes
        = ( char * )calloc(mp->nmatch * (NV_MINSZ + sizeof(void *) + 3), 1);
    ...
    shp->subshell = savesub;
    if (mp->nmatch) {
        for (n = mp->first + (mp->v - v), vsize = 0, i = 0; i < 2 * nmatch;
             i++) {
...
static char *
get_match(Namval_t *np, Namfun_t *fp)
{
...
static char *
name_match(Namval_t *np, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
...
= { sizeof(struct match), 0, get_match, 0, 0, 0, 0, name_match };

static char *
get_version(Namval_t *np, Namfun_t *fp)
{
    return (nv_getv(np, fp));
}

static Sfdouble_t
nget_version(Namval_t *np, Namfun_t *fp)
{
...
static void
put_vpath(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    char *cp;
    if (cp = nv_getval(np))
        vpath_set(cp, 0);
...
= { sizeof(Namfun_t), put_optindex, 0, nget_optindex, 0, 0, clone_optindex };
static const Namdisc_t SECONDS_disc
= { sizeof(struct seconds), put_seconds, get_seconds, nget_seconds };
static const Namdisc_t RAND_disc
= { sizeof(struct rand), put_rand, get_rand, nget_rand };
static const Namdisc_t LINENO_disc
= { sizeof(Namfun_t), put_lineno, get_lineno, nget_lineno };
static const Namdisc_t L_ARG_disc
= { sizeof(Namfun_t), put_lastarg, get_lastarg };
static const Namdisc_t OPTastbin_disc = { sizeof(Namfun_t), put_astbin };
static const Namdisc_t OPTIONS_disc
= { sizeof(Namfun_t), put_options, get_options };


#define MAX_MATH_ARGS 3

static char *
name_math(Namval_t *np, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
...
static Namval_t *
create_math(Namval_t *np, const char *name, int flag, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
    if (!name)
        return (SH_MATHNOD);
    if (name[0] != 'a' || name[1] != 'r' || name[2] != 'g' || name[4]
        || !isdigit(name[3])
...
static char *
get_math(Namval_t *np, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
    Namval_t *mp, fake;
    char *val;
...
static char *
setdisc_any(Namval_t *np, const char *event, Namval_t *action, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
    Namval_t *mp, fake;
    char *name;
...
#if SHOPT_NAMESPACE
static char *
get_nspace(Namval_t *np, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
    if (shp->namespace)
 ...
 */
static int
newconf(const char *name, const char *path, const char *value)
{
    Shell_t *shp = sh_getinterp();
    char *arg;
    if (!name)
        setenviron(value);
    else if (strcmp(name, "UNIVERSE") == 0
             && strcmp(astconf(name, 0, 0), value)) {
        shp->universe = 0;
...
sh_type(const char *path)
{
    ...
    return t
           & ~(SH_TYPE_BASH | SH_TYPE_KSH | SH_TYPE_PROFILE
               | SH_TYPE_RESTRICTED);
}


static char *
get_mode(Namval_t *np, Namfun_t *nfp)
{
 ...
 */
Shell_t *
sh_init(int argc, char *argv[], Shinit_f userinit)
{
    static int beenhere;
    Shell_t *shp;
    size_t n;
...
#    else
        ...
        if ((n = readlink(sfstruse(shp->strbuf), buff, sizeof(buff) - 1))
            > 0) {
            buff[n] = 0;
            shp->gd->shpath = strdup(buff);
        }
#    endif
        else if ((cp && (sh_type(cp) & SH_TYPE_SH))
                 || (argc > 0 && strchr(cp = *argv, '/'))) {
            if (*cp == '/')
...
#    endif
    ...
    if (shp->gd->userid != shp->gd->euserid
        ...
        if (!sh_isoption(shp, SH_PRIVILEGED)
            && shp->gd->userid >= SHOPT_P_SUID) {
...
#endif
#if 0
#    define NV_MKINTTYPE(x, y, z)                                            \
        nv_mkinttype(#x, sizeof(x), ( x )-1 < 0, (y), ( Namdisc_t * )z);
 ...
 */
int
sh_reinit_20120720(Shell_t *shp, char *argv[])
{
    Shopt_t opt;
    Namval_t *np, *npnext;
    Dt_t *dp;
    ...
    nv_scan(
    shp->var_tree, sh_envnolocal, ( void * )&data, NV_ARRAY, NV_ARRAY);
    sh_offstate(shp, SH_INIT);
 ...
 */
Namfun_t *
nv_cover(Namval_t *np)
{
    if (np == IFSNOD || np == PATHNOD || np == SHELLNOD || np == FPATHNOD
        || np == CDPNOD || np == SECONDS || np == ENVNOD || np == LINENO)
        return (np->nvfun);
#ifdef _hdr_locale
...
static Namval_t *
create_sig(Namval_t *np, const char *name, int flag, Namfun_t *fp)
{
    return (0);
}
...
static Namval_t *
next_svar(Namval_t *np, Dt_t *root, Namfun_t *fp)
{
...
static Namval_t *
create_svar(Namval_t *np, const char *name, int flag, Namfun_t *fp)
{
    struct Svars *sp = ( struct Svars * )fp;
    const char *cp = name;
    int i = 0, n;
    ...
    for (i = 0; i < sp->numnodes; i++) {
        ...
        if ((n == 0 || strncmp(name, nq->nvname, n) == 0)
            && nq->nvname[n] == 0)
            goto found;
    }
    nq = 0;
...
static Namfun_t *
clone_svar(Namval_t *np, Namval_t *mp, int flags, Namfun_t *fp)
{
    ...
    for (i = dp->numnodes; --i >= 0;) {
        ...
        if (np->nvalue.cp >= ( char * )sp->data
            && np->nvalue.cp < ( char * )sp->data + sp->dsize)
            mp->nvalue.cp
            = ( char * )(dp->data) + (np->nvalue.cp - ( char * )sp->data);
...
= { 0, 0, 0, 0, 0, create_svar, clone_svar, 0, next_svar };

static char *
name_svar(Namval_t *np, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
...
static int
svar_init(Shell_t *shp, Namval_t *pp, const Shtable_t *tab, size_t extra)
{
    int i, nnodes = 0;
    struct Svars *sp;
    Namval_t *np, **mp;
    ...
    sp = newof(0,
               struct Svars,
               1,
               nnodes * NV_MINSZ + sizeof(Namfun_t) + sizeof(void *) + extra);
    sp->nodes = ( char * )(sp + 1);
...
static void
siginfo_init(Shell_t *shp)
{
    struct Svars *sp;
    svar_init(shp,
              SH_SIG,
              shtab_siginfo,
              sizeof(siginfo_t) + sizeof(char *) + SIGNAME_MAX);
    sp = ( struct Svars * )SH_SIG->nvfun->next;
    sp->dsize = sizeof(siginfo_t) + SIGNAME_MAX;
}

static const char *
siginfocode2str(int sig, int code)
{
...
nv_init(Shell_t *shp)
{
    ...
    shp->bltin_tree
    = inittree(shp, ( const struct shtable2 * )shtab_builtins);
...
static Dt_t *
inittree(Shell_t *shp, const struct shtable2 *name_vals)
{
    Namval_t *np;
    ...
    for (tp = name_vals; *tp->sh_name; tp++, np++) {
        if ((np->nvname = strrchr(tp->sh_name, '.'))
...
env_init(Shell_t *shp)
{
    ...
    for (np0 = np, ep = environ; cp = *ep; ep++) {
        ...
        *dp++ = 0;
        if (mp = dtmatch(shp->var_base, cp)) {
            ...
            mp->nvenv = ( char * )cp;
            dp[-1] = '=';
        } else if (*cp == 'A' && cp[1] == '_' && cp[2] == '_' && cp[3] == 'z'
                   && cp[4] == 0) {
            dp[-1] = '=';
            next = cp + 4;
            continue;
    ...
    while (cp = next) {
        ...
        if (np != SHLVL && nv_isattr(np, NV_IMPORT | NV_EXPORT)) {
            ...
            if ((flag & (NV_INTEGER | NV_UTOL | NV_LTOU))
                == (NV_UTOL | NV_LTOU))
...
bool sh_isoption_20120720
DISABLE(Shell_t *shp, int opt)
{
...
static void
put_trans(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
...
Namfun_t *
nv_mapchar(Namval_t *np, const char *name)
{
