 ...
 */
static int
compare(Dt_t *dt, Void_t *key1, Void_t *key2, Dtdisc_t *disc)
{
    int c, d;
...
static Dtdisc_t env_disc = { 0, -1, sizeof(char *), 0, 0, compare };
 ...
 */
int
env_add(Env_t *ep, const char *str, int flags)
{
 ...
 */
int
env_delete(Env_t *ep, const char *str)
{
    Evar_t *vp = ( Evar_t * ) dtmatch(ep->dt, ( void * ) str);
    if (!vp)
        return (0);
