    ...
    if (sfset(sfstdout, 0, 0) & SF_STRING) {
        ...
        if ((fd = sffileno(sfstdout)) < 0) {
            ...
            if (off = sftell(sfstdout))
                write(fds[1],
                      sfsetbuf(sfstdout, ( Void_t * ) sfstdout, 0),
                      ( size_t ) off);
            sfclose(sfstdout);
...
bool
nv_subsaved(Namval_t *np, int table)
{
    struct subshell *sp;
 ...
 */
Namval_t *
sh_assignok(Namval_t *np, int add)
{
    Namval_t *mp;
    struct Link *lp;
    ...
    nv_clone(
    np, mp, (add ? (nv_isnull(np) ? 0 : NV_NOFREE) | NV_ARRAY : NV_MOVE));
    shp->subshell = save;
    return (np);
}
 ...
 * create new one if in a subshell and one doesn't exist and create is
 * non-zero
 */
Dt_t *
sh_subaliastree(Shell_t *shp, int create)
{
    struct subshell *sp = subshell_data;
    if (!sp || shp->curenv == 0)
 ...
 */
Dt_t *
sh_subfuntree(Shell_t *shp, int create)
{
    struct subshell *sp = subshell_data;
    if (!sp || shp->curenv == 0)
        return (shp->fun_tree);
...
Sfio_t *
sh_subshell(Shell_t *shp, Shnode_t *t, volatile int flags, int comsub)
{
    ...
    if (!comsub || !shp->subshare) {
        ...
        sp->shpwdfd = ((shp->pwdfd >= 0))
                      ? sh_fcntl(shp->pwdfd, F_dupfd_cloexec, 10)
                      : -1;
#ifdef O_SEARCH
        if (sp->shpwdfd < 0)
        ...
        if ((nsig = shp->st.trapmax * sizeof(char *)) > 0
            ...
            nsig += sizeof(char *);
            memcpy(
            savsig = malloc(nsig), ( char * ) &shp->st.trapcom[0], nsig);
    ...
    if (comsub != 2 && jmpval != SH_JMPSUB && shp->st.trapcom[0]
        && shp->subshell) {
