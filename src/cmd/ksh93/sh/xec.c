...
#if SHOPT_SPAWN
static pid_t
sh_ntfork(Shell_t *, const Shnode_t *, char *[], int *, int);
#endif /* SHOPT_SPAWN */

static void
sh_funct(Shell_t *, Namval_t *, int, char *[], struct argnod *, int);
static bool
trim_eq(const char *, const char *);
static void
coproc_init(Shell_t *, int pipes[]);
 ...
 */
static void
l_time(Sfio_t *outfile, clock_t t, int p)
{
    int min, sec, frac;
    int hr;
    if (p) {
...
static int
p_time(Shell_t *shp, Sfio_t *out, const char *format, clock_t *tm)
{
 ...
 */
static int
p_arg(Shell_t *, struct argnod *, int);
static int
p_switch(Shell_t *, struct regnod *);
static int
p_comarg(Shell_t *shp, struct comnod *com)
{
...
extern void
sh_optclear(Shell_t *, void *);

static int
sh_tclear(Shell_t *shp, Shnode_t *t)
{
    int n = 0;
    if (!t)
        return (0);
...
static int
p_arg(Shell_t *shp, struct argnod *arg, int flag)
{
    while (arg) {
...
static int
p_switch(Shell_t *shp, struct regnod *reg)
{
    int n = 0;
    while (reg) {
...
static void
out_pattern(Sfio_t *iop, const char *cp, int n)
{
    int c;
    do {
        switch (c = *cp) {
...
static void
out_string(Sfio_t *iop, const char *cp, int c, int quoted)
{
    if (quoted) {
 ...
 */
static void
put_level(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
...
static struct Level *
init_level(Shell_t *shp, int level)
{
         ...
         int flags)
{
    ...
    while (cp = *argv++) {
        if ((flags & ARG_EXP) && argv[1] == 0)
            ...
            out_string(
            iop, cp, ' ', n ? 0 : (flags & (ARG_RAW | ARG_NOGLOB)) || *argv);
    }
    if (flags & ARG_ASSIGN)
        sfputc(iop, ')');
    else if (iop == stkstd)
    ...
    if (!SH_LEVELNOD->nvfun || !SH_LEVELNOD->nvfun->disc
        || nv_isattr(SH_LEVELNOD, NV_INT16 | NV_NOFREE)
           != (NV_INT16 | NV_NOFREE))
...
static void
free_list(struct openlist *olist)
{
 ...
 */
static int
set_instance(Shell_t *shp, Namval_t *nq, Namval_t *node, struct Namref *nr)
{
    char *sp = 0, *cp;
    Namarr_t *ap;
    Namval_t *np;
...
static void
unset_instance(Namval_t *nq, Namval_t *node, struct Namref *nr, long mode)
{
 ...
 */
static void
print_fun(Namval_t *np, void *data)
{
    char *format;
    NOT_USED(data);
...
static void *
sh_coinit(Shell_t *shp, char **argv)
{
...
bool
sh_coaddfile(Shell_t *shp, char *name)
{
...
sh_coexec(Shell_t *shp, const Shnode_t *t, int filt)
{
    ...
    if (filt) {
        ...
        if (shp->inpipe[2] >= 20000)
            sfprintf(sfstdout,
                     "command exec < /dev/tcp/%s/%d || print -u2 'cannot "
                     "create pipe'\n",
                     host,
                     shp->inpipe[2]);
        sfprintf(
...
#if SHOPT_FILESCAN
static Sfio_t *
openstream(Shell_t *shp, struct ionod *iop, int *save)
{
...
#if SHOPT_NAMESPACE
static Namval_t *
enter_namespace(Shell_t *shp, Namval_t *nsp)
{
    ...
    shp->namespace = nsp;
    if (path
        && (path = nv_search(PATHNOD->nvname, shp->var_tree, HASH_NOSCOPE))
        && (val = nv_getval(path)))
        nv_putval(path, val, NV_RDONLY);
    if (fpath
        && (fpath = nv_search(FPATHNOD->nvname, shp->var_tree, HASH_NOSCOPE))
        && (val = nv_getval(fpath)))
...
int
sh_exec(Shell_t *shp, const Shnode_t *t, int flags)
{
    Stk_t *stkp = shp->stk;
    int unpipe = 0;
    sh_sigcheck(shp);
    if (t && !shp->st.execbrk && !sh_isoption(shp, SH_NOEXEC)) {
        ...
        case TCOM: {
            ...
            if (argn && shp->inpool) {
                ...
                if (!np || !is_abuiltin(np) || *np->nvname == '/'
                    || np == SYSCD) {
                    char **argv, *sp;
        ...
        tryagain:
...
#endif /* SHOPT_BASH */
                    if (np == SYSTYPESET
                        || (np && np->nvalue.bfp == SYSTYPESET->nvalue.bfp)) {
                        if (np != SYSTYPESET) {
                            shp->typeinit = np;
                            tp = nv_type(np);
                        }
                        ...
                        if (checkopt(com, 'n'))
                            ...
                            if (shp->namespace) {
                                ...
                                if (!shp->strbuf2)
                                    shp->strbuf2 = sfstropen();
                                sfprintf(shp->strbuf2,
                                         "%s%s%c",
                                         NV_CLASS,
                                         nv_name(shp->namespace),
                                         0);
                                ...
                                for (sp = xp + 1; sp;) {
                                    if (sp = strchr(sp, '.'))
                                        *sp = 0;
                                    nv_open(
                                    shp->prefix, shp->var_base, NV_VARNAME);
                                    if (sp)
                                        *sp++ = '.';
                                }
                            } else
            ...
            if ((io || argn)) {
                ...
                /* set +x doesn't echo */
                else if ((t->tre.tretyp & FSHOWME)
                         && sh_isoption(shp, SH_SHOWME)) {
                ...
                if (!np && !strchr(com0, '/')) {
                    if (path_search(shp, com0, NIL(Pathcomp_t **), 1)) {
                        ...
                        if (!np || !np->nvalue.ip) {
                            Namval_t *mp
                            = nv_search(com0, shp->bltin_tree, 0);
                            if (mp)
                                np = mp;
                ...
                if (np && pipejob == 2) {
                    if (shp->comsub == 1 && np && is_abuiltin(np)
                        && *np->nvname == '/')
                        np = 0;
                    else {
                        job_unlock();
                ...
                if (np && is_abuiltin(np)) {
                    ...
                    if (jmpval == 0) {
                        ...
                        if (execflg && !shp->subshell && !shp->st.trapcom[0]
                            ...
                            for (fd = 0; fd < shp->gd->lim.open_max; fd++)
                                if ((shp->fdstatus[fd] & IOCLEX)
                                    && fd != shp->infd && (fd != shp->pwdfd))
                                    sh_close(fd);
                        }
                        if (argn)
                            shp->exitval
...
#if 1
                        spawnvex_apply(
                        shp->vex, 0, SPAWN_RESET | SPAWN_FRAME);
#else
                    {
                        int fd;
                        spawnvex_apply(
                        shp->vex, 0, SPAWN_RESET | SPAWN_FRAME);
                        if (shp->comsub && (fd = sffileno(sfstdout)) != 1
                            && fd >= 0)
                    ...
                    if (!(nv_isattr(np, BLT_ENV))) {
                        ...
                        if (shp->pwd || (shp->pwdfd >= 0)) {
                            ...
                            if (statb.st_ino != stata.st_ino
                                || statb.st_dev != stata.st_dev) {
                                /* chdir for directories on HSM/tapeworms may
                                 * take minutes */
                                int err = errno;
                                if (shp->pwdfd >= 0) {
                                    while ((fchdir(shp->pwdfd) < 0)
                                           && errno == EINTR)
                                        errno = err;
                                } else {
                                    while ((chdir(shp->pwd) < 0)
                                           && errno == EINTR)
                                        errno = err;
                                }
                            }
...
#endif /* SHOPT_NAMESPACE */
                                np = nv_search(
                                com0, shp->fun_tree, HASH_NOSCOPE);
                        }

                        if (!np->nvalue.ip) {
                            if (indx == 1) {
                                errormsg(
                                SH_DICT, ERROR_exit(0), e_defined, com0);
                                shp->exitval = ERROR_NOEXEC;
                            } else {
                                errormsg(
                                SH_DICT, ERROR_exit(0), e_found, "function");
...

                    {
                        ...
                        if (*np->nvname == '.') {
                            ...
                            if (ep = strrchr(cp, '.')) {
                                ...
                                *ep = 0;
                                namespace = nv_search(
                                cp - 1, shp->var_base, HASH_NOSCOPE);
                                *ep = '.';
                            }
                        }
             ...
             */
            {
                ...
                if (type & (FCOOP | FAMP))
                    ...
                    if (!sh_isoption(shp, SH_MONITOR)) {
                        if (!(shp->sigflag[SIGINT]
                              & (SH_SIGFAULT | SH_SIGOFF)))
...
#    endif /* SHOPT_COSHELL */
        ...
        case TFIL: {
            ...
            if (shp->inpool) {
                do {
                    ...
                    if (flags
                        && (t->tre.tretyp != TFIL
                            || !(t->lst.lstlef->tre.tretyp & FALTPIPE)))
                        goto coskip1;
        ...
        case TFOR: /* for and select */
        {
            ...
            while (cp && shp->st.execbrk == 0) {
                ...
                flag &= ~OPTIMIZE_FLAG;
                if (t->tre.tretyp & COMSCAN) {
                    if ((cp = nv_getval(sh_scoped(shp, REPLYNOD)))
                        && *cp == 0)
                        refresh++;
                } else
                    cp = *++args;
        ...
        case TWH: /* while and until */
        {
            ...
            if (shp->inpool) {
                int poolfiles;
#    if SHOPT_FILESCAN
                if (type == TWH && tt->tre.tretyp == TCOM && !tt->com.comarg
                    && tt->com.comio) {
...
#    endif /* SHOPT_FILESCAN */
            ...
            while (t) {
                ...
                while (rex) {
                    ...
                    if (rex->argflag & ARG_MAC) {
                        s
                        = sh_macpat(shp, rex, OPTIMIZE | ARG_EXP | ARG_CASE);
                        while (*s == '\\' && s[1] == 0)
                            s += 2;
                    } else
                        s = rex->argval;
        ...
        case TFUN: {
            ...
            if (cp || shp->prefix) {
                ...
                if (shp->prefix) {
                    ...
                    shp->prefix = 0;
                    npv = nv_open(
                    cp, shp->var_tree, NV_NOASSIGN | NV_NOARRAY | NV_VARNAME);
                    shp->prefix = cp;
                    cp = fname;
                } else {
                    sfwrite(stkp, fname, cp++ - fname);
                    sfputc(stkp, 0);
                    npv = nv_open(stkptr(stkp, offset),
                                  shp->var_tree,
            ...
            } else if ((mp = nv_search(fname, shp->bltin_tree, 0))
                       && nv_isattr(mp, BLT_SPC))
        ...
        case TTST: {
            ...
            if ((type & TPAREN) == TPAREN) {
                ...
                if (type & TBINARY)
                    ...
                    shp,
                    &(t->lst.lstrit->arg),
                    ((n == TEST_PEQ || n == TEST_PNE) ? ARG_EXP : 0)
                    | OPTIMIZE);
                shp->savexit = savexit;
                if (trap = shp->st.trap[SH_DEBUGTRAP])
                    argv[0]
        ...
        if (shp->trapnote
            || (shp->exitval && sh_isstate(shp, SH_ERREXIT)) && t && echeck)
            sh_chktrap(shp);
        /* set $_ */
        if (mainloop && com0) {
...
static bool
trim_eq(const char *r, const char *s)
{
    char c;
    while (c = *s++) {
        if (c == '\\')
...
bool
sh_trace(Shell_t *shp, char *argv[], int nl)
{
    char *cp;
    int bracket = 0;
    int decl = (nl & 2);
 ...
 */
pid_t
_sh_fork(Shell_t *shp, pid_t parent, int flags, int *jobid)
{
...
pid_t
sh_fork(Shell_t *shp, int flags, int *jobid)
{
    pid_t parent;
    sigset_t set, oset;
    if (!shp->pathlist)
...
local_exports(Namval_t *np, void *data)
{
    ...
    if ((cp = nv_getval(np))
        && (mp = nv_search(nv_name(np), shp->var_tree, NV_ADD | HASH_NOSCOPE))
        && nv_isnull(mp))
        nv_putval(mp, cp, 0);
}
 ...
 */
Sfdouble_t
sh_mathfun(Shell_t *shp, void *fp, int nargs, Sfdouble_t *arg)
{
    Sfdouble_t d;
 ...
 */
int
sh_fun_20120720(Shell_t *shp, Namval_t *np, Namval_t *nq, char *argv[])
{
    int offset;
    char *base;
    Namval_t node;
...
#undef sh_fun
int
sh_fun(Namval_t *np, Namval_t *nq, char *argv[])
{
 ...
 */
int
cmdrecurse(int argc, char *argv[], int ac, char *av[])
{
    NOT_USED(argc);
    NOT_USED(argv[0]);
    NOT_USED(ac);
 ...
 */
static void
coproc_init(Shell_t *shp, int pipes[])
{
    int outfd;
 ...
 */
static int
run_subshell(Shell_t *shp, const Shnode_t *t, pid_t grp)
{
 ...
 */
static pid_t
sh_ntfork(Shell_t *shp, const Shnode_t *t, char *argv[], int *jobid, int flag)
{
...
#        endif /* JOBS */
        ...
        if (optimize == 0) {
            ...
            if (grp > 0 && !(otype & FAMP)) {
                while (tcsetpgrp(job.fd, job.curpgid) < 0
...
#    endif
        ...
        if (!strchr(path = argv[0], '/')) {
            Namval_t *np;
            if ((np = nv_search(path, shp->track_tree, 0))
                && !nv_isattr(np, NV_NOALIAS) && np->nvalue.cp)
                path = nv_getval(np);
...
#    endif /* JOBS */
        ...
        if (spawnpid < 0 && errno == ENOEXEC) {
            ...
            if (!shp->gd->shpath)
                shp->gd->shpath = pathshell();
            spawnpid = path_spawn(
            shp, shp->gd->shpath, &argv[-1], arge, pp, (grp << 1) | 1);
            if (fd >= 0)
                sh_close(fd);
            argv[0] = argv[-1];
        }
    ...
    if (scope) {
        ...
        if (jmpval == SH_JMPSCRIPT)
            sh_setlist(
            shp, t->com.comset, NV_EXPORT | NV_IDENT | NV_ASSIGN, 0);
    }
 ...
 */
int
sh_funscope_20120720(Shell_t *shp,
                     int argn,
                     char *argv[],
                     int (*fun)(void *),
                     void *arg,
    ...
    if (jmpval == 0) {
        if (shp->fn_depth++ > MAXDEPTH) {
            ...
            if (nref = fp->nref) {
                ...
                for (r = 0; args[r]; r++) {
                    np = nv_search(
                    args[r], shp->var_tree, HASH_NOSCOPE | NV_ADD);
                    if (np && (nq = *nref++)) {
...
#undef sh_funscope
int
sh_funscope(int argn, char *argv[], int (*fun)(void *), void *arg, int execflg)
{
    return (
    sh_funscope_20120720(sh_getinterp(), argn, argv, fun, arg, execflg));
}
 ...
 */
int
sh_eval_20120720(Shell_t *shp, Sfio_t *iop, int mode)
{
    Shnode_t *t;
...
#define SH_TOPFUN 0x8000 /* this is a temporary tksh hack */
    ...
    while (jmpval == 0) {
        ...
        sh_exec(shp,
                t,
                sh_isstate(shp, SH_ERREXIT) | sh_isstate(shp, SH_NOFORK)
                | (mode & ~SH_FUNEVAL));
        if (!io_save)
            break;
    }
...
#undef sh_eval
int
sh_eval(Sfio_t *iop, int mode)
{
    return (sh_eval_20120720(sh_getinterp(), iop, mode));
}

int
sh_run_20120720(Shell_t *shp, int argn, char *argv[])
{
    struct dolnod *dp;
    struct comnod *t
