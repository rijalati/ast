...
static int
canexecute(Shell_t *, char *, int);
static void
funload(Shell_t *, int, const char *);
static void
exscript(Shell_t *, char *, char *[], char *const *);
static bool
path_chkpaths(Shell_t *, Pathcomp_t *, Pathcomp_t *, Pathcomp_t *, int);
static void
path_checkdup(Shell_t *shp, Pathcomp_t *);

static const char *std_path;

static bool
onstdpath(const char *name)
{
          ...
          pid_t pgid)
{
    ...
    while (1) {
...
#ifdef SPAWN_cwd
        {
            ...
            *av0 = arg0;
            if (pid > 0 && shp->comsub && (fd = sffileno(sfstdout)) != 1
                && fd >= 0)
 ...
 */
static char *
path_lib(Shell_t *shp, Pathcomp_t *pp, char *path)
{
    ...
    if (r >= 0) {
        ...
        if (last)
            pcomp.len = last - path;
        memcpy(( void * ) save,
               ( void * ) stkptr(shp->stk, PATH_OFFSET + pcomp.len),
               sizeof(save));
        if (path_chkpaths(
            shp, ( Pathcomp_t * ) 0, ( Pathcomp_t * ) 0, &pcomp, PATH_OFFSET))
            return (stkfreeze(shp->stk, 1));
        memcpy(( void * ) stkptr(shp->stk, PATH_OFFSET + pcomp.len),
               ( void * ) save,
               sizeof(save));
    }
    return (0);
 ...
 */
static void
path_checkdup(Shell_t *shp, Pathcomp_t *pp)
{
    char *name = pp->name;
 ...
 */
Pathcomp_t *
path_nextcomp(Shell_t *shp, Pathcomp_t *pp, const char *name, Pathcomp_t *last)
{
    Pathcomp_t *ppnext;
...
static Pathcomp_t *
defpath_init(Shell_t *shp)
{
    Pathcomp_t *pp
    = ( void * ) path_addpath(shp, ( Pathcomp_t * ) 0, (std_path), PATH_PATH);
    return (pp);
}

static void
path_init(Shell_t *shp)
{
    const char *val;
    Pathcomp_t *pp;
 ...
 */
Pathcomp_t *
path_get(Shell_t *shp, const char *name)
{
    Pathcomp_t *pp = 0;
    if (*name && strchr(name, '/'))
        return (0);
 ...
 */
static int
path_opentype(Shell_t *shp, const char *name, Pathcomp_t *pp, int fun)
{
    int fd = -1;
    struct stat statb;
    Pathcomp_t *oldpp;
 ...
 */
int
path_open(Shell_t *shp, const char *name, Pathcomp_t *pp)
{
...
char *
path_fullname(Shell_t *shp, const char *name)
{
 ...
 */
static void
funload(Shell_t *shp, int fno, const char *name)
{
    ...
    if (shp->fpathdict && (rp = dtmatch(shp->fpathdict, ( void * ) pname))) {
        ...
        do {
            ...
            rp->fdict = funtree;
        } while ((rp = dtnext(shp->fpathdict, rp))
                 && strcmp(pname, rp->fname) == 0);
        sh_close(fno);
        free(( void * ) pname);
        return;
    }
    ...
    error_info.line = 0;
    sh_eval(
    shp, sfnew(NIL(Sfio_t *), buff, IOBSIZE, fno, SF_READ), SH_FUNEVAL);
    sh_close(fno);
    shp->readscript = 0;
#if SHOPT_NAMESPACE
    if (shp->namespace)
...
bool
path_search(Shell_t *shp, const char *name, Pathcomp_t **oldpp, int flag)
{
    Namval_t *np;
    int fno;
    Pathcomp_t *pp = 0;
 ...
 */
Pathcomp_t *
path_absolute(Shell_t *shp, const char *name, Pathcomp_t *pp)
{
    int f, isfun;
    int noexec = 0;
    Pathcomp_t *oldpp;
...
#        if (_AST_VERSION >= 20040404)
                if (dll = dllplug(
                    SH_ID, stkptr(shp->stk, m), NiL, RTLD_LAZY, NiL, 0))
#        else
                if (dll
                    = dllfind(stkptr(shp->stk, m), NiL, RTLD_LAZY, NiL, 0))
#        endif
                {
                    /*
                     * this detects the 2007-05-11 builtin context change and
                     * also the 2008-03-30 opt_info.num change that hit
                     * libcmd::b_head
                     */
...
#    endif
                if (dll
                    && (addr
                        = ( Shbltin_f ) dlllook(dll, stkptr(shp->stk, n)))
                    && (!(np = sh_addbuiltin(
...
canexecute(Shell_t *shp, char *path, int isfun)
{
    ...
    if (S_ISDIR(statb.st_mode))
        errno = EISDIR;
    else if (isfun == 1 || (statb.st_mode & S_IXALL) == S_IXALL
             || sh_access(path, X_OK) >= 0)
        return (fd);
    if (isfun && fd >= 0)
        sh_close(fd);
err:
...
char *
path_relative(Shell_t *shp, const char *file)
{
    const char *pwd;
    const char *fp = file;
...
void
path_exec(Shell_t *shp, const char *arg0, char *argv[], struct argnod *local)
{
    char **envp;
    const char *opath;
...
#ifdef SPAWN_cwd
static int
vexexec(void *ptr, uintmax_t fd1, uintmax_t fd2)
{
    char *devfd;
    int fd = -1;
...
static void
exscript(Shell_t *shp, char *path, char *argv[], char *const *envp)
{
    Sfio_t *sp;
    path = path_relative(shp, path);
    shp->comdiv = 0;
             ...
             int flag)
{
    ...
    if ((old || shp->pathinit)
        && ((flag & (PATH_PATH | PATH_SKIP)) == PATH_PATH))
        ...
        for (ep = 0; n--; cp++) {
            ...
            *cp = 0;
            ...
            if (m == 0
                ...
                memcpy(
                ( void * ) &pp->lib[m], stkptr(shp->stk, offset), pp->len);
                pp->lib[k = m + pp->len] = '/';
...
Pathcomp_t *
path_addpath(Shell_t *shp, Pathcomp_t *first, const char *path, int type)
{
    const char *cp;
    Pathcomp_t *old = 0;
    int offset = stktell(shp->stk);
    ...
    if (old) {
        ...
        if (cp = (sh_scoped(shp, FPATHNOD))->nvalue.cp)
            first = ( void * ) path_addpath(
            shp, ( Pathcomp_t * ) first, cp, PATH_FPATH);
        path_delete(old);
    }
    if (offset)
 ...
 */
void
path_newdir(Shell_t *shp, Pathcomp_t *first)
{
 ...
 */
static char *
talias_get(Namval_t *np, Namfun_t *nvp)
{
