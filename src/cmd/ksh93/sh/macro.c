...
static void
copyto(Mac_t *, int, int);
static void
comsubst(Mac_t *, Shnode_t *, int);
static bool
varsub(Mac_t *);
static void
mac_copy(Mac_t *, const char *, size_t);
static void
tilde_expand2(Shell_t *, int);
static char *
sh_tilde(Shell_t *, const char *);
static char *
special(Shell_t *, int);
static void
endfield(Mac_t *, int);
...
mac_getstring(char *);
static int
charlen(const char *, int);
#if SHOPT_MULTIBYTE
static char *
lastchar(const char *, const char *);
#endif /* SHOPT_MULTIBYTE */
 ...
 */
char *
sh_mactry(Shell_t *shp, char *string)
{
    if (string)
    {
        int jmp_val;
             ...
             int flag)
{
    ...
    mp->assign = !mp->split;
    mp->pattern
    = mp->split && !(flag & ARG_NOGLOB) && !sh_isoption(mp->shp, SH_NOGLOB);
 ...
 */
void
sh_machere(Shell_t *shp, Sfio_t *infile, Sfio_t *outfile, char *string)
{
    int c, n;
    ...
    shp->argaddr = 0;
    mp->sp = outfile;
    mp->split = mp->assign = mp->pattern = mp->patfound = mp->lit = mp->arith
    = mp->let = 0;
    mp->quote = 1;
 ...
 */
char *
sh_macpat(Shell_t *shp, struct argnod *arg, int flags)
{
    char *sp = arg->argval;
 ...
 */
static void
copyto(Mac_t *mp, int endch, int newquote)
{
    int c, n;
    ...
    while (1)
    {
        ...
        case S_ESC:
            ...
            if (mp->pattern)
            {
                ...
                if (!(ere && *cp == '$')
                    && (mp->lit
                        || (mp->quote && !isqescchar(n) && n != S_ENDCH)))
                {
        ...
        case S_COLON:
            ...
            if (n == S_COLON && mp->assign == 2 && *cp == '~' && endch == 0
                && !mp->quote && !mp->lit)
                tilde = stktell(stkp) + (c + 1);
 ...
 */
static void
mac_substitute(Mac_t *mp, char *cp, char *str, int subexp[], int subsize)
{
    int c, n;
    char *first = fcseek(0);
    char *ptr;
...
static char *
nextname(Mac_t *mp, const char *prefix, int len)
{
    char *cp;
    if (len == 0)
    {
...
retry1:
...
#if SHOPT_BASH
            else if (sh_isoption(mp->shp, SH_BASH) && c == '!'
                     && isadigit(mode))
                c = '$';
#endif
        }
    ...
    case S_ALP:
        ...
        do
        {
            ...
            } while ((d = c, (c = fcmbget(&LEN)), isaname(c))
                     || type && c == '.');
...
#if SHOPT_BASH
            if (sh_isoption(mp->shp, SH_BASH) && (c == '}' || c == ':')
                && type == M_VNAME && !nv_isattr(np, NV_REF))
                type = M_EVAL;
#endif
        ...
        if (type)
        {
            if (mp->dotdot)
            {
                ...
                else
                {
                    if (( int )sh_arith(mp->shp, v))
                     ...
                     && !(ap->flags & ARRAY_SCAN) && type != M_SIZE)
            ...
            fcseek(-1);
        if (type <= 1 && np && nv_isvtree(np) && mp->pattern == 1
            && !mp->split)
        {
...
skip:
    ...
    {
        c = fcmbget(&LEN);
        if (isascii(c) && sh_lexstates[ST_BRACE][c] == S_MOD1 && c != '*'
            && c != ':')
            nulflg = 1;
        else if (c != '%' && c != '#')
        {
    ...
    if (type)
    {
        ...
        if (c != RBRACE)
        {
            ...
            offset = stktell(stkp);
            if (newops && sh_isoption(mp->shp, SH_NOUNSET) && *id
                && id != idbuff && (!np || nv_isnull(np)))
                errormsg(SH_DICT, ERROR_exit(1), e_notset, id);
            if (c == ',' || c == '^' || c == '/' || c == ':'
                || ((!v || (nulflg && *v == 0))
                    ^ (c == '+' || c == '#' || c == '%')))
            ...
            {
                sh_lexskip(
                lp, RBRACE, 0, (!newops && mp->quote) ? ST_QUOTE : ST_NESTED);
                stkseek(stkp, offset);
            }
...
retry2:
    if (v && (!nulflg || *v) && c != '+')
    {
        ...
        while (1)
        {
            ...
            {
                flag
                = (type || c == '/') ? (STR_GROUP | STR_MAXIMAL) : STR_GROUP;
                if (c != '/')
                    flag |= STR_LEFT;
                index = nmatch = 0;
                tsize = ( int )strlen(v);
                while (1)
                {
                    ...
                    if (c == '%')
                        nmatch = substring(
                        v, tsize, pattern, match, flag & STR_MAXIMAL);
                    else
                    ...
                    if (nmatch && repstr && !mp->macsub)
                        sh_setmatch(
                        mp->shp, v, vsize, nmatch, match, index++);
                    if (nmatch)
                    {
                        vlast = v;
                        vsize_last = vsize;
                ...
                if (!mp->macsub && (!repstr || (nmatch == 0 && index == 0)))
                    sh_setmatch(
                    mp->shp, vlast, vsize_last, nmatch, match, index++);
    ...
    else if (argp)
    {
        ...
        else if (c == '=')
        {
            if (np)
            {
             ...
             && (!np || nv_isnull(np) || (nv_isarray(np) && !np->nvalue.cp)))
    {
        if (np)
        {
            if (nv_isarray(np))
            {
                sfprintf(
                mp->shp->strbuf, "%s[%s]\0", nv_name(np), nv_getsub(np));
                id = sfstruse(mp->shp->strbuf);
            }
            else
                id = nv_name(np);
 ...
 */
static void
comsubst(Mac_t *mp, Shnode_t *t, volatile int type)
{
    Sfdouble_t num;
    int c;
    char *str;
    ...
    if (type)
    {
        ...
        if (t && t->tre.tretyp == TARITH)
        {
            ...
            if (t->ar.arcomp)
                ...
                num = sh_arith(mp->shp,
                               sh_mactrim(mp->shp, t->ar.arexpr->argval, 3));
            mp->shp->inarith = 0;
        out_offset:
        ...
        else if (type == 2 && t && (t->tre.tretyp & COMMSK) == 0
                 && t->com.comarg)
        {
            Namval_t *np;
            str = NULL;
    ...
    if (t)
    {
        ...
        if (t->tre.tretyp == 0 && !t->com.comarg && !t->com.comset)
        {
            ...
            sp
            = sh_subshell(mp->shp, t, sh_isstate(mp->shp, SH_ERREXIT), type);
        fcrestore(&save);
    }
    else
 ...
 */
static void
mac_copy(Mac_t *mp, const char *str, size_t size)
{
    char *state;
    const char *cp = str;
    ...
    else if (mp->pattern >= 2 || (mp->pattern && nopat) || mp->assign == 3)
    {
        ...
        while (size-- > 0)
        {
            ...
            if (nopat
                && (c == S_PAT || c == S_ESC || c == S_BRACT || c == S_ENDCH)
                && mp->pattern != 3)
                c = 1;
            else if (mp->pattern == 4
                     && (c == S_ESC || c == S_BRACT || c == S_ENDCH
                         || isastchar(n)))
            {
...
#if SHOPT_MULTIBYTE
                    if (n == S_MBYTE
                        && sh_strchr(mp->ifsp, cp - 1, ep - cp + 1) >= 0)
                    {
                        n = mbnsize(cp - 1, ep - cp + 1) - 1;
                        if (n == -2)
                            n = 0;
...
#endif /* SHOPT_MULTIBYTE */
static int
charlen(const char *string, int len)
{
    if (!string)
        return (0);
#if SHOPT_MULTIBYTE
 ...
 */
static int
sh_btilde(int argc, char *argv[], Shbltin_t *context)
{
 ...
 */
static void
tilde_expand2(Shell_t *shp, int offset)
{
    ...
    np = nv_open(shtilde,
                 shp->fun_tree,
                 NV_VARNAME | NV_NOARRAY | NV_NOASSIGN | NV_NOFAIL);
    if (np && !beenhere)
    {
        beenhere = 1;
...
static char *
sh_tilde(Shell_t *shp, const char *string)
{
    char *cp;
    int c;
    struct passwd *pw;
...
special(Shell_t *shp, int c)
{
    ...
    case 0:
        if (sh_isstate(shp, SH_PROFILE) || shp->fn_depth == 0
            || !shp->st.cmdname)
            return (shp->shname);
        else
