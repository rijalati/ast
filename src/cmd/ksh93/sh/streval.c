...
#define pow2size(x)                                                          \
    ((x) <= 2                                                                \
     ? 2                                                                     \
     : (x) <= 4 ? 4 : (x) <= 8 ? 8 : (x) <= 16 ? 16 : (x) <= 32 ? 32 : 64)
...
#define stkpush(stk, v, val, type)                                           \
    ((((v)->offset = round(stktell(stk), pow2size(sizeof(type)))),           \
      ...
      *(( type * )stkptr(stk, (v)->offset)) = (val)),                        \
     (v)->offset)
#define roundptr(ep, cp, type)                                               \
    ((( unsigned char * )(ep))                                               \
     + round(cp - (( unsigned char * )(ep)), pow2size(sizeof(type))))
 ...
 */
static int
_seterror(struct vars *vp, const char *msg)
{
    if (!vp->errmsg.value)
...
static void
arith_error(const char *message, const char *expr, int mode)
{
    level = 0;
    mode = (mode & 3) != 0;
    ...
    while (c = *cp++)
    {
        ...
        case A_PUSHV:
            ...
            if (node.nextop == A_JMP)
                node.nextop = (( unsigned char * )
                               ep)[*(( short * )roundptr(ep, cp + 1, short))];
...
static bool
expr(struct vars *vp, int precedence)
{
    int c, op;
    int invalid, wasop = 0;
 ...
 *
 * (Sfdouble_t)(*convert)(char** end, struct lval* string, int type,
 * Sfdouble_t value) is a user supplied conversion routine that is called when
 * unknown chars are encountered. *end points to the part to be converted and
 * must be adjusted by convert to point to the next non-converted character;
 * if typ is MESSAGE then string points to an error message string
 *
...
strval(Shell_t *shp,
       const char *s,
       char **end,
       Sfdouble_t (*conv)(const char **, struct lval *, int, Sfdouble_t),
       int emode)
{
    Arith_t *ep;
    Sfdouble_t d;
