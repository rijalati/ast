...
#    define array_fixed_data(ap)                                             \
        ((ap) ? (( struct fixed_array * ) ((ap)->fixed))->data : 0)
static void
array_fixed_setdata(Namval_t *, Namarr_t *, struct fixed_array *);
#endif /* SHOPT_FIXEDARRAY */

static Namarr_t *
array_scope(Namval_t *np, Namarr_t *ap, int flags)
{
...
static bool
array_unscope(Namval_t *np, Namarr_t *ap)
{
    Namfun_t *fp;
    if (!ap->scope)
        return (false);
...
static void
array_syncsub(Namarr_t *ap, Namarr_t *aq)
{
    (( struct index_array * ) ap)->cur = (( struct index_array * ) aq)->cur;
}

static bool
array_covered(Namval_t *np, struct index_array *ap)
{
    struct index_array *aq = ( struct index_array * ) ap->header.scope;
    if (!ap->header.fun && aq)
#if SHOPT_FIXEDARRAY
        return (ap->header.fixed
                || ((ap->cur < aq->maxi) && aq->val[ap->cur].cp));
#else
 ...
 */
static void
array_setptr(Namval_t *np, struct index_array *old, struct index_array *new)
{
 ...
 */
static int
arsize(struct index_array *ap, int maxi)
{
...
array_grow(Namval_t *, struct index_array *, int);
...
static union Value *
array_getup(Namval_t *np, Namarr_t *arp, int update)
{
...
bool
nv_arrayisset(Namval_t *np, Namarr_t *arp)
{
    struct index_array *ap = ( struct index_array * ) arp;
    union Value *up;
    if (is_associative(ap))
 ...
 */
static Namval_t *
array_find(Namval_t *np, Namarr_t *arp, int flag)
{
    Shell_t *shp = sh_ptr(np);
    struct index_array *ap = ( struct index_array * ) arp;
    union Value *up;
    ...
    if (wasundef = ap->header.flags & ARRAY_UNDEF) {
        ...
        if (flag & ARRAY_DELETE) {
            ...
            nv_putsub(np,
                      NIL(char *),
                      0,
                      ARRAY_SCAN | ARRAY_NOSCOPE
                      | (ap->header.fixed ? (ARRAY_UNDEF | ARRAY_FIXED) : 0));
#else
        ...
        } else /* same as array[0] */
        {
            if (is_associative(ap))
                (*ap->header.fun)(
                np, "0", flag == ARRAY_ASSIGN ? NV_AADD : 0);
#if SHOPT_FIXEDARRAY
            else if (fp) {
                int n = fp->ndim;
                fp->curi = 0;
    ...
    if (!up->cp && nv_isattr(np, NV_NOTSET) != NV_NOTSET) {
        ...
        if (flag != ARRAY_ASSIGN) {
            ...
            = (arp->fun ? 0 : ( struct index_array * ) arp->scope);
...
bool
nv_arraysettype(Namval_t *np, Namval_t *tp, const char *sub, int flags)
{
    Shell_t *shp = sh_ptr(np);
    Namval_t *nq;
    char *av[2];
...
static Namfun_t *
array_clone(Namval_t *np, Namval_t *mp, int flags, Namfun_t *fp)
{
    ...
    if (!nv_putsub(np,
                   NIL(char *),
                   0,
                   ARRAY_SCAN | ((flags & NV_COMVAR) ? 0 : ARRAY_NOSCOPE))) {
        if (ap->fun)
            (*ap->fun)(np, ( char * ) np, 0);
        skipped = 1;
        goto skip;
    ...
    do {
        ...
        if (nq
            && (((flags & NV_COMVAR) && nv_isvtree(nq)) || nv_isarray(nq))) {
            mq->nvalue.cp = 0;
            if (!is_associative(ap))
                ar->val[ar->cur].np = mq;
...
static char *
array_getval(Namval_t *np, Namfun_t *disc)
{
...
static Sfdouble_t
array_getnum(Namval_t *np, Namfun_t *disc)
{
    Namarr_t *aq, *ap = ( Namarr_t * ) disc;
    Namval_t *mp;
    if ((mp = array_find(np, ap, ARRAY_LOOKUP)) != np) {
...
static void
array_putval(Namval_t *np, const char *string, int flags, Namfun_t *dp)
{
...
#endif /* SHOPT_FIXEDARRAY */
    do {
        int xfree = (ap->fixed || is_associative(ap))
                    ? 0
                    : array_isbit(aq->bits, aq->cur, ARRAY_NOFREE);
            ...
            if (mp) {
                if (is_associative(ap)) {
                    ...
                    if (!array_covered(np, ( struct index_array * ) ap)) {
                        ...
                        char *data
                        = array_fixed_data(( Namarr_t * ) ap->scope);
    ...
    skip:
        ...
        if (fp = ( struct fixed_array * ) ap->fixed) {
            if (fp->ptr) {
                ...
                cp[fp->curi]
                = ( char * ) (np->nvalue.cp ? np->nvalue.cp : Empty);
            }
        } else
#endif /* SHOPT_FIXEDARRAY */
        if (!is_associative(ap)) {
...
= { sizeof(Namarr_t), array_putval, array_getval, array_getnum, 0, 0,
    array_clone };

static void
array_copytree(Namval_t *np, Namval_t *mp)
{
 ...
 */
static struct index_array *
array_grow(Namval_t *np, struct index_array *arp, int maxi)
{
    ...
    if (maxi >= ARRAY_MAX)
        errormsg(
        SH_DICT, ERROR_exit(1), e_subscript, fmtbase(( long ) maxi, 10, 0));
...
bool
nv_atypeindex(Namval_t *np, const char *tname)
{
    Shell_t *shp = sh_ptr(np);
    Namval_t *tp;
    ...
    tp
    = nv_open(stkptr(shp->stk, offset), shp->var_tree, NV_NOADD | NV_VARNAME);
 ...
 */
static Namarr_t *
nv_changearray(Namval_t *np, void *(*fun)( Namval_t *, const char *, int ) )
{
    Namarr_t *ap;
 ...
 */
Namarr_t *
nv_setarray(Namval_t *np, void *(*fun)( Namval_t *, const char *, int ) )
{
    Namarr_t *ap;
    char *value = 0;
    Namfun_t *fp;
    ...
    if (fun && !ap
        ...
        if (fp || (value && value != Empty)) {
            ...
            if (value)
                ...
                Namval_t *mp
                = ( Namval_t * ) ((*fun)(np, NIL(char *), NV_ACURRENT));
                array_copytree(np, mp);
            }
        }
        return (ap);
 ...
 */
Namval_t *
nv_arraychild(Namval_t *np, Namval_t *nq, int c)
{
    Namfun_t *fp;
    Namarr_t *ap = nv_arrayptr(np);
    union Value *up;
    ...
    if ((fp = nq->nvfun) && fp->disc && fp->disc->setdisc
        && (fp = nv_disc(nq, fp, NV_POP)))
        free(( void * ) fp);
    if (!ap->fun) {
        struct index_array *aq = ( struct index_array * ) ap;
...
nv_nextsub(Namval_t *np)
{
    ...
    for (dot = ap->cur + 1; dot < ( unsigned ) ap->maxi; dot++) {
        ...
        if (aq->val[dot].cp == Empty
            && array_elem(&aq->header) < nv_aimax(np) + 1) {
            ap->cur = dot;
        ...
        if (aq->val[dot].cp) {
            ...
            if (array_isbit(aq->bits, dot, ARRAY_CHILD)) {
                ...
                if ((aq->header.flags & ARRAY_NOCHILD) && nv_isvtree(mp)
                    && !mp->nvfun->dsize)
                    continue;
                if (nv_isarray(mp)) {
 ...
 */
Namval_t *
nv_putsub(Namval_t *np, char *sp, long size, int flags)
{
    Shell_t *shp = sh_ptr(np);
    struct index_array *ap = ( struct index_array * ) nv_arrayptr(np);
#if SHOPT_FIXEDARRAY
...
#endif /* SHOPT_FIXEDARRAY */
    {
        ...
        ap->header.flags
        |= (flags
            & (ARRAY_SCAN | ARRAY_NOCHILD | ARRAY_UNDEF | ARRAY_NOSCOPE));
#if 0
    ...
    if (!(flags & ARRAY_FILL))
        ap->header.flags &= ~ARRAY_SCAN;
    ap->header.flags
    |= (flags & (ARRAY_SCAN | ARRAY_NOCHILD | ARRAY_UNDEF | ARRAY_NOSCOPE));
#if SHOPT_FIXEDARRAY
    if (fp)
        return (np);
    else
    ...
    if (sp) {
        ...
        if (!(flags & (ARRAY_SCAN | ARRAY_ADD))
            && !(*ap->header.fun)(np, NIL(char *), NV_ACURRENT))
            np = 0;
    } else if (flags & ARRAY_SCAN)
...
#if SHOPT_FIXEDARRAY
int
nv_arrfixed(Namval_t *np, Sfio_t *out, int flag, char *dim)
{
    Namarr_t *ap = nv_arrayptr(np);
...
static void
array_fixed_setdata(Namval_t *np, Namarr_t *ap, struct fixed_array *fp)
{
    int n = ap->nelem;
    ap->nelem = 1;
...
static bool
array_fixed_init(Namval_t *np, char *sub, char *cp)
{
    Shell_t *shp = sh_ptr(np);
    Namarr_t *ap;
    struct fixed_array *fp;
    ...
    if ((fp->max[0] = ( int ) sh_arith(shp, ( char * ) sub)) <= 0)
        errormsg(SH_DICT,
                 ERROR_exit(1),
                 "%s: fixed array dimension must be > 0",
                 nv_name(np));
...
static char *
array_fixed(Namval_t *np, char *sub, char *cp, int mode)
{
    Shell_t *shp = sh_ptr(np);
    Namarr_t *ap = nv_arrayptr(np);
    struct fixed_array *fp = ( struct fixed_array * ) ap->fixed;
 ...
 */
char *
nv_endsubscript(Namval_t *np, char *cp, int mode, void *context)
{
...
nv_opensub(Namval_t *np)
{
    ...
    if (ap) {
        if (is_associative(ap))
            return (
            ( Namval_t * ) ((*ap->header.fun)(np, NIL(char *), NV_ACURRENT)));
#if SHOPT_FIXEDARRAY
        else if (!(fp = ( struct fixed_array * ) ap->header.fixed)
                 && array_isbit(ap->bits, ap->cur, ARRAY_CHILD))
#else
...
int
nv_arraynsub(Namarr_t *ap)
{
    return (array_elem(ap));
}

union Value *
nv_aivec(Namval_t *np, unsigned char **bitp)
{
    struct index_array *ap = ( struct index_array * ) nv_arrayptr(np);
 ...
 */
void *
nv_associative(Namval_t *np, const char *sp, int mode)
{
    Shell_t *shp = sh_ptr(np);
    ...
    switch (mode) {
    case NV_AINIT:
        if (ap = ( struct assoc_array * ) calloc(
            1, sizeof(struct assoc_array))) {
            ap->header.table = dtopen(&_Nvdisc, Dtoset);
            dtuserdata(ap->header.table, shp, 1);
            ap->cur = 0;
            ap->pos = 0;
    ...
    case NV_ADELETE:
        if (ap->cur) {
            ...
            if (!scope || scope == ap->header.table
                || !nv_search(ap->cur->nvname, scope, 0))
                ap->header.nelem--;
    ...
    case NV_AFREE:
        ...
        if (ap->header.scope) {
            ...
            if (ap->header.nelem == 0
                && (ap->cur = nv_search("0", ap->header.table, 0)))
    ...
    case NV_ANEXT:
        ...
        for (; ap->cur = ap->pos; ap->pos = ap->nextpos) {
            ...
            if (!nv_isnull(ap->cur)) {
                if ((ap->header.flags & ARRAY_NOCHILD)
    ...
    default:
        if (sp) {
            ...
            if (sp == ( char * ) np)
                return (0);
            type
            = nv_isattr(np, NV_PUBLIC & ~(NV_ARRAY | NV_CHILD | NV_MINIMAL));
            if (mode)
            ...
            if (shp->subshell && (mp = nv_search(sp, ap->header.table, 0))
                && nv_isnull(mp))
                ap->cur = mp;
            if ((mp || (mp = nv_search(sp, ap->header.table, mode)))
                ...
                else if (!ap->header.scope
                         || !nv_search(sp, dtvnext(ap->header.table), 0))
                    ap->header.nelem++;
                if (nv_isnull(mp)) {
                    if (ap->header.flags & ARRAY_TREE)
                        nv_setvtree(mp);
 ...
 */
void
nv_setvec(Namval_t *np, int append, int argc, char *argv[])
{
    int arg0 = 0;
    ...
    while (--argc >= 0) {
        nv_putsub(
        np, NIL(char *), ( long ) argc + arg0, ARRAY_FILL | ARRAY_ADD);
...
#undef nv_putsub
Namval_t *
nv_putsub(Namval_t *np, char *sp, long size)
{
    return (
    nv_putsub_20120720(np, sp, size & ARRAY_MASK, size & ~ARRAY_MASK));
}
