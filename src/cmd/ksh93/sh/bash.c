...
static void
put_globignore(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
...
static void
put_funcname(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
...
int
b_shopt(int argc, char *argv[], Shbltin_t *extra)
{
...
/* mode = 0: init, called two times
    ...
    second time after sh_init() is through and with SH_PREINIT state turned
   off mode > 1: re-init mode < 0: shutdown
*/

void
bash_init(Shell_t *shp, int mode)
{
    Sfio_t *iop;
    Namval_t *np;
    ...
    { /* pre-init stage */
         ...
         * needs to go here because --version option is parsed before the init
         * script.
         */
#if 0 /* This was causing a core dump when running set to display all        \
         variables */
