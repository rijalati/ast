...
static Shnode_t *
makeparent(Lex_t *, int, Shnode_t *);
static Shnode_t *
makelist(Lex_t *, int, Shnode_t *, Shnode_t *);
static struct argnod *
qscan(Lex_t *, struct comnod *, int);
static struct ionod *
inout(Lex_t *, struct ionod *, int);
static Shnode_t *
sh_cmd(Lex_t *, int, int);
static Shnode_t *
term(Lex_t *, int);
static Shnode_t *
list(Lex_t *, int);
...
writedefs(Lex_t *lexp,
          struct argnod *arglist,
          int line,
          int type,
          struct argnod *cmd)
{
...
static void
typeset_order(const char *str, int line)
{
    int c, n = 0;
 ...
 */
static Shnode_t *
makeparent(Lex_t *lp, int flag, Shnode_t *child)
{
...
static Shnode_t *
getanode(Lex_t *lp, struct argnod *ap)
{
 ...
 */
static Shnode_t *
makelist(Lex_t *lexp, int type, Shnode_t *l, Shnode_t *r)
{
    Shnode_t *t;
    if (!l || !r)
        sh_syntax(lexp);
...
void *
sh_parse(Shell_t *shp, Sfio_t *iop, int flag)
{
    Shnode_t *t;
    ...
    flag &= ~SH_FUNEVAL;
    if ((flag & SH_NL)
        && (shp->inlineno = error_info.line + shp->st.firstline) == 0)
        shp->inlineno = 1;
#if KSHELL
    shp->nextprompt = 2;
#endif
...
static Shnode_t *
sh_cmd(Lex_t *lexp, int sym, int flag)
{
    Shnode_t *left, *right;
    int type = FINT | FAMP;
    if (sym == NL)
 ...
 */
static Shnode_t *
list(Lex_t *lexp, int flag)
{
    Shnode_t *t = term(lexp, flag);
    int token;
    while (t && ((token = lexp->token) == ANDFSYM || token == ORFSYM))
        t = makelist(lexp,
                     (token == ANDFSYM ? TAND : TORF),
                     t,
                     term(lexp, SH_NL | SH_SEMI));
    return (t);
}
 ...
 */
static Shnode_t *
arithfor(Lex_t *lexp, Shnode_t *tf)
{
    Shnode_t *t, *tw = tf;
    int offset;
    struct argnod *argp;
    ...
    for (n = 0;; n++) {
        ...
        while (offset > ARGVAL
               && ((c = *stkptr(stkp, offset - 1)), isspace(c)))
            offset--;
    ...
    if (n != DOSYM && n != LBRACE)
        sh_syntax(lexp);
    tw->wh.dotre
    = sh_cmd(lexp, n == DOSYM ? DONESYM : RBRACE, SH_NL | SH_SEMI);
    tw->wh.whtyp = TWH;
    return (tf);
}
...
funct(Lex_t *lexp)
{
    ...
    if (!shp->st.filename || sffileno(iop) < 0) {
        ...
        if (sh_isstate(shp, SH_HISTORY) && shp->gd->hist_ptr)
            ...
            if (lexp->sh->heredocs)
                t->funct.functloc
                = sfseek(lexp->sh->heredocs, ( Sfoff_t ) 0, SEEK_END);
            else
    ...
    if (t->funct.functtyp & FPOSIX)
        ...
        if (lexp->token == 0) {
            ...
            char *cp, **argv, **argv0;
            int c;
            t->funct.functargs = ac = ( struct comnod * ) simple(
            lexp, SH_NOIO | SH_FUNDEF, NIL(struct ionod *));
            ...
            if (shp->shcomp
                ...
                np->nvalue.rp->argc
                = (( struct dolnod * ) ac->comarg)->dolnum;
            }
        }
        while (lexp->token == NL)
            lexp->token = sh_lex(lexp);
    ...
    if (jmpval == 0) {
        ...
        if (size) {
            ...
            char *cp, *sp, **argv,
            **old
            = (( struct dolnod * ) t->funct.functargs->comarg)->dolval + 1;
 ...
 */
static struct argnod *
assign(Lex_t *lexp, struct argnod *ap, int type)
{
    int n;
    Shnode_t *t, **tp;
    struct comnod *ac;
...
item(Lex_t *lexp, int flag)
{
    ...
    case SELECTSYM: {
        ...
        if (lexp->kiafile)
            writedefs(
            lexp, lexp->arg, lexp->sh->inlineno, 'v', NIL(struct argnod *));
#endif /* SHOPT_KIA */
        ...
        loop_level++;
        t->for_.fortre
        = sh_cmd(lexp, tok == DOSYM ? DONESYM : RBRACE, SH_NL | SH_SEMI);
        if (--loop_level == 0)
            label_last = label_list;
        break;
    }
    ...
    case LABLSYM: {
        ...
        if (sh_isoption(lexp->sh, SH_NOEXEC) && tok != TWH && tok != TUN
            && tok != TFOR && tok != TSELECT)
            errormsg(SH_DICT,
                     ERROR_warn(0),
                     e_lexlabignore,
                     label_list->argchn.len,
                     label_list->argval);
        return (t);
    }
...
static struct argnod *
process_sub(Lex_t *lexp, int tok)
{
    struct argnod *argp;
    Shnode_t *t;
    ...
    *argp->argval = 0;
    argp->argchn.ap = ( struct argnod * ) makeparent(
    lexp, mode ? TFORK | FPIN | FAMP | FPCL : TFORK | FPOU, t);
 ...
 */
static Shnode_t *
simple(Lex_t *lexp, int flag, struct ionod *io)
{
    struct comnod *t;
    struct argnod *argp;
    int tok;
    ...
    *argtail = 0;
    ...
    if (lexp->kiafile && !(flag & SH_NOIO)) {
        ...
        if (np)
            r = kiaentity(
            lexp, nv_name(np), -1, 'p', -1, 0, lexp->unknown, 'b', 0, "");
        else if (argp)
            r = kiaentity(
            lexp, sh_argstr(argp), -1, 'p', -1, 0, lexp->unknown, 'c', 0, "");
        if (r > 0)
            sfprintf(lexp->kiatmp,
                     "p;%..64d;p;%..64d;%d;%d;c;\n",
                     lexp->current,
 ...
 */
static struct ionod *
inout(Lex_t *lexp, struct ionod *lastio, int flag)
{
...
static struct argnod *
qscan(Lex_t *lp, struct comnod *ac, int argn)
{
    char **cp;
    struct argnod *ap;
    struct dolnod *dp;
...
test_primary(Lex_t *lexp)
{
    ...
    case '(':
        ...
        t = makelist(lexp,
                     TTST | TTEST | TPAREN,
                     t,
                     ( Shnode_t * ) pointerof(lexp->sh->inlineno));
        break;
    case '!':
...
#    ifdef SF_BUFCONST
        if (off2 == off1)
            n = sfprintf(lexp->kiafile,
                         "DIRECTORY\nENTITY;%lld;%d\nDIRECTORY;",
                         ( Sflong_t ) lexp->kiabegin,
                         (size_t)(off1 - lexp->kiabegin));
        else
            n = sfprintf(
            lexp->kiafile,
            "DIRECTORY\nENTITY;%lld;%d\nRELATIONSHIP;%lld;%d\nDIRECTORY;",
