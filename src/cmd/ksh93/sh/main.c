...
static void
exfile(Shell_t *, Sfio_t *, int);
static void
chkmail(Shell_t *shp, char *);
...
bool
sh_source(Shell_t *shp, Sfio_t *iop, const char *file)
{
    char *oid;
    char *nid;
    int fd;
...
int
sh_main(int ac, char *av[], Shinit_f userinit)
{
    int fdin = 0;
    char *name;
    Sfio_t *iop;
    ...
    if ((beenhere++) == 0) {
        ...
        if (!sh_isoption(shp, SH_INTERACTIVE) && !sh_isoption(shp, SH_TFLAG)
            && !sh_isoption(shp, SH_CFLAG) && sh_isoption(shp, SH_SFLAG)
        ...
        if (sh_isoption(shp, SH_LOGIN_SHELL)) {
            ...
            if (!sh_isoption(shp, SH_NOUSRPROFILE)
                && !sh_isoption(shp, SH_PRIVILEGED)) {
                char **files = shp->gd->login_files;
                while ((name = *files++)
                       && !sh_source(shp, iop, sh_mactry(shp, name)))
                    ;
            }
        }
            ...
            if (sh_isoption(shp, SH_SFLAG))
                ...
                if (strmatch(name, e_devfdNN)) {
                    ...
                    if (((type = sh_type(cp = av[0])) & SH_TYPE_SH)
                        ...
                        av[0]
                        = (type & SH_TYPE_LOGIN) ? cp : path_basename(cp);
...
static void
exfile(Shell_t *shp, Sfio_t *iop, int fno)
{
    time_t curtime;
    Shnode_t *t;
    ...
    if (sh_isstate(shp, SH_INTERACTIVE)) {
        if (nv_isnull(PS1NOD))
            nv_putval(PS1NOD,
                      (shp->gd->euserid ? e_stdprompt : e_supprompt),
                      NV_RDONLY);
        sh_sigdone(shp);
    ...
    if (jmpval) {
        ...
        if (sh_isstate(shp, SH_PROFILE)
            && (jmpval == SH_JMPFUN || jmpval == SH_JMPEXIT)) {
...
static void
chkmail(Shell_t *shp, char *files)
{
    char *cp, *sp, *qp;
    char save;
            ...
            if (stat(cp, &statb) >= 0 && statb.st_mtime >= mailtime
                ...
                if (!arglist && S_ISDIR(statb.st_mode)) {
                    ...
                    if (statb.st_size
                        ...
                        SH_DICT,
                        0,
                        sh_mactry(shp, qp ? qp + 1 : ( char * ) e_mailmsg));
                        shp->lastarg = save;
                    }
                    lastmail = statb;
                    break;
