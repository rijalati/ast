...
static Dt_t *Refdict;
static Dtdisc_t _Refdisc = { offsetof(struct Namref, np),
                             sizeof(struct Namval_t *),
...
static void
attstore(Namval_t *, void *);
#ifndef _ENV_H
static void
pushnam(Namval_t *, void *);
static char *
staknam(Shell_t *, Namval_t *, char *);
#endif
static void
rightjust(char *, int, int);
static char *
lastdot(char *, int, void *);
...
#ifdef _ENV_H
void
sh_envput(Shell_t *shp, Namval_t *np)
{
    Env_t *ep = shp->env;
 ...
 */
void
sh_outname(Shell_t *shp, Sfio_t *out, char *name, int len)
{
...
nv_addnode(Namval_t *np, int remove)
{
    ...
    if (sp->rp && sp->numnodes) {
        ...
        if (name && np->nvname[i] == '.' && np->nvname[i + 1] == '_'
            && np->nvname[i + 2] == 0)
            sp->rp = 0;
        else {
            Dt_t *root = nv_dict(shp->last_table);
 ...
 */
struct argnod *
nv_onlist(struct argnod *arg, const char *name)
{
    char *cp;
    int len = strlen(name);
 ...
 */
Namval_t **
sh_setlist(Shell_t *shp, struct argnod *arg, int flags, Namval_t *typ)
{
    char *cp;
    Namval_t *np, *mp;
    ...
    for (; arg; arg = arg->argnxt.ap) {
        ...
        if (arg->argflag & ARG_MAC) {
            ...
            if (*arg->argval == 0 && arg->argchn.ap
                ...
                if (!array && tp->tre.tretyp != TLST && tp->com.comset
                    && !tp->com.comarg && tp->com.comset->argval[0] == 0
                    && tp->com.comset->argval[1] == '[')
                    array |= (tp->com.comset->argflag & ARG_MESSAGE)
                             ? NV_IARRAY
                             : NV_ARRAY;
                ...
                if ((arg->argflag & ARG_APPEND)
                    && (tp->tre.tretyp & COMMSK) == TCOM && tp->com.comset
                    && !nv_isvtree(np)
                    && (((ap = nv_arrayptr(np)) && !ap->fun && !nv_opensub(np))
                        || (!ap && nv_isarray(np) && tp->com.comarg
                            && !((mp = nv_search(
                                  tp->com.comarg->argval, shp->fun_tree, 0))
                ...
                if (nv_isattr(np, NV_RDONLY) && np->nvfun
                    && !(flags & NV_RDONLY))
                ...
                if (nv_istable(np))
                    _nv_unset(np, 0);
                if (typ && !array
                    && (!shp->prefix || nv_isnull(np) || nv_isarray(np))) {
                    if (!(nv_isnull(np)) && !nv_isarray(np))
                        _nv_unset(np, 0);
                    nv_settype(np, typ, 0);
                }
                if ((flags & NV_STATIC) && !nv_isattr(np, NV_EXPORT)
                    && !nv_isnull(np))
                    goto check_type;
                ap = nv_arrayptr(np);
#if SHOPT_FIXEDARRAY
                ...
                if (sh_isoption(shp, SH_BASH) && !array && !ap
                    && !(flags & NV_COMVAR) && !np->nvfun && !tp->com.comset
                    && !tp->com.comarg)
                    array = NV_IARRAY;
                ...
                if ((tp->tre.tretyp & COMMSK) == TCOM && tp->com.comarg
                    && !tp->com.comset
                    && ((array & NV_IARRAY)
                        || !((mp = tp->com.comnamp)
                             && nv_isattr(mp, BLT_DCL)))) {
...
#if SHOPT_FIXEDARRAY
                        if (!nv_isarray(np)
                            || ((ap = nv_arrayptr(np)) && !ap->fixed
                                && (ap->nelem)))
#else
                        if (!nv_isarray(np)
                            || ((ap = nv_arrayptr(np)) && (ap->nelem)))
#endif /* SHOPT_FIXEDARRAY */
                        {
                            if (ap)
                ...
                if (tp->tre.tretyp == 0 && !tp->com.comset
                    ...
                    if (!(arg->argflag & ARG_APPEND)) {
                        if (ap && ap->nelem > 0) {
                            ...
                            if (!ap->fun && !(ap->flags & ARRAY_TREE)
                                && !np->nvfun->next && !nv_type(np)) {
                ...
                if (tp->tre.tretyp == TLST || !tp->com.comset
                    ...
                    if (tp->tre.tretyp != TLST && !tp->com.comnamp
                        && tp->com.comset && tp->com.comset->argval[0] == 0
                        && tp->com.comset->argchn.ap) {
                        if (prefix || nv_name(np))
                        ...
                        shp->typeinit = 0;
                        sh_setlist(
                        shp, tp->com.comset, flags & ~NV_STATIC, 0);
                        shp->prefix = prefix;
                        if (tp->com.comset->argval[1] != '[')
                            nv_setvtree(np);
                        nv_close(np);
                    ...
                    if (*cp != '.' && *cp != '[' && strchr(cp, '[')) {
                        ...
                        np = nv_open(cp,
                                     shp->prefix_root ? shp->prefix_root
                                                      : shp->var_tree,
                                     flag);
                    }
                    if (arg->argflag & ARG_APPEND) {
                        if (nv_isarray(np)) {
                            ...
                            if (nv_type(np)) {
                                sfprintf(
                                shp->strbuf, "%s[%d]\0", nv_name(np), sub);
                                nq = nv_open(sfstruse(shp->strbuf),
                                             shp->var_tree,
                                             flags | NV_ARRAY);
                            }
                        }
                        if (!nv_isnull(np) && np->nvalue.cp != Empty
                            ...
                            sub = 1;
                               ...
                               && !nv_type(np)
                               && nv_isattr(np, NV_MINIMAL | NV_EXPORT)
                                  != NV_MINIMAL) {
                        _nv_unset(np, NV_EXPORT);
                        if (ap && ap->fun) {
                            nv_setarray(np, nv_associative);
                        } else {
                            // nq is initialized to same value as np. When
                            // _nv_unset(np, NV_EXPORT); is called, it free's
                            // memory which is later causing crash at: if (nq
                            // && nv_type(nq)) nv_checkrequired(nq); Reset nq
                            // to 0 to avoid such crashes.
                            nq = 0;
                        }
                    }
                } else {
            ...
            skip:
                ...
                shp->last_table = 0;
                if (shp->prefix) {
                    if (*shp->prefix == '_' && shp->prefix[1] == '.'
                        && nv_isref(L_ARGNOD)) {
                        sfprintf(stkstd,
                                 "%s%s",
                                 nv_name(L_ARGNOD->nvalue.nrp->np),
                                 shp->prefix + 1);
        ...
        np = nv_open(
        cp, shp->prefix_root ? shp->prefix_root : shp->var_tree, flags);
        if (nlp)
            *nlp++ = np;
 ...
 */
static void
stak_subscript(const char *sub, int last)
{
    int c;
    stakputc('[');
    while (c = *sub++) {
 ...
 */
static char *
copystack(Shell_t *shp, const char *prefix, const char *name, const char *sub)
{
...
nv_create(const char *name, Dt_t *root, int flags, Namfun_t *dp)
{
    ...
    if (*cp == '.')
        cp++;
    while (1) {
        // We have to use `memmove()` rather than `strcpy()` because the
        // buffers may overlap.
        if (zerosub && !np)
            ...
            if (top) {
                ...
                if (np = nv_search(name, shp->var_tree, 0)) {
                    ...
                    if (shp->var_tree->walk == shp->var_base
                        || (shp->var_tree->walk != shp->var_tree
                            && shp->namespace
...
#if SHOPT_NAMESPACE
            if (!np && !noscope && *name != '.' && shp->namespace
                && root == shp->var_tree)
...
#if SHOPT_NAMESPACE
                            if (shp->namespace
                                && nv_dict(shp->namespace) == shp->var_tree
 ...
 */
void
nv_delete(Namval_t *np, Dt_t *root, int flags)
{
#if NVCACHE
    int c;
    struct Cache_entry *xp;
    for (c = 0, xp = nvcache.entries; c < NVCACHE;
         xp = &nvcache.entries[++c]) {
        if (xp->np == np)
            xp->root = 0;
    }
#endif
...
nv_open(const char *name, Dt_t *root, int flags)
{
    ...
    for (c = 0, xp = nvcache.entries; c < NVCACHE;
        ...
        if ((*name != '_' || name[1] != '.') && *name == *xp->name
            ...
            && (flags & (NV_ARRAY | NV_NOSCOPE)) == xp->flags
            && memcmp(xp->name, name, xp->len) == 0
            && (name[xp->len] == 0 || name[xp->len] == '='
                || name[xp->len] == '+')) {
    ...
    if (root == shp->fun_tree && sh_isoption(shp, SH_BASH)) {
        c = ((flags & NV_NOSCOPE) ? HASH_NOSCOPE : 0)
            | ((flags & NV_NOADD) ? 0 : NV_ADD);
...
skip:
    ...
    if (c == '=' && np && (flags & NV_ASSIGN)) {
        ...
        if (sh_isstate(shp, SH_INIT)) {
            ...
            isref = nv_isref(np);
#if SHOPT_FIXEDARRAY
            if (sh_isoption(shp, SH_XTRACE) && (ap = nv_arrayptr(np))
                && !ap->fixed)
#else
                    ...
                    if (mp = nv_opensub(np))
                        np = mp;
                             ...
                             && (mp
                                 = nv_open(cp,
                                           shp->var_tree,
                                           NV_NOFAIL | NV_VARNAME | NV_NOARRAY
 ...
 */
void
nv_putval(Namval_t *np, const char *string, int flags)
{
    Shell_t *shp = sh_ptr(np);
    ...
    up = &np->nvalue;
#if SHOPT_FIXEDARRAY
    if (np->nvalue.up && nv_isarray(np) && (ap = nv_arrayptr(np))
        && !ap->fixed)
#else
 ...
 */
static void
rightjust(char *str, int size, int fill)
{
    int n;
    char *cp, *sp;
    n = strlen(str);
...
static int
ja_size(char *str, int size, int type)
{
    char *cp = str;
    int c, n = size;
    int outsize;
...
#ifndef _ENV_H
static char *
staknam(Shell_t *shp, Namval_t *np, char *value)
{
    char *p, *q;
    q = stkalloc(shp->stk,
                 strlen(nv_name(np)) + (value ? strlen(value) : 0) + 2);
...
attstore(Namval_t *np, void *data)
{
    ...
    if (!(nv_isattr(np, NV_EXPORT)))
        return;
    flag = nv_isattr(np,
                     NV_RDONLY | NV_UTOL | NV_LTOU | NV_RJUST | NV_LJUST
                     | NV_ZFILL | NV_INTEGER);
    sfputc(shp->stk, '=');
...
attstore(Namval_t *np, void *data)
{
    ...
    flag &= (NV_RDONLY | NV_UTOL | NV_LTOU | NV_RJUST | NV_LJUST | NV_ZFILL
             | NV_INTEGER);
    *ap->attval++ = '=';
    if ((flag & NV_DOUBLE) == NV_DOUBLE) {
        /* export doubles as integers for ksh88 compatibility */
...
scanfilter(Dt_t *dict, void *arg, void *data)
{
    ...
    if (sp->scanmask ? (k & sp->scanmask) == sp->scanflags
        ...
        if (tp && tp->mapname) {
            if (sp->scanflags == NV_FUNCTION
                || sp->scanflags == (NV_NOFREE | NV_BINARY | NV_RAW)) {
 ...
 */
void
sh_scope(Shell_t *shp, struct argnod *envlist, int fun)
{
    Dt_t *newscope,
    *newroot = (sh_isoption(shp, SH_BASH) ? shp->var_tree : shp->var_base);
    struct Ufunction *rp;
#if SHOPT_NAMESPACE
    if (shp->namespace)
 ...
 */
void
_nv_unset(Namval_t *np, int flags)
{
    Shell_t *shp = sh_ptr(np);
    union Value *up;
#if SHOPT_FIXEDARRAY
...
#if SHOPT_FIXEDARRAY
    else if (np->nvalue.up && nv_isarray(np) && (ap = nv_arrayptr(np))
             && !ap->fixed)
#else
    else if (np->nvalue.up && nv_isarray(np) && nv_arrayptr(np))
#endif /* SHOPT_FIXEDARRAY */
 ...
 */
Namval_t *
sh_scoped(Shell_t *shp, Namval_t *np)
{
...
static void
put_optimize(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
...
= { sizeof(struct optimize), put_optimize, 0, 0, 0, 0, clone_optimize };
...
#endif
    if (!nv_isattr(np, NV_LJUST | NV_RJUST) && (numeric = nv_size(np))
        && up->cp && up->cp[numeric]) {
...
nv_getnum(Namval_t *np)
{
    ...
    if (nv_isref(np)) {
        ...
        np = nv_refnode(np);
        if (str)
            nv_putsub(np, str, 0L, 0);
    }
    if (nv_isattr(np, NV_INTEGER)) {
        if (sh_isoption(shp, SH_NOUNSET)
            && nv_isattr(np, NV_NOTSET) == NV_NOTSET) {
            int i = nv_aindex(np);
 ...
 */
void
nv_newattr(Namval_t *np, unsigned newatts, int size)
{
    Shell_t *shp = sh_ptr(np);
    char *sp;
    char *cp = 0;
    ...
    oldsize = nv_size(np);
    if ((size == oldsize || (n & NV_INTEGER)) && !trans
        && ((n ^ newatts) & ~NV_NOCHANGE) == 0) {
        if (size)
            nv_setsize(np, size);
        nv_offattr(np, ~NV_NOFREE);
...
sh_getenv(const char *name)
{
    ...
    } else if ((np = nv_search(name, shp->var_tree, 0))
 ...
 */
int
putenv(const char *name)
{
    Shell_t *shp = sh_getinterp();
    Namval_t *np;
    if (name) {
        np = nv_open(
        name, shp->var_tree, NV_EXPORT | NV_IDENT | NV_NOARRAY | NV_ASSIGN);
        if (!strchr(name, '='))
            _nv_unset(np, 0);
    }
    return (0);
 ...
 */
static char *
lastdot(char *cp, int eq, void *context)
{
    char *ep = 0;
    int c;
    if (eq)
...
bool
nv_rename(Namval_t *np, int flags)
{
    Shell_t *shp = sh_ptr(np);
    Namval_t *mp = 0, *nr = 0;
    char *cp;
    ...
    if (!nv_isattr(np, NV_MINIMAL))
        nvenv = np->nvenv;
    if (nvenv
        || (cp = nv_name(np)) && nv_isarray(np) && cp[strlen(cp) - 1] == ']')
        arraynp = 1;
    ...
    if (!hp)
        hp = shp->var_tree;
    if (!(nr = nv_open(
          cp, hp, flags | NV_ARRAY | NV_NOSCOPE | NV_NOADD | NV_NOFAIL))) {
#if SHOPT_NAMESPACE
        if (shp->namespace)
    ...
    if (!nr)
        nr = nv_open(
        cp, hp, flags | NV_NOREF | ((flags & NV_MOVE) ? 0 : NV_NOFAIL));
    shp->prefix = prefix;
    ...
    if (!mp && index >= 0 && nv_isvtree(nr)) {
        ...
        if (mp = nv_open(sfstruse(shp->strbuf),
                         shp->var_tree,
 ...
 */
void
nv_setref(Namval_t *np, Dt_t *hp, int flags)
{
    Shell_t *shp = sh_ptr(np);
    Namval_t *nq = 0, *nr = 0;
    char *ep, *cp;
    ...
    if ((nr = nv_open(cp,
                      hp ? hp : shp->var_tree,
                      flags | NV_NOSCOPE | NV_NOADD | NV_NOFAIL)))
        nq = nr;
    else if (hpnext && dtvnext(hpnext) == shp->var_base
             && (nr = nv_open(
                 cp, hpnext, flags | NV_NOSCOPE | NV_NOADD | NV_NOFAIL)))
        nq = nr;
    ...
    if (nq)
        ...
        if (hp && (xp = strchr(cp, '.'))) {
            ...
            *xp = '.';
            if (nr && nv_isvtree(nr)
                && (nr = nv_open(cp, hp, flags | NV_NOSCOPE | NV_NOFAIL)))
                nq = nr;
            else
                nr = 0;
        }
...
#if SHOPT_FIXEDARRAY
    if (nq && ep && nv_isarray(nq) && !((ap = nv_arrayptr(nq)) && ap->fixed)
        && !nv_getsub(nq))
#else
 ...
 */
Shscope_t *
sh_getscope_20120720(Shell_t *shp, int index, int whence)
{
...
#undef sh_getscope
Shscope_t *
sh_getscope(int index, int whence)
{
    Shell_t *shp = sh_getinterp();
...
#undef sh_setscope
Shscope_t *
sh_setscope(Shscope_t *scope)
{
    return (sh_setscope_20120720(sh_getinterp(), scope));
}
...
nv_name(Namval_t *np)
{
    ...
    if (!nv_isattr(np, NV_MINIMAL | NV_EXPORT) && np->nvenv) {
        ...
        if (mp && mp->nvname == 0 || *mp->nvname == 0)
            mp = 0;
        if (mp
            && (!strchr(np->nvname, '.') || (np->nvfun && nv_type(mp))
                || (nv_isarray(mp) && (cp = nv_getsub(mp))
                    && strcmp(np->nvname, cp) == 0))) {
    ...
    if (!(table = shp->last_table) || *np->nvname == '.'
...
#undef nv_setsize
int
nv_setsize(Namval_t *np, int size)
{
