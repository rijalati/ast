...
static Namfun_t *
clone_type(Namval_t *, Namval_t *, int, Namfun_t *);
static Namval_t *
next_type(Namval_t *, Dt_t *, Namfun_t *);

static const Namdisc_t type_disc = {
    sizeof(Namtype_t), put_type,   0, 0,         0,
    create_type,       clone_type, 0, next_type, 0,
#if 0
	read_type
#endif
};
...
size_t
nv_datasize(Namval_t *np, size_t *offset)
{
    size_t s = 0, a = 0;
    ...
    } else if (nv_isattr(np, NV_BINARY)
...
name_chtype(Namval_t *np, Namfun_t *fp)
{
    ...
    if (nv_isattr(np, NV_REF | NV_TAGGED) == (NV_REF | NV_TAGGED))
        shp->last_table = 0;
    cp = nv_name(nq);
    if ((ap = nv_arrayptr(nq)) && !(ap->flags & ARRAY_UNDEF)
        && (sub = nv_getsub(nq)))
...
static void
put_chtype(Namval_t *np, const char *val, int flag, Namfun_t *fp)
{
...
static Namfun_t *
clone_chtype(Namval_t *np, Namval_t *mp, int flags, Namfun_t *fp)
{
    if (flags & NV_NODISC)
        return (0);
...
create_chtype(Namval_t *np, const char *name, int flag, Namfun_t *fp)
{
    ...
    for (fp = pp->nvfun; fp; fp = fp->next) {
        if (fp->disc
            && (fp->disc->createf == create_type
                || fp->disc->createf == create_chtype)) {
            sfprintf(shp->strbuf, "%s.%s", np->nvname, name);
            if (nq
                = (*fp->disc->createf)(pp, sfstruse(shp->strbuf), flag, fp))
                xp->fun.last = fp->last;
            return (nq);
        }
    }
...
= { sizeof(Namchld_t), put_chtype,   0,          0, 0,
    create_chtype,     clone_chtype, name_chtype };

static Namval_t *
findref(void *nodes, int n)
{
...
static int
fixnode(Namtype_t *dp, Namtype_t *pp, int i, struct Namref *nrp, int flag)
{
...
clone_type(Namval_t *np, Namval_t *mp, int flags, Namfun_t *fp)
{
    ...
    for (i = dp->numnodes; --i >= 0;) {
        ...
        if (nq->nvalue.cp || !nv_isvtree(nq) || nv_isattr(nq, NV_RDONLY)) {
            ...
            fp->nofree = 1;
            nr = nv_create(
            stkptr(shp->stk, offset), root, NV_VARNAME | NV_NOADD, fp);
            fp->nofree = save;
...
create_type(Namval_t *np, const char *name, int flag, Namfun_t *fp)
{
    ...
    if (dp->numnodes && dp->strsize < 0) {
        ...
        while (i <= dp->numnodes && (nq = nv_namptr(base, ++i))
            ...
            if (nq->nvname[m] == '.'
                && strncmp(name, &nq->nvname[m + 1], n) == 0
                && nq->nvname[m + n + 1] == 0)
                goto found;
        }
        nq = 0;
    ...
    if (nq) {
        ...
        if (name[n] != '=')
            for (i = 0; i < dp->ndisc; i++) {
                if ((strncmp(name, dp->names[i], n) == 0)
                    && dp->names[i][n] == 0)
                    return (nq);
            }
        if (!(flag & NV_NOFAIL))
...
next_type(Namval_t *np, Dt_t *root, Namfun_t *fp)
{
    ...
    if (!root) {
        ...
        if ((ap = nv_arrayptr(dp->parent)) && !ap->fun
            && (ap->flags & ARRAY_SCAN) && nv_nextsub(dp->parent))
            dp->current = 0;
        else
            return (0);
    }
...
static int
typeinfo(Opt_t *op, Sfio_t *out, const char *str, Optdisc_t *od)
{
    ...
    np = nv_open(
    cp = stkptr(shp->stk, offset), shp->var_tree, NV_NOADD | NV_VARNAME);
    stkseek(shp->stk, offset);
    if (!np) {
        ...
        if (bp = ( Nambfun_t * ) nv_hasdisc(np, nv_discfun(NV_DCADD))) {
            for (i = 0; i < bp->num; i++) {
                if (nv_isattr(bp->bltins[i], NV_OPTGET))
                    sfprintf(
                    out, "\b%s.%s\b(3), ", np->nvname, bp->bnames[i]);
            }
        }
        return (0);
    }
    ...
    for (i = 0; i < dp->numnodes; i++) {
        ...
        if (tp = nv_type(nq)) {
            ...
            sfprintf(out, "\t[+%s?%s.\n", nq->nvname, tp->nvname);
            n = strlen(nq->nvname);
            while ((cp = nv_namptr(dp->nodes, i + 1)->nvname)
            ...
            if (nq->nvalue.cp == Empty && nv_isattr(nq, NV_RDONLY))
                sfprintf(out,
                         "\t[+%s?%s, default value is required at creation\n",
                         nq->nvname,
    ...
    if (dp->ndisc > 0) {
        ...
        n = stktell(shp->stk);
        sfprintf(out,
                 "[+?\b%s\b defines the following discipline functions:]{\n",
                 np->nvname);
        for (i = 0; i < dp->ndisc; i++) {
            char *dot;
...
static int
std_disc(Namval_t *mp, Namtype_t *pp)
{
 ...
 */
Namval_t *
nv_mktype(Namval_t **nodes, int numnodes)
{
    ...
    for (nnodes = 1, i = 1; i < numnodes; i++) {
        ...
        nnodes++;
        if (name && strncmp(&name[m], &np->nvname[m], n) == 0
            && np->nvname[m + n] == '.')
            offset -= sizeof(char *);
        ...
        if (!nv_isarray(np)
            ...
            if ((n = dp->strsize) < 0)
                n = -n;
            iref = nref += dp->nref;
            if (np->nvname[m] == '_' && np->nvname[m + 1] == 0
                && (bp = nv_type(np))) {
                qp = dp;
                nd = dp->ndisc;
                nnodes = dp->numnodes;
    ...
    *cp++ = 0;
    ...
    for (name = 0, offset = 0, k = i = 1; i < numnodes; i++) {
        ...
        if (strcmp(&np->nvname[m], NV_DATA) == 0 && !nv_type(np)) {
            ...
            nv_putval(nq,
                      (val ? val : 0),
                      nv_isattr(np, ~(NV_IMPORT | NV_EXPORT | NV_ARRAY)));
...
void
nv_typename(Namval_t *tp, Sfio_t *out)
{
    char *v, *cp;
    Namtype_t *dp;
    cp = nv_name(tp);
 ...
 */
int
nv_settype(Namval_t *np, Namval_t *tp, int flags)
{
...
nv_mkstruct(const char *name, int rsize, Fields_t *fields, void *context)
{
    ...
    for (fp = fields; fp->name; fp++) {
        ...
        if (strncmp(fp->type, "typeset", 7)) {
            ...
            tp = nv_open(stkptr(shp->stk, offset),
                         shp->var_tree,
...
static void
write_indent(Sfio_t *out, char *str, int n, int indent)
{
    int c, first = 1;
    char *cp = str;
    while (n-- && (c = *str++)) {
...
int
sh_outtype(Shell_t *shp, Sfio_t *out)
{
    Namval_t node, *mp, *tp;
    Dt_t *dp;
    ...
    for (tp = ( Namval_t * ) dtfirst(dp); tp;
        ...
        if (nv_isnull(tp) || !nv_isvtree(tp))
            continue;
        if (indent
            && (strncmp(tp->nvname, shp->prefix, n - 1)
                || tp->nvname[n - 1] != '.' || strchr(tp->nvname + n, '.')))
            continue;
