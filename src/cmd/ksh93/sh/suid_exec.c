...
static void
error_exit(const char *);
static int
in_dir(const char *, const char *);
static int
endsh(const char *);
...
main(int argc, char *argv[])
{
    ...
    if (fstat(n, &statb) < 0 || !S_ISREG(statb.st_mode))
        error_exit(badopen);
    if (stat(p, &statx) < 0 || statb.st_ino != statx.st_ino
...
exec:
    ...
    if (shell == 0 || !endsh(shell)
        || (!in_dir("/bin", shell) && !in_dir("/usr/bin", shell)
...
#ifdef _lib_setreuid
static void
setids(int mode, int owner, int group)
{
    if (mode & S_ISGID)
...
static void
setids(int mode, uid_t owner, gid_t group)
{
    int n, m;
    int pv[2];
...
#    ifdef O_EXCL
    if ((n = open(tmpname, O_WRONLY | O_CREAT | O_EXCL, SPECIAL)) < 0
        || unlink(tmpname) < 0)
#    else
    if ((n = open(tmpname, O_WRONLY | O_CREAT, SPECIAL)) < 0
        || unlink(tmpname) < 0)
#    endif
        error_exit(badexec);
    if (n != FDVERIFY)
...
#    ifdef O_EXCL
            if ((n
                 = open(tmpname, O_WRONLY | O_CREAT | O_TRUNC | O_EXCL, mode))
                < 0)
#    else
            unlink(tmpname);
