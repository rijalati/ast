...
static int
job_chksave(pid_t, long);
static struct process *job_bypid(pid_t);
static struct process *
job_byjid(int);
static char *
job_sigmsg(Shell_t *, int);
static int
job_alloc(Shell_t *);
static void
job_free(int);
static struct process *
job_unpost(Shell_t *, struct process *, int);
static void
job_unlink(struct process *);
static void
job_prmsg(Shell_t *, struct process *);
...
#ifdef JOBS
static void
job_set(struct process *);
static void
job_reset(struct process *);
static struct process *
job_byname(char *);
static struct process *
job_bystring(char *);
...
#ifdef SIGTSTP
static void
job_unstop(struct process *);
static void
job_fgrp(struct process *, int);
...
static void
setsiginfo(Shell_t *shp, siginfo_t *info, struct process *pw)
{
...
void
job_chldtrap(Shell_t *shp, const char *trap, int lock)
{
...
char *
sh_pid2str(Shell_t *shp, pid_t pid)
{
    struct cosh *csp = 0;
    if (pid & COPID_BIT) {
        int id = (pid >> 16) & 0x3f;
...
int
job_cowalk(int (*fun)(struct process *, int), int arg, char *name)
{
    ...
    for (n = 0, pw = job.pwlist; pw; pw = pwnext) {
        pwnext = pw->p_nxtjob;
        if ((cp && val == pw->p_pid)
            || (pw->p_cojob && pw->p_cojob->local == ( void * ) csp)) {
            if (fun) {
                if (pw->p_flag & P_DONE)
                    continue;
                r |= (*fun)(pw, arg);
...
#    ifdef _lib_sigaction
static void
job_waitsafe(int sig, siginfo_t *info, void *context)
#    else
static void
job_waitsafe(int sig)
#    endif
{
...
#    ifdef _lib_sigaction
        if (shp->st.trapcom[SIGCHLD]
            && (WIFSTOPPED(wstat) || WIFCONTINUED(wstat)))
...
#    ifdef DEBUG
            sfprintf(sfstderr,
                     "ksh: job line %4d: reap pid=%d critical=%d unknown job "
                     "pid=%d pw=%x\n",
                     __LINE__,
                     getpid(),
                     job.in_critical,
...
#    endif /* SIGTSTP */
        {
            ...
            if (WIFSIGNALED(wstat)) {
                 ...
                 */
                if (pw->p_pgrp && pw->p_pgrp == job.curpgid
                    && pw->p_exit == SIGINT && sh_isstate(shp, SH_STOPOK)) {
                    pw->p_flag &= ~P_NOTIFY;
...
#    ifdef DEBUG
        sfprintf(sfstderr,
                 "ksh: job line %4d: reap pid=%d critical=%d job %d with pid "
                 "%d flags=%o complete with status=%x exit=%d\n",
                 __LINE__,
                 getpid(),
                 job.in_critical,
                 pw->p_job,
...
#    endif /* DEBUG*/
        ...
        if (pid == pw->p_fgrp && pid == tcgetpgrp(JOBTTY)) {
            ...
            for (; px && (px->p_flag & P_DONE); px = px->p_nxtproc)
                ;
            if (!px && sh_isoption(shp, SH_INTERACTIVE))
                tcsetpgrp(JOBTTY, job.mypid);
        }
        if (!shp->st.trapcom[SIGCHLD] && (pw->p_flag & P_DONE)
            && (pw->p_flag & P_BG) && !(pw->p_flag & P_NOTIFY)) {
            pw->p_flag &= ~P_BG;
            if (pw != pwfg)
                job_unpost(shp, pw, 1);
        }
 ...
 */
void
job_init(Shell_t *shp, int lflag)
{
    int ntry = 0;
    job.fd = JOBTTY;
...
job_close(Shell_t *shp)
{
    ...
    if (possible && !job.jobcontrol)
        return (0);
             ...
             && (!sh_isstate(shp, SH_MONITOR) || sh_isstate(shp, SH_FORKED)))
        return (0);
...
int
job_walk(Shell_t *shp,
         Sfio_t *file,
         int (*fun)(struct process *, int),
         int arg,
         char *joblist[])
{
    struct process *pw;
...
#    endif /* SHOPT_COSHELL */
    ...
    } else if (*jobs == 0) /* current job */
    {
        ...
        while (pw && (pw->p_env != shp->jobenv || pw->p_pgrp == 0))
            pw = pw->p_nxtjob;
        if ((*fun)(pw, arg))
            r = 2;
    } else
 ...
 */
int
job_terminate(struct process *pw, int sig)
{
...
int
job_list(struct process *pw, int flag)
{
    Shell_t *shp = sh_getinterp();
...
#    endif /* SHOPT_COSHELL */
        ...
        if (!px) {
            ...
            if (dir && (pwd = nv_getval(PWDNOD)) && !strcmp(dir, pwd))
                dir = 0;
            hist_list(
            shgd->hist_ptr, outfile, pw->p_name, dir ? '\n' : 0, ";");
            if (dir) {
                char *tilde = "";
                if (!strncmp(dir, home, len) && dir[len] == '/'
                    || dir[len] == 0) {
                    tilde = "~";
                    dir += len;
                }
...
int
job_kill(struct process *pw, int sig)
{
    Shell_t *shp;
    pid_t pid;
    int r;
...
int
job_post(Shell_t *shp, pid_t pid, pid_t join)
{
    struct process *pw;
...
#ifdef DEBUG
    sfprintf(sfstderr,
             "ksh: job line %4d: post pid=%d critical=%d job=%d pid=%d "
             "pgid=%d savesig=%d join=%d\n",
             __LINE__,
             getpid(),
             job.in_critical,
             pw->p_job,
    ...
    if (join && job.pwlist) {
        ...
        /* create a new job */
        while ((pw->p_job = job_alloc(shp)) < 0)
            job_wait(( pid_t ) 1);
        pw->p_nxtproc = 0;
        do
            pw->p_nxtjob = job.pwlist;
 ...
 */
static void
job_prmsg(Shell_t *shp, struct process *pw)
{
 ...
 * If pid < -1, then wait can be interrupted, -pid is waited for (wait
 * builtin) pid=0 to unpost all done processes pid=1 to wait for at least one
 * process to complete pid=-1 to wait for all runing processes
 */
...
job_wait(pid_t pid)
{
    ...
    job_lock();
    if (pid == 0) {
        if (!job.waitall || !job.curjobid
            || !(pw = job_byjid(job.curjobid))) {
            job_unlock();
            goto done;
        }
        jobid = pw->p_job;
    ...
    sfprintf(
    sfstderr,
    "ksh: job line %4d: wait pid=%d critical=%d job=%d pid=%d waitall=%d\n",
    __LINE__,
    getpid(),
    job.in_critical,
    jobid,
        ...
        if (pw && (pw->p_flag & (P_DONE | P_STOPPED))) {
            ...
            if (pw->p_flag & P_STOPPED) {
                ...
                if (sh_isoption(shp, SH_INTERACTIVE)
                    ...
                    pw->p_flag
                    &= ~(P_NOTIFY | P_SIGNALLED | P_STOPPED | P_EXITSAVE);
            } else
#endif /* SIGTSTP */
            {
                if (pw->p_flag & P_SIGNALLED) {
    ...
    if (!job.waitall && sh_isoption(shp, SH_PIPEFAIL))
        return (nochild);
    if (!shp->intrap) {
        job_lock();
        for (pw = job.pwlist; pw; pw = px) {
...
static void
job_fgrp(struct process *pw, int newgrp)
{
    for (; pw; pw = pw->p_nxtproc)
        pw->p_fgrp = newgrp;
}
...
static struct process *
job_unpost(Shell_t *shp, struct process *pwtop, int notify)
{
    struct process *pw;
    ...
    if (!pw || (pw->p_flag & P_BG))
        return (pw);
    for (; pw && (pw->p_flag & P_DONE)
           && (notify || !(pw->p_flag & P_NOTIFY) || pw->p_env);
         pw = pw->p_nxtproc)
        ;
    if (pw)
        return (pw);
...
job_alloc(Shell_t *shp)
{
    ...
    if (j >= jmax) {
        ...
        for (j = 1; j < shgd->lim.child_max; j++) {
            if ((pw = job_byjid(j)) && !job_unpost(shp, pw, 0))
                break;
        }
        j /= CHAR_BIT;
        if (j >= jmax)
...
static char *
job_sigmsg(Shell_t *shp, int sig)
{
    ...
    if (sig >= shp->gd->sigruntime[SH_SIGRTMIN]
        && sig <= shp->gd->sigruntime[SH_SIGRTMAX]) {
        ...
        if (sig > shp->gd->sigruntime[SH_SIGRTMIN]
            ...
            sfsprintf(sigrt,
                      sizeof(sigrt),
                      "SIGRTMIN+%d",
                      sig - shp->gd->sigruntime[SH_SIGRTMIN]);
        return (sigrt);
    }
#endif
 ...
 */
static int
job_chksave(pid_t pid, long env)
{
...
void
job_subrestore(Shell_t *shp, void *ptr)
{
    struct jobsave *jp, *jpnext;
    ...
    for (pw = job.pwlist; pw; pw = pwnext) {
        pwnext = pw->p_nxtjob;
        if (pw->p_env != shp->curenv || pw->p_pid == shp->pipepid)
            continue;
        for (px = pw; px; px = px->p_nxtproc)
            px->p_flag |= P_DONE;
        job_unpost(shp, pw, 0);
