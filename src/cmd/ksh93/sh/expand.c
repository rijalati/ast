...
static int suflen;
static int
scantree(Shell_t *, Dt_t *, const char *, struct argnod **);
#else
...
int
path_expand(Shell_t *shp, const char *pattern, struct argnod **arghead)
{
    glob_t gdata;
    struct argnod *ap;
    glob_t *gp = &gdata;
    ...
    flags = GLOB_GROUP | GLOB_AUGMENTED | GLOB_NOCHECK | GLOB_NOSORT
            | GLOB_STACK | GLOB_LIST | GLOB_DISC;
...
static int
checkfmt(Sfio_t *sp, void *vp, Sffmt_t *fp)
{
    return -1;
}

int
path_generate(Shell_t *shp, struct argnod *todo, struct argnod **arghead)
/*@
    assume todo!=0;
