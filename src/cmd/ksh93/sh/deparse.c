...
static void
p_comlist(const struct dolnod *, int);
static void
p_arg(const struct argnod *, int endchar, int opts);
static void
p_comarg(const struct comnod *);
static void
p_keyword(const char *, int);
static void
p_redirect(const struct ionod *);
static void
p_switch(const struct regnod *);
static void
here_body(const struct ionod *);
static void
p_tree(const Shnode_t *, int);
...
void
sh_deparse(Sfio_t *out, const Shnode_t *t, int tflags)
{
    outfile = out;
    p_tree(t, tflags);
}
 ...
 */
static void
p_tree(const Shnode_t *t, int tflags)
{
    char *cp;
    int save = end_line;
 ...
 */
static void
p_keyword(const char *word, int flag)
{
    int sep;
    if (flag == END)
        sep = end_line;
...
static void
p_arg(const struct argnod *arg, int endchar, int opts)
{
    const char *cp;
    int flag;
    do {
...
static void
p_comarg(const struct comnod *com)
{
    int flag = end_line;
    if (com->comtyp & FAMP)
        sfwrite(outfile, "& ", 2);
...
static void
p_comlist(const struct dolnod *dol, int endchar)
{
    char *cp, *const *argv;
    int flag = ' ', special;
 ...
 */
static void
here_body(const struct ionod *iop)
{
    Sfio_t *infile;
#ifdef xxx
    if (iop->iolink)
