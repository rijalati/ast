...
#            undef pipe
#            define pipe(v)                                                  \
                ((socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0, v) < 0  \
                  || shutdown((v)[1], SHUT_RD) < 0                           \
                  || fchmod((v)[1], S_IWUSR) < 0                             \
                  || shutdown((v)[0], SHUT_WR) < 0                           \
                  || fchmod((v)[0], S_IRUSR) < 0)                            \
                 ? (-1)                                                      \
                 : 0)
#        else
#            undef pipe
...
static int
subexcept(Sfio_t *, int, void *, Sfdisc_t *);
static int
eval_exceptf(Sfio_t *, int, void *, Sfdisc_t *);
static int
slowexcept(Sfio_t *, int, void *, Sfdisc_t *);
static int
pipeexcept(Sfio_t *, int, void *, Sfdisc_t *);
static ssize_t
piperead(Sfio_t *, void *, size_t, Sfdisc_t *);
static ssize_t
slowread(Sfio_t *, void *, size_t, Sfdisc_t *);
static ssize_t
subread(Sfio_t *, void *, size_t, Sfdisc_t *);
static ssize_t
tee_write(Sfio_t *, const void *, size_t, Sfdisc_t *);
static int
io_prompt(Shell_t *, Sfio_t *, int);
static int
io_heredoc(Shell_t *, struct ionod *, const char *, int);
static void
sftrack(Sfio_t *, int, void *);
static const Sfdisc_t eval_disc = { NULL, NULL, NULL, eval_exceptf, NULL };
static Sfdisc_t tee_disc = { NULL, tee_write, NULL, NULL, NULL };
static Sfio_t *
subopen(Shell_t *, Sfio_t *, off_t, long);
...
static Sfdouble_t
nget_cur_eof(Namval_t *np, Namfun_t *fp)
{
...
static int
matchf(void *handle, char *ptr, size_t size)
{
...
bool
sh_iovalidfd(Shell_t *shp, int fd)
{
    ...
    shp->sftable = ( Sfio_t ** ) calloc(
    (n + 1) * (sizeof(int *) + sizeof(Sfio_t *) + sizeof(*fdstatus)), 1);
...
sh_ioinit(Shell_t *shp)
{
    ...
    shp->outpool
    = sfopen(NIL(Sfio_t *), NIL(char *), "sw"); /* pool identifier */
 ...
 */
static int
outexcept(Sfio_t *iop, int type, void *data, Sfdisc_t *handle)
{
    Shell_t *shp = (( struct Iodisc * ) handle)->sh;
    static int active = 0;
    if (type == SF_DPOP || type == SF_FINAL)
        free(( void * ) handle);
    else if (type == SF_WRITE && (*( ssize_t * ) data) < 0
             && sffileno(iop) != 2)
        switch (errno) {
        case EINTR:
        case EPIPE:
 ...
 */
Sfio_t *
sh_iostream(Shell_t *shp, int fd, int fn)
{
    Sfio_t *iop;
 ...
 */
static void
io_preserve(Shell_t *shp, Sfio_t *sp, int f2)
{
    int fd;
    if (sp)
        fd = sfsetfd(sp, 10);
 ...
 */
int
sh_iorenumber(Shell_t *shp, int f1, int f2)
{
    Sfio_t *sp = shp->sftable[f2];

    if (!sh_iovalidfd(shp, f1))
        abort();
    if (!sh_iovalidfd(shp, f2))
        abort();
    if (f1 != f2) {
 ...
 */
int
sh_open(const char *path, int flags, ...)
{
    Shell_t *shp = sh_getinterp();
    Sfio_t *sp;
    int fd;
    ...
    if (strncmp(path, "/etc/", 5) == 0) {
        sfsprintf(buf,
                  sizeof(buf),
                  "%s%s",
                  sh_regress_etc(path, __LINE__, __FILE__),
                  path + 4);
        path = buf;
    }
#endif
 ...
 */
int
sh_iomovefd(Shell_t *shp, int fdold)
{
    int fdnew;

    if (fdold >= shp->gd->lim.open_max)
        if (!sh_iovalidfd(shp, fdold))
            abort();
    if (fdold < 0 || fdold > 9)
        return (fdold);
    fdnew = sh_iomovefd(shp, sh_fcntl(fdold, F_DUPFD_CLOEXEC, 10));
    if (!sh_iovalidfd(shp, fdnew))
        abort();
...
sh_pipe(int pv[])
{
    ...
    if (!sh_iovalidfd(shp, fd[0]))
        abort();
    if (!sh_iovalidfd(shp, fd[1]))
        abort();
...
sh_rpipe(int pv[])
{
    Shell_t *shp = sh_getinterp();
    int fd[2];
    if (pipe2(fd, O_CLOEXEC) < 0 || (pv[0] = fd[0]) < 0
        || (pv[1] = fd[1]) < 0)
        errormsg(SH_DICT, ERROR_system(1), e_pipe);
#if !_lib_pipe2 || !defined(O_CLOEXEC)
    if (pv[0] > 2)
...
#if SHOPT_COSHELL
int
sh_coaccept(Shell_t *shp, int *pv, int out)
{
    int fd = accept4(
    pv[0], ( struct sockaddr * ) 0, ( socklen_t * ) 0, SOCK_CLOEXEC);
    sh_close(pv[0]);
    pv[0] = -1;
    if (fd < 0)
        errormsg(SH_DICT, ERROR_system(1), e_pipe);
...
int
sh_copipe(Shell_t *shp, int *pv, int out)
{
    int r, port = 20000;
    ...
    do {
        ...
        slen = sizeof(sin);
    } while ((r = bind(pv[out], ( struct sockaddr * ) &sin, slen)) == -1
             && errno == EADDRINUSE);
...
static int
pat_seek(void *handle, const char *str, size_t sz)
{
    char **bp = ( char ** ) handle;
    *bp = ( char * ) str;
    return (-1);
...
static int
pat_line(const regex_t *rp, const char *buff, size_t n)
{
...
static int
io_patseek(Shell_t *shp, regex_t *rp, Sfio_t *sp, int flags)
{
    char *cp, *match;
    int r, fd, close_exec;
    ...
    while ((cp = sfreserve(sp, -s, SF_LOCKR))
           || (cp = sfreserve(sp, SF_UNBOUND, SF_LOCKR))) {
        m = n = sfvalue(sp);
        while (n > 0 && cp[n - 1] != '\n')
            n--;
        if (n)
...
static Sfoff_t
file_offset(Shell_t *shp, int fn, char *fname)
{
    Sfio_t *sp;
    char *cp;
    Sfoff_t off;
...
static char *
io_usename(Shell_t *shp, char *name, int *perm, int fno, int mode)
{
    struct stat statb;
 ...
 */
void
sh_vexrestore(Shell_t *shp, int n)
{
 ...
 */
static int
iovex_child(void *context, uintmax_t fd1, uintmax_t fd2)
{
...
#    if 1
    char buff[256];
    int n
    = sfsprintf(buff, sizeof(buff), "%p: fd1=%d fd2=%d \n", sp, fd1, fd2);
    write(-1, buff, n);
#    else
    if (sp) {
...
static void
iovex_stdstream(Shell_t *shp, int fn)
{
    if (fn > 2)
        return;
    if (fn == 0) {
 ...
 */
static int
iovex_stream(void *context, uintmax_t origfd, uintmax_t fd2)
{
    Shell_t *shp = ( Shell_t * ) context;
    ...
    if (sporig) {
        ...
        if (shp->readscript) {
            ...
            sfnew(sporig,
                  ...
                  -1,
                  fd,
                  (fd < 3 ? SF_SHARE | SF_PUBLIC : 0)
                  | (status & (IOREAD | IOWRITE)));
...
static int
iovex_trunc(void *context, uintmax_t origfd, uintmax_t fd2)
{
    Shell_t *shp = ( Shell_t * ) context;
    int r = 0;
    errno = 0;
...
static int
iovex_rename(void *context, uintmax_t origfd, uintmax_t fd2)
{
    Shell_t *shp = *( Shell_t ** ) context;
 ...
 */
int
sh_redirect(Shell_t *shp, struct ionod *iop, int flag)
{
    Sfoff_t off;
    char *fname;
    int fd, iof;
    ...
    for (; iop; iop = iop->ionxt) {
        iof = iop->iofile;
        fn = (iof & IOUFD);
        if (fn == 1 && shp->subshell && !shp->subshare
            && (flag == 2 || isstring))
            sh_subfork();
        ...
        if (!(iof & IORAW)) {
            if (iof & IOLSEEK) {
                struct argnod *ap = ( struct argnod * ) stkalloc(
                shp->stk, ARGVAL + strlen(iop->ioname));
                memset(ap, 0, ARGVAL);
                ap->argflag = ARG_MAC;
                strcpy(ap->argval, iop->ioname);
                fname
                = sh_macpat(shp, ap, (iof & IOARITH) ? ARG_ARITH : ARG_EXP);
                ...
                fname = sh_mactrim(shp,
                                   fname,
                                   (!sh_isoption(shp, SH_NOGLOB)
                                    && sh_isoption(shp, SH_INTERACTIVE))
                                   ? 2
                                   : 0);
        }
        errno = 0;
        ...
        if (iop->iovname) {
            np
            = nv_open(iop->iovname, shp->var_tree, NV_NOASSIGN | NV_VARNAME);
        ...
        if (*fname || (iof & (IODOC | IOSTRG)) == (IODOC | IOSTRG)) {
            if (iof & IODOC) {
                ...
                if ((fd = fname[0]) >= '0' && (fd == '{' || fd <= '9')) {
                    ...
                    if (shp->subshell && dupfd == 1) {
                        ...
                        if ((sfset(sp, 0, 0) & SF_STRING)
                            && (tmpname
                                = pathtemp(NiL, NiL, NiL, "sf", &f))) {
                            Sfoff_t last
                            = sfseek(sp, ( Sfoff_t ) 0, SEEK_END);
                ...
                if ((sp = shp->sftable[dupfd])
                    ...
                    shp->sftable[fd]
                    = sfnew(NIL(Sfio_t *), cp, r, -1, SF_READ | SF_STRING);
                ...
                if (iof & IOAPP) {
                    io_op[2] = '>';
                    o_mode |= O_APPEND;
                } else if ((iof & IOREWRITE)
                           && (flag == 0 || flag == 1 || sh_subsavefd(fn))) {
                    io_op[2] = ';';
                    o_mode |= O_TRUNC;
                    ...
                    if (iof & IOCLOB)
                        ...
                        if (stat(fname, &sb) >= 0) {
...
#endif /* SHOPT_FS_3D */
                            {
                                errno = EEXIST;
                                errormsg(
                                SH_DICT, ERROR_system(1), e_exists, fname);
                            }
                        } else
                            o_mode |= O_EXCL;
                    }
            ...
            openit:
                if (flag != SH_SHOWME) {
                    if ((fd = sh_open(tname ? tname : fname, o_mode, RW_ALL))
                        < 0)
                        errormsg(SH_DICT,
                                 ERROR_system(1),
                                 ((o_mode & O_CREAT) ? e_create : e_open),
                                 fname);
                    if (perm > 0)
#if _lib_fchmod
                        fchmod(fd, perm);
        ...
        traceit:
            ...
            if (trace && fname) {
                ...
                if (np) {
                    ...
                    av += 3;
                sh_debug(
                shp, trace, ( char * ) 0, ( char * ) 0, av, ARG_NOGLOB);
            }
            if (iof & IOLSEEK) {
                sp = shp->sftable[fn];
                r = shp->fdstatus[fn];
                ...
                if (iof & IOARITH) {
                    ...
                    if (!(rp = regcache(fname,
                                        REG_SHELL | REG_NOSUB | REG_NEWLINE
                                        | REG_AUGMENTED | REG_FIRST | REG_LEFT
                                        | REG_RIGHT,
                                        &r))) {
            ...
            if (!np) {
                if (!vex
                    ...
                    if (fd == fn) {
                        ...
                        sh_iosave(
                        shp, fd, indx, tname ? fname : (trunc ? Empty : 0));
                    } else {
                        sh_iosave(
                        shp, fn, indx, tname ? fname : (trunc ? Empty : 0));
                    }
            ...
            if (fd >= 0) {
                if (np) {
                    ...
                    if (sp) {
                        ...
                        sfclose(sp);
                        fd = sh_fcntl(
                        fd, fn < 3 ? F_DUPFD : F_DUPFD_CLOEXEC, fx);
 ...
 */
static int
io_heredoc(Shell_t *shp, struct ionod *iop, const char *name, int traceon)
{
 ...
 */
static ssize_t
tee_write(Sfio_t *iop, const void *buff, size_t n, Sfdisc_t *unused)
{
    NOT_USED(unused);
 ...
 */
void
sh_iosave(Shell_t *shp, int origfd, int oldtop, char *name)
{
    int savefd;
    Sfio_t *sp;
    ...
    if (shp->topfd >= filemapsize) {
        ...
        long moved;
        filemapsize += 8;
        if (!(filemap = ( struct fdsave * ) realloc(
              filemap, filemapsize * sizeof(struct fdsave))))
    ...
    if (flag & IOPICKFD)
        ...
        if ((savefd = sh_fcntl(origfd, F_DUPFD_CLOEXEC, 10)) < 0
            && errno != EBADF) {
            shp->toomany = 1;
            (( struct checkpt * ) shp->jmplist)->mode = SH_JMPERREXIT;
            errormsg(SH_DICT, ERROR_system(1), e_toomany);
...
void
sh_vexsave(Shell_t *shp, int fn, int fd, Spawnvex_f vexfun, void *arg)
{
    Spawnvex_t *vp = shp->vexp;
    Spawnvex_t *vc = shp->vex;
    Sfio_t *sp = 0;
 ...
 */
void
sh_iorestore(Shell_t *shp, int last, int jmpval)
{
    ...
    for (fd = shp->topfd - 1; fd >= last; fd--) {
        ...
        if (filemap[fd].tname == Empty && shp->exitval == 0)
            ...
            io_usename(
            shp, filemap[fd].tname, ( int * ) 0, origfd, shp->exitval ? 2 : 1);
        sh_close(origfd);
        if ((savefd = filemap[fd].save_fd) >= 0) {
            int dupflag
            = (shp->fdstatus[origfd] & IOCLEX) ? F_DUPFD_CLOEXEC : F_DUPFD;
            if (!savestr)
 ...
 */
static int
slowexcept(Sfio_t *iop, int type, void *data, Sfdisc_t *handle)
{
    Shell_t *shp = (( struct Iodisc * ) handle)->sh;
    int n, fno;
    NOT_USED(handle);
...
#    endif /* O_NDELAY */
        ...
        if (errno != EINTR)
            return (0);
        else if (shp->bltinfun && (shp->trapnote & SH_SIGTRAP)
                 && shp->lastsig)
            return (-1);
        n = 1;
...
static ssize_t
piperead(Sfio_t *iop, void *buff, size_t size, Sfdisc_t *handle)
{
    Shell_t *shp = (( struct Iodisc * ) handle)->sh;
    int fd = sffileno(iop);
 ...
 */
static ssize_t
slowread(Sfio_t *iop, void *buff, size_t size, Sfdisc_t *handle)
{
    Shell_t *shp = (( struct Iodisc * ) handle)->sh;
    ...
    while (1) {
        ...
        if (shp->timeout)
            ...
            sh_isstate(shp, SH_GRACE) ? 1000L * TGRACE : 1000L * shp->timeout,
            0,
            time_grace,
            shp);
        rsize = (*readf)(
        shgd->ed_context, sffileno(iop), ( char * ) buff, size, reedit);
        if (timeout)
            timerdel(timeout);
        timeout = 0;
        if (rsize && *( char * ) buff != '\n' && shp->nextprompt == 1
...
int
sh_iocheckfd(Shell_t *shp, int fd, int fn)
{
    int flags, n;
                       ...
                       (statb.st_ino == 0
                        && (statb.st_mode
                            & ~(S_IRUSR | S_IRGRP | S_IROTH | S_IWUSR
                                | S_IWGRP | S_IWOTH | S_IXUSR | S_IXGRP
                                | S_IXOTH | S_ISUID | S_ISGID))
                           == 0)
                       || (S_ISCHR(statb.st_mode)
                           && (statb.st_ino != null_ino
                               || statb.st_dev != null_dev))))
            n |= IONOSEEK;
        else
            n |= IOSEEK;
        shp->sftable[fd] = sp;
...
static int
io_prompt(Shell_t *shp, Sfio_t *iop, int flag)
{
    char *cp;
    char buff[1];
    char *endprompt;
 ...
 */
static int
pipeexcept(Sfio_t *iop, int mode, void *data, Sfdisc_t *handle)
{
 ...
 */
static void
sftrack(Sfio_t *sp, int flag, void *data)
{
    Shell_t *shp = sh_getinterp();
    int fd = sffileno(sp);
    struct checkpt *pp;
...
#endif
    if (fd < 0 || fd == PSEUDOFD
        || (fd >= shp->gd->lim.open_max && !sh_iovalidfd(shp, fd)))
        return;
...
static int
eval_exceptf(Sfio_t *iop, int type, void *data, Sfdisc_t *handle)
{
...
static Sfio_t *
subopen(Shell_t *shp, Sfio_t *sp, off_t offset, long size)
{
    struct subfile *disp;
    if (sfseek(sp, offset, SEEK_SET) < 0)
        return (NIL(Sfio_t *));
    if (!(disp = ( struct subfile * ) malloc(sizeof(struct subfile) + IOBSIZE
                                             + 1)))
        return (NIL(Sfio_t *));
    disp->disc = sub_disc;
    disp->oldsp = sp;
    ...
    sp
    = sfnew(NIL(Sfio_t *), ( char * ) (disp + 1), IOBSIZE, PSEUDOFD, SF_READ);
    sfdisc(sp, &disp->disc);
    return (sp);
}
 ...
 */
static ssize_t
subread(Sfio_t *sp, void *buff, size_t size, Sfdisc_t *handle)
{
 ...
 */
static int
subexcept(Sfio_t *sp, int mode, void *data, Sfdisc_t *handle)
{
    struct subfile *disp = ( struct subfile * ) handle;
    if (mode == SF_CLOSING) {
 ...
 */
void
sh_menu_20120720(Shell_t *shp, Sfio_t *outfile, int argn, char *argv[])
{
    int i, j;
    char **arg;
...
#undef sh_menu
void
sh_menu(Sfio_t *outfile, int argn, char *argv[])
{
 ...
 */
ssize_t
sh_read(int fd, void *buff, size_t n)
{
    int r, err = errno;
    Shell_t *shp = sh_getinterp();
    Sfio_t *sp;
 ...
 */
ssize_t
sh_write(int fd, const void *buff, size_t n)
{
    int r, err = errno;
    Shell_t *shp = sh_getinterp();
    Sfio_t *sp;
 ...
 */
off_t
sh_seek(int fd, off_t offset, int whence)
{
    Shell_t *shp = sh_getinterp();
    Sfio_t *sp;
    if (!sh_iovalidfd(shp, fd))
...
Sfio_t *
sh_fd2sfio_20120720(Shell_t *shp, int fd)
{
    int status;
    Sfio_t *sp = shp->sftable[fd];
...
Sfio_t *
sh_pathopen(Shell_t *shp, const char *cp)
{
    int n;
#ifdef PATH_BFPATH
...
int
sh_stat(const char *path, struct stat *statb)
{
    int r, err = errno;
