...
static char *
fmttoken(Lex_t *, int, char *);
#ifdef SF_BUFCONST
static int
alias_exceptf(Sfio_t *, int, void *, Sfdisc_t *);
#else
static int
alias_exceptf(Sfio_t *, int, Sfdisc_t *);
#endif
static void
setupalias(Lex_t *, const char *, Namval_t *);
static int
comsub(Lex_t *, int);
static void
nested_here(Lex_t *);
static int
here_copy(Lex_t *, struct ionod *);
static bool
stack_grow(Lex_t *);
...
static void
refvar(Lex_t *lp, int type)
{
    Shell_t *shp = lp->sh;
    Stk_t *stkp = shp->stk;
    off_t off = (fcseek(0) - (type + 1))
                - (lp->lexd.first ? lp->lexd.first : fcfirst());
    unsigned long r;
    if (lp->lexd.first) {
        off = (fcseek(0) - (type + 1)) - lp->lexd.first;
        r = kiaentity(lp,
                      lp->lexd.first + lp->lexd.kiaoff + type,
                      off - lp->lexd.kiaoff,
                      'v',
                      -1,
                      -1,
...
sh_lexopen(Lex_t *lp, Shell_t *shp, int mode)
{
    ...
    if (!sh_isoption(shp, SH_DICTIONARY) && sh_isoption(shp, SH_NOEXEC))
        lp->lexd.warn = 1;
    if (!mode) {
        lp->lexd.noarg = lp->lexd.level = lp->lexd.dolparen = lp->lexd.balance
        = 0;
        lp->lexd.nocopy = lp->lexd.docword = lp->lexd.nest = lp->lexd.paren
        = 0;
...
sh_lex(Lex_t *lp)
{
    ...
    while (1) {
        ...
        case S_OP:
            ...
            if (state[n] == S_OP || n == '#') {
                if (n == c) {
                    ...
                    if (!sh_isoption(lp->sh, SH_POSIX) && n == '>'
                        && (sh_isoption(lp->sh, SH_BASH)
                            || sh_isstate(lp->sh, SH_PROFILE))) {
                        if (!sh_isoption(lp->sh, SH_BASH)
                            && !lp->nonstandard) {
                            lp->nonstandard = 1;
                            errormsg(SH_DICT,
                                     ERROR_warn(0),
                ...
                if (n) {
                    ...
                    if (lp->lexd.warn && (n = fcpeek(0)) != RPAREN && n != ' '
                        && n != '\t')
                        errormsg(SH_DICT,
                                 ERROR_warn(0),
                                 e_lexspace,
            ...
            if (c == LPAREN && lp->comp_assign && !lp->lex.intest
                && !lp->lex.incase)
                lp->comp_assign = 2;
            else
                lp->comp_assign = 0;
        ...
        case S_ESC:
            ...
            if (mode == ST_DOL)
                ...
                errormsg(
                SH_DICT, ERROR_warn(0), e_lexfuture, shp->inlineno, n);
#endif /* STR_MAXIMAL */
            break;
        case S_NAME:
            if (!lp->lex.skipword)
        ...
        case S_LIT:
            ...
            {
                if ((c = fcpeek(0)) == LPAREN || c == RPAREN || c == '$'
                    || c == LBRACE || c == RBRACE || c == '[' || c == ']') {
                    if (fcpeek(1) == '\'')
                        fcseek(2);
                }
                continue;
            ...
            if (mode == ST_DOL) {
                ...
                if (oldmode(lp) == ST_QUOTE) /* $' within "" or `` */
                {
                    if (lp->lexd.warn)
                        errormsg(
                        SH_DICT, ERROR_warn(0), e_lexslash, shp->inlineno);
                    mode = ST_LIT;
                }
            }
            if (mode != ST_LIT) {
                if (lp->lexd.warn && lp->lex.last_quote
                    && shp->inlineno > lp->lastline && fcpeek(-2) != '$')
                    errormsg(SH_DICT,
                             ERROR_warn(0),
                             e_lexlongquote,
                             lp->lastline,
        ...
        case S_GRAVE:
            if (lp->lexd.warn && (mode != ST_QUOTE || endchar(lp) != '`'))
                errormsg(
                SH_DICT, ERROR_warn(0), e_lexobsolete1, shp->inlineno);
        ...
        case S_EDOL:
            ...
            if (lp->kiafile)
                refvar(lp, 0);
#endif /* SHOPT_KIA */
            if (lp->lexd.warn && c == LBRACT && !lp->lex.intest
                && !lp->lexd.arith && oldmode(lp) != ST_NESTED)
                errormsg(
                SH_DICT, ERROR_warn(0), e_lexusebrace, shp->inlineno);
            fcseek(-LEN);
            mode = oldmode(lp);
            poplevel(lp);
            break;
        ...
        case S_ERR:
            if ((n = endchar(lp)) == '$')
                goto err;
            if (c == '*'
                || (n = sh_lexstates[ST_BRACE][c]) != S_MOD1 && n != S_MOD2) {
                /* see whether inside `...` */
                mode = oldmode(lp);
                poplevel(lp);
                if ((n = endchar(lp)) != '`')
        ...
        case S_LBRA:
            if ((c = endchar(lp)) == '$') {
                ...
                if (state[c] != S_ERR && c != RBRACE)
                    continue;
                if ((n = sh_lexstates[ST_BEGIN][c]) == 0 || n == S_OP
                    || n == S_NLTOK) {
                    c = LBRACE;
                    goto do_comsub;
                }
        ...
        err:
            ...
            if (n != '$') {
                ...
                if (lp->lexd.warn && c != '/'
                    && sh_lexstates[ST_NORM][c] != S_BREAK
                    && (c != '"' || mode == ST_QUOTE))
                    errormsg(
                    SH_DICT, ERROR_warn(0), e_lexslash, shp->inlineno);
        ...
        case S_META:
            if (lp->lexd.warn && endchar(lp) == RBRACE
                && !lp->lexd.nested_tilde)
                errormsg(
                SH_DICT, ERROR_warn(0), e_lexusequote, shp->inlineno, c);
            continue;
        case S_PUSH:
            fcgetc(n);
            if (n == RPAREN)
        ...
        do_pop:
            ...
            if (lp->lexd.level <= inlevel)
                break;
            if (lp->lexd.level == inlevel + 1 && lp->lex.incase >= TEST_RE
                && !lp->lex.intest) {
                fcseek(-LEN);
                goto breakloop;
            }
            ...
            if (mode == ST_QNEST) {
                if (lp->lexd.warn)
                    errormsg(
                    SH_DICT, ERROR_warn(0), e_lexescape, shp->inlineno, c);
                continue;
            }
            mode = oldmode(lp);
            poplevel(lp);
            ...
            if (n == 1 && c == RPAREN) {
                ...
                /* backward compatibility */
                {
                    if (lp->lexd.warn)
                        errormsg(
                        SH_DICT, ERROR_warn(0), e_lexnested, shp->inlineno);
        ...
        case S_BRACT:
            ...
            if ((n = endchar(lp)) == RBRACT || n == RPAREN
                || (mode == ST_BRACE) || (oldmode(lp) == ST_NONE)
        ...
        case S_BRACE: {
            ...
            if (lp->lex.reservok && state[n] == S_BREAK && isfirst)
                break;
            if (sh_isoption(lp->sh, SH_BRACEEXPAND) && c == LBRACE
                && !assignment && state[n] != S_BREAK && !lp->lex.incase
                && !lp->lex.intest && !lp->lex.skipword) {
                wordflags |= ARG_EXP;
            }
        ...
        epat:
            ...
            if (lp->lexd.warn && c == '[' && n == '^')
                errormsg(
                SH_DICT, ERROR_warn(0), e_lexcharclass, shp->inlineno);
            if (n > 0)
                fcseek(-LEN);
            if (n == '=' && c == '+' && mode == ST_NAME)
                continue;
...
breakloop:
    ...
    if (n == 1) {
        ...
        if (n == LBRACT)
            ...
            c = (wordflags & ARG_EXP);
        n = 1;
    } else if (n > 2 && state[0] == '{' && state[n - 1] == '}'
               && !lp->lex.intest && !lp->lex.incase && (c == '<' || c == '>')
    ...
    if (lp->lex.intest) {
        if (lp->lex.testop1) {
            ...
            if (n == 2 && state[0] == '-' && state[2] == 0
                ...
                if (lp->lexd.warn && state[1] == 'a')
                    errormsg(
                    SH_DICT, ERROR_warn(0), e_lexobsolete2, shp->inlineno);
                lp->digits = state[1];
                lp->token = TESTUNOP;
        ...
        case TEST_SEQ:
            if (lp->lexd.warn && state[1] == 0)
                errormsg(
                SH_DICT, ERROR_warn(0), e_lexobsolete3, shp->inlineno);
            /* FALL THRU */
        default:
            if (lp->lex.testop2) {
    ...
    if (lp->lex.reservok /* && !lp->lex.incase*/ && n <= 2) {
        ...
        if (n == 1 && (c == '{' || c == '}' || c == '!')) {
            if (lp->lexd.warn && c == '{' && lp->lex.incase == 2)
                errormsg(
                SH_DICT, ERROR_warn(0), e_lexobsolete6, shp->inlineno);
        ...
        if (n > 1 && lp->lex.reservok == 1 && mode == ST_NAME
            ...
            if (lp->lex.incase) {
                if (lp->lex.incase > 1)
                    ...
                    c = 0;
            } else if (c == FORSYM || c == CASESYM || c == SELECTSYM
                       || c == FUNCTSYM || c == NSPACESYM) {
                lp->lex.skipword = 1;
 ...
 */
void
sh_lexskip(Lex_t *lp, int close, int copy, int state)
{
    char *cp;
    lp->lexd.nest = close;
...
static int
here_copy(Lex_t *lp, struct ionod *iop)
{
    const char *state;
    int c, n;
    char *bufp, *cp;
 ...
 */
static char *
fmttoken(Lex_t *lp, int sym, char *tok)
{
    int n = 1;
    if (sym < 0)
...
sh_syntax(Lex_t *lp)
{
    ...
    if ((sp = fcfile())
        || (shp->infd >= 0 && (sp = shp->sftable[shp->infd]))) {
...
#endif
        errormsg(
        SH_DICT, ERROR_exit(SYNBAD), e_lexsyntax1, lp->lastline, tokstr, cp);
    else
        errormsg(SH_DICT, ERROR_exit(SYNBAD), e_lexsyntax2, tokstr, cp);
}

static char *
stack_shift(Stk_t *stkp, char *sp, char *dp)
{
    char *ep;
 ...
 */
struct argnod *
sh_endword(Shell_t *shp, int mode)
{
...
#endif /* SHOPT_DOS */
            if (!(inquote & 1)
                || (sh_lexstates[ST_QUOTE][n] && n != RBRACE)) {
                if (n == '\n')
                    dp--;
                else
                    dp[-1] = n;
...
#ifdef SF_BUFCONST
static int
alias_exceptf(Sfio_t *iop, int type, void *data, Sfdisc_t *handle)
#else
static int
...
setupalias(Lex_t *lp, const char *string, Namval_t *np)
{
    ...
    if (ap->np = np) {
        ...
        if (lp->kiafile) {
            unsigned long r;
            r = kiaentity(
            lp, nv_name(np), -1, 'p', 0, 0, lp->current, 'a', 0, "");
            sfprintf(lp->kiatmp,
                     "p;%..64d;p;%..64d;%d;%d;e;\n",
                     lp->current,
                     r,
