...
static int
arg_expand(Shell_t *, struct argnod *, struct argnod **, int);
static void
sh_argset(Arg_t *, char *[]);
...
static int
arraysort(const char *s1, const char *s2)
{
    struct Sort *sp = Sp;
    Shell_t *shp = sp->shp;
    int r = 0, cur = sp->cur;
...
static int
alphasort(const char *s1, const char *s2)
{
    struct Sort *sp = Sp;
    int r = 0;
    char *sp1, *sp2;
...
static int
numsort(const char *s1, const char *s2)
{
    struct Sort *sp = Sp;
    Sfdouble_t d1, d2;
    int r = 0;
...
sh_argopts(int argc, char *argv[], void *context)
{
    ...
    while ((n = optget(argv, setflag ? sh_optset : sh_optksh))) {
        ...
        byname:
            if (!opt_info.arg || !*opt_info.arg || *opt_info.arg == '-') {
                 ...
                 */
                verbose
                = (f ? PRINT_VERBOSE : PRINT_NO_HEADER)
                  | (n == 'O' ? PRINT_SHOPT : 0)
                  | (sh_isoption(shp, SH_BASH) ? PRINT_ALL | PRINT_NO_HEADER
                                               : 0)
    ...
    if (ap->kiafile) {
        ...
        lp->script
        = kiaentity(lp, lp->scriptname, -1, 'p', -1, 0, 0, 's', 0, "");
        lp->fscript
        = kiaentity(lp, lp->scriptname, -1, 'f', -1, 0, 0, 's', 0, "");
        lp->unknown
        = kiaentity(lp, "<unknown>", -1, 'p', -1, 0, 0, '0', 0, "");
...
void
sh_applyopts(Shell_t *shp, Shopt_t newflags)
{
 ...
 */
struct dolnod *
sh_argfree(Shell_t *shp, struct dolnod *blk, int flag)
{
 ...
 */
void
sh_printopts(Shell_t *shp, Shopt_t oflags, int mode, Shopt_t *mask)
{
    const Shtable_t *tp;
    const char *name;
    int on;
 ...
 */
char **
sh_argbuild(Shell_t *shp, int *nargs, const struct comnod *comptr, int flag)
{
    struct argnod *argp;
...
struct argnod *
sh_argprocsub(Shell_t *shp, struct argnod *argp)
{
...
/* Argument expansion */
static int
arg_expand(Shell_t *shp,
           struct argnod *argp,
           struct argnod **argchain,
           int flag)
{
    int count = 0;
