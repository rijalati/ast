...
static int
p_comlist(const struct dolnod *);
static int
p_arg(const struct argnod *);
static int
...
int
sh_tdump(Sfio_t *out, const Shnode_t *t)
{
    outfile = out;
    return (p_tree(t));
}
...
static int
p_comlist(const struct dolnod *dol)
{
    char *cp, *const *argv;
    int n;
