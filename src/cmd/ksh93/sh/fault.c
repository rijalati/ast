...
#define abortsig(sig)                                                        \
    (sig == SIGABRT || sig == SIGBUS || sig == SIGILL || sig == SIGSEGV)
 ...
 */
static int
malloc_done(Vmalloc_t *vm, int type, Void_t *val, Vmdisc_t *dp)
{
...
static int
notify_builtin(Shell_t *shp, int sig)
{
    int action = 0;
#ifdef ERROR_NOTIFY
...
static void
set_trapinfo(Shell_t *shp, int sig, siginfo_t *info)
{
    sigset_t set, oset;
    if (info) {
...
#ifdef _lib_sigaction
void
sh_fault(int sig, siginfo_t *info, void *context)
#else
void
sh_fault(int sig)
#endif
{
    int saved_errno = errno; /* many platforms do not save/restore errno for
                                signal handlers */
    Shell_t *shp = sh_getinterp();
    int flag = 0;
    char *trap;
    ...
    if (pp->mode == SH_JMPCMD
        || (pp->mode == 1 && shp->bltinfun) && !(flag & SH_SIGIGNORE))
        shp->lastsig = sig;
    if (trap) {
        /*
 ...
 */
void
sh_sigtrap(Shell_t *shp, int sig)
{
    int flag;
    void (*fun)(int);
    shp->st.otrapcom = 0;
 ...
 */
void
sh_sigreset(Shell_t *shp, int mode)
{
    char *trap;
#ifdef SIGRTMIN
 ...
 */
void
sh_sigclear(Shell_t *shp, int sig)
{
...
sh_exit_20120720(Shell_t *shp, int xno)
{
    ...
    if ((shp->trapnote & SH_SIGTSTP) && job.jobcontrol) {
        ...
        if (!shp->subshell && sh_isstate(shp, SH_MONITOR)
...
static void
array_notify(Namval_t *np, void *data)
{
 ...
 */
static char *
sig_name(Shell_t *shp, int sig, char *buf, int pfx)
{
    int i;
 ...
 */
void
sh_siglist(Shell_t *shp, Sfio_t *iop, int flag)
{
    ...
    if (flag > 0)
        ...
        for (sig = SH_DEBUGTRAP; sig >= 0; sig--) {
            if (!(trap
                  = shp->st.otrap ? shp->st.otrap[sig] : shp->st.trap[sig]))
                continue;
...
int
sh_trap_20120720(Shell_t *shp, const char *trap, int mode)
{
...
#undef sh_trap
int
sh_trap(const char *trap, int mode)
{
    Shell_t *shp = sh_getinterp();
...
#    undef signal
sh_sigfun_t
sh_signal(int sig, sh_sigfun_t func)
{
