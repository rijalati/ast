...
static void
assign(Namval_t *, const char *, int, Namfun_t *);

int
nv_compare(Dt_t *dict, Void_t *sp, Void_t *dp, Dtdisc_t *disc)
{
    if (sp == dp)
        return (0);
 ...
 */
char *
nv_getv(Namval_t *np, Namfun_t *nfp)
{
 ...
 */
Sfdouble_t
nv_getn(Namval_t *np, Namfun_t *nfp)
{
    Namfun_t *fp;
    Sfdouble_t d = 0;
    Shell_t *shp = sh_ptr(np);
 ...
 */
void
nv_putv(Namval_t *np, const char *value, int flags, Namfun_t *nfp)
{
    Namfun_t *fp, *fpnext;
    Namarr_t *ap;
    if ((fp = nfp) != NIL(Namfun_t *) && !nv_local)
...
block_info(Namval_t *np, struct blocked *pp)
{
    struct blocked *bp;
    void *sub = 0;
    int isub = 0;
...
static void
block_done(struct blocked *bp)
{
    blist = bp = bp->next;
    if (bp && (bp->isub >= 0 || bp->sub))
        nv_putsub(
        bp->np, bp->sub, (bp->isub < 0 ? 0 : bp->isub), ARRAY_SETSUB);
}
 ...
 */
static void
chktfree(Namval_t *np, struct vardisc *vp)
{
    int n;
 ...
 */
static void
assign(Namval_t *np, const char *val, int flags, Namfun_t *handle)
{
    Shell_t *shp = sh_ptr(np);
    ...
    if (val && (tp = nv_type(np))
        && (nr = nv_open(
            val, shp->var_tree, NV_VARNAME | NV_ARRAY | NV_NOADD | NV_NOFAIL))
        && tp == nv_type(nr)) {
    ...
    if (val || isblocked(bp, type)) {
        ...
        if (flags & NV_INTEGER)
            ...
            SH_VALNOD,
            (flags
             & (NV_LONG | NV_DOUBLE | NV_EXPNOTE | NV_HEXFLOAT | NV_SHORT)));
    ...
    if (val) {
        ...
        memcpy(
        &SH_VALNOD->nvname, &node.nvname, sizeof(node) - sizeof(node.nvlink));
 ...
 */
static char *
lookup(Namval_t *np, int type, Sfdouble_t *dp, Namfun_t *handle)
{
    Shell_t *shp = sh_ptr(np);
...
static char *
lookups(Namval_t *np, Namfun_t *handle)
{
    return (lookup(np, LOOKUPS, ( Sfdouble_t * ) 0, handle));
}

static Sfdouble_t
lookupn(Namval_t *np, Namfun_t *handle)
{
    Sfdouble_t d;
 ...
 */
char *
nv_setdisc(Namval_t *np, const char *event, Namval_t *action, Namfun_t *fp)
{
    ...
    while (vp) {
        if (vp->fun.disc
            && (vp->fun.disc->setdisc || vp->fun.disc->putval == assign))
            break;
    ...
    if (!vp) {
        ...
        if (action == np)
            return (( char * ) action);
        if (!(vp = newof(
              NIL(struct vardisc *), struct vardisc, 1, sizeof(Namdisc_t))))
            return (0);
        dp = ( Namdisc_t * ) (vp + 1);
        vp->fun.disc = dp;
...
setdisc(Namval_t *np, const char *event, Namval_t *action, Namfun_t *fp)
{
    ...
    if (action == np)
        ...
        if (tp && (np = ( Namval_t * ) vp->bltins[type])
            && nv_isattr(np, NV_STATICF))
...
static void
putdisc(Namval_t *np, const char *val, int flag, Namfun_t *fp)
{
    Shell_t *shp = sh_ptr(np);
...
Namfun_t *
nv_clone_disc(Namfun_t *fp, int flags)
{
    Namfun_t *nfp;
    int size;
...
bool
nv_adddisc(Namval_t *np, const char **names, Namval_t **funs)
{
    Nambfun_t *vp;
    int n = 0;
    const char **av = names;
    ...
    if (funs)
        memcpy(
        ( void * ) vp->bltins, ( void * ) funs, n * sizeof(Namval_t *));
    else
        while (n >= 0)
            vp->bltins[n--] = 0;
 ...
 */
Namfun_t *
nv_disc(Namval_t *np, Namfun_t *fp, int mode)
{
    Shell_t *shp = sh_ptr(np);
    Namfun_t *lp, **lpp;
    if (nv_isref(np))
 ...
 */
Namfun_t *
nv_hasdisc(Namval_t *np, const Namdisc_t *dp)
{
    Namfun_t *fp;
...
static void
put_notify(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
...
bool
nv_unsetnotify(Namval_t *np, char **addr)
{
    Namfun_t *fp;
    for (fp = np->nvfun; fp; fp = fp->next) {
        if (fp->disc->putval == put_notify
            && (( struct notify * ) fp)->ptr == addr) {
            nv_stack(np, fp);
            nv_stack(np, ( Namfun_t * ) 0);
            if (!(fp->nofree & 1))
                free(( void * ) fp);
...
bool
nv_setnotify(Namval_t *np, char **addr)
{
 ...
 */
static void *
num_clone(Namval_t *np, void *val)
{
    int size;
    void *nval;
    if (!val)
...
void
clone_all_disc(Namval_t *np, Namval_t *mp, int flags)
{
 ...
 */
int
nv_clone(Namval_t *np, Namval_t *mp, int flags)
{
    Namfun_t *fp, *fpnext;
    ...
    if (fp = np->nvfun) {
        ...
        if (nv_isattr(mp, NV_EXPORT | NV_MINIMAL)
            == (NV_EXPORT | NV_MINIMAL)) {
            mp->nvenv = 0;
    ...
    if (mp->nvalue.cp == val && !nv_isattr(np, NV_INTEGER)) {
        if (np->nvalue.cp && np->nvalue.cp != Empty && (flags & NV_COMVAR)
            && !(flags & NV_MOVE)) {
            if (size)
 ...
 */
static char *
clone_getv(Namval_t *np, Namfun_t *handle)
{
    return (np->nvalue.np ? nv_getval(np->nvalue.np) : 0);
}

static Sfdouble_t
clone_getn(Namval_t *np, Namfun_t *handle)
{
    return (np->nvalue.np ? nv_getnum(np->nvalue.np) : 0);
}

static void
clone_putv(Namval_t *np, const char *val, int flags, Namfun_t *handle)
{
    Shell_t *shp = sh_ptr(np);
...
Namval_t *
nv_search(const char *name, Dt_t *root, int mode)
{
 ...
 */
Namval_t *
nv_bfsearch(const char *name, Dt_t *root, Namval_t **var, char **last)
{
    Shell_t *shp = dtuserdata(root, 0, 0);
    ...
    if (!cp) {
        ...
        if (shp->namespace) {
            sfprintf(
            shp->strbuf, "%s.%s%c", nv_name(shp->namespace), name, 0);
    ...
    c = *cp;
    *cp = 0;
    nq = nv_open(stkptr(shp->stk, offset),
                 shp->var_tree,
...
#endif /* SHOPT_NAMESPACE */
    while (nv_isarray(nq) && !nv_isattr(nq, NV_MINIMAL | NV_EXPORT)
           && nq->nvenv && nv_isarray(( Namval_t * ) nq->nvenv))
 ...
 */
Namval_t *
sh_addbuiltin_20120720(Shell_t *shp,
                       const char *path,
                       Shbltin_f bltin,
                       void *extra)
{
    ...
    if (extra == ( void * ) 1)
        name = path;
             ...
             && bltin != ( Shbltin_f ) SYSTYPESET->nvalue.bfp
             && (nq = nv_bfsearch(
                 name, shp->bltin_tree, ( Namval_t ** ) 0, &cp)))
...
#undef sh_addbuiltin
Namval_t *
sh_addbuiltin(const char *path, Shbltin_f bltin, void *extra)
{
...
#undef nv_stack
extern Namfun_t *
nv_stack(Namval_t *np, Namfun_t *fp)
{
...
static Namval_t *
next_table(Namval_t *np, Dt_t *root, Namfun_t *fp)
{
...
static Namval_t *
create_table(Namval_t *np, const char *name, int flags, Namfun_t *fp)
{
    struct table *tp = ( struct table * ) fp;
...
static Namfun_t *
clone_table(Namval_t *np, Namval_t *mp, int flags, Namfun_t *fp)
{
    struct table *tp = ( struct table * ) fp;
    ...
    for (np = ( Namval_t * ) dtfirst(oroot); np;
         np = ( Namval_t * ) dtnext(oroot, np)) {
...
static void
delete_fun(Namval_t *np, void *data)
{
...
static void
put_table(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    ...
    nv_scan(data.sh->fun_tree,
            delete_fun,
            ( void * ) &data,
            NV_FUNCTION,
            NV_FUNCTION | NV_NOSCOPE);
    dtview(root, 0);
 ...
 */
static char *
get_table(Namval_t *np, Namfun_t *fp)
{
    Dt_t *root = (( struct table * ) fp)->dict;
    static Sfio_t *out;
    int first = 1;
...
static const Namdisc_t table_disc = {
    sizeof(struct table), put_table,   get_table, 0,          0,
    create_table,         clone_table, 0,         next_table,
};

Namval_t *
nv_parent(Namval_t *np)
{
 ...
 */
Namval_t *
nv_mount(Namval_t *np, const char *name, Dt_t *dict)
{
    Namval_t *mp, *pp;
    struct table *tp;
...
#if SHOPT_NAMESPACE
Namval_t *
sh_fsearch(Shell_t *shp, const char *fname, int add)
{
    Stk_t *stkp = shp->stk;
