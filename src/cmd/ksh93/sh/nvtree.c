...
static int Indent;
char *
nv_getvtree(Namval_t *, Namfun_t *);
static void
put_tree(Namval_t *, const char *, int, Namfun_t *);
static char *
walk_tree(Namval_t *, Namval_t *, int);

static int
read_tree(Namval_t *np, Sfio_t *in, int n, Namfun_t *dp)
{
...
static Namval_t *
create_tree(Namval_t *np, const char *name, int flag, Namfun_t *dp)
{
    Namfun_t *fp = dp;
    fp->dsize = 0;
    while (fp = fp->next) {
...
static Namfun_t *
clone_tree(Namval_t *np, Namval_t *mp, int flags, Namfun_t *fp)
{
    Namfun_t *dp;
...
= { 0,          put_tree, nv_getvtree, 0, 0,        create_tree,
    clone_tree, 0,        0,           0, read_tree };

static char *
nextdot(const char *str, void *context)
{
    char *cp;
    int c;
    if (*str == '.')
...
void *
nv_diropen(Namval_t *np, const char *name, void *context)
{
        ...
        if (np && ((nfp = nextdisc(np)) || nv_istable(np))) {
            ...
            *save = *dp;
            ...
            if (nv_istable(np))
                ...
                Namarr_t *ap;
                Namval_t *mp = nv_open(
                name, shp->var_tree, NV_VARNAME | NV_NOADD | NV_NOFAIL);
                int sub;
...
nv_dirnext(void *dir)
{
    ...
    while (1) {
        ...
        while (np = dp->hp) {
            ...
            if (dot >= 0)
                nv_putsub(
                dp->table, ( char * )0, xdot, xdot < dot ? 0 : flags);
            ...
            if (!dp->len || strncmp(cp, dp->data, dp->len) == 0) {
                if ((nfp = nextdisc(np))
                    && (nfp->disc->getval || nfp->disc->getnum)
...
static void
outtype(Namval_t *np, Namfun_t *fp, Sfio_t *out, const char *prefix)
{
    char *type = 0;
    Namval_t *tp = fp->type;
    ...
    for (fp = fp->next; fp; fp = fp->next) {
        if (fp->type
            || (fp->disc && fp->disc->typef && (*fp->disc->typef)(np, fp))) {
 ...
 */
void
nv_attribute(Namval_t *np, Sfio_t *out, char *prefix, int noname)
{
    const Shtable_t *tp;
    char *cp;
    ...
    for (fp = np->nvfun; fp; fp = fp->next) {
        if ((typep = fp->type)
            || (fp->disc && fp->disc->typef
                && (typep = (*fp->disc->typef)(np, fp))))
            break;
    }
    if (np == typep) {
        fp = 0;
        ...
        if (prefix && *prefix) {
            if (nv_isvtree(np))
                sfprintf(out, "%s -C ", prefix);
                     ...
                     && nv_isattr(np, ~NV_NOFREE) == NV_MINIMAL
                     && strcmp(np->nvname, "_"))
                sfputr(out, prefix, ' ');
        }
        return;
    ...
    if ((attr = nv_isattr(np, ~NV_NOFREE)) || fp) {
        ...
        for (tp = shtab_attributes; *tp->sh_name; tp++) {
            ...
            if ((attr & mask) == val) {
...
#endif /* SHOPT_FIXEDARRAY */
                    if (ap && !array_assoc(ap) && (xp = ( char ** )(ap + 1))
                        && *xp)
                        ip = nv_namptr(*xp, 0)->nvname;
                }
                if (val == NV_UTOL || val == NV_LTOU) {
                    if ((cp = ( char * )nv_mapchar(np, 0))
                        && strcmp(cp, tp->sh_name + 2)) {
                        sfprintf(out, "-M %s ", cp);
                        continue;
                    }
                }
                ...
                if ((val & (NV_LJUST | NV_RJUST | NV_ZFILL))
                    && !(val & NV_INTEGER) && val != NV_HOST)
            ...
            if (val == NV_INTEGER && nv_isattr(np, NV_INTEGER)) {
                int size = 10;
                if (nv_isattr(np, NV_DOUBLE | NV_EXPNOTE)
                    == (NV_DOUBLE | NV_EXPNOTE)) {
                    size = DBL_DIG;
...
void
nv_outnode(Namval_t *np, Sfio_t *out, int indent, int special)
{
    char *fmtq, *ep, *xp;
    Namval_t *mp;
    Namarr_t *ap = nv_arrayptr(np);
    ...
    while (1) {
        ...
        if (associative || special) {
            Namarr_t *aq;
            if (mp && (aq = nv_arrayptr(mp)) && !aq->fun
                && array_elem(aq) < nv_aimax(mp) + 1)
...
static void
outname(Shell_t *shp, Sfio_t *out, char *name, int len, bool json)
{
    if (json) {
        if (len < 0)
            len = strlen(name);
...
static void
outval(char *name, const char *vname, struct Walk *wp)
{
    ...
    if (!(np = nv_open(vname,
                       root,
                       mode | NV_VARNAME | NV_NOADD | NV_NOASSIGN | NV_NOFAIL
                       | wp->noscope))) {
    ...
    if (special || (nv_isarray(np) && nv_arrayptr(np))) {
        ...
        if (array_elem(nv_arrayptr(np)) == 0) {
            ...
            nq = nv_putsub(np,
                           ...
                           0,
                           ARRAY_SCAN
                           | (wp->out && !nv_type(np) ? ARRAY_NOCHILD : 0));
    }
    if (!wp->out) {
    ...
    if (isarray == 0 && nv_isarray(np)
        && (nv_isnull(np) || np->nvalue.cp == Empty)) /* empty array */
        isarray = 2;
    special |= wp->nofollow;
    ...
    nv_outnode(np,
               wp->out,
               wp->indent,
               special | (wp->flags & (NV_JSON | NV_JSON_LAST)));
    if (*name == '.' && !isarray)
        np->nvfun = fp;
    if (isarray && !special) {
        if (wp->indent > 0) {
 ...
 */
static char **
genvalue(char **argv, const char *prefix, int n, struct Walk *wp)
{
    char *cp, *nextcp, *arg;
            ...
            if (nextcp = nextdot(cp, ( void * )shp)) {
                if (outfile) {
                    *nextcp = 0;
                    np = nv_open(arg,
                                 wp->root,
                                 NV_VARNAME | NV_NOADD | NV_NOASSIGN
                                 | NV_NOFAIL | wp->noscope);
                    if (!np
                        || (nv_isarray(np)
                            && (!(tp = nv_opensub(np)) || !nv_isvtree(tp)))) {
                        *nextcp = '.';
                        continue;
                    }
                    if (*cp != '[' && (tp = nv_type(np))
                        && (ap = nv_arrayptr(np)) && !ap->fun)
                        continue;
                    if (wp->indent >= 0)
                ...
                break;
                       ...
                       && memcmp(arg, argv[1], l = strlen(arg)) == 0
                       && argv[1][l] == '[') {
                int k = 1;
                Namarr_t *aq = 0;
                np
                = nv_open(arg,
                          wp->root,
                          NV_VARNAME | NV_NOADD | NV_NOASSIGN | wp->noscope);
                if (!np)
                    continue;
                ...
                if (argv[1]) {
                    ...
                    if (argv[1][r] == '.'
                        && strncmp(argv[0], argv[1], r) == 0)
                        wp->flags &= ~NV_COMVAR;
                }
                if ((wp->flags & NV_JSON)
                    && (!argv[1] || strlen(argv[1]) < m + n
                        || memcmp(argv[1], arg, m + n - 1)))
 ...
 */
static char *
walk_tree(Namval_t *np, Namval_t *xp, int flags)
{
    Shell_t *shp = sh_ptr(np);
    static Sfio_t *out;
    struct Walk walk;
    ...
    if (shp->last_table && !nv_type(shp->last_table)
        && (cp = nv_name(shp->last_table)) && strcmp(cp, ".sh")
        && (len = strlen(cp)) && strncmp(name, cp, len) == 0
        && name[len] == '.')
        name += len + 1;
    len = strlen(name);
    ...
    while (cp = nv_dirnext(dir)) {
        ...
        if (xp) {
            ...
            if (strlen(cp) <= len)
                continue;
            nq = nv_open(
            cp, walk.root, NV_VARNAME | NV_NOADD | NV_NOASSIGN | NV_NOFAIL);
            if (!nq && (flags & NV_MOVE))
            ...
            mq = nv_open(stkptr(shp->stk, 0),
                         last_root,
                         NV_VARNAME | NV_NOASSIGN | NV_NOFAIL);
            shp->var_tree = dp;
            if (nq && mq) {
 ...
 */
char *
nv_getvtree(Namval_t *np, Namfun_t *fp)
{
#if 1
    int flags = 0, dsize = 0;
#else
    ...
    for (; fp && fp->next; fp = fp->next) {
        if (fp->next->disc
            && (fp->next->disc->getnum || fp->next->disc->getval))
            return (nv_getv(np, fp));
    }
    if (nv_isattr(np, NV_BINARY) && !nv_isattr(np, NV_RAW))
        return (nv_getv(np, fp));
    if (nv_isattr(np, NV_ARRAY) && !nv_type(np)
        && nv_arraychild(np, ( Namval_t * )0, 0) == np)
        return (nv_getv(np, fp));
 ...
 */
static void
put_tree(Namval_t *np, const char *val, int flags, Namfun_t *fp)
{
    struct Namarray *ap;
    int nleft = 0;
