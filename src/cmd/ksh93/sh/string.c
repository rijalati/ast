...
const Shtable_t *
sh_locate(const char *sp, const Shtable_t *table, int size)
{
    int first;
    const Shtable_t *tp;
    int c;
...
int
sh_lookopt(const char *sp, int *invert)
{
    int first;
    const Shtable_t *tp;
    int c;
 ...
 */
char *
sh_substitute(Shell_t *shp, const char *string, const char *oldsp, char *newsp)
/*@
...
/*
 * TRIM(sp)
 * Remove escape characters from characters in <sp> and eliminate quoted
 * nulls.
 */
...
void
sh_utol(char const *str1, char *str2)
/*@
 ...
 */
char *
sh_fmtstr(const char *string, int quote)
{
    ...
    = quote == 'u'
      ? 1
      : quote == 'U' ? 0 : !!(ast.locale.set & AST_LC_unicodeliterals);
#endif
    if (quote == '"')
        goto skip;
    quote = '\'';
    ...
    for (; c; c = mbchar(cp))
    {
        ...
        if (c == quote || !isprint(c))
            ...
            state = 2;
                 ...
                 || (c != ':' && c <= 0x7f && (c = sh_lexstates[ST_NORM][c])
                     && c != S_EPAT))
            state |= 1;
    }
    if (state < 2)
                ...
                default:
                    ...
                    if (!widebyte)
                    {
                        if (lc_unicodeliterals)
                        {
                            ...
                            /*
                             * posix doesn't play the iswrune() game for utf8
                             * locales also, empty strings on error with no
                             * diagnostic just isn't right so source wchars
                             * that have no utf32 counterpart are emitted as
                             * \\w[HEX] => and that's a detectable error under
                             * lc_unicodeliterals
                             */
...
char *
sh_fmtq(const char *string)
{
    return (sh_fmtstr(string, '\''));
}

char *
sh_fmtj(const char *string)
{
    return (sh_fmtstr(string, '"'));
}
 ...
 */
char *
sh_fmtqf(const char *string, int flags, int fold)
{
    const char *cp = string;
    const char *bp;
    const char *vp;
    ...
    if (!cp || !*cp || !fold || fold && strlen(string) < fold)
        return sh_fmtstr(
        cp, (flags & SFFMT_ZERO) ? 'U' : (flags & SFFMT_SIGN) ? 'u' : '\'');
    offset = staktell();
    single = 3;
    c = mbchar(string);
    a = isaletter(c) ? '=' : 0;
...
#if SHOPT_MULTIBYTE
int
sh_strchr(const char *string, const char *dp, size_t size)
{
    wchar_t c, d;
    const char *cp = string;
    mbinit();
...
const char *
_sh_translate(const char *message)
{
