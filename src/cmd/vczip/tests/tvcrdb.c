...
MAIN()
{
    ...
    { /* fill in fields 0 and 1 */
        ...
        k += 1;
        n += 1;
        memcpy(Data + k, Field01[e].str2, n);
        k += n;
        Data[k] = FSEP;
        k += 1;
        ...
        memcpy(Data + k, Field23[e].str1, n);
        k += n;
        Data[k] = FSEP;
        k += 1;
        n += 1;
        memcpy(Data + k, Field23[e].str2, n);
        k += n;
        Data[k] = FSEP;
        k += 1;
        ...
        memcpy(Data + k, Field45[e].str1, n);
        k += n;
        Data[k] = FSEP;
        k += 1;
        n += 1;
        memcpy(Data + k, Field45[e].str2, n);
        k += n;
        Data[k] = RSEP;
        k += 1;
    }
    ...
    if (tbl->fldn != 6 || tbl->recn != N_RECORDS)
        terror("vcrdparse(): fldn=%d, recn=%d, something is not right",
               tbl->fldn,
               tbl->recn);
    ...
    if (r != dtsz)
        terror(
        "vcrdextract(): failed to extract data dtsz=%d rv=%d", dtsz, r);
    ...
    if (plan->pred[0] == 0 && plan->pred[1] == 1 && plan->pred[2] == 2
        && plan->pred[3] == 3 && plan->pred[4] == 4 && plan->pred[5] == 5)
    ...
    if (memcmp(Data, dedt, n) != 0)
        terror("Data do not match");
    twarn(
    "\tTransform+Padded fields+bwt: Raw size=%d Compressed size=%d", n, e);
    ...
    if (memcmp(Data, dedt, n) != 0)
        terror("Data do not match");
    twarn("\tTransform+Padded fields+Vctable: Raw size=%d Compressed size=%d",
          n,
          e);

    /* test compression by padded field plus the table compressor for whole
     * table */
    ...
    if (memcmp(Data, dedt, n) != 0)
        terror("Data do not match");
    twarn(
    "\tTransform+Padded fields+Vctable on whole table: Raw size=%d "
    "Compressed size=%d",
    n,
    e);
    ...
    if (!(envc = vcopen(0, Vcrdb, "fsep=[.].rsep=[\\012]", entbl, VC_ENCODE))
        || !(devc
             = vcopen(0, Vcrdb, "fsep=[.].rsep=[\\012]", detbl, VC_DECODE)))
        terror("vcopen(): can't open Vcrdb handles");
    ...
    if (!(envc
          = vcopen(0, Vcrdb, "pad.fsep=[.].rsep=[\\012]", entbl, VC_ENCODE))
        || !(devc = vcopen(
             0, Vcrdb, "pad.fsep=[.].rsep=[\\012]", detbl, VC_DECODE)))
        terror("vcopen(): can't open Vcrdb handles");
    if ((e = vcapply(envc, test, k, &endt)) <= 0)
        terror("vcapply(): fail to encode");
    if ((n = vcapply(devc, endt, e, &dedt)) != k)
