...
= "[-?\n@(#)$Id: troff2html (AT&T Research) 2004-04-26 $\n]" USAGE_LICENSE
  ...
  "	mode) request.]"
  "[+?The generated \bhtml\b has properly nested begin/end tags, even though "
  "most"
  "	browsers don't care.]"
  ...
  "		[+../groff/share/groff/tmac/tmac.package?for all directories on "
  "\b$PATH\b]"
  "}"
  "[r:register?Initializes the number \aregister\a to \aN\a.]:[registerN]"
  "[s:script?Reads \ascript\a as if it came from a file. \b--script='.nr A "
  "123'\b"
...
value(char *name, int type, int i)
{
    ...
    if (np->internal) {
        ...
        case INDEX('.', 'C'):
            ...
            break;
        case INDEX('.', 'F'):
            return state.original.file ? state.original.file
                                       : error_info.file;
        case INDEX('.', 'H'):
            n = 3000;
            break;
        ...
        case INDEX('d', 'l'):
            if (state.test & 0x1000)
                n = 0;
            else
                n = state.dl;
            break;
        case INDEX('d', 'n'):
            if (state.test & 0x2000)
                n = 0;
            else
                n = state.dn;
            break;
        case INDEX('l', 'n'):
            if (state.test & 0x4000)
                n = 0;
            else
                n = state.ln;
            break;
        case INDEX('n', 'l'):
            if (!(state.test & 0x8000))
                n = 0;
            else
                n = state.nl;
            break;
...
li(int force)
{
    ...
    if (state.env->in.current > state.list->in
        ...
        state.list->ti = state.env->in.current > state.list->in
                         ? state.env->in.current
...
groff_write(Tag_t *tp, Arg_t *ap)
{
    ...
    if (sfputr(sp->sp, s, '\n') < 0)
        error(
        ERROR_SYSTEM | 2, "%s: %s: write error", ap->argv[0], ap->argv[1]);
}
...
troff_di(Tag_t *tp, Arg_t *ap)
{
    ...
    if (!ap->argc) {
        if (dp = state.divert) {
            ...
            if ((n = sfstrtell(state.out)) > 0
                && *(sfstrbase(state.out) + n - 1) != '\n')
                sfputc(state.out, '\n');
            set(dp->tag, popout(), 0);
            message(
            (-2, "%s: pop diversion `%s'", dp->tag->name, dp->tag->body));
            sfstrclose(dp->sp);
            free(dp);
        }
...
troff_ie(Tag_t *tp, Arg_t *ap)
{
    ...
    if (s = join(ap, 1)) {
        ...
        for (;;) {
            ...
            if (v || s[0] != state.env->cc && s[0] != state.env->c2
                || s[1] != 'i')
                break;
            q = f;
            if (s[2] == 'e')
...
troff_sp(Tag_t *tp, Arg_t *ap)
{
    ...
    code_1((ap->argc < 1 || expression(ap->argv[1], NiL, 'v') > 0) ? OP_p
                                                                   : OP_br);
}
...
troff_ti(Tag_t *tp, Arg_t *ap)
{
    ...
    li(1);
    if (state.list > state.list_stack
        && state.env->ti.current < state.list->in)
        state.it.dt = 1;
}
...
static Option_t options[] = { "author",
                              &state.author,
                              0,
                              "background",
                              &state.background,
                ...
                if (tp) {
                    ...
                    default:
                        if (error_info.trace >= 0)
                            error(
                            1, "%s: unknown request", state.tag->argv[0]);
                        sfprintf(
                        state.tmp, "UNKNOWN REQUEST %s", join(state.tag, 0));
            ...
            escape:
                ...
                case '}':
                    if (state.end
                        ...
                        goto passthrough;
                    if (state.cond.flags[state.cond.level]
                        & (COND_EL | COND_IF))
                        state.cond.level--;
                    else
                        state.cond.flags[state.cond.level] &= ~COND_BLOCK;
...
static Var_t vars[] = {
    "(**", "*",    "(+-", "\261", "(ap", "~",   "(bu", "\267", "(bv", "|",
    "(co", "\251", "(dg", "\247", "(fm", "'",   "(lq", "``",   "(rg", "\256",
    "(rq", "''",   "(sq", "\244", "eEQ", ".EN", "eTS", ".TE",  "eig", "..",
    "f",   "",     "f1",  "1",    "f2",  "2",   "f3",  "3",    "f5",  "5",
    "fB",  "3",    "fCW", "5",    "fF",  "5",   "fI",  "2",    "fL",  "5",
    "fM",  "5",    "fR",  "1",    "fX",  "5",
};
                ...
                for (;;) {
                    ...
                    case CODE_1:
                        ...
                        case OP_dl:
                            *(v - 2) = *(v - 1) = CODE_0;
                            tag(
                            op, OP_dl, STACK | LINE | ARG_compact, 0, NiL, 0);
                            if (!label)
                                break;
                            continue;
                        case OP_dd:
        ...
        case ':':
            ...
            if (!(v = strchr(argv[opt_info.index - 1], '=')))
                v = opt_info.name + 2;
                     ...
                     "%s=%s",
                     opt_info.name + 2,
                     fmtquote(v + 1, "\"", "\"", strlen(v + 1), FMT_ALWAYS))))
                error(ERROR_SYSTEM | 3, "out of space");
