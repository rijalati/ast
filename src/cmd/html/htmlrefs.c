...
= "[-?\n@(#)$Id: htmlrefs (AT&T Research) 2012-01-01 $\n]" USAGE_LICENSE
  ...
  "[d:dependents?List each selected local file followed by \b:\b and a list "
  "of"
  ...
  "[l:limit?Limit \b--copy\b and \b--remove\b operations to path names "
  "matching"
  "	\apattern\a.]:[pattern]"
        ...
        if (state->hosts.size)
        {
            ...
            if (*s == '/')
            {
                if (ref && !streq(s, ref->name))
                {
                    ...
                    if (*(s + 2) == '/')
                        s += 2;
                    else if (!state->user.size
                             || !strneq(
                                s + 2, state->user.data, state->user.size)
                             || *(s + 2 + state->user.size) != '/')
                        return 0;
                    else
                        s += 2 + state->user.size;
    ...
    for (;;)
    {
                                ...
                                else if (f)
                                {
                                                ...
                                                && (v = &state->internal))
                                    {
                                        ...
                                        if (t = strrchr(p, '/'))
                                            *t = 0;
                                        m
                                        = v->size + strlen(s) + strlen(p) + 6;
                                        if (!(v->data
                                              = newof(v->data, char, m, 0)))
                                            error(
                                            ERROR_SYSTEM | 3,
                                            "out of space [path pattern]");
                                        v->size
                                        += sfsprintf(v->data + v->size,
                                                     m,
                                                     "%s%s%s?(/*)",
                                                     v->size ? "|" : "",
                                                     s,
        ...
        case '[':
            if (perlwarn && (c = sfgetc(ip)) != EOF)
            {
                ...
                case '#':
                    error(
                    1, "%s: file contains embedded perl constructs", path);
                    perlwarn = 0;
                    break;
                }
            }
...
main(int argc, char **argv)
{
    ...
    if (state->copy.size
        && (stat(state->copy.data, &st) || !S_ISDIR(st.st_mode)))
            ...
            if (!(fp->flags & (CHECKED | COPIED | MISSING)))
            {
                ...
                else if (state->force || st.st_mtime != ts.st_mtime)
                {
                    ...
                    else
                    {
                        ...
                        if (n < 0)
                            error(
                            ERROR_SYSTEM | 2, "%s: read error", fp->name);
                        if (sfclose(op))
                        ...
                        if (n > 0)
                        {
                            if ((st.st_mode &= S_IPERM)
                                ...
                                && chmod(p, st.st_mode))
                                error(
                                ERROR_SYSTEM | 2, "%s: cannot set mode", p);
                        ...
                        else if (!n)
                        {
                            if (state->verbose)
                                sfprintf(sfstdout,
                                         " %s %s\n",
                                         (fp->flags & DIRECTORY) ? "rmdir"
                                                                 : "   rm",
                                         p);
                            if (((fp->flags & DIRECTORY) ? rmdir : remove)(p))
                                error(
                                ERROR_SYSTEM | 2, "%s: cannot remove", p);
                        }
                    }
                }
            }
        if (state->unreferenced)
        {
            if (!(fts = fts_open(( char ** )state->copy.data,
                ...
                error(ERROR_SYSTEM | 3,
                      ...
                      state->copy.data);
            while (ent = scan(state, fts))
                if ((!(fp = dtmatch(state->files, ent->fts_path))
                     || !(fp->flags & COPIED))
                    && (!state->ignore.size
                        ...
                        || strmatch(ent->fts_path, state->limit.data)))
                {
                    if (state->verbose || !state->exec)
                        sfprintf(sfstdout,
                                 " %s %s\n",
                                 (ent->fts_info & FTS_D) ? "rmdir" : "   rm",
                                 ent->fts_path);
                    if (state->exec
                        && ((ent->fts_info & FTS_D) ? rmdir
                                                    : remove)(ent->fts_path))
                        error(
                        ERROR_SYSTEM | 2, "%s: cannot remove", ent->fts_path);
                }
            if (fts_close(fts))
                error(ERROR_SYSTEM | 3,
                      "%s: directory read error",
    ...
    else if (state->unreferenced)
    {
        ...
        dirs[i] = 0;
        if (!(fts = fts_open(
              dirs, FTS_META | FTS_PHYSICAL | FTS_NOPREORDER, order)))
            error(ERROR_SYSTEM | 3,
                  "%s: cannot search directory",
                  state->root.data);
        while (ent = scan(state, fts))
            if (!dtmatch(state->files, ent->fts_path)
                && (!strmatch(ent->fts_name, state->keep.data)
                    || state->skip.size
                             ...
                             || strmatch(ent->fts_path, state->limit.data))
                    {
                        if (state->verbose || !state->exec)
                            sfprintf(sfstdout,
                                     ...
                                     ent->fts_path);
                        if (state->exec
                            && ((ent->fts_info & FTS_D) ? rmdir : remove)(
                               ent->fts_path))
                            error(ERROR_SYSTEM | 2,
                                  "%s: cannot remove",
                                  ent->fts_path);
