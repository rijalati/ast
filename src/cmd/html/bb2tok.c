...
static const Header_t header[] = {
    "a",           "link/",  LINK,   1, "name",     "name",  NAME, 0,
    "postdetails", "header", HEADER, 0, "postbody", "body",  BODY, 0,
    "quote",       "quote",  QUOTE,  0, "code",     "code",  CODE, 0,
    "genmed",      "label",  LABEL,  0, "line",     "line/", LINE, 1,
};
      ...
      int push)
{
    if (!head)
    {
        if (state->keep)
        {
            ...
            if (state->prev)
            {
                ...
                "%s<%s%s>\n",
                (!state->push
                 && (state->prev->lex == HEADER
                     || state->prev->lex == CODE && state->last != '\n'))
                ? "\n"
                : "",
                state->push ? "" : "/",
                state->prev->out);
    ...
    else if (push)
    {
        if (state->prev)
        {
            ...
            if (state->keep
                && (state->prev->lex != head->lex
                    || !head->unary && state->push))
                sfprintf(
                op,
                "%s<%s%s>\n",
                (!state->push
