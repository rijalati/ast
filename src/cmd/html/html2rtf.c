...
= "[-?\n@(#)$Id: html2rtf (AT&T Research) 1999-01-01 $\n]" USAGE_LICENSE
  ...
  "[p:project-file?Appends MS HELP project information to the help project "
  "file"
...
start_li(Tag_t *tp, Attribute_t *ap)
{
    ...
    default:
        sfputr(state.out,
               state.sp && state.sp->list_label ? state.sp->list_label
                                                : "\\bullet",
               -1);
        break;
    }
...
start_ol(Tag_t *tp, Attribute_t *ap)
{
    ...
    if (attribute(ap, "COMPACT"))
        state.sp->flags |= STK_LIST_COMPACT;
    if (!(op = attribute(ap, "START")) || !op->value
        || (state.sp->list_counter = strtol(op->value, &e, 10)) < 0 || *e)
        state.sp->list_counter = 1;
    state.sp->list_type
    = (op = attribute(ap, "TYPE")) && op->value ? *op->value : '1';
    state.sp->list_hanging = state.hanging;
    state.sp->list_indent = state.indent;
    state.hanging += LIST_INDENT;
...
start_th(Tag_t *tp, Attribute_t *ap)
{
    ...
    NoP(tp);
    if (!(op = attribute(ap, "ALIGN")) || !op->value
        || *op->value != 'l' && *op->value != 'L')
        sfputr(state.out, "\\~\\~\\~\\~\\~\\~\\~\\~\\~\\~\\~\\~", -1);
    return 0;
}
                ...
                if (!(tp = ( Tag_t * ) hashget(state.tags, s)))
                    ...
                    if (tp->end) {
                        if (state.sp >= state.sp_max) {
                            ...
                            if (!(state.sp_min
                                  = oldof(state.sp_min, Stack_t, n, 0)))
                                error(ERROR_SYSTEM | 3,
                                      "out of space [tag stack]");
...
static const Entity_t entities[] = {
    "AElig",  "\\'c6",    0, "Aacute",  "\\'c1",     0,
    "Acirc",  "\\'c2",    0, "Agrave",  "\\'c0",     0,
    "Aring",  "\\'c5",    0, "Atilde",  "\\'c3",     0,
    "Auml",   "\\'c4",    0, "Ccedil",  "\\'c7",     0,
    "ETH",    "\\'d0",    0, "Eacute",  "\\'c9",     0,
    "Ecirc",  "\\'ca",    0, "Egrave",  "\\'c8",     0,
    "Euml",   "\\'cb",    0, "Iacute",  "\\'cd",     0,
    "Icirc",  "\\'ce",    0, "Igrave",  "\\'cc",     0,
    "Iuml",   "\\'cf",    0, "Ntilde",  "\\'d1",     0,
    "Oacute", "\\'d3",    0, "Ocirc",   "\\'d4",     0,
    "Ograve", "\\'d2",    0, "Oslash",  "\\'d8",     0,
    "Otilde", "\\'d5",    0, "Ouml",    "\\'d6",     0,
    "THORN",  "\\'de",    0, "Uacute",  "\\'da",     0,
    "Ucirc",  "\\'db",    0, "Ugrave",  "\\'d9",     0,
    "Uuml",   "\\'dc",    0, "Yacute",  "\\'dd",     0,
    "aacute", "\\'e1",    0, "acirc",   "\\'e2",     0,
    "acute",  "\\'b4",    0, "aelig",   "\\'e6",     0,
    "agrave", "\\'e0",    0, "amp",     "&",         0,
    "aring",  "\\'e5",    0, "atilde",  "\\'e3",     0,
    "auml",   "\\'e4",    0, "brvbar",  "\\'a6",     0,
    "ccedil", "\\'e7",    0, "cedil",   "\\'b8",     0,
    "cent",   "\\'a2",    0, "copy",    "\\'a9",     0,
    "curren", "\\'a4",    0, "deg",     "\\'b0",     0,
    "divide", "\\'f7",    0, "eacute",  "\\'e9",     0,
    "ecirc",  "\\'ea",    0, "egrave",  "\\'e8",     0,
    "emdash", "\\emdash", 0, "emspace", "\\emspace", ENT_SPACE,
    "endash", "\\endash", 0, "enspace", "\\enspace", ENT_SPACE,
    "eth",    "\\'f0",    0, "euml",    "\\'eb",     0,
    "frac12", "\\'bd",    0, "frac14",  "\\'bc",     0,
    "frac34", "\\'be",    0, "gt",      ">",         0,
    "iacute", "\\'ed",    0, "icirc",   "\\'ee",     0,
    "iexcl",  "\\'a1",    0, "igrave",  "\\'ec",     0,
    "iquest", "\\'bf",    0, "iuml",    "\\'ef",     0,
    "laquo",  "\\'ab",    0, "lt",      "<",         0,
    "macr",   "\\'af",    0, "micro",   "\\'b5",     0,
    "middot", "\\bullet", 0, "nbsp",    "\\~",       ENT_SPACE,
    "not",    "\\'ac",    0, "ntilde",  "\\'f1",     0,
    "oacute", "\\'f3",    0, "ocirc",   "\\'f4",     0,
    "ograve", "\\'f2",    0, "ordf",    "\\'aa",     0,
    "ordm",   "\\'ba",    0, "oslash",  "\\'f8",     0,
    "otilde", "\\'f5",    0, "ouml",    "\\'f6",     0,
    "para",   "\\'b6",    0, "plusmn",  "\\'b1",     0,
    "pound",  "\\'a3",    0, "quot",    "\"",        0,
    "raquo",  "\\'bb",    0, "reg",     "\\'ae",     0,
    "sect",   "\\'a7",    0, "shy",     "\\'ad",     0,
    "sup1",   "\\'b9",    0, "sup2",    "\\'b2",     0,
    "sup3",   "\\'b3",    0, "szlig",   "\\'df",     0,
    "thorn",  "\\'fe",    0, "times",   "\\'d7",     0,
    "uacute", "\\'fa",    0, "ucirc",   "\\'fb",     0,
    "ugrave", "\\'f9",    0, "uml",     "\\'a8",     0,
    "uuml",   "\\'fc",    0, "yacute",  "\\'fd",     0,
    "yen",    "\\'a5",    0, "yuml",    "\\'ff",     0,
#if 0
	"trademark",	"",		0,
#endif
...
init(void)
{
    ...
    if (state.project
        && !(state.files = hashalloc(
             state.tags, HASH_set, HASH_ALLOCATE, HASH_name, "files", 0)))
