...
static void
workload(Dt_t *dt, Proc_t *proc, int p)
{
    Obj_t *os, * or ;
    ssize_t k, s;
    pid_t pid = getpid();
    ...
    for (k = 0; k < proc->objn; ++k)
    {
        ...
        if ((os = dtinsert(dt, or)) != or)
            tinfo(
            "\t\tProcess %d(%d): Insert %s, get %0x", p, pid, or->str, os);
        else
            os->flag |= INSERT;
        if ((os = dtsearch(dt, or)) != or)
            tinfo("\t\tProcess %d(%d): Just inserted %s but not found",
                  p,
                  pid,
                  or->str);
        Icount += 1;
    ...
    asoincint(
    &State->idone); /* signaling that this workload has been inserted */
    while (asogetint(&State->idone)
           > 0) /* wait until parent signal ok to continue */
        usleep(100);
    ...
    for (k = 0; k < proc->objn; ++k)
    {
        ...
        if (k > SEARCH)
        {
            for (s = 0; s < SEARCH; ++s)
            {
                ...
                if (dtsearch(dt, or))
                    tinfo(
                    "\t\tProcess %d(%d): Search %s !NULL", p, pid, or->str);
            }
        }
    }
...
tmain()
{
    ...
    tinfo(
    "\ttsafehash: Insertion #procs=%d (free shared mem=%d)", k, Disc->size);
    for (k = 0;; asorelax(1 << k), k = (k + 1) & 07)
    ...
    for (k = 0;; asorelax(1 << k), k = (k + 1) & 07)
        if (asogetint(&State->ddone)
            == N_PROC) /* wait until all are deleted */
            break;
    if (dtfirst(dt))
