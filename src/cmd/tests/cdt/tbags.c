...
tmain()
{
    ...
    for (meth = 0; meth < 4; ++meth)
    {
        ...
        for (k = 0; k < R_OBJ; ++k)
            for (i = 0; i < N_OBJ / R_OBJ; ++i)
            {
                ...
                o = (meth == 3 || i % 2 == 0) ? dtappend(dt, obj)
                                              : dtinsert(dt, obj);
                if (o != obj)
                    terror("%s: dtappend (key=%d,ord=%d) failed",
                           name,
                           obj->key,
                           obj->ord);
            }
        ...
        { /* note that dtappend() was used to keep objects in order of
             insertion */
            if (!(o = dtatmost(dt, &proto)))
                terror("%s: dtatmost (key=%d) failed", name, mid);
            if (o->ord != 0)
                terror(
                "%s: dtatmost (key=%d) but ord=%d > 0", name, o->key, o->ord);

            if (!(o = dtatleast(dt, &proto)))
                terror("%s: dtatleast (key=%d) failed", name, mid);
            if (o->ord != R_OBJ - 1)
                terror("%s: dtatleast (key=%d) but ord=%d > 0",
                       name,
                       o->key,
                       o->ord);
        ...
        if (meth == 0) /* testing ordering properties of Dtobag */
        {
            ...
            if (n_obj != R_OBJ)
                terror("%s: dtatmost/dtnext count n_obj=%d != %d",
                       name,
                       n_obj,
                       R_OBJ);
            ...
            if (n_obj != R_OBJ)
                terror("%s: dtatleast/dtprev count n_obj=%d != %d",
                       name,
                       n_obj,
                       R_OBJ);
        ...
        if (n_obj != N_OBJ)
            terror(
            "%s: Walk flattened list n_obj=%d != %d", name, n_obj, N_OBJ);
        if (n_mid != R_OBJ)
            terror(
        ...
        for (i = 0; i < N_OBJ - 1; i += R_OBJ)
        {
            ...
            if ((o = dtremove(dt, obj)) == obj)
                ...
                terror(
                "%s: dtremove (key=%d,ord=%d) wrongly yielded "
                "(key=%d,ord=%d)",
                name,
                obj->key,
                obj->ord,
            ...
            if ((o = dtdelete(dt, obj)) != NIL(Obj_t *))
                ...
                terror(
                "%s: dtdelete matching object to (key=%d,ord=%d) failed",
                name,
                obj->key,
                obj->ord);
        }
        ...
        dtcustomize(
        dt, DT_OPTIMIZE, 1); /* balance the tree in Dtobag to reduce depth */
        if (dtstat(dt, &stat) < 0)
            terror("%s: Couldn't get statistics", name);
        if (stat.size != n_obj)
            terror(
            "%s: stat.size=%d != %d (actual size)", name, stat.size, n_obj);
