...
static Dt_t *
opendictionary(int num, pid_t pid, char *store, int type)
{
    Vmalloc_t *vm;
    Vmdisc_t *vmdc;
    Dt_t *dt;
    ...
    if (!(vmdc = vmdcshare(store, proj, MEMSIZE, type)))
        terror("[store=%s]: Failed creating discipline for shared memory",
               store);
    ...
    if (!(dt = dtopen(&mmdc->disc, TMETHOD)))
        terror("Process[num=%d,pid=%d]: Can't open dictionary for %s",
               num,
               pid,
               store);
...
pingpong(char *procnum)
{
    ...
    for (k = 0; p < N_CONCUR; asorelax(1 << k),
        k = (k + 1) & 07) /* wait until all inserters are running */
...
tmain()
{
    ...
    for (k = 0; k < N_OBJ; ++k) {
        if (random() % 2 == 0) {
            ...
            if (dtinsert(mapdt, om) != om)
                terror("Parent[pid=%d]: dtinsert failed k=%d store=%s",
                       ppid,
                       k,
                       Mapstore);
        } else {
    ...
    if ((k + p) != N_OBJ)
        terror("Parent[pid=%d]: expecting %d objects but (mapdt+shmdt)=%d",
               ppid,
               N_OBJ,
               k + p);
    ...
    for (k = 0; k < N_OBJ; ++k) {
        ...
        if (om && os)
            terror(
            "Parent[pid=%d]: object %d is in both dictionaries", ppid, k);
        if (!om && !os)
            terror(
            "Parent[pid=%d]: object %d is in neither dictionary", ppid, k);
    }
