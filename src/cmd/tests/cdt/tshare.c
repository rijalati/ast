...
#if __STD_C
static int
event(Dt_t *dt, int type, Void_t *obj, Dtdisc_t *disc)
#else
static int event(dt, type, obj, disc) Dt_t *dt;
int type;
Void_t *obj;
Dtdisc_t *disc;
#endif
...
#if __STD_C
static Void_t *
memory(Dt_t *dt, Void_t *buf, size_t size, Dtdisc_t *disc)
#else
static Void_t *memory(dt, buf, size, disc) Dt_t *dt;
Void_t *buf;
size_t size;
Dtdisc_t *disc;
#endif
...
Dtdisc_t Disc = { 0,       sizeof(long), -1,     newint, NIL(Dtfree_f),
                  compare, hashint,      memory, event };
