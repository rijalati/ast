...
tmain()
{
    ...
    for (i = ( long )dtlast(dt), k = 7; k >= 1;
         i = ( long )dtprev(dt, i), k -= 1)
        if (i != k)
    ...
    for (i = 1; i < 20000; ++i)
        if (( long )dtinsert(dt, i) != i)
            terror("Can't insert");
    dtmethod(dt, Dtoset);
    for (i = 1, k = ( long )dtfirst(dt); i < 20000;
         ++i, k = ( long )dtnext(dt, k))
        if (i != k)
            terror("Bad value");
    ...
    for (i = 0, k = ( long )dtfirst(dt); k != 0;
         i += 1, k = ( long )dtnext(dt, k))
        count[k] += 1;
    for (i = 1; i < 100; ++i)
        if (count[i] != 1)
