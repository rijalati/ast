...
Dtdisc_t Disc = { 0,       sizeof(long),    -1,
                  newint,  NIL(Dtfree_f),   compare,
                  hashint, NIL(Dtmemory_f), NIL(Dtevent_f) };

Dtdisc_t Rdisc = { 0,       sizeof(long),    -1,
                   newint,  NIL(Dtfree_f),   rcompare,
                   hashint, NIL(Dtmemory_f), NIL(Dtevent_f) };
...
tmain()
{
    ...
    if ((g = ( char * ) dtinsert(dt, x) - ( char * ) 0) != x)
        terror("Insert -- expected %ld, got %ld", x, g);
    for (k = 0, i = ( long ) dtfirst(dt); i;
         k = i, i = ( long ) dtnext(dt, i))
        if (i < k)
    ...
    for (i = 0; i <= 5; ++i)
        if (count[i] != i)
            terror(
            "dtnext count failed -- expected %d, got %ld", i, count[i]);
    for (i = 0; i <= 5; ++i)
        count[i] = 0;
    ...
    for (i = 0; i <= 5; ++i)
        if (count[i] != i)
            terror(
            "dtprev count failed -- expected %d, got %ld", i, count[i]);
    for (k = 0, i = ( long ) dtfirst(dt); i;
         k = i, i = ( long ) dtnext(dt, i))
        if (i < k)
            terror("Disorder %ld >= %ld", k, i);
    ...
    for (k = 0, i = ( long ) dtatmost(dt, 5L); i == 5;
         i = ( long ) dtnext(dt, i))
        k += 1;
    if (k != 10)
        terror("Did not see all 5's k=%d", k);

    for (k = 0, i = ( long ) dtatleast(dt, 3L); i == 3;
         i = ( long ) dtprev(dt, i))
        k += 1;
    if (k != 10)
