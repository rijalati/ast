...
infof(Opt_t *op, Sfio_t *sp, const char *s, Optdisc_t *dp)
{
    ...
    if (streq(s, "options"))
        return sfprintf(
        sp,
        "[Z:zoom?Do it as fast as possible.]\fmore#1\f\fmore#2\f[B:boom?Dump "
        "into \afile\a.]:[file]");
    if (streq(s, "zero"))
    ...
    if (streq(s, "more#4"))
        return sfprintf(
        sp, "[F:fudge?Fudge the statistics to satisfy everyone.]");
    if (streq(s, "more#5"))
        return sfprintf(sp,
                        "\bred\b, \borange\b, \byellow\b, \bgreen\b, "
                        "\bblue\b, \bindigo\b, \bviolet\b");
    if (streq(s, "more#6"))
...
main(int argc, char **argv)
{
    ...
    if (!(command = *++argv) || !(usage = *++argv))
        error(ERROR_USAGE | 4,
              "[ - | + usage ... ] command-name usage-string [ arg ... ]");
    argv += str;
    ...
    for (;;) {
        for (;;) {
            ...
            if (n == '?') {
                ...
                sfprintf(sfstdout,
                         "return=%d option=%s name=%s arg%-.1s=%s num=%I*d\n",
                         n,
                         opt_info.option,
                         opt_info.name,
                         &opt_info.assignment,
