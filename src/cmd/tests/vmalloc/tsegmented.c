...
** The allocation method will have to handle the fact that these are not
*contiguous.
*/
...
static Void_t *
segmem(Vmalloc_t *vm, Void_t *ca, size_t cs, size_t ns, Vmdisc_t *dc)
{
    if (ns > SEGZ)
        return NIL(Void_t *);
    else if (ca && cs > 0)
...
#define NBLOCK (SEGZ / (ZBLOCK + 64))
    {
        ...
        for (n = 0; n < NBLOCK;
             n += 2) /* free all blocks to remerge free areas */
            vmfree(vm, blk[n]);
        if (!(big = vmalloc(vm, SEGZ / 4)))
            terror("vmalloc failed");
