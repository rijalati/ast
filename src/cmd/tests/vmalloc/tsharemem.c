...
static int
working(char *store, char *typeascii, char *pnumascii, ssize_t size)
{
    int type, pnum;
    ssize_t k, f;
    Vmdisc_t *dc;
    ...
    tinfo("Process %d[pid=%d]: simulation starting with store %s",
          pnum,
          pid,
          store);
    ...
    tinfo(
    "Process %d[pid=%d]: (n_alloc=%d, n_free=%d, n_busy=%d) simulation "
    "completed",
    pnum,
    pid,
    n_alloc,
...
static pid_t
makeprocess(char *program, char *store, int type, int pnum)
{
    pid_t pid;
...
tmain()
{
    ...
    for (type = -1; type < 2; type += 2) /* -1 is mmap, 1 is shm */
    {
        ...
        if (type < 0)
            ...
            tinfo("Test for shm shared memory passed");
    }
