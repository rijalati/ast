...
void *
emphemeral(void *arg) /* an empheral thread, allocate a few times, free then
                         return */
{
    ssize_t k, sz;
    void *blk, *emp;
...
tmain()
{
    ...
    if (Empperiod > 0)
    {
        ...
        if (!(Emp = malloc(sz)))
            terror(
            "Failed allocating shared list of emphemeral objects nalloc=%d",
            nalloc);
        memset(Emp, 0, sz);
        Curbusy += sz;
    }
    ...
    tinfo("Nstep=%d Nthread=%d ProbSmall=%d, Emphe(p=%d,n=%d)",
          ...
          Empcount);
    tinfo(
    "\tSmall(min=%d,max=%d,life=%d,probre=%d) "
    "Large(min=%d,max=%d,life=%d,probre=%d)",
    Smalllo,
    Smallhi,
    Smalllf,
    Smallre,
