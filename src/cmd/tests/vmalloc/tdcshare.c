...
tmain()
{
    ...
    if (k = tchild()) {
        ...
        tinfo(
        "Child[pid=%d]: allocating heap memory before opening shm region",
        cpid);
        for (k = 0; k < ALLOCN; ++k)
        ...
        if (!(dc = vmdcshare(shmfile, 1, MAPSIZE, 0))
            ...
            terror("Child[pid=%d]: Can't open shm region in child process",
                   cpid);
        tinfo("Child[pid=%d]: shm region opened", cpid);

        tinfo(
        "Child[pid=%d]: allocating heap memory before opening map region",
        cpid);
        for (k = 0; k < ALLOCN; ++k)
            if (!(addr = malloc(ALLOCZ)))
                terror("Child[pid=%d]: Can't allocate segment %d", cpid, k);
        ...
        if (!(dc = vmdcshare(shmfile, 1, MAPSIZE, -1))
            || !(shm = vmopen(dc, Vmbest, 0)))
            terror(
            "Parent[pid=%d]: Can't create shm region in parent process",
            ppid);
        ...
        if (!(dc = vmdcshare(mapfile, -1, MAPSIZE, -1))
            || !(map = vmopen(dc, Vmbest, 0)))
            terror(
            "Parent[pid=%d]: Can't create map region in parent process",
            ppid);
