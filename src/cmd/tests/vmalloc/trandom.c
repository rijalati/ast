...
#define ALLOCSIZE()                                                          \
    (RAND() % 100 == 0                                                       \
     ? (RAND() % Z_BIG + Z_BIG / 2)                                          \
     : RAND() % 10 == 0 ? (RAND() % Z_MED + Z_MED) : (RAND() % Z_TINY + 1))
...
tmain()
{
    ...
    for (k = 0; k < N_OBJ; ++k)
    {
        ...
        for (o = List[k]; o; o = next)
        {
            ...
            else /* resizing */
            {
                o->size = ALLOCSIZE();
                if (!(o->obj
                      = vmresize(Vmregion, o->obj, o->size, VM_RSMOVE)))
    ...
    if (vmstat(Vmregion, &sb) < 0)
        terror("Vmstat failed");
    tinfo(
    "After simulation loop: Busy=(%u,%u) Free=(%u,%u) Extent=%u #segs=%d",
    sb.n_busy,
    sb.s_busy,
    sb.n_free,
    sb.s_free,
    ...
    if (vmstat(Vmregion, &sb) < 0)
        terror("Vmstat failed");
    tinfo(
    "Allocating a small block: Busy=(%u,%u) Free=(%u,%u) Extent=%u #segs=%d",
    sb.n_busy,
    sb.s_busy,
    sb.n_free,
    sb.s_free,
