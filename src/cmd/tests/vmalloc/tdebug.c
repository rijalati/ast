...
#define AFTER 7
    ...
    if ((addr[2] = vmresize(vm, addr[2], 256, VM_RSMOVE | VM_RSCOPY))
        == NIL(Void_t *))
    ...
    if (vmresize(vm, ( Void_t * ) 3, 256, VM_RSMOVE | VM_RSCOPY)
        != NIL(Void_t *))
    ...
    vmfree(vm, addr[3]);
    if ((addr[3] = vmresize(vm, addr[3], 256, VM_RSMOVE | VM_RSCOPY))
        != NIL(Void_t *))
    ...
    if ((addr[4] = vmresize(vm, addr[4], 256, VM_RSMOVE | VM_RSCOPY))
        == NIL(Void_t *))
        terror("Failed resizing");
    addr[4][256 + AFTER] = 0;
