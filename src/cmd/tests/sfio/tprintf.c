...
#else
int coordprint(f, v, fe) Sfio_t *f;
Void_t *v;
Sffmt_t *fe;
#endif
{
...
#else
int nulprint(f, val, fe) Sfio_t *f;
Void_t *val;
Sffmt_t *fe;
#endif
{
...
#else
int shortarg(f, val, fe) Sfio_t *f;
Void_t *val;
Sffmt_t *fe;
#endif
{
...
#else
int transarg(f, val, fe) Sfio_t *f;
Void_t *val;
Sffmt_t *fe;
#endif
{
...
#if __STD_C
void
stkprint(char *buf, int n, char *form, ...)
#else
void stkprint(buf, n, form, va_alist) char *buf;
int n;
char *form;
va_dcl
#endif
...
tmain()
{
    ...
    sfsprintf(
    buf1, sizeof(buf1), "%4d %4o %4x %4o %4x %s", 10, 11, 12, 11, 10, "abc");
    sfsprintf(
    buf2, sizeof(buf2), "%!%2$4d %3$4O %4$4X %3$4O %2$4x %5$s", &fe);
    if (strcmp(buf1, buf2) != 0)
    ...
    fe.extf = shortarg;
    sfsprintf(
    buf1, sizeof(buf1), "%hu %ho %hi %hu %ho %hd", -2, -1, 0, 1, 2, 3);
    ...
    sfsprintf(
    buf2, sizeof(buf2), "%!%D %O %F %S %C", &fe, -1, -1, -1., "s", 'c');
    if (strcmp(buf1, buf2) != 0)
    ...
    if (buf2[0] != 1 || buf2[1] != 0 || buf2[2] != 2 || buf2[3] != 0
        || buf2[4] != 3)
    ...
    if (strcmp(buf1,
               "1345 1234 11145401322 322 3e+02 -0.01 0.01 1e-05 1 -1    |"))
    ...
    if ((i = sfaprints(&s, "%d", 123)) != 3 || strcmp(s, "123") != 0)
        terror(
        "sfaprints() failed -- expected \"123\" [3], got \"%s\" [%d]", s, i);
    free(s);
