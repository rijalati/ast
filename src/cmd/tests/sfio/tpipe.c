...
tmain()
{
    ...
    if (sfnew(sfstdout, buf, sizeof(buf), SF_UNBOUND, SF_STRING | SF_WRITE)
        != sfstdout)
    ...
    SYNC if (!(s = sfreserve(fr, 4, 0))) terror("sfreserve failed");
    if (to)
        alarm(0);
    ...
    close(fd[0]);
    if (!(fw = sfnew(
          NIL(Sfio_t *), NIL(Void_t *), sizeof(buf), fd[1], SF_WRITE)))
        terror("Can't open stream");
