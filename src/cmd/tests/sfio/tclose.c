...
#if __STD_C
static int
except(Sfio_t *f, int type, Void_t *obj, Sfdisc_t *disc)
#else
static int except(f, type, obj, disc) Sfio_t *f;
int type;
Void_t *obj;
Sfdisc_t *disc;
#endif
...
static Sfdisc_t Wdisc
= { NIL(Sfread_f), NIL(Sfwrite_f), NIL(Sfseek_f), except };
    ...
    if (argc > 1) /* to act as a coprocess that read/write ten lines */
    {
        ...
        for (i = 0; i < n; ++i)
        {
            ...
            if ((rv = sfputr(sfstdout, s, '\n')) != sfvalue(sfstdin))
                terror(
                "Failed to write rv=%d stdin=%d", rv, sfvalue(sfstdin));
        }
        sfsync(sfstdout);
        texit(0);
    }
