...
#if __STD_C
static void
tstputmesg(int line, char *form, va_list args)
#else
...
static int
asoerror(int type, const char *mesg)
{
...
#endif
{
    ...
    tstinfo("Parent[pid=%d]: waiting for %d child%s",
            parent,
            nproc,
            nproc == 1 ? "" : "ren");
    while ((pid = wait(&status)) > 0) {
        if (proc) {
            ...
            if (n >= nproc)
                tstwarn("Parent[pid=%d]: process UNKNOWN[pid=%d] status=%d",
                        parent,
                        pid,
                        status);
        }
