                ...
                size_t size)
{
    ...
    if (s[sizeof(VDB_MAGIC) + 1] != '\n') {
        ...
        if (t = strchr(s, '\n'))
            *t = 0;
        strncpy(
        state.volume, s + sizeof(VDB_MAGIC) + 1, sizeof(state.volume) - 2);
    }
    ...
    if (*t++ == vdb->delimiter)
        do {
            ...
            if (strneq(t, VDB_DATE, sizeof(VDB_DATE) - 1))
                ...
                f->st->st_mode
                = (strtol(t + sizeof(VDB_MODE), NiL, 8) & X_IPERM) | X_IFREG;
        } while (t = s);
    f->st->st_dev = 0;
    f->st->st_ino = 0;
    f->st->st_nlink = 1;
...
vdb_putheader(Pax_t *pax, Archive_t *ap, File_t *f)
{
    ...
    if (state.complete)
        complete(
        ap, f, state.record.header ? state.record.headerlen : f->namesize);
    ...
    n = (c == '=') ? ':' : '=';
    sfprintf(vdb->directory,
             "%c%s%c%I*u%c%I*u%c%s%c%I*u%c%s%c0%o\n",
             c,
             f->name,
             c,
             sizeof(ap->io->offset),
...
vdb_putepilogue(Pax_t *pax, Archive_t *ap)
{
    ...
    sfprintf(vdb->directory,
             ...
             VDB_FIXED,
             sizeof(ap->io->offset),
             ap->io->offset + ap->io->count + sizeof(VDB_DIRECTORY),
             VDB_DELIMITER,
             VDB_FIXED,
             sizeof(Sfoff_t),
             sftell(vdb->directory) - sizeof(VDB_DIRECTORY) + VDB_LENGTH + 1);
    paxwrite(pax, ap, sfstrbase(vdb->directory), sfstrtell(vdb->directory));
