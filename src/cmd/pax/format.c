...
getprologue(Archive_t *ap)
{
    ...
    f->skip = 0;
    ap->checkdelta
    = !ap->delta || ap->delta->format && !(ap->delta->format->flags & PSEUDO);
    f->delta.base = 0;
    f->uncompressed = 0;
    f->delta.checksum = 0;
    f->delta.index = 0;
                ...
                if (ap->parent && !state.ordered) {
                    if ((n = open(state.tmp.file,
                        ...
                        error(ERROR_SYSTEM | 3,
                              "%s: cannot create %s base temporary file %s",
                              ap->name,
                              cp->undo[0],
                              state.tmp.file);
                    ops[1]
                ...
                if (ap->parent && !state.ordered) {
                    if (n = procclose(proc))
                        error(3,
                              ...
                              n);
                    if ((ap->io->fd
                         = open(state.tmp.file, O_RDONLY | O_BINARY))
                        < 0)
                        error(ERROR_SYSTEM | 3,
                              "%s: cannot read %s base temporary file %s",
                              ap->name,
                              cp->undo[0],
                              state.tmp.file);
                    if (remove(state.tmp.file))
                        error(ERROR_SYSTEM | 1,
                              "%s: cannot remove %s base temporary file %s",
                              ap->name,
                              cp->undo[0],
                              state.tmp.file);
                } else {
    ...
    if (!(fp = ap->expected)) {
        ...
        for (;;) {
            ...
            while (fp = nextformat(fp))
                if ((fp->flags & ARCHIVE) && fp->getprologue) {
                    ...
                    convert(ap,
                            SECTION_CONTROL,
                            (fp->flags & CONV) ? CC_NATIVE : CC_ASCII,
                            CC_NATIVE);
                    if ((n = (*fp->getprologue)(
                         &state,
                         fp,
                         ap,
                         f,
                         ((fp->flags & CONV) || CC_NATIVE == CC_ASCII) ? buf
                                                                       : cvt,
                         sizeof(buf)))
                        < 0)
                        return 0;
                    ...
                    ap->checksum = ap->old.checksum = ap->memsum
                    = ap->old.memsum = 0;
                }
            if (fp)
                break;
        ...
        if (skipped)
            error(
            1,
            "%s: %I*d byte%s skipped before identifying %s format archive",
            ap->name,
            sizeof(skipped),
            skipped,
            skipped == 1 ? "" : "s",
    ...
    if (ap->volume > 1) {
        if (ap->delta) {
            if (state.operation == (IN | OUT)
                ...
                error(3,
                      "%s: %s archive cannot be multi-volume",
                      ap->name,
                      ap->parent ? "base" : "delta");
            ap->delta = 0;
        }
                ...
                if (s < buf + n) {
                    if (n == BLOCKSIZE) {
                        if (!x && ap->format->event
                            ...
                            if (i)
                                error(1,
                                      "%s: %d junk block%s after volume %d",
                                      ap->name,
                                      i,
                                      i == 1 ? "" : "s",
                                      ap->volume);
    ...
    if (ap->selected > state.selected) {
        ...
        if (ap->delta && (ap->delta->format->flags & DELTA)) {
            if (ap->format->event
                ...
                (*ap->format->event)(
                &state, ap, NiL, NiL, PAX_EVENT_DELTA_EXTEND);
            else
                putinfo(ap, NiL, ap->delta->index + 1, 0);
        }
        if (!ap->format->putepilogue
            || (boundary = (*ap->format->putepilogue)(&state, ap)) <= 0)
...
getkeytime(Archive_t *ap, File_t *f, int index)
{
    ...
    op = &options[index];
    message((-5,
             "getkeytime %s level=%d entry=%d:%d",
             op->name,
             op->level,
             op->entry,
             ap->entry));
...
getheader(Archive_t *ap, File_t *f)
{
    ...
    do {
        ...
        if (!f->name)
            error(3,
                  "%s: %s format entry %d.%d name not set",
                  ap->name,
                  ap->type,
                  ap->volume,
                  ap->entry);
    ...
    if (error_info.trace) {
        ...
        if (f->record.format)
            ...
            *s = 0;
        message((-1,
                 "archive=%s path=%s name=%s entry=%d.%d size=%I*u "
                 "uncompressed=%I*u delta=%c%s",
                 ap->name,
                 f->path,
                 f->name,
                 ap->volume,
                 ...
                 sizeof(f->st->st_size),
                 f->st->st_size,
                 sizeof(f->uncompressed),
                 f->uncompressed,
                 f->delta.op ? f->delta.op : DELTA_nop,
                 s));
    }
#endif
    if (ap->sum > 0) {
...
putheader(Archive_t *ap, File_t *f)
{
    ...
    if (state.install.sp && !f->extended) {
        ...
        if (f->st->st_uid != state.uid
            && ((f->st->st_mode & S_ISUID)
