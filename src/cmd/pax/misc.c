...
getformat(char *name, int must)
{
    ...
    while (fp = nextformat(fp))
        if (!strcasecmp(name, fp->name)
            || fp->match
               && strgrpmatch(
                  name, fp->match, NiL, 0, STR_ICASE | STR_LEFT | STR_RIGHT))
            return fp;
    if (must)
...
ordered(Archive_t *ap, const char *prv, const char *cur)
{
    if (pathcmp(prv, cur) > 0)
        error(
        3, "%s: %s: archive member must appear before %s", ap->name, prv, cur);
}
           ...
           Sflong_t *pn)
{
    ...
    if (fmt->t_str) {
        ...
        if (op->macro && !(op->flags & OPT_DISABLE)) {
            ...
            case OPT_delta_op:
                if (f->uncompressed
                    && (k = (st->st_size * 100) / f->uncompressed) < 100)
                    sfsprintf(s = fmtbuf(32),
                              32,
                              "%c%02d.%1d%%",
...
undoable(Archive_t *ap, Format_t *fp)
{
    ...
    if (!pathpath(cp->undo[0], NiL, PATH_EXECUTE, buf, sizeof(buf))) {
        if (!cp->undotoo[0]
            ...
            error(3,
                  "%s: %s: command required to read compressed archive",
                  ap->name,
                  cp->undo[0]);
