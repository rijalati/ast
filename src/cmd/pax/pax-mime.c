...
mime_getheader(Pax_t *pax, Archive_t *ap, File_t *f)
{
    ...
    if (paxread(
        pax, ap, s = state.tmp.buffer, mime->length + 2, mime->length + 2, 1)
        <= 0
        || memcmp(s, mime->magic, mime->length))
        error(3,
    ...
    else if (*(s += mime->length) == '-' && *(s + 1) == '-')
    {
        ...
        error(
        3,
        "%s: corrupt %s format member header -- separator line not found",
        ap->name,
        ap->format->name);
    f->name = 0;
    for (;;)
    {
        ...
        if (strncasecmp(s, "content-", 8))
            error(3,
                  "%s: corrupt %s format member header",
                  ap->name,
                  ap->format->name);
        if (t = strchr(s, ':'))
            s = t + 1;
        ...
        for (;;)
        {
            ...
            for (; *s == ';' || isspace(*s); s++)
                ;
            if (!f->name && n == 4 && !strncasecmp(t, "name", 4)
                || n == 8 && !strncasecmp(t, "filename", 8))
