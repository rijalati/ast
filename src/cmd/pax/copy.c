...
filein(Archive_t *ap, File_t *f)
{
    ...
    if (f->skip)
            ...
            if (!ap->format->getdata
                ...
                for (c = f->st->st_size; c > 0; c -= n) {
                    ...
                    if (ap->format->checksum)
                        checksum = (*ap->format->checksum)(
                        &state, ap, f, s, n, checksum);
                }
            }
            if (ap->format->checksum && checksum != f->checksum)
                error(1,
                      "%s: %s checksum error (0x%08x != 0x%08x)",
                      f->name,
                      ap->format->name,
                      checksum,
                      f->checksum);
        ...
        case DELTA_verify:
            ...
            else if (st.st_size != f->delta.base->size
                     || state.modtime
                        && tvcmp(tvmtime(&t1, &st), tvmtime(&t2, f->st)))
...
copy(Archive_t *ap, int (*copyfile)(Ftw_t *))
{
    ...
    if (ap) {
        ...
        if (ap->delta && ap->delta->format != ap->expected && ap->expected)
            error(3,
                  "%s: archive format %s does not match requested format %s",
                  ap->name,
        ...
        if (state.append || state.update) {
            ...
            if (!(ap->format->flags & APPEND))
                error(3,
                      "%s: archive format %s does support append/update",
                      ap->name,
                      ap->format->name);
            if (state.update)
