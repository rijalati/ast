       ...
       Sfulong_t number)
{
    ...
    message((-5,
             "putkey %s=%s",
             op->name,
             value ? value : sfstrseek(ap->tmp.key, 0, SEEK_CUR)));
    n
    += strlen(op->name) + 3 + ((op->flags & OPT_VENDOR) ? sizeof(VENDOR) : 0);
    o = 0;
    for (;;) {
...
tar_checksum(Archive_t *ap, int check, unsigned long sum)
{
    ...
    if (!ap->convert[SECTION_CONTROL].on)
        ...
        map = ((ap->mio.mode & O_ACCMODE) == O_WRONLY)
              ? ap->convert[SECTION_CONTROL].f2t
              : ap->convert[SECTION_CONTROL].t2f;
        while (p < e) {
            c = map[*p++];
            u += c;
            if (check) {
    ...
    if (check) {
        ...
        if (sum == (s &= TAR_SUMASK)) {
            if (!ap->old.warned) {
                ap->old.warned = 1;
                error(1,
                      "%s: %s format archive generated with signed checksums",
                      ap->name,
                      ap->format->name);
            }
            return 1;
        ...
        if (ap->entry > 1) {
            if (s != u)
                error(state.keepgoing ? 1 : 3,
                      "%s: %s format checksum error (%ld != %ld or %ld)",
                      ap->name,
                      ap->format->name,
                      sum,
                      u,
                ...
                error(state.keepgoing ? 1 : 3,
                      "%s: %s format checksum error (%ld != %ld)",
                      ap->name,
                      ap->format->name,
                      sum,
                      u);
...
tar_longname(Archive_t *ap, File_t *f)
{
    ...
    case TAR:
        ...
        if (ap->format->variant != PAX && state.strict) {
            error(2,
                  "%s: file base name too long -- %d max",
                  f->name,
                  TARSIZEOF(name));
            f->skip = 1;
        } else
    ...
    toolong:
        error(2,
              "%s: file name too long -- %d max",
              f->name,
              TARSIZEOF(prefix) + TARSIZEOF(name) + !!strchr(f->name, '/'));
        f->skip = 1;
        return -1;
    }
    return 0;
...
synthesize(Archive_t *ap, File_t *f, char *name, int type, char *buf, size_t n)
{
    ...
    ap->sum -= 2;
    initfile(
    ap, &h, &st, name, X_IFREG | X_IRUSR | X_IWUSR | X_IRGRP | X_IROTH);
    h.extended = type;
...
extend(Archive_t *ap, File_t *f, int type)
{
    ...
    if (!f)
        initfile(
        ap, f = &h, &st, ".", X_IFREG | X_IRUSR | X_IWUSR | X_IRGRP | X_IROTH);
    switch (type) {
    case EXTTYPE:
    case TARTYPE:
        sp = ap->tmp.extended;
    ...
    if (pos = hashscan(state.options, 0)) {
        while (hashnext(pos)) {
            ...
            if ((op->flags & (OPT_HEADER | OPT_READONLY)) == OPT_HEADER
                ...
                message((-5,
                         "extend %s level=%d:%d:%d entry=%d:%d "
                         "perm=(%s,%I*d) temp=(%s,%I*d)",
                         op->name,
                         op->level,
                         lev,
                         alt,
                         ...
                         ap->entry,
                         op->perm.string,
                         sizeof(op->perm.number),
                         op->perm.number,
                         op->temp.string,
                         sizeof(op->temp.number),
                         op->temp.number));
                vp = &op->perm;
                s = vp->string;
                switch (op->index) {
                ...
                case OPT_gname:
                    if (op->flags & OPT_SET)
                        f->gidname = s;
                             ...
                             || strlen(f->gidname) < TARSIZEOF(gname)
                                && portable(ap, f->gidname))
                        continue;
                    s = f->gidname;
                    break;
                case OPT_size:
                ...
                case OPT_uname:
                    if (op->flags & OPT_SET)
                        f->uidname = s;
                             ...
                             || strlen(f->uidname) < TARSIZEOF(uname)
                                && portable(ap, f->uidname))
                        continue;
                    s = f->uidname;
                    break;
                default:
    ...
    if (type == TARTYPE)
        ...
        if (type == EXTTYPE) {
            ...
            if (f->linkpath
                && (tar_longlink(ap, f) < 0 || !portable(ap, f->linkpath)))
...
again:
    ...
    if (tar_getoctal(
        "mode", tar->header.mode, 7, sizeof(f->st->st_mode), &f->st->st_mode))
        goto nope;
    if (tar_getoctal(
        "uid", tar->header.uid, 7, sizeof(f->st->st_uid), &f->st->st_uid))
        goto nope;
    if (tar_getoctal(
        "gid", tar->header.gid, 7, sizeof(f->st->st_gid), &f->st->st_gid))
        goto nope;
    if (tar_getoctal("mtime",
                     tar->header.mtime,
                     11,
                     sizeof(f->st->st_mtime),
                     &f->st->st_mtime))
        goto nope;
    ...
    if (!tar_checksum(ap, 1, num) && ap->entry == 1) {
        if (!ap->swapio) {
            ...
            for (i = 1; i < 4; i++) {
                ...
                if (!tar_getoctal(
                    "chksum", tar->header.chksum, 7, sizeof(num), &num)
                    && tar_checksum(ap, 1, num)) {
                    ap->swapio = i;
    ...
    if (tar_getoctal(
        "size", tar->header.size, 11, sizeof(f->st->st_size), &f->st->st_size))
        goto nope;
    if (ap->format->variant != OLD) {
        if (!streq(tar->header.magic, TMAGIC)) {
            ...
            ap->format = &pax_tar_format;
                   ...
                   tar->header.version, TVERSION, TARSIZEOF(version))) {
            tp = &pax_tar_format;
            error(1,
                  "%s: %s format version %-.*s incompatible with "
                  "implementation version %-.*s -- assuming %s",
                  ap->name,
                  ap->format->name,
                  TARSIZEOF(version),
                  tar->header.version,
    ...
    *(( char * )tar->header.name + TARSIZEOF(name)) = 0;
    if (ap->format->variant != OLD && *tar->header.prefix) {
        ...
        sfsprintf(f->name,
                  TARSIZEOF(prefix) + TARSIZEOF(name) + 2,
                  "%-.*s/%-.*s",
                  TARSIZEOF(prefix),
                  tar->header.prefix,
                  TARSIZEOF(name),
    ...
    *(( char * )tar->header.linkname + TARSIZEOF(name)) = 0;
    ...
    case GLBTYPE:
        ...
        if (f->st->st_size > 0) {
            ...
            if (!(s = paxget(pax, ap, f->st->st_size, NiL)))
                error(3,
                      "invalid %s format '%c' extended header",
                      ap->format->name,
                      tar->header.typeflag);
            s[f->st->st_size - 1] = 0;
            setoptions(
            s, f->st->st_size, NiL, state.usage, ap, tar->header.typeflag);
        }
        ap->sum -= 2;
    ...
    case LREGTYPE:
        if ((n = f->st->st_size) > 0) {
            if (!(s = paxget(pax, ap, n, NiL))) {
                error(2,
                      "%s: invalid %s format long path header",
                      ap->name,
                      ap->format->name);
                return 0;
            }
            op = &options[tar->header.typeflag == LLNKTYPE ? OPT_linkpath
                                                           : OPT_path];
            op->level = 6;
            op->entry = ap->entry;
    ...
    case VERTYPE:
        error(
        1,
        "version file archive members not supported -- regular file assumed");
        goto regular;

    default:
        error(
        1,
        "%s: %s: unknown %s format file type `%c' -- regular file assumed",
        ap->name,
        f->name,
        ap->format->name,
        tar->header.typeflag);
...
tar_putheader(Pax_t *pax, Archive_t *ap, File_t *f)
{
    ...
    if (f->extended)
        ...
        case TAR:
            ...
            if ((i = tar_longname(ap, f)) < 0) {
                ...
                synthesize(ap,
                           f,
                           headname(ap, f, "@PaxPathText.%(sequence)s"),
                           LREGTYPE,
                           f->name,
                           f->namesize);
            }
            if (tar_longlink(ap, f) < 0) {
                ...
                synthesize(ap,
                           f,
                           headname(ap, f, "@PaxLinkText.%(sequence)s"),
                           LLNKTYPE,
                           f->linkpath,
                           strlen(f->linkpath) + 1);
            }
    ...
    if (f->extended)
        ...
        linked:
            sfsprintf(tar->header.linkname,
                      TARSIZEOF(linkname) + 1,
                      "%s",
                      f->longlink
                      ? headname(ap, f, "@PaxLinkFile.%(sequence)s")
                      : f->linkpath);
            break;
        case SOFTLINK:
        ...
        default:
            ...
            default:
                if (!f->skip && !f->delta.op)
                    error(
                    1,
                    "%s: %s: unknown file type %07o -- regular file assumed",
                    ap->name,
                    f->name,
                    f->type);
                /*FALLTHROUGH*/
    ...
    sfsprintf(tar->header.devmajor,
              TARSIZEOF(devmajor),
              "%0*o ",
              TARSIZEOF(devmajor) - 1,
              i ? major(idevice(f->st)) : 0);
    sfsprintf(tar->header.devminor,
              TARSIZEOF(devminor),
              "%0*o ",
              TARSIZEOF(devminor) - 1,
              i ? minor(idevice(f->st)) : 0);
    sfsprintf(tar->header.mode,
              TARSIZEOF(mode),
              "%0*o ",
              TARSIZEOF(mode) - 1,
              f->st->st_mode & X_IPERM);
    sfsprintf(tar->header.uid,
              TARSIZEOF(uid),
              "%0*lo ",
              TARSIZEOF(uid) - 1,
          ...
          unsigned long event)
{
    ...
    case PAX_EVENT_SKIP_JUNK:
        ...
        if (!isdigit(hdr->chksum[0]) || !isdigit(hdr->chksum[1])
            ...
            || !isdigit(hdr->chksum[6])
            || tar_getoctal("chksum", hdr->chksum, 7, sizeof(sum), &sum)
            || !tar_checksum(ap, -1, sum))
            return 1;
        return 0;
    }
    return 0;
...
pax_putprologue(Pax_t *pax, Archive_t *ap, int append)
{
    ...
    if (!(ap->data = newof(0, Tar_t, 1, 0)) || !(ap->tmp.global = sfstropen())
        || !(ap->tmp.extended = sfstropen()) || !(ap->tmp.key = sfstropen()))
        nospace();
    if (append)
        return 0;
    ...
    if (ap->delta && !(ap->delta->format->flags & PSEUDO)) {
        ...
        if (ap->delta->base) {
            putkey(ap,
                   ap->tmp.global,
                   &options[OPT_delta_base_size],
                   NiL,
                   ap->delta->base->size);
            putkey(ap,
                   ap->tmp.global,
                   &options[OPT_delta_base_checksum],
                   NiL,
                   ap->delta->base->checksum & 0xffffffff);
        }
    }
    extend(ap, NiL, GLBTYPE);
    return 1;
...
Format_t pax_tar_format = { "oldtar",
                            0,
...
Format_t pax_pax_format = { "pax",
                            0,
                            "POSIX 1003.1-2001 extended ustar",
                            PAX,
                            ARCHIVE | DELTAINFO | LINKTYPE | SLASHDIR
                            | STANDARD | SUM | IN | OUT | APPEND,
                            DEFBUFFER,
                            DEFBLOCKS,
                            BLOCKSIZE,
                            PAXNEXT(pax),
