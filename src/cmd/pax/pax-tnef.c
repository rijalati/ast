    ...
    *(f->name = paxstash(pax, &ap->stash.head, NiL, PATH_MAX)) = 0;
    ...
    while (paxread(pax, ap, s = state.tmp.buffer, ( off_t )0, ( off_t )9, 0)
        ...
        if (size > part) {
            ...
            while (size > part) {
                ...
                if (!(s = paxget(pax, ap, part, NiL)))
                    error(3,
                          "%s: %s format 0x%04x attribute data truncated -- "
                          "%d expected",
                          ap->name,
                          ap->format->name,
                          name,
        ...
        if (!(s = paxget(pax, ap, size + 2, NiL)))
            error(
            3,
            "%s: %s format 0x%04x attribute data truncated -- %d expected",
            ap->name,
            ap->format->name,
            name,
            size);
        ...
        if (checksum != n)
            error(3,
                  "%s: %s format attribute data checksum error",
                  ap->name,
                  ap->format->name);
        if (level == 2)
            switch (name) {
            ...
            case 0x9005: /* attachment */
                    ...
                    while (n-- && s < e) {
                        ...
                        case 0x0102:
                            ...
                            while (m--) {
                                ...
                                if (name == 0x3707)
                                    f->name
                                    = paxstash(pax, &ap->stash.head, s, z);
                                s += z;
                            }
                            break;
                        }
                ...
                if (!*f->name) {
                    ...
                    z = sfsprintf(state.tmp.buffer,
                                  ...
                                  ap->entry);
                    f->name
                    = paxstash(pax, &ap->stash.head, state.tmp.buffer, z);
                }
                return 1;
            }
    }
