    ...
    for (;;) {
        ...
        while (ss->bp >= ss->block + state.blocksize) {
            ...
            if (!(n = swapget(3, ss->bp + BLKHDR_blksiz, 4)))
                ...
                error(
                3, "invalid %s format blocksize=%ld", ap->format->name, n);
            ss->bp += BLKHDR_SIZE;
        }
        ...
        if (i == REC_file) {
            if (header) {
                ...
                if (swapget(1, p, 2) != FILHDR_MAGIC)
                    error(
                    3, "invalid %s format file header", ap->format->name);
                p += 2;
            ...
            while ((c + i) < f->st->st_size && i < ss->lastsize)
                ...
                default:
                    error(state.keepgoing ? 1 : 3,
                          "%s: invalid %s format data record format=%d",
                          f->name,
                          ap->format->name,
                          ss->recfmt);
                    goto next;
...
Format_t pax_vmsbackup_format = { "vmsbackup",
                                  0,
