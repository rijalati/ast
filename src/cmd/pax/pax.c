...
action(const char *command, int pattern)
{
    ...
    if (pattern)
    {
        ...
        if (c = regcomp(re,
                        s,
                        REG_SHELL | REG_AUGMENTED | REG_DELIMITED
           ...
           int type)
{
    ...
    for (;;)
    {
        ...
        case OPT_delete:
            if (y && *v)
                sfprintf(opt.ignore_all,
                         "%s(%s)",
                         sfstrtell(opt.ignore_all) ? "|" : "",
                         v);
            break;
        ...
        case OPT_file:
            ...
            if (ap->name)
                error(
                3, "%s: %s: archive name already specified", v, ap->name);
            ap->name = strdup(v);
            break;
        case OPT_filter:
            if (y && *v)
                ...
                do
                {
                    ...
                    if (!fp)
                    {
                        if (!pathpath("lib/pax",
                                      ...
                                      sizeof(tmp1))
                            || sfsprintf(
                               tmp2, sizeof(tmp2) - 1, "%s/%s.fmt", tmp1, s)
                               <= 0
        ...
        case OPT_unblocked:
            if (!*v)
                getarchive(IN)->io->unblocked = getarchive(OUT)->io->unblocked
                = y;
            else
                while (*v)
                    switch (*v++)
        ...
        case OPT_yes:
            ...
            break;
        default:
            if (!type && !(op->flags & OPT_HEADER)
                || !(op->flags & (OPT_IGNORE | OPT_SET)))
                error(
                1, "%s: option ignored [index=%d]", op->name, op->index);
            break;
        }
    }
    if (line && !hdr)
...
{
    return strmatch(name, pattern)
           || (op->flags & OPT_VENDOR)
              && strmatch(sfprints("%s.%s", VENDOR, name), pattern);
}
...
optinfo(Opt_t *op, Sfio_t *sp, const char *s, Optdisc_t *dp)
{
    ...
    case 'f':
        ...
        while (ar = ardirlist(ar))
            sfprintf(
            sp, "[+%s?%s; for input only]", ar->name, ar->description);
    ...
    case 'l':
        ...
        "%s",
        "	[+----?subformats ----]"
        "	[+case\b::\bp\b\a1\a::\bs\b\a1\a::...::\bp\b\an\a::\bs\b\an\a?"
        "Expands"
...
main(int argc, char **argv)
{
    ...
    state.header.extended
    = state.strict ? HEADER_EXTENDED_STD : HEADER_EXTENDED;
    ...
    state.mkdir = 1;
    state.modtime = 1;
    if (!(state.tmp.fmt = sfstropen()) || !(state.tmp.lst = sfstropen())
        || !(state.tmp.str = sfstropen()))
        nospace();
    ...
    if (!sfstrtell(opt.listformat))
        sfputr(opt.listformat,
               (state.list && state.verbose)
               ? "%(mode)s %2(nlink)d %-8(uname)s "
                 "%-8(gname)s%8(device:case::%(size)llu:*:%(device)s)s "
                 "%(mtime)s %(delta.op:case:?*:%(delta.op)s "
                 ")s%(path)s%(linkop:case:?*: %(linkop)s %(linkpath)s)s"
               : "%(delta.op:case:?*:%(delta.op)s "
                 ")s%(path)s%(linkop:case:?*: %(linkop)s %(linkpath)s)s",
               ' ');
    ...
    else
    {
        if (state.blocksize)
            ...
            state.blocksize
            = (state.operation == (IN | OUT) ? FILBLOCKS : DEFBLOCKS)
              * BLOCKSIZE;
    ...
    if (state.maxout && state.maxout <= state.blocksize)
        error(3,
              "--maxout=%#i must be greater than --blocksize=%#i",
              state.maxout,
              state.blocksize);
    message((-1,
             "blocksize=%d buffersize=%d recordsize=%d",
             state.blocksize,
             state.buffersize,
             state.record.size));
    ...
    if (ap = state.out)
    {
        ...
        if (ap->compress)
        {
            ...
            if (dup(n) != 1)
                error(3,
                      "%s: cannot redirect %s filter output",
                      ap->name,
                      ap->compress->name);
            close(n);
        ...
        if (state.checksum.name)
        {
            ...
            if (!(state.checksum.sp = sfopen(NiL, state.checksum.path, "w")))
                error(3,
                      "%s: cannot write checksum temporary",
                      state.checksum.path);
            sfprintf(
        ...
        if (state.install.name)
        {
            ...
            if (!(state.install.sp = sfopen(NiL, state.install.path, "w")))
                error(
                3, "%s: cannot write install temporary", state.install.path);
        }
    }
    if (!(state.linktab = hashalloc(NiL,
                                    HASH_set,
                                    HASH_ALLOCATE,
                                    HASH_namesize,
                                    sizeof(Fileid_t),
    ...
    if ((state.operation & IN) && !state.list
        ...
        while (hashnext(pos))
        {
            ...
            if (op->name == pos->bucket->name)
                sfprintf(sfstderr,
                         ...
                         op->temp.number,
                         op->temp.string,
                         (op->flags & OPT_HEADER) ? " HEADER" : "",
                         (op->flags & OPT_READONLY) ? " READONLY" : "");
        }
        hashdone(pos);
    }
#endif
    ...
    case IN:
        ...
        copyin(state.in);
        if (state.exact)
            for (state.pattern = state.patterns; state.pattern->pattern;
                 state.pattern++)
                if (!state.pattern->matched)
                    error(2,
                          "%s: %s: file not found in archive",
                          state.in->name,
    ...
    case (IN | OUT):
        ...
        else
        {
            ...
            if (stat(state.destination, &st) || !S_ISDIR(st.st_mode))
                error(
                3, "%s: destination must be a directory", state.destination);
            state.dev = st.st_dev;
...
finish(int code)
{
    ...
    if (state.summary)
    {
        ...
        message((-1,
                 "%s totals entries=%d count=%I*d expand=%I*d offset=%I*d "
                 "BLOCKSIZE=%I*d n=%I*d blocks=%I*d",
                 ap->name,
                 ap->entries,
                 sizeof(ap->io->count),
                 ap->io->count,
