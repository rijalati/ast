                ...
                size_t size)
{
    ...
    if ((swap = swapop(&verify.magic, &magic.magic, sizeof(magic.magic))) < 0)
        return 0;
    message((-2,
             "%s: magic swap=%d magic=%08x major=%d minor=%d",
             ap->name,
             swap,
             magic.magic,
             magic.major,
    ...
    if (magic.major == 1) {
        ...
        message((-2,
                 "%s: lead name=%s archnum=%d osnum=%d sigtype=%d",
                 ap->name,
                 state.volume,
                 lead.archnum,
                 lead.osnum,
        ...
        case 5:
            for (;;) {
                ...
                message((-2,
                         "%s: verify magic=%08x major=%d minor=%d type=%d",
                         ap->name,
                         verify.magic,
                         verify.major,
                         verify.minor,
                ...
                message((-2,
                         "%s: head entries=%lu datalen=%lu num=%lu",
                         ap->name,
                         head.entries,
                         head.datalen,
                         num));
        ...
        default:
            error(
            2,
            "%s: %s format version %d.%d signature type %d not supported",
            ap->name,
            fp->name,
            magic.major,
            magic.minor,
    ...
    state.volume[0] = i;
    ap->package
    = strdup(sfprints("%s %d.%d", fp->name, magic.major, magic.minor));
    return 1;
}
