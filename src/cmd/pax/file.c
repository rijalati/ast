...
apply(Archive_t *ap, File_t *f, Filter_t *fp)
{
    ...
    if ((rfd = open(state.tmp.file, O_RDONLY | O_BINARY)) < 0) {
        ...
        if (remove(state.tmp.file))
            error(1,
                  "%s: cannot remove filter temporary %s",
                  f->path,
                  state.tmp.file);
        return -1;
    }
    if (remove(state.tmp.file))
        error(
        1, "%s: cannot remove filter temporary %s", f->path, state.tmp.file);
    ...
    if (streq(*fp->argv, "nocomment")) {
        ...
        *fp->patharg = arg;
        ...
        if (n < 0)
            error(
            ERROR_SYSTEM | 2, "%s: %s filter read error", f->path, *fp->argv);
        if (n = procclose(proc))
...
openin(Archive_t *ap, File_t *f)
{
    ...
    if (f->type != X_IFREG)
        ...
        rfd = apply(ap, f, fp);
    else if ((rfd = open(f->st->st_size ? f->path : "/dev/null",
                         O_RDONLY | O_BINARY))
             < 0)
        error(ERROR_SYSTEM | 2, "%s: cannot read", f->path);
...
openout(Archive_t *ap, File_t *f)
{
    ...
    if (f->delta.op == DELTA_delete) {
        if (exists)
            ...
            default:
                if (remove(f->name))
                    error(
                    ERROR_SYSTEM | 2, "%s: cannot remove file", f->name);
                else
                    listentry(f);
                break;
            }
    ...
    case X_IFIFO:
        if (exists
            && (prune(ap, f, &st)
                || state.update && f->st->st_dev != st.st_dev))
            return -1;
        if (!(ap->format->flags & KEEPSIZE))
            f->st->st_size = 0;
        break;
    ...
    default:
        error(1,
              "%s: unknown file type 0%03o -- creating regular file",
              f->name,
              f->type >> 12);
        /*FALLTHROUGH*/
    case X_IFREG:
        ...
        if (state.intermediate) {
            ...
            for (n = 0;; n++) {
                ...
                ap->path.temp, sizeof(ap->path.temp), d, error_info.id, &ifd);
                if (e)
                    *e = '/';
                message(
        ...
        again:
            if (!(exists & 0007)) {
                ...
                error(ERROR_SYSTEM | 2,
                      ...
                      f->name,
                      (exists & 0100)
                      ? ERROR_translate(0, 0, 0, ", cannot enable user write")
                      : "",
                      (exists & 0010)
                      ? ERROR_translate(0, 0, 0, ", cannot remove")
                      : "");
                return -1;
            }
        }
        if (perm != f->perm)
            ...
            error(ERROR_SYSTEM | 1,
                  "%s: cannot restore original mode %s",
                  f->name,
                  fmtperm(st.st_mode & S_IPERM));
        return fd;
    }
}
...
getfile(Archive_t *ap, File_t *f, Ftw_t *ftw)
{
    ...
    f->name = map(ap, name);
    if (state.files && state.operation == (IN | OUT)
        && dirprefix(state.destination, name, 0))
        return 0;
    ...
    message((-2,
             "getfile(): path=%s name=%s mode=%s size=%I*d mtime=%s",
             name,
             f->name,
             fmtmode(f->st->st_mode, 1),
             sizeof(f->st->st_size),
             f->st->st_size,
             fmttime("%K", f->st->st_mtime)));
    return 1;
}
...
validout(Archive_t *ap, File_t *f)
{
    ...
    return ap->format->validate ? ((*ap->format->validate)(&state, ap, f) > 0)
                                : 1;
}
...
addlink(Archive_t *ap, File_t *f)
{
    ...
    if (p = ( Link_t * )hashget(state.linktab, ( char * )&id)) {
        ...
        f->linktype = HARDLINK;
        f->linkpath = p->name;
        if (state.pass && (state.operation & OUT)
            || !state.pass && state.operation == OUT)
            return 0;
    linked:
    ...
    case X_IFLNK:
        ...
        if (state.owner) {
            ...
            if (lchown(f->name, post.uid, post.gid) < 0)
                error(
                1, "%s: cannot chown to (%d,%d)", f->name, post.uid, post.gid);
        }
#endif
#if _lib_lchmod
        if (f->restoremode) {
                ...
                if (lchmod(f->name, f->perm & state.modemask))
                    ...
                    if (lstat(f->name, &st))
                        error(1, "%s: not found", f->name);
                             ...
                             ^= (st.st_mode & (S_ISUID | S_ISGID | S_ISVTX)))
                        error(
                        1, "%s: mode %s not set", f->name, fmtmode(m, 0) + 1);
                }
            }
        }
#endif
...
{
    if (*name && tvtouch(name, ap, mp, cp, 0) && errno != ENOENT
        && errno != ENOTDIR)
...
seekable(Archive_t *ap)
{
    ...
    if ((wfd = open(
        ...
        error(ERROR_SYSTEM | 3,
              "%s: cannot create seekable temporary %s",
              ap->name,
              state.tmp.file);
    if ((rfd = open(state.tmp.file, O_RDONLY | O_BINARY)) < 0)
        error(ERROR_SYSTEM | 3,
              "%s: cannot open seekable temporary %s",
              ap->name,
              state.tmp.file);
    if (remove(state.tmp.file))
        error(ERROR_SYSTEM | 1,
              "%s: cannot remove seekable temporary %s",
              ap->name,
              state.tmp.file);
    ap->io->seekable = 1;
    z = 0;
