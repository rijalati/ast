    ...
    if (ap->delta && !f->extended) {
        ...
        if (!f->delta.base) {
            ...
            if ((f->delta.checksum ^ x) & 0xffffffff)
                error(2,
                      "%s: %s: corrupt archive: checksum mismatch -- "
        ...
        if (n = f->delta.index - ap->delta->index) {
            if (n > 0)
                ...
                error(2,
                      "%s: %s: corrupt archive: delta index out of sync by "
                      "%d file%s",
                      ap->name,
                      f->name,
                      -n,
...
putdeltatrailer(Archive_t *ap, File_t *f)
{
    ...
    if (f->delta.op && ap->delta) {
        if (!(ap->format->flags & STANDARD)) {
            ...
            if (n != DELTA_TRAILER)
                error(PANIC,
                      "delta trailer size %d shoulda been %d",
                      n,
                      DELTA_TRAILER);
            bwrite(ap, s, n);
        }
...
initdelta(Archive_t *ap, Format_t *dp)
{
    if (!ap->delta && !(ap->delta = newof(0, Delta_t, 1, 0)))
        nospace();
    if (!ap->delta->tab
        && !(ap->delta->tab = hashalloc(
             NiL, HASH_set, HASH_ALLOCATE, HASH_name, "delta", 0)))
        nospace();
    ap->delta->format = dp;
}
...
deltabase(Archive_t *ap)
{
    ...
    if ((bp->io->mode & O_ACCMODE) != O_WRONLY)
        bp->sum++;
    if ((bp->io->fd = open(bp->name, bp->io->mode | O_BINARY)) < 0
        || fstat(bp->io->fd, &st))
            ...
            if (!d->mark && selectfile(ap, d->info)
                ...
                if (!d->uncompressed) {
                    ...
                    paxdelta(NiL,
                             ap,
                             d->info,
                             DELTA_DEL | DELTA_BIO | DELTA_SIZE,
                             ap->delta->base,
                             d->size,
                             DELTA_TAR | DELTA_FD | DELTA_FREE | DELTA_OUTPUT
                             | DELTA_COUNT | DELTA_LIST,
                             wfd,
                             0);
                else
                    paxdelta(NiL,
                             ap,
                             d->info,
                             DELTA_DEL | DELTA_FD | DELTA_OFFSET | DELTA_SIZE,
                             ap->delta->base->io->fd,
                             d->offset,
                             d->size,
                             DELTA_TAR | DELTA_FD | DELTA_FREE | DELTA_OUTPUT
    ...
    if (op->delta && (op->delta->format->flags & DELTAIO)) {
        if (f->type == X_IFREG && f->linktype == NOLINK
            ...
            if (d) {
                ...
                message(
                (-2,
                 "delta: delta: file=%s offset=%ld size=%ld expand=%d",
                 f->name,
                 d->offset,
                 d->size,
                 d->uncompressed));
                if (state.ordered) {
                    if (!d->uncompressed)
                        paxdelta(ip,
                                 op,
                                 f,
                                 DELTA_SRC | DELTA_BIO | DELTA_SIZE,
                                 op->delta->base,
                                 d->size,
                                 DELTA_TAR | DELTA_FD | DELTA_FREE
                                 | DELTA_SIZE,
                                 f->fd,
                                 f->st->st_size,
                        ...
                        op,
                        f,
                        DELTA_SRC | DELTA_FD | DELTA_SIZE | DELTA_FREE,
                        dfd,
                        d->uncompressed,
                        DELTA_TAR | DELTA_FD | DELTA_FREE | DELTA_SIZE,
                        f->fd,
                        f->st->st_size,
                        DELTA_DEL | DELTA_TEMP | DELTA_OUTPUT,
                        &f->fd,
                    ...
                    paxdelta(ip,
                             op,
                             f,
                             DELTA_SRC | DELTA_FD | DELTA_OFFSET | DELTA_SIZE,
                             op->delta->base->io->fd,
                             d->offset,
                             d->size,
                             DELTA_TAR | DELTA_FD | DELTA_FREE | DELTA_SIZE,
                             ...
                             0);
                                   ...
                                   op,
                                   d->info,
                                   DELTA_DEL | DELTA_FD | DELTA_OFFSET
                                   | DELTA_SIZE,
                                   op->delta->base->io->fd,
                                   d->offset,
                                   d->size,
                    ...
                    paxdelta(ip,
                             ...
                             dfd,
                             d->uncompressed,
                             DELTA_TAR | DELTA_FD | DELTA_SIZE | DELTA_FREE,
                             f->fd,
                             f->st->st_size,
                             DELTA_DEL | DELTA_TEMP | DELTA_OUTPUT,
                             &f->fd,
            ...
            f->st->st_size = 0;
            if (f->delta.op == DELTA_update && f->type != X_IFREG
                && f->st->st_mode == d->mode)
    ...
    while (getprologue(ip)) {
        while (getheader(ip, f)) {
            ...
            if (f->ro)
                ...
                case DELTA_pass:
                    if (validout(op, f) && selectfile(op, f)) {
                        if (ip->delta
                            ...
                            if (remove(f->path))
                                error(1,
                                      "%s: cannot remove filter temporary",
                                      p,
                                      f->path);
                            f->path = p;
                            f->delta.op = 0;
                    ...
                    if (validout(op, f) && selectfile(op, f)) {
                        if (state.ordered)
                            ...
                            paxdelta(ip,
                                     op,
                                     f,
                                     DELTA_SRC | DELTA_FD | DELTA_OFFSET
                                     | DELTA_SIZE,
                                     ip->delta->base->io->fd,
                                     f->delta.base->offset,
                                     f->delta.base->size,
                ...
                case DELTA_verify:
                    if (!f->delta.base
                        || f->delta.base->mtime.tv_sec != f->st->st_mtime)
                        error(3,
                              "%s: base archive mismatch [%s#%d]",
                              f->name,
                              __FILE__,
        ...
        if (ap->checkdelta) {
            ...
            if (ap->delta && ap->delta->format) {
                ...
                if (bp = ap->delta->base) {
                    ...
                    message(
                    (-5,
                     "checkdelta: %s size=%I*d:%I*d checksum=%08x:%08x",
                    ...
                    if (!ap->delta->compress) {
                        if (!ap->ordered) {
                            ...
                            if (bp->size != size)
                                error(3,
                                      "%s: %s: base archive size mismatch -- "
                        ...
                        if ((bp->checksum ^ checksum) & 0xffffffff)
                            error(1,
                                  "%s: %s: base archive checksum mismatch -- "
                                  "expected %08lx, got %08lx",
                                  ap->name,
                                  bp->name,
                                  checksum & 0xffffffff,
...
delread(void *buf, int n, Vdoff_t off, Vddisc_t *vd)
{
    ...
    message((-6,
             "delread: op=%o buf=%p n=%d off=%I*d nxt=%I*d",
             dp->op,
             buf,
             n,
             sizeof(off),
...
delwrite(void *buf, int n, Vdoff_t off, Vddisc_t *vd)
{
    ...
    message((-6,
             "delwrite: op=%o buf=%p n=%d off=%I*d",
             dp->op,
             buf,
             n,
             sizeof(off),
...
paxdelta(Archive_t *ip, Archive_t *ap, File_t *f, int op, ...)
{
...
#if DEBUG
        if (mp)
            sfprintf(mp, " %s [", dataname[op & DELTA_DATA]);
#endif
        if (op & DELTA_BIO)
...
#if DEBUG
            if (mp)
                sfprintf(mp, " size=%I*d", sizeof(dp->vd.size), dp->vd.size);
#endif
        }
    ...
    if (gen->pfd) {
        if (!(state.test & 0000020) && fp->variant != DELTA_88
            && (state
                .buffer[bufferclash
                        ? (state.delta.bufferindex = !state.delta.bufferindex)
                        : state.delta.bufferindex]
                .base
    ...
    case DELTA_PATCH:
...

        {
            ...
            if (gen == &data[DELTA_DEL]) {
                    ...
                    if (op == DELTA_DEL) /*NOP*/
                        ...
                        case DELTA_FD:
                            if (!(data[op].vd.data
                                ...
                                nospace();
                            if (data[op].base
                                && lseek(data[op].fd, data[op].base, SEEK_SET)
                                   != data[op].base)
                ...
                case DELTA_PATCH:
                    error(1,
                          "AHA %s %s/%s SRC=%d:%p TAR=%d:%p DEL=%d:%p",
                          fp->name,
                          ap->name,
                          f->path,
                          data[DELTA_SRC].vd.size,
                          data[DELTA_SRC].vd.data,
                          data[DELTA_TAR].vd.size,
                          data[DELTA_TAR].vd.data,
                          data[DELTA_DEL].vd.size,
                          data[DELTA_DEL].vd.data);
                    n = 0;
                    break;
                }
            } else {
                    ...
                    if (op == DELTA_TAR) /*NOP*/
                        ...
                        case DELTA_BUFFER:
                            ...
                            if ((data[op].fd = open(tmp,
                                ...
                                error(
                                3,
                                "%s: cannot create delta temporary file",
                                tmp);
                            if (write(data[op].fd,
                                      data[op].vd.data,
                                      data[op].vd.size)
                                ...
                                error(ERROR_SYSTEM | 3,
                                      ...
                                      f->name);
                            close(data[op].fd);
                            if ((data[op].fd = open(tmp, O_RDONLY | O_BINARY))
                                < 0)
                                error(ERROR_SYSTEM | 3,
                                      "%s: cannot read delta temporary file",
                                      tmp);
                            if (remove(tmp))
                                error(
                                ERROR_SYSTEM | 1,
                ...
                case DELTA_PATCH:
                    error(
                    PANIC, "paxdelta: %s update not supported", fp->name);
                    break;
                }
            }
        }
