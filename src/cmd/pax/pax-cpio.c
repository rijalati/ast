    ...
    if (n = cpio->ops - cpio->opsbuf) {
        ...
        *cpio->ops++ = 0;
        if ((f->namesize += n) > CPIO_NAMESIZE)
            error(
            1, "%s: extended ops may crash older cpio programs", f->name);
    }
}
                 ...
                 size_t size)
{
    ...
    if (size >= CPIO_HEADER && buf[0] == '0'
        && sfsscanf(( char * )buf, "%6o", &magic) == 1 && magic == CPIO_MAGIC)
        return init(ap);
    return 0;
}
...
cpio_common(Pax_t *pax, Archive_t *ap, File_t *f, int align, int header)
{
    ...
    if (f->name[f->namesize - 1]) {
        ...
        error(state.keepgoing ? 1 : 3,
              "%s: entry %d.%d file name terminating null missing",
              ap->name,
              ap->volume,
              ap->entry);
    }
...
cpio_getheader(Pax_t *pax, Archive_t *ap, File_t *f)
{
    ...
    if (paxread(pax, ap, state.tmp.buffer, ( off_t )0, ( off_t )CPIO_HEADER, 0)
        <= 0)
        return 0;
    state.tmp.buffer[CPIO_HEADER] = 0;
    if (state.tmp.buffer[0] == '0'
        && sfsscanf(state.tmp.buffer,
                    "%6o%6lo%6lo%6lo%6lo%6lo%6lo%6lo%11lo%6o%11I*o",
                    &f->magic,
                    &lst.dev,
                    &lst.ino,
                    &lst.mode,
...
#    endif
    ...
    sfsprintf(
    state.tmp.buffer,
    state.tmp.buffersize,
    "%0.6lo%0.6lo%0.6lo%0.6lo%0.6lo%0.6lo%0.6lo%0.6lo%0.11lo%0.6o%0.11I*o",
    ...
    ( long )f->namesize,
    sizeof(intmax_t),
    (intmax_t)(f->st->st_size + (f->type == X_IFLNK ? f->linkpathsize : 0)));
...
cpio_putepilogue(Pax_t *pax, Archive_t *ap)
{
    ...
    putinfo(ap,
            strcpy(buf, CPIO_TRAILER),
            ap->delta && !(ap->delta->format->flags & PSEUDO)
            ? ap->delta->index + 1
            : 0,
            0);
           ...
           unsigned long event)
{
    ...
    case PAX_EVENT_BUG_19951031:
        ...
        if (f->st->st_size == f->linkpathsize
            && paxread(pax,
                       ap,
                       state.tmp.buffer,
                       ( off_t )0,
            ...
            if (strtol(state.tmp.buffer, NiL, 8) == CPIO_MAGIC
                ...
                if (!ap->warnlinkhead) {
                    ...
                    error(
                    1,
                    "%s: compensating for invalid %s header hard link sizes",
                    ap->name,
                    ap->format->name);
                }
            }
                ...
                size_t size)
{
    ...
    if (size >= ASC_HEADER && buf[0] == '0'
        && sfsscanf(( char * )buf, "%6o", &magic) == 1 && magic == ASC_MAGIC)
        return init(ap);
    return 0;
}
...
asc_getheader(Pax_t *pax, Archive_t *ap, File_t *f)
{
    ...
    if (paxread(pax, ap, state.tmp.buffer, ( off_t )0, ( off_t )ASC_HEADER, 0)
        <= 0)
        return 0;
    state.tmp.buffer[ASC_HEADER] = 0;
    if (state.tmp.buffer[0] == '0'
        && sfsscanf(state.tmp.buffer,
                    "%6o%8lx%8lx%8lx%8lx%8lx%8lx%8I*x%8lx%8lx%8lx%8lx%8x%8lx",
                    &f->magic,
                    &lst.ino,
                    &lst.mode,
                    &lst.uid,
...
asc_putheader(Pax_t *pax, Archive_t *ap, File_t *f)
{
    ...
    sfsprintf(state.tmp.buffer,
              state.tmp.buffersize,
              "%0.6lo%0.8lx%0.8lx%0.8lx%0.8lx%0.8lx%0.8lx%0.8lx%0.8lx%0.8lx%"
              "0.8lx%0.8lx%0.8lx%0.8lx%0.8lx%0.8lx",
...
binary_putheader(Pax_t *pax, Archive_t *ap, File_t *f)
{
    ...
    binary_short(hdr.size,
                 f->st->st_size + (f->type == X_IFLNK ? f->linkpathsize : 0));
    hdr.dev = f->st->st_dev;
    hdr.ino = f->st->st_ino;
...
Format_t pax_aschk_format = { "aschk",
                              0,
...
Format_t pax_binary_format = { "binary",
                               "binary-cpio",
