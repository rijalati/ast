...
getlabel(Pax_t *pax, Archive_t *ap, File_t *f)
{
    ...
    if (!ap->io->blocked && !*slt->last && slt->buf[3] == '2'
        && (strneq(slt->buf, "HDR", 3) || strneq(slt->buf, "EOF", 3)
            || strneq(slt->buf, "EOV", 3)))
...
putlabels(Pax_t *pax, Archive_t *ap, File_t *f, char *type)
{
    ...
    slt->buf,
    sizeof(slt->buf),
    "%s1%-17.17s000001%04d%04d000100 %02d%03d 00000 %06d%-6.6sD%-7.7s       ",
    type,
    f->id,
    slt->section,
    slt->sequence,
    ...
    do {
        if (strneq(slt->buf, "HDR", 3)) {
            ...
            if (type == 1) {
...
#if SAVESET
                if (streq(slt->format, SAVESET_ID)
                    && streq(slt->implementation, SAVESET_IMPL))
                    ap->format = SAVESET;
#endif
                f->st->st_mtime = 0;
                ...
                default:
                    error(2,
                          "%s record format %c not supported",
                          ap->format->name,
                          f->record.format);
                    f->skip = 1;
                }
...
slt_getdata(Pax_t *pax, Archive_t *ap, File_t *f, int wfd)
{
    ...
    for (;;) {
        ...
        if (n == 0) {
            ...
            while (getlabel(pax, ap, f)) {
                if (strneq(slt->buf, "EOV1", 4))
                    k = 0;
                         ...
                         && !strneq(slt->buf, "EOV", 3)
                         && !strneq(slt->buf, "UTL", 3) && ++n >= 16
                         && !state.keepgoing)
                    error(3,
                          "%s: %s: %d invalid %s end of file/volume labels "
                          "detected",
                          ap->name,
                          f->name,
                          n,
            ...
            if (n)
                error(
                1,
                "%s: %s: %d invalid %s end of file/volume labels detected",
                ap->name,
                f->name,
                n,
                ap->format->name);
            ...
            for (;;) {
                ...
                if (getprologue(ap)) {
                    ...
                    if (getheader(ap, &v)) {
                        ...
                        error(
                        1,
                        "volume containing %s id %s section %d required",
                        f->name,
                        f->id,
                        f->record.section);
                    }
    ...
    if (f->st->st_size && f->st->st_size != size)
        error(1,
              "%s: header size %I*d does not match data size %I*d",
              f->name,
              sizeof(f->st->st_size),
              f->st->st_size,
              sizeof(size),
...
slt_putprologue(Pax_t *pax, Archive_t *ap, int append)
{
    ...
    strncpy(
    slt->implementation, IMPLEMENTATION, sizeof(slt->implementation) - 1);
    if (ap->format->flags & CONV)
        sfsprintf(slt->standards,
                  sizeof(slt->standards),
                  "%-5.5s%-5.5s%-5.5s%-4.4s",
                  "ATT",
                  "1",
                  "EBCDIC",
            ...
            for (;;) {
                if (p >= &rec[state.record.size] && state.record.size) {
                    ...
                    if (state.record.format == 'S'
                        ...
                        if (!f->record.partial
                            && !(f->record.partial
                                 = newof(0, char, state.blocksize, 0)))
                            nospace();
...
eof:
    ...
    if (truncated)
        error(1,
              "%s: %d out of %d record%s truncated",
              f->name,
              truncated,
              count,
              count == 1 ? "" : "s");
