...
#define circular(p)                                                          \
    (strneq((p)->value, "$(", 2)                                             \
     && strneq((p)->value + 2, JCL_AUTO, sizeof(JCL_AUTO) - 1)               \
     && strneq(                                                              \
        (p)->value + sizeof(JCL_AUTO) + 1, (p)->name, strlen((p)->name))     \
     && streq((p)->value + sizeof(JCL_AUTO) + strlen((p)->name) + 1, ")"))

struct Jcmcard_s;
typedef struct Jcmcard_s Jcmcard_t;
struct Jcmevent_s;
typedef struct Jcmevent_s Jcmevent_t;
struct Jcmjob_s;
typedef struct Jcmjob_s Jcmjob_t;
struct Jcmlib_s;
typedef struct Jcmlib_s Jcmlib_t;
struct Jcmlist_s;
typedef struct Jcmlist_s Jcmlist_t;
struct Jcmset_s;
typedef struct Jcmset_s Jcmset_t;
struct Jcmshout_s;
typedef struct Jcmshout_s Jcmshout_t;
struct Jcmvar_s;
typedef struct Jcmvar_s Jcmvar_t;
        ...
        if (event->job->memlib == state.dummy
            ...
            for (v = event->job->set; v; v = v->next)
                if (v->value)
                    ...
                    JCL_AUTO,
                    v->name,
                    fmtquote(v->value, "$'", "'", strlen(v->value), 0));
                else
                ...
                case 'I':
                    if (job) {
                        if (index) {
                            ...
                            for (n = 0; n < index; n++)
                                reqs
                                = append(reqs, getevent(s, s + 20, 0, 0, n));
                            s += 23;
                        }
                        for (s++; *s; s += 24)
                ...
                case 'O':
                    if (job) {
                        for (s++; *s; s += 25)
                            if (s[24] == '-')
                                clear
                                = append(clear, getevent(s, s + 20, 0, 0, 0));
                            else
                                raise
                                = append(raise, getevent(s, s + 20, 0, 0, 0));
                    }
                    break;
                case 'Q':
                    if (state.verbose)
                        ...
                        while (*s) {
                            ...
                            for (t = shout->data; !isalpha(*s); s++)
                                if (t < &shout->data[sizeof(shout->data) - 1])
                                    *t++ = *s;
                            *t = 0;
                            for (s += (copy(shout->to, s, 16) - shout->to);
                                 *s == ' ';
                                 s++)
                                ;
                            shout->pri = *s++;
