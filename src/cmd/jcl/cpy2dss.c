...
#define cpyinit(d, e)                                                        \
    (memset(d, 0, sizeof(Cpydisc_t)),                                        \
     (d)->version = CPY_VERSION,                                             \
     (d)->errorf = (Error_f)(e))
...
struct Cpyfield_s;
typedef struct Cpyfield_s Cpyfield_t;
        ...
        if (!strcasecmp(s, "BINARY")) {
                ...
                if (*e || f->comp <= 0 || f->comp > 9) {
                    if (cpy->disc->errorf)
                        (*cpy->disc->errorf)(cpy,
                                             cpy->disc,
                                             2,
                                             "%s: syntax error: COMP number "
                                             "in range 1..9 expected",
                                             s);
                    continue;
                }
            }
            ...
            if (!(p = ( Cpyfield_t * )dtmatch(cpy->dt, s))) {
                ...
                if (!(p = ( Cpyfield_t * )dtmatch(cpy->dt, s))) {
                    if (cpy->disc->errorf)
                        ...
                        cpy->disc,
                        2,
                        "%s: internal error: dup not found",
                        s);
                    return 0;
                }
            }
            ...
            if (*e) {
                if (cpy->disc->errorf)
                    ...
                    cpy->disc,
                    2,
                    "%s: syntax error: minimum OCCURS number expected",
                    s);
                continue;
            }
            if (!(s = cpylex(cpy)) || !*s)
            ...
            if (!strcasecmp(s, "TO")) {
                ...
                if (*e) {
                    if (cpy->disc->errorf)
                        ...
                        cpy->disc,
                        2,
                        "%s: syntax error: maximum OCCURS number expected",
                        s);
                    continue;
                }
                if (!(s = cpylex(cpy)) || !*s)
            ...
            for (;;) {
                ...
                default:
                    if (cpy->disc->errorf)
                        (*cpy->disc->errorf)(
                        cpy, cpy->disc, 2, "%s: unknown PIC", s - 1);
                    break;
                }
                break;
            }
...
static char *typename(Cpyfield_t *field)
{
    char *type;
    ...
    if (field = cpyfield(cpy)) {
        ...
        while (next = cpyfield(cpy))
            if (i) {
                ...
                offset += width;
                if (!member && (i = strlen(parent->name))
                    && !memcmp(parent->name, next->name, i)
                    && strmatch(next->name + i, "?(_)(siz|SIZ|LEN|len)*"))
                    member = next;
                parent = next;
            }
            ...
            if (state.record && RECTYPE(state.record) != REC_delimited) {
                ...
                case REC_variable:
                    ...
                    sfprintf(sfstdout,
                             "%s<TYPE>%s</>\n",
                             INDENT(lev + 3),
                             REC_V_LITTLE(state.record) ? "le_t" : "be_t");
                    break;
                }
                sfprintf(sfstdout, "%s</>\n", INDENT(lev + 1));
            }
                    ...
                    if (state.sized && field->parent) {
                        if (field->flags & CPY_number) {
                            ...
                            sfprintf(op,
                                     "%s<WIDTH>%s</>\n",
                                     INDENT(lev + 2),
                                     field->parent->sized->name);
                            field->parent->sized = 0;
                        }
                    }
                    if (state.text
                        || state.variable && delimiter == state.terminator)
                        sfprintf(op,
                                 "%s<DELIMITER>%s</>\n",
                                 INDENT(lev + 2),
                                 delimiter);
                ...
                if (state.output == CPY_BYTEMASK)
                    ...
                    if (field->structure) {
                        ...
                        for (member = field->next;
                            ...
                            width
                            += member->width
                               * (member->maxdimension ? member->maxdimension
                                                       : 1);
                        if (field->maxdimension)
                            width *= field->maxdimension;
...
main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        case 'C':
            ...
            if (i <= 0 || i > 9 || j <= 0 || j > 9 || *e)
                error(
                2, "%s: COMP from:to expected in range 1..9", opt_info.arg);
            state.comp[i] = j;
            continue;
        case 'd':
    ...
    if (state.quotebegin && state.quoteend
        && streq(state.quotebegin, state.quoteend))
        state.quoteend = 0;
    if (!*argv)
