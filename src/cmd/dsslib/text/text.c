...
static Dssformat_t text_format = { "text",     "text format (2010-05-28)",
                                   CXH,        textident,
                                   textfopen,  textread,
                                   textwrite,  0,
                                   textfclose, 0,
...
invalid:
    if (disc->errorf)
        (*disc->errorf)(
        NiL, disc, ERROR_SYSTEM | 2, "%s: invalid schema", schema);
drop:
    free(meth);
    return 0;
}
