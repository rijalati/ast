...
struct Invalid_s;
typedef struct Invalid_s Invalid_t;
struct State_s;
...
invalidcmp(Dt_t *dict, void *a, void *b, Dtdisc_t *disc)
{
    ...
    if (!(r = strcmp(ap->variable->name, bp->variable->name))) {
        if (cxisstring(ap->variable->type)
            || cxisbuffer(ap->variable->type)) {
            az = ap->value.buffer.size;
            bz = bp->value.buffer.size;
            if (!(r = memcmp(ap->value.buffer.data,
...
validate_beg(Cx_t *cx, Cxexpr_t *expr, void *data, Cxdisc_t *disc)
{
    ...
    sfprintf(cx->buf,
             "%s%s",
             strchr(dss_lib_validate.description, '['),
             validate_usage);
    s = sfstruse(cx->buf);
    for (;;) {
        ...
        case 'r':
            if (!(state->setf = cxcallout(cx,
                ...
                if (cx->disc->errorf)
                    (*cx->disc->errorf)(
                    NiL, cx->disc, 3, "reair requires CX_SET callout");
                return -1;
            }
            continue;
        case 's':
                ...
                if (cxisnumber(field->variable->type)) {
                    ...
                    if (constraint->max)
                        number(expr->op,
                               ...
                               &field->variable->format);
                } else if (cxisstring(field->variable->type)
                           && constraint->def)
                    sfprintf(expr->op,
                             " default=\"%-.*s\"",
                             constraint->def->string.size,
                             constraint->def->string.data);
                if (constraint->expression)
                    sfprintf(
                    expr->op, " expression=\"%s\"", constraint->expression);
                if (constraint->pattern)
                    sfprintf(
                    expr->op, " pattern=\"%s\"", constraint->pattern);
            }
            sfprintf(expr->op, "\n");
        }
        goto bad;
    ...
    if (!(state->getf = cxcallout(
        ...
        if (cx->disc->errorf)
            (*cx->disc->errorf)(
            NiL, cx->disc, 3, "validation requires CX_GET callout");
        goto bad;
    }
    if (!state->verbose) {
...
validate_sel(Cx_t *cx, Cxexpr_t *expr, void *data, Cxdisc_t *disc)
{
    ...
    for (field = state->field; field; field = field->next) {
        ...
        if (constraint = field->variable->format.constraint) {
            ...
            if (cxisnumber(field->variable->type)) {
                if (constraint->min
                    && o.value.number < constraint->min->number) {
                    if (state->verbose && disc->errorf)
                        (*disc->errorf)(
                        NiL,
                        disc,
                        1,
                        "%s%s: %1.15Lg violates min constraint %1.15Lg",
                        cxlocation(cx, data),
                        field->variable->name,
                        o.value.number,
                        constraint->min->number);
                ...
                if (constraint->max
                    && o.value.number > constraint->max->number) {
                    if (state->verbose && disc->errorf)
                        (*disc->errorf)(
                        NiL,
                        disc,
                        1,
                        "%s%s: %1.15Lg violates max constraint %1.15Lg",
                        cxlocation(cx, data),
                        field->variable->name,
                        o.value.number,
                        constraint->max->number);
    ...
    if (state->summary) {
        ...
        for (field = state->field; field; field = field->next)
            if (field->invalid || field->discarded || field->repaired) {
                ...
                sfprintf(expr->op,
                         "%16s  %11I*u %11I*u %11I*u\n",
                         field->variable->name,
                         sizeof(field->invalid),
                         field->invalid,
                         sizeof(field->discarded),
                         field->discarded,
                         sizeof(field->repaired),
                         field->repaired);
            }
    }
    vmclose(state->vm);
...
static Cxquery_t queries[] = { { "validate",
...
Dsslib_t dss_lib_validate = { "validate",
                              "validate query"
                              "[-1lms5P?\n@(#)$Id: dss validate query (AT&T "
                              "Research) 2003-04-05 $\n]" USAGE_LICENSE,
                              CXH,
                              0,
                              0,
