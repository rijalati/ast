...
struct Physical_s;
typedef struct Physical_s Physical_t;
struct Record_s;
typedef struct Record_s Record_t;
struct Section_s;
typedef struct Section_s Section_t;
struct Size_s;
typedef struct Size_s Size_t;
struct Table_s;
...
#define FW(f, w)                                                             \
    (((w) << 4) | ((f) & (CX_STRING | CX_INTEGER | CX_UNSIGNED | CX_FLOAT)))
...
flatget(Record_t *r, int index)
{
                            ...
                            if ((c = *t) == x) {
                                if (++t >= e) {
                                    if (r->dss->disc->errorf)
                                        ...
                                        2,
                                        "%sinvalid escape",
                                        cxlocation(r->cx, r->record),
                                        p->field->variable.name);
                                    r->index = r->nfields + 1;
                                    goto empty;
                                }
                                c = *t;
            ...
            if (w->keyed != r->serial) {
                ...
                while (s < e) {
                    if ((u = s) < e
                        ...
                        for (s++;
                             s < e && r->table->id[*( unsigned char * ) s];
                             s++)
                            ;
                    t = s;
                    for (;;) {
                        ...
                        if (r->table->id[*( unsigned char * ) h] & 1) {
                            for (g = h++;
                                 h < e
                                 && r->table->id[*( unsigned char * ) h];
                                 h++)
                                ;
                            if (h < e && *h == r->table->span
                                && (!r->table->unknown
                                    || dtmatch(r->table->dict, g)))
                                break;
                        }
                        s = h;
                    }
    ...
    found:
        if (w->field->width) {
            ...
            if (w->field->physical.format.width
                ...
                if (r->dss->disc->errorf)
                    ...
                    r->dss->disc,
                    1,
                    "%s%s: variable field width %d exceeds fixed maximum %d "
                    "-- maximum assumed",
                    cxlocation(r->cx, r->record),
                    w->field->variable.name,
                    n,
        ...
        if (m = w->field->map)
            ...
            m, fmtbuf(w->siz), ( char * ) s, w->siz);
        w->ret.type = w->field->variable.type;
        if ((*w->field->physical.type->internalf)(r->cx,
                                                  w->field->physical.type,
                                                  NiL,
                                                  &w->field->physical.format,
                                                  &w->ret,
                                                  ( char * ) s,
                                                  w->siz,
                                                  r->cx->rm,
...
flatgetbinary(Record_t *r, int index)
{
    ...
    if (w->serial != r->serial) {
        ...
        case FW(CX_UNSIGNED | CX_INTEGER, 8):
            w->ret.value.number
            = (int64_t)(*( uint64_t * ) (r->buf + w->off));
            break;
#endif
        case FW(CX_INTEGER, 1):
            w->ret.value.number
            = *( unsigned _ast_int1_t * ) (r->buf + w->off);
            break;
        ...
        default:
            if (!(w->siz = w->field->physical.format.width))
                w->ret.value.string.size = 0;
            else if ((w->field->physical.format.flags & (CX_STRING | CX_NUL))
                     == CX_STRING)
                w->ret.value.string.size
                = (s = memchr(
                   w->ret.value.string.data = r->buf + w->off, 0, w->siz))
                  ? (s - (r->buf + w->off))
                  : w->siz;
            else
    ...
    if (flat->magic->version) {
        ...
        if (flat->fixed && magicid->size != flat->fixed) {
            if (disc->errorf)
                ...
                disc,
                2,
                "%s: data size %d does not match schema size %d",
                file->path,
                magicid->size,
                flat->fixed);
            return -1;
        ...
        if (magicid->version > flat->magic->version) {
            if (disc->errorf)
                ...
                disc,
                2,
                "%s: data version %s is newer than schema version %s",
                file->path,
                fmtversion(magicid->size),
                fmtversion(flat->fixed));
            return -1;
        }
...
size_get(Dssfile_t *file, Size_t *z, Dssdisc_t *disc)
{
    ...
    error(-1,
          "AHA:flat#%d size_get size=%d width=%d reserve=%d",
          __LINE__,
          z->size,
          z->width,
          z->reserve);
    ...
    case 'e':
        s
        = ( char * ) ccmapcpy((( Flat_t * ) disc)->e2a, z->buf, s, z->width);
        n = strntoul(s, z->width, NiL, z->base);
        break;
    case 'l':
...
flatread(Dssfile_t *file, Dssrecord_t *record, Dssdisc_t *disc)
{
    ...
    for (;;) {
            ...
            if (flat->fixed && flat->terminator >= 0
                ...
                if (s
                    ...
                    for (k = 0;; k++) {
                        ...
                        if ((( unsigned char * ) s)[k] == flat->terminator
                            && (f->physical.format.flags & CX_STRING))
                            break;
                    }
                    sfread(file->io, s, 0);
                } else
                ...
                if (flat->terminator >= 0) {
                    ...
                    if (disc->errorf && !(file->dss->flags & DSS_QUIET))
                        (*disc->errorf)(NiL,
                                        disc,
                                        1,
                                        "%sflat record terminator ignored",
                                        cxlocation(r->cx, record));
                }
            }
        }
        error(-1,
              "AHA:flat#%d terminator=%d continuator=%d delimiter=%d "
              "fixed=%d record=%d",
              __LINE__,
              flat->terminator,
              flat->continuator,
        ...
        if (flat->terminator >= 0) {
            ...
            if (flat->fixed) {
                if (flat->sufficient = flat->fixed == (i - 1))
                    ...
                    if (disc->errorf)
                        ...
                        disc,
                        2,
                        "%sinvalid record length %d -- record ignored",
                        cxlocation(r->cx, record),
                        i);
                    continue;
            ...
            if (!(s = ( char * ) sfreserve(file->io, i, flat->variable))
                && (!flat->variable || !(i = sfvalue(file->io))
                    || !(s = ( char * ) sfreserve(
                         file->io, i, flat->variable))))
                break;
        } else {
            if (disc->errorf)
                (*disc->errorf)(
                ...
                disc,
                2,
                "%snon-unique terminator record read not implemented",
                cxlocation(r->cx, record));
            return -1;
        }
...
eof:
    if (sfvalue(file->io) && disc->errorf)
        (*disc->errorf)(NiL,
                        disc,
                        2,
                        "%slast record truncated (%ld/%ld)",
                        cxlocation(r->cx, record),
                        ( long ) sfvalue(file->io),
                        ( long ) flat->fixed);
    return 0;
}
...
flatwrite(Dssfile_t *file, Dssrecord_t *record, Dssdisc_t *disc)
{
        ...
        if (n < 0)
            ...
            if ((f->physical.format.flags & (CX_STRING | CX_BUFFER))
                ...
                if (f->physical.format.flags & CX_QUOTEALL) {
                    ...
                    q = 0;
                for (e = (s = b = ( unsigned char * ) flat->valbuf) + n;
                     s < e;
                     s++)
    ...
    if (flat->record) {
        ...
        if (size_put(file, n, flat->record, disc)
            || sfwrite(file->io, sfstrbase(io), n) != n)
            return -1;
    }
    return 0;
}
...
keycmp(Dt_t *dt, void *va, void *vb, Dtdisc_t *disc)
{
    ...
    char *t = (( Table_t * ) disc)->identified ? (( Table_t * ) disc)->id
                                               : ( char * ) 0;
    int c;
    int d;
        ...
        for (q = k; q; q = q->next) {
            if (q->qualification
                ...
                if (disc->errorf)
                    ...
                    disc,
                    ERROR_SYSTEM | 2,
                    "%s: %s: %s: cannot compile key qualification expression",
                    q->field->variable.name,
                    q->name,
                    q->qualification);
                return -1;
            }
        ...
        Dssdisc_t *disc)
{
    ...
    if (t && !t->qualified) {
        ...
        for (k = ( Key_t * ) dtfirst(t->dict); k;
            ...
            if (k->next && !k->qualification)
                error(
                1,
                "%s: %s: field key is ambiguous -- qualification required",
                k->field->variable.name,
                k->name);
    }
    off = 0;
    ...
    if (r->kfields <= r->nfields
        && (!(r->table = t) && !(r->table = tabinit(flat, disc))
            || tabcomp(flat, r->table, disc)))
        return 0;
    if (flat->fixed)
        r->index = r->nfields;
    r->dss = file->dss;
...
flatfopen(Dssfile_t *file, Dssdisc_t *disc)
{
        ...
        if (file->skip && !sfreserve(file->io, file->skip, -1)) {
            if (disc->errorf)
                (*disc->errorf)(NiL,
                                disc,
                                ERROR_SYSTEM | 2,
                                "%s: unexpected EOF in magic header",
                                file->path);
            return -1;
        }
    ...
    if (!(file->data = flat->root.record = recinit(flat,
                                                   file,
                                                   NiL,
                                                   flat->root.table,
                                                   flat->fields,
    ...
    if ((file->flags & DSS_FILE_WRITE) && flat->magic) {
        ...
        if (flat->magic->version) {
            ...
            if (flat->magic->string)
                ...
                if (flat->magic->swap > 0)
                    swapmem(
                    flat->magic->swap, num.buf, num.buf, flat->magic->length);
...
noswap:
    if (disc->errorf)
        (*disc->errorf)(NiL,
                        disc,
                        ERROR_SYSTEM | 2,
                        "%s: binary record data swap not implemented yet",
                        file->path);
    return -1;
}
...
static Dssformat_t flat_format = { "flat",     "flat format (2010-11-10)",
                                   CXH,        flatident,
                                   flatfopen,  flatread,
                                   flatwrite,  0,
                                   flatfclose, 0,
       ...
       Cxdisc_t *disc)
{
    ...
    x = (*((( Record_t * ) DSSDATA(data))->getf))(
    ( Record_t * ) DSSDATA(data), pc->data.variable->index);
    r->type = x->type;
    r->refs = x->refs;
    r->value.number = x->value.number;
#else
    *r = *(*((( Record_t * ) DSSDATA(data))->getf))(
    ( Record_t * ) DSSDATA(data), pc->data.variable->index);
#endif
    return 0;
}
...
static Tags_t *
flat_field_beg(Tag_t *, Tagframe_t *, const char *, Tagdisc_t *);
static int
flat_field_end(Tag_t *, Tagframe_t *, Tagdisc_t *);

static Tags_t *
flat_field_physical_beg(Tag_t *, Tagframe_t *, const char *, Tagdisc_t *);
static int
flat_field_physical_end(Tag_t *, Tagframe_t *, Tagdisc_t *);

static Tags_t *
flat_array_physical_beg(Tag_t *, Tagframe_t *, const char *, Tagdisc_t *);
static int
flat_array_physical_end(Tag_t *, Tagframe_t *, Tagdisc_t *);

static int
flat_field_name_dat(Tag_t *tag,
                    Tagframe_t *fp,
                    const char *data,
                    Tagdisc_t *disc)
{
...
static int
flat_field_description_dat(Tag_t *tag,
                           Tagframe_t *fp,
                           const char *data,
                           Tagdisc_t *disc)
{
...
static int
flat_field_details_dat(Tag_t *tag,
                       Tagframe_t *fp,
                       const char *data,
                       Tagdisc_t *disc)
{
...
static int
flat_field_delimiter_dat(Tag_t *tag,
                         Tagframe_t *fp,
                         const char *data,
                         Tagdisc_t *disc)
{
    ...
    if (flat->format->code != CC_NATIVE && (fp->attr & TAG_ATTR_conv))
        flat->format->delimiter
        = ccmapc(flat->format->delimiter, CC_NATIVE, flat->format->code);
    return 0;
}

static int
flat_field_multiple_dat(Tag_t *tag,
                        Tagframe_t *fp,
                        const char *data,
                        Tagdisc_t *disc)
{
...
static int
flat_field_escape_dat(Tag_t *tag,
                      Tagframe_t *fp,
                      const char *data,
                      Tagdisc_t *disc)
{
...
static int
flat_field_quotebegin_dat(Tag_t *tag,
                          Tagframe_t *fp,
                          const char *data,
                          Tagdisc_t *disc)
{
...
static int
flat_field_quoteend_dat(Tag_t *tag,
                        Tagframe_t *fp,
                        const char *data,
                        Tagdisc_t *disc)
{
...
static int
flat_field_quoteall_dat(Tag_t *tag,
                        Tagframe_t *fp,
                        const char *data,
                        Tagdisc_t *disc)
{
...
static int
flat_field_fixedpoint_dat(Tag_t *tag,
                          Tagframe_t *fp,
                          const char *data,
                          Tagdisc_t *disc)
{
...
static int
flat_field_width_dat(Tag_t *tag,
                     Tagframe_t *fp,
                     const char *data,
                     Tagdisc_t *disc)
{
...
static int
flat_field_remainder_dat(Tag_t *tag,
                         Tagframe_t *fp,
                         const char *data,
                         Tagdisc_t *disc)
{
...
static int
flat_field_codeset_dat(Tag_t *tag,
                       Tagframe_t *fp,
                       const char *data,
                       Tagdisc_t *disc)
{
...
static int
flat_array_delimiter_dat(Tag_t *tag,
                         Tagframe_t *fp,
                         const char *data,
                         Tagdisc_t *disc)
{
...
= { "SIZE",
    "Fixed array size or variable size field name.",
    0,
    0,
    flat_array_size_dat,
    0,
    "PHYSICAL",
    "Physical (file representation) details; the"
    ...
    0,
    flat_array_physical_beg,
    0,
    flat_array_physical_end,
    "DELIMITER",
    "Array value delimiter character.",
    0,
    0,
    flat_array_delimiter_dat,
    0,
    0 };

static Tags_t *
flat_array_physical_beg(Tag_t *tag,
                        Tagframe_t *fp,
                        const char *name,
                        Tagdisc_t *disc)
{
...
static int
flat_field_key_name_dat(Tag_t *tag,
                        Tagframe_t *fp,
                        const char *data,
                        Tagdisc_t *disc)
{
...
static int
flat_field_key_qualification_dat(Tag_t *tag,
                                 Tagframe_t *fp,
                                 const char *data,
                                 Tagdisc_t *disc)
{
    ...
    while (c = *s++) {
        if (*s == '[') {
            ...
            case ':':
                ...
                if (disc->errorf)
                    (*disc->errorf)(
                    NiL, disc, 2, "[%s: unknown character class", s);
                return -1;
            case '.':
            case '=':
                ...
                if (disc->errorf)
                    (*disc->errorf)(
                    NiL, disc, 2, "[%s: invalid collation element", s);
                return -1;
            }
        }
        tab->id[c] |= flags;
...
static int
flat_field_key_id1_dat(Tag_t *tag,
                       Tagframe_t *fp,
                       const char *data,
                       Tagdisc_t *disc)
{
...
static int
flat_field_key_id2_dat(Tag_t *tag,
                       Tagframe_t *fp,
                       const char *data,
                       Tagdisc_t *disc)
{
...
static int
flat_field_key_span_dat(Tag_t *tag,
                        Tagframe_t *fp,
                        const char *data,
                        Tagdisc_t *disc)
{
...
static int
flat_field_key_unknown_dat(Tag_t *tag,
                           Tagframe_t *fp,
                           const char *data,
                           Tagdisc_t *disc)
{
...
= { "NAME",
    ...
    0,
    0,
    flat_field_key_name_dat,
    0,
    "QUALIFICATION",
    "Key qualification expression; the key is active"
    ...
    0,
    0,
    flat_field_key_qualification_dat,
    0,
    "ID1",
    "Characters that may appear anywhere in key names,"
    " interpreted as the contents of an RE character"
    " class. The default is \b[:alpha:]_\b.",
    0,
    0,
    flat_field_key_id1_dat,
    0,
    "ID2",
    "Characters that may appear after the first character"
    " in key names, interpreted as the contents of an RE"
    " character class. The default is"
    " \b[:alnum:]_.,-\b.",
    0,
    0,
    flat_field_key_id2_dat,
    0,
    "SPAN",
    "The value is the key assignment character. If set"
    ...
    0,
    0,
    flat_field_key_span_dat,
    0,
    "UNKNOWN",
    "Span unknown but otherwise syntactically correct"
    " keys.",
    0,
    0,
    flat_field_key_unknown_dat,
    0,
    0 };

static Tags_t *
flat_field_key_beg(Tag_t *tag,
                   Tagframe_t *fp,
                   const char *name,
                   Tagdisc_t *disc)
{
    ...
    if (name && !(( Field_t * ) flat->lastfield->structure.parent)->table
        && !((( Field_t * ) flat->lastfield->structure.parent)->table
             = tabinit(flat, ( Dssdisc_t * ) disc)))
        return 0;
    return &tags_key[0];
}

static int
flat_field_key_dat(Tag_t *tag,
                   Tagframe_t *fp,
                   const char *data,
                   Tagdisc_t *disc)
{
...
= { "NAME",
    ...
    " are used as physical defaults.",
    0,
    flat_field_physical_beg,
    0,
    flat_field_physical_end,
    "MAP",
    "Field value map;"
    ...
    flat_field_map_end,
    "TYPE",
    "Field type. The intrinsic types are number and"
    ...
    0,
    "DETAILS",
    "An optional type-specific comma-separated string"
    " of name=value pairs.",
    0,
    0,
    flat_field_details_dat,
    0,
    "DELIMITER",
    "Field delimiter character.",
    0,
    0,
    flat_field_delimiter_dat,
    0,
    "MULTIPLE",
    "Multiple adjacent delimiters are equivalent to one.",
    0,
    0,
    flat_field_multiple_dat,
    0,
    "ESCAPE",
    "Field delimiter and/or quote escape character.",
    0,
    0,
    flat_field_escape_dat,
    0,
    "QUOTE",
    "Field quote begin and end character.",
    0,
    0,
    flat_field_quotebegin_dat,
    0,
    "QUOTEBEGIN",
    "Field quote begin character.",
    0,
    0,
    flat_field_quotebegin_dat,
    0,
    "QUOTEEND",
    "Field quote end character.",
    0,
    0,
    flat_field_quoteend_dat,
    0,
    "QUOTEALL",
    "Field quotes are the first and last characetrs.",
    0,
    0,
    flat_field_quoteall_dat,
    0,
    "CODESET",
    "Field codeset name; one of { native ascii ebcdic }.",
    0,
    0,
    flat_field_codeset_dat,
    0,
    "FIXEDPOINT",
    "Fixed point width.",
    0,
    0,
    flat_field_fixedpoint_dat,
    0,
    "WIDTH",
    "Field fixed width in bytes.",
    0,
    0,
    flat_field_width_dat,
    0,
    "REMAINDER",
    "Field is variable length up to the end of record.",
    0,
    0,
    flat_field_remainder_dat,
    0,
    "BASE",
    "Numeric field representation base.",
    0,
    0,
    flat_field_base_dat,
    0,
    "FILL",
    "Fixed width field fill character.",
    0,
    0,
    flat_field_fill_dat,
    0,
    "KEY",
    "name=value keyed field details. Keyed fields are"
    ...
    0,
    "CONSTRAINT",
    "Field value constraints. Constraints, when"
...
static Tags_t *
flat_field_physical_beg(Tag_t *tag,
                        Tagframe_t *fp,
                        const char *name,
                        Tagdisc_t *disc)
{
...
flat_field_beg(Tag_t *tag, Tagframe_t *fp, const char *name, Tagdisc_t *disc)
{
    ...
    if (name) {
        ...
        f->variable.format.delimiter = f->physical.format.delimiter
        = flat->delimiter;
        f->variable.format.flags = f->physical.format.flags = flat->flags;
        f->variable.format.escape = f->physical.format.escape = flat->escape;
        f->variable.format.quotebegin = f->physical.format.quotebegin
        = flat->quotebegin;
        f->variable.format.quoteend = f->physical.format.quoteend
        = flat->quoteend;
                   ...
                   Tagdisc_t *disc)
{
    ...
    default:
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, ERROR_SYSTEM | 2, "%s: invalid size field type", data);
        return -1;
    }
    return 0;
}
...
static int
flat_size_offset_dat(Tag_t *tag,
                     Tagframe_t *fp,
                     const char *data,
                     Tagdisc_t *disc)
{
...
= { "TYPE",
    "Size field physical type. The types are:"
    ...
    0,
    "OFFSET",
    "Size field offset in bytes.",
    0,
    0,
    flat_size_offset_dat,
    0,
    "BASE",
    "Size field field representation base.",
    0,
    0,
    flat_size_base_dat,
    0,
    "SIZE",
    "Size field total width in bytes, <WIDTH>+<OFFSET>"
...
static int
flat_section_count_dat(Tag_t *tag,
                       Tagframe_t *fp,
                       const char *data,
                       Tagdisc_t *disc)
{
...
static int
flat_section_delimiter_dat(Tag_t *tag,
                           Tagframe_t *fp,
                           const char *data,
                           Tagdisc_t *disc)
{
...
static int
flat_section_pattern_dat(Tag_t *tag,
                         Tagframe_t *fp,
                         const char *data,
                         Tagdisc_t *disc)
{
...
static int
flat_section_size_dat(Tag_t *tag,
                      Tagframe_t *fp,
                      const char *data,
                      Tagdisc_t *disc)
{
...
= { "COUNT",
    "Number of delimiters, patterns or sized blocks.",
    0,
    0,
    flat_section_count_dat,
    0,
    "DELIMITER",
    "Delimiter character.",
    0,
    0,
    flat_section_delimiter_dat,
    0,
    "PATTERN",
    "Regular expression; ignored if"
    " delimiter or size specified.",
    0,
    0,
    flat_section_pattern_dat,
    0,
    "SIZE",
    "Fixed size in bytes.",
    0,
    0,
    flat_section_size_dat,
    0,
    0 };
...
static int
flat_description_dat(Tag_t *tag,
                     Tagframe_t *fp,
                     const char *data,
                     Tagdisc_t *disc)
{
...
static int
flat_magic_string_dat(Tag_t *tag,
                      Tagframe_t *fp,
                      const char *data,
                      Tagdisc_t *disc)
{
...
static int
flat_magic_number_dat(Tag_t *tag,
                      Tagframe_t *fp,
                      const char *data,
                      Tagdisc_t *disc)
{
...
static Value_t flat_swap_val[] = {
    "none", SWAP_none, "native", SWAP_native, "be", SWAP_be, "le", SWAP_le,
};

static int
flat_magic_swap_dat(Tag_t *tag,
                    Tagframe_t *fp,
                    const char *data,
                    Tagdisc_t *disc)
{
...
static int
flat_magic_version_dat(Tag_t *tag,
                       Tagframe_t *fp,
                       const char *data,
                       Tagdisc_t *disc)
{
...
= { "STRING",
    ...
    0,
    0,
    flat_magic_string_dat,
    0,
    "NUMBER",
    "Magic number value.",
    0,
    0,
    flat_magic_number_dat,
    0,
    "LENGTH",
    "Magic number/string size in bytes.",
    0,
    0,
    flat_magic_size_dat,
    0,
    "SIZE",
    "Magic header size in bytes. If omitted then an AST"
    ...
    0,
    "SWAP",
    "Magic header binary field swap operation:"
    ...
    0,
    "VERSION",
    "Magic version stamp, either YYYYMMDD or 0xWWXXYYZZ.",
    0,
    0,
    flat_magic_version_dat,
    0,
    0 };
        ...
        if (flat->magic->size < flat->magic->length) {
            if (disc->errorf)
                (*disc->errorf)(NiL,
                                disc,
                                2,
                                "magic header size %u is smaller than "
                                "string/number length %u",
                                flat->magic->size,
                                flat->magic->length);
            return -1;
        }
        if (!flat->magic->string
            ...
            if (disc->errorf)
                (*disc->errorf)(NiL,
                                disc,
                                2,
                                "magic number length %u must be a power of 2 "
...
static int
flat_physical_swap_dat(Tag_t *tag,
                       Tagframe_t *fp,
                       const char *data,
                       Tagdisc_t *disc)
{
...
static int
flat_physical_continue_dat(Tag_t *tag,
                           Tagframe_t *fp,
                           const char *data,
                           Tagdisc_t *disc)
{
...
static int
flat_physical_delimiter_dat(Tag_t *tag,
                            Tagframe_t *fp,
                            const char *data,
                            Tagdisc_t *disc)
{
...
static int
flat_physical_escape_dat(Tag_t *tag,
                         Tagframe_t *fp,
                         const char *data,
                         Tagdisc_t *disc)
{
...
static int
flat_physical_quotebegin_dat(Tag_t *tag,
                             Tagframe_t *fp,
                             const char *data,
                             Tagdisc_t *disc)
{
...
static int
flat_physical_quoteend_dat(Tag_t *tag,
                           Tagframe_t *fp,
                           const char *data,
                           Tagdisc_t *disc)
{
...
static int
flat_physical_quoteall_dat(Tag_t *tag,
                           Tagframe_t *fp,
                           const char *data,
                           Tagdisc_t *disc)
{
...
static int
flat_physical_multiple_dat(Tag_t *tag,
                           Tagframe_t *fp,
                           const char *data,
                           Tagdisc_t *disc)
{
...
static int
flat_physical_codeset_dat(Tag_t *tag,
                          Tagframe_t *fp,
                          const char *data,
                          Tagdisc_t *disc)
{
...
static int
flat_physical_terminator_dat(Tag_t *tag,
                             Tagframe_t *fp,
                             const char *data,
                             Tagdisc_t *disc)
{
...
static Tags_t *
flat_physical_key_beg(Tag_t *tag,
                      Tagframe_t *fp,
                      const char *name,
                      Tagdisc_t *disc)
{
...
static Tags_t tags_flat_physical[] = {
    "SWAP",
    "Binary record field swap operation:"
    ...
    0,
    0,
    flat_physical_swap_dat,
    0,
    "DELIMITER",
    "Default field delimiter and continuation replacement"
    " character.",
    0,
    0,
    flat_physical_delimiter_dat,
    0,
    "ESCAPE",
    "Default field delimiter and/or quote escape character.",
    0,
    0,
    flat_physical_escape_dat,
    0,
    "QUOTE",
    "Default field quote begin and end character.",
    0,
    0,
    flat_physical_quotebegin_dat,
    0,
    "QUOTEBEGIN",
    "Default field quote begin character.",
    0,
    0,
    flat_physical_quotebegin_dat,
    0,
    "QUOTEEND",
    "Default field quote end character.",
    0,
    0,
    flat_physical_quoteend_dat,
    0,
    "QUOTEALL",
    "Field quotes are the first and last characetrs.",
    0,
    0,
    flat_physical_quoteall_dat,
    0,
    "MULTIPLE",
    "Multiple adjacent delimiters are equivalent to one.",
    0,
    0,
    flat_physical_multiple_dat,
    0,
    "CODESET",
    "Default records codeset name; one of { native ascii ebcdic }.",
    0,
    0,
    flat_physical_codeset_dat,
    0,
    "CONTINUE",
    "Terminator continuation (escape) character.",
    0,
    0,
    flat_physical_continue_dat,
    0,
    "TERMINATOR",
    "Default record termination character.",
    0,
    0,
    flat_physical_terminator_dat,
    0,
    "KEY",
    "Default field key attributes.",
    0,
    flat_physical_key_beg,
    0,
    0,
};

static Tags_t *
flat_physical_beg(Tag_t *tag,
                  Tagframe_t *fp,
                  const char *name,
                  Tagdisc_t *disc)
{
...
= { "NAME",
    ...
    0,
    "LIBRARY",
    "Required type/map library name;"
    ...
    0,
    "MAGIC",
    "File magic number (identification header)"
    " section definitions.",
    0,
    flat_magic_beg,
    0,
    ...
    0,
    "COMPRESS",
    "Preferred compression method; compression is applied"
    ...
    0,
    "HEADER",
    "File header section definitions.",
    0,
    flat_header_beg,
    0,
    0,
    "TRAILER",
    "File trailer section definitions.",
    0,
    flat_trailer_beg,
    0,
    0,
    ...
    flat_size_end,
    "RECORD",
    "Record size definitions; same as <BLOCK>.",
    0,
    flat_size_beg,
    0,
    flat_size_end,
    "PHYSICAL",
    "Default physical record and field attributes.",
    0,
    flat_physical_beg,
    0,
    0,
...
defaults(Cxtype_t *type, Cxformat_t *format, int binary, Dssdisc_t *disc)
{
    ...
    if (!(format->flags & (CX_INTEGER | CX_FLOAT | CX_STRING | CX_BUFFER)))
        format->flags |= type->format.flags
                         & (CX_BINARY | CX_UNSIGNED | CX_INTEGER | CX_FLOAT
                            | CX_STRING | CX_BUFFER);
                ...
                *s++ = 'l';
                ...
                default:
                    if (format->base < 64) {
                        ...
                        *s++ = '.';
                        s += sfsprintf(
                        s, &details[sizeof(details)] - s, "%d", format->base);
                        break;
                    }
                    /*FALLTHROUGH*/
                case 0:
         ...
         Dssmeth_t *meth)
{
    ...
    if (schema && *schema) {
        if (!(sp
            ...
            return 0;
        if (!(tag = tagopen(sp, path, 1, &tags[0], &flat->dsstagdisc.tagdisc))
            || tagclose(tag))
            goto drop;
        sfclose(sp);
        sp = 0;
    ...
    for (f = flat->fields; f; f = f->next) {
        ...
        if (!(f->physical.format.flags
              & (CX_INTEGER | CX_FLOAT | CX_STRING | CX_BUFFER)))
            f->physical.format.flags
                ...
                if (f->width && f->physical.format.width) {
                    ...
                    if (n > 0) {
                        if (disc->errorf)
                            ...
                            disc,
                            1,
                            "%s: maximum variable field size shortened from "
                            "%d to %d to comply with fixed record size %d",
                            f->variable.name,
                            f->physical.format.width,
                            n,
                            flat->record->reserve);
    ...
    if (flat) {
        if (flat->prototype) {
            for (f = flat->fields; f; f = f->next) {
                ...
                sfprintf(sfstdout,
                         "%s%c",
                         cxisvoid(f->variable.type) ? "" : f->variable.name,
                         n);
            }
        }
        switch (flat->list) {
        case 'h':
            ...
            *a = 0;
            ...
            sfprintf(sfstdout, " */\n\n");
            sfprintf(sfstdout,
                     "#define %s_RECORD(data)	"
                     "(_%s_record_=(%s_record_t*)DSSDATA(data))\n",
                     u,
                     p,
                     t);
            sfprintf(sfstdout, "\n");
            sfprintf(
            sfstdout, "typedef Cxvalue_t* (*%s_get_f)(void*,int);\n", t);
            sfprintf(sfstdout, "\n");
            sfprintf(
            sfstdout,
            "typedef struct %s_field_s		/* record field			*/\n",
            t);
            sfprintf(sfstdout, "{\n");
            sfprintf(sfstdout,
                     "	Cxvalue_t	value;		/* value (first for dynamic "
                     "Q)	*/\n");
            sfprintf(
            sfstdout,
            ...
            sfprintf(
            sfstdout,
            "typedef struct %s_record_s		/* current record info		*/\n",
            t);
            sfprintf(sfstdout, "{\n");
            sfprintf(
            sfstdout,
            "	%s_field_t*	fields;		/* fields (first for dynamic Q)	*/\n",
            t);
            sfprintf(
            sfstdout,
            "	%s_get_f	getf;		/* getf (second for dynamic Q)	*/\n",
            t);
            ...
            for (f = flat->fields; f; f = f->next) {
                ...
                n += sfprintf(sfstdout,
                              "((*_%s_record_->getf)(_%s_record_,%d)->",
                              p,
                              p,
                              f->variable.index);
                ...
                if (f->variable.format.flags & (CX_BUFFER | CX_STRING)) {
                    ...
                    sfprintf(
                    sfstdout,
                    "((*_%s_record_->getf)(_%s_record_,%d)->%s.size)\n",
                    p,
                    p,
                    f->variable.index,
                    a);
        ...
        case 'o':
            if (!flat->fixed) {
                if (disc->errorf)
                    ...
                    disc,
                    2,
                    "variable width record offsets not supported");
                return -1;
            }
                ...
                if (f->variable.format.flags & CX_BUFFER) {
                    ...
                    if ((offset % f->physical.format.width)
                        ...
                        if (disc->errorf)
                            ...
                            disc,
                            1,
                            "%s: field size %u offset %u is not aligned",
                            f->variable.name,
                            f->physical.format.width,
                            offset);
                    }
            ...
            if (flat->binary && (fixed = offset % 8)) {
                ...
                if (disc->errorf)
                    (*disc->errorf)(NiL,
                                    disc,
                                    1,
                                    "%s: record has %u unused pad byte%s",
                                    flat->meth.name,
                                    fixed,
                                    fixed == 1 ? "" : "s");
            }
        ...
        case 's':
            if (!flat->fixed) {
                if (disc->errorf)
                    ...
                    disc,
                    2,
                    "variable width record structs not supported");
                return -1;
            }
            for (f = flat->fields; f; f = f->next)
                if (f->variable.format.flags & CX_BUFFER) {
            ...
            for (f = flat->fields; f; f = f->next) {
                if ((f->variable.format.flags & CX_BINARY)
                    && ((offset % f->physical.format.width)
                        || (f->physical.format.width
                            & (f->physical.format.width - 1))
                        || f->physical.format.width > 8)
                    || f->width)
                    break;
                offset += f->physical.format.width;
            }
...
= { "flat",   "fixed-width and/or field-delimited flat file data",
    CXH,      flatmeth,
    flatopen, flatclose,
    0,        0,
    0,        0,
...
flatten_beg(Cx_t *cx, Cxexpr_t *expr, void *data, Cxdisc_t *disc)
{
    ...
    if (!flatten->cx->getf
        && !(flatten->cx->getf = cxcallout(flatten->cx,
                                           CX_GET,
        ...
        if (!(f->flatten = ( Cxvariable_t * ) g)) {
            ...
            if (cxisnumber(f->variable.type) && cxisstring(f->flatten->type))
                f->flattengetf = flattenstr2num;
                     ...
                     && cxisnumber(f->flatten->type))
                f->flattengetf = flattennum2str;
        }
        if (g && g->physical.format.code != f->physical.format.code
    ...
    if (flatten->flat->binary && (offset %= 8)) {
        ...
        if (disc->errorf)
            (*disc->errorf)(NiL,
                            ...
                            2,
                            "%s: record has %u unused pad byte%s",
                            flatten->dss->meth->name,
                            offset,
                            offset == 1 ? "" : "s");
        goto bad;
    }
    if (!(flatten->file = dssfopen(
          flatten->dss, NiL, expr->op, DSS_FILE_WRITE, &flat_format)))
        goto bad;
...
flatten_act(Cx_t *cx, Cxexpr_t *expr, void *data, Cxdisc_t *disc)
{
        ...
        if (n < 0)
            ...
            if ((f->physical.format.flags & (CX_STRING | CX_BUFFER))
                ...
                if (f->physical.format.flags & CX_QUOTEALL) {
                    ...
                    q = 0;
                for (e = (s = b = ( unsigned char * ) flatten->flat->valbuf)
                         + n;
                     s < e;
                     s++)
                    if (*s == f->physical.format.delimiter
    ...
    if (flatten->flat->record) {
        ...
        if (size_put(flatten->file, n, flatten->flat->record, disc)
            || sfwrite(flatten->file->io, sfstrbase(io), n) != n)
            return -1;
    }
    return 0;
}
...
= { { "flatten",
      "query to flatten input data to flat schema format",
      CXH,
      flatten_beg,
      0,
      flatten_act,
