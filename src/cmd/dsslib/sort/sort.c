...
sort_beg(Cx_t *cx, Cxexpr_t *expr, void *data, Cxdisc_t *disc)
{
    ...
    if (!(state->file
          = dssfopen(DSS(cx), "-", state->sortstack, DSS_FILE_WRITE, 0)))
        goto bad;
    sfprintf(
    ...
    while (s = *argv++)
    {
        ...
        if (!(variable = cxvariable(cx, s, NiL, disc)))
            goto bad;
        if (rskey(
            state->sortkey, t ? t : cxisnumber(variable->type) ? num : "", 0))
            goto bad;
...
Dsslib_t dss_lib_sort = { "sort",
                          "sort query"
                          "[-1lms5P?\n@(#)$Id: dss sort query (AT&T "
                          "Research) 2011-10-18 $\n]" USAGE_LICENSE,
                          CXH,
                          0,
                          0,
