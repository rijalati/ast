            ...
            if (file->format->readf == xmlread)
            {
                if (!xml->root)
                {
                    if (disc->errorf)
                        ...
                        disc,
                        2,
                        "%s variable names must be qualified by at least the "
                        "immediate containing tag",
                        file->format->name);
                    return -1;
                }
                x = *end;
...
static Dssformat_t xml_format = { "xml",     "xml format (2010-05-19)",
                                  CXH,       xmlident,
                                  xmlfopen,  xmlread,
                                  xmlwrite,  0,
                                  xmlfclose, 0,
...
static Dssformat_t json_format = { "json",    "json format (2010-05-19)",
                                   CXH,       jsonident,
                                   xmlfopen,  jsonread,
                                   jsonwrite, 0,
                                   xmlfclose, 0,
       ...
       Cxdisc_t *disc)
{
    return (r->value.variable
            = xmlvar(cx, b->value.string.data, "string", disc))
           ? 0
           : -1;
}
...
static Tags_t tags[]
= { "METHOD", "Method name; must be xml.", 0, 0,       0, 0,
    "XML",    "xml method schema.",        0, xml_beg, 0, 0,
    0 };
        ...
        Dssmeth_t *meth)
{
    ...
    if (schema && *schema)
    {
        if (!(sp
            ...
            return 0;
        if (!(tag = tagopen(sp, path, 1, &tags[0], &xml->dsstagdisc.tagdisc))
            || tagclose(tag))
            goto drop;
        sfclose(sp);
        sp = 0;
...
= { "xml", "xml and json method", CXH, xmlmeth, xmlopen, 0, 0 };
