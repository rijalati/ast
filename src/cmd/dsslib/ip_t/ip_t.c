...
{
    return itl2external(
    cx, type, 0, 1, 1, details, &format, value, buf, size, disc);
}

static ssize_t
as16path_internal(Cx_t *cx,
                  Cxtype_t *type,
                  const char *details,
                  Cxformat_t *format,
                  Cxoperand_t *ret,
    ...
    if (!cxisstring(pat)) {
        if (disc->errorf)
            (*disc->errorf)(NiL,
                            disc,
                            2,
                            "%s: match requires %s pattern",
                            sub->name,
                            cx->state->type_string->name,
                            sub->name);
        return 0;
    }
...
{
    return itl4external(
    cx, type, 0, 1, 0, details, &format, value, buf, size, disc);
}

static ssize_t
cluster_internal(Cx_t *cx,
                 Cxtype_t *type,
                 const char *details,
                 Cxformat_t *format,
                 Cxoperand_t *ret,
                   ...
                   Cxdisc_t *disc)
{
    ...
    formats[1] = format;
    return itl2external(
    cx, type, 0, 2, 0, details, formats, value, buf, size, disc);
}

static ssize_t
community_internal(Cx_t *cx,
                   Cxtype_t *type,
                   const char *details,
                   Cxformat_t *format,
                   Cxoperand_t *ret,
                  ...
                  Cxdisc_t *disc)
{
    ...
    formats[1] = format;
    return itl1external(
    cx, type, 0, 8, 0, details, formats, value, buf, size, disc);
}

static ssize_t
extended_internal(Cx_t *cx,
                  Cxtype_t *type,
                  const char *details,
                  Cxformat_t *format,
                  Cxoperand_t *ret,
                    ...
                    Cxdisc_t *disc)
{
    ...
    formats[1] = format;
    return itl1external(
    cx, type, 0, 1, 0, details, formats, value, buf, size, disc);
}

static ssize_t
identifier_internal(Cx_t *cx,
                    Cxtype_t *type,
                    const char *details,
                    Cxformat_t *format,
                    Cxoperand_t *ret,
...
= { "as16path-re",
    "Matches on this type treat a string pattern as an ire(3) 16 bit integer "
    "list regular expression. Each number in the list is a distinct token. ^ "
    "$ * + . {n,m} [N1 .. Nn] are supported, and - is equivalent to .*. "
    "Adjacent numbers may be separated by space, comma, / or _; multiple "
    "adjacent separators are ignored in the match. For example, '[!1 100]' "
    "matches all lists that contain neither 1 nor 100, and '^[!1 100]-701$' "
    "matches all lists that don't start with 1 or 100 and end with 701.",
    CXH,
    aspath_match_comp,
    aspath_match_exec,
    aspath_match_free };
...
= { "as32path-re",
    "Matches on this type treat a string pattern as an ire(3) 32 bit integer "
    "list regular expression. Each number in the list is a distinct token. ^ "
    "$ * + . {n,m} [N1 .. Nn] are supported, and - is equivalent to .*. "
    "Adjacent numbers may be separated by space, comma, / or _; multiple "
    "adjacent separators are ignored in the match. For example, '[!1 100]' "
...
= { "aspath-re",      0, CXH, aspath_match_comp, aspath_match_exec,
    aspath_match_free };
...
= { "cluster-re",
    "Matches on this type treat a string pattern as an ire(3) integer list "
    "regular expression. Each number in the list is a distinct token. ^ $ * "
    "+ . {n,m} [N1 .. Nn] are supported, and - is equivalent to .*. Adjacent "
    "numbers may be separated by space, comma, / or _; multiple adjacent "
    "separators are ignored in the match. For example, '[!1 100]' matches "
    "all lists that contain neither 1 nor 100, and '^[!1 100]-701$' matches "
    "all lists that don't start with 1 or 100 and end with 701.",
    CXH,
    cluster_match_comp,
    match_list_exec,
    match_list_free };
...
= { "community-re",
    "Matches on this type treat a string pattern as an ire(3) integer list "
    "regular expression. Each number in the list is a distinct token. Pairs "
    "are separated by :. ^ $ * + . {n,m} [N1 .. Nn] are supported, and - is "
    "equivalent to .*. Adjacent numbers may be separated by space, comma, / "
    "or _; multiple adjacent separators are ignored in the match. If a : "
    "tuple separator is omitted then :.* is assumed. For example, '[!1 100]' "
    "matches all lists that contain neither 1 nor 100 as the first pair "
    "element, and '^[!1: :100]-701:999$' matches all lists that don't start "
    "with 1 as the first pair element or 100 as the second pair element and "
    "end with 701:999.",
    CXH,
...
= { "extended-re",
    "Matches on this type treat a string pattern as an ire(3) integer list "
    "regular expression for tuples with 8 integer elements in the range "
    "0..255. Each number in the list is a distinct token. Elements are "
    "separated by :. ^ $ * + . {n,m} [N1 .. Nn] are supported, and - is "
    "equivalent to .*. Adjacent numbers may be separated by space, comma, / "
    "or _; multiple adjacent separators are ignored in the match. If a : "
    "tuple separator is omitted then :.* is assumed. For example, '[!1 100]' "
    "matches all lists that contain neither 1 nor 100 as the first tuple "
    "element, and '^[!1: :100]-:201:199$' matches all lists that don't start "
    "with 1 as the first tuple element or 100 as the last tuple element and "
    "end with :201:199.",
    CXH,
    extended_match_comp,
    match_list_exec,
...
= { "values-re",
    ...
    "separators are ignored in the match. For example, '[!1 100]' matches "
    "all lists that contain neither 1 nor 100, and '^[!1 100]-701$' matches "
    "all lists that don't start with 1 or 100 as the first element and end "
    "with 701.",
    CXH,
    values_match_comp,
    match_list_exec,
...
= { "prefix-v4-match",
    "Matches on this type treat a string pattern as an ipv4 prefix table and "
    "test whether the subject is matched by the table. If the first "
    "character of the pattern is \b<\b then the remainder of the string is "
    "the path name of a file containing a prefix table. If the pattern is a "
    "\bipv4prefix_t\b then matches test if the subject is matched by the "
    "prefix.",
    CXH,
    prefix_match_comp,
    prefix_match_exec,
...
= { "prefix-v6-match",
    "Matches on this type treat a string pattern as an ipv6 prefix table and "
    "test whether the subject is matched by the table. If the first "
    "character of the pattern is \b<\b then the remainder of the string is "
    "the path name of a file containing a prefix table. If the pattern is an "
    "\bipv6prefix_t\b then matches test if the subject is matched by the "
    "prefix.",
    CXH,
    prefix_match_comp,
    prefix_match_exec,
...
= { "prefix-match",   0, CXH, prefix_match_comp, prefix_match_exec,
    prefix_match_free };
                ...
                Cxdisc_t *disc)
{
    ...
    r->value.number
    = (PTMIN(aa, ab) >= PTMIN(ba, bb) && PTMAX(aa, ab) <= PTMAX(ba, bb))
      == (pc->op == CX_MATCH);
    return 0;
}

static int
op_match_ip6_NP(Cx_t *cx,
                Cxinstruction_t *pc,
                Cxoperand_t *r,
                Cxoperand_t *a,
                Cxoperand_t *b,
                ...
                Cxdisc_t *disc)
{
    ...
    if (!(ap = ( unsigned char * ) a->value.buffer.data)
        ...
        r->value.number = (fvcmp(16,
                                 ptvmin(16, r0, ap, ap[IP6BITS]),
                                 ptvmin(16, r1, bp, bp[IP6BITS]))
                           >= 0
                           && fvcmp(16,
                                    ptvmax(16, r0, ap, ap[IP6BITS]),
                                    ptvmax(16, r1, bp, bp[IP6BITS]))
                              <= 0)
                          == (pc->op == CX_MATCH);
    return 0;
}
...
= { { "as16_t",
      ...
      2,
      0,
      { 0, 0, CX_UNSIGNED | CX_INTEGER, 2, 5 },
      0 },
    { "as32_t",
      ...
      4,
      0,
      { 0, 0, CX_UNSIGNED | CX_INTEGER, 4, 11 },
      0 },
    { "as_t",
      0,
      CXH,
    ...
    { "as16path_t",
      ...
      0,
      2,
      { "The format details string is the format character (\b1\b or \b.\b: "
        "dotted 1-byte, \bd\b: signed decimal, \bo\b: octal, \bx\b: "
        "hexadecimal, \bu\b: unsigned decimal (default)), followed by the "
        "separator string.",
        "u," },
      &match_as16path },
    { "as32path_t",
      ...
      4,
      { "The format details string is the format character (\b1\b or \b.\b: "
        "dotted 1-byte, \b2\b: dotted 2-byte, \bd\b: signed decimal, \bo\b: "
        "octal, \bx\b: hexadecimal, \bu\b: unsigned decimal (default)), "
        "followed by the separator string.",
        "u," },
      &match_as32path },
    { "aspath_t",
      0,
    ...
    { "cluster_t",
      ...
      0,
      4,
      { "The format details string is the format character (\b.\b: dotted "
        "quad, \bd\b: signed decimal, \bo\b: octal, \bx\b: hexadecimal, "
        "\bu\b: default unsigned decimal), followed by the separator string.",
        ".," },
      &match_cluster },
    { "community_t",
    ...
    { "extended_t",
      ...
      8,
      { "The format details string is the format character (\b.\b: dotted "
        "elements, \bd\b: signed decimal, \bo\b: octal, \bx\b: hexadecimal, "
        "\bu\b: default unsigned decimal), followed by the separator string.",
        "u," },
      &match_extended },
    { "ipv4addr_t",
      ...
      4,
      0,
      { 0, 0, CX_UNSIGNED | CX_INTEGER, 4, 16 },
      &match_prefixv4 },
    { "ipv6addr_t",
    ...
    { "ipv4prefix_t",
      ...
      5,
      0,
      { "The format details string is a \bprintf\b(3) format specification "
        "for the integer arguments \aaddress,bits\a; e.g., \b%2$u|%1$08x\b "
        "prints the decimal bits followed by the hexadecimal prefix address.",
        0,
        CX_UNSIGNED | CX_INTEGER,
        8,
        19 },
      &match_prefixv4 },
    { "ipv6prefix_t",
      "/length appended to an ipv6addr_t prefix. The details string \"C\" "
