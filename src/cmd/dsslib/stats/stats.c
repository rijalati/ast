...
struct Total_s;
typedef struct Total_s Total_t;
...
stats_beg(Cx_t *cx, Cxexpr_t *expr, void *data, Cxdisc_t *disc)
{
    ...
    for (;;) {
        ...
        case 'g':
            ...
            group->string
            = cxisstring(variable->type) || cxisbuffer(variable->type);
            if (state->group)
            ...
            for (group = state->group; group; group = group->next) {
                group->width = group->variable->format.print
                               ...
                               : group->variable->type->format.print
                                 ...
                                 : FW;
                sfprintf(
                expr->op, " %*s", group->width, group->variable->name);
            }
        sfprintf(expr->op, "\n");
    }
    if (state->buckets) {
...
static Cxquery_t queries[] = { { "stats",
...
Dsslib_t dss_lib_stats = { "stats",
                           "stats query"
                           "[-1lms5P?\n@(#)$Id: dss stats query (AT&T "
                           "Research) 2011-09-11 $\n]" USAGE_LICENSE,
                           CXH,
                           0,
                           0,
