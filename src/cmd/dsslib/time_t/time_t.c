                 ...
                 Cxdisc_t *disc)
{
    ...
    s
    = tmxfmt(buf, size, CXDETAILS(details, format, type, precise->format), t);
    if (s == (buf + size - 1))
        return 2 * size;
    return s - buf;
}
...
static Cxvariable_t tm_struct[] = {
    ...
    CXV("isdst",
        "number",
        TIME_T_isdst,
        "Daylight savings time in effect [0-1].")
    CXV("ns", "number", TIME_T_ns, "Residual nanoseconds [0-999999999]."){ 0 }
};
...
= { { "tm_hour_t",
      ...
      0,
      0,
      { "The format details string is a \bprintf\b(3) format string.",
        "%d",
    ...
    { "ns_t",
      ...
      0,
      0,
      { "The format details string is a \bstrftime\b(3)/\bstrptime\b(3) "
        "format string.",
        "%K.%9N",
        CX_UNSIGNED | CX_INTEGER,
        8 } },
