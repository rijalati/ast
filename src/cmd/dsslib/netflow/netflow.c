...
static Cxvariable_t fields[] = {
    ...
    CXV("bytes",
        "number",
        NETFLOW_bytes,
        "Bytes sent in duration (synthesized).")
    CXV("count",
        "number",
        NETFLOW_count,
        "Number of records that follow in packet (header).")
    CXV("direction",
        "number",
        NETFLOW_direction,
    ...
    CXV("dst_addrv4",
        "ipv4addr_t",
        NETFLOW_dst_addrv4,
        "Destination ipv4 address.") CXV("dst_addrv6",
                                         "ipv6addr_t",
                                         NETFLOW_dst_addrv6,
    ...
    CXV("dst_maskv4",
        "number",
        NETFLOW_dst_maskv4,
        "Destination ipv4 address prefix mask bits.")
    CXV("dst_maskv6",
        "number",
        NETFLOW_dst_maskv6,
        "Destination ipv6 address prefix mask bits.")
    CXV("dst_port",
        "number",
        NETFLOW_dst_port,
        "TCP/UDP destination port number.") CXV("dst_prefix",
                                                "ipprefix_t",
                                                NETFLOW_dst_prefix,
                                                "Destination ip prefix.")
    CXV("dst_prefixv4",
        "ipv4prefix_t",
        NETFLOW_dst_prefixv4,
        "Destination ipv4 prefix.") CXV("dst_prefixv6",
                                        "ipv6prefix_t",
                                        NETFLOW_dst_prefixv6,
    ...
    CXV("end",
        "ns_t",
        NETFLOW_end,
        "Flow end time in 64 bit nanoseconds since the epoch (synthesized).")
    CXV("engine_id",
        "number",
        NETFLOW_engine_id,
    ...
    CXV("engine_type",
        "number",
        NETFLOW_engine_type,
        "Type of flow switching engine 0: RP, 1: VIP/linecard.")
    CXV("first",
        "elapsed_t",
        NETFLOW_first,
        "Elapsed milliseconds at flow start.")
    CXV("flags", "number", NETFLOW_flags, "Reason flow was discarded, etc.")
    CXV("flow_active_timeout",
        "number",
        NETFLOW_flow_active_timeout,
        "Timeout value (in seconds) for active flow cache entries.")
    CXV("flow_inactive_timeout",
        "number",
        NETFLOW_flow_inactive_timeout,
        "Timeout value (in seconds) for inactive flow cache entries.")
    CXV("flow_label",
        "number",
        NETFLOW_flow_label,
        "ipv6 RFC 2460 flow label.") CXV("flow_sequence",
                                         "number",
                                         NETFLOW_flow_sequence,
                                         "Flow sequence counter (header).")
    CXV("flows",
        "number",
        NETFLOW_flows,
    ...
    CXV("forwarding_code",
        "number",
        NETFLOW_forwarding_code,
        "Forwarding reason code.") CXV("forwarding_status",
                                       "number",
                                       NETFLOW_forwarding_status,
                                       "Forwarding status 0: unknown, 1: "
                                       "forwarded, 2: dropped, 3: consumed.")
    CXV("fragment_offset",
        "number",
        NETFLOW_fragment_offset,
        "Fragmented packet fragment-offset.")
    ...
    CXV("icmp_type",
        "number",
        NETFLOW_icmp_type,
        "Internet Control Message Protocol packet type coded as "
        "((type*256)+code).")
    ...
    CXV("in_bytes",
        "number",
        NETFLOW_in_bytes,
        "Incoming counter for the number of bytes associated with an ip "
        "Flow.") CXV("in_dst_mac",
                     "number",
                     NETFLOW_in_dst_mac,
                     "Incoming destination MAC address.")
    CXV("in_permanent_bytes",
        "number",
        NETFLOW_in_permanent_bytes,
        "Permanent flow byte count.") CXV("in_permanent_packets",
                                          "number",
                                          NETFLOW_in_permanent_packets,
                                          "Permanent flow packet count.")
    CXV("in_packets",
        "number",
        NETFLOW_in_packets,
        "Incoming counter for the number of packets associated with an ip "
        "Flow.") CXV("in_src_mac",
                     "number",
                     NETFLOW_in_src_mac,
                     "Incoming source MAC address.")
    CXV("input_snmp", "number", NETFLOW_input_snmp, "Input interface index.")
    CXV("ip_protocol_version",
        "number",
        NETFLOW_ip_protocol_version,
        "ip version 6: ipv6, 4 or not specified: ipv4.")
    CXV("last", "elapsed_t", NETFLOW_last, "Elapsed milliseconds at flow end.")
    CXV("max_packet_length",
        "number",
        NETFLOW_max_packet_length,
        "Maximum incoming ip packet length.")
    CXV("max_ttl",
        "number",
        NETFLOW_max_ttl,
        "Maximum TTL on incoming packets.")
    CXV("min_packet_length",
        "number",
        NETFLOW_min_packet_length,
        "Minimum incoming ip packet length.")
    CXV("min_ttl",
        "number",
        NETFLOW_min_ttl,
        "Minimum TTL on incoming packets.")
    CXV("mpls_label_1",
        "number",
        NETFLOW_mpls_label_1,
        "Stack position 1 MPLS label: 20 bits MPLS label, 3 bits "
        "experimental, 1 bit end-of-stack.")
    CXV("mpls_label_2",
        "number",
        NETFLOW_mpls_label_2,
        "Stack position 2 MPLS label: 20 bits MPLS label, 3 bits "
        "experimental, 1 bit end-of-stack.")
    CXV("mpls_label_3",
        "number",
        NETFLOW_mpls_label_3,
        "Stack position 3 MPLS label: 20 bits MPLS label, 3 bits "
        "experimental, 1 bit end-of-stack.")
    CXV("mpls_label_4",
        "number",
        NETFLOW_mpls_label_4,
        "Stack position 4 MPLS label: 20 bits MPLS label, 3 bits "
        "experimental, 1 bit end-of-stack.")
    CXV("mpls_label_5",
        "number",
        NETFLOW_mpls_label_5,
        "Stack position 5 MPLS label: 20 bits MPLS label, 3 bits "
        "experimental, 1 bit end-of-stack.")
    CXV("mpls_label_6",
        "number",
        NETFLOW_mpls_label_6,
        "Stack position 6 MPLS label: 20 bits MPLS label, 3 bits "
        "experimental, 1 bit end-of-stack.")
    CXV("mpls_label_7",
        "number",
        NETFLOW_mpls_label_7,
        "Stack position 7 MPLS label: 20 bits MPLS label, 3 bits "
        "experimental, 1 bit end-of-stack.")
    CXV("mpls_label_8",
        "number",
        NETFLOW_mpls_label_8,
        "Stack position 8 MPLS label: 20 bits MPLS label, 3 bits "
        "experimental, 1 bit end-of-stack.")
    CXV("mpls_label_9",
        "number",
        NETFLOW_mpls_label_9,
        "Stack position 9 MPLS label: 20 bits MPLS label, 3 bits "
        "experimental, 1 bit end-of-stack.")
    CXV("mpls_label_10",
        "number",
        NETFLOW_mpls_label_10,
        "Stack position 10 MPLS label: 20 bits MPLS label, 3 bits "
        "experimental, 1 bit end-of-stack.")
    CXV("mpls_top_label_class",
        "number",
        NETFLOW_mpls_top_label_class,
        "Forwarding Equivalent Class corresponding to the MPLS Top Label.")
    CXV("mpls_top_label_type",
        "number",
        NETFLOW_mpls_top_label_type,
        "MPLS top label type: 0x00 UNKNOWN 0x01 TE-MIDPT 0x02 ATOM 0x03 VPN "
        "0x04 BGP 0x05 LDP.") CXV("mul_dst_bytes",
                                  "number",
                                  NETFLOW_mul_dst_bytes,
                                  "Multicast outgoing byte count.")
    CXV("mul_dst_packets",
        "number",
        NETFLOW_mul_dst_packets,
        "Multicast outgoing packet count.")
    CXV("mul_igmp_type",
        "number",
        NETFLOW_mul_igmp_type,
        "Internet Group Management Protocol packet type coded.")
    CXV("nsec", "number", NETFLOW_nsec, "Residual nanoseconds (header).") CXV(
    "option_headers",
    "number",
    NETFLOW_option_headers,
    "Bit-encoded field identifying ipv6 option headers found in the flow.")
    CXV("out_bytes",
        "number",
        NETFLOW_out_bytes,
        "Outgoing counter for the number of bytes associated with an ip "
        "Flow.") CXV("out_dst_mac",
                     "number",
                     NETFLOW_out_dst_mac,
    ...
    CXV("out_packets",
        "number",
        NETFLOW_out_packets,
        "Outgoing counter for the number of packets associated with an ip "
        "Flow.") CXV("out_src_mac",
                     "number",
                     NETFLOW_out_src_mac,
    ...
    CXV("router_sc",
        "ipaddr_t",
        NETFLOW_router_sc,
        "Router shortcut ip address (V7).")
    CXV("router_scv4",
        "ipv4addr_t",
        NETFLOW_router_scv4,
        "Router shortcut ipv4 address.") CXV("router_scv6",
                                             "ipv6addr_t",
                                             NETFLOW_router_scv6,
                                             "Router shortcut ipv6 address.")
    CXV("sampler_algorithm",
        "number",
        NETFLOW_sampler_algorithm,
        "0x01: deterministic, 0x02: random.") CXV("sampler_interval",
                                                  "number",
                                                  NETFLOW_sampler_interval,
                                                  "Sampling interval.")
    ...
    CXV("src_maskv4",
        "number",
        NETFLOW_src_maskv4,
        "Source ipv4 address prefix mask bits.")
    CXV("src_maskv6",
        "number",
        NETFLOW_src_maskv6,
        "Source ipv6 address prefix mask bits.")
    ...
    CXV("src_prefixv4",
        "ipv4prefix_t",
        NETFLOW_src_prefixv4,
        "Source ipv4 prefix.") CXV("src_prefixv6",
                                   "ipv6prefix_t",
                                   NETFLOW_src_prefixv6,
                                   "Source ipv6 prefix.")
    CXV("src_tos",
        "number",
        NETFLOW_src_tos,
        "ip type-of-service upon entering incoming interface.")
    CXV("src_vlan",
        "number",
        NETFLOW_src_vlan,
    ...
    CXV("start",
        ...
        "(synthesized).") CXV("tcp_flags",
                              "number",
                              NETFLOW_tcp_flags,
                              "Cumulative OR of tcp flags for this flow.")
    CXV("tcp_misseq_cnt",
        "number",
        NETFLOW_tcp_misseq_cnt,
        "Number of mis-sequenced tcp packets (V1).")
    CXV("tcp_retx_cnt",
        "number",
        NETFLOW_tcp_retx_cnt,
        "Number of mis-seq with delay > 1sec (V1).")
    CXV("tcp_retx_secs",
        "number",
        NETFLOW_tcp_retx_secs,
        "Number of seconds between mis-sequenced packets (V1).")
    CXV("time",
        "time_t",
        NETFLOW_time,
        "Current time in seconds since the epoch (header).")
    CXV("tos", "number", NETFLOW_tos, "ip type-of-service (synthesized).")
    CXV("total_bytes_exp",
        "number",
        NETFLOW_total_bytes_exp,
        "The number of bytes exported by the observation domain.")
    CXV("total_flows_exp",
        "number",
        NETFLOW_total_flows_exp,
        "The number of flows exported by the observation domain.")
    CXV("total_packets_exp",
        "number",
        NETFLOW_total_packets_exp,
        "The number of packets exported by the observation domain.")
    CXV("uptime",
        "elapsed_t",
        NETFLOW_uptime,
        "Elapsed milliseconds since the router booted (header).")
       ...
       Cxdisc_t *disc)
{
    ...
    case NETFLOW_dst_mask:
        r->value.number = (rp->set & NETFLOW_SET_dst_addrv6)
                          ? rp->dst_addrv6[IP6BITS]
                          : rp->dst_maskv4;
        break;
    ...
    case NETFLOW_packets:
        r->value.number
        = rp->packets ? rp->packets
                      : rp->out_packets ? rp->out_packets : rp->in_packets;
        break;
    ...
    case NETFLOW_src_mask:
        r->value.number = (rp->set & NETFLOW_SET_src_addrv6)
                          ? rp->src_addrv6[IP6BITS]
                          : rp->src_maskv4;
        break;
    ...
    default:
        if (disc->errorf)
            (*disc->errorf)(cx,
                            disc,
                            ERROR_PANIC,
                            "%s: variable index %d not implemented",
                            vp->name,
                            vp->index);
        return -1;
    }
...
#define BUFFER(n, f)                                                         \
    {                                                                        \
        offsetof(Netflow_t, n), sizeof(flow->n), NETFLOW_BUFFER, f           \
    }
#define NUMBER(n, f)                                                         \
    {                                                                        \
        offsetof(Netflow_t, n), sizeof(flow->n), NETFLOW_NUMBER, f           \
    }
...
= { "netflow",
    ...
    0,
    0,
    "%(time:%+u%K)s %(prot)d %(src_addr)s:%(src_port)d "
    "%(dst_addr)s:%(dst_port)d %(hop)s" };
