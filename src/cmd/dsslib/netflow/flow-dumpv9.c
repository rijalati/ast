        ...
        for (;;) {
            ...
            while (!pp->count--) {
                ...
                if (file->dss->flags & DSS_DEBUG)
                    sfprintf(
                    sfstderr,
                    "header version %d size %d flowsets %d offset %I*d\n",
                    rp->version,
                    20,
                    pp->count,
                    sizeof(file->offset),
            ...
            if (n > FLOW_META) {
                ...
                if (!tp || !tp->size) {
                    if (!(file->dss->flags & DSS_QUIET) && disc->errorf)
                        ...
                        disc,
                        1,
                        "%s%d: undefined template -- flowset ignored",
                        cxlocation(file->dss->cx, record),
                        n);
                    file->count++;
                    pp->count = 0;
                    continue;
            ...
            if (n == FLOW_TEMPLATE) {
                ...
                while (pp->data < (pp->last - 6)) {
                    ...
                    while (m--) {
                        ...
                        if (file->dss->flags & DSS_DEBUG)
                            sfprintf(
                            sfstderr,
                            "template %2d scope %2d size %2d offset %3d\n",
                            tp->id,
                            n,
                            z,
                            o);
                ...
                if (!(file->dss->flags & DSS_QUIET) && disc->errorf)
                    ...
                    disc,
                    1,
                    "%s%d: template index %d definition ignored",
                    cxlocation(file->dss->cx, record),
                    n);
                file->count++;
            }
...
= { "dumpv9",
    "Cisco netflow v9 dump format (2008-12-10) flow templates are retained "
    "across input files",
    CXH,
    dumpv9ident,
    dumpv9fopen,
