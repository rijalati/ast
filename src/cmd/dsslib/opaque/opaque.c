...
opaqueread(Dssfile_t *file, Dssrecord_t *record, Dssdisc_t *disc)
{
    ...
    if (record->size && disc->errorf)
        (*disc->errorf)(NiL,
                        disc,
                        2,
                        "%slast record truncated -- record ignored",
...
{
    return sfwrite(file->io, record->data, record->size) == record->size ? 0
                                                                         : -1;
}

static Dssformat_t opaque_format = { "opaque",
    ...
    if (options) {
        ...
        for (;;) {
            ...
            case 'n':
                ...
                strncopy(
                opaque->magic.name, opt_info.arg, sizeof(opaque->magic.name));
                continue;
            case 's':
            ...
            case 't':
                opaque->magic.magic = MAGICID;
                strncopy(
                opaque->magic.type, opt_info.arg, sizeof(opaque->magic.type));
                continue;
            case 'v':
                opaque->magic.magic = MAGICID;
...
static Dssmeth_t method = { "opaque",
                            "opaque fixed record data with optional magic",
                            CXH,
                            opaquemeth };

Dsslib_t dss_lib_opaque = { "opaque",
                            "opaque method"
                            "[-1ls5Pp0?\n@(#)$Id: dss opaque method (AT&T "
                            "Research) 2002-11-22 $\n]" USAGE_LICENSE,
                            CXH,
                            0,
                            &method };
