        ...
        if (n > 20
            ...
            if (r.value.buffer.size > elementsof(rp->data)) {
                ...
                if (r.value.buffer.size > elementsof(rp->data)) {
                    ...
                    disc,
                    1,
                    "%s: AS path too long -- %d duplicates removed",
                    file->format->name,
                    n - o);
            }
            ...
            for (;;) {
                ...
                case 'C':
                    ...
                    if ((n = r.value.buffer.size)
                        ...
                        if (disc->errorf && !(file->dss->flags & DSS_QUIET))
                            ...
                            disc,
                            1,
                            "%s: cluster list too long -- truncated to %d",
                            file->format->name,
                            n);
                    }
                    ...
                    continue;
                case 'I':
                    if ((i = (*bgp->type_cluster->internalf)(
                         file->dss->cx,
                         bgp->type_cluster,
                         NiL,
                         &bgp->type_cluster->format,
                         &r,
                         p,
                         e - p,
                         file->dss->cx->rm,
                ...
                case 'T':
                    ...
                    if (p == e && disc->errorf
                        && !(file->dss->flags & DSS_QUIET))
                        (*disc->errorf)(NiL,
                                        disc,
                                        1,
                                        "%s: %s: invalid date/time",
                                        file->format->name,
                                        p);
                    p = e;
                    continue;
...
= { "cisco",    "cisco router dump format (2009-03-15)",
    CXH,        ciscoident,
    ciscoopen,  ciscoread,
    ciscowrite, 0,
    ciscoclose, 0,
