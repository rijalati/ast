...
tableident(Dssfile_t *file, void *buf, size_t n, Dssdisc_t *disc)
{
    ...
    for (;;)
    {
        ...
        if (isdigit(*f) && !strtoip4(f, NiL, &addr, &bits)
            || !strtoip6(f, NiL, prefix, prefix + IP6BITS)
               && (file->caller = file))
            break;
    }
    return 1;
    ...
    for (;;)
    {
        ...
        if (state->v6)
        {
            ...
            if ((rp->set & BGP_SET_prefixv6)
                && memcmp(rp->prefixv6, op->prefixv6, sizeof(rp->prefixv6)))
                rp->attr |= BGP_best;
            else if ((rp->set & BGP_SET_hopv6)
                     && !memcmp(rp->hop.v6, op->hop.v6, sizeof(rp->hop.v6)))
                continue;
