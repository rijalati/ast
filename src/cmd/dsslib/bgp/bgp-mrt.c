...
#define BE4(p)                                                               \
    ((BE1(p) << 24) | (BE1(p + 1) << 16) | (BE1(p + 2) << 8) | (BE1(p + 3)))
#define BE(b, p, n) memcpy(b, p, n)

#define DATA(sp, rp)                                                         \
    (rp = &sp->route, sp->osize = sp->size, sp->temp = sizeof(rp->data))
#define SAVE(sp, rp)                                                         \
    (memcpy(&sp->save, rp, BGP_FIXED + (sp->osize = sp->size)))
#define ZERO(sp, rp)                                                         \
    (sp->size = 0, memset(rp, 0, BGP_FIXED), sp->unknown.size = 0)
...
#define PREV(sp, rp)                                                         \
    (memcpy(rp = &sp->route, &sp->save, BGP_FIXED + sp->osize))
...
#    define AET(p)                                                           \
        (swapmem(int_swap, &anonymize.time, p, 4), anonymize.time++)
...
{
    if (anonymize.size) {
        if (sfwrite(anonymize.io, anonymize.head, MRT_HEAD) != MRT_HEAD
            || sfwrite(anonymize.io, anonymize.data, anonymize.size)
               != anonymize.size) {
            if (disc->errorf)
                (*disc->errorf)(NiL,
                                disc,
...
#define PAYLOAD(f, b, n)                                                     \
    do {                                                                     \
        if (((f)->dss->test & 0x0010) && n > 0)                              \
            payload(b, n);                                                   \
    } while (0)
...
symbol(int group, int index)
{
    ...
    case GROUP_ATTR:
        ...
        case MRT_ATTR_AS32_AGGREGATOR:
            return "AS32_AGGREGATOR";
        case MRT_ATTR_SSA_DEPRECATED:
            return "SSA_DEPRECATED";
        case MRT_ATTR_CONNECTOR_DEPRECATED:
            return "CONNECTOR_DEPRECATED";
        case MRT_ATTR_AS_PATHLIMIT_DEPRECATED:
            return "AS_PATHLIMIT_DEPRECATED";
        case MRT_ATTR_PMSI_TUNNEL:
            return "PMSI_TUNNEL";
        case MRT_ATTR_TUNNEL_ENCAPSULATION:
            return "TUNNEL_ENCAPSULATION";
        case MRT_ATTR_TRAFFIC_ENGINEERING:
            return "TRAFFIC_ENGINEERING";
        case MRT_ATTR_EXTENDED_COMMUNITY_V6:
            return "EXTENDED_COMMUNITY_V6";
        case MRT_ATTR_AIGP:
            return "AIGP";
        case MRT_ATTR_PE_DISTINGUISHER_LABELS:
...
mrtopen(Dssfile_t *file, Dssdisc_t *disc)
{
    ...
    if (!(file->data
          = ( void * ) vmnewof(file->dss->vm, 0, Mrtstate_t, 1, 0))) {
        if (disc->errorf)
   ...
   Dssdisc_t *disc)
{
    if ((state->buf + 8) > end) {
        if (disc->errorf && !(file->dss->flags & DSS_QUIET))
            (*disc->errorf)(NiL,
                            disc,
                            1,
                            "rd %s size %u -- %d available",
                            symbol(GROUP_STATE, state->state),
                            8,
                            ( int ) (end - state->buf));
        return -1;
    }
    ...
    if (file->dss->flags & DSS_DEBUG)
        sfprintf(
        sfstderr,
        "                      %-4.4s  rd type %u number %u as %u addr %s\n",
        head,
        rd->type,
        rd->number,
        rd->as32 ? rd->as32 : rd->as,
        fmtip4(rd->addr.v4, -1));
    state->buf += 8;
    return 0;
}
       ...
       Dssdisc_t *disc)
{
    ...
    case MRT_AFI_IPV4:
        ...
        while (i++ < 4)
            v4 <<= 8;
        if (state->best != state->message && (state->best = state->message)
            || rp->addr.v4 != v4 || rp->bits != bits) {
            rp->addr.v4 = v4;
            rp->bits = bits;
            rp->attr |= BGP_best;
        }
    ...
    case MRT_AFI_IPV6:
        ...
        while (i < 16)
            v6[i++] = 0;
        if (state->best != state->message && (state->best = state->message)
            || memcmp(rp->prefixv6, v6, sizeof(rp->prefixv6))) {
        ...
        if (file->dss->flags & DSS_DEBUG)
            sfprintf(sfstderr,
                     ...
                     rp->type,
                     symbol(GROUP_AFI, rp->afi),
                     fmtip6(rp->prefixv6, rp->prefixv6[IP6BITS]));
        break;
    }
    return 0;
nope:
    if (disc->errorf && !(file->dss->flags & DSS_QUIET))
        ...
        disc,
        1,
        "nlri %s %s prefix bytes %d too large -- %d max, %d available",
        symbol(GROUP_STATE, state->state),
        symbol(GROUP_AFI, rp->afi),
        n,
        i,
     ...
     Dssdisc_t *disc)
{
    ...
    case MRT_SAFI_VPN_MLPS_LABEL:
        ...
        m *= 2;
        BGPVEC(
        state, rp, Bgpnum_t, np, m, &rp->labels, "nlri label list", disc);
        for (q = 0; q < m; q += 2) {
            v = AE2(state->buf) << 8;
            state->buf += 2;
            n = BE1(state->buf);
        ...
        if (file->dss->flags & DSS_DEBUG)
            sfprintf(
            sfstderr,
            "                      nlri  %c  %s/%s  label %u labels %d\n",
            rp->type,
            symbol(GROUP_AFI, rp->afi),
            symbol(GROUP_SAFI, rp->safi),
            rp->label,
            rp->labels.size / 2);
    ...
    workaround:
        if (1 > ( int ) (end - state->buf)) {
            if (disc->errorf && !(file->dss->flags & DSS_QUIET))
                ...
                disc,
                1,
                "nlri %s.%s size %d too large -- %d available",
                symbol(GROUP_SAFI, rp->safi),
                symbol(GROUP_MCAST_VPN, m),
                2,
                ( int ) (end - state->buf));
            goto nope;
        }
        ...
        default:
            /*
             * XXX: workaround for data bug discovered 2012-08-03 by the data
             * guys
             */
        ...
        if (file->dss->flags & DSS_DEBUG)
            sfprintf(
            sfstderr,
            "                            %s.%s size %d available %d/%d\n",
            symbol(GROUP_SAFI, rp->safi),
            symbol(GROUP_MCAST_VPN, m),
            l,
            ( int ) (end - state->buf),
            ( int ) (state->end - state->buf));
        ...
        if (q & VPN_multicast) {
            ...
            default:
                if (disc->errorf && !(file->dss->flags & DSS_QUIET))
                    (*disc->errorf)(
                    NiL, disc, 1, "%u: unknown mcast source length", i);
                goto skip;
            }
            i = BE1(state->buf++);
            switch (i) {
            ...
            default:
                if (disc->errorf && !(file->dss->flags & DSS_QUIET))
                    (*disc->errorf)(
                    NiL, disc, 1, "%u: unknown mcast group length", i);
                goto skip;
            }
        }
        if (q & VPN_originator) {
            ...
            } else if (rp->afi == MRT_AFI_IPV6
                       || ( int ) (end - state->buf) == 16) {
     ...
     Dssdisc_t *disc)
{
    ...
    if (file->dss->flags & DSS_DEBUG)
        sfprintf(sfstderr,
                 "                      attr  flag 0x%02x  %s size %u "
                 "available %d/%d\n",
                 flag,
                 symbol(GROUP_ATTR, type),
                 size,
                 ( int ) (end - state->buf),
                 ( int ) (state->end - state->buf));
    if ((nxt = state->buf + size) > end) {
        if (disc->errorf && !(file->dss->flags & DSS_QUIET))
            (*disc->errorf)(NiL,
                            disc,
                            1,
                            "nlri attribute %s size %u -- %d available",
                            symbol(GROUP_ATTR, type),
                            size,
                            ( int ) (end - state->buf));
        goto nope;
    ...
    case MRT_ATTR_AS_PATH:
        if (!state->as32 && (m = size)) {
            ...
            for (i = j = 0; i <= (m - 4); i += 2) {
                /* <type> 1:set 2:sequence 3:confed_sequence 4:confed_set
                 * <length> */
                k = BE1(state->buf + i);
                if (k < 1 || k > 4)
                    break;
             ...
             * there is a small chance of hitting an AS32 encoding that is
             * also a valid AS16 how about an RFC that addresses all the XXX
             * in this parser
             */

            if (i == m) {
                BGPVEC(
                state, rp, Bgpasn_t, ap, j, &rp->path, "AS16 path", disc);
    ...
    case MRT_ATTR_AS32_PATH:
        if (m = size) {
            ...
            BGPVEC(
            state, rp, Bgpnum_t, ap32, j, &rp->path32, "AS32 path", disc);
    ...
    case MRT_ATTR_PMSI_TUNNEL:
        BGPPERM(
        state, rp, Bgptunnel_t, ta, 1, 0, "PMSI tunnel attribute", disc);
        if (!ta)
            goto nope;
    ...
    unknown:
        if ((sizeof(state->unknown.data) - state->unknown.size) > 14) {
            ...
            i += sfsprintf(state->unknown.data + i,
                           sizeof(state->unknown.data) - i,
                           "0x%02x:%u:%u:0x",
                           flag,
                           type,
                           size);
...
pathattr(Dssfile_t *file, Mrtstate_t *state, Bgproute_t *rp, Dssdisc_t *disc)
{
    ...
    if (file->dss->flags & DSS_DEBUG)
        sfprintf(sfstderr,
                 "                      attr  size %4u  available %d\n",
                 size,
                 ( int ) (state->end - end) + size);
    if (end > state->end) {
        /*
        ...
        if (r == 4) {
            ...
            if (n > state->bugsiz) {
                if (!(state->bugbuf
                      = vmnewof(file->dss->vm, state->bugbuf, char, n, 0))) {
                    if (disc->errorf)
                        (*disc->errorf)(
                        NiL, disc, ERROR_SYSTEM | 2, "out of space [bugbuf]");
                    return -1;
                }
            }
            ...
            if (sfread(file->io, end - r, r) != ( ssize_t ) r) {
                if (disc->errorf)
                    ...
                    disc,
                    2,
                    "header size invalid -- 4 byte bug workaround failed");
                return -1;
            }
        } else {
            if (disc->errorf && !(file->dss->flags & DSS_QUIET))
                ...
                disc,
                1,
                "%u: invalid path attribute buffer size -- %d available",
                size,
                ( int ) (state->end - state->buf));
            end = state->end;
        }
...
mrtread(Dssfile_t *file, Dssrecord_t *record, Dssdisc_t *disc)
{
    ...
    for (;;) {
        ...
        case 0:
            ...
            if (file->dss->flags & DSS_DEBUG)
                sfprintf(sfstderr,
                         "message %6lu  record %6lu  time %8lu  %s.%s size "
                         "%I*u  offset %I*u\n",
                         state->message,
                         file->count,
                         state->time,
                         symbol(GROUP_MESSAGE, type),
                         symbol(type, subtype),
                         sizeof(size),
                         MRT_HEAD + size,
                         sizeof(file->offset),
            ...
            case MRT_TABLE_DUMP_V2:
                ...
                case MRT_TABLE_PEER_INDEX_TABLE:
                    ...
                    if (state->peers > state->maxpeers) {
                        ...
                        if (!(state->peer = vmnewof(file->dss->vm,
                                                    state->peer,
                                                    Mrtpeer_t,
                                                    state->maxpeers,
                                                    0))) {
                    ...
                    for (i = 0; i < state->peers; i++) {
                        ...
                        if (file->dss->flags & DSS_DEBUG)
                            ...
                            i,
                            state->peer[i].as,
                            (state->peer[i].type & PEER_IPV6)
                            ? fmtip6(state->peer[i].addr.v6, -1)
                            : fmtip4(state->peer[i].addr.v4, -1));
                    }
                    continue;
                case MRT_TABLE_RIB_IPV4_UNICAST:
                    afi = MRT_AFI_IPV4;
            ...
            case MRT_BGP4MP_ET:
                ...
                if (BE2(state->buf + 2) == MRT_AFI_IPV6) {
                    ...
                    AE(
                    rp->src_addr.v6, state->buf + 4, sizeof(rp->src_addr.v6));
                    AE(rp->dst_addr.v6,
                       state->buf + 20,
                       sizeof(rp->dst_addr.v6));
                    state->buf += 36;
                } else {
...
mrtwrite(Dssfile_t *file, Dssrecord_t *record, Dssdisc_t *disc)
{
    ...
    if (disc->errorf)
        (*disc->errorf)(
        NiL, disc, 2, "%s: record write not implemented", file->format->name);
    return -1;
}
...
= { "mrt" ANONYMIZE_FORMAT_NAME,
    "mrt binary format" ANONYMIZE_FORMAT_DESCRIPTION
    " (2012-08-08) -T0x0010 enables payload trace "
