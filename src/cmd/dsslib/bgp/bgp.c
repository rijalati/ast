...
#define CXD(name, type, index, description)                                  \
    { name, description, { { 0 }, CX_DEPRECATED }, 0, ( Cxtype_t * )type,    \
      0,    index },
...
= { CXV("afi",
        "number",
        BGP_afi,
        "Announce address family identifier: 1:ipv4, 2:ipv6.")
    CXV("agg_addr", "ipaddr_t", BGP_agg_addr, "Aggregator AS address.")
    CXV("agg_addrv4",
        "ipv4addr_t",
        BGP_agg_addrv4,
        "Aggregator AS ipv4 address.") CXV("agg_addrv6",
                                           "ipv6addr_t",
                                           BGP_agg_addrv6,
                                           "Aggregator AS ipv6 address.")
    CXV("agg_addr32", "ipaddr_t", BGP_agg_addr32, "Aggregator AS32 address.")
    CXV("agg_addr32v4",
        "ipv4addr_t",
        BGP_agg_addr32v4,
        "Aggregator AS32 ipv4 address.") CXV("agg_addr32v6",
                                             "ipv6addr_t",
                                             BGP_agg_addr32v6,
                                             "Aggregator AS32 ipv6 address.")
    ...
    CXV("labels",
        "labels_t",
        BGP_labels,
        "NLRI label list; LHS is label, RHS is COS and end of stack bit.")
    ...
    CXV("origin",
        "number",
        BGP_origin,
        "Origin: 'i':igp, 'e':egp, '?':incomplete, '0':unset.")
    ...
    CXV("prefix",
        "ipprefix_t",
        BGP_prefix,
        "Routing ddress prefix and length.")
    CXV("prefixv4",
        "ipv4prefix_t",
        BGP_prefixv4,
        "ipv4 routing prefix and length.")
    CXV("prefixv6",
        "ipv6prefix_t",
        BGP_prefixv6,
        "ipv6 routing prefix and length.")
    CXV("rd", "rd_t", BGP_rd, "Route distinguisher.")
    CXV("rib_failure", "number", BGP_rib_failure, "RIB failure.")
    CXV("safi",
        "number",
        BGP_safi,
        "Announce subsequent address family identifier bits: 1:unicast, "
        "2:multicast, 4:MLPS-label, 5:MCAST-VPN, 128:MLPS-labeled-VPN.")
    ...
    CXV("type",
        "number",
        BGP_type,
        "Record type:: 'A':announce, 'K':keepalive, 'N':notification, "
        "'O':open, 'S':state, 'T':table, 'W':withdraw.")
    CXV("unknown",
        "string",
        BGP_unknown,
        "Unknown attributes in the form: "
        "\"<flag-i>:<type-i>:<size-i>:<hex-data-i>;...\".")
    ...
    CXV("MESSAGE",
        "number",
        BGP_MESSAGE,
        "Message group bit, toggled for each message.")
    CXV("PART",
        "number",
        BGP_PART,
        "Message part bit, 0 for first part, 1 for remainder.")
    CXD("cluster_len",
        "number",
        BGP_cluster_len,
        "DEPRECATED -- use sizeof(cluster). Cluster id list length.")
    CXD("community_len",
        "number",
        BGP_community_len,
        "DEPRECATED -- use sizeof(community). Community of interest id list "
        "length.")
    CXD("extended_len",
        "number",
        BGP_extended_len,
        "DEPRECATED -- use sizeof(extended). Extended community list length.")
    CXD("labels_len",
        "number",
        BGP_labels_len,
        "DEPRECATED -- use sizeof(labels). NLRI label list length.")
    CXD("path_len",
        "number",
        BGP_path_len,
        "DEPRECATED -- use sizeof(path). AS path length.")
    CXD("path16_len",
        "number",
        BGP_path16_len,
        "DEPRECATED -- use sizeof(path16). AS16 path length.")
    CXD("path32_len",
        "number",
        BGP_path32_len,
        "DEPRECATED -- use sizeof(path32). AS32 path length."){ 0 } };
        ...
        Cxdisc_t *disc)
{
    ...
    case BGP_agg_addrv4:
        r->value.number
        = (rp->set & BGP_SET_agg_addrv6) ? 0 : rp->agg_addr.v4;
        break;
    case BGP_agg_addrv6:
        r->value.buffer.data
        = (rp->set & BGP_SET_agg_addrv6) ? rp->agg_addr.v6 : noipv6;
        r->value.buffer.size = sizeof(rp->agg_addr.v6);
        r->value.buffer.elements = 0;
        break;
    case BGP_agg_addr32:
    ...
    case BGP_agg_addr32v4:
        r->value.number
        = (rp->set & BGP_SET_agg_addr32v6) ? 0 : rp->agg_addr32.v4;
        break;
    case BGP_agg_addr32v6:
        r->value.buffer.data
        = (rp->set & BGP_SET_agg_addr32v6) ? rp->agg_addr32.v6 : noipv6;
        r->value.buffer.size = sizeof(rp->agg_addr32.v6);
        r->value.buffer.elements = 0;
        break;
    case BGP_agg_as:
    ...
    case BGP_bits:
        r->value.number
        = (rp->set & BGP_SET_prefixv6) ? rp->prefixv6[IP6BITS] : rp->bits;
        break;
    case BGP_bitsv4:
    ...
    case BGP_dpa_addrv4:
        r->value.number
        = (rp->set & BGP_SET_dpa_addrv6) ? 0 : rp->dpa_addr.v4;
        break;
    case BGP_dpa_addrv6:
        r->value.buffer.data
        = (rp->set & BGP_SET_dpa_addrv6) ? rp->dpa_addr.v6 : noipv6;
        r->value.buffer.size = sizeof(rp->dpa_addr.v6);
        r->value.buffer.elements = 0;
        break;
    case BGP_dpa_as:
    ...
    case BGP_dst_addrv4:
        r->value.number
        = (rp->set & BGP_SET_dst_addrv6) ? 0 : rp->dst_addr.v4;
        break;
    case BGP_dst_addrv6:
        r->value.buffer.data
        = (rp->set & BGP_SET_dst_addrv6) ? rp->dst_addr.v6 : noipv6;
        r->value.buffer.size = sizeof(rp->dst_addr.v6);
        r->value.buffer.elements = 0;
        break;
    case BGP_dst_as:
    ...
    case BGP_hopv6:
        r->value.buffer.data
        = (rp->set & BGP_SET_hopv6) ? rp->hop.v6 : noipv6;
        r->value.buffer.size = sizeof(rp->hop.v6);
        r->value.buffer.elements = 0;
        break;
    case BGP_id:
    ...
    case BGP_new_state:
        r->value.number
        = rp->type == BGP_TYPE_state_change ? rp->new_state : 0;
        break;
    case BGP_old_state:
        r->value.number
        = rp->type == BGP_TYPE_state_change ? rp->old_state : 0;
        break;
    case BGP_origin:
    ...
    case BGP_prefix:
        ...
        break;
    case BGP_prefixv4:
        r->value.number = (rp->set & BGP_SET_prefixv6)
                          ? 0
                          : (( Cxnumber_t )rp->addr.v4 * 64 + rp->bits);
        break;
    case BGP_prefixv6:
        r->value.buffer.data
        = (rp->set & BGP_SET_prefixv6) ? rp->prefixv6 : noipv6;
        r->value.buffer.size = sizeof(rp->prefixv6);
        r->value.buffer.elements = 0;
        break;
    case BGP_rd:
    ...
    case BGP_src_addrv4:
        r->value.number
        = (rp->set & BGP_SET_src_addrv6) ? 0 : rp->src_addr.v4;
        break;
    case BGP_src_addrv6:
        r->value.buffer.data
        = (rp->set & BGP_SET_src_addrv6) ? rp->src_addr.v6 : noipv6;
        r->value.buffer.size = sizeof(rp->src_addr.v6);
        r->value.buffer.elements = 0;
        break;
    case BGP_src_as:
    ...
    case BGP_labels_len:
        ...
        break;
    case BGP_path_len:
        r->value.number = rp->path.size
                          ? rp->path.elements
                          : rp->path32.size ? rp->path32.elements : 0;
        break;
    case BGP_path16_len:
...
static Cxmember_t bgp_member = { bgp_get, 0, ( Dt_t * )&bgp_fields[0] };

static Cxvariable_t aigp_fields[]
= { CXV("aigp", "values_t", BGP_AIGP_aigp, "AIGP TLV."){ 0 } };
    ...
    default:
        if (disc->errorf)
            (*disc->errorf)(NiL,
                            disc,
                            1,
                            "aigp.%s index %d not implemented in get()",
                            pc->data.variable->name,
...
static Cxmember_t aigp_member = { aigp_get, 0, ( Dt_t * )&aigp_fields[0] };

static Cxvariable_t mvpn_fields[]
= { CXV("group_addr", "ipaddr_t", BGP_MVPN_group_addr, "Group address.")
    CXV("group_addrv4",
        "ipv4addr_t",
        BGP_MVPN_group_addrv4,
        "Group ipv4 address.") CXV("group_addrv6",
                                   "ipv6addr_t",
                                   BGP_MVPN_group_addrv6,
                                   "Group ipv6 address.")
    CXV("key", "bgp_t", BGP_MVPN_key, "Route key NLRI.")
    CXV("originator", "ipaddr_t", BGP_MVPN_originator, "Group address.")
    CXV("originatorv4",
        "ipv4addr_t",
        BGP_MVPN_originatorv4,
        "Group ipv4 address.") CXV("originatorv6",
                                   "ipv6addr_t",
                                   BGP_MVPN_originatorv6,
                                   "Group ipv6 address.")
    ...
    CXV("src_addrv4",
        "ipv4addr_t",
        BGP_MVPN_src_addrv4,
        "Source ipv4 address.") CXV("src_addrv6",
                                    "ipv6addr_t",
                                    BGP_MVPN_src_addrv6,
                                    "Source ipv6 address.")
    ...
    CXV("type", "number", BGP_MVPN_type, "Route type."){ 0 } };
         ...
         Cxdisc_t *disc)
{
    ...
    case BGP_MVPN_group_addrv4:
        r->value.number
        = (mp->set & BGP_MVPN_SET_group_addrv6) ? 0 : mp->group_addr.v4;
        break;
    case BGP_MVPN_group_addrv6:
        r->value.buffer.data
        = (mp->set & BGP_MVPN_SET_group_addrv6) ? mp->group_addr.v6 : noipv6;
        r->value.buffer.size = sizeof(mp->group_addr.v6);
        r->value.buffer.elements = 0;
        break;
    case BGP_MVPN_key:
    ...
    case BGP_MVPN_originatorv4:
        r->value.number
        = (mp->set & BGP_MVPN_SET_originatorv6) ? 0 : mp->originator.v4;
        break;
    case BGP_MVPN_originatorv6:
        r->value.buffer.data
        = (mp->set & BGP_MVPN_SET_originatorv6) ? mp->originator.v6 : noipv6;
        r->value.buffer.size = sizeof(mp->originator.v6);
        r->value.buffer.elements = 0;
        break;
    case BGP_MVPN_rd:
    ...
    case BGP_MVPN_src_addrv4:
        r->value.number
        = (mp->set & BGP_MVPN_SET_src_addrv6) ? 0 : mp->src_addr.v4;
        break;
    case BGP_MVPN_src_addrv6:
        r->value.buffer.data
        = (mp->set & BGP_MVPN_SET_src_addrv6) ? mp->src_addr.v6 : noipv6;
        r->value.buffer.size = sizeof(mp->src_addr.v6);
        r->value.buffer.elements = 0;
        break;
    ...
    default:
        if (disc->errorf)
            (*disc->errorf)(NiL,
                            disc,
                            ERROR_PANIC | 3,
                            "mvpn.%s index %d not implemented in mvpn_get()",
                            pc->data.variable->name,
                            pc->data.variable->index);
        return -1;
    }
...
static Cxmember_t mvpn_member = { mvpn_get, 0, ( Dt_t * )&mvpn_fields[0] };
...
= { CXV("addr", "ipaddr_t", BGP_RD_addr, "Address.")
    ...
    CXV("type", "number", BGP_RD_type, "Route type."){ 0 } };
       ...
       Cxdisc_t *disc)
{
    ...
    case BGP_RD_addrv6:
        r->value.buffer.data
        = (rd->set & BGP_RD_SET_addrv6) ? rd->addr.v6 : noipv6;
        r->value.buffer.size = sizeof(rd->addr.v6);
        r->value.buffer.elements = 0;
        break;
    case BGP_RD_as:
    ...
    default:
        if (disc->errorf)
            (*disc->errorf)(NiL,
                            disc,
                            ERROR_PANIC | 3,
                            "rd.%s index %d not implemented in get()",
                            pc->data.variable->name,
                            pc->data.variable->index);
        return -1;
    }
    ...
    default:
        if (disc->errorf)
            (*disc->errorf)(NiL,
                            disc,
                            1,
                            "ta.%s index %d not implemented in get()",
                            pc->data.variable->name,
                            pc->data.variable->index);
        return -1;
    }
...
= { tunnel_get, 0, ( Dt_t * )&tunnel_fields[0] };
...
= { "bgp",   "BGP router dump and announce/withdraw messages",
    CXH,     bgpmeth,
    bgpopen, bgpclose,
    0 };
