...
= { CXV("time", "ns_t", LSA_VARI_TIME, "Time stamp.")
    ...
    CXV("lsu_lsa_no",
        "number",
        LSA_VARI_LSU_LSA_NO,
        "LSA number within the packe t.")
    CXV("lsu_no_lsas",
        "number",
        LSA_VARI_LSU_NO_LSAS,
        "Number of LSAs in the packe t.")
    ...
    CXV("lsa_data",
        "buffer",
        LSA_VARI_DATA,
        "LSA payload data -- lsa_type determines content semantics.")
    CXV("lsa_len",
        "number",
        LSA_VARI_LEN,
        "LSA header + payload data length -- use sizeof(lsa_data) for "
        "payload size."){ 0 } };
...
static Dssmeth_t method = { "lsa",         "OSPF LSA logs", CXH,
                            ospf_lsa_meth, ospf_lsa_open,   ospf_lsa_close };
