...
#define OSPF_DD_FLAGS_MASTER                                                 \
    (OSPF_DD_FLAGS_MS | OSPF_DD_FLAGS_M | OSPF_DD_FLAGS_I)
...
#define LSA_MASK(lsa)                                                        \
    ((lsa->lsa_type == LST_NTW) ? (lsa)->lsa_data.lu_nt.lsntw_mask : HOSTMASK)
...
#define LS_BOUND(type)                                                       \
    ((((type) > LST_NULL) && ((type) <= LST_ASEX)) || ((type) == LST_NSSA)   \
...
#define LSA_HDR_LEN (sizeof(ospf_lsa_hdr_t))
    ...
    typedef struct _ospf_pkt_t
    {
        ...
        uint32_t
        pkt_data[OSPF_PKT_MAXLEN / sizeof(uint32_t) - sizeof(ospf_pkt_hdr_t)];
    } ospf_pkt_t;
