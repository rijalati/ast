 ...
 */
static int
identf_lsar_lsa_file(Dssfile_t *file,
                     void *buff,
                     size_t size,
                     Dssdisc_t *disc);
 ...
 */
static int
open_lsar_lsa_file(Dssfile_t *file, Dssdisc_t *disc);
 ...
 */
static int
close_lsar_lsa_file(Dssfile_t *file, Dssdisc_t *disc);
...
static int
identf_lsar_lsa_file(Dssfile_t *fp, void *data, size_t size, Dssdisc_t *disc)
{
    mrt_hdr_t *mrt_hdr;
    ...
    if (size < sizeof(mrt_hdr_t)) {
        if (disc->errorf && (fp->dss->flags & DSS_DEBUG)) {
            ...
            disc,
            2,
            "%s: not enough bytes for an MRT header" FORMAT_NAME);
        }
        return 0;
    }
...
open_lsar_lsa_file(Dssfile_t *fp, Dssdisc_t *disc)
{
    ...
    if (fp->flags & DSS_FILE_WRITE) {
         ...
         */
        MD_CALLOC(
        state_p->lsu_pkt_p, uint8_t *, INIT_LSU_PKT_MAX_LEN, sizeof(uint8_t));
...
read_lsar_lsa_rec(Dssfile_t *fp, Dssrecord_t *rp, Dssdisc_t *disc)
{
    ...
    memcpy(&(lsa_can_p->lsu_auth),
           &(lsu_pkt_p->pkt_lsu_hdr.pkt_hdr_auth),
