 ...
 */
static int
identf_text_lsa_file(Dssfile_t *fp,
                     void *buff_p,
                     size_t size,
                     Dssdisc_t *disc);
...
static int
identf_text_lsa_file(Dssfile_t *fp, void *buff_p, size_t size, Dssdisc_t *disc)
{
    char *s;
    size_t n;
    ...
    if (!strneq(s, magic_version, sizeof(magic_name) - 1)) {
        if (disc->errorf && (fp->dss->flags & DSS_DEBUG)) {
            ...
            disc,
            2,
            "%s: magic version \"%-.*s\" is not the expected \"%s\"",
            FORMAT_NAME,
            strlen(magic_version),
            s,
            magic_version);
        }
        return 0;
