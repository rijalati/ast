...
#define FOUR_OCTECT_PADDED_LEN(len)                                          \
    (BIT_TEST((len), 0x00000003) == 0) ? ((((uint32_t)(len)) >> 2) << 2)     \
                                       : ((((uint32_t)(len)) >> 2) << 2) + 4
...
len, void_ptr, byte_index, var, bytes_left)                                  \
     ...
     */
    float
    four_bytes_to_float(uint32_t l);
    ...
    extern char *
    rtid_to_dotted_decimal(rtid_t rtrid);

    /* Returns pointer to dotted decimal string for area id (host order) */
    extern char *
    areaid_to_dotted_decimal(areaid_t area_id);
     ...
     */
    extern int
    ipaddr_to_mask_len(ipaddr_t ipaddr);
     ...
     */
    extern char *
    get_prefix_dotted_decimal(ipaddr_t addr, ipaddr_t mask);
     ...
     */
    extern int
    does_prefix_range_overlap(ipaddr_t addr1,
                              ipaddr_t mask1,
                              ipaddr_t addr2,
                              ipaddr_t mask2);
     ...
     */
    extern ipaddr_t
    get_cover_prefix(ipaddr_t addr1,
                     ipaddr_t mask1,
                     ipaddr_t addr2,
                     ipaddr_t mask2,
                     ipaddr_t *mask);
    ...
    /*
     * Returns 1 if 'addr' (host order) is an IP multicast address; 0
     * otherwise.
     */
    extern int
    is_this_mcast_addr(ipaddr_t addr);

    /* Breaks 'val' into second and microsec part. */
    extern void
    get_time_as_sec_microsec(double val, time_t *sec, time_t *micro_sec);

    /* Returns time string for 'time_val' in local time-zone. */
    extern char *
    get_time_str_local(double time_val);

    /* Returns time string for 'time_val' in GMT time-zone. */
    extern char *
    get_time_str_gmt(double time_val);
     ...
     */
    extern char *
    get_time_str_wtz_local(double time_val, char **tz);
     ...
     */
    extern char *
    get_time_str_wtz_gmt(double time_val, char **tz);
     ...
     */
    extern char *
    get_dotted_time_str_local(double time_val);
     ...
     */
    extern char *
    get_dotted_time_str_gmt(double time_val);
     ...
     */
    extern char *
    get_dotted_time_str(const char *time_str);
     ...
     */
    extern char *
    get_curr_time_str_wtz_local(char **tz);
     ...
     */
    extern char *
    get_curr_time_str_wtz_gmt(char **tz);
     ...
     */
    extern char *
    get_dotted_curr_time_str_local();
     ...
     */
    extern int
    mon_name_to_no(const char *mon_name, int mon_name_len, char *mon_no_str);

    /* Returns no of times char 'c' occurs in string 'str'. */
    extern int
    str_no_of_chars(char *str, char c);
     ...
     */
    extern int
    get_sec_bf_hr_bdry(int interval, int local_tz);
     ...
     */
    extern int
    get_sec_bf_day_bdry(int interval, int local_tz);
