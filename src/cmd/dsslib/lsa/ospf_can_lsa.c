 ...
 */
static int
identf_can_lsa_file(Dssfile_t *fp, void *buff, size_t size, Dssdisc_t *disc);
 ...
 */
static int
read_aligned_words(Dssfile_t *fp,
                   void *dest_p,
                   int swap,
...
identf_can_lsa_file(Dssfile_t *fp, void *data, size_t size, Dssdisc_t *disc)
{
    ...
    if (disc->errorf && (fp->dss->flags & DSS_DEBUG))
        ...
        disc,
        2,
        "%s: format \"%s\" type \"%s %s\" version %lu swap %03o",
        fp->path,
        FORMAT_NAME,
        MAGIC_NAME,
        MAGIC_TYPE,
...
open_can_lsa_file(Dssfile_t *fp, Dssdisc_t *disc)
{
    ...
    if (BIT_TEST(fp->flags, DSS_FILE_WRITE)) {
        ...
        if (!BIT_TEST(fp->flags, DSS_FILE_APPEND)) {
            ...
            strncpy(magic_hdr.name,
                    MAGIC_NAME,
                    (sizeof(magic_hdr.name) / sizeof(char)) - 1);
            strncpy(magic_hdr.type,
                    MAGIC_TYPE,
        ...
        can_state_p->version
        = swapget(swap ^ int_swap, &mp->version, sizeof(mp->version));
