...
= "[-?\n@(#)$Id: release (AT&T Research) 2000-01-28 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?\brelease\b lists the changes within the date range "
  "specified"
  ...
  "[+?The entries for each \afile\a are annotated with the file directory "
  "name.]"
  "[f:from?Entries older than \adate\a are omitted.]:[date]"
  "[r:release?List all changes that include the first \acount\a release "
  "marks."
...
usage()
{
    fprintf(
    stderr,
    "Usage: release [-V] [-h hi-date] [-l lo-date] [-r count] [ file ...]\n");
    exit(2);
}
