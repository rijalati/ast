...
= "[-?\n@(#)$Id: mamake (AT&T Research) 2013-04-29 $\n]" USAGE_LICENSE
  ...
  "			makefiles.]"
  "	}"
  "[+?\bmamprobe\b(1) is called to probe and generate system specific "
  "variable"
  ...
  "[n:?Print actions but do not execute. Recursion actions (see \b-r\b) are "
  "still"
...
#define add(b, c)                                                            \
    (((b)->nxt >= (b)->end) ? append(b, "") : NiL, *(b)->nxt++ = (c))
...
#define CHUNK 1024
#define KEY(a, b, c, d)                                                      \
    (((( unsigned long ) (a)) << 15) | ((( unsigned long ) (b)) << 10)       \
     | ((( unsigned long ) (c)) << 5) | ((( unsigned long ) (d))))
...
{
    fprintf(stderr,
            "Usage: %s [-iknFKNV] [-f mamfile] [-r pattern] [-C directory] "
            "[-D level] [target ...] [name=value ...]\n",
            state.id);
    exit(2);
}
...
buffer(void)
{
    ...
    else if (!(buf = newof(0, Buf_t, 1, 0))
             || !(buf->buf = newof(0, char, CHUNK, 0)))
...
appendn(Buf_t *buf, char *str, int n)
{
    ...
    if ((n + 1) >= (buf->end - buf->nxt)) {
        ...
        if (!(buf->buf = newof(buf->buf, char, m, 0)))
            report(
            3, "out of space [buffer resize]", NiL, ( unsigned long ) 0);
        buf->end = buf->buf + m;
        buf->nxt = buf->buf + i;
    }
        ...
        for (;;) {
            ...
            if (!state.view) {
                ...
                if (ts.st_dev == st.st_dev && ts.st_ino == st.st_ino)
                    ...
                    if (p <= state.pwd)
                        report(3,
                               "cannot determine viewpath offset",
                               s,
                               ( unsigned long ) 0);
                }
            }
...
input(void)
{
    ...
    if (state.peek)
        ...
        return 0;
             ...
             && *(e = state.input + strlen(state.input) - 1) == '\n')
        *e = 0;
    state.sp->line++;
    return state.input;
}
...
execute(char *s)
{
    ...
    if (!state.shell
        && (!(state.shell = ( char * ) search(state.vars, "SHELL", NiL))
            || !strcmp(state.shell, sh)))
        state.shell = sh;
    buf = buffer();
...
probe(void)
{
    ...
    if (!(s = ( char * ) search(state.vars, "INSTALLROOT", NiL)))
        report(
        3, "variable must be defined", "INSTALLROOT", ( unsigned long ) 0);
    append(buf, s);
...
make(Rule_t *r)
{
    ...
    while (s = input()) {
        ...
        case KEY('d', 'o', 'n', 'e'):
            ...
            if (cmd && state.active
                ...
                if (state.explain && !state.force) {
                    if (!r->time)
                        ...
                        fprintf(stderr,
                                "%s [%lu] older than prerequisites [%lu]\n",
                                r->name,
                                r->time,
                                z);
                }
            ...
            r->flags |= RULE_made;
            if (!(r->flags
                  & (RULE_dontcare | RULE_error | RULE_exists | RULE_generated
                     | RULE_implicit | RULE_virtual)))
...
scan(Dict_item_t *item, void *handle)
{
    ...
    static char *files[]
    = { "Nmakefile", "nmakefile", "Makefile", "makefile" };
...
descend(Dict_item_t *item, void *handle)
{
    ...
    if (!state.active
        && (!(r->flags & RULE_active)
            || !(r = ( Rule_t * ) search(state.leaf, r->name, NiL))))
        return 0;
...
main(int argc, char **argv)
{
    ...
    while ((s = *++argv) && *s == '-') {
        ...
        for (;;) {
            ...
            case 'D':
                ...
                if (!*++s && !(s = *++argv)) {
                    report(
                    2, "option value expected", t, ( unsigned long ) 0);
                    usage();
                } else
                    switch (*t) {
                    case 'f':
    ...
    if (state.directory && chdir(state.directory))
        report(
        3, "cannot change working directory", NiL, ( unsigned long ) 0);
    view();
