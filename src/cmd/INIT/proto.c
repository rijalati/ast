...
int replace __PARAM__((const char *newfile, const char *oldfile, int preserve),
                      (newfile, oldfile, preserve))
__OTORP__(const char *newfile; const char *oldfile; int preserve;)
{
    struct stat st;
    time_t ut[2];
...
static struct fsminit fsminit[] = {
    ...
    (0 + 26),
    { 023 },
    (((((0401 + 1)) - 0401) << (7 + 1)) | (((0 + 28) + 14))),
    },
    {
    (0 + 26),
    { '.' },
    (((((0401 + 29)) - 0401) << (7 + 1)) | (((0 + 28) + 13))),
    },
...
void ppfsm __PARAM__((int op, char *s), (op, s)) __OTORP__(int op; char *s;)
{
    int c;
    int n;
    int i;
...
static char *number __PARAM__((char *p, long n), (p, n))
__OTORP__(char *p; long n;)
{
    long d;
...
static int sstrlen __PARAM__((const char *s), (s)) __OTORP__(const char *s;)
{
    const char *b;
...
static int sstrncmp __PARAM__((const char *s, const char *t, int n),
                              (s, t, n))
__OTORP__(const char *s; const char *t; int n;)
{
    const char *e = s + n;
...
static char *strcopy __PARAM__((char *s, const char *t), (s, t))
__OTORP__(char *s; const char *t;)
{
    while (*s++ = *t++)
        ;
    return s - 1;
...
static void proto_error __PARAM__((char *iob, int level, char *msg, char *arg),
                                  (iob, level, msg, arg))
__OTORP__(char *iob; int level; char *msg; char *arg;)
{
    char *p;
    char buf[1024];
...
static char *memcopy __PARAM__((char *s, char *t, int n), (s, t, n))
__OTORP__(char *s; char *t; int n;)
{
    char *e = t + n;
...
static int lookup __PARAM__((const Item_t *item, const char *name, int size),
                            (item, name, size))
__OTORP__(const Item_t *item; const char *name; int size;)
{
    int c;
    int i;
    ...
    for (i = 0; item[i].data; i++)
        if (c == item[i].data[0] && size == item[i].size
            && !sstrncmp(name, item[i].data, size))
            return i;
    return -1;
}


static void copy __PARAM__((Buffer_t * b, char *s, int n), (b, s, n))
__OTORP__(Buffer_t *b; char *s; int n;)
{
    if (n < 0)
        n = sstrlen(s);
    while (n--)
...
#line 225
static void
comment __PARAM__((Notice_t * notice, Buffer_t *b, char *s, int n, int u),
                  (notice, b, s, n, u))
__OTORP__(Notice_t *notice; Buffer_t * b; char *s; int n; int u;)
{
    int i;
    int m;
    int x;
...
static void
expand __PARAM__((Notice_t * notice, Buffer_t *b, const Item_t *item),
                 (notice, b, item))
__OTORP__(Notice_t *notice; Buffer_t * b; const Item_t *item;)
{
    char *t;
    char *e;
    int q;
...
static void copyright __PARAM__((Notice_t * notice, Buffer_t *b), (notice, b))
__OTORP__(Notice_t *notice; Buffer_t * b;)
{
    char *x;
    char *t;
    time_t clock;
...
static int push __PARAM__(
(Stack_t * sp, char *file, char *parent, char *info, int size, Buffer_t *buf),
(sp, file, parent, info, size, buf))
__OTORP__(Stack_t *sp; char *file; char *parent; char *info; int size;
          Buffer_t * buf;)
{
    char *s;
    char *t;
...
(char *p, int size, char *file, char *options, int cc1, int cc2, int cc3),
(p, size, file, options, cc1, cc2, cc3))
__OTORP__(char *p; int size; char *file; char *options; int cc1; int cc2;
          int cc3;)
{
    char *s;
    char *v;
    ...
    if (notice.type != 4) {
        ...
        if (notice.type == 6 || notice.type == 7) {
            ...
            if (notice.item[21].data) {
                ...
                &buf,
                "http://www.eclipse.org/org/documents/epl-v10.html",
                sizeof("http://www.eclipse.org/org/documents/epl-v10.html")
                - 1,
                0);
            else
                comment(&notice,
            ...
            if (notice.item[21].data) {
                ...
                sizeof("A copy of the Source Code Agreement is available")
                - 1,
                0);
                copy(&tmp, "at the ", -1);
                if (i >= 0)
            ...
            comment(&notice,
                    &buf,
                    "If you have copied or used this software without "
                    "agreeing",
                    sizeof("If you have copied or used this software without "
                           "agreeing")
                    - 1,
                    0);
            comment(
            ...
            sizeof("to the terms of the license you are infringing on") - 1,
            0);
            comment(&notice,
                    &buf,
                    "the license and copyright and are violating",
                    sizeof("the license and copyright and are violating") - 1,
                    0);
            if (i >= 0)
                expand(&notice, &tmp, &notice.item[i]);
            copy(&tmp, "'s", -1);
            ...
            sizeof("This is free software; you can redistribute it and/or")
            - 1,
            0);
            comment(&notice,
                    &buf,
                    ...
                    "License",
                    sizeof("modify it under the terms of the GNU General "
                           "Public License")
                    - 1,
                    0);
            comment(&notice,
                    ...
                    sizeof("as published by the Free Software Foundation;")
                    - 1,
                    0);
            comment(
            &notice,
            ...
            comment(&notice,
                    ...
                    sizeof("This software is distributed in the hope that it")
                    - 1,
                    0);
            comment(&notice,
                    &buf,
                    "will be useful, but WITHOUT ANY WARRANTY;",
                    sizeof("will be useful, but WITHOUT ANY WARRANTY;") - 1,
                    0);
            comment(
            &notice,
            &buf,
            ...
            comment(&notice,
                    ...
                    sizeof("or FITNESS FOR A PARTICULAR PURPOSE.") - 1,
                    0);
            comment(
            &notice,
            &buf,
            ...
            comment(&notice,
                    ...
                    sizeof("You should have received a copy of the") - 1,
                    0);
            comment(&notice,
                    &buf,
                    "GNU General Public License",
                    sizeof("GNU General Public License") - 1,
                    0);
            comment(&notice,
                    &buf,
                    "along with this software (see the file COPYING.)",
                    sizeof("along with this software (see the file COPYING.)")
                    - 1,
                    0);
            comment(&notice,
                    &buf,
                    "If not, a copy is available at",
                    sizeof("If not, a copy is available at") - 1,
                    0);
            comment(&notice,
                    &buf,
            ...
            comment(&notice,
                    ...
                    "or",
                    sizeof("Redistribution and use in source and binary "
                           "forms, with or")
                    - 1,
                    -1);
            comment(&notice,
                    ...
                    "following",
                    sizeof("without modification, are permitted provided "
                           "that the following")
                    - 1,
                    -1);
            comment(&notice,
            ...
            comment(&notice,
                    ...
                    "above",
                    sizeof("   1. Redistributions of source code must retain "
                           "the above")
                    - 1,
                    -1);
            comment(
            ...
            sizeof("      copyright notice, this list of conditions and the")
            - 1,
            -1);
            comment(&notice,
                    &buf,
            ...
            comment(&notice,
                    ...
                    "above",
                    sizeof("   2. Redistributions in binary form must "
                           "reproduce the above")
                    - 1,
                    -1);
            comment(
            ...
            comment(&notice,
                    ...
                    "other",
                    sizeof("      following disclaimer in the documentation "
                           "and/or other")
                    - 1,
                    -1);
            comment(&notice,
            ...
            comment(&notice,
                    ...
                    "or",
                    sizeof("      names of its contributors may be used to "
                           "endorse or")
                    - 1,
                    -1);
            comment(&notice,
                    ...
                    "without",
                    sizeof("      promote products derived from this "
                           "software without")
                    - 1,
                    -1);
            comment(&notice,
            ...
            sizeof("THIS SOFTWARE IS PROVIDED BY THE COPYRIGHT HOLDERS AND")
            - 1,
            -1);
            comment(&notice,
                    &buf,
                    ...
                    "WARRANTIES,",
                    sizeof("CONTRIBUTORS \"AS IS\" AND ANY EXPRESS OR "
                           "IMPLIED WARRANTIES,")
                    - 1,
                    -1);
            comment(
            ...
            sizeof("INCLUDING, BUT NOT LIMITED TO, THE IMPLIED WARRANTIES OF")
            - 1,
            -1);
            comment(
            &notice,
            ...
            sizeof("MERCHANTABILITY AND FITNESS FOR A PARTICULAR PURPOSE ARE")
            - 1,
            -1);
            comment(&notice,
                    &buf,
                    ...
                    "CONTRIBUTORS",
                    sizeof("DISCLAIMED. IN NO EVENT SHALL THE COPYRIGHT "
            ...
            sizeof("BE LIABLE FOR ANY DIRECT, INDIRECT, INCIDENTAL, SPECIAL,")
            - 1,
            -1);
            comment(&notice,
                    &buf,
                    ...
                    "LIMITED",
                    sizeof("EXEMPLARY, OR CONSEQUENTIAL DAMAGES (INCLUDING, "
                           "BUT NOT LIMITED")
                    - 1,
                    -1);
            comment(&notice,
                    ...
                    "OF USE,",
                    sizeof("TO, PROCUREMENT OF SUBSTITUTE GOODS OR SERVICES; "
                           "LOSS OF USE,")
                    - 1,
                    -1);
            comment(&notice,
                    ...
                    "CAUSED AND ON",
                    sizeof("DATA, OR PROFITS; OR BUSINESS INTERRUPTION) "
            ...
            comment(&notice,
                    ...
                    "LIABILITY,",
                    sizeof("ANY THEORY OF LIABILITY, WHETHER IN CONTRACT, "
                           "STRICT LIABILITY,")
                    - 1,
                    -1);
            comment(&notice,
                    ...
                    "ANY WAY",
                    sizeof("OR TORT (INCLUDING NEGLIGENCE OR OTHERWISE) "
                           "ARISING IN ANY WAY")
                    - 1,
                    -1);
            comment(
            ...
            sizeof("OUT OF THE USE OF THIS SOFTWARE, EVEN IF ADVISED OF THE")
            - 1,
            -1);
            comment(&notice,
                    &buf,
            ...
            comment(&notice,
                    ...
                    "or implied",
                    sizeof("This software is provided 'as-is', without any "
                           "express or implied")
                    - 1,
                    -1);
            comment(&notice,
                    ...
                    "for any",
                    sizeof("warranty. In no event will the authors be held "
                           "liable for any")
                    - 1,
                    -1);
            comment(&notice,
            ...
            comment(&notice,
                    ...
                    "for any",
                    sizeof("Permission is granted to anyone to use this "
                           "software for any")
                    - 1,
                    -1);
            comment(&notice,
                    ...
                    "alter it and",
                    sizeof("purpose, including commercial applications, and "
                           "to alter it and")
                    - 1,
                    -1);
            comment(&notice,
                    ...
                    "restrictions:",
                    sizeof("redistribute it freely, subject to the following "
                           "restrictions:")
                    - 1,
                    -1);
            comment(&notice, &buf, (( char * )0), 0, 0);
            comment(&notice,
                    ...
                    "misrepresented;",
                    sizeof(" 1. The origin of this software must not be "
                           "misrepresented;")
                    - 1,
                    -1);
            comment(&notice,
                    ...
                    "software. If",
                    sizeof("    you must not claim that you wrote the "
                           "original software. If")
                    - 1,
                    -1);
            comment(&notice,
                    &buf,
                    ...
                    sizeof("    you use this software in a product, an "
                           "acknowledgment in the")
                    - 1,
                    -1);
            comment(&notice,
                    ...
                    "not",
                    sizeof("    product documentation would be appreciated "
                           "but is not")
                    - 1,
                    -1);
            comment(
            ...
            comment(&notice,
                    ...
                    "such,",
                    sizeof(" 2. Altered source versions must be plainly "
                           "marked as such,")
                    - 1,
                    -1);
            comment(
            ...
            sizeof("    and must not be misrepresented as being the original")
            - 1,
            -1);
            comment(
            ...
            comment(&notice,
                    ...
                    "source",
                    sizeof(" 3. This notice may not be removed or altered "
                           "from any source")
                    - 1,
                    -1);
            comment(&notice,
            ...
            comment(&notice,
                    ...
                    "person",
                    sizeof("Permission is hereby granted, free of charge, to "
                           "any person")
                    - 1,
                    0);
            comment(&notice,
                    ...
                    sizeof("obtaining a copy of this software and associated")
                    - 1,
                    0);
            comment(
            &notice,
            ...
            sizeof("documentation files (the \"Software\"), to deal in the")
            - 1,
            0);
            comment(&notice,
                    &buf,
                    ...
                    "limitation",
                    sizeof("Software without restriction, including without "
                           "limitation")
                    - 1,
                    0);
            comment(&notice,
                    ...
                    "distribute,",
                    sizeof("the rights to use, copy, modify, merge, publish, "
                           "distribute,")
                    - 1,
                    0);
            comment(
            ...
            sizeof("sublicense, and/or sell copies of the Software, and to")
            - 1,
            0);
            comment(&notice,
                    &buf,
                    ...
                    "so,",
                    sizeof("permit persons to whom the Software is furnished "
                           "to do so,")
                    - 1,
                    0);
            comment(&notice,
            ...
            comment(&notice,
                    ...
                    "shall",
                    sizeof("The above copyright notice and this permission "
                           "notice shall")
                    - 1,
                    0);
            comment(
            ...
            sizeof("be included in all copies or substantial portions of the")
            - 1,
            0);
            ...
            comment(&notice,
                    ...
                    "ANY",
                    sizeof("THE SOFTWARE IS PROVIDED \"AS IS\", WITHOUT "
                           "WARRANTY OF ANY")
                    - 1,
                    0);
            comment(&notice,
                    ...
                    "THE",
                    sizeof("KIND, EXPRESS OR IMPLIED, INCLUDING BUT NOT "
                           "LIMITED TO THE")
                    - 1,
                    0);
            comment(
            ...
            sizeof("WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR")
            - 1,
            0);
            comment(&notice,
                    &buf,
                    ...
                    "AUTHORS",
                    sizeof("PURPOSE AND NONINFRINGEMENT. IN NO EVENT SHALL "
                           "THE AUTHORS")
                    - 1,
                    0);
            comment(
            ...
            sizeof("OR COPYRIGHT HOLDERS BE LIABLE FOR ANY CLAIM, DAMAGES OR")
            - 1,
            0);
            comment(&notice,
                    &buf,
                    ...
                    "OR",
                    sizeof("OTHER LIABILITY, WHETHER IN AN ACTION OF "
                           "CONTRACT, TORT OR")
                    - 1,
                    0);
            comment(&notice,
                    ...
                    "THE",
                    sizeof("OTHERWISE, ARISING FROM, OUT OF OR IN CONNECTION "
                           "WITH THE")
                    - 1,
                    0);
            comment(
            ...
            if (notice.type == 15) {
                ...
                if (notice.item[21].data) {
                    ...
                    sizeof("and is not to be disclosed or used except in")
                    - 1,
                    1);
                    comment(&notice,
                            &buf,
                            "accordance with applicable agreements",
                            sizeof("accordance with applicable agreements")
                            - 1,
                            1);
                }
                comment(&notice, &buf, (( char * )0), 0, 0);
            ...
            if (notice.item[21].data) {
                ...
                if (notice.item[5].data || notice.item[2].data) {
                    ...
                    if ((notice.item[15].size
                         + (notice.item[5].data
                            ? (notice.item[5].size + notice.item[8].size)
                            : notice.item[2].size))
                        >= ((70 - 32) - 6))
                        comment(&notice,
                                &buf,
                ...
                sizeof("under the terms and conditions of the license in")
                - 1,
                0);
                expand(&notice, &tmp, &notice.item[21]);
                comment(&notice,
                ...
                sizeof("The copyright notice above does not evidence any")
                - 1,
                0);
                comment(
                &notice,
...
#line 351
static char *linesync __PARAM__((Proto_t * proto, char *p, long n),
                                (proto, p, n))
__OTORP__(Proto_t *proto; char *p; long n;)
{
...
static char *init __PARAM__((Proto_t * proto, char *op, int flags),
                            (proto, op, flags))
__OTORP__(Proto_t *proto; char *op; int flags;)
{
    char *s;
...
#line 440
        ...
        (
        "\n#if !defined(__PROTO__)\n#  if defined(__STDC__) || "
        "defined(__cplusplus) || defined(_proto) || defined(c_plusplus)\n#   "
        " if defined(__cplusplus)\n#      define __LINKAGE__	\"C\"\n#    "
        "else\n#      define __LINKAGE__\n#    endif\n#    define "
        "__STDARG__\n#    define __PROTO__(x)	x\n#    define "
        "__OTORP__(x)\n#    define __PARAM__(n,o)	n\n#    if "
        ...
        " else\n#    define __VA_START__(p,a)	va_start(p)\n#  endif\n#  if "
        "!defined(__INLINE__)\n#    if defined(__cplusplus)\n#      define "
        "__INLINE__	extern __MANGLE__ inline\n#    else\n#      if "
...
#line 492
static char *nns __PARAM__((char *s), (s)) __OTORP__(char *s;)
{
    while (*s == ' ' || *s == '\t' || *s == '\n')
        s++;
    return s;
...
static int lex __PARAM__((Proto_t * proto, long flags), (proto, flags))
__OTORP__(Proto_t *proto; long flags;)
{
    char *ip;
    char *op;
    int c;
...
fsm_terminal:
    ...
    case ((0 + 28) + 1):
        ...
        default:
            ...
            if ((flags & ((1L << 5) | (1L << 15))) == (1L << 5))
                ...
                (*op++ = (c));
            rp = _pp_fsmtab[((rp) >= _pp_fsmtab[(0 + 16)]
                             && (rp) <= _pp_fsmtab[(0 + 18)])
                            ? (0 + 16)
                            : (0 + 14)];
            break;
    ...
    case ((0 + 28) + 6):
        ...
        if (*proto->tp == '_'
            && !sstrncmp(proto->tp, "__STDPP__directive", 6))
            c = '#';
        else
            c = (0401 + 0);
    ...
    fsm_newline:
        ...
        if (flags & (1L << 3)) {
...

            {
                ...
                flags &= ~((1L << 2) | (1L << 3) | (1L << 7) | (1L << 8)
                           | (1L << 15) | (1L << 17) | (1L << 22) | (1L << 23)
                           | (1L << 25) | (1L << 26));
            }
            call = 0;
            group = 0;
    ...
    if (!(flags & ((1L << 10) | (1L << 11) | (1L << 21)))) {
        if (!(flags & (1L << 3)))
            ...
            case '#':
                ...
                if (proto->brace == 0 && paren == 0 && last != '='
                    && (flags
                        & ((1L << 0) | (1L << 1) | (1L << 3) | (1L << 15)
                           | (1L << 19) | (1L << 23) | (1L << 25)))
                       == ((1L << 15) | (1L << 25))
...
#line 1262
                                ...
                                if (*ip == 'e' && *++ip == 'x' && *++ip == 't'
                                    ...
                                    op = linesync(proto, op, proto->line);
                                    op = strcopy(
                                    (op), ("#define __MANGLE__ __LINKAGE__"));
                                    break;
                                }
                                flags |= (1L << 2) | (1L << 15);
                                im = bp - 1;
        ...
        if ((flags & ((1L << 5) | (1L << 15))) == ((1L << 5) | (1L << 15))
            && ((flags & ((1L << 3) | (1L << 23))) || proto->brace
                || c != '(' && c != ')' && c != '*' && c != (0401 + 0)))
            (op = proto->op);
        else
...
#line 2133
char *pppopen __PARAM__((char *file,
                         int fd,
                         char *notice,
                         char *options,
                         char *package,
                         char *comment,
                         int flags),
                        (file, fd, notice, options, package, comment, flags))
__OTORP__(char *file; int fd; char *notice; char *options; char *package;
          char *comment;
          int flags;)
{
    Proto_t *proto;
...
#line 2243
    ...
    if (flags & (1 << 7))
        proto->flags |= (1L << 13);
    if (!(proto->flags & (1L << 29)) && file && (m = sstrlen(file)) > 2
        && file[--m] == 'y' && file[--m] == '.')
        proto->flags |= (1L << 29);
    ...
    if (proto->flags & (1L << 18)) {
        ...
        if (proto->op == proto->ob) {
            ...
            if ((proto->flags & ((1L << 4) | (1L << 16))) == (1L << 4))
                proto->op = strcopy((proto->op),
                                    ("/* NOTE: some constructs may not have "
                                     "been converted */\n"));
        }
        n = proto->op - proto->ob;
        proto->op = proto->ob;
...
proto __PARAM__((char *file,
                 ...
                 char *comment,
                 int flags),
                (file, license, options, package, copy, comment, flags))
__OTORP__(char *file; char *license; char *options; char *package; char *copy;
          char *comment;
          int flags;)
{
    char *b;
...
static char *type __PARAM__((char *file, char *comment), (file, comment))
__OTORP__(char *file; char *comment;)
{
    char *suffix;
    int i;
...
int main __PARAM__((int argc, char **argv), (argc, argv))
__OTORP__(int argc; char **argv;)
{
    char *b;
    char *file;
    int fd;
