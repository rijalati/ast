...
= "[-?\n@(#)$Id: file (AT&T Research) 2011-08-01 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?\bfile\b tests and attempts to classify each \afile\a "
  "argument."
  ...
  "[c:mime?List the \bmime\b(1) classification for each \afile\a. Although "
  "the"
  ...
  "[f:files|file-list?\afile\a contains list of file names, one per line, "
  "that"
  "	are classified.]:[file]"
...
main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        case 'd':
            ...
            continue;
        case 'f':
            if (streq(opt_info.arg, "-") || streq(opt_info.arg, "/dev/stdin")
                || streq(opt_info.arg, "/dev/fd/0"))
                list = sfstdin;
