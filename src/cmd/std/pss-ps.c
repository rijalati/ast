...
#    define PSS_default                                                      \
        (PSS_pgrp | PSS_pid | PSS_ppid | PSS_sid | PSS_state | PSS_tgrp      \
         | PSS_tty | PSS_uid)
...
ps_read(Pss_t *pss, Pss_id_t pid)
{
    ...
    if (pid || !state->scan) {
        ...
        if (!(state->ps = sfpopen(NiL, pss->buf, "r"))
            || !sfgetr(state->ps, '\n', 0))
            return -1;
    }
    return 1;
    ...
    for (po = pso; po->field; po++)
        if (po->field & fields) {
            ...
            case PSS_start:
                c = 0;
                for (t = s; *s; s++)
                    if (*s == '_' || isalpha(*s) && isdigit(*(s + 1))
                        || isdigit(*s) && isalpha(*(s + 1)))
                        c = 1;
