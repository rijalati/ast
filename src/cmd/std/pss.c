...
pssopen(Pssdisc_t *disc)
{
    ...
    if (!(pss->ttybyname = dtnew(pss->vm, &pss->ttybynamedisc, Dtset))
        ...
        goto bad;
    pss->meth = (_pss_ps && ((disc->flags & PSS_PS) || getenv("_PSS_ps")))
                ? _pss_ps
                : _pss_method;
...
pssclose(Pss_t *pss)
{
    ...
    if ((pss->disc->flags & PSS_VERBOSE) && pss->disc->errorf)
        (*pss->disc->errorf)(
        pss, pss->disc, 1, "%s: method done", pss->meth->name);
    vmclose(pss->vm);
    return r;
}
                ...
                do {
                    ...
                    default:
                        if (pss->disc->errorf)
                            ...
                            pss->disc,
                            2,
                            "%08lx selection not implemented",
                            mp->field);
                        return 0;
                    }
                    for (dp = mp->data; dp; dp = dp->next)
    ...
    for (;;) {
        while (ent = readdir(dir)) {
            if (D_NAMLEN(ent) + (base - path) + 1 > sizeof(path))
                continue;
            if (!sub
                && (ent->d_name[0] != 'c' && ent->d_name[0] != 't'
                    && ent->d_name[0] != 'p' && ent->d_name[0] != 'v'
                    || ent->d_name[1] != 'o' && ent->d_name[1] != 't'))
                continue;
