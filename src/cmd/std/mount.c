...
= "[-?\n@(#)$Id: mount (AT&T Research) 2011-02-11 $\n]" USAGE_LICENSE
  ...
  "[M:mtab|fstab?Use \afile\a instead of the default filesystem "
  "table.]:[file]"
  ...
  "[r:readonly?Mount the filesystems read only. Equivalent to "
  "\b--option=ro\b.]"
  "[t|T:types?Limit the filesystem table scan to entries matching the "
  "\atype\a"
...
= "[-?\n@(#)$Id: umount (AT&T Research) 1999-11-19 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?\bumount\b unmounts one or more currently mounted "
  "filesystems,"
...
#    endif
  ...
  extern int
  mount(const char *, const char *, int, const char *, const char *, int);
extern int
umount(const char *, int);
extern int
unmount(const char *);
...
main(int argc, char **argv)
{
    ...
    if (s = strrchr(*argv, '/'))
        ...
        s = *argv;
    error_info.id = s;
    usage = (state.unmount = strmatch(s, "*u?(n)mount")) ? unmount_usage
                                                         : mount_usage;
    state.matchflags = 0;
    ...
    argv += opt_info.index;
    if ((s = *argv) && !*++argv && !state.mtab
        || state.mtab && (!*state.mtab || streq(state.mtab, "-"))
        || !state.mtab && (state.all || state.match))
        state.mtab = FSTAB;
