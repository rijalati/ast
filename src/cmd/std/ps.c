...
#define FIELDS_l                                                             \
    "flags,state,user,pid,ppid,pri,nice,size,rss,wchan,tty,time,cmd"
...
= "[-1o?\n@(#)$Id: ps (AT&T Research) 2011-12-13 $\n]" USAGE_LICENSE
  ...
  "[c:class?Equivalent to \b--fields=" FIELDS_c
  "\b.]"
  ...
  "[D:define?Define \akey\a with optional \avalue\a. \avalue\a will be "
  "expanded"
  ...
  "[f:full?Equivalent to \b--fields=" FIELDS_f
  "\b.]"
  "[F:format?Append to the listing format string (if \b--format\b is "
  "specified"
  ...
  "	%[#-+]][\awidth\a[.\aprecis\a[.\abase\a]]]]]](\aid\a[:\aheading\a]])"
  "\achar\a."
  ...
  "	list.]:[pgrp...]"
  "[G:groups?List processes with real group id names or numbers in the "
  "\agroup\a"
  "	list.]:[group...]"
  "[h!:heading?Output a heading line.]"
  "[j:jobs?Equivalent to \b--fields=" FIELDS_j
  "\b.]"
  "[l:long?Equivalent to \b--fields=" FIELDS_l
  "\b.]"
  "[L:leaders?List session leaders.]"
  "[n:namelist?Specifies an alternate system namelist \afile\a. Ignored by"
  "	this implementation.]"
  "[N:default?Equivalent to \b--fields=" FIELDS_default
  "\b. This is the"
  ...
  "[s:sessions?List processes with session leaders in the \asid\a "
  "list.]:[sid...]"
  "[t:terminals|ttys?List processes with controlling terminals in the \atty\a"
  "	list.]:[tty...]"
  "[T:tree|forest?Display the process tree hierarchy, including the parents "
  "and"
  "	children of all selected processes, in the \bCMD\b field list.]"
  "[u|U:users?List processes with real user id names or numbers in the "
  "\auser\a"
  "	list.]:[user...]"
...
static Key_t keys[] = {
    { 0 },
    { "addr", "ADDR", "Physical address.", PSS_addr, KEY_addr, 8 },
    {
    "class",
    "CLS",
    "Scheduling class.",
    ...
    { "comm",
      ...
      0,
      KEY_cmd },
    { "cpu", "%CPU", "Cpu percent usage.", PSS_cpu, KEY_cpu, 4 },
    { "etime", "ELAPSED", "Elapsed time since start.", 0, KEY_etime, 7 },
    { "flags",
      "F",
    ...
    { "group",
      ...
      0,
      KEY_gid },
    { "job", "JOB", "Job id.", PSS_job, KEY_job, 5, PID_MAX, 1, 0, 0 },
    { "nice", "NI", "Adjusted scheduling priority.", PSS_nice, KEY_nice, 4 },
    {
    "npid",
    "NPID",
    "Native process id.",
    ...
    { "pgrp",
      ...
      0,
      0 },
    { "pid", "PID", "Process id.", PSS_pid, KEY_pid, 5, PID_MAX, 1, 0, 0 },
    { "ppid", "PPID", "Parent process id.", PSS_ppid, KEY_ppid, 5, PID_MAX, 1 },
    { "pri", "PRI", "Scheduling priority.", PSS_pri, KEY_pri, 3 },
    { "processor", "PROC", "Assigned processor.", PSS_proc, KEY_proc, 3 },
    {
    "refcount",
    "REFS",
    "Reference count.",
    ...
    { "rss",
      ...
      KEY_rss,
      5 },
    { "sid", "SID", "Session id.", PSS_sid, KEY_sid, 5, PID_MAX, 1 },
    { "size",
      "SIZE",
      ...
      KEY_size,
      6 },
    { "start", "START", "Start time.", PSS_start, KEY_start, 8 },
    { "state", "S", "Basic state.", PSS_state, KEY_state, 1 },
    {
    "tgrp",
    "TGRP",
    "Terminal group id.",
    ...
    { "time", "TIME", "usr+sys time.", PSS_time, KEY_time, 6 },
    {
    "tty",
    "TT",
    ...
    { "user",
      ...
      0,
      KEY_uid },
    { "wchan", "WCHAN", "Wait address.", PSS_wchan, KEY_wchan, 8 },
    ...
    if (streq(s, "formats"))
        for (i = 1; i < elementsof(keys); i++)
        {
            ...
            if (keys[i].head)
                sfprintf(
                sp,
                "%s The title string is \b%s\b and the default width is "
                "%d.%s]",
                keys[i].desc,
                keys[i].head,
                keys[i].width,
                (!keys[i].field || (state.pss->meth->fields & keys[i].field))
                ? ""
                : " Not available on this system.");
            else
                sfprintf(
                sp, "Equivalent to \b%s\b.]", keys[keys[i].index].name);
        }
    return 0;
}
...
key(void *handle, Sffmt_t *fp, const char *arg, char **ps, Sflong_t *pn)
{
    ...
    else
    {
        ...
        case KEY_start:
            ...
            {
                u = pp->ps->start;
                s = fmttime((state.now - u) >= (24 * 60 * 60) ? "%y-%m-%d"
                                                              : "%H:%M:%S",
                            u);
            }
            else
...
ps(Ps_t *pp)
{
    ...
    for (kp = state.fields; kp; kp = kp->next)
    {
        ...
        case KEY_start:
            ...
            u = pr->start;
            s = fmttime(
            (state.now - u) >= (24 * 60 * 60) ? "%y-%m-%d" : "%H:%M:%S", u);
            goto string;
        case KEY_state:
            *(s = sbuf) = pr->state;
            *(s + 1) = 0;
            ...
            for (pp = ( Ps_t * )dtfirst(state.byorder); pp;
                    ...
                    && !pp->seen)
                {
                    ...
                    do
                    {
                        xp->seen = 1;
                        if (xp->ps->pss
                            & (PSS_ANCESTOR | PSS_EXPLICIT | PSS_MATCHED))
                        {
...
head(void)
{
    ...
    {
        sfkeyprintf(
        state.heading ? sfstdout : state.wrk, NiL, state.format, key, NiL);
...
addkey(const char *k, int ignore)
{
    ...
    if (streq(s, "?"))
    {
        ...
        for (kp = keys + 1; kp < keys + elementsof(keys); kp++)
        {
            ...
            sfprintf(sfstdout,
                     ...
                     ap->desc,
                     ap == kp ? "" : " [alias]",
                     (!ap->field || (state.pss->meth->fields & ap->field))
                     ? ""
                     : " [not available]");
        }
        exit(0);
    ...
    do
    {
        ...
        if (!kp->already)
        {
            kp->already = keys[kp->cancel].already = keys[kp->cancel].skip
            = 1;
            if (state.lastfield)
...
main(int argc, char **argv)
{
    ...
    if (!(state.keys = dtopen(&kd, Dtset))
        || !(state.bypid = dtopen(&nd, Dtset))
        || !(state.byorder = dtopen(&sd, Dtset)))
        error(ERROR_SYSTEM | 3, "out of space");
    ...
    if (state.children || state.parents || !state.pids)
    {
        ...
        if (!state.pids && !state.pssdisc.match
            && !(state.pssdisc.flags
                 & (PSS_ALL | PSS_ATTACHED | PSS_DETACHED | PSS_LEADER
                    | PSS_NOLEADER)))
        {
