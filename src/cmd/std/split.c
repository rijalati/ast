...
= "[-?\n@(#)$Id: csplit (AT&T Research) 2003-08-21 $\n]" USAGE_LICENSE
  ...
  "[f:prefix?Use \aprefix\a to generate the file name "
  "prefix.]:[prefix:=\bxx\b]"
  "[k:keep-files?Do not remove output files on errors.]"
  "[a|n:digits?Use \adigits\a in the generated file name "
  "suffixes.]#[digits:=2]"
...
main(int argc, char **argv)
{
    ...
    argv += opt_info.index;
    argc -= opt_info.index;
    if (error_info.errors || !(flags & C_FLAG) && argc > 2
        || (flags & C_FLAG) && argc < 2)
    ...
    if (flags & C_FLAG) {
        ...
        while (sp = *argv++) {
            ...
            case '{':
                if (!op)
                    error(ERROR_exit(1),
                          "%s: pattern expected for repeat count",
                          *(argv - 1));
                if (*++sp == '*' && *(sp + 1) == '}' && !*(sp + 2))
                    op->repeat = 0;
                else {
            ...
            default:
                if ((size = strtoll(sp, &sp, 10)) <= 0 || *sp)
                    error(
                    ERROR_exit(1), "%s: invalid line number", *(argv - 1));
