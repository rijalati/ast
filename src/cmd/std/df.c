...
= "[-?\n@(#)$Id: df (AT&T Research) 2010-08-11 $\n]" USAGE_LICENSE
  ...
  "[f:format?Append to the listing format string. The \bls\b(1), \bpax\b(1) "
  "and"
  ...
  "[i:inodes?Display inode usage instead of block usage. There is at least "
  "one"
  ...
  " \"%#..1(filesystem)s %#(type)s %#(blocks)s %#(used)s %#(available)s  "
  "%#(capacity)s  %(mounted)s\"]"
...
static Key_t keys[] = {
    ...
    { 0 },
    { "available", "Available", "Unused block count.", KEY_available, 0, 5 },
    { "blocks", 0, "Total block count.", KEY_blocks, 0, 0 },
    { "capacity",
      "Capacity",
    ...
    { "filesystem",
      ...
      -19,
      0 },
    { "iavailable", "Iavailable", "Unused inode count.", KEY_iavailable, 7, 6 },
    { "icapacity",
      "Icapacity",
      ...
      4,
      4 },
    { "inodes", "Inodes", "Total inode count.", KEY_inodes, 7, 3 },
    { "iused", "Iused", "Used inode count.", KEY_iused, 7, 3 },
    { "mounted", "Mounted on", "Mounted on path.", KEY_mounted, -19, 5 },
    { "size", "Size", "Native block size.", KEY_native, 4, 3 },
    { "options", "Options", "\bmount\b(1) options.", KEY_options, -29, 3 },
    { "type", "Type", "Filesystem type.", KEY_type, 10, 3 },
    { "used", "Used", "Used block count.", KEY_used, 0, 3 },
...
static const char fmt_opt[]
= "%#..1(filesystem)s %#(type)s  %#(options)s %(mounted)s";
...
#        define F_FRSIZE(v)                                                  \
            ((v)->f_frsize ? (v)->f_frsize                                   \
                           : (v)->f_bsize ? (v)->f_bsize : 1024)
#    else
    ...
    if (streq(s, "formats"))
        for (i = 1; i < elementsof(keys); i++)
        {
            ...
            if (keys[i].width)
                ...
                sfprintf(sp,
                         "%s determined by the \b-b\b, \b-g\b, \b-k\b, "
                         "\b-m\b and \b-n\b options.]",
                         keys[i].heading ? "is" : "are");
        }
    return 0;
}
...
key(void *handle, Sffmt_t *fp, const char *arg, char **ps, Sflong_t *pn)
{
    ...
    else
    {
        ...
        case KEY_blocks:
            s = (df->total || df->ttotal)
                ? scale(df->fraction, df->total, df->ttotal)
                : "-";
            break;
        case KEY_capacity:
            s = (df->total || df->ttotal)
                ? (sfsprintf(
                   state.buf, sizeof(state.buf), "%3d%%", df->percent),
                   state.buf)
                : "-";
            break;
        case KEY_environ:
        ...
        case KEY_icapacity:
            s = (df->total || df->ttotal)
                ? (sfsprintf(
                   state.buf, sizeof(state.buf), "%3d%%", df->ipercent),
                   state.buf)
                : "-";
            break;
        case KEY_inodes:
        ...
        case KEY_native:
            if ((n = F_FRSIZE(&df->vfs)) >= 1024)
                sfsprintf(
                state.buf, sizeof(state.buf), "%I*3dk", sizeof(n), n / 1024);
            else
                sfsprintf(
                state.buf, sizeof(state.buf), "%I*4d", sizeof(n), n);
            s = state.buf;
            break;
        case KEY_options:
            if (!(s = df->mnt->options))
        ...
        case KEY_used:
            s = (df->total || df->ttotal)
                ? scale(df->fraction, df->used, df->tused)
                : "-";
            break;
        default:
            return 0;
...
main(int argc, char **argv)
{
    ...
    if (!(fmt = sfstropen()) || !(state.mac = sfstropen())
        || !(state.tmp = sfstropen()))
    ...
    else
    {
        ...
        if (rem && (match = newof(0, int, n, 0)))
        {
            ...
            while (df.mnt = mntread(mp))
                ...
                for (n = 0; n < argc; n++)
                    if (argv[n] && (i = strlen(df.mnt->dir)) > 1
                        && i > match[n] && strneq(argv[n], df.mnt->dir, i)
                        && (argv[n][i] == '/' || !argv[n][i]))
                        match[n] = i;
            mntclose(mp);
            if (!(mp = mntopen(NiL, "r")))
