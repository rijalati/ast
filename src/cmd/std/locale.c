...
= "[-?\n@(#)$Id: locale (AT&T Research) 2009-10-21 $\n]" USAGE_LICENSE
  ...
  "[+?If the \b--all\b or \b--undefined\b options are specified and no "
  "operands"
  ...
  "[b:abbreviated?List abbreviated locale names: \alanguage\a and "
  "\aterritory\a"
  "	as the two character ISO codes; non-default \acharset\a and"
  "	\aattributes\a. This is the default.]"
  "[c:category?List the category names for each operand on the standard "
  "output.]"
  "[e:element?The operands are interpreted as collation elements. Each "
  "element"
  ...
  "[l:local?List the locale names returned by the local system "
  "\bsetlocale\b(3)."
  ...
  "	\aattributes\a.]"
  "[x:transform?The operands are interpreted as strings. Each string is "
  "listed"
...
list_locale(Sfio_t *sp, Keyword_t *key, Lc_t *lc, unsigned int flags)
{
    ...
    static unsigned long types[]
    = { LC_abbreviated, LC_qualified, LC_verbose, LC_local };
...
static int
scan(Sfio_t *, Keyword_t *, unsigned long);
            ...
            for (i = 1; i < AST_LC_COUNT; i++) {
                ...
                sfprintf(sfstdout,
                         ...
                         fmtquote(s,
                                  ...
                                  "\"",
                                  strlen(s),
                                  (value || !getenv(state.categories[i].name))
                                  ? FMT_ALWAYS
                                  : 0));
            }
