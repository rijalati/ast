...
info_full(Pss_t *pss, Pssent_t *pe)
{
    ...
    if (pe->state != PSS_ZOMBIE) {
        ...
        if (fields & PSS_command)
            pe->command
            = ((s = strrchr(pr->pname, '/')) && *++s) ? s : pr->pname;
        pe->args = pr->pname;
    }
    pe->flags = pr->pflag;
    pe->ppid = pr->ppid;
...
= { "info",
    ...
    PSS_args | PSS_command | PSS_flags | PSS_gid | PSS_nice | PSS_pgrp
    | PSS_pid | PSS_ppid | PSS_pri | PSS_sid | PSS_size | PSS_state | PSS_time
    | PSS_tty | PSS_uid,
    info_init,
    info_read,
    info_part,
