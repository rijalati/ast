...
static State_t state = {
    ...
    { "cbs", NUMBER, "Conversion buffer size (logical record length)." },
    {
    "conv",
    CONV,
    "Convert input.",
    ...
    { "ospecial", OSPECIAL, "currently ignored" },
    {
    "swab",
    SWAP,
    "swap byte pairs",
...
static Desc_t desc[] = {
    "codeset", "", "conversion", "", 0, 0, "number", 0, "file", 0,
};
...
main(int argc, char **argv)
{
    ...
    while (f = optget(argv, usage))
    {
        ...
        case NUMBER:
            ...
            if (*v)
                error(3,
                      "%s: %s: invalid numeric expression",
                      op->name,
                      opt_info.arg);
            break;
        }
    ...
    default:
        error(3,
              ...
              state.conv.name,
              state.ascii.value.string,
              state.ebcdic.value.string,
              state.ibm.value.string);
    }
    if (streq(state.from.value.string, state.to.value.string))
        state.cvt = (iconv_t)(-1);
    else if ((state.cvt
              = iconv_open(state.to.value.string, state.from.value.string))
             == (iconv_t)(-1))
        error(3,
              "cannot convert from %s to %s",
              *state.from.value.string ? state.from.value.string
                                       : ccmapname(CC_NATIVE),
              *state.to.value.string ? state.to.value.string
                                     : ccmapname(CC_NATIVE));
    ...
    if ((state.conv.value.number & ISPECIAL)
        ...
        state.in.special = 1;
    if (state.in.special && !(state.conv.value.number & BLOCK)
        && (n = state.ibs.value.number))
        sfsetbuf(state.in.fp, NiL, n);
    if ((state.conv.value.number & OSPECIAL)
        || fstat(sffileno(state.out.fp), &st) || !S_ISREG(st.st_mode))
        state.out.special = 1;
    if (state.out.special && !(state.conv.value.number & UNBLOCK)
    ...
    if (n = state.skip.value.number)
    {
        ...
        if (sfseek(state.in.fp, n, SEEK_SET) != n)
        {
            ...
            if (n > 0)
                error(3,
                      "%s: cannot seek past end of file",
                      state.ifn.value.string);
        }
    }
    ...
    else
    {
        ...
        while (state.in.complete != r)
        {
            ...
            while (state.in.complete != r)
            {
                ...
                if (m >= c)
                {
                     ...
                     * previous write was in the middle of an output block,
                     * and adjust the # of bytes to write accordingly; don't
                     * bother adjusting the remains, since we're done
                     */
                ...
                else
                {
                    ...
                    /*
                     * see if writing the amount read in will cross an
                ...
                if (state.cvt != (iconv_t)(-1))
                {
                    ...
                    if ((d = iconv_write(
                         state.cvt, state.tmp, &cb, &cc, &state.iconv))
                        < 0)
                        d = 0;
        ...
        if (sfsync(state.out.fp))
            error(
            ERROR_SYSTEM | 3, "%s: write error", state.ofn.value.string);
    }
