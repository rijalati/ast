...
init(void *handle, int fdmax)
{
    ...
    do
        info(DEF | NEW, *state.argv);
    while (*state.argv && *++state.argv);
    ...
    if (state.perserver <= 0 || state.perserver > n)
        state.perserver = n;
    if (state.peruser <= 0
        && (state.peruser = (3 * (state.pool - 1)) / 2) <= 0)
        state.peruser = 1;
...
service(void *handle, int fd)
{
    ...
    case SHELL:
#ifdef O_NONBLOCK
        if (state.con[fd].flags >= 0
            && fcntl(fd, F_SETFL, state.con[fd].flags | O_NONBLOCK) < 0)
    ...
    case INIT:
            ...
            while (e = strchr(s, '\n')) {
                *e++ = 0;
                if (strneq(s, CO_ENV_ATTRIBUTES, sizeof(CO_ENV_ATTRIBUTES) - 1)
                    ...
                    else if (strneq(s, CO_OPT_HOME, sizeof(CO_OPT_HOME) - 1)
                             && s[sizeof(CO_OPT_HOME) - 1] == '='
                             && (sp = search(
                                 GET, s + sizeof(CO_OPT_HOME), NiL, NiL)))
                        (state.con[fd].info.user.home = sp)->home++;
                    else if (strneq(
                             s, CO_OPT_INDIRECT, sizeof(CO_OPT_INDIRECT) - 1)
                             && s[sizeof(CO_OPT_INDIRECT) - 1] == '=')
                        state.con[fd].info.user.pump
                        = strdup(s + sizeof(CO_OPT_INDIRECT));
                }
                s = e;
            }
        }
        ...
        if (state.indirect.con) {
            ...
            if (state.con[fd].info.user.flags & USER_DUP)
                state.con[fd].info.user.fds[2]
                = state.con[fd].info.user.fds[1];
            else if ((state.con[fd].info.user.fds[2]
                      = csopen(state.con[fd].info.user.pump, 0))
                     < 0
            ...
            if (state.identify) {
                ...
                if (state.con[n].info.ident.shell != jp->shell
                    ...
                    if (*(s = state.con[fd].info.pass.job->label))
                        ...
                        sfprintf(state.string, " %d", jp->pid);
    ...
    case USER:
        ...
        case 's':
        case 'S':
            if (tokscan(state.buf, NiL, "%s %s %s %d %s", NiL, &s, &t, &n1, &x)
                != 5)
                error_info.errors++;
            else
                server(fd, *s, *t, n1, x);
...
main(int argc, char **argv)
{
    ...
    if (!strmatch(s, "/dev/fdp/*"))
        state.indirect.con = 1;
    ...
    if (n != SERVER) {
        ...
        error(
        ERROR_SYSTEM | 3, "%s: cannot connect to server", state.service);
    }
    state.argv = argv + 1;
