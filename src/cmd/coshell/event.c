...
static const Request_t requests[] = {
    "a*ll",    REQ_all,     1, 1,  "c*lear", REQ_clear, 1, -1,
    "d*elete", REQ_delete,  1, -1, "e*xit",  REQ_exit,  0, 0,
    "hold",    REQ_hold,    0, -1, "i*nfo",  REQ_info,  0, 0,
    "l*ist",   REQ_list,    0, 1,  "n*ext",  REQ_next,  0, 0,
    "q*uit",   REQ_exit,    0, 0,  "r*aise", REQ_raise, 1, -1,
    "release", REQ_release, 0, -1, "s*et",   REQ_set,   0, 0,
    "stop",    REQ_stop,    0, 0,  "t*est",  REQ_test,  1, 1,
    "w*ait",   REQ_wait,    1, 1,
};
...
info(State_t *state, Connection_t *con, Css_t *css)
{
    ...
    log(state,
        con,
        'I',
        "info server='%s' version=%d.%d host=%s pid=%d uid=%d gid=%d",
        fmtident(usage),
        EVENT_MAJOR,
        EVENT_MINOR,
        csname(css->state, 0),
      ...
      Data_t *dat)
{
    ...
    case REQ_test:
        ...
        break;
    case REQ_wait:
        if (val.dptr && !state->hold
            && !(dat->flags & (DATA_clear | DATA_hold)))
            log(state, con, 'I', "%s raised", key.dptr);
        ...
        if (tokscan(
            ...
            if (!(
                        ...
                        if (r->index == REQ_set) {
                            ...
                            if (*t) {
                                log(state,
                                    con,
                                    'E',
                                    "%s: invalid elapsed time expression",
                                    opt_info.arg);
                                err = 1;
                                break;
                            }
                ...
                if (!err) {
                    ...
                    if (r->min && n < r->min)
                        sfprintf(state->usrf,
                                 "E %s: at least %d argument%s expected\n",
                                 s,
                                 r->min,
                                 r->min == 1 ? "" : "s");
                        ...
                        log(state,
                            con,
                            'E',
                            "%s: at most %d argument%s expected",
                            s,
                            r->max,
                            r->max == 1 ? "" : "s");
                                ...
                                if (EVENT(con->list.dptr)
                                    ...
                                    if ((!con->older || data.time < con->older)
                                        ...
                                        log(state,
                                            ...
                                            fmttime("%K", data.time),
                                            data.raise,
                                            (data.flags & DATA_clear)
                                            ? " CLEAR"
                                            : "",
                                            (data.flags & DATA_hold) ? " HOLD"
                                                                     : "");
                                        break;
                                    }
                                }
...
db(State_t *state)
{
    ...
    if (!(state->dbm = dbm_open(state->path,
                                O_RDWR | O_CREAT,
    ...
    if (val.dptr) {
        ...
        if (memcmp(&data.expire, ident_name, sizeof(data.expire)))
            error(3,
                  "%s: %s: invalid db -- ident mismatch, expected %s",
                  state->path,
                  ident_key,
                  ident_name);
        u4 = IDENT_SWAP;
        ...
        if (val.dptr)
            error(3,
                  "%s: %s: invalid db -- ident entry expected",
                  state->path,
                  ident_key);
...
main(int argc, char **argv)
{
    ...
    if (server) {
        ...
        state.disc.flags = CSS_DAEMON | CSS_LOG | CSS_ERROR | CSS_INTERRUPT
                           | CSS_WAKEUP | CSS_PRESERVE;
