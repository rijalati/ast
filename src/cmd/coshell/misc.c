    ...
    if (s && (isalpha(*s) || *s == '_')) {
        ...
        if (!set) {
            ...
            case HASHKEY4('b', 'i', 'a', 's'):
                return (osp
                        ? osp->bias == sp->bias
                        : op ? miscop(sp->bias, op, strton(cp, e, NiL, 100))
                             : sp->bias);
            case HASHKEY3('c', 'p', 'u'):
            ...
            case HASHKEY4('i', 'd', 'l', 'e'):
                STATS(sp, osp);
                return (osp
                        ? osp->stat.idle == sp->stat.idle
                        : op ? miscop(sp->stat.idle, op, strelapsed(cp, e, 1))
                             : sp->stat.idle);
            case HASHKEY4('j', 'o', 'b', 's'):
                return (osp ? osp->running == sp->running : sp->running);
            case HASHKEY4('l', 'o', 'a', 'd'):
                STATS(sp, osp);
                return (osp ? (osp->stat.load / osp->scale)
                              == (sp->stat.load / sp->scale)
                            : op ? miscop(sp->stat.load / sp->scale,
                                          op,
                                          strton(cp, e, NiL, 100))
                                 : sp->stat.load / sp->scale);
            case HASHKEY3('m', 'i', 'n'):
        ...
        for (;;) {
            ...
            for (;;) {
                if (!*cp) {
                    ...
                    if (osp)
                        return (!miscget(osp, bp, 1, &bp, NiL));
                    return (0);
                }
                if (*cp++ == ' ')
                    break;
    ...
    if (s) {
        ...
        for (;;) {
            ...
            case HASHKEY6('m', 'a', 'x', 'l', 'o', 'a'):
                if (!sp
                    && (p->global.maxload = strton(v, NiL, NiL, 100)) >= 0)
            ...
            case HASHKEY6('p', 'r', 'o', 'f', 'i', 'l'):
                ...
                continue;
            case HASHKEY6('r', 'a', 't', 'i', 'n', 'g'):
                if (p->rating = sp ? sp->rating : strton(v, NiL, NiL, 100))
                    p->set |= SETRATING;
                continue;
