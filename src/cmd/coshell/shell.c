...
shellopen(Coshell_t *sp, int fd)
{
    ...
    if (!sp) {
        ...
        if (sp == state.shell) {
            ...
            "/dev/fd/4 >/dev/null 2>&1 3<%s 4<&3 5>&- 6>&- 7>&- 8>&- 9>&- &'",
            sh,
            CO_ENV_HOST,
            sp->name,
            CO_ENV_TYPE,
...
shellexec(Cojob_t *jp, char *msg, int fd)
{
    ...
    if (!jp || !(sp = jp->shell) || sp == &state.wait) {
        ...
        if (!jp) {
            ...
            state.jobnext = jp;
            if (attr.set & SETLABEL)
                strcpy(jp->label, attr.label);
            else
                *jp->label = 0;
            jp->rid = id;
            jp->fd = fd;
    ...
    sfprintf(state.string,
             "{\ntrap 'set %s$?; trap \"\" 0; print -u3 x %d $1 $(times); "
             "3>&-; exit $1' 0 HUP INT QUIT TERM%s\n",
             (flags & CO_SILENT) ? "" : "+x ",
             jp - state.job,
             (flags & CO_IGNORE) ? "" : " ERR");
    if (!out) {
    ...
    if (!out && !(out = con[fd].info.user.pump))
        out = state.pump;
    else if (*out != '/' && *out != '&')
        sfprintf(state.string, "%s/", pwd);
    sfprintf(state.string, "%s 2%s", out, red);
    if (!err && !(err = con[fd].info.user.pump))
        err = state.pump;
    else if (*err != '/' && *err != '&')
        sfprintf(state.string, "%s/", pwd);
