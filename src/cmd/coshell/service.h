...
#define match(p, a, o)                                                       \
    ((!((a)->set & SETNAME) || strmatch((p)->name, (a)->name))               \
     && ((((a)->set | o) & (SETMISC | DEF | NEW | SET)) == SETMISC           \
         ? miscmatch(p, (a)->misc)                                           \
         : ((a)->set & SETNAME) ? 1 : streq((p)->type, state.home->type)))
...
extern void
attributes(char *, Coattr_t *, Coattr_t *);
extern int
byname(const char *, const char *);
extern int
byrank(const char *, const char *);
extern int
bytemp(const char *, const char *);
extern int
command(int, char **);
extern void
drop(int);
...
jobkill(Cojob_t *, int);
extern void
miscadd(Coshell_t *, char *);
extern int
miscmatch(Coshell_t *, char *);
extern Coshell_t *
search(int, char *, Coattr_t *, Coattr_t *);
extern void
server(int, int, int, int, char *);
extern void
shellcheck(void);
extern void
shellclose(Coshell_t *, int);
extern void
shellexec(Cojob_t *, char *, int);
extern int
shellopen(Coshell_t *, int);
extern char *
stream(int, char *);
extern void
update(Coshell_t *);
