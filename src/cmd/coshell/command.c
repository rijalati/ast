    ...
    for (;;)
    {
        ...
        case 'R':
            ...
            sfprintf(state.string,
                     ...
                     NiL,
                     NiL);
            if (!att[0] || att[0] == '-' && !att[1])
                sfprintf(state.string, " %s", NiL);
            else
                sfprintf(state.string, " (%d:%s)", strlen(att), att);
        ...
        default:
            if (ap)
                error(3, "%c: unknown command -- use h for help", c);
...
");
        ...
        sfprintf(state.string, "#%05d\n", n - 7);
        if (!cmdsend(fd, c, sfstrseek(state.string, 0, SEEK_SET), n, &code))
            break;
    }
    return (code);
}
...
server(int fd, int op, int sub, int arg, char *dat)
{
        ...
        if (dat)
        {
            ...
            while (s = tokread(t))
            {
                if (!(sp = search(GET, s, NiL, NiL)))
                    ...
                    shellclose(sp, con[fd].info.user.fds[2]);
            }
            tokclose(t);
        }
        break;
            ...
            for (n = 0; n <= state.fdtotal; n++)
                ...
                case PASS:
                    if (jp = con[n].info.pass.job)
                        sfprintf(state.string,
                                 ...
                                 jp->shell->name,
                                 jp->pid,
                                 con[n].info.pass.serialize ? " serialize"
                                                            : "");
                    else
                        sfprintf(state.string,
                                 "%3d  pass  %-3d zombie\n",
                ...
                case SHELL:
                    sfprintf(state.string,
                             "%3d  shell %s\n",
                             n,
                             con[n].info.shell->name);
                    break;
                case USER:
                    if (con[n].info.user.fds[0] == n)
                        sfprintf(state.string,
                                 "%3d  query %-3d %-3d %s\n",
                                 n,
                                 con[n].info.user.fds[1],
                                 con[n].info.user.fds[2],
                                 (sp = con[n].info.user.home) ? sp->name
                                                              : "");
                    else
                        sfprintf(state.string,
                                 "%3d  user  %-3d %-3d %-3d %s\n",
                                 n,
                                 con[n].info.user.fds[0],
                                 con[n].info.user.fds[1],
                                 con[n].info.user.fds[2],
                                 (sp = con[n].info.user.home) ? sp->name
                                                              : "");
    ...
    case 'g':
        ...
        sfprintf(state.string,
                 "    access  %-7s",
                 state.access < cs.time
                 ? "EXPIRED"
                 : fmtelapsed(state.access - cs.time, 1));
        ...
        sfprintf(state.string,
                 "     clock  %-6s ",
                 fmtelapsed(cs.time - state.clock, 1));
        sfprintf(state.string, "      jobs  %-6d ", state.jobs);
        sfprintf(
        state.string, "      real  %-6s ", fmtelapsed(state.real, 1));
        sfprintf(
        state.string, "      user  %-6s\n", fmtelapsed(state.user, CO_QUANT));
        ...
        sfprintf(
        state.string, "      busy  %-6s ", fmtelapsed(state.busy, 1));
        sfprintf(
        state.string, "     grace  %-6s ", fmtelapsed(state.grace, 1));
        ...
        sfprintf(
        state.string, "   maxidle  %-6s ", fmtelapsed(state.maxidle, 1));
        sfprintf(state.string, "   perhost  %-6d ", state.perhost);
        sfprintf(state.string, "      pool  %-6d\n", state.pool);

        sfprintf(
        state.string, "   disable  %-6s ", fmtelapsed(state.disable, 1));
    ...
    case 'j':
        for (jp = state.job; jp <= state.jobmax; jp++)
            if (jp->pid)
            {
                ...
                sfprintf(state.string,
                         "%3d%4d%4d%6s%7s%c%-16s %s%s%s",
                         jp - state.job,
                         jp->fd,
                         jp->rid,
                         t,
                         fmtelapsed(cs.time - jp->start, 1),
                         jp->ref <= 0
                         ? '*'
                         : jp->lost ? (jp->lost > cs.time ? '?' : '!') : ' ',
                         jp->shell->name,
                         v,
                         *v ? " " : "",
                         jp->label);
                if (jp->sig)
                    sfprintf(state.string, " [%s]", fmtsignal(jp->sig));
                sfputc(state.string, '\n');
            }
        break;
    case 'k':
        if (arg == NOARG)
            error(
            ERROR_OUTPUT | 2, con[fd].info.user.fds[2], "job id required");
        ...
        if (dat)
        {
            ...
            while (s = tokread(t))
            {
                if (!(sp = search(NEW, s, NiL, NiL)))
                    ...
                    shellopen(sp, con[fd].info.user.fds[2]);
            }
            tokclose(t);
        }
        break;
        ...
        if (n = sv - state.shellv)
        {
            *sv = 0;
            ...
            case 'a':
                sfprintf(
                state.string,
                "HOST         CONF\n%-12.12s busy=%s grace=%s name=%s "
                "percpu=%d perserver=%d peruser=%d pool=%d\n",
                CS_HOST_LOCAL,
                fmtelapsed(state.busy, 1),
                fmtelapsed(state.grace, 1),
                state.shell->name,
                state.percpu,
                state.perserver,
                state.peruser,
            ...
            case 't':
                sfprintf(state.string,
                         "CON OPEN USERS     UP CONNECT UPDATE OVERRIDE   "
                         "IDLE   TEMP     RANK HOST\n");
                break;
            case 'p':
                sfprintf(
                state.string, "PID    HOST\n%-6d %s\n", getpid(), cs.cs);
                break;
            default:
                sfprintf(state.string,
                         "CON JOBS TOTAL   USER    SYS   IDLE CPU  LOAD "
            ...
            while (sp = *sv++)
            {
                ...
                case 'a':
                    ...
                    if (sp->bias != BIAS)
                        sfprintf(
                        state.string, " bias=%s", fmtfloat(sp->bias));
                    if (sp->cpu != 1)
                    ...
                    if (sp->idle)
                        sfprintf(
                        state.string, " idle=%s", fmtelapsed(sp->idle, 1));
                ...
                default:
                    if (sp->fd < 0)
                        ...
                        sfsprintf(t = num[0], sizeof(num[0]), "%d", sp->fd);
                    else
                        t = (sp->temp && !(sp->flags & IGN)) ? "-" : "@";
                    if (sub == 'o' || sub == 's' || sub == 't')
                        sfprintf(
                        state.string,
                        "%3s%c%4d%6d%7s%8s%7s%7s/%d%7s%7s%9s %s\n",
                        t,
                        ...
                        fmtelapsed(
                        sp->update > cs.time ? sp->update - cs.time : 0, 1),
                        fmtelapsed(
                        sp->override > cs.time ? sp->override - cs.time : 0,
                        1),
                        sp->home,
                        fmtelapsed(sp->idle, 1),
                        fmtfloat(sp->temp
                                 >> (CHAR_BIT * sizeof(sp->temp) / 2)),
                        fmtfloat(sp->rank),
                        sp->name);
                    else
                        sfprintf(
                        state.string,
                        "%3s %4d%6d%7s%7s%7s%c%3d%6s%7s%6s %-8.8s %s\n",
                        t,
                        sp->running,
                        sp->total,
                        ...
                        fmtelapsed(
                        sp->stat.up < 0 ? -sp->stat.up : sp->stat.idle, 1),
                        sp->stat.up < 0
                        ? '-'
                        : (sp->mode & SHELL_DISABLE)
                          ? '$'
                          : (sp->mode & SHELL_DENIED)
                            ? '!'
                            : sp->idle && sp->stat.idle < sp->idle
                              ? (sp->home ? '~' : '*')
                              : ' ',
                        sp->cpu,
    ...
    case 't':
        ...
        sfprintf(state.string,
                 "SHELLS  USERS    JOBS  CMDS     UP   REAL   USER    SYS    "
                 " CPU  LOAD RATING\n");
        ...
        sfsprintf(num[3],
                  ...
                  state.override);
        sfprintf(state.string,
                 "%6s%7s%8s%6d%7s%7s%7s%7s%8s%6s%7s\n",
                 num[0],
                 num[1],
                 num[2],
                 state.cmds,
                 fmtelapsed(cs.time - state.start, 1),
                 fmtelapsed(state.real, 1),
                 fmtelapsed(state.user, CO_QUANT),
                 fmtelapsed(state.sys, CO_QUANT),
                 num[3],
                 fmtfloat(tot.stat.load),
                 fmtfloat(tot.rating));
        break;
    case 'u':
        for (n = 0; n <= state.fdtotal; n++)
            if (con[n].type == USER)
            {
                ...
                sfprintf(state.string,
                         ...
                         n,
                         con[n].info.user.pid,
                         con[n].info.user.running,
                         con[n].info.user.total,
                         cspath(con[n].info.user.fds[2], 0),
                         con[n].info.user.attr.label);
            }
        break;
    case 'v':
        sfprintf(state.string, "%s\n", state.version);
    ...
    if ((n = sfstrtell(state.string)) > 0)
        cswrite(
        con[fd].info.user.fds[1], sfstrseek(state.string, 0, SEEK_SET), n);
}
