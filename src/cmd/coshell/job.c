...
jobcheck(Coshell_t *only)
{
        ...
        if (jp->pid && ((sp = jp->shell) == only || !only))
        {
            if (jp->pid > 0)
            {
                ...
                if (jp->busy == HOG)
                {
                    ...
                    error(ERROR_OUTPUT | 2,
                          state.con[jp->fd].info.user.fds[2],
                          "job %d pid %d is no longer hogging %s",
                          jp - state.job,
                          jp->pid,
                          sp->name);
                    jp->busy = 0;
    ...
    if (state.jobwait || state.shellwait)
...
jobdone(Cojob_t *jp)
{
    ...
    if (jp->fd > 0)
    {
        ...
        n = sfsprintf(buf,
                      ...
                      jp->rid,
                      jp->status,
                      fmtelapsed(jp->user, CO_QUANT),
                      fmtelapsed(jp->sys, CO_QUANT));
