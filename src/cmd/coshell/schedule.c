...
#define RNK(p, a)                                                            \
    (((p)->mode & SHELL_DISABLE)                                             \
     ...
     : ((p)->flags & IGN)                                                    \
       ? (INT_MAX / 2)                                                       \
       : ((((p)->stat.load / (p)->scale)                                     \
           + ((p)->running * BIAS + CPU((p)->cpu, (a)->bias)))               \
          * (p)->bias * (((p) == state.home && (p)->cpu == 1) ? 400 : 100)   \
          / (p)->rating))
    ...
    if (!(op & JOB)) {
        if (a->global.set) {
            ...
            if (a->global.set & SETIDENTIFY)
                state.identify
                = dupstring(state.identify, a->global.identify);
            ...
            if (a->global.set & SETSCHEDULE) {
                ...
                if (state.scheduler.fd > 0)
                    ...
                    = dupstring(state.scheduler.name, name);
                else
            ...
            if (a->global.set
                & (SETBUSY | SETMAXIDLE | SETPERCPU | SETPERHOST
                   | SETPERSERVER | SETPERUSER))
                jobcheck(NiL);
        }
    ...
    if (op & JOB) {
        ...
        do {
            ...
            message((-6,
                     ...
                     ap->name,
                     (a->set & SETNAME) ? a->name : "*",
                     ((a->set | op) & (SETMISC | DEF | NEW | SET)) == SETMISC
                     ? a->misc
                     : "*",
                     matched));
            if (!(scan & ((1 << W_TEMP) - 1)))
        ...
        if (mp
            && (!sp
                || nopen < state.pool + noverride
                   && RNK(mp, a) < PCT(sv, H_RANK)))
            sp = mp;
        if (!sp && (op & DEF))
            sp = state.home;
        message((-4,
                 "open=%d override=%d sp=%s mp=%s xp=%s dp=%s dt=%s",
                 nopen,
                 noverride,
                 sp ? sp->name : "*",
                 mp ? mp->name : "*",
...
empty:
    if (!(op & NEW) && (op & (DEF | GET)))
        return (0);
    if (state.check.host && !strmatch(a->name, state.check.host)
        && !strmatch(csntoa(addr), state.check.host))
        return (0);
...
found:
    ...
    case SET:
        ...
        if (a->global.set & (SETIDLE | SETLOAD | SETUPDATE | SETUSERS)) {
            ...
            if (a->global.set & SETLOAD)
                sp->stat.load = a->stat.load;
            if (a->global.set & SETUPDATE)
                sp->update = a->stat.up;
            if (a->global.set & SETUSERS)
                sp->stat.users = a->stat.users;
        }
        break;
    case DEF | NEW:
        a->set &= ~sp->flags;
    ...
    if (a->set & SETACCESS)
        sp->access = dupstring(sp->access, a->access);
    if (a->set & SETBYPASS)
    ...
    if (!sp->update && !(op & GET)
        || (a->set
            & (SETBIAS | SETCPU | SETIDLE | SETIGNORE | SETRATING | SETSCALE))
    ...
    if (csstat(sp->name, &sp->stat)) {
        ...
        if (sp->stat.up < 0) {
            ...
            if (n > PCT(X_RANK, C_RANK)) {
                if (n >= 2 * X_RANK)
                    ...
                    n = PCT(X_RANK, C_RANK)
                        + (n - PCT(X_RANK, C_RANK))
                          / (2 * X_RANK - PCT(X_RANK, C_RANK) - 1)
                          * (X_RANK - PCT(X_RANK, C_RANK) - 1);
            }
        }
        ...
        if (sp->stat.up < 0)
            ...
            if (sp->fd > 0 && (2 * sp->stat.up) < (cs.time - sp->start))
                shellclose(sp, -1);
            if (!sp->fd && !IDLE(sp->stat.idle, sp->idle))
                sp->update += sp->idle - sp->stat.idle;
        }
        message((-4,
                 "%s: %s=%s idle=%s load=%s users=%d rank=%s",
                 sp->name,
                 sp->stat.up < 0 ? "down" : "up",
                 fmtelapsed(sp->stat.up < 0 ? -sp->stat.up : sp->stat.up, 1),
                 fmtelapsed(sp->stat.idle, 1),
                 fmtfloat(sp->stat.load),
                 sp->stat.users,
                 fmtfloat(sp->rank)));
    }
}
