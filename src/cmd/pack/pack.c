...
= "[-?\n@(#)$Id: pack (AT&T Research) 2003-04-28 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?\bpack\b attempts to store the specified files in a "
  "compressed "
  ...
  "[+?\bpack\b uses Huffman (minimum redundancy) codes on a byte-by-byte "
  "basis. "
  ...
  "}"
  "[f:force?Pack the file even if the packed size is larger than the "
  "original.]"
        ...
        else
        {
            ...
            if (!(hp = huffinit(fpin, ( Sfoff_t )-1)))
                ...
                error(2, "%s: write error", infile);
                     ...
                     && block(huffisize(hp)) <= block(huffosize(hp) + dsize))
...
static void
vprint(Huff_t *hp, int dsize)
{
    sfprintf(
    sfstdout, "	from %lld to %lld bytes\n", huffisize(hp), huffosize(hp));
    sfprintf(
    sfstdout, "	Huffman tree has %d levels below root\n", hp->maxlev);
