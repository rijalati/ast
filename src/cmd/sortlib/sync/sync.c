...
= "[-1lp0s5P?\n@(#)$Id: dfsort (AT&T Research) 2013-02-21 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?The \bsync\b \bsort\b(1) discipline applies an IBM "
  "\bDFSORT\b"
  ...
  "[c:control?Specifies the control file path name. Control file details may "
  "be"
...
#define CALLOUT(s, f, r, d)                                                  \
    ((s)->intercept ? (*(s)->intercept)(f, r, d, &(s)->exitstate)            \
                    : (*f)(r, d, &(s)->exitstate))
...
dfsort(Rs_t *rs, int op, Void_t *data, Void_t *arg, Rsdisc_t *disc)
{
    ...
    case RS_SUMMARY:
        ...
        if (!ss->summaryexit)
            c = RS_ACCEPT;
        else if ((c = CALLOUT(ss, ss->summaryexit, rp, rp->equal)) < 0
                 || c == RS_DELETE)
            return c;
        if (ss->sum)
...
rs_disc(Rskey_t *key, const char *options)
{
    ...
    if (ss->size)
        ...
        for (v = key->input; s = *v; v++)
            if ((t = strrchr(s, '%')) && !strchr(t, '/')) {
                ...
                p = s;
                if (RECTYPE(f) != REC_variable
                    || RECTYPE(ss->format) != REC_variable
                    || REC_V_SIZE(ss->format) < REC_V_SIZE(f))
                    ss->format = f;
                if (!ss->suffix && *u == '.')
                    ss->suffix = u;
            }
    ...
    if (ss->initexit)
        events |= RS_OPEN;
    if (ss->expr || ss->copy || ss->readexit || ss->in || ss->skip
        || ss->stop)
        events |= RS_READ;
