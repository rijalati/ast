        ...
        if (op < 0) {
            ...
            if (op > 0) {
                ...
                if (n > w) {
                    ...
                    memcpy(ext->buf + (a - r->data) + n,
                           a + w,
                           r->datalen - (w + (a - r->data)));
                    s = ext->buf + (s - r->data);
                    a = ext->buf + (a - r->data);
                ...
                for (;;) {
                    ...
                    for (;;) {
                        ...
                        case 0:
                            ...
                            default:
                                error(2,
                                      "%s: invalid summary field operation",
                                      s - 1);
                                goto drop;
                            }
                            while (isalnum(*s))
                        ...
                        default:
                            error(
                            2, "%s: invalid summary field specification", s);
                            goto drop;
                        }
                        while (*s == ':' || isspace(*s))
                            s++;
                    ...
                    if (!sum->type)
                        sum->type = cxattr(
                        dss->cx, "integer", NiL, &sum->format, dss->cx->disc);
                    for (prv = 0, cur = state->sum; cur;
                         cur = (prv = cur)->next)
                        if (sum->beg.field < cur->beg.field
                            || sum->beg.field == cur->beg.field
                               && sum->end.field < cur->end.field)
                            break;
                    if (prv)
                        prv->next = sum;
                    else
    ...
    for (sum = state->sum; sum; sum = sum->next)
        if (sum->format.code) {
            ...
            if (sum->format.code) {
                ...
                sum->pam
                = ccmap(CCOUT(sum->format.code), CCIN(sum->format.code));
            }
        }
