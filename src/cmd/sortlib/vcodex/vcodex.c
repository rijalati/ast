        ...
        if (state->input.use >= 0) {
            if ((i = push(( Sfio_t * )data,
                ...
                if (!*state->input.suffix && ZIPSUFFIX(arg, s))
                    strncopy(
                    state->input.suffix, s, sizeof(state->input.suffix));
            }
        }
        break;
    case RS_TEMP_WRITE:
        if (state->temporary.use > 0
            ...
            if (push(( Sfio_t * )data, NiL, state->temporary.trans, VC_ENCODE)
                ...
                tempid(state, data),
                state->temporary.trans);
                return -1;
            }
            if (state->verbose)
                error(0,
                      "sort vcodex encode temporary-%d (%s)",
                      tempid(state, data),
                      state->temporary.trans);
            return 1;
        }
...
rs_disc(Rskey_t *key, const char *options)
{
    ...
    state->disc.events
    = RS_FILE_WRITE | RS_FILE_READ | RS_TEMP_WRITE | RS_TEMP_READ;
    return &state->disc;
drop:
    free(state);
    return 0;
