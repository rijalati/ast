...
= "[-1lp0s5P?\n@(#)$Id: glean (AT&T Research) 2007-04-20 $\n]" USAGE_LICENSE
  ...
  "[+EXAMPLES]"
  "{"
  "[+sort -t, -k1,1 -k2,2n -lglean,flush,min='\"3,3n\"',min='\"4,4\"' "
  "old.dat -?Sorts "
...
glean(Rs_t *rs, int op, Void_t *data, Void_t *arg, Rsdisc_t *disc)
{
    ...
    case RS_READ:
        ...
        message(
        (-2,
         "glean record p=%p %I*u/%I*u key='%-.*s' r:%d:%-.*s: '%-.*s'",
         p,
         sizeof(p->count),
         p->count,
         sizeof(state->total),
        ...
        for (f = state->field; f; f = f->next)
        {
            ...
            else
            {
                ...
                (-1,
                 ...
                 f->mm,
                 p->lim[f->index].len,
                 fmtdata(p->lim[f->index].data, p->lim[f->index].len)));
