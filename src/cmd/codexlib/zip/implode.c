...
/* You can do whatever you like with this source file, though I would
   ...
   32k. c3   10 Apr 92  M. Adler        added a little memory tracking if
   DEBUG c4   11 Apr 92  M. Adler        added NOMEMCPY do kill use of
   memcpy() c5   21 Apr 92  M. Adler        added the WSIZE #define to allow
   reducing the 32K window size for specialized applications. c6   31 May 92
   M. Adler        added typecasts to eliminate some warnings c7   27 Jun 92
   G. Roelofs      added more typecasts. c8   17 Oct 92  G. Roelofs changed
   ULONG/UWORD/byte to ulg/ush/uch. c9   19 Jul 93  J. Bush         added more
   typecasts (to return values); made l[256] array static for Amiga. c10   8
   Oct 93  G. Roelofs      added used_csize for diagnostics; added buf and
   unshrink arguments to flush(); undef'd various macros at end for Turbo C;
    ...
    c13  25 Aug 94  M. Adler        fixed distance-length comment (orig c9
   fix) c14  22 Nov 95  S. Maxwell      removed unnecessary "static" on auto
   array c15   6 Jul 96  W. Haidinger    added ulg typecasts to flush() calls
 */
...
static ssize_t
explode_lit8(State_t *state, char *buff, size_t size)
...
static ssize_t
explode_lit4(State_t *state, char *buff, size_t size)
...
static ssize_t
explode_nolit8(State_t *state, char *buff, size_t size)
...
static ssize_t
explode_nolit4(State_t *state, char *buff, size_t size)
...
implode_init(Codex_t *p)
{
    ...
    if (state->method & EXPLODE_LIT)
    {
        ...
        if (huff(
            state->l, 256, 256, NULL, NULL, &state->tb, &state->bb, state->vm)
            != 0)
            return -1;
        ...
        if (huff(
            state->l, 64, 0, cplen3, extra, &state->tl, &state->bl, state->vm)
            != 0)
            return -1;
    ...
    else
    {
        ...
        if (huff(
            state->l, 64, 0, cplen2, extra, &state->tl, &state->bl, state->vm)
            != 0)
            return -1;
