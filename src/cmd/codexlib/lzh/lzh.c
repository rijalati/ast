    ...
    if (total) {
        if (state->codex->disc->errorf)
            (*state->codex->disc->errorf)(NiL,
                                          state->codex->disc,
                                          2,
                                          "%s: invalid compression table",
                                          state->codex->meth->name);
        return -1;
    }
...
read_tree_c(State_t *state)
{
    ...
    return make_table(
    state, N1, state->c_len, 12, state->c_table, elementsof(state->c_table));
}
...
static ui1 table_fix[16] = { 3, 0x01, 0x04, 0x0c, 0x18, 0x30 };
...
static ui1 table_s0[16] = { 2, 0x01, 0x01, 0x03, 0x06, 0x0D, 0x1F, 0x4E };
...
decode_c_s1(State_t *state)
{
    ...
    if (!state->blocksize
        && (!(state->blocksize = GETBITS(state, 16))
            || get_p_len(state, NT, TBIT, 3) || get_c_len(state)
            || get_p_len(state, state->snp, state->pbit, -1)))
        return -1;
    state->blocksize--;
    b = PEEKBITS(state, 16);
    j = state->c_table[b >> 4];
...
static Decode_f decoders[] = {
    init_c_d0, decode_c_d0, init_c_d1, decode_c_d0, init_c_s0, decode_c_s0,
    init_c_s1, decode_c_s1, init_c_s2, decode_c_s2, init_c_s3, decode_c_s3,

    init_p_d0, decode_p_d0, init_p_d0, decode_p_d0, init_p_s0, decode_p_s0,
    init_p_s1, decode_p_s1, init_p_s2, decode_p_s2, init_p_s3, decode_p_s3,
};
    ...
    if (n == 'd')
        ...
        if (disc->errorf)
            (*disc->errorf)(NiL,
                            disc,
                            2,
                            "%s: d[ynamic] or s[tatic] decoder name expected",
                            s - 1);
        return -1;
    }
