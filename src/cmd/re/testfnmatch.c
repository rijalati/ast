...
help(void)
{
    ...
    H("DESCRIPTION\n");
    H(
    "  testfnmatch reads fnmatch(3) test specifications, one per line, "
    "from\n");
    H(
    "  the standard input and writes one output line for each failed test. "
    "A\n");
    H(
    "  summary line is written after all tests are done. Unsupported "
    "features\n");
    H(
    "  are noted before the first test, and tests requiring these "
    "features\n");
    ...
    H(
    "  specification is a global control line. One or more of [SK] may be\n");
    H(
    "  specified; the test will be repeated for each mode. Specifications\n");
    ...
    H("\n");
    H(
    "  Field 4: the test outcome. This is either OK, NOMATCH, or a "
    "testre(1T)\n");
