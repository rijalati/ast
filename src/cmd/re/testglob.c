...
help(void)
{
    ...
    H("DESCRIPTION\n");
    H(
    "  testglob reads glob(3) test specifications, one per line, from the\n");
    H(
    ...
    "these\n");
    H(
    "  features are silently ignored. Each test is repeated with "
    "GLOB_LIST,\n");
    ...
    H(
    "  -n	do not repeat tests with GLOB_LIST, GLOB_STACK, or "
    ...
    H("\n");
    H(
    "  Field 1: the glob(3) flags to apply, one character per "
    "GLOB_feature\n");
    H(
    "  flag. The test is skipped if GLOB_feature is not supported by the\n");
                ...
                if (i > cwd) {
                    ...
                    if (access(path, F_OK)) {
                        if (k) {
                            ...
                            if (close(creat(
                                path, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)))
