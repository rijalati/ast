...
help(void)
{
    ...
    H("DESCRIPTION\n");
    H(
    "  testmatch reads strgrpmatch(3) test specifications, one per line, "
    "from\n");
    H(
    "  the standard input and writes one output line for each failed test. "
    "A\n");
    H(
    "  summary line is written after all tests are done. Each successful\n");
    H(
    "  test is run again with no subexpression pointer array. Unsupported\n");
    H(
    "  features are noted before the first test, and tests requiring "
    "these\n");
    H("  features are silently ignored.\n");
    H("\n");
#if OLD
    H(
    "  This version tests the legacy ad-hoc implementation with roots in "
    "the\n");
    ...
    H(
    "  STR_feature flag. The test is skipped if STR_feature is not "
    "supported\n");
    H(
    "  by the implementation. If the first character is not [SK] then the\n");
    H("  specification is a global control line. Note that no distinction\n");
    H(
    "  is made between SRE and KRE tests. STR_MAXIMAL is set by default.\n");
    ...
    H("\n");
    H(
    "  Field 2: the strgrpmatch expression pattern; SAME uses the pattern\n");
    H("    from the previous specification.\n");
    H("\n");
    H("  Field 3: the string to match.\n");
    H("\n");
    H(
    "  Field 4: the test outcome. This is either OK, NOMATCH, BADPAT, or\n");
    H(
    "    the match array, a list of (m,n) entries with m and n being "
    "first\n");
    H(
    "    and last+1 positions in the field 3 string, or NULL if "
    "subexpression\n");
...
#if !defined(STR_ICASE) && !defined(STR_LEFT) && !defined(STR_MAXIMAL)       \
&& !defined(STR_RIGHT)
...
#if STR_ICASE == NOTEST && STR_LEFT == NOTEST && STR_MAXIMAL == NOTEST       \
&& STR_RIGHT == NOTEST
#    define NOT_SUPPORTED 1
#endif
    ...
    *p = 0;
    ...
    while (state.file = *argv++) {
        ...
        if (s = state.file) {
            ...
            if (subunitlen == strlen(unit)
                && !memcmp(subunit, unit, subunitlen))
                subunit = 0;
            else
            ...
            for (p = spec; *p; p++) {
                ...
                case 'C':
                    ...
                    if (!(skip & level))
                        ...
                        = note(level,
                               skip,
                               "locales not supported by old strmatch()");
#else
                    if (locale)
    ...
    skip:
        ...
        if (state.ignore.count)
            printf(", %d ignored mismatche%s",
                   state.ignore.count,
                   state.ignore.count == 1 ? "" : "s");
        if (state.warnings)
            printf(
            ", %d warning%s", state.warnings, state.warnings == 1 ? "" : "s");
        ...
        if (state.signals)
            printf(
            ", %d signal%s", state.signals, state.signals == 1 ? "" : "s");
