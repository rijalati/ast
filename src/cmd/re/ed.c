...
helpwrite(int fd, const void *buf, size_t len)
{
    ...
    NoP(fd);
    n = ed.help ? sfwrite(sfstderr, buf, len)
                : ed.verbose ? sfputr(ed.msg, "?", '\n') : 0;
...
lineget(off_t off)
{
    ...
    if (!(s = sfgetr(ed.tmp, 0, 0)))
        error(ERROR_SYSTEM | 2,
              "temp file read error at offset %I*d",
              sizeof(off),
              off);
    return s;
}
...
shell(void)
{
    ...
    if (ed.given)
    {
        ...
        if (!(ed.iop = sfopen(NiL, ed.tmpfile, "w")))
            error(
            ERROR_SYSTEM | 2, "%s: cannot create temp file", ed.tmpfile);
...
commands(void)
{
    ...
    for (;;)
    {
        ...
        case 'S':
            ...
            s = strchr(usage, '\n') + 5;
            sfprintf(ed.msg,
                     "file=\"%s\"%s%s%s prompt=\"%s\" tmp=%lu%s event=%lu "
                     "version=\"%-.*s\"\n",
                     sfstrbase(ed.buffer.file),
                     ed.modified ? " modified" : "",
                     ed.help ? " help" : "",
                     ed.verbose ? " verbose" : "",
                     sfstrbase(ed.buffer.prompt),
                     ed.tmpoff,
                     ed.tmpoff > BLOCK_TMP ? "[file]" : "",
                     ed.event,
                     strchr(s, '\n') - s,
                     s);
            continue;
