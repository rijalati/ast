...
static const char *export[] = { CO_ENV_HOST, CO_ENV_TYPE };

static const char *queuedefs[] = { "a.4j1n2u", "b.2j2n90w2u", "c.h8j2u60w" };
...
queue(State_t *state, char *s)
{
    ...
    if (*s && *s != '#')
    {
        ...
        if (!state->queue)
            state->queue = que;
        if (!que->owner
            && !(que->owner = dtopen(&state->table.owner.discipline, Dtset)))
            error(ERROR_SYSTEM | 3,
                  "out of space [queue %s owner hash]",
                  que->name);
        que->allow = -1;
        que->specific = 0;
        while ((s = t) && *s)
...
submit(State_t *state, Job_t *job)
{
    ...
    error(0,
          ...
          job->queue->name,
          fmttime(AT_TIME_FORMAT, job->start),
          job->label);
}
            ...
            if (job->start <= cs.time)
            {
                        ...
                        || job->owner->running < state->peruser))
                {
                    if (job->pid = execute(state, job))
                    {
                        ...
                        message(
                        (-2, "exec job=%s pid=%lu", job->name, job->pid));
                        error(0,
                              "%s %s %lu exec \"%s\"",
                              job->name,
                              fmtuid(job->owner->user->uid),
...
listqueue(Dt_t *dt, void *object, void *handle)
{
    ...
    error(ERROR_OUTPUT | 0,
          con->fd,
          "%-3s %5lu %3d %3d %3d %3d %3d %2d.%02d %5.5s%s",
          que->name,
          que->total,
          que->pending,
          que->running,
...
command(State_t *state, Connection_t *con, char *s, int n, char *data)
{
    ...
    case AT_INFO:
        error(ERROR_OUTPUT | 0,
              con->fd,
              "at service daemon pid %ld user %s",
              state->con[0].id.pid,
              fmtuid(state->admin[0]));
        error(ERROR_OUTPUT | 0,
              con->fd,
              "QUE TOTAL SUB RUN MAX USR PRI  LOAD  WAIT ACCESS");
        visit.state = state;
        visit.con = con;
        if (que)
                ...
                for (t = b + skip; *t; t++)
                    if (isalnum(*t))
                    {
                        ...
                        while (isalnum(*++t))
                            ;
                        c = t - u;
                        if (c == 3 && strneq(u, "for", 3)
                            || c == 2 && strneq(u, "if", 2)
                            || c == 5 && strneq(u, "while", 5))
                            continue;
                        break;
                    }
                m = 0;
...
noexec:
    if (sp)
        sfclose(sp);
    error(ERROR_SYSTEM | ERROR_OUTPUT | 2,
          con->fd,
          "%s: cannot save action",
          job->name);
    drop(state, job);
    return -1;
noqueue:
...
commit(void)
{
    ...
    if (now >= rollover)
    {
        ...
        if (rename(AT_LOG_FILE, buf))
            error(ERROR_SYSTEM | AT_STRICT,
                  "%s: cannot rename log file to %s",
                  AT_LOG_FILE,
                  buf);
        t = rollover;
        tm = tmmake(&t);
    ...
    if ((fd = open(AT_LOG_FILE,
        ...
        < 0
        || fd != 2 && (dup2(fd, 2) != 2 || close(fd)))
        error(ERROR_SYSTEM | AT_STRICT,
              "%s: cannot append to log file",
              AT_LOG_FILE);
    if (rolled)
        error(0, "log file rollover");
    commiting--;
...
init(const char *path)
{
    ...
    *ap++ = 0;
    s = state->buf;
    if (!pathpath(
        AT_JOB_DIR, "", PATH_ABSOLUTE | PATH_EXECUTE, s, state->bufsiz)
        || lstat(s, &ds) || !AT_DIR_OK(&ds))
        error(ERROR_SYSTEM | 3, "%s: job directory not found", AT_JOB_DIR);
    if (ds.st_uid != state->admin[0])
        error(ERROR_SYSTEM | 3,
              "%s: job directory uid %d != effective uid %d",
              s,
              ds.st_uid,
              state->admin[0]);
    ...
    *b++ = '/';
    sfsprintf(
    b, state->bufsiz - (b - s), "%s", csname(state->css->state, 0L));
    if (lstat(s, &hs)
        && (mkdir(s, AT_DIR_MODE) || chmod(s, AT_DIR_MODE) || lstat(s, &hs)))
        error(ERROR_SYSTEM | 3, "%s: cannot create job host directory", s);
    if (!AT_DIR_OK(&hs) || ds.st_uid != hs.st_uid || chdir(s))
        error(ERROR_SYSTEM | 3,
              "%s: invalid job host directory %s [dir.uid=%d host.uid=%d]",
              s,
              fmtmode(hs.st_mode, 0),
              ds.st_uid,
              hs.st_uid);
    ...
    if (hs.st_uid != state->admin[0])
        error(AT_STRICT,
              "%s: directory owner %s does not match daemon %s",
              s,
              fmtuid(hs.st_uid),
              fmtuid(state->admin[0]));
    ...
    if (!S_ISREG(xs.st_mode))
        error(3,
              "%s: invalid mode %s -- regular file expected",
              s,
              fmtmode(xs.st_mode, 0));
    if ((xs.st_mode & (S_IXUSR | S_IXGRP | S_IWOTH | S_IXOTH))
        != (S_IXUSR | S_IXGRP | S_IXOTH))
        error(3, "%s: invalid mode %s", s, fmtmode(xs.st_mode, 0));
    if (!(xs.st_mode & S_ISUID) && geteuid() != 0 && geteuid() != xs.st_uid)
        error(
        3, "%s: invalid euid %d -- %d expected", s, geteuid(), xs.st_uid);
#if 0
	if (!AT_EXEC_OK(&ds, &xs))
    ...
    *ap = xs.st_uid;
    ...
    if (!(state->table.job.handle
          = dtopen(&state->table.job.discipline, Dtoset)))
    ...
    if (!(state->table.pid.handle
          = dtopen(&state->table.pid.discipline, Dtset)))
        error(ERROR_SYSTEM | 3, "out of space [pid table]");
    state->table.queue.discipline.key = offsetof(Queue_t, name);
    if (!(state->table.queue.handle
          = dtopen(&state->table.queue.discipline, Dtoset)))
