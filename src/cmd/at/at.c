...
= "[-?\n@(#)$Id: at (AT&T Research) 2000-05-09 $\n]" USAGE_LICENSE
  ...
  "}"
  "[r:remove?Remove command named by the \ajob\a ... operands from the "
  "queue.]"
  ...
  "[L:log?Write the log file path name on the standard output and exit. The "
  "log"
...
main(int argc, char **argv)
{
    ...
    default:
        ...
        if (!fs3d(FS3D_TEST)
            || !(s = pathpath(
                 "3d", NiL, PATH_ABSOLUTE | PATH_EXECUTE, buf, sizeof(buf))))
            s = pathshell();
        if (op & MAIL)
            sfputc(sp, AT_MAIL);
        skip = sfstrtell(sp) + 1;
    ...
    else
    {
        if (op & JOB)
        {
            ...
            if (every)
                sfprintf(
                sfstdout, " and repeat every +%d %s", interval, every);
            sfprintf(sfstdout, "\n");
        }
        sfputr(sfstdout, s, -1);
    }
