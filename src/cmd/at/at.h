...
#        define AT_DIR_MODE                                                  \
            (S_IRUSR | S_IWUSR | S_IXUSR | S_IXGRP | S_IXOTH | S_IRGRP       \
             | S_IROTH)
...
#            define AT_EXEC_OK(d, x)                                         \
                (S_ISREG((x)->st_mode)                                       \
                 && ((x)->st_mode & (S_IXUSR | S_IXGRP | S_IWOTH | S_IXOTH)) \
                    == (S_IXUSR | S_IXGRP | S_IXOTH))
#            define AT_DIR_OK(d)                                             \
                (S_ISDIR((d)->st_mode)                                       \
                 && (((d)->st_mode) & S_IPERM) == AT_DIR_MODE)
#            define AT_JOB_OK(d, j)                                          \
                ((d)->st_uid == (j)->st_uid && S_ISREG((j)->st_mode)         \
                 && (((j)->st_mode) & S_IPERM) == AT_JOB_MODE)
#            define AT_OLD_OK(d, j)                                          \
                ((d)->st_uid == (j)->st_uid && S_ISREG((j)->st_mode)         \
                 && (((j)->st_mode)                                          \
                     & (S_IPERM & ~(AT_JOB_MODE ^ AT_RUN_MODE)))             \
                    == AT_JOB_MODE)
...
#        define AT_OLD_OK(d, j)                                              \
            ((d)->st_uid == (j)->st_uid && S_ISREG((j)->st_mode)             \
             && (((j)->st_mode) & (S_IPERM & ~(AT_JOB_MODE ^ AT_RUN_MODE)))  \
                == AT_JOB_MODE)
#        define AT_RUN_OK(d, j)                                              \
            ((d)->st_uid == (j)->st_uid && S_ISREG((j)->st_mode)             \
             && (((j)->st_mode) & S_IPERM) == AT_RUN_MODE                    \
