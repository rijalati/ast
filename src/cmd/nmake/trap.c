...
struct Alarms_s;
typedef struct Alarms_s Alarms_t;
...
{
    SIGHUP,  SIGINT, SIGALRM, SIGTERM, SIGPIPE,
#ifdef SIGILL
    SIGILL,
#endif
#ifdef SIGIOT
                ...
                if (!state.compileonly)
                    ...
                    case SIGALRM:
                        ...
                        while ((a = trap.alarms) && a->time <= t) {
                            ...
                            maketop(
                            r,
                            (P_dontcare | P_force | P_ignore | P_repeat)
                            | ((r->property & P_make) ? 0 : P_foreground),
                            s);
                        }
                        setwakeup();
                        continue;
                    default:
                        ...
                        if ((r = catrule(external.interrupt, ".", s, 0))
                            ...
                            message(
                            (-1,
                             "trap %s handler %s status TERMINATE return %s",
                             s,
                             r->name,
                             w));
                        }
