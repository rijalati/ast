...
static Option_t options[] = /* option table			*/
{
    ...
    { "believe",
      ...
      " files on all view levels. The top view is level 0.",
      "level:=0" },
    { "compatibility",
      OPT_compatibility,
    ...
    { "include",
      ...
      0,
      0,
      "Add \adirectory\a to the makefile search list.",
      "directory" },
    { "intermediate",
      OPT_intermediate,
    ...
    { "tolerance",
      ...
      " of more that 5 seconds soon becomes intolerable.",
      "seconds" },
    { "touch",
      OPT_touch,
      ( char * )&state.touch,
...
setop(Option_t *op, int n, char *s, int type)
{
    ...
    case OPT(OPT_corrupt):
        if (!s)
            s = "-";
        if ((*s == *(state.corrupt = "accept") || *s == '-')
            && (!*(s + 1) || !strcmp(s, state.corrupt)))
            ;
        else if (*s == *(state.corrupt = "error")
                 && (!*(s + 1) || !strcmp(s, state.corrupt)))
            state.corrupt = 0;
    ...
    case OPT(OPT_include):
        addprereq(catrule(internal.source->name, external.source, NiL, 1),
                  makerule(s),
                  PREREQ_APPEND);
        if (!(op->flags & OPT_READONLY))
            break;
    ...
    case OPT(OPT_preprocess):
        if (!state.preprocess) {
            ...
            if (!state.compatibility)
                error(
                1,
                "makefile preprocessing is obsolete -- use make statements");
        }
        return;
    ...
    case OPT(OPT_mam):
        ...
        if (s) {
            ...
            if (!*s || streq(s, "-") || streq(s, "/dev/fd/1")
                ...
                error(
                ERROR_SYSTEM | 3, "%s: cannot write mam output file", s);
            ...
            set_insert:
                ...
                if ((nop->flags & (Ob | On | Os)) != (n & (Ob | On | Os)))
                    error(1,
                          ...
                          (nop->flags & Ob)
                          ? "boolean"
                          : (nop->flags & On) ? "numeric" : "string valued");
            }
    ...
    case OPT(OPT_targetprefix):
        ...
        if (state.targetprefix = s)
            for (;;) {
                ...
                case '\\':
                    error(3,
                          "--%s: %s: value must not contain metarule or "
                          "shell pattern characters",
                          op->name,
                          state.targetprefix);
                    break;
                default:
    ...
    case OPT(OPT_tolerance):
        if ((state.tolerance = n) > 60)
            error(1,
                  "the time comparison tolerance should probably be less "
                  "than a minute");
        return;
 ...
 *	'+' only the value is generated, "" if Os option not set, 0 for Ob|Oi not
 *set
...
getop(Sfio_t *sp, char *name, int setting)
{
    ...
    if ((op = getoption(name)) && !(flag = 0)
        || name[0] == 'n' && name[1] == 'o' && (op = getoption(&name[2]))
           && (flag = Oi)
        || name[0] && !name[1] && (op = optflag(name[0])) && (flag = Of))
...
optset(int i, char *v, Sfio_t *scope)
{
    ...
    if (i > 0) {
        ...
        n = (op->flags & On)
            ? (opt_info.arg ? opt_info.num : 0)
            : (op->flags & Ob) ? (opt_info.num != 0)
                               : (opt_info.num != 0) == !(op->flags & Oi);
        if (*opt_info.option == '+')
            n = !n;
...
punt(int old)
{
    ...
    if (old) {
        ...
        for (av = state.argv; *av; putptr(vec, *av++))
            ;
        sfprintf(
        internal.tmp, "--%s=%d", optflag(OPT_reread)->name, state.reread + 1);
        if (state.preprocess)
...
{
    return getoption(name) != 0
           || name[0] == 'n' && name[1] == 'o' && getoption(&name[2]) != 0;
}
