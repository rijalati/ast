...
statefile(void)
{
    ...
    if (!state.statefile && state.makefile) {
        ...
        if (!state.writestate || streq(state.writestate, "-")
            || !stat(state.writestate, &st) && S_ISDIR(st.st_mode)
               && (dir = state.writestate))
          ...
          Rule_t **pv)
{
    ...
    if (state.compile < COMPILED) {
        ...
        if (state.test & 0x00000200)
            error(2,
                  "STATEVIEW %d %s state file load delayed until after "
                  "makefile read",
                  view,
                  name);
#endif
    ...
    if (!(state.view[view].flags & BIND_LOADED)) {
        if (view < state.readstate) {
            ...
            if (fp = sfopen(NiL, file, "br")) {
                ...
                else if (state.corrupt && *state.corrupt == 'a'
    ...
    if (name) {
        ...
        if (state.test & 0x00000200)
            error(2,
                  "STATEVIEW %s [%s] test %d [%s] -> %s [%s]",
                  name,
                  s ? timestr(s->time) : "no rule",
                  view,
                  timestr(r->time),
                  v ? v->name : null,
        ...
        if (v) {
            ...
            if (s
                && (op == RULE
                    && (s->event >= v->event && s->event
                        || !v->time && (v->property & P_force))
                    || op == PREREQS && s->time >= v->time))
                return s;
...
again:
    ...
    if (state.maxview && state.readstate && state.makefile && !nobind) {
        ...
        k = 0;
        if (!tstbit(*b,
                    i = (r->property & (P_statevar | P_use | P_virtual))
                        && state.targetview > 0
                        ? state.targetview
                        : r->view)
            ...
            if ((r->property & P_statevar) && r->time == OLDTIME) {
                ...
                if (state.test & 0x00000200)
                    error(2,
                          "STATERULE %s search %d..%d targetview=%d",
                          nam,
                          j,
                          k,
                          state.targetview);
...
badlock(char *file, int view, Time_t date)
{
    ...
    if (d > 24 * 60 * 60)
        ...
        error(1,
              "another make has been running on %s in %s for the past %s",
              state.makefile,
              state.view[view].path,
              fmtelapsed(d, 1));
    else
        error(1,
              "another make is running on %s in %s",
              state.makefile,
              state.view[view].path);
        ...
        if (!state.ignorelock) {
            ...
            for (fd = 1; fd <= state.maxview; fd++) {
                ...
                if (!stat(sfstruse(internal.nam), &st) && st.st_uid != uid)
                    badlock(
                    sfstrbase(internal.nam), fd, LOCKTIME(&st, lockmtime));
            }
        }
        locktime = 0;
        for (;;) {
    ...
    if (state.makefile) {
        ...
        if (state.readstate && (fp = sfopen(NiL, file = statefile(), "br"))) {
            ...
            if (load(fp, file, 0, 10) > 0)
                ...
                error(3,
                      "use -%c%c to accept current state or -%c to remake",
                      OPT(OPT_accept),
                      OPT(OPT_readstate),
                      OPT(OPT_readstate));
...
bindstate(Rule_t *r, char *val)
{
...
#endif
    if (state.maxview && (r->property & P_statevar)
        && (s = staterule(VAR, r, NiL, 0)))
        r = s;
    ...
    if (r->property & P_statevar) {
        ...
        message((-2,
                 "checking state variable %s value `%s'",
                 r->name,
        ...
        if (!r->time || r->statedata && !streq(r->statedata, val)
            ...
            if (!r->view && !(r->property & P_accept)) {
                if (r->time)
                    ...
                    reason((
                    1, "state variable %s initialized to `%s'", r->name, val));
            }
            state.savestate = 1;
            if (r->statedata != val)
                r->statedata = strdup(val);
...
checkcurrent(Rule_t *r, Stat_t *st)
{
    ...
    if (state.test & 0x00000100)
        error(2,
              "statetime(%s): dir=%s n=%d time=[%s]",
              r->name,
              s,
              n,
              timestr(n ? NOTIME : tmxgetmtime(st)));
#endif
...
statetime(Rule_t *r, int sync)
{
        ...
        if (st.st_nlink <= 1 && !S_ISDIR(st.st_mode)
            ...
            if (!localsync && !state.override && r->time && r->time != OLDTIME
                ...
                if (((n = (tmxsec(r->time) - ( unsigned long )st.st_mtime - 1))
                    ...
                    if (a > 1)
                        error(
                        state.regress ? -1 : 1,
                        "%s file system time %s local time by at least %s",
                        r->name,
                        n > 0 ? "lags" : "leads",
                        fmtelapsed(a, 1));
            ...
            if (localsync > 0) {
                ...
                for (;;) {
                    ...
                    error(state.regress ? -1 : 1,
                          "the utime(2) or utimes(2) system call is botched "
                          "for the filesystem containing %s (the current "
                          "time is adjusted by %lu seconds) -- the state may "
                          "be out of sync",
                          r->name,
                          tmxsec(localskew));
                    ...
                    if (localskew > tmxsns(1, 0)) {
                        ...
                        t = tmxsns(tmxsec(t), 0);
                        if (!tmxtouch(r->name, TMX_NOTIME, t, TMX_NOTIME, 0)
                            && !stat(r->name, &st) && tmxgetmtime(&st) == t)
    ...
    if (!skip
        ...
        s->time = zerostate
                  ? 0
                  : (r->property & P_virtual) ? r->time : tmxgetmtime(&st);
        s->event = CURTIME;
        state.savestate = 1;
    }
    return s->time;
