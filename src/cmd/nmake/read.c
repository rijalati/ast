...
readrules(void)
{
    ...
    else if (*s)
    {
        ...
        readfile(sfstruse(tmp),
                 COMP_BASE | (state.explicitrules ? COMP_RULES : 0),
                 NiL);
        ...
        else if (fp = sfopen(NiL, s = x->name, "br"))
        {
            ...
            if (state.global)
                error(1,
                      ...
                      r->name,
                      state.forceread
                      ? " -- should be compiled before local makefiles"
                      : null);
            else if (state.writeobject)
                error(
                state.exec || state.mam.out ? -1 : 1, "%s: recompiling", s);
        }
    }
    ...
    if (!state.global)
    {
        ...
        if (s)
        {
            ...
            if (!(sp = fapply(internal.internal,
                ...
                error(
                3, "%s: error in makefile converter \"%s\"", r->name, s);
            free(t);
            preprocess = -1;
        }
        if (needrules)
...
readfile(char *file, int type, char *filter)
{
        ...
        && (r->time || strneq(r->name, "/dev/", 5) && !rstat(r->name, &st, 0)))
    {
        ...
        r->dynamic |= D_scanned;
        file = r->name;
        if (rfp = filter ? fapply(internal.internal,
                                  null,
                                  file,
                                  filter,
                                  CO_ALWAYS | CO_LOCAL | CO_URGENT)
    ...
    if (!(type & COMP_DONTCARE))
        error((type & COMP_INCLUDE) ? 2 : 3,
              ...
              : (type & COMP_GLOBAL)
                ? " global rules"
                : (type & COMP_BASE) ? " base rules" : null);
