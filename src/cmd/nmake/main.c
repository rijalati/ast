...
intercept(Sfio_t *sp, int level, int flags)
{
    ...
    NoP(sp);
    NoP(flags);
    if ((state.mam.level = level) > 0 && !state.hold && (r = internal.error)
...
main(int argc, char **argv)
{
    ...
    settypes("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_",
             C_ID1 | C_ID2 | C_VARIABLE1 | C_VARIABLE2);
    ...
    setvar(
    external.lib,
    strdup((s = pathpath(t, argv[0], PATH_EXECUTE, buf, SF_BUFSIZE)) ? s : t),
    V_import);
            ...
            if (!state.mam.statix || *state.mam.label)
            {
                ...
                if (!state.mam.root || streq(state.mam.root, internal.pwd))
                    ...
                    sfprintf(state.mam.out,
                             ...
                             state.mam.label,
                             state.mam.root,
                             mamname(makerule(internal.pwd)));
                buf = sfstrbase(tmp);
                if (state.fsview
                    && !mount(NiL,
                              buf,
                              FS3D_GET | FS3D_ALL | FS3D_SIZE(sfstrsize(tmp)),
                              NiL))
                    sfprintf(
                    state.mam.out, "%sinfo view %s\n", state.mam.label, buf);
            }
        }
    }
    ...
    if (!state.makefile)
    {
        ...
        if (!s)
        {
            ...
            if (readfile("-", COMP_FILE, NiL))
                optcheck(1);
            if (*(s = sfstruse(imp)))
                error(state.errorid ? 1 : 3,
                      "a makefile must be specified when %s omitted",
                      s);
            else
    ...
    if (state.believe)
    {
        if (!state.maxview)
            ...
            error(3,
                  "%s: option currently works in 2d only",
                  optflag(OPT_believe)->name);
    }
...
finish(int n)
{
    ...
    case 0:
        ...
        message((-1,
                 "%s cleanup",
                 state.interrupt ? "interrupt" : n > 0 ? "error" : "normal"));
    ...
    case 7:
        ...
        if (state.test & 0x00002000)
        {
            ...
            vmstat(Vmheap, &vs);
            error(0,
                  "vm region %zu segments %zu busy %zu:%zu free %zu:%zu",
                  vs.extent,
                  vs.n_seg,
                  vs.n_busy,
                  vs.s_busy,
    ...
    case 8:
        ...
        for (i = 0; i < elementsof(state.io); i++)
            if (state.io[i] != sfstdin && state.io[i] != sfstdout
                && state.io[i] != sfstderr)
                sfclose(state.io[i]);
