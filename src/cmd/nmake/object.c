...
struct OLD_list_s;
typedef struct OLD_list_s OLD_list_t;
...
compcheck(const char *s, char *v, void *h)
{
    ...
    if (!r->complink || !(r->mark & M_compile)
...
compvar(const char *s, char *u, void *h)
{
    ...
    if ((v->property & V_compiled)
        || state.stateview < 0 && !(v->property & V_frozen)
           && ((v->property & V_import)
               || (v->property & (V_oldvalue | V_readonly)) == V_readonly)
        || state.stateview == 0 && !(v->property & V_retain))
        return 0;
...
compile(char *objfile, char *select)
{
    ...
    if (sferror(sp))
        error(ERROR_SYSTEM | 3,
              "%s: object file optional header write error",
              state.tmpfile);
    ...
    if (select) {
        ...
        if (state.stateview == 0) {
            ...
            if ((100 * object.garbage / (object.rules ? object.rules : 1)
                 < PCTGARBAGE
    ...
    if (sferror(sp))
        error(
        ERROR_SYSTEM | 3, "%s: object file rule write error", state.tmpfile);
    ...
    if (sferror(sp))
        error(ERROR_SYSTEM | 3,
              "%s: object file prerequisite write error",
              state.tmpfile);
    ...
    if (sferror(sp))
        error(ERROR_SYSTEM | 3,
              "%s: temporary object file header write error",
              state.tmpfile);
    ...
    if (state.mam.dynamic || state.mam.regress) {
        ...
        sfprintf(state.mam.out,
                 ...
                 state.mam.label,
                 state.mam.dynamic ? mamname(r) : null,
                 error_info.id);
        mampop(state.mam.out, r, 0);
    }
    if (state.stateview == 0 && object.garbage && object.garbage > cs.rules)
        message((-1,
                 "%d%% [%d/%d] state file garbage collection recovery",
                 (object.garbage - cs.rules) * 100 / object.garbage,
                 object.garbage - cs.rules,
                 object.garbage));
}
...
atomize(const char *s, char *v, void *h)
{
    ...
    if (isoldrule(r)) {
        ...
        error(PANIC,
              ...
              r->name,
              r == getrule(r->name) ? null : " -- duplicate hash");
#endif
        return 0;
    }
    for (p = r->prereqs; p; p = p->next)
...
objectfile(void)
{
    ...
    if (!state.objectfile && state.makefile && state.writeobject) {
        ...
        if (streq(state.writeobject, "-")
            || !stat(state.writeobject, &st) && S_ISDIR(st.st_mode)
               && (dir = state.writeobject))
                ...
                if (getstring(sp) && (off = sfgetu(sp)) && sfgetu(sp))
                    while (!sfeof(sp) && sfseek(sp, off, SEEK_SET) == off
                            ...
                            while (n = sfgetu(sp)) {
                                ...
                                if (n
                                    ...
                                    if (x = bindfile(
                                        ...
                                        /*
                                         * put bound makefile prereqs in state
                                         * as a query courtesy
                                         */

                                        if (!(n & COMP_BASE)
                                            && *x->name != '/')
                                            staterule(RULE, x, NiL, 1)->time
                                            = t;
                                    ...
                                    if (tn && t == tmxsns(tm, 0))
                                        tn = 0;
                                    tm = (lowres && tm == tmxsec(t))
                                         ? t
                                         : tmxsns(tm, tn);

                                    /*
                                     * check prerequisite file time with
                                     * previous
                                     */

                                    debug(
                                    (-4,
                                     "%s%s%s%s%sprerequisite %s [%s] state "
                                     "[%s]",
                                     (n & COMP_DONTCARE) ? "optional " : null,
                                     (n & COMP_BASE) ? "base " : null,
                                     (n & COMP_FILE) ? "-f " : null,
                                     (n & COMP_GLOBAL) ? "-g " : null,
                                     (n & COMP_INCLUDE) ? "include " : null,
                                     s,
                                     timestr(t),
                                     timestr(tm)));
                                    if (t != tm) {
                                    ...
                                    /*
                                     * check that explicit prerequisite still
                                     * specified
                                     */
                                    ...
                                    if (n & (COMP_FILE | COMP_GLOBAL)) {
                                        ...
                                        if (!p) {
                                            ...
                                            state.exec || state.mam.out ? -1
                                                                        : 1,
                                            "%s: %sfile %s was specified "
                                            "last time",
                                            r->name,
                                            (n & COMP_GLOBAL) ? "global "
                                                              : null,
                                            s);
                                            break;
                                        }
                        ...
                        nope:
                            ...
                            /*
                             * check for explicit file prereqs not specified
                             * last time
                             */

                            for (p = internal.globalfiles->prereqs; p;
                                 p = p->next)
                                if (p->rule->mark & M_compile)
                            ...
                            for (p = internal.makefiles->prereqs; p;
                                 p = p->next)
                                if (p->rule->mark & M_compile)
                                    p->rule->mark &= ~M_compile;
                                else if (ok) {
                                    ...
                                    if (state.writeobject)
                                        error(
                                        state.exec || state.mam.out ? -1 : 1,
                                        "%s: file %s not specified last time",
                                        r->name,
                                        p->rule->name);
                                }
...
load(Sfio_t *sp, const char *objfile, int source, int ucheck)
{
    ...
    if (ucheck && st.st_uid != geteuid()
        && (n = CURSECS - st.st_mtime) < ucheck * 60)
        error(1,
              "%s: another user was here %s ago",
              state.makefile,
              fmtelapsed(n, 1));
    ...
    if (memcmp(s, MAGIC, MAGICSIZE) || !(s = getstring(sp))
        ...
        if ((old_magic = OLD_MAGIC,
             (old_swap
              = swapop(&old_header.magic, &old_magic, sizeof(old_magic))))
            < 0
            && (old_magic = OLD_OLD_MAGIC,
                (old_swap
                 = swapop(&old_header.magic, &old_magic, sizeof(old_magic))))
               < 0)
            goto badmagic;
        ...
        if (old_trailer.magic != old_header.magic
            || old_trailer.size != st.st_size)
            goto badmagic;
        ...
        off = sizeof(old_header) + rules * sizeof(old_rule)
              + lists * sizeof(old_list) + variables * sizeof(old_var);
    ...
    message((-3,
             "%s sequence=%d lists=%d rules=%d variables=%d strings=%d",
             ident,
             sequence,
             lists,
             rules,
    ...
    if (lowres && !object.lowres) {
        ...
        if (!state.silent)
            error(1,
                  "%s: low time resolution state file -- subsecond "
                  "differences ignored",
                  objfile);
    }
    ...
    for (xv = v + variables; v < xv; v++) {
        ...
        if ((state.exec || !state.base || state.compileonly)
            && (v->property & V_frozen)
            && (!(x = getvar(v->name))
                && ((v->property & V_oldvalue)
                    || (v->property & V_import) && *v->value)
                || x
                   && ((x->property & (V_append | V_readonly))
                       == (V_append | V_readonly)
            ...
            error((state.exec || state.mam.out) && !state.explain ? -1 : 1,
                  ...
                  objfile,
                  ((v->property | (x ? x->property : 0)) & V_import)
                  ? "environment "
                  : ((x ? x->property : 0) & V_readonly) ? "command argument "
                                                         : null,
                  v->name);
            recompile = 1;
    ...
    for (xr = r + rules; r < xr; r++) {
        ...
        if (old) {
            ...
            case 2: /* 01/24/89 */
                ...
                r->dynamic = ((r->dynamic & 0x000003ffL))
            ...
            case 0: /* 1989-09-11 */
                if ((r->property & P_attribute)
                    && (r->attribute && !(r->property & P_use)
                        && !streq(r->name, internal.attribute->name)
                        || r->scan && !streq(r->name, internal.scan->name)))
                    r->dynamic |= D_index;
                /*FALLTHROUGH*/
            case 1: /* 1991-07-17 */
                if (r->property & P_staterule) {
                    ...
                    for (a = internal.scan->prereqs; a; a = a->next)
                        if (r->scan == a->rule->scan) {
                            error((state.exec || state.mam.out)
                                  ...
                                  ? -1
                                  : 1,
                                  "%s: %s %s definition clashes with %s",
                                  objfile,
                                  r->name,
                                  internal.scan->name,
                                  a->rule->name);
        ...
        if (r->dynamic & D_compiled) {
            ...
            if (n && (o || (state.questionable & 0x00000020))) {
                ...
                if (!o) {
                    ...
                    if (o->dynamic & (D_bound | D_scanned)) {
                        ...
                        r->status = o->status;
                        r->view = o->view;
                        r->property &= ~(P_parameter | P_state | P_staterule
                                         | P_statevar);
                        r->property |= o->property
                                       & (P_parameter | P_state | P_staterule
                                          | P_statevar);
                        r->dynamic &= ~(D_bound | D_entries | D_global
                                        | D_regular | D_scanned);
                        r->dynamic |= o->dynamic
            ...
            if (state.stateview >= 0) {
                ...
                r->property
                &= (P_attribute | P_dontcare | P_ignore | P_parameter
                    | P_state | P_staterule | P_statevar | P_terminal);
                r->dynamic &= D_garbage;
            }
        }
#if BINDINDEX
        ...
        for (xd = d + lists; d < xd;) {
            if (old) {
                ...
                if (strcmp(ident, corrupt) < 0
                    ...
                    error(1,
                          "%s: pre-%s make object corruption repaired",
                          objfile,
                          corrupt);
                    corrupt = 0;
                }
    ...
    if (!state.list) {
        ...
        if (state.stateview < 0) {
            ...
            if (state.global)
                state.global++;
            for (xr = r + rules; r < xr; r++)
                if ((r->property & (P_immediate | P_target))
                    == (P_immediate | P_target))
                    immediate(r);
            if (state.global)
                state.global--;
        }
