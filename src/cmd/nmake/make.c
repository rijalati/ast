                    ...
                    if (u->active && !state.mam.statix) {
                        if (state.expandview) {
                            mount(u->name,
                                  ...
                                  FS3D_GET | FS3D_VIEW
                                  | FS3D_SIZE(sfstrsize(internal.tmp)),
                                  NiL);
                            u->active->original
    ...
    *ttarget = 0;
    ...
    if (r->property & P_use) {
        ...
        if (r->status == NOTYET) {
            ...
            error(PANIC,
                  "%s rule %s has invalid status=%d",
                  internal.use->name,
                  r->name,
                  r->status);
#endif
        tevent = ((r->property & P_metarule) && (r1->property & P_implicit)
                  && r1->action)
                 ? 0
                 : r->time;
    ...
    message(
    (-1, "make(%s%s%s)", r->name, arg ? " " : null, arg ? arg : null));
    error_info.indent++;
    r3 = r;
    r3name = unbound(r3);
    zero(frame);
                ...
                if (!(r->property & P_ignore)) {
                    if (r->dynamic & D_aliaschanged)
                        tevent = CURTIME;
                             ...
                             && parent->scan == SCAN_IGNORE
                             && (r0 || (r0 = staterule(RULE, r, NiL, 0)))
                             && tevent < r0->time)
                        tevent = r0->time;
                    *ttarget = tevent;
                    PREVIEW(parent, r);
    ...
    if ((r3 == r || (r->property & P_target) && !(r3->property & P_target))
        && (!(r->dynamic & D_alias) || r3name == unbound(r)))
        r3 = 0;
    otargetview = state.targetview;
    if (state.maxview && !(r->property & P_state) && !r->scan
    ...
    r->dynamic &= ~(D_hasafter | D_hasbefore | D_hasmake | D_hasscope
                ...
                if (!(r->property & P_accept) && !state.accept) {
                    if (r->dynamic & D_aliaschanged)
                        ...
                        if (r->time && r->time < r0->time
                            && (r->dynamic & D_regular))
                            error(1,
                                  "%s has been replaced by an older version",
                                  unbound(r));
                        if (r0->event && r0->time)
                            reason((1,
                                    "%s [%s] has changed [%s]",
                            ...
                            reason((1,
                                    "%s [%s] has changed in view %s",
                                    r->name,
                                    timestr(r->time),
                                    state.view[r->view].path));
                        else
                            reason((1,
                                    "%s [%s] has no previous state",
                                    r->name,
                                    timestr(r->time)));
                    }
                    must = 1;
                    ...
                    if (n > (u * u) / 2) {
                        ...
                        for (q = p; q; q = q->next)
                            if (!q->next
                                ...
                                message(
                                (-2,
                                 "require loop avoidance skips to %s "
                                 "[total=%lu uniqe=%lu]",
                                 q->next ? q->next->rule->name : "END",
                                 n,
                                 u));
                                break;
                            }
    ...
    if (!errors
        ...
        if ((r2 = metaget(r, r->active, stem, &r4))
            && !(state.questionable & 0x00100000)
            && ((state.questionable & 0x00200000)
        ...
        if (r2) {
            ...
            if (mam)
                sfprintf(mam,
                         ...
                         mamname(r),
                         r4->name,
                         state.mam.statix ? localview(r2) : r2->name,
                         stem);
            debug(
            (-9, "metarule=%s source=%s stem=%s", r4->name, r2->name, stem));
            frame.stem = stem;
            ...
            error(state.keepgoing || state.unwind ? 1 : 3,
                  "can't find source for %s",
    ...
    message((-2,
             ...
             timestr(tevent),
             errors ? " ERRORS" : null,
             errors && state.unwind >= error_info.indent ? " ignored" : null,
             must ? " must" : null));
        ...
        if (state.targetview >= 0) {
            ...
            if (!tstbit(r->checked[CONSISTENT], state.targetview)) {
                reason((1,
                        ...
                        r->name,
                        state.view[state.targetview].path));
                tevent = CURTIME;
            }
        }
...
makeafter(Rule_t *r, Flags_t property)
{
    ...
    if ((r->dynamic & (D_hasafter | D_triggered))
        ...
        message((-2,
                 "check explicit %safter `prerequisites'",
                 (property & P_dontcare) ? "dontcare " : null));
        for (p = r->prereqs; p; p = p->next)
