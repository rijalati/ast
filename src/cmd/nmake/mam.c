...
mamerror(int fd, const void *b, size_t n)
{
    ...
    return (fd == sffileno(sfstderr) && error_info.write == write)
           ? sfwrite(sfstderr, b, n)
           : write(fd, b, n);
}
...
mamname(Rule_t *r)
{
    ...
    if (state.mam.statix)
    {
        ...
        = ((r->property & P_target) || !state.user)
          ? unbound(r)
          : (state.mam.regress || state.expandview) ? r->name : localview(r);
    if (state.mam.root
        && (*a == '/'
            ...
            || stat(r->name, &st)))
    {
        ...
        pathcanon(a, 0, 0);
        if (!strncmp(a, state.mam.root, state.mam.rootlen)
            && *(a + state.mam.rootlen) == '/')
...
mampop(Sfio_t *sp, Rule_t *r, Flags_t flags)
{
    ...
    if (!state.mam.hold)
    {
        ...
        sfprintf(sp,
                 ...
                 (flags & P_virtual) && !(r->property & P_state)
                 && ((r->property & P_virtual)
                     || !(r->dynamic
                          & (D_entries | D_member | D_membertoo | D_regular))
                        && (!s || !s->time))
                 ? " virtual"
                 : s && (s->dynamic & D_built) ? " generated" : null);
    }
}
...
mamout(Rule_t *r)
{
    ...
    if ((r->property & (P_after | P_before | P_virtual))
        || (r->property & P_dontcare)
           && !(state.mam.dontcare || r->prereqs || r->action && *r->action
                || (r0 = staterule(RULE, r, NiL, 0))
                   && (r0->dynamic & D_built) && r0->action && *r0->action
                || !(r->dynamic & D_global)))
        return 0;
