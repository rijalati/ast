...
acceptrule(Rule_t *r)
{
    ...
    if (r->status != TOUCH)
        r->status
        = r->time ? EXISTS : (r->property & P_dontcare) ? IGNORE : FAILED;
}
    ...
    if (t = strrchr(s, '/')) {
        *t = 0;
        ...
        if (r
            ...
            commit(job, s);
            if (((job->flags & CO_ALWAYS) || state.exec && state.touch)
                && (mkdir(s, st.st_mode & (S_IRWXU | S_IRWXG | S_IRWXO))
                    || stat(s, &st)))
                error(1,
            ...
            if (state.mam.out) {
                ...
                dumpaction(
                state.mam.out, MAMNAME(job->target), sfstruse(tmp), NiL);
                sfstrclose(tmp);
            }
            r = makerule(s);
...
static int
done(Joblist_t *job, int, Cojob_t *);
...
execute(Joblist_t *job)
{
    ...
    if (state.targetcontext
        || state.maxview && !state.fsview && *job->target->name != '/'
           && (!(job->target->dynamic & D_regular) || job->target->view))
        commit(job, job->target->name);
    if ((state.mam.dynamic || state.mam.regress) && state.user
        && !(job->target->property
             & (P_after | P_before | P_dontcare | P_make | P_state
                | P_virtual)))
    ...
    if (!(job->flags & CO_ALWAYS)) {
        if (state.touch) {
            ...
            if (!(job->target->property & (P_attribute | P_virtual))) {
                ...
                if ((job->target->property & (P_joint | P_target))
                    == (P_joint | P_target))
                    for (p = job->target->prereqs->rule->prereqs; p;
                         p = p->next)
        ...
        if (!state.coshell) {
            ...
            if (state.serialize && state.jobs > 1)
                flags |= CO_SERIALIZE;
            if (!(state.coshell = coopen(
                  getval(CO_ENV_SHELL, VAL_PRIMARY), flags, sfstruse(sp))))
...
#if !_HUH_1992_02_29 /* i386 and ftx m68k dump without this statement --     \
                        help */
        message(
        (-99, "execute: %s: t=0x%08x &t=0x%08x", job->target->name, t, &t));
#endif
        if (state.mam.out)
    ...
    if (job->status == INTERMEDIATE)
        ...
        for (p = job->target->prereqs; p; p = p->next) {
            ...
            if (a->status == MAKING && !a->semaphore
                || !(a->property & P_make) && a->status == UPDATE)
                return 0;
        }
    after:
#if __GNUC__ == 3 && (sparc || _sparc || __sparc)                            \
&& !_HUH_2008_10_25 /* gcc code generation bug -- first hit on solaris --    \
                       not sure if for all gcc 3.* */
...
#    endif
        if (!jobs.firstjob) {
            ...
            if (!warned) {
                ...
                error(state.mam.regress || state.regress ? -1 : 1,
                      ...
                      "workaround -- pass this on to the vendor",
                      __LINE__,
                      __GNUC__,
                      __GNUC_MINOR__,
                      __GNUC_PATCHLEVEL__);
    ...
    if (job->target->status != TOUCH)
        job->target->status
        = (job->flags & CO_ERRORS)
          ? ((job->target->property & P_dontcare) ? IGNORE : FAILED)
          : EXISTS;
    tm = statetime(job->target, 0);
    if (n = cojob && (state.mam.dynamic || state.mam.regress) && state.user
            && !(job->target->property
                 & (P_after | P_before | P_dontcare | P_make | P_state
                    | P_virtual)))
        sfprintf(state.mam.out,
                 "%scode %s %d %s %s%s%s\n",
                 state.mam.label,
                 (job->target != state.frame->target
                  || (job->target->property & P_after))
                 ? mamname(job->target)
                 : "-",
                 EXIT_CODE(cojob->status),
                 timefmt(NiL, tm),
                 timefmt(NiL, CURTIME),
                 (job->target->dynamic & D_same) ? " same" : null,
                 cojob->status && (job->flags & CO_IGNORE) ? " ignore"
                                                           : null);
    if ((job->target->property & (P_joint | P_target))
        == (P_joint | P_target))
        for (p = job->target->prereqs->rule->prereqs; p; p = p->next)
            if (p->rule != job->target) {
                if (p->rule->status != TOUCH)
                    p->rule->status
                    = (job->flags & CO_ERRORS)
                      ? ((p->rule->property & P_dontcare) ? IGNORE : FAILED)
                      : EXISTS;
                ...
                if (n)
                    sfprintf(state.mam.out,
                             ...
                             EXIT_CODE(cojob->status),
                             timefmt(NiL, tm),
                             (p->rule->dynamic & D_same) ? " same" : null,
                             cojob->status && (job->flags & CO_IGNORE)
                             ? " ignore"
                             : null);
            }
...
#if __GNUC__ >= 4                                                            \
&& !_HUH_2006_01_11 /* gcc code generation bug -- first hit on macos -- not  \
                       sure if for all gcc 4.* */
#    ifndef __GNUC_MINOR__
#        define __GNUC_MINOR__ 0
#    endif
#    ifndef __GNUC_PATCHLEVEL__
                        ...
                        if (!job) {
                            ...
                            if (!warned) {
                                ...
                                error(state.mam.regress || state.regress ? -1
                                                                         : 1,
                                      "command.c:%d: gcc %d.%d.%d code "
                                      "generation bug workaround -- pass "
                ...
                while (job = jobs.firstjob) {
                    if (error_info.trace || state.explain)
                        error(state.explain ? 0 : -1,
                              ...
                              : "missing intermediates accepted");
    ...
    for (;;) {
        ...
        if (r = getrule(external.jobdone)) {
            ...
            sfprintf(jobs.tmp,
                     ...
                     job->target->name,
                     cojob->status,
                     fmtelapsed(cojob->user, CO_QUANT),
        ...
        if (cojob->status) {
            if (n = !EXITED_TERM(cojob->status) || EXIT_CODE(cojob->status)) {
                if ((job->target->dynamic & D_hasafter)
                    ...
                    n = 0;
                error(n ? 2 : state.explain ? 0 : -1,
                      "%s%s code %d making %s%s",
                      n ? "*** " : null,
                      ERROR_translate(
                      NiL,
                      NiL,
                      NiL,
                      EXITED_TERM(cojob->status) ? "termination" : "exit"),
                      EXIT_CODE(cojob->status),
                      job->target->name,
                      (job->flags & CO_IGNORE)
        ...
        message((-3,
                 "job: %s: interrupt=%d clear=%d status=%d flags=%08x",
                 job->target->name,
                 state.interrupt,
                 clear,
                 cojob->status,
...
complete(Rule_t *r, List_t *p, Time_t *tm, Flags_t flags)
{
    ...
    for (tprereqs = 0; p; p = p->next) {
        ...
        if (r->status == UPDATE && !(r->property & P_make)
            && !(flags & P_implicit))
            r->status = EXISTS;
        if (recent
        ...
        if (r->status == FAILED)
            errors++;
        if (!(r->property & (P_after | P_before | P_ignore))
            && r->time > tprereqs)
            tprereqs = r->time;
    }
    if (tm)
...
drop(void)
{
    if (state.coshell) {
        ...
        message((-1,
                 "jobs %d user %s sys %s",
                 state.coshell->total,
                 fmtelapsed(state.coshell->user, CO_QUANT),
                 fmtelapsed(state.coshell->sys, CO_QUANT)));
...
trigger(Rule_t *r, Rule_t *a, char *action, Flags_t flags)
{
    ...
    if (state.exec && !state.touch
        || (a->property & P_always) && (!state.never || (flags & CO_URGENT)))
        flags |= CO_ALWAYS;
    if ((a->property | r->property) & P_local)
        flags |= CO_LOCAL;
    if (!state.jobs || (r->property & P_foreground)
        || (r->property & (P_make | P_functional)) == P_functional
        || (r->dynamic & D_hasmake))
    ...
    if (state.coshell
        ...
        while ((cozombie(state.coshell) || cojobs(state.coshell) > n)
               && block(0))
            ;
        if ((flags & CO_FOREGROUND) && r->active && r->active->parent
            && r->active->parent->prereqs
            && copending(state.coshell) > cojobs(state.coshell))
        ...
        for (;;) {
                ...
                case FAILED:
                    ...
                    if ((r->property & (P_joint | P_target))
                        ...
                        for (p = r->prereqs->rule->prereqs; p; p = p->next)
                            if (p->rule != r)
                                p->rule->status
                                = (p->rule->property & P_dontcare) ? IGNORE
                                                                   : FAILED;
                    return;
                case MAKING:
                    if (a->active)
                        error(
                        1, "%s: prerequisite %s is active", r->name, a->name);
                    else
                        n = BLOCKED;
                    break;
                }
            ...
            if (makebefore(r)) {
                ...
                if ((r->property & (P_joint | P_target))
                    ...
                    for (p = r->prereqs->rule->prereqs; p; p = p->next)
                        if (p->rule != r)
                            p->rule->status = (p->rule->property & P_dontcare)
                                              ? IGNORE
                                              : FAILED;
                return;
            }
...
dumpjobs(int level, int op)
{
    ...
    if (state.coshell && error_info.trace <= level) {
        ...
        case JOB_status:
            sfprintf(sfstderr,
                     "JOB  STATUS       TARGET  tot=%d pend=%d done=%d\n",
                     state.coshell->total,
