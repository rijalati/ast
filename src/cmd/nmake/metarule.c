...
metainfo(int type, char *s1, char *s2, int force)
{
    ...
    sfprintf(internal.met,
             "%s.%c.%s%s%s",
             internal.metarule->name,
             type,
             s1 ? s1 : null,
             s2 ? ">" : null,
             s2 ? s2 : null);
 ...
 *
 * NOTE: c==0 (PREREQ_APPEND alias) is used to cut off part of the closure
 * recursion
 */
...
metaclose(Rule_t *in, Rule_t *out, int c)
{
    ...
    for (q = internal.metarule->prereqs; q; q = q->next) {
        if (metamatch(stem, out->name, q->rule->name)
            && (x = metainfo('O', q->rule->name, NiL, 0))) {
            for (p = x->prereqs; p; p = p->next) {
                ...
                if (!x->action || (state.questionable & 0x00000100)) {
                    if (c == PREREQ_DELETE)
                        ...
                        s = ((y = metarule(q->rule->name, z->name, 0))
                             && y->uname)
                            ? y->uname
                            : q->rule->name;
                    }
        ...
        if (c != PREREQ_DELETE && metamatch(stem, q->rule->name, in->name)
            && !streq(stem, "%")
            && (x = metainfo('I', q->rule->name, NiL, 0))) {
...
metaget(Rule_t *r, Frame_t *active, char *stem, Rule_t **meta)
{
        ...
        for (p = internal.metarule->prereqs; p; p = p->next)
            if (metamatch(stem, u, p->rule->name)
                ...
                for (matched = 1, q = x->prereqs; q; q = q->next)
                    if ((m = metarule(q->rule->name, p->rule->name, 0))
                        && (!(r->property & P_terminal)
                            || (m->property & P_terminal))
                        && !(m->property & f)) {
                        if (!tmp)
                            tmp = sfstropen();
                        if (b != u && *u != '/' && *q->rule->name
                            && ((state.questionable & 0x00000008)
                                || !strchr(q->rule->name, '/'))) {
                            char *x = u;
                            ...
                            for (;;) {
                                ...
                                if ((s = bindfile(NiL, t, 0))
                                    && s->status != UPDATE
                                    && (s->time || (s->property & P_target))
                                    || !(state.questionable & 0x08000000)
                                       && m->action && !*m->action
                                       && (s = makerule(t))) {
                                    sfstrclose(tmp);
                                    if (buf)
                        ...
                        for (v = terminal; v; v = v->next)
                            if (metarule(v->rule->name, NiL, 0)) {
                                ...
                                if ((s = bindfile(NiL, sfstruse(buf), 0))
                                    && s->status != UPDATE && s->time) {
                                    s->status = EXISTS;
                                    s = makerule(t);
                                    sfstrclose(buf);
                                    sfstrclose(tmp);
    ...
    if (prereqs) {
        ...
        for (p = terminal; p; p = p->next)
            if (m = metarule(p->rule->name, NiL, 0))
                for (q = prereqs; q; q = q->next)
                    if (metamatch(t, q->rule->name, p->rule->name)
                        && streq(t, b) && (s = bindfile(q->rule, NiL, 0))
                        && s->time) {
                        s->status = EXISTS;
                        goto unconstrained;
                    }
    ...
    for (p = terminal; p; p = p->next)
        if ((m = metarule(p->rule->name, NiL, 0))
            && (!(r->property & P_terminal) || (m->property & P_terminal))
            && !(m->property & f)) {
    ...
    if (!matched && !(r->property & P_terminal)
        && (x = metainfo('N', NiL, NiL, 0)) && x->prereqs) {
        /*
        ...
        if (!matched) {
            ...
            if (prereqs) {
                ...
                debug((-9,
                       "check explicit nonterminal unconstrained metarule "
                       "prerequisites"));
                t = sfstrbase(internal.met);
                for (p = x->prereqs; p; p = p->next)
                    if (m = metarule(p->rule->name, NiL, 0))
            ...
            debug((-9,
                   "check implicit nonterminal unconstrained metarule "
                   "prerequisites"));
...
primary:
    ...
    if (meta) {
        if (m->uname) {
            ...
            if (!(x = metarule(m->uname, p->rule->name, 0)) || x->uname) {
                ...
                if (y = metainfo('O', m->uname, NiL, 0))
                    for (q = y->prereqs; q; q = q->next)
                        if (metamatch(stem, u, q->rule->name)
                            && (x = metarule(m->uname, q->rule->name, 0))) {
                            if (!x->uname)
                                break;
                            x = 0;
                        }
            ...
            if (x) {
                ...
                if (!((y = makerule(sfstruse(internal.met)))->property
                      & P_terminal)
                    && !(y->property & f)) {
                    *meta = x;
                    s = y;
    ...
    else if ((x = metainfo('S', m->name, NiL, 0))
             && !(x->property & P_terminal))
        for (p = x->prereqs; p; p = p->next) {
            metaexpand(internal.met, stem, p->rule->name);
            if ((x = getrule(sfstruse(internal.met)))
                && (x->property & P_terminal))
                return 0;
        }
found:
    if (meta && (state.targetcontext || state.targetprefix)
        && (t = strrchr(u = unbound(r), '/'))) {
        *t = 0;
        if (!strchr(s->name, '/')) {
            sfprintf(internal.met,
                     "%s%s%s",
                     u,
                     state.targetprefix ? state.targetprefix : "/",
                     s->name);
