...
{
    "break", CON_break, "continue", CON_continue, "elif",   CON_elif,
    "else",  CON_else,  "end",      CON_end,      "error",  CON_error,
    "eval",  CON_eval,  "exit",     CON_exit,     "for",    CON_for,
    "if",    CON_if,    "let",      CON_let,      "local",  CON_local,
    "print", CON_print, "read",     CON_read,     "return", CON_return,
    "rules", CON_rules, "set",      CON_set,      "while",  CON_while,
};
...
declare(char *t, int line, long flags)
{
    ...
    p->newv.property
    |= (v->property & (V_auxiliary | V_scan)) | (flags & V_scope);
        ...
        if (!ap && *t == '-') {
            ...
            *argv = 0;
            ...
            if (argv) {
                if (!formal)
                    error(3,
                          "%s: only %d actual argument%s expected",
                          t,
                          argc - 1,
                          argc == 2 ? null : "s");
                if ((a = getarg(&argv, NiL)) && !optional
                    && streq(a, "...")) {
                    optional = 1;
                    if (*v)
                        *(v - 1) = ' ';
    ...
    if (argc) {
        if (formal) {
            if (!optional) {
                ...
                if (!a || !streq(a, "..."))
                    error(
                    3, "%s: actual argument expected", formal, argc - 1);
            }
            while (formal) {
    ...
    if (isdigit(*s)) {
        ...
        if (streq(t, "rules"))
            rules(s);
                 ...
                 && strmatch(t,
                             "assert|define|elif|else|endif|endmac|error|if|"
                             "ifdef|ifndef|include|line|macdef|pragma|"
...
readline(int lead)
{
    ...
    if (s = pp->pushback) {
        pp->pushback = 0;
        if (pp->fp) {
            if (lead > 0 && (t = sfstrseek(pp->ip, 0, SEEK_CUR) - 1) > s
                && *(t - 1) == '\\') {
    ...
    pushback:
        do {
            ...
            if (pp->prompt)
                error(ERROR_PROMPT,
                      lead > 0 && pp->cp == pp->block ? PS1 : PS2);
            if (pp->fp != sfstdin || !state.coshell
                || !state.coshell->outstanding)
                break;
            sps[0] = pp->fp;
            sps[1] = state.coshell->msgfp;
            if ((n = sfpoll(sps, 2, -1)) <= 0)
                continue;
            if (sps[0] == state.coshell->msgfp
                || n > 1 && sps[1] == state.coshell->msgfp)
                while (block(1))
                    ;
        ...
        for (;;) {
            ...
            eof:
                if (q == COMMENT)
                    ...
                    error(lead > 0 ? 2 : 1,
                          "EOF in %c...%c quote starting at line %d",
                          q,
                          q,
                          start);
            ...
            case '/':
                if (!q
                    && (sfstrtell(pp->ip) == line
                        || isspace(*(sfstrseek(pp->ip, 0, SEEK_CUR) - 1))))
                    switch (c = sfgetc(pp->fp)) {
                    case EOF:
                        sfputc(pp->ip, '/');
                        goto eof;
                    ...
                    mid:
                        for (;;)
                            ...
                            comeof:
                                error(
                                2,
                                "EOF in /*...*/ comment starting at line %d",
                                start);
                                goto end;
                            case '\n':
                                error_info.line++;
...
again:
    while (s = lin = readline(lead)) {
                    ...
                    if (i) {
                        ...
                        case CON_else:
                            ...
                            if (*t != 'i' || *(t + 1) != 'f'
                                ...
                                if (*t)
                                    error(
                                    1, "%s: tokens after else ignored", t);
                        ...
                        case CON_for:
                            ...
                            if (c & CON_skip)
                                ...
                                do
                                    putptr(tp, t = getarg(&tok, NiL));
                                while (t);
                                if (*(pp->cp->loop.f.args = ( char ** )
                                      sfstrseek(tp, 0, SEEK_SET))) {
                        ...
                        case CON_return:
                            if (!(pp->cp->flags & CON_skip)) {
                                if (state.frame->target->property
                                    & P_functional) {
                                    n = sfstrtell(sp);
                                    expand(sp, t);
                                    s = sfstrbase(sp) + n;
                                    ...
                                    while (t > s && isspace(*(t - 1)))
                                        t--;
                                    sfstrseek(
                                    sp, t - sfstrbase(sp), SEEK_SET);
                                    sfputc(sp, 0);
                                    setvar(state.frame->target->name,
                                           sfstrseek(sp, n, SEEK_SET),
                                           0);
                                    debug((-5,
                                           "%s returns `%s'",
                                           state.frame->target->name,
                                           sfstrseek(sp, 0, SEEK_CUR)));
                                } else if (*t) {
                                    if ((tm = timenum(t, &e)) != TMX_NOTIME
                                        && *e) {
                                        if ((n = expr(sp, t)) == -1)
                                            tm = TMX_NOTIME;
                                        else
                                    ...
                                    if (tm == TMX_NOTIME) {
                                        ...
                                        (-5,
                                         "return [%s]",
                                         timestr(internal.internal->time)));
                                    } else {
                                        pp->status = EXISTS;
                        ...
                        case CON_error:
                            if (!(pp->cp->flags & CON_skip)) {
                                ...
                                sfputc(sp, 0);
                                i
                                = strtol(sfstrseek(sp, n, SEEK_SET), &tok, 0);
                            ...
                            if (!(pp->cp->flags & CON_skip)) {
                                    ...
                                    for (;;) {
                                        ...
                                        case 'p':
                                            if (!*t
                                                ...
                                                error(
                                                3,
                                                "-%c: file argument expected",
                                                m);
                                                t = "-";
                                            }
                                            if (state.io[d]) {
                                                if (state.io[d] != sfstdin
                                                    && state.io[d] != sfstdout
                                                    && state.io[d]
                                                       != sfstderr)
                                                    sfclose(state.io[d]);
                                        ...
                                        case 'u':
                                            if (!*t && !(t = getarg(&a, NiL)))
                                                error(
                                                3,
                                                "-u: unit argument expected");
                                            switch (c = *t++) {
                                            case 'm':
                                                if ((state.io[d = elementsof(
                                                                  state.io)
                                                                  - 1]
                                                     = state.mam.out)
                                                    && *state.mam.label)
                                ...
                                if (i == CON_print) {
                                    if (state.io[d]
                                        ...
                                        error(
                                        2, "unit %d not open for writing", d);
                                } else if (n != -1 || *t) {
                                    if (state.io[d]
                                        && (sfset(state.io[d], 0, 0)
                                            & SF_READ)) {
                                        if (!*(a = t)
                                            ...
                                            error(
                                            2,
                                            "one variable argument expected");
                                        else {
                                            if (f)
                                                error(1,
                                                      "read format ignored");
                                        ...
                                        error(
                                        2, "unit %d not open for reading", d);
                                }
                            }
                            continue;
                        ...
                        case CON_set:
                            if (!(pp->cp->flags & CON_skip)) {
                                ...
                                sfputc(sp, 0);
                                set(
                                sfstrseek(sp, n, SEEK_SET), 1, pp->scoped);
                            }
                            continue;
                        }
                    }
        ...
        if (pp->cp->flags & CON_skip)
            debug(
            (-8, "%s:%d:skip: `%s'", error_info.file, error_info.line, s));
        else
            debug(
            (-7, "%s:%d:data: `%s'", error_info.file, error_info.line, s));
#endif
        if (!(pp->cp->flags & CON_skip)) {
            for (t = s; isspace(*t); t++)
                ;
                    ...
                    if (*t++ == ':') {
                        ...
                        *t = 0;
                        if (!(*opr = getrule(s - 1))
                            ...
                            *(t - 1) = ':';
                            op = readfile(
                            sfstruse(tmp), COMP_INCLUDE | COMP_DONTCARE, NiL);
                            sfstrclose(tmp);
                            if (!op || !(*opr = getrule(s - 1))
                                || !((*opr)->property & P_operator)) {
                                *opr = internal.op;
...
static const Namval_t nametypes[] = {
    "altstate", NAME_altstate, "assignment", NAME_assignment,
    "context",  NAME_context,  "dynamic",    NAME_dynamic,
    "glob",     NAME_glob,     "identifier", NAME_identifier,
    "intvar",   NAME_intvar,   "option",     NAME_option,
    "path",     NAME_path,     "staterule",  NAME_staterule,
    "statevar", NAME_statevar, "variable",   NAME_variable,
};
...
assertion(char *lhs, Rule_t *opr, char *rhs, char *act, int op)
{
    ...
    if (opr) {
        debug((-6,
               "operator: lhs=`%s' %s rhs=`%s' act=`%-.1024s'",
               lhs,
               opr->name,
               rhs,
               act));
    ...
    if (internal.assert_p->prereqs
        && (opr = associate(internal.assert_p, NiL, lhs, NiL)) && opr->prereqs
        && (opr = opr->prereqs->rule) && (opr->property & P_operator)
        && !opr->uname) {
    ...
    debug(
    (-6, "assertion: lhs=`%s' rhs=`%-.1024s' act=`%-.1024s'", lhs, rhs, act));
        ...
        if (!(s = getarg(&rhs, NiL)))
            ...
            while (s = getarg(&rhs, NiL)) {
                if (streq(s, "blocked")) {
                    ...
                    error(1,
                          "%s: options are "
                          "{blocked,buckets,hash,jobs,nametype,rules,stack,"
                          "variables,view}",
                          s);
            }
        else
        ...
        if (r->property & P_attribute) {
             ...
             */
                ...
                if (att->rule.semaphore < UCHAR_MAX)
                    ...
                    error(1,
                          "%s: maximum semaphore count is %d",
                          r->name,
                          UCHAR_MAX - 1);
            ...
            if ((set.op & (A_metarule | A_special)) == A_metarule || !*s) {
                if (!x)
                    ...
                    error(
                    1,
                    "multiple prerequisite patterns in metarule assertion");
                else {
                    while (c = *s++) {
                        if (c == '%')
                ...
                if (state.user <= 1 && state.reading && state.makefile
                    && (s = strchr(r->name, '=')) && *(s + 1) == '=') {
                    *s = 0;
            ...
            if (!(set.rule.dynamic & D_dynamic) && !(r->dynamic & D_scope)
                && isdynamic(r->name))
    ...
    prereqs = p;
    if (*act || (set.op & (A_null | A_target))
        || (set.rule.property & (P_make | P_local)) == (P_make | P_local))
    ...
    while (name) {
        ...
        if ((set.op & (A_metarule | A_special)) == A_metarule) {
            ...
            if (in) {
                ...
                if (*s != '%' || *(s + 1) != ATTRNAME || !(x = getrule(s + 1))
                    || !(x->property & P_attribute))
                    x = makerule(s);
                addprereq(in,
                          x,
                          ((set.op & A_insert) || *(s + strlen(s) - 1) != '%')
                          ? PREREQ_INSERT
                          : PREREQ_APPEND);
                if (set.op & A_negate) {
                    *name = ATTRCLEAR;
                    merge(
                    &clr.rule, makerule(name), MERGE_ATTR | MERGE_FORCE);
                    *name = ATTRNAME;
                }
                name = getarg(&lhs, &set.op);
            } else {
                ...
                c = ((set.op & A_clear) && !prereqs && !*act) ? PREREQ_DELETE
                                                              : PREREQ_APPEND;
                out = *(r->name + 1) ? r : 0;
                ...
                if (in) {
                    ...
                    if (out) {
                        ...
                        if (name = getarg(&lhs, &set.op)) {
                            addprereq(
                            metainfo('S', in->name, out->name, 1), out, c);
                            do {
                                x = makerule(name);
                                addprereq(
                                metainfo('P', in->name, x->name, 1), out, c);
                                addprereq(
                                metainfo('S', in->name, out->name, 1), x, c);
                                metaclose(in, x, c);
                            } while (name = getarg(&lhs, &set.op));
                        }
                    } else
                        addprereq(
                        metainfo((set.rule.property & P_terminal) ? 'T' : 'N',
                                 NiL,
                                 NiL,
                                 1),
                        in,
            ...
            name = getarg(&lhs, &set.op);
            if (!internal.main->prereqs && !state.global
                && !(set.rule.property & P_operator) && !(set.op & A_special)
                && !special(r) && !special(&set.rule)) {
                internal.main->prereqs = cons(r, NiL);
                internal.main->dynamic &= ~D_compiled;
            }
        }
        if ((r->property & P_readonly)
            || (r->property & P_staterule)
               && !istype(*(r->name + 1), C_ID1)) {
            if (r == internal.readonly)
                continue; /* drop this in 2000 */
            if (pp->fp != sfstdin) {
                error(2,
                      "%s: %s atom cannot appear as target",
                      r->name,
                      (r->property & P_readonly) ? "readonly" : "staterule");
                continue;
            } else if (r->property & P_readonly)
                error(1, "%s: modifying readonly atom", r->name);
        }
        if (!((r->property | set.rule.property) & P_immediate)
            && (r->status == UPDATE || r->status == MAKING)) {
            ...
            if (set.op & (A_clear | A_copy | A_delete | A_insert)) {
                error(
                2, "%s: cannot reorder active target prerequisites", r->name);
                continue;
            }
            isactive = 1;
        } else
        ...
        if (prereqs) {
            ...
            if ((set.op & A_insert)
                ...
                r->prereqs = (set.op & A_insert) ? append(p, r->prereqs)
                                                 : append(r->prereqs, p);
            }
            remdup(r->prereqs);
            if (r->property & P_state)
        ...
        if (!isactive
            && ((set.rule.dynamic & D_dynamic) || isdynamic(r->name)))
            r->dynamic |= D_dynamic;
        if (((set.rule.property | clr.rule.property) & P_functional)
            && !(r->property & P_state)
            && ((v = getvar(r->name)) || (v = setvar(r->name, null, 0)))) {
        ...
        if ((set.rule.property & P_target)
            && !((clr.rule.property | r->property) & P_target)) {
            r->property |= P_target;
            if (state.targetcontext && (s = strrchr(r->name, '/')))
                makerule(s + 1)->dynamic |= D_context;
        }
        if ((set.rule.property & P_use)
            && (!(r->property & P_attribute) || !r->attribute))
            r->property |= P_use;
        ...
        if ((set.rule.property & P_attribute)
        ...
        if (!state.init && !state.readonly
            && (!state.op && state.reading || !(r->property & P_immediate)))
...
assignment(char *lhs, int op, char *rhs)
{
    ...
    if (internal.assign_p->prereqs
        && (r = associate(internal.assign_p, NiL, lhs, NiL)) && r->prereqs
        && (r = r->prereqs->rule) && (r->property & P_operator)
        && !r->uname) {
        s = r->uname = r->name;
        r->name = (op & OP_APPEND)
                  ? "+="
                  : (op & OP_AUXILIARY) ? "&=" : (op & OP_STATE) ? "==" : "=";
    ...
    debug((-6,
           "assignment: lhs=`%s' %s%srhs=`%-.1024s'",
           lhs,
           (op & OP_APPEND) ? "[append] " : null,
           (op & OP_STATE) ? "[state] " : null,
           rhs));
    if (!(s = getarg(&lhs, NiL)))
...
rules(char *s)
{
    ...
    if (state.rules) {
        ...
        if (strcmp(sfstruse(internal.nam), sfstruse(internal.wrk)))
            error(
            3, "%s: incompatible with current base rules %s", t, state.rules);
    } else if (t == null)
        state.rules = null;
    else
...
nextarg(char *s, char **p, char **end, long *val)
{
    ...
    if ((c = *s) == MARK_QUOTE) {
        ...
        *end = s;
        ...
        sfsprintf(
        arg = buf, sizeof(buf), "%ld", *val = strexpr(s, &s, makeexpr, NiL));
        end = 0;
    } else
...
parse(Sfio_t *fp, char *bp, char *name, Sfio_t *scoped)
{
    ...
    if (pp->fp = fp) {
        pp->ip = sfstropen();
        pp->prompt
        = fp == sfstdin && isatty(sffileno(fp)) && isatty(sffileno(sfstderr));
        if (fp == sfstdin)
            sfdcslow(fp);
    } else {
        pp->bp = bp;
            ...
            if (*lhs || *act) {
                ...
                if (*lhs) {
                    ...
                    while ((op = *rhs++) && !isspace(op))
                        sfputc(internal.nam,
                               islower(op) ? toupper(op)
                                           : isupper(op) ? tolower(op) : op);
                    rhs--;
                ...
                if (!(opr = getrule(alt))
                    || !(opr->property
                         & (P_attribute | P_functional | P_immediate))) {
                    if (pp->fp == sfstdin) {
                        if ((op = *++alt) == 'P'
                            && (!*(alt + 1) || !strcmp(alt, "PRINT"))) {
                            while (isspace(*rhs))
                                rhs++;
                            sfputr(sfstdout, rhs, '\n');
                            sfstrclose(tmp);
...
quit:
    ...
    if (pp->cp > pp->block)
        error(3,
              "missing %d closing end statement%s",
              pp->cp - pp->block,
              pp->cp - pp->block == 1 ? null : "s");
    if (pp->fp)
        sfstrclose(pp->ip);
    else if (pp->newline)
        *pp->bp = '\n';
