...
listrule(const char *s, char *v, void *h)
{
    ...
    if (!dumpall && state.list
        && (r->name != s
            || !(r->property
                 & (P_accept | P_after | P_always | P_archive | P_before
                    | P_command | P_force | P_foreground | P_functional
                    | P_ignore | P_immediate | P_implicit | P_joint | P_local
                    | P_make | P_multiple | P_operator | P_parameter | P_read
    ...
    if (dumpall || !state.list) {
        ...
        if (r->view > r->preview)
            sfprintf(
            sp, "view=%c/%c ", VIEWOFFSET + r->view, VIEWOFFSET + r->preview);
        else if (r->view)
        ...
        if ((r->property & (P_metarule | P_use)) == P_use)
            sfputr(sp, internal.use->name, ' ');
        if (r->property & P_virtual)
    ...
    if ((dumpall || !state.list) && (r->property & P_statevar)
        && r->statedata)
        sfprintf(sp, " state: %s\n", r->statedata);
    return 0;
}
