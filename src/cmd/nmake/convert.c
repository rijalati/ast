        ...
        if (op == 'C' || op == '/' || op == 'Y' || op == '?') {
            ...
            for (cnt = 0; c = *ed; ed++) {
                if (c == '(')
                    ...
                    if (c == '\\' && !*++ed)
                        error(3,
                              "unterminated lhs of %s: %s",
                              op == 'C' ? "substitution" : "conditional",
                              editcontext(eb, ed));
                }
            }
            newp = ed;
                ...
                if (mp->cmd.type != ED_QUAL) {
                    ...
                    if (op == 'T') {
                        if (bound)
                            ...
                            if (qual & ED_NOWAIT)
                                xp += sfsprintf(
                                xp, EXBUF, " %snowait", state.longflag);
                ...
                *xp++ = *a++;
                ...
                if (vp) {
                    ...
                    if (*var == '"') {
                        ...
                        *(a - 1) = 0;
                        xp += sfsprintf(
                        xp, EXBUF, "%sdefault=%s ", state.longflag, var);
                        *(a - 1) = c;
                        var = 0;
                    }
                }
...
byop(const char *a, const char *b)
{
    ...
    if (ap->cmd.type == ED_QUAL)
        return (-1);
    if (bp->cmd.type == ED_QUAL)
        return (1);
    if (ap->cmd.op < bp->cmd.op)
        return (-1);
    if (ap->cmd.op > bp->cmd.op)
        return (1);
    if (!ap->cmd.arg)
