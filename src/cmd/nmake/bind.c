...
#define HACKSPACE(f, s)                                                      \
    for (s = f; s = strchr(s, ' ');                                          \
         *s++ = (state.test & 0x00080000) ? '?' : FILE_SPACE)
#define FIGNORE(s)                                                           \
    ((s)[0] == '.' && ((s)[1] == 0 || (s)[1] == '.' && (s)[2] == 0))
...
#else
    table.file
    = hashalloc(table.ar, HASH_set, HASH_ALLOCATE, HASH_name, "files", 0);
...
unique(Rule_t *r)
{
        ...
        && (!state.mam.statix || S_ISDIR(st.st_mode)))
    {
        ...
        if (r->name == d->name
            || (r->dynamic & D_alias) && makerule(r->name)->name == d->name
            || (x = makerule(d->name)) == r)
            return d;
        ...
        message((-2,
                 "%s %s is also specified as %s",
                 (r->property & P_archive) ? "archive" : "directory",
                 unbound(r),
                 x->name));
        r->dynamic |= D_alias;
        ...
        if (d = unique(r))
        {
            ...
            if (d->directory && (dirp = opendir(s)))
            {
                ...
                if (!(r->dynamic & D_bound) && !stat(s, &st))
                {
                    ...
                    if (!r->view
                        && ((state.questionable & 0x00000800)
                            || !(r->property & P_target))
                        && *s == '/'
                        && (strncmp(s, internal.pwd, internal.pwdlen)
                            || *(s + internal.pwdlen) != '/'))
                        r->dynamic |= D_global;
                }
                closedir(dirp);
...
bindalias(Rule_t *r, Rule_t *x, char *path, Rule_t *d, int force)
{
    ...
    if (state.test & 0x00000040)
        error(2,
              "bindalias: path=%s r=%s%s%s x=%s%s%s",
              path,
              r->name,
              r->uname ? "==" : null,
              r->uname ? r->uname : null,
              x->name,
              x->uname ? "==" : null,
              x->uname ? x->uname : null);
#endif
...
bindfile(Rule_t *r, char *name, int flags)
{
...
#if DEBUG
        message((-11,
                 "bindfile(%s): dir=%s base=%s",
                 name,
                 dir ? dir : internal.dot->name,
                 base));
#endif
        ndirs = 0;
        if (!(flags & BIND_DOT))
            ...
            if (s && !streq(s, name))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        s = sfstrseek(buf, 0, SEEK_SET);
                        canon(s);
                        if (state.fsview && state.expandview
                            && tmxgetmtime(&st) && iview(&st))
                            mount(s,
                                  s,
                                  FS3D_GET | FS3D_VIEW
                                  | FS3D_SIZE(sfstrsize(buf)),
                ...
                if (!(d->mark & M_directory))
                {
                    ...
                    if (d->name != internal.dot->name)
                    {
                        ...
                        if ((x = getrule(s)) && (x->property & P_target))
                        {
                            ...
                            tm
                            = (x->time
                               || (x = staterule(RULE, x, NiL, 0)) && x->time)
                              ? x->time
                              : CURTIME;
                            goto context;
                        }
    ...
    if (found)
    {
        ...
        if (!(state.questionable & 0x00010000) && dir
            && (r->dynamic & D_regular) && *(s = name) != '/')
            while (s = strchr(s, '/'))
...
unbind(const char *s, char *v, void *h)
{
        ...
        || h && (r->dynamic & D_alias) && (makerule(r->name)->mark & M_mark))
    {
        message((-2,
                 ...
                 r->name,
                 h ? " check-alias" : null,
                 s && streq(s, r->name) ? null : " diff-hash"));
        r->mark &= ~M_mark;
        ...
        else
        {
            ...
            r->dynamic &= ~(D_bound | D_entries | D_global | D_member
                            | D_membertoo | D_regular | D_scanned);
            r->must = 0;
            r->scan = 0;
            r->status = NOTYET;
            r->time = 0;
    ...
    if (r->view && r->uname)
    {
        if (state.mam.statix)
        {
            ...
            for (i = 1; i <= state.maxview; i++)
                if (strneq(r->name, state.view[i].path, state.view[i].pathlen)
                    && r->name[state.view[i].pathlen] == '/')
