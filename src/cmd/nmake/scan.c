...
static Namval_t scantab[] = {
    ".IGNORE", SCAN_IGNORE, ".NULL", SCAN_NULL, ".STATE", SCAN_STATE,
};
                    ...
                    for (b = a + 1; b <= last; b++)
                    {
                        if (b->pattern && b->pattern[i] != m->pattern[i])
                        {
                            if ((*v = (u - v)) != (u - v))
                                error(
                                3,
                                "pattern transition offset %d too large",
                                (u - v));
                            v += 2;
...
scancompile(Rule_t *r, int flags)
{
    ...
    do
    {
        ...
        case 'C':
            ...
            ss->data
            = hashalloc(NiL, HASH_set, HASH_ALLOCATE, HASH_name, r->name, 0);
            break;
        case 'D':
        ...
        while (s = *p++)
        {
            ...
            *n = 0;
            ...
            case 'I':
                if (!i)
                    error(
                    3, "%% arg match missing from pattern [%s]", a->pattern);
                if (i > SCANARGS)
                    error(
                    3, "too many %% arg matches in pattern [%s]", a->pattern);
                break;
            }
        }
                                ...
                                if (!strcmp(w, "include"))
                                {
                                    ...
                                    if (!(u->mark & M_scan))
                                    {
                                            ...
                                            && ss->action[1].type == 'I')
                                        {
                                            ...
                                            u->attribute
                                            |= ss->action[1].attribute.set;
                                            u->property
                                            &= ~ss->action[1].property.clear;
                                            u->property
                                            |= ss->action[1].property.set;
                                        }
                                        u->scan = r->scan;
                                        staterule(RULE, u, NiL, 1)->scan
                                        = r->scan;
                                        p = cons(u, p);
                                    }
                                }
        ...
        do
        {
            ...
            if (u->status == UPDATE)
            {
                ...
                else
                {
                    ...
                    error(
                    1,
                    "%s : %s: implicit reference before action completed",
...
scanexec(int fd, Rule_t *r, Scan_t *ss, List_t *p)
{
    ...
    if (ss->external)
    {
        ...
        if (!(
            sp = fapply(
            r, null, r->name, ss->external, CO_ALWAYS | CO_LOCAL | CO_URGENT)))
                    ...
                    if (typ == DIG || typ == SPC
                        ...
                        for (;;)
                        {
                            ...
                            else if (!c)
                            {
                                ...
                                if ((c = read(
                                     fd, g = buf + SCANBUFFER, SCANBUFFER))
                                    <= 0)
                                    goto done;
                                g[c] = 0;
                            }
...
scan(Rule_t *r, Time_t *tm)
{
    ...
    if (tm)
        *tm = 0;
    if ((r->property & (P_attribute | P_state | P_virtual))
        || !(r->dynamic & D_regular))
        return 0;
