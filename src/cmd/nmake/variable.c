            ...
            int op)
{
    ...
    if (r->scan == SCAN_IGNORE && !(state.questionable & 0x02000000))
        top = -1;
    else if ((x = staterule(PREREQS, r, NiL, 0))
             && (x->property & P_implicit))
        prereqs[i++] = x->prereqs;
    else
        top = 1;
    if (top) {
            ...
            do {
                ...
                if (x->mark & M_generate) {
                    ...
                    if (all || ((x->property & P_state) != 0) == dostate) {
                        ...
                        : state.localview
                          ? localview(x)
                          : ((x->dynamic & D_alias) ? x->uname : x->name),
                        -1);
                    }
                    if (top >= 0)
...
#    endif
        ...
        for (;;) {
            ...
            if (!*s) {
                ...
                if (!(r = getrule(s)))
                    ...
                    case '&':
                    case '?':
                        if (staterule(RULE, NiL, s, -1)
                            || staterule(PREREQS, NiL, s, -1))
                            r = makerule(s);
                        break;
                    }
                if (t)
                ...
                if (!r)
                    goto done;
                s = r->name;
                if (*s == ATTRNAME && !r->active && !(r->dynamic & D_cached)
            ...
            case '~': /* all target prerequisites */
                ...
                val = 0;
                e = (c == '>' && !(state.questionable & 0x01000000)
            ...
            case '?': /* all explicit and generated prerequisites   */
                sep
                = genprereqs(internal.val, r, c == '&', c == '?', sep, op);
                val = 0;
                break;
            ...
            case '^': /* original bound name */
                ...
                if ((r->property & (P_joint | P_target))
                    ...
                    if (r->active->original
                        ...
                        val = state.localview
                              ? localview(makerule(r->active->original))
                              : r->active->original;
                    break;
                }
                for (p = r->prereqs->rule->prereqs; p; p = p->next)
                    if (p->rule->active && (t = p->rule->active->original)) {
                        if (sep)
                            ...
                            sep = 1;
                        sfputr(internal.val,
                               state.localview ? localview(makerule(t)) : t,
                               -1);
                    }
                val = 0;
                break;
    ...
    if (state.reading && !state.global && istype(*s, C_ID1)
        && (v = setvar(s, null, 0)))
        v->property |= V_frozen;
    return null;
}
