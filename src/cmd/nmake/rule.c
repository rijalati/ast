...
#define FUNC(name, func)                                                     \
    ((setvar(name, NiL, V_builtin | V_functional))->builtin = (func))
...
nametype(const char *name, char **e)
{
    ...
    case '(':
        ...
        break;
    default:
        q = istype(*s, C_ID1) ? (NAME_identifier | NAME_variable)
    ...
    for (;;) {
        ...
        default:
            if ((q & NAME_variable)
                && !istype(*(s - 1), C_VARIABLE1 | C_VARIABLE2))
                q &= ~(NAME_identifier | NAME_variable);
            else if ((q & NAME_identifier)
                     && !istype(*(s - 1), C_ID1 | C_ID2))
                q &= ~NAME_identifier;
            continue;
        }
        break;
...
makerule(char *name)
{
    ...
    message((-13,
             "adding %s %s",
             (r->property & P_state) ? "state variable" : "atom",
             r->name));
#endif
    return r;
}
...
special(Rule_t *r)
{
    ...
    if (r->property
        & (P_functional | P_metarule | P_operator | P_repeat | P_state))
        return 1;
...
associate(Rule_t *a, Rule_t *r, char *s, List_t **pos)
{
    ...
    } while (r && s == r->name && r->uname && !(r->property & P_state)
             && (s = r->uname) != r->name);
    return 0;
}
...
#define IGNORECHANGE(r, q)                                                   \
    (((q)->property & (P_joint | P_ignore))                                  \
     || ((q)->dynamic & D_alias) && getrule((q)->name) == r)
        ...
        if ((r->dynamic & (D_entries | D_regular)) == D_entries || EXPLAIN) {
            for (p = oldprereqs;
                 ...
                 && (newprereqs->rule != p->rule
                     || !((newprereqs->rule->dynamic ^ p->rule->dynamic)
                          & (D_alias | D_bound))
...
immediate(Rule_t *r)
{
    ...
    if (r == internal.retain || r == internal.state) {
        ...
        if (!prereqs)
            ...
            error(2,
                  "%s: cannot sync until make object compiled",
                  prereqs->rule->name);
        else
    ...
    if (a->property & P_attribute) {
        ...
        if (x && !(n & P_readonly)) {
            n |= x->property
                 & ~(P_attribute | P_immediate | P_implicit | P_internal
                     ...
                     | P_statevar | P_target);
            if ((x->property & P_implicit)
                || (x->property & (P_metarule | P_terminal)) == P_terminal)
                n &= ~P_implicit;
        }
        ...
        if (a != internal.retain) {
            if (a->attribute & r->attribute)
                return 1;
            if (x
                && (a->attribute & x->attribute & ~internal.ignore->attribute)
                && ((r->property & (P_attribute | P_use)) != P_attribute
                    || r == internal.accept || r == internal.ignore
                    || r == internal.retain))
                return 1;
        }
        if (attrname) {
            ...
            if (a == internal.make)
                return n & P_make;
            if (a == internal.readonly)
                return n & P_readonly;
            if (a == internal.scan)
                return r->scan || x && x->scan;
            if (a == internal.semaphore)
                return r->semaphore;
            if (a == internal.target)
                return n & P_target;
            ...
            if (a->property & n
                ...
                if (a == internal.always)
                    return n & P_always;
                if (a == internal.archive)
                    return n & P_archive;
                if (a == internal.before)
                    return n & P_before;
                if (a == internal.command)
                    return n & P_command;
                if (a == internal.dontcare)
                    return n & P_dontcare;
                if (a == internal.force)
                    return n & P_force;
                if (a == internal.foreground)
                    return n & P_foreground;
                if (a == internal.functional)
                    return n & P_functional;
                if (a == internal.immediate)
                    return n & P_immediate;
                if (a == internal.implicit)
                    return n & P_implicit;
                if (a == internal.joint)
                    return n & P_joint;
                if (a == internal.local)
                    return n & P_local;
                if (a == internal.multiple)
                    return n & P_multiple;
                if (a == internal.op)
                    return n & P_operator;
                if (a == internal.parameter)
                    return n & P_parameter;
                if (a == internal.read)
                    return n & P_read;
                if (a == internal.repeat)
                    return n & P_repeat;
                if (a == internal.state)
                    return n & P_state;
                if (a == internal.terminal)
                    return n & P_terminal;
                if (a == internal.use)
                    return n & P_use;
                if (a == internal.virt)
        ...
        if (attrname) {
            ...
            if (a == internal.built)
                return n & D_built;
            if (a == internal.entries)
                return n & D_entries;
            if (a == internal.global)
                return n & D_global;
            if (a == internal.member)
                return n & (D_member | D_membertoo);
            if (a == internal.regular)
                return n & D_regular;
            if (a == internal.scanned)
                return n & D_scanned;
            if (a == internal.source)
                return n & D_source;
            if (a == internal.triggered)
                return n & D_triggered;
            if (a == internal.file)
                return (n & D_bound) && !(r->property & (P_state | P_virtual))
                       && r->time;
            ...
            if (a == internal.internal)
                return n & P_internal;
            if (a == internal.metarule)
                return n & P_metarule;
            if (a == internal.staterule)
                return n & P_staterule;
            if (a == internal.statevar)
                return n & P_statevar;
            ...
            n = r->status;
            if (a == internal.exists)
                return n == EXISTS;
            if (a == internal.failed)
                return n == FAILED;
            if (a == internal.making)
                return n == MAKING;
            if (a == internal.notyet)
                return n == NOTYET;
...
merge(Rule_t *from, Rule_t *to, int op)
{
    ...
    if (from->name) {
        if (from == to
             ...
             *need to be examined for cases other than (state.mam.statix)
             */
            ...
            if (!state.exec && state.mam.statix && (from->dynamic & D_alias)
                ...
                to->uname = from->name;
                if (!(tostate = staterule(RULE, to, NiL, 0))
        ...
        if (to->name)
            debug((-4,
                   "merging %s%s into %s",
                   (op & MERGE_ATTR) ? "attributes of " : null,
                   from->name,
                   to->name));
#endif
    }
    if (!(to->dynamic & D_bound) || (op & (MERGE_ASSOC | MERGE_FORCE)))
        to->property
        |= from->property
           & (P_accept | P_after | P_always | P_archive | P_before | P_command
              | P_dontcare | P_force | P_foreground | P_functional | P_ignore
              | P_implicit | P_joint | P_local | P_make | P_multiple
              | P_parameter | P_read | P_repeat | P_terminal | P_virtual);
    else
        to->property
        |= from->property
           & (P_accept | P_after | P_always | P_archive | P_before | P_command
              | P_force | P_foreground | P_functional | P_implicit | P_joint
              | P_local | P_make | P_multiple | P_parameter | P_read
              | P_repeat | P_terminal | P_virtual);
    ...
    if (op & MERGE_ALL) {
        ...
        to->property
        |= from->property & (P_accept | P_immediate | P_target | P_use);
        ...
        if (from->attribute && from != internal.accept
            ...
            && ((to->property & (P_attribute | P_use)) != P_attribute
                || to == internal.accept || to == internal.ignore
                || to == internal.retain))
            to->attribute |= from->attribute;
        if (from->scan)
            to->scan = from->scan;
...
mergestate(Rule_t *from, Rule_t *to)
{
    ...
    else if ((from->dynamic & D_alias)
             && (tostate = staterule(RULE, to, NiL, 0)))
    ...
    if (state.test & 0x00000800) {
        error(2,
              ...
              from->name,
              fromstate->name,
              timestr(fromstate->time),
              timestr(fromstate->event));
        error(2,
              "MERGESTATE   to: %s: %s time=[%s] event=[%s]",
              to->name,
              tostate->name,
              timestr(tostate->time),
              timestr(tostate->event));
    }
#endif
...
negate(Rule_t *from, Rule_t *to)
{
    ...
    &= ~(from->property
         & (P_accept | P_after | P_always | P_archive | P_before | P_command
            | P_dontcare | P_force | P_functional | P_ignore | P_immediate
            | P_implicit | P_local | P_make | P_multiple | P_parameter
            | P_repeat | P_target | P_terminal | P_use | P_virtual));
...
rassociate(char *s, int flags)
{
    ...
    r = rinternal(s, flags);
    for (p = r->prereqs; p; p = p->next)
        if (!(p->rule->property & P_attribute) && p->rule->name[0] == '%'
            && p->rule->name[1] == ATTRNAME
            && (a = getrule(p->rule->name + 1))
            && (a->property & P_attribute))
            p->rule = a;
    return r;
}
...
{
    sfprintf(
    internal.val, "%d", state.coshell ? state.coshell->outstanding : 0);
    ...
    if (*s == '-' && (id = *args++) && (r = getrule(*args++))
        ...
        for (;;) {
            ...
            default:
                ...
                setvar(sfstruse(internal.wrk),
                       opt_info.arg && *opt_info.arg
                       ? opt_info.arg
                       : opt_info.num ? "1" : null,
                       0);
                continue;
...
initrule(void)
{
    ...
    INIT(serialize,
         "-",
         P_make | P_virtual | P_force | P_repeat | P_foreground | P_ignore
         | P_multiple);
    ...
    if ((unique(r) || !r->time) && !streq(r->name, internal.dot->name)
        ...
        if (state.maxview < MAXVIEW - 1) {
            ...
            message((-2,
                     "view[%d]: %s %s",
                     state.maxview,
                     state.view[state.maxview].path->name,
                ...
                if (!n || *s != '.' || *(s + 1) != '.'
                    ...
                    for (c = 0; c <= n; c++) {
                        ...
                        sfprintf(internal.tmp,
                                 "%s/%s",
                                 c ? state.view[c].path->name : internal.pwd,
                                 s);
#else
                        sfprintf(internal.tmp,
                                 "%s/%s",
                                 c ? state.view[c].path : internal.pwd,
                                 s);
#endif
