...
= "[-n?\n@(#)$Id: pr (AT&T Research) 2013-09-13 $\n]" USAGE_LICENSE
  ...
  "[a:across?Print columns across rather than down; \b-\b\acolumns\a must be "
  ">1.]"
  ...
  "[h:header?Use \aheader\a instead of the file name in header "
  "text.]:[header]"
  ...
  "[m:multiple?Print the input files one per column, truncating lines to "
  "fit.]"
  "[n:number?Number lines with \adigits\a digits followed by \asep\a.]:?"
  "	[sep[digits]]:=\\t5]"
  "[o:indent|margin|offset?Indent each line with \aindent\a "
  "spaces.]#[indent:=0]"
  ...
  "[P:page?Start printing with page \apage\a. Can also be entered as "
  "+\apage\a.]"
  "	#[page:=1]"
  "[r!:warn?Warn about unreadable input files.]"
  "[s:separate?Separate columns with \astring\a.]:?[string]"
  "[t|T!:headers?Generate headers and trailers and pass form feeds in the "
  "input"
  "	to the output.]"
...
prheader(Pr_t *pp)
{
    ...
    if (sfprintf(
        pp->outfile, "%s  %s Page %d\n\n\n", pp->date, header, pp->pageno)
        < 0)
        return -1;
    return 0;
}
 ...
 */
static int
outcol(Pr_t *pp, char *buff, int size, int spaces)
{
    char *state = pp->state;
    int n = S_NL, col = 0;
    char *cp, *buffend;
...
b_pr(int argc, char **argv, Shbltin_t *context)
{
    ...
    do {
        ...
        if (pp->streams)
            ...
            if (pp->columns)
                prcol(pp);
            else if (pp->width || pp->igap || pp->ogap || pp->offset
                     || pp->numwidth || (pp->flags & D_FLAG))
                prline(pp);
            else
                prpage(pp);
            if (fp != sfstdin)
