...
#define skip(ut)                                                             \
    (!*ut.ut_user || !*ut.ut_line                                            \
     || dead(ut) | !strcasecmp(ut.ut_user, "login"))
...
static void
outline(Sfio_t *out, struct utmpx *up, State_t *sp)
{
    struct stat statb;
    time_t t = up->ut_time;
    char *date = ctime(&t);
...
static int
who(Sfio_t *in, Sfio_t *out, int flags)
{
    struct utmpx ut;
    char *line = 0;
    int count = 0;
...
int
b_who(int argc, char **argv, Shbltin_t *context)
{
    int n, flags = 0;
    Sfio_t *sp;
