...
= "[-?\n@(#)$Id: cat (AT&T Research) 2013-09-13 $\n]" USAGE_LICENSE
  ...
  "[n:number?Causes a line number to be inserted at the beginning of each "
  "line.]"
  ...
  "	new-line.]"
  "[D:dos-output?Output files are opened in \atext\amode which inserts "
  "carriage"
  "	returns in front of new-lines on some systems.]"
            ...
            for (;;)
            {
                if ((n = states[*++cp]) == T_ENDBUF)
                {
                    if (cp < end || last != '\n')
                        break;
                    if (!(nxt
                          = ( unsigned char * )(*reserve)(ip, SF_UNBOUND, 0)))
                    {
