...
static const Tty_t Ttable[] = {
    ...
    { "rows",
      ...
      0,
      24,
      C("\an\a is the number of lines for display") },
    { "cols",
      WIND,
      W_SIZE,
      IG,
      1,
      80,
      C("\an\a is the number of columns for display") },
    ...
    { "erase",
      ...
      VERASE,
      'H',
      C("Erase the last character entered") },
    ...
    { "dsusp",
      ...
      VDSUSP,
      'Y',
      C("Send a terminal stop signal after flushing the input") },
#endif /* VDSUSP */
#ifdef VSUSP
    { "susp",
      CHAR,
    ...
    { "lnext",
      ...
      VLNEXT,
      'V',
      C("Enter the next input character literally") },
#endif /* VLNEXT */
    ...
    { "min",
      ...
      VMIN,
      0,
      C("Mininmum number of characters to read in raw mode") },
    { "time",
      NUM,
      T_CHAR,
      0,
      VTIME,
      0,
      C("Number of .1 second intervals with raw mode") },
    ...
    { "ixon",
      ...
      IXON,
      IXON,
      C("Enable (disable) XON/XOFF flow control. \bstop\b character stops "
        "output") },
#ifdef IXANY
    { "ixany",
      BIT,
    ...
    { "isig",
      ...
      ISIG,
      ISIG,
      C("Enable (disable) \bintr\b, \bquit\b, and \bsusp\b special "
        "characters") },
    { "icanon",
      BIT,
      L_FLAG,
      ...
      ICANON,
      ICANON,
      C("Enable (disable) \berase\b, \bkill\b, \bwerase\b, and \brprnt\b "
        "special characters") },
...
#    define ECHOPRT 0
    ...
    { "xcase",
      ...
      XCASE,
      XCASE,
      C("Enable (disable) \bicanon\b uppercase as lowercase with '\\' "
        "prefix") },
#endif /* XCASE */
#ifdef DEFECHO
    ...
    { "flusho",
      ...
      FLUSHO,
      FLUSHO,
      C("Discard (do not discard) written data. Cleared by subsequent "
        "input") },
#endif /* FLUSHO */
#ifdef PENDIN
    { "pendin",
      ...
      PENDIN,
      PENDIN,
      C("Redisplay pending input at next read and then automatically clear "
        "\bpendin\b") },
#endif /* PENDIN */
    { "noflsh",
      BIT,
      ...
      NOFLSH,
      NOFLSH,
      C("Disable (enable) flushing after \bintr\b and \bquit\b special "
        "characters") },
#ifdef TOSTOP
    { "tostop",
      BIT,
      ...
      TOSTOP,
      TOSTOP,
      C("Stop (do not stop) background jobs that try to write to the "
        "terminal") },
#endif /* TOSTOP */
#ifdef OLCUC
    { "olcuc",
...
#    endif
    ...
    { "evenp",
      ...
      PARENB,
      0,
      C("Same as \bparenb -parodd cs7\b") },
    { "oddp",
      MIXED,
      C_FLAG,
      IG,
    ...
    { "ek",
      ...
      0,
      0,
      C("Reset the \berase\b and \bkill\b special characters to their "
        "default values") },
    { "sane",
      SANE,
      C_FLAG,
      ...
      0,
      0,
      C("Reset all modes to some reasonable values") },
    ...
    { "lcase",
      ...
      0,
      0,
      C("Set \bxcase\b, \biuclc\b, and \bolcuc\b") },
...
#    define cntl(x)                                                          \
        (                                                                    \
        ((x) == '?')                                                         \
        ? ccmapc(0177, CC_ASCII, CC_NATIVE)                                  \
        : ccmapc(ccmapc(x, CC_NATIVE, CC_ASCII) & 037, CC_ASCII, CC_NATIVE))
#endif

static void
sane(struct termios *sp)
{
    const Tty_t *tp;
...
static int
gin(char *arg, struct termios *sp)
{
    int i;
    if (*arg++ != ':')
        return (0);
...
static void
output(struct termios *sp, int flags)
{
    const Tty_t *tp;
    struct termios tty;
    int delim = ' ';
    ...
    for (i = 0; i < elementsof(Ttable); i++) {
        ...
        case CHAR:
...
#else
            {
                ...
                "%s = ^%c;%c",
                tp->name,
                off == 0177 ? '?' : ccmapc(off ^ 0100, CC_ASCII, CC_NATIVE),
                delim);
            }
#endif
            delim = ' ';
...
static void
set(char *argv[], struct termios *sp)
{
    const Tty_t *tp;
    int c, off;
    char *cp;
...
static void
listgroup(Sfio_t *sp, int type, const char *description)
{
    int i;
    sfprintf(sp, "[+");
    for (i = 0; i < elementsof(Ttable); i++) {
...
static void
listmask(Sfio_t *sp, unsigned int mask, const char *description)
{
    int i;
    sfprintf(sp, "[+");
    for (i = 0; i < elementsof(Ttable); i++) {
...
static void
listmode(Sfio_t *sp, const char *name)
{
...
infof(Opt_t *op, Sfio_t *sp, const char *s, Optdisc_t *dp)
{
    ...
    listgroup(sp,
              SPEED,
              "Attempt to set input and output baud rate to number given.  A "
              "value of \b0\b causes immediate hangup");
    listchars(sp, NUM);
    ...
    for (;;) {
        ...
        case ':':
            if (!opt_info.offset)
                ...
                error(
                ERROR_exit(1), "%s: unknown mode", argv[opt_info.index]);
            break;
        case '?':
