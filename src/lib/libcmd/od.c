...
= "[-?\n@(#)$Id: od (AT&T Research) 2013-09-13 $\n]" USAGE_LICENSE
  ...
  "bytes.]#? [length:=3]"
  "[t:format|type?The data item output \aformat\a and \asize\a. A decimal "
  "byte "
...
static const Size_t csize[] = { "char", 'C', 0, 1, 0, 1, 0, 0, 0, 0 };

static const Size_t isize[] = { "char",
                                'C',
                                0,
                                0,
                                0,
...
static const Size_t fsize[] = { "float",
                                'F',
                                'e',
                                0,
                                0,
...
static const Size_t asize[] = { "float",
                                'F',
                                'a',
                                0,
                                0,
...
static const Type_t type[] = {
    { "ISO/IEC 646:1991 named characters",
      ...
      0,
      0 },
    { "hexadecimal floating point", 'A', 0, 0, 0, asize, 0, 0, 0, 0, 0 },
    { "binary character", 'b', 0, 0, bform, csize, 8, 0, 0, 0, 0 },
    ...
    { "locale character or backslash escape (current and future escapes)",
      ...
      0,
      0 },
    { "signed decimal", 'd', 0, 0, 0, isize, 4, 6, 11, 21, 31 },
    { "floating point", 'f', 0, 0, 0, fsize, 0, 0, 0, 0, 0 },
    { "\b--map\b mapped character or hexadecimal value if not printable",
      ...
      0,
      0 },
    { "octal", 'o', 0, "0", 0, isize, 3, 6, 11, 22, 33 },
    { "locale character or backslash escape (\\b \\f \\n \\r \\t)",
      ...
      0,
      0 },
    { "unsigned decimal", 'u', 0, 0, 0, isize, 3, 5, 10, 20, 30 },
    { "hexadecimal", 'x', 0, "0", 0, isize, 2, 4, 8, 16, 32 },
    { "printable bytes", 'z', 0, 0, 0, 0, 1, 0, 0, 0, 0 },
};
            ...
            *e++ = '%';
            if (s = ( char * )tp->fill)
                while (*e = *s++)
                    e++;
            e += sfsprintf(
            e, sizeof(fp->form) - (e - fp->form), "%d", fp->width);
            if (prec)
                e += sfsprintf(
                e, sizeof(fp->form) - (e - fp->form), ".%d", prec);
            if (base)
                e += sfsprintf(e,
                               sizeof(fp->form) - (e - fp->form),
                               "%s.%d",
                               prec ? "" : ".",
                               base);
            if (s = ( char * )zp->qual)
        ...
        for (;;)
        {
            ...
            else
            {
                if (state->printable && fp == state->form)
                {
                    ...
                    if (c = (state->block - (ep - bp)) / state->size
                            * (state->width + 1))
                        sfprintf(op, "%*s", c, "");
                    sfputc(op, ' ');
                    if (state->mbmax)
    ...
    for (;;)
    {
        ...
        respan:
            ...
            if ((m = span - state->span) > state->block)
                m = state->block;
            if (block(
                state, sfstdout, state->span, state->span + m, state->offset))
                goto bad;
            state->offset += m;
            if (!state->verbose)
...
optsize(Sfio_t *sp, const Size_t *zp)
{
    ...
    for (n = 0; zp->name; zp++)
        if (zp->qual && (*zp->qual != zp->name || *(zp->qual + 1)))
            n += sfprintf(
            sp, "[%c|%s?sizeof(%s)]", zp->name, zp->qual, zp->desc);
        else
...
b_od(int argc, char **argv, Shbltin_t *context)
{
    ...
    base:
        sfsprintf(
        state.base, sizeof(state.base), "%%0%d%s%c ", BASE_WIDTH, QUAL, n);
        break;
    default:
    ...
    if (error_info.errors)
        goto done;
    for (fp = state.form; fp; fp = fp->next)
        if ((n = (state.size / fp->size.external) * (fp->width + 1) - 1)
            > state.width)
            state.width = n;
    for (fp = state.form; fp; fp = fp->next)
