...
= "[-?\n@(#)$Id: expr (AT&T Research) 2010-08-11 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?\bexpr\b evaluates an expression given as arguments and "
  "writes "
  ...
  "[+?Terms of the expression must be separate arguments.  A string argument "
  "is "
...
static int
expr_or(State_t *, Node_t *);

static int
getnode(State_t *state, Node_t *np)
{
    char *sp;
    char *cp;
    int i;
...
static int
expr_cond(State_t *state, Node_t *np)
{
...
static int
expr_mult(State_t *state, Node_t *np)
{
...
static int
expr_add(State_t *state, Node_t *np)
{
...
static int
expr_cmp(State_t *state, Node_t *np)
{
