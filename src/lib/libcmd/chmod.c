...
= "[-?\n@(#)$Id: chmod (AT&T Research) 2012-04-20 $\n]" USAGE_LICENSE
  ...
  "}"
  "[+?The \aperm\a portion consists of zero or more of the following "
  "letters:]{"
  "[+r?Read permission.]"
  ...
  "}"
  "[+?The \aop\a portion consists of one or more of the following "
  "characters:]{"
  ...
  "}"
  "[+SEE ALSO?\bchgrp\b(1), \bchown\b(1), \blchmod\b(1), \btw\b(1), "
  "\bgetconf\b(1), "
  "\bls\b(1), \bumask\b(2)]";
...
b_chmod(int argc, char **argv, Shbltin_t *context)
{
    ...
    while (!sh_checksig(context) && (ent = fts_read(fts)))
        ...
        case FTS_DC:
            if (!force)
                error(
                ERROR_warn(0), "%s: directory causes cycle", ent->fts_path);
            break;
        case FTS_DNR:
            if (!force)
                error(
                ERROR_system(0), "%s: cannot read directory", ent->fts_path);
            goto anyway;
        case FTS_DNX:
            if (!force)
                error(ERROR_system(0),
