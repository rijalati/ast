...
= "[-?\n@(#)$Id: ls (AT&T Research) 2013-12-06 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?For each directory argument \bls\b lists the contents; for "
  "each"
  ...
  "	%[-+]][\awidth\a[.\aprecis\a[.\abase\a]]]]]](\aid\a[:\asubformat\a]])"
  "\achar\a."
  ...
  "		[+device?major/minor device numbers if block or character special "
  "device]"
  ...
  "	}"
  "[F:classify?Append a character for typing each entry. Turns on "
  "\b--physical\b.]"
  "[g:group?\b--long\b with no owner info.]"
  "[G?\b--long\b with no group info.]"
  "[h:scale|binary-scale|human-readable?Scale sizes to powers of 1024 { Ki "
  "Mi Gi Ti Pi Xi }.]"
  "[i:inode?List the file serial number.]"
  "[I:ignore?Do not list implied entries matching shell "
  "\apattern\a.]:[pattern]"
  ...
  "[N:literal|show-controls-chars?Print raw entry names (don't treat e.g. "
  ...
  "[J:quote-style|quoting-style?Quote entry names according to "
  "\astyle\a:]:[style:=question]{"
  "	[c:C?C \"...\" quote.]"
  ...
  "	[i:iso?Equivalent to \b+" TIME_ISO
  "\b.]"
  "	[10:posix-iso?No change for the C or posix locales, \biso\b otherwise.]"
  "	[f:full-iso?Equivalent to \b+" TIME_FULL_ISO
  "\b.]"
  "	[l:long-iso?Equivalent to \b+" TIME_LONG_ISO
  "\b.]"
  "	[11:posix-full-iso?No change for the C or posix locales, \bfull-iso\b"
  "		otherwise.]"
  "	[L:locale?Equivalent to \b+" TIME_LOCALE
  "\b.]"
  ...
  "[105:testsize?Shift file sizes left \ashift\a bits and set file block "
  "counts"
...
#define VISIBLE(s, f)                                                        \
    ((f)->fts_level <= 0                                                     \
     || (!(s)->ignore || !strmatch((f)->fts_name, (s)->ignore))              \
        && (!((s)->lsflags & LS_NOBACKUP)                                    \
            || (f)->fts_name[(f)->fts_namelen - 1] != '~')                   \
        && (((s)->lsflags & LS_ALL) || (f)->fts_name[0] != '.'               \
            || ((s)->lsflags & LS_MOST)                                      \
               && ((f)->fts_name[1] && (f)->fts_name[1] != '.'               \
                   || (f)->fts_name[2])))
...
#define BLOCKS(s, st)                                                        \
    (((s)->blocksize == LS_BLOCKSIZE)                                        \
     ? iblocks(st)                                                           \
     : (iblocks(st) * LS_BLOCKSIZE + (s)->blocksize - 1) / (s)->blocksize)
#define PRINTABLE(s, t)                                                      \
    (((s)->lsflags & LS_PRINTABLE) ? printable(s, t) : (t))
    ...
    if (state->lsflags & LS_ESCAPE)
    {
        ...
        if (state->lsflags & LS_SHELL)
            return fmtquote(s,
                            ...
                            "'",
                            strlen(s),
                            (state->lsflags & LS_ALWAYS) ? FMT_ALWAYS : 0);
...
key(void *handle, Sffmt_t *fp, const char *arg, char **ps, Sflong_t *pn)
{
    ...
    if (kp->macro && !kp->disable)
    {
        ...
        case KEY_devmajor:
            if (st)
                n = (S_ISBLK(st->st_mode) || S_ISCHR(st->st_mode))
                    ? major(idevice(st))
                    : major(st->st_dev);
            break;
        case KEY_devminor:
        ...
        case KEY_path:
            if (ent)
                s = ent->fts_path ? PRINTABLE(state, ent->fts_path)
                                  : PRINTABLE(state, ent->fts_name);
            break;
        case KEY_perm:
            if (st)
...
pr(State_t *state, FTSENT *ent, int fill)
{
    ...
    if (ent->fts_level == 0
        ...
        && !(ent->fts_info & FTS_D)
        && !lstat(ent->fts_path ? ent->fts_path : ent->fts_name,
                  ent->fts_statp)
        ...
        else
        {
            ...
            if ((state->lsflags & LS_ACROSS) && n > 1)
            {
                ...
                do
                {
                    ...
                    if (w <= state->width)
                        o = n;
                } while (c < state->width / 2 && (n = (i + c) / (c + 1))
                         && ++c);
                n = o ? o : 1;
                c = (i - 1) / n + 1;
                k = 0;
...
walk(const char *path,
     int (*userf)(State_t *, FTSENT *),
     int flags,
     int (*comparf)(FTSENT *const *, FTSENT *const *),
     State_t *state)
{
    FTS *f;
    FTSENT *e;
...
#define set(s, f)                                                            \
    (opt_info.num ? ((s)->lsflags |= (f)) : (((s)->lsflags &= ~(f)), 0))
#define clr(s, f)                                                            \
    (opt_info.num ? ((s)->lsflags &= ~(f)) : ((s)->lsflags |= (f)))
...
b_ls(int argc, char **argv, Shbltin_t *context)
{
    ...
    while (n = optget(argv, usage))
    {
        ...
        case 'w':
            ...
            if (*e)
                error(2,
                      "%s: invalid screen width specification at `%s'",
                      opt_info.arg,
                      e);
            break;
        case 'x':
        ...
        case 'J':
            ...
            case 's':
                state.lsflags
                |= LS_ESCAPE | LS_PRINTABLE | LS_QUOTE | LS_SHELL;
                break;
            case 'S':
                state.lsflags
                |= LS_ALWAYS | LS_ESCAPE | LS_PRINTABLE | LS_QUOTE | LS_SHELL;
        ...
        case -104:
            ...
            if (*e)
                error(
                3, "%s: invalid date string", opt_info.arg, opt_info.option);
            break;
        case -105:
    ...
    else if (!(state.lsflags
               & (LS_DIRECTORY | LS_BLOCKS | LS_LONG | LS_MARK | LS_MARKDIR
                  | LS_TIME
    ...
    if (!sfstrtell(fmt))
    {
        ...
        if (state.lsflags & LS_LONG)
        {
            ...
            if (!(state.lsflags & LS_NOUSER))
                sfprintf(
                fmt, " %%-8(uid)%c", (state.lsflags & LS_NUMBER) ? 'd' : 's');
            if (!(state.lsflags & LS_NOGROUP))
                sfprintf(
                fmt, " %%-8(gid)%c", (state.lsflags & LS_NUMBER) ? 'd' : 's');
            sfputr(fmt, " %8(device:case::%(size)u:*:%(device)s)s", -1);
            sfprintf(fmt,
                     " %%(%s)s ",
                     (state.timeflags & LS_ATIME)
                     ? "atime"
                     : (state.timeflags & LS_CTIME) ? "ctime" : "mtime");
        }
        sfputr(fmt, "%(name)s", -1);
