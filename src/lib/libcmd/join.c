...
= "[-?\n@(#)$Id: join (AT&T Research) 2013-09-13 $\n]" USAGE_LICENSE
  ...
  "[+DESCRIPTION?\bjoin\b performs an \aequality join\a on the files "
  "\afile1\a "
  ...
  "[+?The \ajoin field\a is a field in each file on which files are "
  "compared. "
  ...
  "[o:output]:[list?Construct the output line to comprise the fields "
  "specified "
  ...
  "be specified.]"
  "[t:separator|tabs]:[delim?Use \adelim\a as the field separator for both "
  "input"
  "	and output.]"
  ...
  "[+?The following obsolete option forms are also recognized: \b-j\b "
  "\afield\a"
  ...
  "[+>0?An error occurred.]"
  "}"
  "[+SEE ALSO?\bcut\b(1), \bcomm\b(1), \bpaste\b(1), \bsort\b(1), "
  "\buniq\b(1)]";
...
#if DEBUG_TRACE
static int i1, i2, i3;
#    define outfield(p, i, n, f)                                             \
        (sfprintf(                                                           \
         sfstdout, "[F%d#%d:%d,%d]", __LINE__, i1 = i, i2 = n, i3 = f),      \
         outfield(p, i1, i2, i3))
#endif
    ...
    if (out = jp->outlist) {
        while ((n = *out++) >= 0) {
            if (n == JOINFIELD) {
                ...
                i = n & 1;
                j = (mode < 0 && i || mode > 0 && !i) ? jp->file[i].nfields
                                                      : n >> 2;
            }
...
#endif
            {
                if (!(jp->outmode & C_COMMON)) {
                    ...
                    if (sfseek(jp->file[0].iop,
                        ...
                        || !(cp1 = getrec(jp, 0, 0))) {
                        error(
                        ERROR_SYSTEM | 2, "%s: seek error", jp->file[0].name);
                        return -1;
                    }
                ...
                next:
                    if (n2 > jp->samesize) {
                        ...
                        if (!(jp->same
                              = newof(jp->same, char, jp->samesize, 0))) {
...
#if DEBUG_TRACE
        sfprintf(
        sfstdout, "[O#%d:%02o:%02o]", __LINE__, jp->ooutmode, jp->outmode);
#endif
        cp1 = (!cp1 && cmp && hi < 0 && !jp->file[1].hit
               && ((jp->ooutmode ^ C_ALL) <= 1 || jp->outmode == 2))
              ? cp2
              : getrec(jp, 1, 0);
        cmp = 1;
        n = 1;
