...
= "[-?\n@(#)$Id: date (AT&T Research) 2011-01-27 $\n]" USAGE_LICENSE
  ...
  "\b%_[EO]]K\b for space separator, %OK adds \b.%N\b, \b%EK\b adds "
  "\b%.N%z\b, \b%_EK\b adds \b.%N %z\b]"
  ...
  "[u:utc|gmt|zulu|universal?Output dates in \acoordinated universal time\a "
  "(UTC).]"
    ...
    if (listzones) {
        ...
        while (listzones->standard) {
            ...
            sfprintf(sfstdout,
                     "%3s %4s %4s %4d %4d\n",
                     s,
                     *listzones->standard ? listzones->standard : "-",
                     listzones->daylight ? listzones->daylight : "-",
                     listzones->west,
                     listzones->dst);
            listzones++;
            show = 1;
