...
= "[-?\n@(#)$Id: sum (AT&T Research) 2012-04-20 $\n]" USAGE_LICENSE
  ...
  "[c:check?Each \afile\a is interpreted as the output from a previous "
  "\bsum\b."
...
static void
verify(State_t *, char *, char *, Sfio_t *);
                ...
                else
                {
                    ...
                    else if (!state->permissions)
                    {
                        if (state->silent)
                            ...
                            error(
                            2, "%s: uid should be %s", file, fmtuid(uid));
                    }
                    ...
                    else if (!state->permissions)
                    {
                        if (state->silent)
                            ...
                            error(
                            2, "%s: gid should be %s", file, fmtgid(gid));
                    }
                    if (state->permissions && (uid >= 0 || gid >= 0))
                    {
                        if (chown(file, uid, gid) < 0)
                        {
                            if (uid < 0)
                                error(ERROR_SYSTEM | 2,
                                      "%s: cannot change group to %s",
                                      file,
                                      fmtgid(gid));
                            else if (gid < 0)
                                error(ERROR_SYSTEM | 2,
                                      "%s: cannot change user to %s",
                                      file,
                                      fmtuid(uid));
                            else
                                error(ERROR_SYSTEM | 2,
                                      "%s: cannot change user to %s and "
                                      "group to %s",
                                      file,
                                      fmtuid(uid),
                                      fmtgid(gid));
                        ...
                        else
                        {
                            if (uid < 0)
                                ...
                                error(
                                1,
                                "%s: changed user to %s and group to %s",
                                file,
                                fmtuid(uid),
                                fmtgid(gid));
                        }
...
list(State_t *state, Sfio_t *lp)
{
    ...
    while (file = sfgetr(lp, '\n', 1))
        ...
        {
            pr(
            state, sfstdout, sp, file, state->permissions, NiL, state->check);
            closefile(sp);
        }
}
...
b_cksum(int argc, char **argv, Shbltin_t *context)
{
    ...
    if (!state.sum && !(state.sum = sumopen(error_info.id))
        && !(state.sum = sumopen(astconf("UNIVERSE", NiL, NiL))))
        ...
        while (!sh_checksig(context) && (ent = fts_read(fts)))
            ...
            case FTS_SL:
                if (!(flags & FTS_PHYSICAL)
                    || (flags & FTS_META) && ent->fts_level == 1)
    ...
    if (state.total)
    {
        sumprint(state.sum,
                 sfstdout,
                 state.flags | SUM_TOTAL | SUM_SCALE,
                 state.scale);
        sfputc(sfstdout, '\n');
    }
    sumclose(state.sum);
