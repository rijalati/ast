...
preserve(State_t *state, const char *path, struct stat *ns, struct stat *os)
{
    ...
    if ((state->preserve & PRESERVE_TIME)
    ...
    if (state->preserve & PRESERVE_IDS) {
        ...
        if (n && chown(state->path, os->st_uid, os->st_gid))
            ...
            case 02:
                error(ERROR_SYSTEM | 2,
                      "%s: cannot reset owner to %s",
                      path,
                      fmtuid(os->st_uid));
                break;
            case 03:
                error(ERROR_SYSTEM | 2,
                      "%s: cannot reset owner to %s and group to %s",
                      path,
                      fmtuid(os->st_uid),
                      fmtgid(os->st_gid));
                break;
        ...
        if (state->preserve && state->op != LN
            ...
            if (stat(state->path, &st))
                ...
                if ((ent->fts_statp->st_mode & S_IPERM)
                    ...
                    error(ERROR_SYSTEM | 2,
                          "%s: cannot reset directory mode to %s",
                          state->path,
                          fmtmode(st.st_mode & S_IPERM, 0) + 1);
                ...
                while (sub = fts_read(fts)) {
                    if (strneq(s, sub->fts_name, n) && sub->fts_name[n] == '.'
                        && strneq(sub->fts_name + n + 1,
                                  ...
                                  state->suflen)
                        && (m = strtol(
                            sub->fts_name + n + state->suflen + 1, &e, 10))
            ...
            if ((!S_ISREG(ent->fts_statp->st_mode)
                ...
                if (n) {
                    error(ERROR_SYSTEM | 2,
                          ...
                          ent->fts_path,
                          state->path,
                          n == 1 ? ERROR_translate(0, 0, 0, "read")
                                 : n == 2 ? ERROR_translate(0, 0, 0, "write")
                ...
                if (stat(state->path, &st))
                    ...
                    if ((state->preserve & PRESERVE_PERM)
                        ...
                        && chmod(state->path,
                                 ent->fts_statp->st_mode & state->perm))
                        error(ERROR_SYSTEM | 2,
                              "%s: cannot reset mode to %s",
                              state->path,
                              fmtmode(st.st_mode & state->perm, 0) + 1);
                    if (state->preserve & (PRESERVE_IDS | PRESERVE_TIME))
                        preserve(state, state->path, &st, ent->fts_statp);
                }
            }
...
b_cp(int argc, char **argv, Shbltin_t *context)
{
    ...
    for (;;) {
        ...
        case 'A':
            ...
            for (;;) {
                ...
                case 'e':
                    state->preserve
                    |= PRESERVE_IDS | PRESERVE_PERM | PRESERVE_TIME;
                    continue;
                case 'i':
    ...
    if (state->backup) {
        ...
        if (!state->suffix
            && !(state->suffix = getenv("SIMPLE_BACKUP_SUFFIX")))
            state->suffix = "~";
