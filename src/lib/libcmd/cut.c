...
= "[-?\n@(#)$Id: cut (AT&T Research) 2013-09-13 $\n]" USAGE_LICENSE
  ...
  "[R|r:reclen]#[reclen?If \areclen\a > 0, the input will be read as fixed "
  "length "
  ...
  "[D:line-delimeter|output-delimiter]:[ldelim?The line delimiter character "
  "for "
...
cutinit(int mode, char *str, Delim_t *wdelim, Delim_t *ldelim, size_t reclen)
{
    ...
    cut->nosplit
    = (mode & (C_BYTES | C_NOSPLIT)) == (C_BYTES | C_NOSPLIT) && cut->mb;
                    ...
                    for (;;) {
                        ...
                        case SP_WIDE:
                            ...
                            while ((c = mbchar(&w, cp, ep - cp, &cut->q)),
                                ...
                                /* mb char spanning buffer boundary -- fun
                                 * stuff */
