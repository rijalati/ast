...
= "[-?\n@(#)$Id: grep (AT&T Research) 2014-01-26 $\n]" USAGE_LICENSE
  ...
  "\aname\a.]:[name:=empty]"
  "[o:only-matching?Print only the non-empty matching parts of matching "
  "lines, "
...
grep(char *id, int options, int argc, char **argv, Shbltin_t *context)
{
    ...
    while (!sh_checksig(context) && (ent = fts_read(fts)))
        ...
        case FTS_DC:
            error(
            ERROR_WARNING | 1, "%s: directory causes cycle", ent->fts_path);
            break;
        case FTS_DNR:
            error(
            ERROR_SYSTEM | 2, "%s: cannot read directory", ent->fts_path);
            break;
        case FTS_DNX:
            error(
            ERROR_SYSTEM | 2, "%s: cannot search directory", ent->fts_path);
            break;
        }
quit:
