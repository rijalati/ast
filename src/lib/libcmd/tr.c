...
next:
    if (tr->last >= 0) {
        while (++tr->prev <= tr->last) {
            ...
            return (!tr->type || !tr->convert)
                   ? tr->prev
                   : tr->convert == 'l' ? towlower(tr->prev)
                                        : towupper(tr->prev);
        }
        if (tr->isit)
            tr->isit = 0;
    ...
    case -1:
        ...
        if (tr->warn)
            error(2,
                  "%s operand: \\x%02x: invalid multibyte character byte",
                  typename[tr->type],
                  c);
        break;
    case 0:
            ...
            if ((o = inend - inp) <= 0
                ...
                if (w < 0) {
                    ...
                    if (tr->warn && eline != line) {
                        ...
                        error(
                        2,
                        "line %zu: \\x%02x: invalid multibyte character byte",
                        line,
                        w);
                    }
                    continue;
