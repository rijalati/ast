...
#endif
        if ((co->outstanding + co->svc_outstanding)
            > (co->running + co->svc_running))
    ...
    do
    {
        ...
        for (;;)
        {
            if (co->flags & CO_DEBUG)
            {
                ...
                errormsg(state.lib,
                         2,
                         "coshell %d wait %lu.%d timeout=%d "
                         "outstanding=<%d,%d> running=<%d,%d>",
                         co->index,
                         serial,
                         loop,
                         timeout,
            ...
            while ((n = sfpoll(&co->msgfp, 1, to)) < 1)
            {
                ...
                for (cj = co->jobs; cj; cj = cj->next)
                    if (cj->pid > 0)
                    {
                        ...
                        = sfsprintf(buf,
                                    sizeof(buf),
                                    "kill -0 %d 2>/dev/null || echo k %d "
                                    "`wait %d 2>/dev/null; echo $?` >&$%s\n",
                                    cj->pid,
                                    cj->id,
                                    cj->pid,
                                    CO_ENV_MSGFD);
            ...
            case 'x':
                ...
                if (cj->pid > 0 || cj->service
                    || (co->flags & (CO_INIT | CO_SERVER)))
                {
                nuke:
                    if (cj->pid > 0 && type != 'k')
                    {
