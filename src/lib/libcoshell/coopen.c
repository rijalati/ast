...
= { "cross",    CO_CROSS,    "debug",   CO_DEBUG,   "devfd",  CO_DEVFD,
    ...
    "separate", CO_SEPARATE, "service", CO_SERVICE, 0,        0 };
...
coopen(const char *path, int flags, const char *attributes)
{
    ...
    if (!state.type
        && (!(s = getenv(CO_ENV_TYPE)) || !(state.type = strdup(s))))
        state.type = "";
    ...
    for (i = 0; i < elementsof(sh); i++)
        if ((s = sh[i]) && *s && (s = strdup(s))) {
            if ((n = tokscan(s, NiL, " %v ", av, elementsof(av) - 1)) > 0) {
                ...
                av[n] = 0;
                sfsprintf(
                evbuf, sizeof(evbuf), "%s=%d", CO_ENV_MSGFD, co->gsmfd);
                ev[0] = evbuf;
                ev[1] = 0;
                if ((co->mode & CO_MODE_SEPARATE)
                    || (proc
...
nope:
    i = errno;
    if (!(s = sh[1])
        || (s = (t = strrchr(s, '/')) ? (t + 1) : s) && !strmatch(s, "?(k)sh")
           && !streq(s, CO_ID))
