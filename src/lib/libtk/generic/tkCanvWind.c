...
static void ScaleWinItem _ANSI_ARGS_((Tk_Canvas canvas,
                                      ...
                                      double scaleX,
                                      double scaleY));
static void TranslateWinItem _ANSI_ARGS_(
...
static int WinItemCoords _ANSI_ARGS_((Tcl_Interp * interp,
                                      ...
                                      int argc,
                                      char **argv));
static void WinItemLostSlaveProc _ANSI_ARGS_((ClientData clientData,
                                              Tk_Window tkwin));
static void WinItemRequestProc _ANSI_ARGS_((ClientData clientData,
                                            Tk_Window tkwin));
static void WinItemStructureProc _ANSI_ARGS_((ClientData clientData,
                                              XEvent *eventPtr));
    ...
    if (oldWindow != winItemPtr->tkwin) {
        ...
        if (winItemPtr->tkwin != NULL) {
            ...
            for (ancestor = canvasTkwin;; ancestor = Tk_Parent(ancestor)) {
                ...
                badWindow:
                    Tcl_AppendResult(interp,
                                     ...
                                     " in a window item of this canvas",
                                     ( char * ) NULL);
                    winItemPtr->tkwin = NULL;
                    return TCL_ERROR;
                }
            }
            if ((( Tk_FakeWin * ) (winItemPtr->tkwin))->flags
                & TK_TOP_LEVEL) {
                goto badWindow;
            }
            ...
            Tk_CreateEventHandler(winItemPtr->tkwin,
                                  ...
                                  ( ClientData ) winItemPtr);
            Tk_ManageGeometry(
            winItemPtr->tkwin, &canvasGeomType, ( ClientData ) winItemPtr);
        }
    }
...
static void
DeleteWinItem(canvas, itemPtr, display) Tk_Canvas canvas; /* Overall info
                                                             about widget. */
...
static void
ComputeWindowBbox(canvas, winItemPtr) Tk_Canvas canvas; /* Canvas that
                                                           contains item. */
 ...
 * must be redisplayed (not used). */
{
    ...
    Tk_CanvasWindowCoords(canvas,
                          ( double ) winItemPtr->header.x1,
                          ( double ) winItemPtr->header.y1,
                          &x,
                          &y);
...
static double WinItemToPoint(canvas,
                             itemPtr,
                             pointPtr) Tk_Canvas canvas; /* Canvas containing
                                                            item. */
                        ...
                        * size. */
{
    ...
    DisplayWinItem(winItemPtr->canvas,
                   ( Tk_Item * ) winItemPtr,
                   ( Display * ) NULL,
                   ( Drawable ) None,
                   0,
