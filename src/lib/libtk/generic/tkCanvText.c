...
static void DisplayText _ANSI_ARGS_((Tk_Canvas canvas,
                                     ...
                                     int width,
                                     int height));
static int GetSelText _ANSI_ARGS_(
...
static void ScaleText _ANSI_ARGS_((Tk_Canvas canvas,
                                   ...
                                   double scaleY));
static void
SetTextCursor _ANSI_ARGS_((Tk_Canvas canvas, Tk_Item *itemPtr, int index));
...
static void TextInsert _ANSI_ARGS_(
(Tk_Canvas canvas, Tk_Item *itemPtr, int beforeThis, char *string));
static int
TextToArea _ANSI_ARGS_((Tk_Canvas canvas, Tk_Item *itemPtr, double *rectPtr));
static double TextToPoint _ANSI_ARGS_((Tk_Canvas canvas,
                                       Tk_Item *itemPtr,
                                       double *pointPtr));
static int TextToPostscript _ANSI_ARGS_(
(Tcl_Interp * interp, Tk_Canvas canvas, Tk_Item *itemPtr, int prepass));
static void TranslateText _ANSI_ARGS_(
...
static void
DeleteText(canvas, itemPtr, display) Tk_Canvas canvas; /* Info about overall
                                                          canvas widget. */
...
static void ComputeTextBbox(canvas,
                            textPtr) Tk_Canvas canvas; /* Canvas that contains
                                                          item. */
...
#define MAX_LINES 100
    ...
    for (numLines = 0; (numLines < MAX_LINES); numLines++) {
        ...
        numChars = TkMeasureChars(textPtr->fontPtr,
                                  p,
                                  (textPtr->text + textPtr->numChars) - p,
                                  0,
                                  wrapPixels,
                                  0,
                          ...
                          * must be redisplayed (not used). */
{
    ...
    for (linePtr = textPtr->linePtr, i = textPtr->numLines; i > 0;
        ...
        if ((textInfoPtr->selItemPtr != itemPtr)
            ...
            inSelect
            = textInfoPtr->selectLast + 1 - (lineIndex + beforeSelect);
...
static double
TextToPoint(canvas, itemPtr, pointPtr) Tk_Canvas canvas; /* Canvas containing
                                                            itemPtr. */
...
int *indexPtr;      /* Where to store converted index. */
{
    ...
    if (string[0] == 'e') {
        ...
        *indexPtr = TkMeasureChars(textPtr->fontPtr,
                                   linePtr->firstChar,
                                   linePtr->numChars,
                                   linePtr->x,
                                   x,
...
/* ARGSUSED */
static void
SetTextCursor(canvas, itemPtr, index) Tk_Canvas canvas; /* Record describing
                                                           canvas widget. */
                   ...
                   * NULL character. */
{
    ...
    strncpy(buffer,
            textPtr->text + textInfoPtr->selectFirst + offset,
            ( size_t )count);
    buffer[count] = '\0';
    return count;
}
