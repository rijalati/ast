...
= { { TK_CONFIG_BORDER,
      ...
      "background",
      "Background",
      DEF_LISTBOX_BG_COLOR,
      Tk_Offset(Listbox, normalBorder),
    ...
    { TK_CONFIG_PIXELS,
      ...
      "borderWidth",
      "BorderWidth",
      DEF_LISTBOX_BORDER_WIDTH,
      Tk_Offset(Listbox, borderWidth),
      0 },
    { TK_CONFIG_ACTIVE_CURSOR,
      ...
      "Cursor",
      DEF_LISTBOX_CURSOR,
      Tk_Offset(Listbox, cursor),
      TK_CONFIG_NULL_OK },
    { TK_CONFIG_BOOLEAN,
      "-exportselection",
      "exportSelection",
      "ExportSelection",
      DEF_LISTBOX_EXPORT_SELECTION,
      Tk_Offset(Listbox, exportSelection),
      0 },
    { TK_CONFIG_SYNONYM,
      "-fg",
      "foreground",
    ...
    { TK_CONFIG_COLOR,
      ...
      "highlightBackground",
      "HighlightBackground",
      DEF_LISTBOX_HIGHLIGHT_BG,
      Tk_Offset(Listbox, highlightBgColorPtr),
      0 },
    { TK_CONFIG_COLOR,
      "-highlightcolor",
      "highlightColor",
      "HighlightColor",
      DEF_LISTBOX_HIGHLIGHT,
      Tk_Offset(Listbox, highlightColorPtr),
      0 },
    { TK_CONFIG_PIXELS,
      "-highlightthickness",
      "highlightThickness",
      ...
      Tk_Offset(Listbox, highlightWidth),
      0 },
    { TK_CONFIG_RELIEF,
      "-relief",
      "relief",
    ...
    { TK_CONFIG_BORDER,
      ...
      Tk_Offset(Listbox, selBorder),
      TK_CONFIG_COLOR_ONLY },
    { TK_CONFIG_BORDER,
      "-selectbackground",
      "selectBackground",
      "Foreground",
      DEF_LISTBOX_SELECT_MONO,
      Tk_Offset(Listbox, selBorder),
      TK_CONFIG_MONO_ONLY },
    { TK_CONFIG_PIXELS,
      "-selectborderwidth",
      "selectBorderWidth",
      "BorderWidth",
      DEF_LISTBOX_SELECT_BD,
      Tk_Offset(Listbox, selBorderWidth),
      0 },
    { TK_CONFIG_COLOR,
      "-selectforeground",
      "selectForeground",
      "Background",
      DEF_LISTBOX_SELECT_FG_COLOR,
      Tk_Offset(Listbox, selFgColorPtr),
      TK_CONFIG_COLOR_ONLY },
    { TK_CONFIG_COLOR,
      "-selectforeground",
      "selectForeground",
      "Background",
      DEF_LISTBOX_SELECT_FG_MONO,
      Tk_Offset(Listbox, selFgColorPtr),
      TK_CONFIG_MONO_ONLY },
    { TK_CONFIG_UID,
      "-selectmode",
      "selectMode",
      "SelectMode",
      DEF_LISTBOX_SELECT_MODE,
      Tk_Offset(Listbox, selectMode),
      0 },
    { TK_CONFIG_BOOLEAN,
      "-setgrid",
      "setGrid",
      ...
      Tk_Offset(Listbox, setGrid),
      0 },
    { TK_CONFIG_STRING,
      "-takefocus",
      "takeFocus",
      "TakeFocus",
      DEF_LISTBOX_TAKE_FOCUS,
      Tk_Offset(Listbox, takeFocus),
      TK_CONFIG_NULL_OK },
    { TK_CONFIG_INT,
      "-width",
      "width",
      "Width",
      DEF_LISTBOX_WIDTH,
      Tk_Offset(Listbox, width),
      0 },
    { TK_CONFIG_STRING,
      "-xscrollcommand",
      "xScrollCommand",
      "ScrollCommand",
      DEF_LISTBOX_SCROLL_COMMAND,
      Tk_Offset(Listbox, xScrollCmd),
      TK_CONFIG_NULL_OK },
    { TK_CONFIG_STRING,
      "-yscrollcommand",
      "yScrollCommand",
...
static void ChangeListboxOffset _ANSI_ARGS_((Listbox * listPtr, int offset));
static void ChangeListboxView _ANSI_ARGS_((Listbox * listPtr, int index));
static int ConfigureListbox _ANSI_ARGS_(
(Tcl_Interp * interp, Listbox *listPtr, int argc, char **argv, int flags));
static void DeleteEls _ANSI_ARGS_((Listbox * listPtr, int first, int last));
...
static int GetListboxIndex _ANSI_ARGS_((Tcl_Interp * interp,
                                        ...
                                        int *indexPtr));
static void
InsertEls _ANSI_ARGS_((Listbox * listPtr, int index, int argc, char **argv));
static void ListboxCmdDeletedProc _ANSI_ARGS_((ClientData clientData));
static void ListboxComputeGeometry _ANSI_ARGS_(
(Listbox * listPtr, int fontChanged, int maxIsStale, int updateGrid));
static void ListboxEventProc _ANSI_ARGS_((ClientData clientData,
                                          XEvent *eventPtr));
static int ListboxFetchSelection
...
static void
ListboxRedrawRange _ANSI_ARGS_((Listbox * listPtr, int first, int last));
static void ListboxScanTo _ANSI_ARGS_((Listbox * listPtr, int x, int y));
static void ListboxSelect
_ANSI_ARGS_((Listbox * listPtr, int first, int last, int select));
...
static int ListboxWidgetCmd _ANSI_ARGS_(
(ClientData clientData, Tcl_Interp *interp, int argc, char **argv));
static int NearestListboxElement _ANSI_ARGS_((Listbox * listPtr, int y));
...
char **argv;           /* Argument strings. */
{
    ...
    Tk_CreateEventHandler(listPtr->tkwin,
                          ...
                          ListboxEventProc,
                          ( ClientData ) listPtr);
    Tk_CreateSelHandler(listPtr->tkwin,
                        XA_PRIMARY,
                        XA_STRING,
...
static void DestroyListbox(memPtr) char *memPtr; /* Info about listbox widget.
                                                  */
{
...
static void DisplayListbox(clientData)
ClientData clientData; /* Information about window. */
{
    ...
    pixmap = Tk_GetPixmap(listPtr->display,
                          Tk_WindowId(tkwin),
                          Tk_Width(tkwin),
                          Tk_Height(tkwin),
                          Tk_Depth(tkwin));
    ...
    for (elPtr = listPtr->firstPtr, i = 0; (elPtr != NULL) && (i <= limit);
        ...
        if (elPtr->selected) {
            ...
            if (right == 0) {
                Tk_3DVerticalBevel(tkwin,
                                   pixmap,
                                   listPtr->selBorder,
                                   x + width - listPtr->selBorderWidth,
                                   y,
                                   listPtr->selBorderWidth,
                                   listPtr->lineHeight,
                                   0,
        ...
        x = listPtr->inset + listPtr->selBorderWidth - elPtr->lBearing
            - listPtr->xOffset;
        XDrawString(
        listPtr->display, pixmap, gc, x, y, elPtr->text, elPtr->textLength);
...
static void ListboxCmdDeletedProc(clientData)
...
static void ChangeListboxView(listPtr,
                              index) Listbox *listPtr; /* Information about
                                                          widget. */
...
static void ChangeListboxOffset(listPtr,
                                offset) Listbox *listPtr; /* Information about
                                                             widget. */
...
static void ListboxScanTo(listPtr, x, y) Listbox *listPtr; /* Information
                                                              about widget. */
...
static int NearestListboxElement(listPtr,
                                 y) Listbox *listPtr; /* Information about
                                                         widget. */
...
static void ListboxLostSelection(clientData)
ClientData clientData; /* Information about listbox widget. */
{
    Listbox *listPtr = ( Listbox * ) clientData;
...
/* ARGSUSED */
static void
ListboxRedrawRange(listPtr, first, last) Listbox *listPtr; /* Information
                                                              about widget. */
...
static void ListboxUpdateVScrollbar(
listPtr) Listbox *listPtr; /* Information about widget. */
{
    char string[100];
    double first, last;
    int result;
    ...
    result
    = Tcl_VarEval(interp, listPtr->yScrollCmd, string, ( char * ) NULL);
    if (result != TCL_OK) {
        Tcl_AddErrorInfo(
...
static void ListboxUpdateHScrollbar(
listPtr) Listbox *listPtr; /* Information about widget. */
{
    char string[60];
    ...
    Tcl_Preserve(( ClientData ) interp);
    result
    = Tcl_VarEval(interp, listPtr->xScrollCmd, string, ( char * ) NULL);
    if (result != TCL_OK) {
        Tcl_AddErrorInfo(
