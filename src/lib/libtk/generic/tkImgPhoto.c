 ...
 */
static Tk_ConfigSpec configSpecs[] = { { TK_CONFIG_STRING,
                                         "-data",
                                         ( char * )NULL,
                                         ( char * )NULL,
                                         ( char * )NULL,
                                         Tk_Offset(PhotoMaster, dataString),
                                         TK_CONFIG_NULL_OK },
                                       { TK_CONFIG_STRING,
                                         "-format",
                                         ( char * )NULL,
                                       ...
                                       { TK_CONFIG_STRING,
                                         ...
                                         ( char * )NULL,
                                         ( char * )NULL,
                                         Tk_Offset(PhotoMaster, fileString),
                                         TK_CONFIG_NULL_OK },
                                       { TK_CONFIG_DOUBLE,
                                         "-gamma",
                                         ( char * )NULL,
                                       ...
                                       { TK_CONFIG_INT,
                                         ...
                                         ( char * )NULL,
                                         DEF_PHOTO_HEIGHT,
                                         Tk_Offset(PhotoMaster, userHeight),
                                         0 },
                                       { TK_CONFIG_UID,
                                         "-palette",
                                         ( char * )NULL,
                                       ...
                                       { TK_CONFIG_INT,
                                         ...
                                         ( char * )NULL,
                                         DEF_PHOTO_WIDTH,
                                         Tk_Offset(PhotoMaster, userWidth),
                                         0 },
                                       { TK_CONFIG_END,
                                         ( char * )NULL,
                                         ( char * )NULL,
...
static int ImgPhotoCmd _ANSI_ARGS_(
(ClientData clientData, Tcl_Interp *interp, int argc, char **argv));
static int ParseSubcommandOptions _ANSI_ARGS_((struct SubcommandOptions
                                               * optPtr,
                                               Tcl_Interp *interp,
                                               int allowedOptions,
                                               int *indexPtr,
                                               int argc,
                                               char **argv));
static void ImgPhotoCmdDeletedProc _ANSI_ARGS_((ClientData clientData));
static int ImgPhotoConfigureMaster _ANSI_ARGS_((Tcl_Interp * interp,
                                                PhotoMaster *masterPtr,
                                                int argc,
                                                char **argv,
                                                int flags));
static void ImgPhotoConfigureInstance _ANSI_ARGS_((PhotoInstance
                                                   * instancePtr));
static void
ImgPhotoSetSize _ANSI_ARGS_((PhotoMaster * masterPtr, int width, int height));
static void ImgPhotoInstanceSetSize _ANSI_ARGS_((PhotoInstance * instancePtr));
...
static int ReclaimColors _ANSI_ARGS_((ColorTableId * id, int numColors));
...
static int MatchStringFormat _ANSI_ARGS_((Tcl_Interp * interp,
                                          ...
                                          int *heightPtr));
static void Dither
_ANSI_ARGS_((PhotoMaster * masterPtr, int x, int y, int width, int height));
static void DitherInstance _ANSI_ARGS_(
(PhotoInstance * instancePtr, int x, int y, int width, int height));
...
void Tk_CreatePhotoImageFormat(formatPtr) Tk_PhotoImageFormat *formatPtr;
 ...
 * to Tk_CreatePhotoImageFormat previously. */
{
    ...
    *copyPtr = *formatPtr;
    copyPtr->name
    = ( char * )ckalloc(( unsigned )(strlen(formatPtr->name) + 1));
                            ...
                            * it will be returned in later callbacks. */
{
    ...
    masterPtr->imageCmd = Tcl_CreateCommand(interp,
                                            name,
                                            ImgPhotoCmd,
...
char **argv;           /* Argument strings. */
{
             ...
             && (strncmp(argv[1], "configure", length) == 0))
    {
        ...
        return ImgPhotoConfigureMaster(
        interp, masterPtr, argc - 2, argv + 2, TK_CONFIG_ARGV_ONLY);
    }
    else if ((c == 'c') && (length >= 3)
             && (strncmp(argv[1], "copy", length) == 0))
    {
        ...
        options.name = NULL;
        if (ParseSubcommandOptions(&options,
                                   interp,
        ...
        if (((options.options & OPT_TO) == 0) || (options.toX2 < 0))
        {
            ...
            {
                width
                = (width - options.subsampleX - 1) / -options.subsampleX;
            }
            ...
            {
                height
                = (height + options.subsampleY - 1) / options.subsampleY;
            }
        ...
        block.pixelPtr
        += options.fromX * block.pixelSize + options.fromY * block.pitch;
        block.width = options.fromX2 - options.fromX;
        block.height = options.fromY2 - options.fromY;
        Tk_PhotoPutZoomedBlock(( Tk_PhotoHandle )masterPtr,
                               &block,
                               options.toX,
                               options.toY,
    ...
    else if ((c == 'g') && (strncmp(argv[1], "get", length) == 0))
    {
        ...
        sprintf(string, "%d %d %d", pixelPtr[0], pixelPtr[1], pixelPtr[2]);
    ...
    else if ((c == 'p') && (strncmp(argv[1], "put", length) == 0))
    {
        ...
        memset(( VOID * )&options, 0, sizeof(options));
        options.name = NULL;
        if (ParseSubcommandOptions(
            &options, interp, OPT_TO, &index, argc, argv)
            != TCL_OK)
        {
            return TCL_ERROR;
        }
        ...
        for (y = 0; y < dataHeight; ++y)
        {
            ...
            {
                dataWidth = listArgc;
                pixelPtr = ( unsigned char * )ckalloc(( unsigned )dataWidth
                                                      * dataHeight * 3);
            ...
            {
                if (!XParseColor(Tk_Display(tkwin),
                                 Tk_Colormap(tkwin),
                                 listArgv[x],
                                 &color))
                {
        ...
        block.offset[1] = 1;
        block.offset[2] = 2;
        Tk_PhotoPutBlock(( ClientData )masterPtr,
                         &block,
                         options.toX,
                         options.toY,
                         options.toX2 - options.toX,
             ...
             && (strncmp(argv[1], "read", length) == 0))
    {
        ...
        {
            ImgPhotoSetSize(
            masterPtr, options.toX + width, options.toY + height);
        }
        ...
        result = (*imageFormat->fileReadProc)(interp,
                                              f,
                                              options.name,
                                              options.format,
             ...
             && (strncmp(argv[1], "redither", length) == 0))
    {
        ...
        if (argc == 2)
        {
            ...
            {
                /*
                 * Tell the core image code that part of the image has
                 * changed.
                 */
    ...
    else if ((c == 'w') && (strncmp(argv[1], "write", length) == 0))
    {
        ...
        options.format = NULL;
        if (ParseSubcommandOptions(
            &options, interp, OPT_FORMAT | OPT_FROM, &index, argc, argv)
            != TCL_OK)
        {
            return TCL_ERROR;
        }
             ...
             imageFormat = imageFormat->nextPtr)
        {
            if ((options.format == NULL)
                || (strncasecmp(options.format,
                                imageFormat->name,
                                strlen(imageFormat->name))
                    == 0))
            {
                matched = 1;
        ...
        block.width = options.fromX2 - options.fromX;
        block.height = options.fromY2 - options.fromY;
        return (*imageFormat->fileWriteProc)(
        interp, options.name, options.format, &block);
    }
    else
    {
        Tcl_AppendResult(
...
static int ParseSubcommandOptions(optPtr,
                                  interp,
                                  allowedOptions,
                                  optIndexPtr,
                                  argc,
                                  argv) struct SubcommandOptions *optPtr;
                         ...
                         * such as TK_CONFIG_ARGV_ONLY. */
{
        ...
        && (masterPtr->fileString != oldFileString))
    {
        ...
        realFileName
        = Tcl_TranslateFileName(interp, masterPtr->fileString, &buffer);
        if (realFileName == NULL)
        {
            return TCL_ERROR;
        }
...
static void ImgPhotoConfigureInstance(instancePtr)
...
static ClientData
ImgPhotoGet(tkwin, masterData) Tk_Window tkwin; /* Window in which the
                        ...
                        * image. */
{
         ...
         instancePtr = instancePtr->nextPtr)
    {
            ...
            && (Tk_Display(tkwin) == instancePtr->display))
        {
            ...
            if (instancePtr->refCount == 0)
            {
                ...
                Tcl_CancelIdleCall(DisposeInstance,
    ...
    gcValues.foreground
    = (white != NULL) ? white->pixel : WhitePixelOfScreen(Tk_Screen(tkwin));
    gcValues.background
    = (black != NULL) ? black->pixel : BlackPixelOfScreen(Tk_Screen(tkwin));
    gcValues.graphics_exposures = False;
    instancePtr->gc = Tk_GetGC(
    tkwin, GCForeground | GCBackground | GCGraphicsExposures, &gcValues);
    ...
    {
        DitherInstance(
        instancePtr, validBox.x, validBox.y, validBox.width, validBox.height);
    }
...
static void ImgPhotoDisplay(clientData,
                            display,
                            drawable,
                            imageX,
                            imageY,
                           ...
                           * correspond to imageX and imageY. */
{
    ...
    TkSetRegion(
    display, instancePtr->gc, instancePtr->masterPtr->validRegion);
    XSetClipOrigin(
...
static void ImgPhotoDelete(masterData)
...
static void ImgPhotoCmdDeletedProc(clientData)
...
static void ImgPhotoSetSize(masterPtr, width, height) PhotoMaster *masterPtr;
int width, height;
{
        ...
        || (validBox.y + validBox.height > ( unsigned )height))
    {
        ...
        TkIntersectRegion(
        masterPtr->validRegion, clipRegion, masterPtr->validRegion);
...
static void ImgPhotoInstanceSetSize(instancePtr)
                             ...
                             * changed. */
{
        ...
        || (instancePtr->error == NULL))
    {
                ...
                || (validBox.width == masterPtr->width)))
        {
            ...
            {
                memset(( VOID * )(newError + h * masterPtr->width * 3),
                       0,
...
char *palette;              /* Palette specification string. */
{
    ...
    if ((endp == palette) || ((*endp != 0) && (*endp != '/')) || (nRed < 2)
        || (nRed > 256))
    {
        return 0;
    }
    ...
    else
    {
        ...
        if ((endp == palette) || (*endp != 0) || (nBlue < 2) || (nBlue > 256))
        {
            return 0;
        }
        mono = 0;
...
static int CountBits(mask) pixel mask; /* Value to count the 1 bits in. */
{
    int n;
...
static void GetColorTable(instancePtr)
...
static void FreeColorTable(colorPtr)
...
static void AllocateColors(colorPtr)
...
#define CGFRAC(i, n, g) (( int )(65535 * pow(( double )(i) / (n), (g))))
    ...
    mono
    = sscanf(colorPtr->id.palette, "%d/%d/%d", &nRed, &nGreen, &nBlue) <= 1;
    ...
    for (;;)
    {
        ...
        pixels
        = ( unsigned long * )ckalloc(numColors * sizeof(unsigned long));
        for (i = 0; i < numColors; ++i)
        {
            if (!XAllocColor(
                colorPtr->id.display, colorPtr->id.colormap, &colors[i]))
            {
        ...
        XFreeColors(
        colorPtr->id.display, colorPtr->id.colormap, pixels, i, 0);
        ckfree(( char * )colors);
        ckfree(( char * )pixels);
...
static void DisposeColorTable(clientData)
                        ...
                        * colors are to be released. */
{
    ...
    if (colorPtr->pixelMap != NULL)
    {
        if (colorPtr->numColors > 0)
        {
            XFreeColors(colorPtr->id.display,
                        colorPtr->id.colormap,
                        colorPtr->pixelMap,
                        colorPtr->numColors,
                        0);
...
static void DisposeInstance(clientData)
...
static int MatchFileFormat(interp,
                           f,
                           fileName,
                           formatString,
                           imageFormatPtr,
                            ...
                            * returned here. */
{
         ...
         formatPtr = formatPtr->nextPtr)
    {
        ...
        {
            if (strncasecmp(
                formatString, formatPtr->name, strlen(formatPtr->name))
                != 0)
            {
                continue;
            }
        ...
        {
            fseek(f, 0L, SEEK_SET);
            if ((*formatPtr->fileMatchProc)(
                f, fileName, formatString, widthPtr, heightPtr))
            {
                if (*widthPtr < 1)
                {
                    *widthPtr = 1;
...
static int MatchStringFormat(interp,
                             string,
                             formatString,
                             imageFormatPtr,
                             widthPtr,
                            ...
                            * returned here. */
{
         ...
         formatPtr = formatPtr->nextPtr)
    {
        ...
        if ((formatPtr->stringMatchProc != NULL)
            && (*formatPtr->stringMatchProc)(
               string, formatString, widthPtr, heightPtr))
        {
            break;
        }
    }
    ...
    if (formatPtr == NULL)
    {
        ...
        {
            Tcl_AppendResult(
            interp, "couldn't recognize image data", ( char * )NULL);
        }
        return TCL_ERROR;
    }
...
Tk_PhotoHandle Tk_FindPhoto(imageName) char *imageName; /* Name of the desired
                                                           photo image. */
{
    Tcl_HashEntry *entry;
                    ...
                    * to be updated. */
{
    ...
    {
        ImgPhotoSetSize(
        masterPtr, MAX(xEnd, masterPtr->width), MAX(yEnd, masterPtr->height));
    }
    ...
    &rect, masterPtr->validRegion, masterPtr->validRegion);
...
void Tk_PhotoPutZoomedBlock(handle,
                            blockPtr,
                            x,
                            y,
                            width,
...
int width, height;          /* Dimensions of the block to be dithered. */
{
        ...
        || (colorPtr->visualInfo.class == TrueColor))
    {
        ...
        result
        = sscanf(colorPtr->id.palette, "%d/%d/%d", &nRed, &nGreen, &nBlue);
        if ((nRed >= 256)
    ...
    imagePtr->data
    = ( char * )ckalloc(( unsigned )(imagePtr->bytes_per_line * nLines));
        ...
        for (y = yStart; y < yEnd; ++y)
        {
            ...
            if (colorPtr->flags & COLOR_WINDOW)
            {
                ...
                for (x = xStart; x < xEnd; ++x)
                {
                    ...
                    default:
                        XPutPixel(
                        imagePtr, x - xStart, y - yStart, ( unsigned )i);
                    }
                }
            }
...
void Tk_PhotoBlank(handle) Tk_PhotoHandle handle; /* Handle for the image to
                                                     be blanked. */
{
    PhotoMaster *masterPtr;
    PhotoInstance *instancePtr;
    ...
    {
        memset(
        ( VOID * )instancePtr->error,
        0,
        (size_t)(masterPtr->width * masterPtr->height * sizeof(schar)));
    }
...
int width, height;     /* Desired minimum dimensions of the image. */
{
    ...
    if ((width != masterPtr->width) || (height != masterPtr->height))
    {
        ImgPhotoSetSize(masterPtr,
                        MAX(width, masterPtr->width),
