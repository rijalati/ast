                       ...
                       * may have TK_CONFIG_ARGV_ONLY set. */
{
    ...
    for (specPtr = specs; specPtr->type != TK_CONFIG_END; specPtr++) {
        ...
        specPtr->specFlags
        = (specPtr->specFlags & ~TK_CONFIG_OPTION_SPECIFIED) | INIT;
    }
...
static int DoConfig(interp, tkwin, specPtr, value, valueIsUid, widgRec)
Tcl_Interp *interp;     /* Interpreter for error reporting. */
Tk_Window tkwin;        /* Window containing widget (needed to
                         * set up X resources). */
...
static char *FormatConfigInfo(interp, tkwin, specPtr, widgRec)
                             ...
                             * if result is static. */
{
    ...
    *freeProcPtr = NULL;
    ...
    case TK_CONFIG_CUSTOM:
        result
        = (*specPtr->customPtr->printProc)(specPtr->customPtr->clientData,
                                           tkwin,
                                           widgRec,
                                           specPtr->offset,
                                           freeProcPtr);
...
int Tk_ConfigureValue(interp, tkwin, specs, widgRec, argvName, flags)
Tcl_Interp *interp;   /* Interpreter for error reporting. */
Tk_Window tkwin;      /* Window corresponding to widgRec. */
Tk_ConfigSpec *specs; /* Describes legal options. */
char *widgRec;        /* Record whose fields contain current
                       ...
                       * for them to be considered. */
{
    ...
    interp, tkwin, specPtr, widgRec, interp->result, &interp->freeProc);
    return TCL_OK;
}
...
/* ARGSUSED */
void Tk_FreeOptions(specs, widgRec, display, needFlags)
Tk_ConfigSpec *specs; /* Describes legal options. */
char *widgRec;        /* Record whose fields contain current
                       * values for options. */
