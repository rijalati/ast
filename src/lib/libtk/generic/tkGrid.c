 ...
 * with some non-default property.  This limit is used to head off mistakes
 * and denial of service attacks by limiting the amount of storage required.
 */
...
static int
AdjustOffsets _ANSI_ARGS_((int width, int elements, SlotInfo *slotPtr));
static void ArrangeGrid _ANSI_ARGS_((ClientData clientData));
static int CheckSlotData
_ANSI_ARGS_((Gridder * masterPtr, int slot, int slotType, int checkOnly));
...
static void GridStructureProc _ANSI_ARGS_((ClientData clientData,
                                           XEvent *eventPtr));
static void GridLostSlaveProc _ANSI_ARGS_((ClientData clientData,
                                           Tk_Window tkwin));
static void GridReqProc _ANSI_ARGS_((ClientData clientData, Tk_Window tkwin));
...
char **argv;           /* Argument strings. */
{
    ...
    if ((c == 'b') && (strncmp(argv[1], "bbox", length) == 0)) {
         ...
         */
                ...
                && (length >= 3))
               || ((c == 'r')
                   && (strncmp(argv[1], "rowconfigure", length) == 0)
                   && (length >= 2))) {
        Tk_Window master;
        ...
        if ((argc == 4) && (ok == TCL_OK)) {
            sprintf(interp->result,
                    "-minsize %d -pad %d -weight %d",
                    slotPtr[slot].minSize,
                    slotPtr[slot].pad,
                    slotPtr[slot].weight);
            return (TCL_OK);
        } else if (argc == 4) {
            sprintf(
            interp->result, "-minsize %d -pad %d -weight %d", 0, 0, 0);
            return (TCL_OK);
        }

        /*
         * Loop through each option value pair, setting the values as
         * required. If only one option is given, with no value, the current
         * value is returned.
         */
...
static void GridReqProc(clientData,
                        tkwin) ClientData clientData; /* Grid's information
...
static void ArrangeGrid(clientData)
                        ...
                        * are to be re-layed out. */
{
    ...
    realHeight = Tk_Height(masterPtr->tkwin)
                 - 2 * Tk_InternalBorderWidth(masterPtr->tkwin);
    slotPtr->startX
    = AdjustOffsets(realWidth,
                    MAX(slotPtr->columnEnd, slotPtr->columnMax),
                    slotPtr->columnPtr);
    slotPtr->startY = AdjustOffsets(
    realHeight, MAX(slotPtr->rowEnd, slotPtr->rowMax), slotPtr->rowPtr);
                     ...
                     * in pixels,  or 0 (not currently used). */
{
    ...
    for (offset = slot = 0; slot < gridCount; slot++) {
        ...
        for (slavePtr = layoutPtr[slot].binNextPtr; slavePtr != NULL;
            ...
            int span
            = (slotType == COLUMN) ? slavePtr->numCols : slavePtr->numRows;
...
static Gridder *GetGrid(tkwin) Tk_Window tkwin; /* Token for window for which
...
static void SetGridSize(masterPtr) Gridder *masterPtr; /* The geometry master
                                                          for this grid. */
{
...
static void InitMasterData(masterPtr) Gridder *masterPtr;
{
    size_t size;
    if (masterPtr->masterDataPtr == NULL) {
...
static void Unlink(slavePtr) Gridder *slavePtr; /* Window to unlink. */
{
    ...
    if (masterPtr->slavePtr == slavePtr) {
        ...
        for (slavePtr2 = masterPtr->slavePtr;;
             slavePtr2 = slavePtr2->nextPtr) {
            if (slavePtr2 == NULL) {
                     ...
                     * window name. */
{
    ...
    for (numWindows = i = 0; i < argc; i++) {
        ...
        if (firstChar == REL_HORIZ
            && (numWindows == 0 || *argv[i - 1] == REL_SKIP
                || *argv[i - 1] == REL_VERT)) {
            Tcl_AppendResult(interp,
    ...
    for (j = 0; j < numWindows; j++) {
        ...
        for (width = 1;
             width + j < numWindows && *argv[j + width] == REL_VERT;
             width++) {
            /* Null Body */
        }
        ...
        for (match = 0, slavePtr = masterPtr->slavePtr; slavePtr != NULL;
            ...
            if (slavePtr->numCols == width && slavePtr->column == lastColumn
...
static void StickyToString(flags, result) int flags; /* the sticky flags */
...
static int StringToSticky(string) char *string;
{
    int sticky = 0;
    char c;
