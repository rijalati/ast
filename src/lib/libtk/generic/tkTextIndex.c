...
static char *ForwBack _ANSI_ARGS_((char *string, TkTextIndex *indexPtr));
static char *StartEnd _ANSI_ARGS_((char *string, TkTextIndex *indexPtr));
...
TkTextSegment *
TkTextIndexToSeg(indexPtr, offsetPtr) TkTextIndex *indexPtr; /* Text index. */
...
int TkTextSegToOffset(segPtr,
                      linePtr) TkTextSegment *segPtr; /* Segment whose offset
                                                         is desired. */
...
void TkTextPrintIndex(indexPtr,
                      string) TkTextIndex *indexPtr; /* Pointer to index. */
char *string; /* Place to store the position.  Must have
               * at least TK_POS_CHARS characters. */
{
    sprintf(string,
            "%d.%d",
            TkBTreeLineIndex(indexPtr->linePtr) + 1,
            indexPtr->charIndex);
}
...
static char *
ForwBack(string, indexPtr) char *string; /* String to parse for additional
                                          * info about modifier (count and
                                          * units). Points to "+" or "-" that
                                          * starts modifier. */
...
static char *
StartEnd(string, indexPtr) char *string; /* String to parse for additional
                                          * info about modifier (count and
                                          * units). Points to first character
                                          * of modifer word. */
