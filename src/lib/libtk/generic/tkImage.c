...
void Tk_CreateImageType(typePtr)
                        ...
                        * to Tk_CreateImageType previously. */
{
    ...
    *typePtr2 = *typePtr;
    typePtr2->name
    = ( char * ) ckalloc(( unsigned ) (strlen(typePtr->name) + 1));
...
char *Tk_NameOfImage(imageMaster) Tk_ImageMaster imageMaster; /* Token for
                                                                 image. */
{
    ImageMaster *masterPtr = ( ImageMaster * ) imageMaster;
...
noSuchImage:
    Tcl_AppendResult(
    interp, "image \"", name, "\" doesn't exist", ( char * ) NULL);
    return NULL;
}
...
void Tk_FreeImage(image) Tk_Image image; /* Token for image that is no longer
                                          * needed by a widget. */
{
...
static void DeleteImage(masterPtr)
...
void TkDeleteAllImages(mainPtr) TkMainInfo *mainPtr; /* Structure describing
                                                      * application that is
                                                      * going away. */
{
    Tcl_HashSearch search;
