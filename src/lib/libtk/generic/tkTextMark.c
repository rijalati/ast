...
void TkTextMarkSegToIndex(textPtr,
                          markPtr,
...
int TkTextMarkNameToIndex(textPtr,
                          name,
                          indexPtr) TkText *textPtr; /* Text widget containing
                                                        mark. */
...
/*ARGSUSED*/
static int MarkLayoutProc(textPtr,
                          indexPtr,
                          segPtr,
                          offset,
                          maxX,
...
/* ARGSUSED */
void TkTextInsertDisplayProc(chunkPtr,
                             x,
                             y,
                             height,
                             baseline,
                            ...
                            * corresponds to y. */
{
    ...
    if (textPtr->flags & INSERT_ON) {
        Tk_Fill3DRectangle(textPtr->tkwin,
                           dst,
                           textPtr->insertBorder,
                           x - textPtr->insertWidth / 2,
                           y,
                           textPtr->insertWidth,
                           height,
                           textPtr->insertBorderWidth,
                           TK_RELIEF_RAISED);
...
static void MarkCheckProc(markPtr,
                          linePtr) TkTextSegment *markPtr; /* Segment to
                                                              check. */
TkTextLine *linePtr; /* Line containing segment. */
{
    Tcl_HashSearch search;
