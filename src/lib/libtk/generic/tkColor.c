              ...
              * suitable for passing to XParseColor). */
{
    ...
    if (*name != '#')
    {
        ...
        if (XAllocNamedColor(display, nameKey.colormap, name, &screen, &color)
            != 0)
        {
...
XColor *Tk_GetColorByValue(tkwin,
                           colorPtr) Tk_Window tkwin; /* Window where color
                                                         will be used. */
...
GC Tk_GCForColor(colorPtr,
                 drawable) XColor *colorPtr; /* Color for which a GC is
                                              * desired. Must have been
                    ...
                    * allocated). */
{
    ...
    if (tkColPtr->gc == None)
    {
        ...
        tkColPtr->gc = XCreateGC(
        DisplayOfScreen(tkColPtr->screen), drawable, GCForeground, &gcValues);
    }
    return tkColPtr->gc;
}
...
void Tk_FreeColor(colorPtr) XColor *colorPtr; /* Color to be released.  Must
                         ...
                         * color. */
{
    ...
    for (stressPtr = dispPtr->stressPtr;; stressPtr = stressPtr->nextPtr)
    {
        if (stressPtr == NULL)
        {
            ...
            Tk_Display(tkwin), VisualIDMask, &template, &numFound);
            if (numFound < 1)
            {
    ...
    while (1)
    {
        ...
        if (XAllocColor(
            dispPtr->display, colormap, &stressPtr->colorPtr[closest])
            != 0)
        {
