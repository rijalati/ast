...
_ANSI_ARGS_((TkText * textPtr, TkTextSegment *ewPtr, int argc, char **argv));
static void EmbWinDelayedUnmap _ANSI_ARGS_((ClientData clientData));
static int EmbWinDeleteProc _ANSI_ARGS_((TkTextSegment * segPtr,
                                         TkTextLine *linePtr,
                                         int treeGone));
static void EmbWinDisplayProc _ANSI_ARGS_((TkTextDispChunk * chunkPtr,
                                           int x,
                                           int y,
                                           int lineHeight,
                                           int baseline,
                       ...
                       * options. */
{
    ...
    if (Tk_ConfigureWidget(textPtr->interp,
                           textPtr->tkwin,
                           configSpecs,
                           argc,
                           argv,
    ...
    if (oldWindow != ewPtr->body.ew.tkwin) {
        ...
        if (ewPtr->body.ew.tkwin != NULL) {
            ...
            &textPtr->windowTable, Tk_PathName(ewPtr->body.ew.tkwin), &new);
...
XEvent *eventPtr;      /* Describes what just happened. */
{
    ...
    &ewPtr->body.ew.textPtr->windowTable, Tk_PathName(ewPtr->body.ew.tkwin)));
    ewPtr->body.ew.tkwin = NULL;
...
/*ARGSUSED*/
static int EmbWinLayoutProc(textPtr,
                            indexPtr,
                            ewPtr,
                            offset,
                            maxX,
...
static void
EmbWinUndisplayProc(textPtr,
                    chunkPtr) TkText *textPtr; /* Overall information about
                                                * text widget. */
...
static void EmbWinBboxProc(chunkPtr,
                           index,
                           y,
                           lineHeight,
                           baseline,
