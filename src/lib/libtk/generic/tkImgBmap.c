...
static int ImgBmapCmd _ANSI_ARGS_(
(ClientData clientData, Tcl_Interp *interp, int argc, char **argv));
static void ImgBmapCmdDeletedProc _ANSI_ARGS_((ClientData clientData));
static void ImgBmapConfigureInstance _ANSI_ARGS_((BitmapInstance
                                                  * instancePtr));
static int ImgBmapConfigureMaster
_ANSI_ARGS_((BitmapMaster * masterPtr, int argc, char **argv, int flags));
                          ...
                          * such as TK_CONFIG_ARGV_ONLY. */
{
    ...
    if (Tk_ConfigureWidget(masterPtr->interp,
    ...
    if ((masterPtr->maskFileString != NULL)
        ...
        masterPtr->maskData = TkGetBitmapData(masterPtr->interp,
                                              masterPtr->maskDataString,
                                              masterPtr->maskFileString,
                                              &maskWidth,
                                              &maskHeight,
                                              &dummy1,
                                              &dummy2);
        ...
        if ((maskWidth != masterPtr->width)
            ...
            = "bitmap and mask have different sizes";
            return TCL_ERROR;
        }
    }
...
static void ImgBmapConfigureInstance(instancePtr)
...
static int NextBitmapWord(
parseInfoPtr) ParseInfo *parseInfoPtr; /* Describes what we're reading
...
static ClientData
ImgBmapGet(tkwin, masterData) Tk_Window tkwin; /* Window in which the instance
                                                * will be used. */
...
static void ImgBmapDisplay(clientData,
                           display,
                           drawable,
                           imageX,
                           imageY,
...
static void ImgBmapDelete(masterData)
...
static void ImgBmapCmdDeletedProc(clientData)
