...
static XWindowChanges defChanges = { 0, 0, 1, 1, 0, 0, Above };
...
static TkWindow *AllocWindow _ANSI_ARGS_((TkDisplay * dispPtr,
                                          int screenNum,
                                          TkWindow *parentPtr));
static Tk_Window CreateTopLevelWindow _ANSI_ARGS_(
(Tcl_Interp * interp, Tk_Window parent, char *name, char *screenName));
static void DeleteWindowsExitProc _ANSI_ARGS_((ClientData clientData));
static void DoConfigureNotify _ANSI_ARGS_((TkWindow * winPtr));
static TkDisplay *GetScreen _ANSI_ARGS_((Tcl_Interp * interp,
                                         char *screenName,
                                         int *screenPtr));
static int NameWindow _ANSI_ARGS_(
(Tcl_Interp * interp, TkWindow *winPtr, TkWindow *parentPtr, char *name));
                     ...
                     * parent. */
{
    ...
    if (parent != NULL) {
        if (NameWindow(interp, winPtr, ( TkWindow * )parent, name)
            != TCL_OK) {
...
TkDisplay *TkGetDisplay(display) Display *display; /* X's display pointer */
{
    TkDisplay *dispPtr;
...
#define FIXED_SIZE 200
    ...
    hPtr
    = Tcl_CreateHashEntry(&parentPtr->mainPtr->nameTable, pathName, &new);
                     ...
                     * form "prog instance". */
{
    ...
    tkwin
    = CreateTopLevelWindow(interp, ( Tk_Window )NULL, baseName, screenName);
    if (tkwin == NULL) {
        return NULL;
    }
...
Tk_Window Tk_CreateWindow(interp, parent, name, screenName)
Tcl_Interp *interp; /* Interpreter to use for error reporting.
                     ...
                     * window. */
{
    ...
    if (screenName == NULL) {
        winPtr
        = AllocWindow(parentPtr->dispPtr, parentPtr->screenNum, parentPtr);
...
Tk_Window Tk_CreateWindowFromPath(interp, tkwin, pathName, screenName)
Tcl_Interp *interp; /* Interpreter to use for error reporting.
                     * Interp->result is assumed to be
                     * initialized by the caller. */
...
#define FIXED_SPACE 5
    ...
    if (p == NULL) {
        Tcl_AppendResult(
        interp, "bad window path name \"", pathName, "\"", ( char * )NULL);
        return NULL;
    }
    numChars = p - pathName;
    ...
    if (screenName == NULL) {
        ...
        interp, parent, pathName + numChars + 1, screenName);
    }
}
...
void Tk_DestroyWindow(tkwin) Tk_Window tkwin; /* Window to destroy. */
{
    ...
    if (winPtr->window != None) {
#if defined(MAC_TCL) || defined(WIN_TCL)
        ...
        Tcl_DeleteHashEntry(
        Tcl_FindHashEntry(&dispPtr->winTable, ( char * )winPtr->window));
        winPtr->window = None;
    }
    dispPtr->destroyCount--;
    UnlinkWindow(winPtr);
    ...
    if (winPtr->mainPtr != NULL) {
        if (winPtr->pathName != NULL) {
            ...
            Tcl_DeleteHashEntry(
            Tcl_FindHashEntry(&winPtr->mainPtr->nameTable, winPtr->pathName));
        }
        winPtr->mainPtr->refCount--;
        if (winPtr->mainPtr->refCount == 0) {
            ...
            if ((winPtr->mainPtr->interp != NULL)
                ...
                for (cmdPtr = commands; cmdPtr->name != NULL; cmdPtr++) {
                    Tcl_CreateCommand(
                    winPtr->mainPtr->interp,
                    cmdPtr->name,
                    TkDeadAppCmd,
                    ( ClientData )NULL,
                    ( void(*) _ANSI_ARGS_((ClientData)) )NULL);
                }
                Tcl_CreateCommand(winPtr->mainPtr->interp,
                                  "send",
                                  TkDeadAppCmd,
                                  ( ClientData )NULL,
                                  ( void(*) _ANSI_ARGS_((ClientData)) )NULL);
...
void Tk_MapWindow(tkwin) Tk_Window tkwin; /* Token for window to map. */
{
    TkWindow *winPtr = ( TkWindow * )tkwin;
    XEvent event;
...
void Tk_MakeWindowExist(tkwin) Tk_Window tkwin; /* Token for window. */
{
    TkWindow *winPtr = ( TkWindow * )tkwin;
    TkWindow *winPtr2;
    Window parent;
    ...
    &winPtr->dispPtr->winTable, ( char * )winPtr->window, &new);
    Tcl_SetHashValue(hPtr, winPtr);
    winPtr->dirtyAtts = 0;
...
void Tk_UnmapWindow(tkwin) Tk_Window tkwin; /* Token for window to unmap. */
{
    TkWindow *winPtr = ( TkWindow * )tkwin;
...
XWindowChanges *valuePtr; /* New values. */
{
    ...
    if (winPtr->window != None) {
        XConfigureWindow(
        winPtr->display, winPtr->window, valueMask, valuePtr);
        if (!(winPtr->flags & TK_TOP_LEVEL)) {
            DoConfigureNotify(winPtr);
        }
    } else {
...
void Tk_MoveWindow(tkwin, x, y) Tk_Window tkwin; /* Window to move. */
...
void Tk_ResizeWindow(tkwin, width, height) Tk_Window tkwin; /* Window to
                                                               resize. */
...
int width, height; /* New dimensions for window. */
{
    ...
    winPtr->changes.height = ( unsigned )height;
    if (winPtr->window != None) {
        XMoveResizeWindow(winPtr->display,
                          winPtr->window,
                          x,
                          y,
                          ( unsigned )width,
...
void Tk_SetWindowBorderWidth(tkwin,
                             width) Tk_Window tkwin; /* Window to modify. */
int width; /* New border width for window. */
{
    ...
    if (winPtr->window != None) {
        ...
        winPtr->display, winPtr->window, ( unsigned )width);
        if (!(winPtr->flags & TK_TOP_LEVEL)) {
            DoConfigureNotify(winPtr);
        }
    } else {
...
void Tk_ChangeWindowAttributes(tkwin,
                               valueMask,
                               attsPtr) Tk_Window tkwin; /* Window to
                                                            manipulate. */
...
/* New values for some attributes. */
{
    ...
    if (valueMask & CWDontPropagate) {
        winPtr->atts.do_not_propagate_mask = attsPtr->do_not_propagate_mask;
    }
    ...
    if (winPtr->window != None) {
        ...
        winPtr->display, winPtr->window, valueMask, attsPtr);
    } else {
        winPtr->dirtyAtts |= valueMask;
    }
}

void Tk_SetWindowBackground(tkwin, pixel) Tk_Window tkwin; /* Window to
                                                              manipulate. */
                      ...
                      * window's background. */
{
    ...
    if (winPtr->window != None) {
        ...
        winPtr->dirtyAtts
        = (winPtr->dirtyAtts & ( unsigned )~CWBackPixmap) | CWBackPixel;
    }
}

void Tk_SetWindowBackgroundPixmap(tkwin,
                                  pixmap) Tk_Window tkwin; /* Window to
                                                              manipulate. */
                ...
                * background. */
{
    ...
    if (winPtr->window != None) {
        XSetWindowBackgroundPixmap(winPtr->display, winPtr->window, pixmap);
    } else {
        winPtr->dirtyAtts
        = (winPtr->dirtyAtts & ( unsigned )~CWBackPixel) | CWBackPixmap;
    }
}
                                                 ...
                                                 * window's border. */
{
    ...
    if (winPtr->window != None) {
        ...
        winPtr->dirtyAtts
        = (winPtr->dirtyAtts & ( unsigned )~CWBorderPixmap) | CWBorderPixel;
    }
}

void Tk_SetWindowBorderPixmap(tkwin,
                              pixmap) Tk_Window tkwin; /* Window to
                                                          manipulate. */
Pixmap pixmap; /* Pixmap to use for window's
                * border. */
{
    ...
    if (winPtr->window != None) {
        XSetWindowBorderPixmap(winPtr->display, winPtr->window, pixmap);
    } else {
        winPtr->dirtyAtts
        = (winPtr->dirtyAtts & ( unsigned )~CWBorderPixel) | CWBorderPixmap;
    }
}

void Tk_DefineCursor(tkwin,
                     cursor) Tk_Window tkwin; /* Window to manipulate. */
...
void Tk_UndefineCursor(tkwin) Tk_Window tkwin; /* Window to manipulate. */
{
    Tk_DefineCursor(tkwin, None);
}

void Tk_SetWindowColormap(tkwin, colormap) Tk_Window tkwin; /* Window to
                                                               manipulate. */
...
static void DoConfigureNotify(winPtr) TkWindow *winPtr; /* Window whose
                                                         * configuration was
                                                         * just changed. */
{
    XEvent event;
                     ...
                     * belong to the same main window as tkwin. */
{
    ...
    hPtr
    = Tcl_FindHashEntry(&(( TkWindow * )tkwin)->mainPtr->nameTable, pathName);
    if (hPtr == NULL) {
        Tcl_AppendResult(
        interp, "bad window path name \"", pathName, "\"", ( char * )NULL);
        return NULL;
...
char *Tk_DisplayName(tkwin) Tk_Window tkwin; /* Window whose display name is
                                                desired. */
{
...
static void UnlinkWindow(winPtr) TkWindow *winPtr; /* Child window to be
                                                      unlinked. */
{
    TkWindow *prevPtr;
...
Tk_Window
Tk_MainWindow(interp) Tcl_Interp *interp; /* Interpreter that embodies the
...
int Tk_StrictMotif(tkwin) Tk_Window tkwin; /* Window whose application is
                                            * to be checked. */
{
...
static void OpenIM(dispPtr) TkDisplay *dispPtr; /* Tk's structure for the
                                                   display. */
{
    ...
    if ((XGetIMValues(dispPtr->inputMethod, XNQueryInputStyle, &stylePtr, NULL)
         != NULL)
        || (stylePtr == NULL)) {
        goto error;
    }
...
static void
DeleteWindowsExitProc(clientData) ClientData clientData; /* Not used. */
{
...
int Tk_Init(interp) Tcl_Interp *interp; /* Interpreter to initialize. */
{
    char *p;
    int argc, code;
    char **argv, *args[20];
    ...
    if (p != NULL) {
        ...
        if (Tk_ParseArgv(interp,
                         ( Tk_Window )NULL,
                         &argc,
                         argv,
                         argTable,
                         TK_ARGV_DONT_SKIP_FIRST_ARG | TK_ARGV_NO_DEFAULTS)
            != TCL_OK) {
            ckfree(( char * )argv);
            goto argError;
        }
    ...
    if (geometry != NULL) {
        ...
        code
        = Tcl_VarEval(interp, "wm geometry . ", geometry, ( char * )NULL);
        if (code != TCL_OK) {
            goto done;
        }
    }
