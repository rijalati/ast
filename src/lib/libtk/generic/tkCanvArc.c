...
static Tk_ConfigSpec configSpecs[] = { { TK_CONFIG_DOUBLE,
                                         "-extent",
                                         ( char * )NULL,
                                         ( char * )NULL,
                                         "90",
                                         Tk_Offset(ArcItem, extent),
                                       ...
                                       { TK_CONFIG_COLOR,
                                         ...
                                         ( char * )NULL,
                                         "black",
                                         Tk_Offset(ArcItem, outlineColor),
                                         TK_CONFIG_NULL_OK },
                                       { TK_CONFIG_BITMAP,
                                         "-outlinestipple",
                                         ( char * )NULL,
                                         ( char * )NULL,
                                         ( char * )NULL,
                                         Tk_Offset(ArcItem, outlineStipple),
                                         TK_CONFIG_NULL_OK },
                                       { TK_CONFIG_DOUBLE,
                                         "-start",
                                         ( char * )NULL,
...
static void ComputeArcBbox _ANSI_ARGS_((Tk_Canvas canvas, ArcItem *arcPtr));
...
static int CreateArc _ANSI_ARGS_((Tcl_Interp * interp,
                                  ...
                                  char **argv));
static void
DeleteArc _ANSI_ARGS_((Tk_Canvas canvas, Tk_Item *itemPtr, Display *display));
...
static int ArcCoords _ANSI_ARGS_((Tcl_Interp * interp,
                                  ...
                                  char **argv));
static int
ArcToArea _ANSI_ARGS_((Tk_Canvas canvas, Tk_Item *itemPtr, double *rectPtr));
static double ArcToPoint _ANSI_ARGS_((Tk_Canvas canvas,
                                      Tk_Item *itemPtr,
                                      double *coordPtr));
static int ArcToPostscript _ANSI_ARGS_(
...
static void ScaleArc _ANSI_ARGS_((Tk_Canvas canvas,
                                  ...
                                  double scaleX,
                                  double scaleY));
static void TranslateArc _ANSI_ARGS_(
(Tk_Canvas canvas, Tk_Item *itemPtr, double deltaX, double deltaY));
static int
AngleInRange _ANSI_ARGS_((double x, double y, double start, double extent));
static void ComputeArcOutline _ANSI_ARGS_((ArcItem * arcPtr));
static int HorizLineToArc _ANSI_ARGS_((double x1,
                                       double x2,
                                       double y,
                                       double rx,
                                       double ry,
                                       double start,
                                       double extent));
static int VertLineToArc _ANSI_ARGS_((double x,
                                      double y1,
                                      double y2,
                                      double rx,
                                      double ry,
...
static void
DeleteArc(canvas, itemPtr, display) Tk_Canvas canvas; /* Info about overall
                                                         canvas. */
...
/* ARGSUSED */
static void ComputeArcBbox(canvas,
                           arcPtr) Tk_Canvas canvas; /* Canvas that contains
                                                        item. */
    ...
    if (arcPtr->outlineGC != None)
    {
        ...
        if (arcPtr->width <= 2)
        {
            ...
            else if (arcPtr->style == pieSliceUid)
            {
                ...
                (arcPtr->bbox[1] + arcPtr->bbox[3]) / 2.0,
                &cx,
                &cy);
                XDrawLine(
                display, drawable, arcPtr->outlineGC, cx, cy, x1, y1);
                XDrawLine(
...
/* ARGSUSED */
static double
ArcToPoint(canvas, itemPtr, pointPtr) Tk_Canvas canvas; /* Canvas containing
                                                           item. */
                   ...
                   * area.  */
{
    ...
    if (HorizLineToArc(
        tRect[0], tRect[2], tRect[1], rx, ry, arcPtr->start, arcPtr->extent)
        || HorizLineToArc(
...
static void ComputeArcOutline(arcPtr) ArcItem *arcPtr; /* Information about
                                                          arc. */
{
    ...
    if (arcPtr->style == chordUid)
    {
        ...
        TkGetButtPoints(arcPtr->center2,
                        ...
                        outlinePtr + 2);
        outlinePtr[4]
        = arcPtr->center2[0] + outlinePtr[2] - arcPtr->center1[0];
        outlinePtr[5]
        = arcPtr->center2[1] + outlinePtr[3] - arcPtr->center1[1];
    ...
    else if (arcPtr->style == pieSliceUid)
    {
        ...
        TkGetButtPoints(arcPtr->center1,
                        vertex,
                        ( double )arcPtr->width,
                        0,
                        outlinePtr,
