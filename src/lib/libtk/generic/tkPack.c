...
static void PackReqProc _ANSI_ARGS_((ClientData clientData, Tk_Window tkwin));
...
_ANSI_ARGS_((Tcl_Interp * interp, Tk_Window tkwin, int argc, char *argv[]));
...
static int XExpansion _ANSI_ARGS_((Packer * slavePtr, int cavityWidth));
static int YExpansion _ANSI_ARGS_((Packer * slavePtr, int cavityHeight));
...
char **argv;           /* Argument strings. */
{
    ...
    if ((c == 'a') && (length >= 2)
        ...
        if (prevPtr->masterPtr == NULL) {
            Tcl_AppendResult(
            interp, "window \"", argv[2], "\" isn't packed", ( char * ) NULL);
            return TCL_ERROR;
        }
        return PackAfter(
        interp, prevPtr, prevPtr->masterPtr, argc - 3, argv + 3);
        ...
        sprintf(buffer,
                ...
                slavePtr->padY / 2);
        Tcl_AppendResult(
        interp, buffer, " -side ", sideNames[slavePtr->side], ( char * ) NULL);
        ...
        if ((packPtr != NULL) && (packPtr->masterPtr != NULL)) {
            Tk_ManageGeometry(
            tkwin2, ( Tk_GeomMgr * ) NULL, ( ClientData ) NULL);
...
/* ARGSUSED */
static void PackReqProc(clientData,
                        tkwin) ClientData clientData; /* Packer's information
                                                       * about window that got
                                                       * new preferred
                                                       * geometry.  */
...
static void ArrangePacking(clientData)
                        ...
                        * are to be re-layed out. */
{
    ...
    for (slavePtr = masterPtr->slavePtr; slavePtr != NULL;
        ...
        width
        = Tk_ReqWidth(slavePtr->tkwin) + slavePtr->doubleBw + slavePtr->iPadX;
        if ((slavePtr->flags & FILLX) || (width > (frameWidth - borderX))) {
            width = frameWidth - borderX;
        }
        height = Tk_ReqHeight(slavePtr->tkwin) + slavePtr->doubleBw
                 + slavePtr->iPadY;
        if ((slavePtr->flags & FILLY) || (height > (frameHeight - borderY))) {
        ...
        if (masterPtr->tkwin == Tk_Parent(slavePtr->tkwin)) {
            ...
            if ((width <= 0) || (height <= 0)) {
                ...
                Tk_MaintainGeometry(
                slavePtr->tkwin, masterPtr->tkwin, x, y, width, height);
            }
        }
...
static int XExpansion(slavePtr,
                      cavityWidth) Packer *slavePtr; /* First in list of
                                                      * remaining slaves. */
...
static int YExpansion(slavePtr,
                      cavityHeight) Packer *slavePtr; /* First in list of
                                                       * remaining slaves. */
...
static Packer *GetPacker(tkwin) Tk_Window tkwin; /* Token for window for which
                                                  * packer structure is
                                                  * desired. */
{
    Packer *packPtr;
    ...
    tkwin, StructureNotifyMask, PackStructureProc, ( ClientData ) packPtr);
    return packPtr;
}
                     ...
                     * against which to pack. */
{
    ...
    for (; argc > 0; argc -= 2, argv += 2, prevPtr = packPtr) {
        ...
        if (Tcl_SplitList(interp, argv[1], &optionCount, &options)
            != TCL_OK) {
            return TCL_ERROR;
        }
        packPtr->side = TOP;
        ...
        for (index = 0; index < optionCount; index++) {
            ...
            if ((c == 't') && (strncmp(curOpt, "top", length)) == 0) {
                ...
                if (Tk_GetAnchor(interp, options[index + 1], &packPtr->anchor)
                    != TCL_OK) {
                    goto error;
                }
                index++;
...
static void Unlink(packPtr) Packer *packPtr; /* Window to unlink. */
{
...
static void DestroyPacker(memPtr) char *memPtr; /* Info about packed window
                                                 * that is now dead. */
{
...
XEvent *eventPtr;      /* Describes what just happened. */
{
    ...
    if (eventPtr->type == ConfigureNotify) {
        ...
        if (packPtr->doubleBw
            != 2 * Tk_Changes(packPtr->tkwin)->border_width) {
            if ((packPtr->masterPtr != NULL)
                && !(packPtr->masterPtr->flags & REQUESTED_REPACK)) {
                packPtr->doubleBw
                = 2 * Tk_Changes(packPtr->tkwin)->border_width;
                packPtr->masterPtr->flags |= REQUESTED_REPACK;
                Tcl_DoWhenIdle(ArrangePacking,
                               ( ClientData ) packPtr->masterPtr);
            }
        }
        ...
        for (slavePtr = packPtr->slavePtr; slavePtr != NULL;
             slavePtr = nextPtr) {
            Tk_ManageGeometry(
            slavePtr->tkwin, ( Tk_GeomMgr * ) NULL, ( ClientData ) NULL);
            Tk_UnmapWindow(slavePtr->tkwin);
        ...
        Tcl_DeleteHashEntry(
        Tcl_FindHashEntry(&packerHashTable, ( char * ) packPtr->tkwin));
        if (packPtr->flags & REQUESTED_REPACK) {
                     ...
                     * window name. */
{
    ...
    for (j = 0; j < numWindows; j++) {
            ...
            if ((c == 'a') && (strncmp(argv[i], "-after", length) == 0)
                ...
                if (j == 0) {
                    ...
                    positionGiven = 1;
                       ...
                       && (strncmp(argv[i], "-anchor", length) == 0)
                       && (length >= 2)) {
        ...
        if (slave == masterPtr->tkwin) {
            Tcl_AppendResult(
            interp, "can't pack ", argv[j], " inside itself", ( char * ) NULL);
            return TCL_ERROR;
        }
