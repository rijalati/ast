...
void Tk_GeometryRequest(tkwin,
                        reqWidth,
                        reqHeight) Tk_Window tkwin; /* Window that geometry
...
void Tk_SetInternalBorder(tkwin,
                          width) Tk_Window tkwin; /* Window that will have
                                                     internal border. */
...
gotSlave:
    ...
    for (ancestor = slavePtr->master;; ancestor = Tk_Parent(ancestor)) {
        ...
        if (ancestor == parent) {
            if ((x != Tk_X(slavePtr->slave)) || (y != Tk_Y(slavePtr->slave))
...
void Tk_UnmaintainGeometry(slave,
                           master) Tk_Window slave; /* Slave for geometry
                                                       management. */
Tk_Window master; /* Master for slave; must be a descendant
                   * of slave's parent. */
{
...
static void MaintainCheckProc(clientData)
ClientData clientData; /* Pointer to MaintainMaster structure
                        * for the master window. */
{
    MaintainMaster *masterPtr = ( MaintainMaster * )clientData;
