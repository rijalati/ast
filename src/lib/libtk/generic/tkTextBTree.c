...
#define CSEG_SIZE(chars)                                                     \
    (( unsigned )(Tk_Offset(TkTextSegment, body) + 1 + (chars)))
#define TSEG_SIZE                                                            \
    (( unsigned )(Tk_Offset(TkTextSegment, body) + sizeof(TkTextToggle)))
...
static void
IncCount _ANSI_ARGS_((TkTextTag * tagPtr, int inc, TagInfo *tagInfoPtr));
...
TkTextBTree TkBTreeCreate(textPtr) TkText *textPtr;
{
    BTree *treePtr;
    Node *rootPtr;
...
void TkBTreeDestroy(tree) TkTextBTree tree; /* Pointer to tree to delete. */
{
...
static void DestroyNode(nodePtr) Node *nodePtr;
{
    if (nodePtr->level == 0) {
        TkTextLine *linePtr;
        TkTextSegment *segPtr;
...
static void DeleteSummaries(summaryPtr) Summary *summaryPtr; /* First in list
                                                              * of node's tag
                                                              * summaries. */
{
    Summary *nextPtr;
...
void TkBTreeInsertChars(indexPtr, string)
TkTextIndex *indexPtr; /* Indicates where to insert text.
...
SplitSeg(indexPtr) TkTextIndex *indexPtr; /* Index identifying position
                                           * at which to split a segment. */
{
    TkTextSegment *prevPtr, *segPtr;
    int count;

    for (count = indexPtr->charIndex,
        prevPtr = NULL,
        segPtr = indexPtr->linePtr->segPtr;
         segPtr != NULL;
...
static void CleanupLine(linePtr) TkTextLine *linePtr; /* Line to be cleaned
                                                         up. */
{
    ...
    while (1) {
        ...
        for (prevPtrPtr = &linePtr->segPtr, segPtr = *prevPtrPtr;
            ...
            if (segPtr->typePtr->cleanupProc != NULL) {
                *prevPtrPtr
                = (*segPtr->typePtr->cleanupProc)(segPtr, linePtr);
...
void TkBTreeDeleteChars(index1Ptr, index2Ptr)
TkTextIndex *index1Ptr; /* Indicates first character that is
                         * to be deleted. */
                         ...
                         * last one that is to be deleted. */
{
    ...
    if (index1Ptr->linePtr != index2Ptr->linePtr) {
        ...
        for (segPtr = lastPtr; segPtr != NULL; segPtr = segPtr->nextPtr) {
            if (segPtr->typePtr->lineChangeProc != NULL) {
                (*segPtr->typePtr->lineChangeProc)(segPtr,
                                                   index2Ptr->linePtr);
            }
        }
...
TkTextLine *TkBTreeFindLine(tree, line) TkTextBTree tree; /* B-tree in which
                                                             to find line. */
...
TkTextLine *TkBTreeNextLine(linePtr) TkTextLine *linePtr; /* Pointer to
                                                           * existing line in
                                                           * B-tree. */
{
    Node *nodePtr;
...
TkTextLine *TkBTreePreviousLine(linePtr) TkTextLine *linePtr; /* Pointer to
                                                               * existing line
                                                               * in B-tree. */
{
    Node *nodePtr;
...
int TkBTreeLineIndex(linePtr) TkTextLine *linePtr; /* Pointer to existing line
                                                    * in B-tree. */
{
    TkTextLine *linePtr2;
...
/* ARGSUSED */
void TkBTreeLinkSegment(segPtr, indexPtr)
...
void TkBTreeTag(index1Ptr, index2Ptr, tagPtr, add)
TkTextIndex *index1Ptr; /* Indicates first character in
                         * range. */
TkTextIndex *index2Ptr; /* Indicates character just after the
                         * last one in range. */
...
static void ChangeNodeToggleCount(nodePtr,
                                  tagPtr,
                                  delta) Node *nodePtr; /* Node whose toggle
                                                         * count for a tag
                                                         * must be changed. */
                    ...
                    * count for tag (may be negative). */
{
    ...
    while (nodePtr->level > 0) {
        ...
        for (node2Ptr = nodePtr->children.nodePtr; node2Ptr != ( Node * )NULL;
             node2Ptr = node2Ptr->nextPtr) {
...
int TkBTreeNextTag(searchPtr)
TkTextSearch *searchPtr; /* Information about search in
...
int TkBTreeCharTagged(indexPtr,
...
#define NUM_TAG_INFOS 10
    ...
    tagInfo.tagPtrs = ( TkTextTag ** )ckalloc(( unsigned )NUM_TAG_INFOS
                                              * sizeof(TkTextTag *));
    tagInfo.counts
    = ( int * )ckalloc(( unsigned )NUM_TAG_INFOS * sizeof(int));
        ...
        for (siblingPtr = nodePtr->parentPtr->children.nodePtr;
            ...
            for (summaryPtr = siblingPtr->summaryPtr; summaryPtr != NULL;
                ...
                if (summaryPtr->toggleCount & 1) {
                    IncCount(
                    summaryPtr->tagPtr, summaryPtr->toggleCount, &tagInfo);
                }
            }
        }
    }
                      ...
                      * increment count here. */
{
    ...
    if (tagInfoPtr->numTags == tagInfoPtr->arraySize) {
        ...
        newTags = ( TkTextTag ** )ckalloc(
        ( unsigned )(newSize * sizeof(TkTextTag *)));
        memcpy(( VOID * )newTags,
               ( VOID * )tagInfoPtr->tagPtrs,
        ...
        memcpy(( VOID * )newCounts,
               ( VOID * )tagInfoPtr->counts,
...
void TkBTreeCheck(tree) TkTextBTree tree; /* Tree to check. */
{
    BTree *treePtr = ( BTree * )tree;
    Summary *summaryPtr;
    Node *nodePtr;
    ...
    for (entryPtr = Tcl_FirstHashEntry(&treePtr->textPtr->tagTable, &search);
        ...
        if (nodePtr == ( Node * )NULL) {
            if (tagPtr->toggleCount != 0) {
                panic(
                "TkBTreeCheck found \"%s\" with toggles (%d) but no root",
                tagPtr->name,
                tagPtr->toggleCount);
            }
        ...
        if (count != tagPtr->toggleCount) {
            panic(
            "TkBTreeCheck toggleCount (%d) wrong for \"%s\" should be (%d)",
            tagPtr->toggleCount,
            tagPtr->name,
            count);
        }
...
static void CheckNodeConsistency(nodePtr) Node *nodePtr; /* Node whose subtree
                                                          * should be
                                                          * checked. */
{
    Node *childNodePtr;
        ...
        for (linePtr = nodePtr->children.linePtr; linePtr != NULL;
            ...
            for (segPtr = linePtr->segPtr; segPtr != NULL;
                ...
                if ((segPtr->size == 0) && (!segPtr->typePtr->leftGravity)
                    ...
                    panic(
                    "CheckNodeConsistency: wrong segment order for gravity");
                }
                if ((segPtr->nextPtr == NULL)
                    && (segPtr->typePtr != &tkTextCharType)) {
...
static void Rebalance(treePtr, nodePtr) BTree *treePtr; /* Tree that is being
                                                           rebalanced. */
...
static void RecomputeNodeCounts(
nodePtr) Node *nodePtr; /* Node whose tag summary information
                         * must be recomputed. */
{
    Summary *summaryPtr, *summaryPtr2;
    Node *childPtr;
     ...
     * have no summary information, and they become the tagRootPtr for the
     * tag.
     */
...
int TkBTreeNumLines(tree) TkTextBTree tree; /* Information about tree. */
{
    BTree *treePtr = ( BTree * )tree;
                        ...
                        * used). */
{
    ...
    newPtr
    = ( TkTextSegment * )ckalloc(CSEG_SIZE(segPtr->size + segPtr2->size));
...
/* ARGSUSED */
static void
CharCheckProc(segPtr, linePtr) TkTextSegment *segPtr; /* Segment to check. */
TkTextLine *linePtr; /* Line containing segment. */
{
    /*
    ...
    if (segPtr->nextPtr == NULL) {
        ...
        if (segPtr->nextPtr->typePtr == &tkTextCharType) {
            panic(
            "CharCheckProc: adjacent character segments weren't merged");
        }
    }
}
                        ...
                        * get cleaned up. */
{
    ...
    if (segPtr->body.toggle.inNodeCounts) {
        ChangeNodeToggleCount(
        linePtr->parentPtr, segPtr->body.toggle.tagPtr, -1);
        segPtr->body.toggle.inNodeCounts = 0;
    }
    return 1;
}
...
TkTextLine *linePtr;   /* Line that now contains segment. */
{
    ...
    if (segPtr->typePtr == &tkTextToggleOffType) {
        for (prevPtr = segPtr, segPtr2 = prevPtr->nextPtr;
            ...
            if (counts != 0) {
                ChangeNodeToggleCount(
                linePtr->parentPtr, segPtr->body.toggle.tagPtr, -counts);
            }
...
static void ToggleLineChangeProc(segPtr,
                                 linePtr) TkTextSegment *segPtr; /* Segment to
                                                                    check. */
...
static void ToggleCheckProc(segPtr,
                            linePtr) TkTextSegment *segPtr; /* Segment to
                                                               check. */
TkTextLine *linePtr; /* Line containing segment. */
{
    ...
    needSummary
    = (segPtr->body.toggle.tagPtr->tagRootPtr != linePtr->parentPtr);
...
int TkBTreeCharsInLine(linePtr) TkTextLine *linePtr; /* Line whose characters
                                                      * should be counted. */
{
    TkTextSegment *segPtr;
    int count;
