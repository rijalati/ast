...
static int FileMatchGIF _ANSI_ARGS_(
...
static int DoExtension _ANSI_ARGS_((FILE * fd, int label, int *transparent));
static int GetCode _ANSI_ARGS_((FILE * fd, int code_size, int flag));
static int GetDataBlock _ANSI_ARGS_((FILE * fd, unsigned char *buf));
static int LWZReadByte _ANSI_ARGS_((FILE * fd, int flag, int input_code_size));
static int ReadColorMap _ANSI_ARGS_(
(FILE * fd, int number, unsigned char buffer[3][MAXCOLORMAPSIZE]));
static int
ReadGIFHeader _ANSI_ARGS_((FILE * f, int *widthPtr, int *heightPtr));
...
static int ReadGIFHeader(f, widthPtr, heightPtr) FILE *f; /* Image file to
                                                             read the header
                                                             from */
int *widthPtr, *heightPtr; /* The dimensions of the image are
                            * returned here. */
...
static int ReadColorMap(fd, number, buffer) FILE *fd;
int number;
...
static int DoExtension(fd, label, transparent) FILE *fd;
int label;
int *transparent;
{
...
static int GetDataBlock(fd, buf) FILE *fd;
unsigned char *buf;
{
    unsigned char count;
...
static int LWZReadByte(fd, flag, input_code_size) FILE *fd;
int flag;
int input_code_size;
{
    static int fresh = 0;
...
static int GetCode(fd, code_size, flag) FILE *fd;
int code_size;
int flag;
{
