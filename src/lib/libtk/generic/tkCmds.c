...
static Tk_Window
GetDisplayOf _ANSI_ARGS_((Tcl_Interp * interp, Tk_Window tkwin, char **argv));
...
char **argv;           /* Argument strings. */
{
    ...
    if (argc == 4) {
        ...
        if (argv[3][0] == 0) {
            ...
            interp, winPtr->mainPtr->bindingTable, object, argv[2]);
        }
        if (argv[3][0] == '+') {
            argv[3]++;
            append = 1;
...
void TkBindEventProc(winPtr, eventPtr) TkWindow *winPtr; /* Pointer to info
                                                            about window. */
...
#define MAX_OBJS 20
    ...
    if ((winPtr->mainPtr == NULL)
        || (winPtr->mainPtr->bindingTable == NULL)) {
        return;
    }
...
void TkFreeBindingTags(winPtr) TkWindow *winPtr; /* Window whose tags are to
                                                    be released. */
{
    int i;
    char *p;
...
#define SETUP(name)                                                          \
    ...
    if ((c == 'a') && (strcmp(argv[1], "atom") == 0)) {
        ...
        SETUP("colormapfull");
        interp->result
        = (TkCmapStressed(window, Tk_Colormap(window))) ? "1" : "0";
        ...
        SETUP("screendepth");
        sprintf(
        interp->result, "%d", DefaultDepthOfScreen(Tk_Screen(window)));
        ...
        case PseudoColor:
            interp->result = "pseudocolor";
            break;
        case GrayScale:
            interp->result = "grayscale";
            break;
        case DirectColor:
            interp->result = "directcolor";
            break;
        case TrueColor:
            interp->result = "truecolor";
            break;
        case StaticColor:
            interp->result = "staticcolor";
            break;
        case StaticGray:
            interp->result = "staticgray";
            break;
        default:
        ...
        Tk_Display(window), VisualScreenMask, &template, &count);
        if (visInfoPtr == NULL) {
...
static TkWindow *GetToplevel(tkwin) Tk_Window tkwin; /* Window for which the
                                                      * toplevel should be
                                                      * deterined. */
{
