...
static void SlaveStructureProc _ANSI_ARGS_((ClientData clientData,
                                            XEvent *eventPtr));
static int ConfigureSlave
_ANSI_ARGS_((Tcl_Interp * interp, Slave *slavePtr, int argc, char **argv));
...
static Slave *FindSlave(tkwin) Tk_Window tkwin; /* Token for desired slave. */
{
    Tcl_HashEntry *hPtr;
    Slave *slavePtr;
    int new;
...
static void UnlinkSlave(slavePtr) Slave *slavePtr; /* Slave structure to be
                                                      unlinked. */
{
    Master *masterPtr;
    Slave *prevPtr;
...
static Master *FindMaster(tkwin) Tk_Window tkwin; /* Token for desired master.
                                                   */
{
    Tcl_HashEntry *hPtr;
    Master *masterPtr;
...
char **argv;        /* String values for arguments. */
{
    ...
    for (; argc > 0; argc -= 2, argv += 2)
    {
        ...
        else if ((c == 'b') && (strncmp(argv[0], "-bordermode", length) == 0))
        {
            c = argv[1][0];
            length = strlen(argv[1]);
            ...
            else if ((c == 'o') && (strncmp(argv[1], "outside", length) == 0))
            {
        ...
        else if ((c == 'h') && (strncmp(argv[0], "-height", length) == 0))
        {
            ...
            {
                if (Tk_GetPixels(
                    interp, slavePtr->tkwin, argv[1], &slavePtr->height)
                    != TCL_OK)
                {
                    result = TCL_ERROR;
                    goto done;
        ...
        else if ((c == 'w') && (strncmp(argv[0], "-width", length) == 0))
        {
            ...
            {
                if (Tk_GetPixels(
                    interp, slavePtr->tkwin, argv[1], &slavePtr->width)
                    != TCL_OK)
                {
                    result = TCL_ERROR;
                    goto done;
        ...
        {
            if (Tk_GetPixels(interp, slavePtr->tkwin, argv[1], &slavePtr->x)
                != TCL_OK)
            {
                result = TCL_ERROR;
                goto done;
        ...
        {
            if (Tk_GetPixels(interp, slavePtr->tkwin, argv[1], &slavePtr->y)
                != TCL_OK)
            {
                result = TCL_ERROR;
                goto done;
