...
double TkLineToPoint(end1Ptr,
                     end2Ptr,
                     pointPtr) double end1Ptr[2]; /* Coordinates of first
                                                     end-point of line. */
...
int TkLineToArea(end1Ptr,
                 end2Ptr,
                 rectPtr) double end1Ptr[2]; /* X and y coordinates for one
                                              * endpoint of line. */
...
capStyle,
joinStyle,
rectPtr) double *coordPtr; /* Points to an array of coordinates for
...
double TkPolygonToPoint(polyPtr, numPoints, pointPtr) double
*polyPtr;         /* Points to an array coordinates for
...
int TkPolygonToArea(polyPtr, numPoints, rectPtr) double
*polyPtr;        /* Points to an array coordinates for
                  * closed polygon:  x0, y0, x1, y1, ...
                  * The polygon may be self-intersecting. */
int numPoints;   /* Total number of points at *polyPtr. */
...
/* ARGSUSED */
double TkOvalToPoint(ovalPtr,
                     width,
                     filled,
                     pointPtr) double ovalPtr[4]; /* Pointer to array of four
...
double pointPtr[2]; /* Coordinates of point. */
{
    ...
    scaledDistance
    = hypot(xDelta / ((ovalPtr[2] + width - ovalPtr[0]) / 2.0),
...
int TkOvalToArea(ovalPtr,
                 rectPtr) double *ovalPtr; /* Points to coordinates definining
                                            * the bounding rectangle for the
                                            * oval: x1, y1, x2, y2.  X1 must
                                            * be less than x2 and y1
                                            * less than y2. */
double *rectPtr; /* Points to coords for rectangle, in the
                  * order x1, y1, x2, y2.  X1 and y1 must
                  * be lower-left corner. */
...
/* ARGSUSED */
void TkIncludePoint(itemPtr,
                    pointPtr) Tk_Item *itemPtr; /* Item whose bounding box is
                                                 * being calculated. */
...
void TkBezierScreenPoints(canvas, control, numSteps, xPointPtr)
...
void TkBezierPoints(control,
                    numSteps,
                    coordPtr) double control[]; /* Array of coordinates for
                                                 * four control points:  x0,
                                                 * y0, x1, y1,
                                                 * ... x3 y3. */
int numSteps;                                   /* Number of curve points to
                                                 * generate.  */
...
int TkMakeBezierCurve(canvas,
                      pointPtr,
                      numPoints,
                      numSteps,
                      xPoints,
                      dblPoints) Tk_Canvas canvas; /* Canvas in which curve is
                                                    * to be drawn. */
...
void TkMakeBezierPostscript(interp, canvas, pointPtr, numPoints)
...
int numPoints;      /* Number of points at pointPtr. */
{
        ...
        && (pointPtr[1] == pointPtr[numCoords - 1]))
    {
        ...
        control[0],
        Tk_CanvasPsY(canvas, control[1]),
        control[2],
        Tk_CanvasPsY(canvas, control[3]),
        control[4],
...
int TkGetMiterPoints(p1,
                     p2,
                     p3,
                     width,
                     m1,
                     m2) double p1[]; /* Points to x- and y-coordinates of
                                       * point before vertex. */
