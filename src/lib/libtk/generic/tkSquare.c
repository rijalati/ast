...
static Tk_ConfigSpec configSpecs[] = { { TK_CONFIG_BORDER,
                                         "-background",
                                         "background",
                                         "Background",
                                         "#cdb79e",
                                         Tk_Offset(Square, bgBorder),
...
static int SquareConfigure _ANSI_ARGS_(
(Tcl_Interp * interp, Square *squarePtr, int argc, char **argv, int flags));
                     ...
                     * Tk_ConfigureWidget. */
{
    ...
    if ((squarePtr->gc == None) && (squarePtr->doubleBuffer)) {
        ...
        squarePtr->gc = Tk_GetGC(
        squarePtr->tkwin, GCFunction | GCGraphicsExposures, &gcValues);
    }
...
static void SquareCmdDeletedProc(clientData)
...
static void SquareDestroy(memPtr) char *memPtr; /* Info about square widget.
                                                 */
{
...
static void KeepInWindow(squarePtr) Square *squarePtr; /* Pointer to widget
                                                          record. */
{
    int i, bd;
    bd = 0;
