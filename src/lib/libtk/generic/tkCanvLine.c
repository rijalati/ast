...
static int ArrowheadPostscript _ANSI_ARGS_(
(Tcl_Interp * interp, Tk_Canvas canvas, LineItem *linePtr, double *arrowPtr));
...
static int ConfigureLine _ANSI_ARGS_((Tcl_Interp * interp,
                                      ...
                                      char **argv,
                                      int flags));
static int ConfigureArrows _ANSI_ARGS_((Tk_Canvas canvas, LineItem *linePtr));
...
static int LineCoords _ANSI_ARGS_((Tcl_Interp * interp,
                                   ...
                                   char **argv));
static int
LineToArea _ANSI_ARGS_((Tk_Canvas canvas, Tk_Item *itemPtr, double *rectPtr));
static double LineToPoint _ANSI_ARGS_((Tk_Canvas canvas,
                                       Tk_Item *itemPtr,
                                       double *coordPtr));
static int LineToPostscript _ANSI_ARGS_(
...
static void ScaleLine _ANSI_ARGS_((Tk_Canvas canvas,
                                   ...
                                   double scaleX,
                                   double scaleY));
static void TranslateLine _ANSI_ARGS_(
...
static Tk_ConfigSpec configSpecs[] = { { TK_CONFIG_UID,
                                         "-arrow",
                                         ( char * )NULL,
                                         ( char * )NULL,
                                         "none",
                                         Tk_Offset(LineItem, arrow),
                                       ...
                                       { TK_CONFIG_CUSTOM,
                                         ...
                                         ( char * )NULL,
                                         "8 10 3",
                                         Tk_Offset(LineItem, arrowShapeA),
                                         TK_CONFIG_DONT_SET_DEFAULT,
                                         &arrowShapeOption },
                                       { TK_CONFIG_CAP_STYLE,
                                         "-capstyle",
                                       ...
                                       { TK_CONFIG_INT,
                                         ...
                                         ( char * )NULL,
                                         "12",
                                         Tk_Offset(LineItem, splineSteps),
                                         TK_CONFIG_DONT_SET_DEFAULT },
                                       { TK_CONFIG_BITMAP,
                                         "-stipple",
                                         ( char * )NULL,
                                         ( char * )NULL,
                                         ( char * )NULL,
                                         Tk_Offset(LineItem, fillStipple),
                                         TK_CONFIG_NULL_OK },
                                       { TK_CONFIG_CUSTOM,
                                         "-tags",
                                         ( char * )NULL,
...
char **argv;        /* Arguments describing line. */
{
    ...
    if (ConfigureLine(interp, canvas, itemPtr, argc - i, argv + i, 0)
        == TCL_OK)
    {
        return TCL_OK;
    }
                     ...
                     * x2, y2, ... */
{
    ...
    else
    {
        ...
        if (linePtr->numPoints != numPoints)
        {
            ...
            linePtr->coordPtr
            = ( double * )ckalloc(( unsigned )(sizeof(double) * argc));
            linePtr->numPoints = numPoints;
        }
        for (i = argc - 1; i >= 0; i--)
        {
            if (Tk_CanvasGetCoord(
                interp, canvas, argv[i], &linePtr->coordPtr[i])
                != TCL_OK)
            {
                return TCL_ERROR;
            }
...
int flags;          /* Flags to pass to Tk_ConfigureWidget. */
{
    ...
    if (Tk_ConfigureWidget(
        interp, tkwin, configSpecs, argc, argv, ( char * )linePtr, flags)
        != TCL_OK)
    {
        return TCL_ERROR;
    }
...
static void
DeleteLine(canvas, itemPtr, display) Tk_Canvas canvas; /* Info about overall
                                                          canvas widget. */
...
static void ComputeLineBbox(canvas,
                            linePtr) Tk_Canvas canvas; /* Canvas that contains
                                                          item. */
                          ...
                          * must be redisplayed (not used). */
{
    ...
    XDrawLines(
    display, drawable, linePtr->gc, pointPtr, numPoints, CoordModeOrigin);
...
/* ARGSUSED */
static double
LineToPoint(canvas, itemPtr, pointPtr) Tk_Canvas canvas; /* Canvas containing
                                                            item. */
...
double *pointPtr; /* Pointer to x and y coordinates. */
{
         ...
         count--, coordPtr += 2)
    {
        ...
        if (((linePtr->capStyle == CapRound) && (count == numPoints))
            || ((linePtr->joinStyle == JoinRound) && (count != numPoints)))
        {
        ...
        if (count == numPoints)
        {
            TkGetButtPoints(coordPtr + 2,
                            coordPtr,
                            ( double )linePtr->width,
                            linePtr->capStyle == CapProjecting,
                            poly,
                            poly + 2);
        }
    ...
    if (linePtr->arrow != noneUid)
    {
        if (linePtr->arrow != lastUid)
        {
            ...
            linePtr->firstArrowPtr, PTS_IN_ARROW, pointPtr);
            if (dist <= 0.0)
            {
                bestDist = 0.0;
                goto done;
        ...
        {
            dist
            = TkPolygonToPoint(linePtr->lastArrowPtr, PTS_IN_ARROW, pointPtr);
            if (dist <= 0.0)
            {
                bestDist = 0.0;
                goto done;
...
double *rectPtr;
{
    ...
    result = TkThickPolyLineToArea(linePoints,
                                   numPoints,
                                   ( double )linePtr->width,
                                   linePtr->capStyle,
                                   linePtr->joinStyle,
    ...
    if (linePtr->arrow != noneUid)
    {
        if (linePtr->arrow != lastUid)
        {
            if (TkPolygonToArea(linePtr->firstArrowPtr, PTS_IN_ARROW, rectPtr)
                != result)
            {
                result = 0;
                goto done;
                        ...
                        * record. */
{
    ...
    if ((Tk_CanvasGetCoord(interp, linePtr->canvas, argv[0], &a) != TCL_OK)
        || (Tk_CanvasGetCoord(interp, linePtr->canvas, argv[1], &b) != TCL_OK)
        || (Tk_CanvasGetCoord(interp, linePtr->canvas, argv[2], &c) != TCL_OK))
    {
        goto syntaxError;
    }
...
LineItem *linePtr;                         /* Item to configure for arrows. */
{
    ...
    if (linePtr->arrow != lastUid)
    {
        ...
        {
            poly = ( double * )ckalloc(
            ( unsigned )(2 * PTS_IN_ARROW * sizeof(double)));
                     ...
                     * final Postscript is being created. */
{
    ...
    {
        Tk_CanvasPsPath(
        interp, canvas, linePtr->coordPtr, linePtr->numPoints);
    }
    else
    {
        if (linePtr->fillStipple == None)
        ...
        else
        {
            ...
            if (numPoints > MAX_STATIC_POINTS)
            {
                pointPtr = ( double * )ckalloc(
                ( unsigned )(numPoints * 2 * sizeof(double)));
            }
            numPoints = TkMakeBezierCurve(canvas,
                                          linePtr->coordPtr,
                                          linePtr->numPoints,
    ...
    if (linePtr->firstArrowPtr != NULL)
    {
        ...
        if (ArrowheadPostscript(
            interp, canvas, linePtr, linePtr->firstArrowPtr)
            != TCL_OK)
        {
            return TCL_ERROR;
        }
    ...
    if (linePtr->lastArrowPtr != NULL)
    {
        ...
        if (ArrowheadPostscript(interp, canvas, linePtr, linePtr->lastArrowPtr)
            != TCL_OK)
        {
            return TCL_ERROR;
        }
