...
#define MERGABLE(a, c, k)                                                    \
    ((a) > 0 && A_ISTINY(a) && (c) > 0 && C_ISTINY(c) && (k) >= K_SELF)
...
#if __STD_C
static int
vdfold(Table_t *tab, int output)
#else
static int vdfold(tab, output) Table_t *tab;
int output;
#endif
{
    reg ulong key, n;
...
#if __STD_C
long
_vddelta_01(Vddisc_t *source, Vddisc_t *target, Vddisc_t *delta, long window)
#else
long _vddelta_01(source,
                 target,
                 delta,
