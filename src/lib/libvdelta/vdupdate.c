...
#if __STD_C
static int
vdunfold(Utable_t *tab)
#else
static int vdunfold(tab) Utable_t *tab;
#endif
{
    reg long size, copy;
...
#if __STD_C
long
vdupdate(Vddisc_t *source, Vddisc_t *target, Vddisc_t *delta)
#else
long vdupdate(source, target, delta) Vddisc_t *source; /* source data	*/
