...
#    define A3(x, t)                                                         \
        ((t = (x)),                                                          \
         (t + (t << 5) + ((t + (t << 4)) << 6) + ((t + (t << 4)) << 11)))
#endif
#define HINIT(h, s, t)                                                       \
    ((h = A3(s[0], t)), (h += A2(s[1], t)), (h += A1(s[2], t) + s[3]))
...
#define K_INIT(quick, recent, rhere)                                         \
    {                                                                        \
        quick[rhere = 0] = (1 << 7);                                         \
        while ((rhere += 1) < K_QSIZE)                                       \
            quick[rhere] = rhere + (1 << 7);                                 \
        recent[rhere = 0] = (1 << 8);                                        \
        while ((rhere += 1) < K_RTYPE)                                       \
            recent[rhere] = (rhere + 1) * (1 << 8);                          \
    }
...
#define INIT(io, delta)                                                      \
    ((io)->endb = (io)->next = (io)->data = NIL(uchar *),                    \
     (io)->size = 0,                                                         \
     (io)->here = 0,                                                         \
     (io)->delta = (delta))
#define VDPUTC(io, c)                                                        \
...
#define STRWRITE(tab, s, n)                                                  \
    {                                                                        \
        memcpy((tab)->delta, (s), (n));                                      \
        (tab)->delta += (n);                                                 \
    }
#define STRPUTU(tab, u, buf)                                                 \
    {                                                                        \
        reg uchar *s, *endb;                                                 \
        reg ulong v = (u);                                                   \
        s = (endb = &buf[sizeof(buf)]) - 1;                                  \
        *s = I_CODE(v);                                                      \
        while ((v >>= I_SHIFT))                                              \
            *--s = I_CODE(v) | I_MORE;                                       \
        v = endb - s;                                                        \
        memcpy((tab)->delta, s, v);                                          \
        (tab)->delta += v;                                                   \
    }
#define STRGETC(tab) (*(tab)->delta++)
#define STRREAD(tab, s, n)                                                   \
    {                                                                        \
        memcpy((s), (tab)->delta, (n));                                      \
        (tab)->delta += (n);                                                 \
    }
