         ...
         n = ( Grnode_t * )dtlink(gr->nodes, n))
    {
        ...
        for (path = NIL(Gredge_t *); nc;) /* depth-first search */
        {
            ...
            for (ec = cl->cycl; ec; ec = ec->link)
            {
                ...
                ep->inext = en;
                en = grfind(ec->head)->iedge; /* catenate lists */
...
ssize_t
grbrweight(Gredge_t *e, ssize_t w)
{
    if (w > 0)
        BREDGE(e)->wght = w;
...
static Grdata_t *
brdata(Gralgo_t *algo, int type, Grdata_t *data)
{
    Brnode_t *brn;
    Bredge_t *bre;
