...
static void
nodefree(Dt_t *nodes, Void_t *node, Dtdisc_t *disc)
{
...
static void
edgefree(Dt_t *edges, Void_t *edge, Dtdisc_t *disc)
{
...
Grnode_t *
grnode(Graph_t *gr, Void_t *label, int type)
{
    Grnode_t node, *nd;
    Gredge_t *e, *enext;
    ssize_t sz;
...
Gredge_t *
gredge(Graph_t *gr, Grnode_t *tail, Grnode_t *head, Void_t *label, int type)
{
    ...
    ed->tail = edge.tail;
    ed->head = edge.head;
    ed->inext = edge.head->iedge;
    edge.head->iedge = ed;
    ed->onext = edge.tail->oedge;
    edge.tail->oedge = ed;
...
Grdata_t *
_grdata(Grdata_t **data, Gralgo_t *algo, int type)
{
    Grdata_t *dt, *pd;
    ...
    if (pd && dt) /* isolate from list */
        ...
        *data = dt->next;
        ...
        dt->algo = algo; /* set the algorithm that made this data */
