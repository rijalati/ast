...
rsnew(Rsdisc_t *disc)
#else
Rs_t *rsnew(disc) Rsdisc_t *disc; /* discipline describing record types	*/
#endif
{
    reg Rs_t *rs;
...
#if __STD_C
int
rsinit(reg Rs_t *rs, Rsmethod_t *meth, ssize_t c_max, int type, Rskey_t *key)
#else
...
#if __STD_C
Rs_t *
rsopen(Rsdisc_t *disc, Rsmethod_t *meth, ssize_t c_max, int type)
#else
