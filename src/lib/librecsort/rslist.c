...
#define ADDOBJ(list, tail, obj)                                              \
    (tail = tail ? (tail->right = obj) : (list = obj))
...
#if __STD_C
static int
uninsert(Union_t **list, int n, Union_t *un)
#else
static int uninsert(list, n, un) Union_t **list;
int n;
Union_t *un;
#endif
{
...
#if __STD_C
static Rsobj_t *
unionize(Rsobj_t **objlist, int n)
#else
static Rsobj_t *unionize(objlist, n) Rsobj_t **objlist;
int n;
#endif
{
    reg int p, cmp;
