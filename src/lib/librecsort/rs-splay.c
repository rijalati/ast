...
#if __STD_C
static int
splayinsert(Rs_t *rs, reg Rsobj_t *obj)
#else
static int splayinsert(rs, obj) Rs_t *rs;
reg Rsobj_t *obj;
#endif
{
    reg int cmp;
...
splaylist(Rs_t *rs)
#else
static Rsobj_t *splaylist(rs) Rs_t *rs;
#endif
{
    reg Rsobj_t *list;
    reg Rssplay_t *splay = ( Rssplay_t * ) rs->methdata;
...
/* public method */
static Rsmethod_t _Rssplay = { splayinsert, splaylist, sizeof(Rssplay_t),
                               RS_MTSPLAY,  "splay",   "Splay tree sort." };
