...
#endif
{
    ...
    sfprintf(sp,
             ...
             ? "all"
             : fp->keep == kp->state->print
               ? "print"
               : fp->keep == kp->state->dict ? "dict"
                                             : fp->keep ? "UNKNOWN" : "all");
    sfprintf(sp,
             "\t      trans = %s\n",
             fp->trans == kp->state->ident
             ? "ident"
             : fp->trans == kp->state->fold
               ? "fold"
               : fp->trans ? "UNKNOWN" : "ident");
    sfprintf(sp, "\t      bflag = %d\n", fp->bflag);
    sfprintf(sp, "\t      eflag = %d\n", fp->eflag);
    sfprintf(sp, "\t      rflag = %d\n", fp->rflag);
    sfprintf(
...
#endif
{
    ...
    sfprintf(sp,
             "\talignsize=%ld procsize=%ld recsize=%ld\n",
             kp->alignsize,
             kp->procsize,
             kp->recsize);
    sfprintf(sp,
             "\tmerge=%d reverse=%d stable=%d uniq=%d ignore=%d verbose=%d\n",
             kp->merge,
             !!(kp->type & RS_REVERSE),
             !(kp->type & RS_DATA),
             !!(kp->type & RS_UNIQ),
    ...
    sfprintf(sp, "\n");
    sfprintf(sp,
             "\trecord format %s data%s 0x%08x key%s %d\n",
             fmtrec(kp->disc->data, 0),
             (kp->disc->type & RS_DSAMELEN) ? " DSAMELEN" : "",
             kp->disc->data,
             (kp->disc->type & RS_KSAMELEN) ? " KSAMELEN" : "",
             kp->disc->keylen);
