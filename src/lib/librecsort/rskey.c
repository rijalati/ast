...
#define putdig(x)                                                            \
    (nib ? (*dig = encode(x) << 4, nib = 0) : (*dig++ |= encode(x), nib = 1))

static ssize_t
#if __STD_C
key_n_code(Rskey_t *kp,
           Rskeyfield_t *f,
           unsigned char *dp,
           size_t len,
           unsigned char *cp,
           unsigned char *zp)
#else
key_n_code(kp, f, dp, len, cp, zp) Rskey_t *kp;
Rskeyfield_t *f;
unsigned char *dp;
size_t len;
unsigned char *cp;
...
out:
    if (sigdig < 0 || sigdig >= 2047)
    {
        ...
        if (kp->keydisc->errorf)
            (*kp->keydisc->errorf)(
            kp, kp->keydisc, 1, "%-.*s: numeric field overflow", len, dp);
        dig = cp + 1;
        *dig = 0;
        nib = 0;
    }
...
key_p_code(Rskey_t *kp,
           ...
           unsigned char *zp)
#else
key_p_code(kp, f, dp, len, cp, zp) Rskey_t *kp;
Rskeyfield_t *f;
unsigned char *dp;
size_t len;
unsigned char *cp;
...
#endif
{
    ...
    if (sigdig >= 2047)
    {
        ...
        if (kp->keydisc->errorf)
            (*kp->keydisc->errorf)(
            kp, kp->keydisc, 1, "%-.*s: numeric field overflow", dp);
        dig = cp + 1;
        *dig = 0;
        nib = 0;
    }
...
key_z_code(Rskey_t *kp,
           ...
           unsigned char *zp)
#else
key_z_code(kp, f, dp, len, cp, zp) Rskey_t *kp;
Rskeyfield_t *f;
unsigned char *dp;
size_t len;
unsigned char *cp;
...
key_j_code(Rskey_t *kp,
           ...
           unsigned char *zp)
#else
key_j_code(kp, f, dp, len, cp, zp) Rskey_t *kp;
Rskeyfield_t *f;
unsigned char *dp;
size_t len;
unsigned char *cp;
...
key_t_code(Rskey_t *kp,
           ...
           unsigned char *zp)
#else
key_t_code(kp, f, dp, len, cp, zp) Rskey_t *kp;
Rskeyfield_t *f;
unsigned char *dp;
size_t len;
unsigned char *cp;
    ...
    if (kp->xfrmbuf && len)
    {
        ...
        for (;;)
        {
            if (kp->xfrmsiz < n)
            {
                ...
                if (!(kp->xfrmbuf
                      = vmnewof(Vmheap, kp->xfrmbuf, unsigned char, n, 0)))
                {
                    if (kp->keydisc->errorf)
                        (*kp->keydisc->errorf)(
                        kp,
                        kp->keydisc,
                        1,
                        "%-.*s: multibyte field overflow -- falling back to "
                        "native collation",
                        dp);
                    goto native;
                }
        ...
        if (m)
        {
            if (kp->keydisc->errorf)
                ...
                kp->keydisc,
                1,
                "key coder collation overflow (%d/%I*u) -- falling back to "
                "native collation",
                m,
                sizeof(zp - cp),
                zp - cp);
...
key_m_code(Rskey_t *kp,
           ...
           unsigned char *zp)
#else
key_m_code(kp, f, dp, len, cp, zp) Rskey_t *kp;
Rskeyfield_t *f;
unsigned char *dp;
size_t len;
unsigned char *cp;
...
code(Rs_t *rs,
     ...
     Rsdisc_t *disc)
#else
code(rs, dat, datlen, key, keylen, disc) Rs_t *rs;
unsigned char *dat;
size_t datlen;
unsigned char *key;
size_t keylen;
...
#else
conflict(kp, c) Rskey_t *kp;
int c;
#endif
{
    (*kp->keydisc->errorf)(
    kp, kp->keydisc, 1, "%c: key type conflicts with previous value", c);
}
...
#else
checkfield(kp, fp, key, c) Rskey_t *kp;
Rskeyfield_t *fp;
char *key;
int c;
#endif
{
    if (c || fp->begin.field < 0 || fp->end.field < 0 || fp->begin.index < 0
        || fp->end.index < -1)
    {
        if (kp->keydisc->errorf)
        {
            if (key)
                (*kp->keydisc->errorf)(kp,
                                       kp->keydisc,
                                       2,
                                       "%s: invalid key field specification",
                                       key);
            else
                (*kp->keydisc->errorf)(
                kp,
                kp->keydisc,
                2,
                "field[%d]: invalid key field specification",
                fp->index);
        }
    ...
    if (kp->keydisc->errorf && fp->coder == key_n_code && fp->keep)
        (*kp->keydisc->errorf)(
        kp, kp->keydisc, 1, "n: key type conflicts with d,i");
    return 0;
}
...
#else
addcoder(kp, fp, np, c, b) Rskey_t *kp;
Rskeyfield_t *fp;
Rskeycode_f np;
int c;
int b;
...
#else
addtable(kp, c, op, np) Rskey_t *kp;
int c;
unsigned char **op;
unsigned char *np;
#endif
...
#endif
{
        ...
        if (!fp->aflag)
        {
            ...
            if (!kp->field.prev)
            {
                if (kp->keydisc->errorf)
                    ...
                    kp->keydisc,
                    2,
                    "field[%d]: global accumulate invalid",
                    fp->index);
                kp->keydisc->flags |= RSKEY_ERROR;
                return 0;
            }
    ...
    if (kp->keydisc->errorf && fp != kp->tail)
        (*kp->keydisc->errorf)(
        kp, kp->keydisc, 1, "field spec precedes global option %c", c);
    return s - b;
}
...
#else
rskeyopt(kp, key, all) Rskey_t *kp;
char *key;
int all;
#endif
{
...
#else
rskey(kp, key, obsolete) Rskey_t *kp;
char *key;
int obsolete;
#endif
{
        ...
        if (!*t)
        {
            if (n != kp->fixed && kp->fixed)
            {
                if (kp->keydisc->errorf)
                    ...
                    kp->keydisc,
                    2,
                    "%s: fixed record length mismatch -- %d expected",
                    key,
                    kp->fixed);
                kp->keydisc->flags |= RSKEY_ERROR;
                return -1;
    ...
    if ((standard = !obsolete) && *s == ':')
    {
        if (n)
        {
            if (n != kp->fixed && kp->fixed)
            {
                if (kp->keydisc->errorf)
                    ...
                    kp->keydisc,
                    2,
                    "%s: fixed record key length mismatch -- %d expected",
                    key,
                    kp->fixed);
                kp->keydisc->flags |= RSKEY_ERROR;
                return -1;
        ...
        if (kp->fixed && (o + n) > kp->fixed)
        {
            if (kp->keydisc->errorf)
                ...
                kp->keydisc,
                2,
                "%s: fixed field exceeds record length %d",
                key,
                kp->fixed);
            kp->keydisc->flags |= RSKEY_ERROR;
            return -1;
    ...
    else if (!(fp = vmnewof(Vmheap, 0, Rskeyfield_t, 1, 0)))
    {
        if (kp->keydisc->errorf)
            (*kp->keydisc->errorf)(
            kp, kp->keydisc, 2, "out of space [field]");
        kp->keydisc->flags |= RSKEY_ERROR;
        return -1;
    }
    else
...
#endif
{
        ...
        else if (!fp->end.index && fp->end.field)
        {
            if (kp->tab[0] && fp->eflag)
            {
                if (kp->keydisc->errorf)
                    ...
                    kp->keydisc,
                    2,
                    "skipping blanks right after tab-char is ill-defined");
                kp->keydisc->flags |= RSKEY_ERROR;
                return -1;
            }
            fp->end.index--;
            ...
            if (fp->begin.index > kp->fixed)
            {
                if (kp->keydisc->errorf)
                    (*kp->keydisc->errorf)(kp,
                                           kp->keydisc,
                                           2,
                                           "field[%d]: begin index %d is "
                                           "greater than fixed record size",
                                           fp->index,
                                           fp->begin.index);
                kp->keydisc->flags |= RSKEY_ERROR;
                return -1;
    ...
    {
        kp->head = fp;
        if (!fp->next && !kp->tab[0] && !fp->begin.field && !fp->end.field
            && fp->end.index > 0 && fp->flag == 't'
            && fp->trans == kp->state->ident && fp->keep == kp->state->all
            && !fp->bflag && !fp->eflag && !fp->rflag)
        {
    ...
    if (kp->coded)
    {
        ...
        if (!(kp->field.positions
              = vmnewof(Vmheap, 0, unsigned char *, kp->field.maxfield, 0)))
        {
            if (kp->keydisc->errorf)
                (*kp->keydisc->errorf)(
                kp, kp->keydisc, 2, "out of space [positions]");
            kp->keydisc->flags |= RSKEY_ERROR;
            return -1;
        }
    }
    ...
    if (n & (n - 1))
    {
        for (m = 1; m < n; m <<= 1)
            if (m >= (LONG_MAX >> CHAR_BIT))
            {
                if (kp->keydisc->errorf)
                    (*kp->keydisc->errorf)(
                    kp, kp->keydisc, 2, "%ld: invalid alignment size", n);
                kp->keydisc->flags |= RSKEY_ERROR;
                return -1;
            }
        if (kp->keydisc->errorf)
            (*kp->keydisc->errorf)(
            kp, kp->keydisc, 1, "%ld: alignment size rounded to %ld", n, m);
        n = m;
    }
    kp->alignsize = n--;
    kp->insize = (kp->insize < kp->alignsize)
                 ? kp->alignsize
                 : roundof(kp->insize, kp->alignsize);
    kp->outsize = (kp->outsize && kp->outsize < kp->alignsize)
                  ? kp->alignsize
                  : roundof(kp->outsize, kp->alignsize);
    kp->procsize = (kp->procsize < kp->alignsize)
                   ? kp->alignsize
                   : roundof(kp->procsize, kp->alignsize);
