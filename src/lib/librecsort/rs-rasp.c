...
#if __STD_C
static int
raspinsert(Rs_t *rs, reg Rsobj_t *obj)
#else
static int raspinsert(rs, obj) Rs_t *rs;
reg Rsobj_t *obj;
#endif
{
    reg ssize_t cr, mp;
...
radix(Rsobj_t *list)
#else
static Rsobj_t *radix(list) Rsobj_t *list;
#endif
{
    reg Rsobj_t *work, *r;
    reg ssize_t ph;
...
#if __STD_C
static Rsobj_t *
listmerge(reg Rsobj_t *one, reg Rsobj_t *two, reg int n)
#else
static Rsobj_t *listmerge(one, two, n) reg Rsobj_t *one;
reg Rsobj_t *two;
...
#endif
{
    ...
    endone = one->left;
    one->left->right = NIL(Rsobj_t *);
    endtwo = two->left;
    two->left->right = NIL(Rsobj_t *);
...
#if __STD_C
static Rsobj_t *
flatten(reg Rsobj_t *r)
#else
static Rsobj_t *flatten(r) reg Rsobj_t *r;
#endif
{
...
rasplist(Rs_t *rs)
#else
static Rsobj_t *rasplist(rs) Rs_t *rs;
#endif
{
...
static Rsmethod_t _Rsrasp = {
    raspinsert, rasplist, sizeof(Rsrasp_t),
    RS_MTRASP,  "rasp",   "Initial radix split into a forest of splay trees."
};
