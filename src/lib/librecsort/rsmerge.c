...
mgflush(reg Rs_t *rs)
#else
static int mgflush(rs) reg Rs_t *rs;
#endif
{
    reg Rsobj_t *r;
    reg ssize_t n;
...
#if __STD_C
static int
mgrefresh(Rs_t *rs, Merge_t *mg)
#else
static int mgrefresh(rs, mg) Rs_t *rs;
Merge_t *mg;
#endif
{
    ssize_t datalen, rsc;
...
#if __STD_C
static int
mgclose(Rs_t *rs, Merge_t *mg)
#else
static int mgclose(rs, mg) Rs_t *rs;
Merge_t *mg;
#endif
{
    int ret;
...
#define MGCOMPARE(rs, one, two, reverse)                                     \
    ((one)->order == (two)->order                                            \
     ? mgcompare(rs, one, two, reverse)                                      \
     : (one)->order < (two)->order ? (reverse ? 1 : -1)                      \
                                   : (reverse ? -1 : 1))
...
#if __STD_C
static int
mgcompare(Rs_t *rs, Rsobj_t *one, Rsobj_t *two, int reverse)
#else
static int mgcompare(rs, one, two, reverse) Rs_t *rs;
reg Rsobj_t *one;
reg Rsobj_t *two;
int reverse;
#endif
...
#if __STD_C
static int
mginsert(Rs_t *rs, Merge_t **list, int n, Merge_t *mg)
#else
static int mginsert(rs, list, n, mg) Rs_t *rs;
Merge_t **list;
int n;
Merge_t *mg;
#endif
...
#if __STD_C
static int
mgmove(reg Rs_t *rs, reg Merge_t *mg, ssize_t n)
#else
static int mgmove(rs, mg, n) reg Rs_t *rs;
reg Merge_t *mg;
ssize_t n;
#endif
{
    ...
    for (n_obj = n < 0 ? 0 : n;;)
    {
                    ...
                    do
                    {
                        for (;;)
                        {
                            ...
                            if (w >= out.datalen)
                                break;
                            RSRESERVE(
                            rs, rsrv, endrsrv, cur, out.datalen, goto done);
                        }
                        cur += out.datalen;
                    } while (c == RS_INSERT);
                }
        ...
        else if (rs->type & RS_ITEXT)
        {
            for (; n_obj > 0; --n_obj)
            {
                ...
                {
                    MGRESERVE(
                    mg, mgrsrv, mgendrsrv, mgcur, s, goto last_chunk);
                    x = mgendrsrv - mgcur;
#if _PACKAGE_ast
                    if (rsc & ~0xff) /* Recfmt_t record descriptor */
                    {
...
#if __STD_C
static int
mgwrite(reg Rs_t *rs, reg Merge_t *mg, reg int n)
#else
...
#if __STD_C
static int
mgerror(Rs_t *rs, Merge_t **list, int n)
#else
static int mgerror(rs, list, n) Rs_t *rs;
Merge_t **list;
int n;
#endif
{
