...
gen(Excc_t *cc, Exnode_t *expr)
{
    ...
    case ID:
        if (cc->ccdisc->ccf)
            ...
            sfprintf(
            cc->ccdisc->text, "%s", expr->data.variable.symbol->name);
        return;
    case INC:
        sfprintf(cc->ccdisc->text, "%s++", x->data.variable.symbol->name);
        return;
    case ITERATE:
        if (expr->op == DYNAMIC) {
            ...
            sfprintf(
            cc->ccdisc->text,
            "for (%stmp_%d = (Exassoc_t*)dtfirst(%s); %stmp_%d && (%s = "
            "%stmp_%d->name); %stmp_%d = (Exassoc_t*)dtnext(%s, %stmp_%d)) {",
            cc->id,
            cc->tmp,
            expr->data.generate.array->data.variable.symbol->name,
            cc->id,
            cc->tmp,
            expr->data.generate.index->name,
            cc->id,
            cc->tmp,
            cc->id,
            cc->tmp,
    ...
    case SWITCH:
        t = x->type;
        sfprintf(
        cc->ccdisc->text, "{ %s %stmp_%d = ", extype(t), cc->id, ++cc->tmp);
        gen(cc, x);
        sfprintf(cc->ccdisc->text, ";");
    ...
    case S2I:
        sfprintf(cc->ccdisc->text,
                 "strto%s(",
                 sizeof(intmax_t) > sizeof(long) ? "ll" : "l");
        gen(cc, x);
