...
= { 0,   X2I, X2I, X2F, X2S, I2X, 0,   0,   I2F, I2S, I2X, 0, 0,
...
excast(Expr_t *p, Exnode_t *x, int type, Exnode_t *xref, int arg)
{
    ...
    if (x && x->type != type && type && type != VOID)
    {
        ...
        if (EXTERNAL(t2t) && !p->disc->convertf)
            exerror(
            "cannot cast %s to %s", TYPENAME(x->type), TYPENAME(type));
        if (x->op != CONSTANT)
