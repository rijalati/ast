...
trace(Expr_t *ex, int lev, char *op, int c)
{
    ...
    case INTEGER:
        s = " INTEGER ";
        sfsprintf(
        t = buf, sizeof(buf), "%I*d", sizeof(exlval.integer), exlval.integer);
        break;
    case LABEL:
        s = " LABEL ";
        t = exlval.id->name;
    ...
    case UNSIGNED:
        s = " UNSIGNED ";
        sfsprintf(
        t = buf, sizeof(buf), "%I*u", sizeof(exlval.integer), exlval.integer);
        break;
    case BREAK:
        s = " break";
        break;
    ...
    error(
    TRACE_lex + lev, "%s: [%d] %04d%s%s", op, ex->input->nesting, c, s, t);
}
