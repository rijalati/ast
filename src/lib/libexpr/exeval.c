...
getdyn(Expr_t *ex, Exnode_t *expr, void *env, Exassoc_t **assoc)
{
    ...
    if (expr->data.variable.index) {
        ...
        if (!(
            b = ( Exassoc_t * )dtmatch(
            ( Dt_t * )expr->data.variable.symbol->local.pointer, v.string))) {
...
prformat(Sfio_t *sp, void *vp, Sffmt_t *dp)
{
    ...
    if (fmt->args) {
        if (node
            = (dp->fmt == '*') ? fmt->args->param[dp->size] : fmt->args->arg)
    ...
    if (!sp) {
        ...
        if (v.integer < 0 || v.integer >= elementsof(ex->file)
            || !(sp = ex->file[v.integer])
               && !(sp = ex->file[v.integer] = sfnew(
                    NiL, NiL, SF_UNBOUND, v.integer, SF_READ | SF_WRITE))) {
...
eval(Expr_t *ex, Exnode_t *expr, void *env)
{
        ...
        if (x->op == DYNAMIC) {
            if (x->type == STRING) {
                ...
                if (e = assoc ? assoc->value.string
                              : x->data.variable.symbol->value->data.constant
                                .value.string)
                    vmfree(ex->vm, e);
            }
            if (assoc)
    ...
    case ITERATE:
        ...
        if (expr->data.generate.array->op == DYNAMIC) {
            ...
            for (assoc = ( Exassoc_t * )dtfirst(
                 ( Dt_t * )expr->data.generate.array->data.variable.symbol
                 ->local.pointer);
                 assoc;
                 assoc = ( Exassoc_t * )dtnext(
                 ( Dt_t * )expr->data.generate.array->data.variable.symbol
                 ->local.pointer,
                 assoc)) {
                v.integer++;
            ...
            ex,
            expr,
            expr->data.generate.array->data.variable.symbol,
            expr->data.generate.array->data.variable.reference,
            env,
            0,
            ex->disc);
    ...
    case CALL:
        ...
        for (n = 0, a = expr->data.call.procedure->value->data.procedure.args;
            ...
            if (n < elementsof(args)) {
                save[n] = a->data.operand.left->data.variable.symbol->value
                          ->data.constant.value;
        ...
        v = exeval(
        ex, expr->data.call.procedure->value->data.procedure.body, env);
        for (n = 0, a = expr->data.call.procedure->value->data.procedure.args;
             a && n < elementsof(save);
             a = a->data.operand.right)
            a->data.operand.left->data.variable.symbol->value->data.constant
        ...
        case F2S:
            ...
            if (expr->data.operand.left->op != DYNAMIC
                ...
                tmp.data.constant.value.string = exstash(ex->tmp, ex->ve);
                       ...
                       0,
                       ex->disc))
                exerror("%s: cannot convert value to string",
    ...
    default:
        ...
        case X2F:
            ...
            if ((*ex->disc->convertf)(
                ...
                0,
                ex->disc))
                exerror("%s: cannot convert value to floating",
                        expr->data.operand.left->data.variable.symbol->name);
            tmp.type = FLOATING;
            return tmp.data.constant.value;
        case X2I:
            ...
            if ((*ex->disc->convertf)(
                ...
                0,
                ex->disc))
                exerror("%s: cannot convert value to integer",
                        expr->data.operand.left->data.variable.symbol->name);
            tmp.type = INTEGER;
            return tmp.data.constant.value;
        case X2S:
