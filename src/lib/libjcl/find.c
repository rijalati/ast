...
#define REGULAR(p, s)                                                        \
    (stat((p), (s)) >= 0 && (S_ISREG((s)->st_mode) || streq(p, "/dev/null")))
        ...
        Sfio_t **spp)
{
    ...
    if (level && jcl->disc->errorf)
        (*jcl->disc->errorf)(
        NiL, jcl->disc, ERROR_SYSTEM | level, "%s: not found", name);
    return 0;
found:
    if (spp && !(*spp = sfopen(NiL, s, "r")) && level > 0
        && jcl->disc->errorf)
        (*jcl->disc->errorf)(
        NiL, jcl->disc, ERROR_SYSTEM | level, "%s: cannot read", s);
