...
dsn(Jcl_t *jcl, Jcldd_t *dd, const char *path, int mark)
{
    ...
    sfprintf(jcl->vp,
             "%s",
             fmtquote(path, "\"", "\"", strlen(path), FMT_SHELL | FMT_PARAM));
...
jclrun(Jcl_t *scope)
{
    ...
    if (!(jcl = jclopen(scope,
                        scope->step->command,
                        scope->flags | scope->step->flags,
                        scope->disc)))
        return -1;
    if (jcl->flags & JCL_EXEC) {
        if (!(co = coopen(pathshell(),
                          (jcl->flags & (JCL_EXEC | JCL_TRACE))
                          == (JCL_EXEC | JCL_TRACE)
                          ? CO_ANY
                          : (CO_ANY | CO_SILENT),
                          NiL))) {
            if (jcl->disc->errorf)
                (*jcl->disc->errorf)(
                NiL, jcl->disc, 2, "%s: cannot connect to coshell", jcl->name);
            jclclose(jcl);
            return -1;
        }
        start = tmxgettime();
    ...
    code = 0;
    if (jcl->name
        && (!scope || !scope->scope || (scope->scope->flags & JCL_SCOPE))) {
        ...
        if (jcl->flags & JCL_SUBDIR) {
            if (jcl->flags & JCL_EXEC) {
                ...
                for (;;) {
                    sfsprintf(
                    subdir, sizeof(subdir), "%s.%s.%d", jcl->name, t, ++n);
                    if (!mkdir(subdir,
                    ...
                    if (errno != EEXIST) {
                        if (jcl->disc->errorf)
                            ...
                            jcl->disc,
                            ERROR_SYSTEM | 2,
                            "%s: cannot create job subdirectory",
                            subdir);
                        jclclose(jcl);
                        return -1;
                    }
                ...
                if (chdir(subdir)) {
                    if (jcl->disc->errorf)
                        ...
                        jcl->disc,
                        ERROR_SYSTEM | 2,
                        "%s: cannot run in job subdirectory",
                        subdir);
                    jclclose(jcl);
                    return -1;
                }
        ...
        for (top = scope;
             !top->name && !(top->flags & JCL_SCOPE) && top->scope;
             top = top->scope)
            ;
        ...
        for (dd = ( Jcldd_t * )dtfirst(step->dd); dd;
            ...
            if (dd->flags & JCL_DD_ALIAS) {
                ...
                if (!xx) {
                    if (jcl->disc->errorf)
                        (*jcl->disc->errorf)(
                        NiL, jcl->disc, 2, "%s: DD not defined", dd->path);
                    code = -1;
                    goto bad;
                }
                k = dd->link;
        ...
        if (!jcl->tmp) {
            ...
            if (dd) {
                ...
                for (s = jcl->tmp; *s; s++)
                    if (*s == '@')
                        *s = '-';
                if ((jcl->flags & (JCL_VERBOSE | JCL_EXEC | JCL_SUBTMP))
                    == JCL_VERBOSE)
                    sfprintf(sfstdout,
                             "trap 'code=$?; rm -rf %s*; exit $code' 0 1 2\n",
                             jcl->tmp);
            }
        }
        jcl->steps++;
        if (jcl->flags
            & (JCL_LISTEXEC | JCL_LISTINPUTS | JCL_LISTJOBS | JCL_LISTOUTPUTS
               | JCL_LISTPROGRAMS | JCL_LISTSCRIPTS)) {
            if ((jcl->flags & JCL_LISTPROGRAMS) && (step->flags & JCL_PGM))
                uniq(step->command,
                     (jcl->flags & JCL_LISTPARMS) && step->parm ? step->parm
                                                                : NiL,
                     JCL_LISTPROGRAMS,
                     jcl->disc);
            else if ((jcl->flags & (JCL_LISTJOBS | JCL_LISTSCRIPTS))
                     && !(step->flags & JCL_PGM)
                     && (s = jclfind(jcl, step->command, 0, 0, NiL)))
        ...
        if (jcleval(jcl, step->cond, code)) {
            if (step->flags & JCL_PGM) {
                ...
                for (dd = ( Jcldd_t * )dtfirst(step->dd); dd;
                    ...
                    default:
                        if (!(dd->flags & JCL_DD_DUMMY)
                            && (jcl->flags & (JCL_EXEC | JCL_VERBOSE))
                            && dd->path && *dd->path != '&'
                ...
                for (sym = ( Jclsym_t * )dtfirst(step->syms); sym;
                    ...
                    " %s=%s",
                    fmtquote(
                    sym->name, "'", "'", strlen(sym->name), FMT_SHELL),
                    fmtquote(sym->value, "$'", "'", strlen(sym->value), 0));
                if (dd = std[0]) {
                ...
                if (dd = std[3])
                    sfprintf(jcl->tp,
                             " <<%s\n%s%s",
                             fmtquote(dd->dlm, "'", "'", strlen(dd->dlm), 1),
                             dd->here,
                             dd->dlm);
                sfprintf(jcl->tp, "\ncode=$?\n");
            ...
            else if ((jcl->flags & (JCL_TRACE | JCL_EXEC))
                     == (JCL_TRACE | JCL_EXEC))
                sfprintf(
                sfstderr, "+ : %s PROC %s\n", step->name, step->command);
            if (step->flags & JCL_PGM) {
                ...
                if (sfsync(sfstdout)) {
                    if (jcl->disc->errorf)
                        (*jcl->disc->errorf)(
                        NiL, jcl->disc, 2, "write error");
                    code = -1;
                    break;
                }
                if (!(jcl->flags & JCL_EXEC))
                    code = 0;
                else if (!(cj = coexec(co,
                                       s,
                                       CO_APPEND,
                                       redirect[0].file,
                                       redirect[1].file,
            ...
            if (jclrc(jcl, step, code) < 0
                && ((jcl->flags & JCL_EXEC) || !(jcl->flags & JCL_LIST)))
                break;
        }
    }
bad:
    if (jcl == top) {
        if (jcl->tmp && (jcl->flags & (JCL_EXEC | JCL_SUBTMP)) == JCL_EXEC) {
            ...
            if (!(s = sfstruse(jcl->tp)))
                nospace(jcl, NiL);
            if (cj = coexec(
                co, s, CO_APPEND, redirect[0].file, redirect[1].file, NiL))
                cowait(co, cj, -1);
        }
        ...
        if ((jcl->flags & (JCL_SUBDIR | JCL_VERBOSE))
            == (JCL_SUBDIR | JCL_VERBOSE)) {
            if (jcl->flags & JCL_EXEC)
                sfprintf(
                sfstdout, "COMPLETED AT %s\n", fmttime("%K", time(NiL)));
            else
                sfputr(sfstdout, "date +'COMPLETED AT %K'\n}", '\n');
        }
        if ((jcl->flags & (JCL_EXEC | JCL_SUBDIR)) == (JCL_EXEC | JCL_SUBDIR)
            && chdir("..") && jcl->disc->errorf)
            (*jcl->disc->errorf)(
            NiL,
            jcl->disc,
            ERROR_SYSTEM | 2,
            "%s: cannot chdir to job subdirectory parent",
            subdir);
        if ((jcl->flags & (JCL_EXEC | JCL_VERBOSE))
            == (JCL_EXEC | JCL_VERBOSE)) {
