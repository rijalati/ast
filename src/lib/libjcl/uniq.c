        ...
        && *(s - 1) != '%')
    {
        ...
        for (;;)
        {
            ...
            default:
                if (disc->errorf)
                    (*disc->errorf)(
                    NiL, disc, 1, "%s: %c: unknown RECFM", path, *s);
                continue;
            }
            break;
        }
        ...
        if (disc->errorf)
        {
            ...
            if (recfm != u->recfm && recfm && u->recfm)
                (*disc->errorf)(NiL,
                                disc,
                                2,
                                "%s: inconsistent recfm %s",
                                u->value,
                                (recfm & JCL_RECFM_V) ? "v" : "f");
        }
        return u->value;
    }
    if (!size)
...
static const Label_t label[] = {
    { JCL_LISTAUTOEDITS, 'A' }, { JCL_LISTEXEC, 'E' },
    { JCL_LISTINPUTS, 'I' },    { JCL_LISTJOBS, 'J' },
    { JCL_LISTOUTPUTS, 'O' },   { JCL_LISTPROGRAMS, 'P' },
    { JCL_LISTSCRIPTS, 'S' },   { JCL_LISTVARIABLES, 'V' },
};
