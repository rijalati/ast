...
match(const char *name)
{
    ...
    if (all >= 0)
    {
        ...
        case 1:
            ...
            break;
        default:
            state.match[all].length = strlen(
            state.match[all].string
            = state.match[all - 1].string + state.match[all - 1].length + 1);
...
jclmap(Jcl_t *jcl, const char *file, Jcldisc_t *disc)
{
            ...
            else if (streq(op, "map"))
            {
                ...
                if (*s == '*')
                {
                    if (*(s + 1))
                    {
                        if (disc->errorf)
                            ...
                            disc,
                            2,
                            "%s: %s: only \"*.\" at beginning, \".*.\" in "
                            "middle, or \".*\" at end supported",
                            op,
                            s);
                        sfclose(sp);
                        return -1;
                ...
                if (k >= elementsof(state.match))
                {
                    if (disc->errorf)
                        ...
                        disc,
                        2,
                        "%s: %s: only %d *. prefixes supported",
                        op,
                        arg[0],
                        elementsof(state.match) - 1);
                    sfclose(sp);
                    return -1;
                }
                if (state.matches < k)
            ...
            else if (streq(op, "set"))
            {
                ...
                while (s = arg[n++])
                {
                    ...
                    if (*s == '-' || *s == '+')
                        ...
                        for (v = 0; s = nv[v]; v++)
                            if (!jclsym(jcl, s, NiL, JCL_SYM_SET)
                                && disc->errorf)
                                (*disc->errorf)(
                                NiL, disc, 1, "%s: invalid assignment", s);
                }
    ...
    for (k = 0; k <= state.matches; k++)
    {
        ...
        if (mp = state.match[k].all)
            message((
            -9, "map[%d]   %2d %2d %s", k, mp->length, mp->same, mp->prefix));
    }
    return 0;
}
