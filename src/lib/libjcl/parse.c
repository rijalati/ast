...
#define DDLIB(s)                                                             \
    (streq((s), "JCLLIB") || streq((s), "JOBLIB") || streq((s), "PROCLIB"))
#define ID1(c)                                                               \
    (isalpha(c) || (c) == '_' || (c) == '$' || (c) == '@' || (c) == '#')
#define ID(c)                                                                \
    (isalnum(c) || (c) == '_' || (c) == '$' || (c) == '@' || (c) == '#')
...
autonumber(Jcl_t *jcl, char *s, long *r)
{
    ...
    *r = strtol(s, &t, 10);
    if (s == t && jcl->disc->errorf)
        (*jcl->disc->errorf)(
        NiL, jcl->disc, 1, "%s: invalid control-m numeric operand", b);
    return t;
}
...
#define OPERAND()                                                            \
    do {                                                                     \
        if (p) {                                                             \
            ...
            if (jcl->disc->errorf)                                           \
                (*jcl->disc->errorf)(                                        \
                NiL, jcl->disc, 1, "%%%%%s: left operand expected", f);      \
        }                                                                    \
    } while (0)
            ...
            case 'C':
                if (streq(f, "CALCDATE") || streq(f, "CALCDTE")) {
                    ...
                    if (jcl->disc->errorf && (*e || *v))
                        ...
                        jcl->disc,
                        1,
                        "%%%%%s: %s: invalid control-m date operand",
                        f,
                        t);
                    tm = tmmake(&x);
                    tm->tm_mday += i;
...
notyet:
    if (jcl->disc->errorf)
        (*jcl->disc->errorf)(
        NiL, jcl->disc, 1, "%%%%%s: control-m function not implemented", f);
    return s;
}
                        ...
                        if (!q) {
                            ...
                            if (isspace(c)) {
                                ...
                                if (a == 2) {
                                     ...
                                     * be delayed until the other procedure
                                     * context is active
                                     */
                ...
                *op = 0;
                if (!streq(val, step->name)) {
                    if (jcl->disc->errorf)
                        ...
                        jcl->disc,
                        2,
                        "%s: step back reference not supported",
                        val);
                    return -1;
                }
                *op++ = '.';
    ...
    if (*s == '(') {
        ...
        *s = 0;
        if (!*t)
            ...
            if (jcl->disc->errorf)
                (*jcl->disc->errorf)(
                NiL, jcl->disc, 2, "%s: unknown IF expression token", t);
            return -1;
        }
        *s = c;
    }
...
eval(Jcl_t *jcl, char *s, char **e, int prec)
{
    ...
    static const char *opname[] = { "NOP", "ONLY", "EVEN", "LT", "LE", "EQ",
                                    "NE",  "GE",   "GT",   "OR", "AND" };
#endif
        ...
        if (!op || isupper(*t) && ID(*s)) {
            if (jcl->disc->errorf)
                (*jcl->disc->errorf)(NiL,
                                     jcl->disc,
                                     2,
                                     "%s: operator expected in IF expression",
                                     t);
            return -1;
        }
        if (op > prec) {
...
IF(Jcl_t *jcl)
{
    ...
    message(
    (-5, "if      %4d  %s == %d", jcl->ie->line, sfstrbase(jcl->tp), r));
    return r;
}
                ...
                if (dd->path && (dd->recfm & (JCL_RECFM_F | JCL_RECFM_V))
                    ...
                    for (cat = dd->cat; cat; cat = cat->next)
                        if (!streq(cat->path, dummy))
                            cat->path
                            = mark(cat->path, 0, dd->lrecl, jcl->disc);
                }
        return step;
    }
    if (jcl->disc->errorf && jcl->eof && jcl->ie)
        (*jcl->disc->errorf)(
        NiL, jcl->disc, 2, "IF on line %d has no ENDIF", jcl->ie->line);
    return 0;
}
