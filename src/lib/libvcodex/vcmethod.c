...
#    if __STD_C
static Vcmethod_t *
plugin(Void_t *dll, const char *path)
#    else
static Vcmethod_t *plugin(dll, path) Void_t *dll;
char *path;
#    endif
{
    Vcmethod_t *meth;
    ...
    if (dls = dllsopen(VC_ID, NiL, NiL))
    {
        while (dle = dllsread(dls))
            if (dll = dlopen(dle->path, RTLD_LAZY))
                ...
                errorf(
                "dll", NiL, 1, "%s: dlopen failed: %s", dle->path, dlerror());
        dllsclose(dls);
    }
#endif
...
#if __STD_C
Vcmethod_t *
vcgetmeth(char *name, int portable)
#else
Vcmethod_t *vcgetmeth(name, portable) char *name;
...
#if __STD_C
int
vcwalkmeth(Vcwalk_f walkf, Void_t *disc)
#else
int vcwalkmeth(walkf, disc) Vcwalk_f walkf;
Void_t *disc;
#endif
{
    Vcmtlist_t *mtl;
...
#endif
{
    ...
    while (isblank(*data))
        data += 1;
    if (*data
        == '=') /* name=value syntax, get value with C syntax characters */
    {
...
#if __STD_C
char *
vcsubstring(char *data, int csep, char *val, ssize_t vlsz, int type)
#else
...
#if __STD_C
char *
vcgetident(Vcmethod_t *meth, char *buf, ssize_t n)
#else
char *vcgetident(meth, buf, n) Vcmethod_t *meth; /* method to get ID from	*/
