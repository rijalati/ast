...
#if __STD_C
static ssize_t
rdhuffman(Freq_t freq)
#else
static ssize_t rdhuffman(freq) Freq_t freq;
#endif
{
    ssize_t k, w;
    Freq_t cdsz;
...
#if __STD_C
static int
rdsepar(Vcrdformat_t *rdf, Vcchar_t *data, ssize_t dtsz, int rs)
#else
...
#endif
{
    ...
    for (fs = 0; fs < 256;
         ++fs) /* keep those with least variation in #fields */
        if (fsep[fs].typn > fsep[bestfs].typn)
            fsep[fs].typn = -1;

    for (fs = 0; fs < 256;
         ++fs) /* among those keeps the ones with max #fields */
        if (fsep[fs].typn == fsep[bestfs].typn
            && fsep[fs].typ[0] > fsep[bestfs].typ[0])
            bestfs = fs;
    for (fs = 0; fs < 256; ++fs)
        if (fsep[fs].typn == fsep[bestfs].typn
            && fsep[fs].typ[0] < fsep[bestfs].typ[0])
...
static void
fldfreq(Freq_t *fldf, ssize_t *fldz, ssize_t nf, Freq_t *colf)
#else
...
#ifdef DEBUG
static int
printmark(ssize_t *mark, ssize_t nc, int mrkc)
{
    ssize_t c;
    for (c = 0; c < nc - 1; ++c)
...
static void
fldmark(ssize_t *mark, Vcchar_t *data, ssize_t nc, ssize_t nr, Freq_t *colf)
#else
...
static ssize_t
fldpartition(ssize_t *fldz, ssize_t *mark, ssize_t nc, ssize_t algn)
#else
...
rdfixed(Vcrdformat_t *rdf,
        ...
        int merge)
#else
static int rdfixed(rdf, data, nc, nr, algn, merge) Vcrdformat_t *rdf;
Vcchar_t *data;
ssize_t nc;
ssize_t nr;
ssize_t algn;
...
#if __STD_C
Vcrdformat_t *
vcrdformat(Vcchar_t *data, ssize_t dtsz, int rsep, ssize_t algn, int merge)
#else
Vcrdformat_t *
