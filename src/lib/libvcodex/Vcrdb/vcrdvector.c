...
#if __STD_C
int
vcrdvector(Vcrdtable_t *tbl, ssize_t f, Vcchar_t *trdt, ssize_t trsz, int type)
#else
...
#define err_return(x)                                                        \
    ...
    { /**/
        ...
        if (tbl->info->flen
            && (flen = tbl->info->flen[f]) > 0) /* fixed length field */
        {
        ...
        for (nsort = NIL(Sort_t *); csort;)
        {
            ...
            for (p = 0; p < so->size; ++p)
            {
                ...
                if (type == VC_DECODE)
                    ...
                    c = rcrd[r].dtsz <= cpos ? -1 : rcrd[r].data[cpos];
                if (c >= 0)
                {
                    cmin = c < cmin ? c : cmin;
                    cmax = c > cmax ? c : cmax;
    ...
    for (r = 1; r < tbl->recn; ++r)
    {
        Vcchar_t *s, *t;
        ssize_t ns, nt;
        s = rcrd[vect[r - 1]].data;
        ns = rcrd[vect[r - 1]].dtsz;
        t = rcrd[vect[r]].data;
        nt = rcrd[vect[r]].dtsz;
        for (p = 0; p < ns && p < nt; ++p)
        { /**/
