...
#if __STD_C
Vcrdtable_t *
vcrdparse(Vcrdinfo_t *info, Vcchar_t *data, ssize_t dtsz, int type)
#else
...
#endif
{
    ...
    if (!tbl || !tbl->info || (recn = tbl->recn) <= 0 || f < 0
        || f >= tbl->fldn)
        return -1;
...
#if __STD_C
static ssize_t
text2byte(Vcrdinfo_t *info, Vcchar_t *dt, Vcchar_t *cvdt, int flde, int *type)
#else
    ...
    for (f = 0, ty = VCRD_DOT;; ++f)
    {
        ...
        for (byte = 0, dign = 0;; ++dt)
        {
            ...
            else if (*dt == info->dot || *dt == info->slash)
            {
                ...
                if (*dt == info->slash) /* exactly one more conversion after
                                           this */
                    ty |= VCRD_SLASH;
...
#endif
{
    ...
    if (type != 0) /* encoding */
    {
        ...
        if ((z = text2byte(info, rcrd[0].data, NIL(Vcchar_t *), flde, &type))
            <= 0)
            return 0;
...
#if __STD_C
void
vcrdclose(Vcrdtable_t *tbl)
#else
void vcrdclose(tbl) Vcrdtable_t *tbl;
#endif
{
    ssize_t f;
        ...
        & VCRD_FIELD) /* note that field-oriented format implies VCRD_PAD */
    {
        for (f = fldi; f < endf; ++f)
        {
            for (rcrd = fld[f].rcrd, r = 0; r < tbl->recn; ++r)
            {
                ...
                data += dtsz;
                if (fld[f].type
                    & VCRD_PAD) /* pad field to make all parts equal */
    ...
    else /* record-oriented output */
    {
        for (r = 0; r < tbl->recn; ++r)
        {
            ...
            for (f = 0; f < endf; ++f)
            {
                ...
                memcpy(
                data, fld[f].rcrd[r].data, (dtsz = fld[f].rcrd[r].dtsz));
                data += dtsz;
                if (fld[f].type
                    & VCRD_PAD) /* pad field to make all parts equal */
                    for (fsep = data[-1]; dtsz < fld[f].maxz; ++dtsz)
