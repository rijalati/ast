...
#if _STD_C
static ssize_t
hamming(Vcchar_t *s1, ssize_t n1, Vcchar_t *s2, ssize_t n2)
#else
static ssize_t hamming(s1, n1, s2, n2) Vcchar_t *s1; /* first string		*/
ssize_t n1;
...
lcs(Vcchar_t *s1,
    ...
    ssize_t *last)
#    else
static ssize_t lcs(s1, n1, s2, n2, here, last) Vcchar_t *s1; /* first string
                                                              */
ssize_t n1;
Vcchar_t *s2; /* second string	*/
ssize_t n2;
...
/* Get the compressive entropy of a field after permuting records by some
 * transform vector */
#if __STD_C
ssize_t
rdentropy(Vcrdtable_t *tbl,
          ssize_t f,
          ssize_t *vect,
          ssize_t *smpl,
          ssize_t n_smpl,
...
#endif
{
    ...
    DEBUG_PRINT(2, "smpl=%d\n", n_smpl);

    for (maxz = 0, f = 0; f < fldn; ++f)
        if (tbl->fld[f].maxz > maxz)
            maxz = tbl->fld[f].maxz;
    maxz *= 2 * tbl->recn; /* to adjust the compressive entropy for computing
                              optimum branching */
    ...
    /* compute the maximum branching that determines the optimum dependency
     * relations */
...
#if __STD_C
void
vcrdfreeplan(Vcrdplan_t *pl)
#else
void vcrdfreeplan(pl) Vcrdplan_t *pl;
#endif
{
    if (pl)
        free(pl);
...
#if __STD_C
int
vcrdexecplan(Vcrdtable_t *tbl, Vcrdplan_t *pl, int type)
#else
