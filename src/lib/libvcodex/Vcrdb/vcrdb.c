...
#if __STD_C
static ssize_t
rdb(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t rdb(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
{
    ...
    if (!rdb->vcw && !(rdb->type & RDB_PLAIN)) {
        if (!(rdb->type & RDB_BWT))
            ...
            if (!(vcw = vcopen(0, Vchuffman, 0, 0, VC_ENCODE)))
                ...
                rdb->vcw = vcw;
            if (!(vcw = vcopen(
                  0, Vcrle, "0", rdb->vcw, VC_ENCODE | VC_CLOSECODER)))
                GOTO(done);
            else
                rdb->vcw = vcw;
            if (!(vcw = vcopen(
                  0, Vcmtf, "0", rdb->vcw, VC_ENCODE | VC_CLOSECODER)))
                GOTO(done);
            else
                rdb->vcw = vcw;
            if (!(vcw
                  = vcopen(0, Vcbwt, 0, rdb->vcw, VC_ENCODE | VC_CLOSECODER)))
                GOTO(done);
            else
                rdb->vcw = vcw;
        }
    ...
    for (f = 0; f < tbl->fldn; ++f) {
        if (!(tbl->fld[f].type & VCRD_FIXED))
            vcrdattrs(
            tbl, f, VCRD_DOT, 1); /* try to convert IP data into bytes */
    ...
    if (!rc->plan) /* need a tranformation plan */
    {
        if (rdb->type & RDB_PLAIN)
            ...
            rc->plan = vcrdmakeplan(tbl, rdb->vcw);
        if (!rc->plan)
            goto done;
    }
    ...
    for (f = 0; f < tbl->fldn; ++f) {
        z
        = (tbl->fld[f].type & (VCRD_FIXED | VCRD_DOT | VCRD_SLASH | VCRD_PAD))
          ? tbl->fld[f].maxz
          : 0;
...
#if RDB_BDV
        if (rdb->type & RDB_BDV) {
            if (vcrdvector(tbl, f, dt, z, VC_ENCODE) < 0
                || vcrdsize(tbl) != z)
                GOTO(done);
        } else
#endif
        ...
        if (vc->coder) /* recoding data if required */
        {
            ...
            if (FLEN(rc, f) > 0 || (rdb->type & RDB_PAD)
                || (tbl->fld[f].type & VCRD_FIXED)) {
                vcsetmtarg(vc->coder,
                           "columns",
                           TYPECAST(Void_t *, tbl->fld[f].maxz),
                           2);
            } else {
                if (rc->info.rsep >= 0)
                    vcsetmtarg(
                    vc->coder, "rsep", TYPECAST(Void_t *, rc->info.rsep), 1);
                if (rc->info.fsep >= 0)
                    vcsetmtarg(
                    vc->coder, "fsep", TYPECAST(Void_t *, rc->info.fsep), 1);
            }
    ...
    {
        vcioputu(&io,
                 rc->plan ? rc->plan->pred[f] : f); /* predictor of field */

        type
        = tbl->fld[f].type & (VCRD_FIXED | VCRD_DOT | VCRD_SLASH | VCRD_PAD);
        vcioputu(&io,
                 type ? tbl->fld[f].maxz : 0); /* length of a fixed field */
...
#if __STD_C
static ssize_t
unrdb(Vcodex_t *vc, const Void_t *data, size_t dtsz, Void_t **out)
#else
static ssize_t unrdb(vc, data, dtsz, out) Vcodex_t *vc;
Void_t *data;
size_t dtsz;
Void_t **out;
#endif
{
    ...
    for (f = 0; f < rc->info.fldn; ++f) { /* read the predictor */
        if (vciomore(&io) <= 0 || (rc->plan->pred[f] = vciogetu(&io)) < 0
            || rc->plan->pred[f] >= rc->info.fldn)
            GOTO(done);
        ...
        if (vciomore(&io) <= 0 || (rc->fld[f].type = vciogetc(&io)) < 0)
            GOTO(done);
        if (f > 0
            && (rc->fld[f].type & RDB_WHOLE) != (rc->fld[0].type & RDB_WHOLE))
            GOTO(done); /* WHOLE should be uniform to all fields */

        if (rc->info.flen[f]
            == 0) { /* old Vcrdb coding of a converted field of form x.y... */
            if (flen > 0
                && (rc->fld[f].type == 0 || rc->fld[f].type == RDB_SLASH))
    ...
    /* get data out */
    if ((z = vcrdextract(tbl, -1, NIL(Vcchar_t *), 0, VCRD_RECORD)) < 0
        || z != size)
        GOTO(done);
    ...
    if (out)
        *out = output;
    rv = z; /**/
    DEBUG_TALLY(1, N_recn, tbl->recn);
    DEBUG_PRINT(2, "#records=%d\n", N_recn);
...
#if __STD_C
static ssize_t
rdbextract(Vcodex_t *vc, Vcchar_t **datap)
#else
static ssize_t rdbextract(vc, datap) Vcodex_t *vc;
...
#endif
{
    ...
    for (z = 0, arg = &_Rdbargs[0]; arg->name; ++arg) {
        if (!(rdb->type & TYPECAST(int, arg->data))
            || !(TYPECAST(int, arg->data) & RDB_HEADER))
            continue;
        z += strlen(arg->name) + 1;
    }
    if (z == 0)
...
#if __STD_C
static int
rdbrestore(Vcmtcode_t *mtcd)
#else
static int rdbrestore(mtcd) Vcmtcode_t *mtcd;
#endif
{
    Vcmtarg_t *arg;
    char *data;
...
#if __STD_C
static int
rdbevent(Vcodex_t *vc, int type, Void_t *params)
#else
static int rdbevent(vc, type, params) Vcodex_t *vc;
int type;
Void_t *params;
#endif
{
...
= { rdb,
    ...
    rdbevent,
    "rdb",
    "Transforming relational data by field dependency.",
