...
#endif
{
    ...
    for (k = 0; k < ns;) { /* starting index of this code in the table */
        ...
        } else /* internal node, find all codes sharing same prefix */
        {
            ...
            if (bldtrie(trie, clen, bits, &nd, len, sort + k, m - k, lev + 1)
                < 0)
                return -1;
...
#if __STD_C
static int
bitscmp(Void_t *one, Void_t *two, Void_t *disc)
#else
static int bitscmp(one, two, disc) Void_t *one;
Void_t *two;
Void_t *disc;
#endif
{
...
#if __STD_C
Vchtrie_t *
vchbldtrie(ssize_t nsym, ssize_t *size, Vcbit_t *bits)
#else
Vchtrie_t *vchbldtrie(nsym, size, bits) ssize_t nsym; /* alphabet size or
                                                         #symbols	*/
