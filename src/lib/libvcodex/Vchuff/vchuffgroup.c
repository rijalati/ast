...
#define GRPSIZE(v, sz, o, n)                                                 \
    do {                                                                     \
        ...
        v = 0;                                                               \
        for (; nn > 0; nn -= VCH_SW)                                         \
            GRPsize(v, sz, oo, nn >= VCH_SW ? VCH_SW : nn);                  \
    } while (0)


#if __STD_C
static int
objcmp(Void_t *one, Void_t *two, Void_t *hdl)
#else
static int objcmp(one, two, hdl) Void_t *one;
Void_t *two;
Void_t *hdl;
#endif
{
...
#if __STD_C
static int
grpinit(Group_t *grp, Vchobj_t *data, size_t dtsz, ssize_t ptsz)
#else
static int grpinit(grp, data, dtsz, ptsz) Group_t *grp;
Vchobj_t *data;
size_t dtsz;
ssize_t ptsz;
#endif
{
    ...
    grp->cmpsz
    = (dtsz < VCH_SIZE ? VCH_SIZE : dtsz) * VC_BITSIZE; /* starting cost */
    grp->ntbl = 0;
    ...
    for (d = 0, p = 0, i = 0; i < npts; i += 1, d += ptsz) {
        ...
        vcqsort(
        grp->obj + grp->ppos[i], p - grp->ppos[i], sizeof(Obj_t), objcmp, 0);
    }
    grp->ppos[npts] = p;
...
#if __STD_C
static int
partcmp(Void_t *one, Void_t *two, Void_t *disc)
#else
static int partcmp(one, two, disc) Void_t *one;
Void_t *two;
Void_t *disc;
#endif
{
...
#if __STD_C
static void
grppart(Group_t *grp, ssize_t ntbl, int niter)
#else
...
#endif
{
    ...
    {      /**/
        ...
        for (; n > 0; --n) {
             ...
             */
            tbl[p].maxs
            = vchsize(VCH_SIZE, freq[p], tbl[p].size, &tbl[p].runb);
            tbl[q].maxs
            = vchsize(VCH_SIZE, freq[q], tbl[q].size, &tbl[q].runb);
    ...
    for (iter = 1;; iter++) { /**/
            ...
            if ((z = tbl[k].maxs) > 0) {
                ...
                for (p = 0; p < VCH_SIZE; ++p) {
                    if (fr[p] != 0)
                        ...
                        sz[p] = z; /* 0-freq obj gets dflt length */
                }
            }
            tbl[k].cost = 0;
            tbl[k].nblks = 0;
        ...
        if (ntbl > 1) /* add the cost of encoding the indices */
        {
            ...
            CLRTABLE(pfr, VCH_SIZE);
            ADDFREQ(pfr, Vcchar_t *, dt, n);
...
#if __STD_C
static ssize_t
grphuff(Vcodex_t *vc, const Void_t *data, size_t dtsz, Void_t **out)
#else
...
#endif
{
    ...
    if ((itbl = vclogi(grp->npts)) > GRP_IMAX)
        ...
        itbl = GRP_IMIN;
    /**/ DEBUG_PRINT(2, "grphuff: dtsz=%d, ", dtsz);
    DEBUG_PRINT(2, "ptsz=%d, ", ptsz);
    /**/ DEBUG_PRINT(2, "npts=%d, ", grp->npts);
    DEBUG_PRINT(2, "itbl=%d\n", itbl);
    ...
    n_output = (ntbl + 1) * (2 * VCH_SIZE)
               + (grp->cmpsz + 7) / 8; /* upper-bound for output size */
    ...
    for (k = 0; k < ntbl; ++k) {
        ...
        else /* coding a code table */
        {
            dt = vcionext(&io);
            n = vciomore(&io);
            if ((n = vchputcode(VCH_SIZE, tbl[k].size, tbl[k].maxs, dt, n))
                < 0)
                return -1;
            else
                vcioskip(&io, n);
...
#if __STD_C
static ssize_t
grpunhuff(Vcodex_t *vc, const Void_t *orig, size_t dtsz, Void_t **out)
#else
...
#endif
{
    ...
    for (k = 0; k < ntbl; ++k) {
        ...
        else /* construct code table and trie for fast matching	*/
        {
            dt = vcionext(&io);
            n = vciomore(&io);
            if ((n = vchgetcode(VCH_SIZE, tbl[k].size, tbl[k].maxs, dt, n))
                < 0)
                goto done;
            else
                vcioskip(&io, n);
...
#if __STD_C
static int
grpevent(Vcodex_t *vc, int type, Void_t *params)
#else
static int grpevent(vc, type, params) Vcodex_t *vc;
int type;
Void_t *params;
#endif
{
