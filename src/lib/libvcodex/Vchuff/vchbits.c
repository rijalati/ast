...
#if __STD_C
static int
sizecmp(Void_t *one, Void_t *two, Void_t *disc)
#else
static int sizecmp(one, two, disc) Void_t *one;
Void_t *two;
Void_t *disc;
#endif
{
...
#if __STD_C
ssize_t
vchbits(ssize_t nsym, ssize_t *size, Vcbit_t *bits)
#else
ssize_t vchbits(nsym, size, bits) ssize_t nsym; /* alphabet size or #symbols
                                                 */
