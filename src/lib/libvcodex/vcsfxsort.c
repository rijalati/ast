...
static int
chktodo(Vcsfx_t *sfx, Vcinx_t lo, Vcinx_t hi)
{
    static int Dblev = -1;
...
static int
cmpsfx(Vcsfx_t *sfx, Vcinx_t p, Vcinx_t s)
{
    ...
    if (p < s)
        sfprintf(sfstdout,
                 "cmpsfx: ps=%d > ss=%d because np=%d < ns=%d\n",
                 ps - sfx->str,
                 ss - sfx->str,
                 p,
                 s);
...
static int
chksorted(Vcsfx_t *sfx, Vcinx_t lo, Vcinx_t hi)
{
...
static int
chkone2one(Vcinx_t *list, Vcinx_t size)
{
    Vcinx_t i;
    Vcchar_t *chk;
...
static int
chkinverse(Vcinx_t *idx, Vcinx_t *inv, Vcinx_t size)
{
    Vcinx_t i;
...
static int
chkheader(Vcsfx_t *sfx, Vcinx_t *lo, Vcinx_t *hi, Vcinx_t len)
{
    for (lo += 1; lo <= hi; ++lo)
        /**/ DEBUG_ASSERT(memcmp(sfx->str + lo[-1], sfx->str + lo[0], len)
                        == 0);
}

/* make sure that [min,le) and (re,max] have been completely sorted */
static int
chkbounds(Vcsfx_t *sfx, Vcinx_t *min, Vcinx_t *max, Vcinx_t *le, Vcinx_t *re)
{
...
#define GMAX(g, x, y, e)                                                     \
    (((g)[((x) << 8) + (y) + 1]) - (((x) == (e) && (y) == 255) ? 1 : 0) - 1)

#if __STD_C
static void
sfxbsort(Vcsfx_t *sfx, Vcinx_t *grp)
#else
static void sfxbsort(sfx, grp) Vcsfx_t *sfx;
Vcinx_t *grp;
#endif
{
    int i, j, v;
...
static void
sfxswap(Vcinx_t *min, Vcinx_t *mid, Vcinx_t *max)
#else
static void sfxswap(min, mid, max) Vcinx_t *min;
Vcinx_t *mid;
Vcinx_t *max;
#endif
{
...
static void
sfxqsort(Vcsfx_t *sfx, Vcinx_t *min, Vcinx_t *max, Vcinx_t hdr)
#else
static void sfxqsort(sfx, min, max, hdr) Vcsfx_t *sfx;
...
q_sort:
    if ((sz = max - min + 1) <= INSERTSORT) /* insertion sort */
    {
...
static int
sfxosort(Vcsfx_t *sfx, Vcinx_t x, Vcinx_t *grp, int dir)
#else
...
#endif
{
         ...
         nd = ( Grnode_t * )dtnext(gr->nodes, nd))
    {
        if (!nd->iedge) /* no incoming edge, hence a root */
        {
            if (!hd)
                ...
                (tl = tl->link = nd)->link = NIL(Grnode_t *);
        }
    }
    for (nd = hd; nd; nd = nd->link) /* top-sort */
    {
        ...
        l = grp[(x << 8) + TYPECAST(Vcinx_t, nd->label)];
        r = inv[idx[l]];
...
#endif
{
    ...
    /**/ DEBUG_PRINT(2, "Cn=%d,", Cn);
    DEBUG_PRINT(2, "Cz=%d\n", Cz);
    /**/ DEBUG_PRINT(2, "Gn=%d,", Gn);
    DEBUG_PRINT(2, "Gz=%d\n", Gz);
