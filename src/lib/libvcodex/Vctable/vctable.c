...
#if __STD_C
static Vctblplan_t *
tbltrain(const Void_t *train, size_t trsz, ssize_t ncols, int flags)
#else
static Vctblplan_t *
tbltrain(train, trsz, ncols, flags) Void_t *train; /* training data */
size_t trsz;
ssize_t ncols;
int flags;
#endif
...
#if __STD_C
static void
clrindex(Table_t *tbl)
#else
static void clrindex(tbl) Table_t *tbl;
#endif
{
    Tblindex_t *ip, *endip;
...
#if __STD_C
static ssize_t *
getindex(Table_t *tbl, ssize_t pred1, ssize_t pred2)
#else
static ssize_t *getindex(tbl, pred1, pred2) Table_t *tbl; /* to store sorted
                                                             indices	*/
...
#if __STD_C
static ssize_t
table(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t table(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
...
re_train: /* recurse to here if necessary to retrain data */
    ...
    /**/ DEBUG_PRINT(2, "Ncols=%d, ", ncols);
    DEBUG_PRINT(2, "Compressed ratio=%.2f\n", ratio);
...
#if __STD_C
static ssize_t
untable(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t untable(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
...
#if __STD_C
static int
tblevent(Vcodex_t *vc, int type, Void_t *params)
#else
static int tblevent(vc, type, params) Vcodex_t *vc;
int type;
Void_t *params;
#endif
{
...
Vcmethod_t _Vctable = { table,
                        untable,
                        tblevent,
                        "table",
                        "Fixed-length record table transform.",
                        "[-?\n@(#)$Id: vcodex-table (AT&T Research) "
                        "2003-01-01 $\n]" USAGE_LICENSE,
                        _Tbldata,
                        16 * 1024 * 1024,
                        0 };
