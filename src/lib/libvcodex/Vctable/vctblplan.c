...
#if __STD_C
static int
plevent(Graph_t *gr, int type, Void_t *arg, Grdisc_t *disc)
#else
static int plevent(gr, type, arg, disc) Graph_t *gr;
int type;
Void_t *arg;
Grdisc_t *disc;
#endif
...
#if __STD_C
static int
plnodecmp(Dt_t *dt, Void_t *one, Void_t *two, Dtdisc_t *disc)
#else
static int plnodecmp(dt, one, two, disc) Dt_t *dt;
Void_t *one;
Void_t *two;
Dtdisc_t *disc;
#endif
...
#if __STD_C
Vctblplan_t *
vctblmakeplan(const Void_t *train, size_t dtsz, size_t ncols, int flags)
#else
Vctblplan_t *
...
#endif
{
    ...
    { /* create the node corresponding to this column */
        ...
        nd->wght = (*entropyf)(NIL(Vcchar_t *),
                               ...
                               &nd->repn);
        if (nd->repn
            >= nrows) /* single run, keep separate from predictions */
            continue;
    ...
    if ((w = grbranching(gr)) < 0)
        goto done; /**/
    DEBUG_PRINT(2, "Grbranching weight = %d\n", w);
...
#if __STD_C
void
vctblfreeplan(Vctblplan_t *plan)
#else
void vctblfreeplan(plan) Vctblplan_t *plan;
#endif
{
    if (plan->zip)
        vcclose(plan->zip);
...
#if __STD_C
ssize_t
vctblencodeplan(Vctblplan_t *plan, Void_t **codep)
#else
ssize_t vctblencodeplan(plan, codep) Vctblplan_t *plan; /* the plan to be
                                                           coded		*/
...
#if __STD_C
Vctblplan_t *
vctbldecodeplan(Void_t *data, size_t size)
#else
Vctblplan_t *vctbldecodeplan(data, size) Void_t *data;
size_t size;
#endif
{
    Vctblcolumn_t *trans;
