...
#if __STD_C
static int
rtdatacmp(Dt_t *dt, void *o1, void *o2, Dtdisc_t *disc)
#else
static int rtdatacmp(dt, o1, o2, disc) Dt_t *dt;
void *o1;
void *o2;
Dtdisc_t *disc;
#endif
...
#if __STD_C
static int
rtordncmp(Dt_t *dt, void *o1, void *o2, Dtdisc_t *disc)
#else
static int rtordncmp(dt, o1, o2, disc) Dt_t *dt;
void *o1;
void *o2;
Dtdisc_t *disc;
#endif
...
#if __STD_C
static unsigned int
rthash(Dt_t *dt, void *o, Dtdisc_t *disc)
#else
static unsigned int rthash(dt, o, disc) Dt_t *dt;
void *o;
Dtdisc_t *disc;
#endif
{
...
#if __STD_C
static void
rtfree(Dt_t *dt, void *o, Dtdisc_t *disc)
#else
static void rtfree(dt, o, disc) Dt_t *dt;
void *o;
Dtdisc_t *disc;
#endif
{
...
= { 0,         0,      DTOFFSET(Rtval_t, dlnk), NIL(Dtmake_f), rtfree,
    rtdatacmp, rthash, NIL(Dtmemory_f),         NIL(Dtevent_f) };

static Dtdisc_t Rtordn = { 0,
                           0,
                           DTOFFSET(Rtval_t, olnk),
                           NIL(Dtmake_f),
                           NIL(Dtfree_f),
                           rtordncmp,
                           NIL(Dthash_f),
...
#endif
{
    ...
    if (dz <= 0 || (dz % vcsizem(vmax)) != 0
        || (dz / vcsizem(vmax)) != rf->vcnt)
        return -1;
    vcioinit(&io, dt, dz);
...
#if __STD_C
static void
rtctxtclear(Rtctxt_t *rc)
#else
static void rtctxtclear(rc) Rtctxt_t *rc;
#endif
{
    ssize_t k;
...
#if __STD_C
static ssize_t
rtable(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t rtable(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
{
    ...
    if (!vc || !data || !(rt = vcgetmtdata(vc, Rtable_t *))
    ...
    for (rowz = 0, f = 0; f < fldn;
         ++f) /* estimate row size of encoding table */
    {
        if (rc->fld[f].type
            & (RT_ISEQ | RT_ASIS)) /* fixed-length coding in table */
    ...
    DEBUG_PRINT(2, "fldn=%d ", fldn);
    DEBUG_PRINT(2, "recn=%d ", recn);
    DEBUG_PRINT(2, "rowz=%d\n", rowz);
    ...
    for (edt = (dt = ( Vcchar_t * ) data) + size; dt < edt;) {
        for (f = 0;;) {
                       ...
                       & RT_ISEQ) /* code as an integer sequence */
            {
                ...
                /**/ DEBUG_ASSERT(
                z == rc->fld[f].vcnt * vcsizem(rc->fld[f].vmax));
                if (z <= 0)
                    GOTO(done);
                vcioskip(&io, z);
    ...
    if (rv < 0) /* if an error happened, clear context */
        rtctxtclear(rc);
    return rv;
}
...
#if __STD_C
static ssize_t
unrtable(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t unrtable(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
{
    ...
    for (f = 0; f < fldn; ++f) {
        ...
        if (type
            & (RT_CLEAR | RT_ISEQ | RT_ASIS)) /* clear all current values */
        {
            if (rc->fld[f].val)
    ...
    for (enddt = (dt = output) + dtsz; vciomore(&io) > 0;) {
        for (f = 0; f < fldn; ++f) {
            if (rc->fld[f].type & RT_ASIS) /* field's own uncoded data */
            {
                ...
                z
                = (rc->fld[f].type & RT_FIXZ) ? rc->fldz[f] : rc->fld[f].vcnt;
                       ...
                       & RT_ISEQ) /* field coded as integer sequence */
            {
                ...
                valdt = vcionext(&io);
                vcioskip(&io, v);
                if ((z = rti2a(rt, rc->fld + f, valdt, v, dt, enddt - dt))
                    < 0)
                    GOTO(done);
                dt += z;
...
#if __STD_C
static int
rtevent(Vcodex_t *vc, int type, Void_t *params)
#else
static int rtevent(vc, type, params) Vcodex_t *vc;
int type;
Void_t *params;
#endif
{
...
Vcmethod_t _Vcrtable = { rtable,
                         unrtable,
                         rtevent,
                         "rtable",
                         "Transforming/Compressing a relational data table.",
                         "[-?\n@(#)$Id: vcodex-rtable (AT&T Research) "
                         "2011-07-20 $\n]" USAGE_LICENSE,
                         _Rtargs,
                         4 * 1024 * 1024,
                         0 };
