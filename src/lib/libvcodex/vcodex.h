...
Help needed to define signed 32
- bit integer type.
#    endif
...
#    if !defined(Vcuint32_t)
  Help needed to define unsigned 32
- bit integer type.
#    endif
...
#    define vcioputs(io, str, sz)                                            \
        (memcpy((io)->next, (str), (sz)), (io)->next += (sz))
#    define vciogets(io, str, sz)                                            \
        (memcpy((str), (io)->next, (sz)), (io)->next += (sz))
#    define vciomove(io1, io2, sz)                                           \
        (memcpy((io2)->next, (io1)->next, (sz)),                             \
         (io1)->next += (sz),                                                \
         (io2)->next += (sz))
#    define vcioputm(io, v, m) _vcioputm((io), (Vcint_t)(v), (Vcint_t)(m))
#    define vciogetm(io, m) _vciogetm((io), (Vcint_t)(m))
#    define vcioputu(io, v)                                                  \
        ((Vcint_t)(v) < (Vcint_t)(1 << 7)                                    \
         ? (*(io)->next++ = (Vcchar_t)(v), 1)                                \
         : _vcioputu((io), (Vcint_t)(v)))
#    define vciogetu(io) _vciogetu((io))
#    define vcioput2(io, v, a, z)                                            \
        _vcioput2((io), (Vcint_t)(v), (Vcchar_t)(a), (Vcchar_t)(z))
...
** For convenience, the Vcio_t structure itself provides two fields
*(bits,nbits)
** usable as (bt,nb). In this way, applications that perform bits ops in
*function
...
typedef ssize_t(*Vclzparse_f)
_ARG_((Vclzparse_t *, int, Vclzmatch_t *, ssize_t));
...
#                define VCLIB(m)                                             \
                    Vcmethod_t *m = &_##m;                                   \
                    extern __EXPORT__ Vcmethod_t *vcodex_lib(                \
                    const char *path)                                        \
                    {                                                        \
                        return m;                                            \
                    }                                                        \
                    ...
                    {                                                        \
                        return VCODEX_PLUGIN_VERSION;                        \
                    }
#            else
#                define VCLIB(m)                                             \
                    Vcmethod_t *m = &_##m;                                   \
                    extern Vcmethod_t *vcodex_lib(const char *path)          \
                    {                                                        \
                        return m;                                            \
                    }                                                        \
                    unsigned long plugin_version(void)                       \
...
#            define VCLIB(m)                                                 \
                Vcmethod_t *m = &_ /**/ m;                                   \
                extern Vcmethod_t *vcodex_lib(path) char *path;              \
                {                                                            \
                    return m;                                                \
                }                                                            \
