...
#if __STD_C
ssize_t
vcperiod(const Void_t *data, ssize_t dtsz)
#else
ssize_t vcperiod(data, dtsz) Void_t *data;
ssize_t dtsz;
#endif
{
    Vcchar_t *dt;
...
#define NEIGHBOR 2 /* #neighbors to check on each side */
    peak = sfx->idx;
    memset(peak, 0, sz * sizeof(Vcinx_t));
    ...
    maxs = maxs / 8 > 0
           ? maxs / 8
           : maxs / 4 > 0 ? maxs / 4 : maxs / 2 > 0 ? maxs / 2 : maxs;
