...
#if __STD_C
static Void_t
tblfree(Dt_t *dt, Void_t *obj, Dtdisc_t *disc)
#else
static Void_t tblfree(dt, obj, disc) Dt_t *dt;
Void_t *obj;
Dtdisc_t *disc;
#endif
{
...
#if __STD_C
static int
tblcmp(Dt_t *dt, Void_t *obj1, Void_t *obj2, Dtdisc_t *disc)
#else
static int tblcmp(dt, obj1, obj2, disc) Dt_t *dt;
Void_t *obj1;
Void_t *obj2;
Dtdisc_t *disc;
#endif
...
#if __STD_C
static ssize_t
amapart(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t amapart(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
    ...
    for (tbl = dtfirst(szdt); tbl; tbl = dtnext(szdt, tbl)) {
        ...
        if (vc->coder && tbl->dtsz > 0) {
            ...
            if (tbl != sztbl)
                ...
                if (var->type
                    == AMA_FULLLINE) /* pass the new-line character */
            ...
            sfsprintf(schema,
                      sizeof(schema),
                      "[%d]",
                      z); /* pass value as schema and #columns */
#else
            sprintf(
            schema, "[%d]", z); /* pass value as schema and #columns */
#endif
        ...
        zipsz
        += vcsizeu(tbl->dtsz) + tbl->dtsz; /* total data size for table */
    }
...
#if __STD_C
static ssize_t
amaunpart(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t amaunpart(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
    ...
    for (size = 0, ntbl -= 1; ntbl > 0; --ntbl) {
        ...
        else /* count 0-length records via sztbl */
        {
            if (var->type == AMA_TEXTLINE)
                ...
                sz = -1; /* AMA_FULLLINE is never empty */
...
#if __STD_C
static int
amaevent(Vcodex_t *vc, int type, Void_t *params)
#else
static int amaevent(vc, type, params) Vcodex_t *vc;
int type;
Void_t *params;
#endif
{
