...
#if __STD_C
static ssize_t
amadiff(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t amadiff(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
    ...
    if (ncols > 0) /* fixed-length rows */
    {
        if ((sz = (size / ncols) * ncols) > 0)
        {
            ...
            if ((sz -= ncols) > 0)
            {
                ...
                else if ((a = ALIGN(df, short)) == ALIGN(dt, short)
                         && a == ALIGN(pd, short))
...
#if __STD_C
static ssize_t
unamadiff(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t unamadiff(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
...
Vcmethod_t _Vcamadiff = { amadiff,
                          unamadiff,
                          0,
                          "amadiff",
                          "Xor-ing rows in an AMA table.",
                          "[-?\n@(#)$Id: vcodex-amadiff (AT&T Research) "
                          "2003-01-01 $\n]" USAGE_LICENSE,
                          0,
                          1024 * 1024,
                          0 };
