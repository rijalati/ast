...
#if __STD_C
static void
vcdflsadd(Vcdiff_t *vcd, ssize_t dt, ssize_t az)
#else
static void vcdflsadd(vcd, dt, az) Vcdiff_t *vcd;
ssize_t dt;
ssize_t az;
#endif
{
...
#if __STD_C
static void
vcdflsaddr(Vcdiff_t *vcd, ssize_t ad, ssize_t md)
#else
static void vcdflsaddr(vcd, ad, md) Vcdiff_t *vcd;
ssize_t ad;
ssize_t md;
#endif
{
    if (md <= VCD_HERE + vcd->cache->s_near)
        ...
        vcioputc(vcd->addr, ad); /* must be same[] cache */
}

#if __STD_C
static void
vcdflscopy(Vcdiff_t *vcd, ssize_t ad, ssize_t cz, ssize_t md)
#else
static void vcdflscopy(vcd, ad, cz, md) Vcdiff_t *vcd;
ssize_t ad;
ssize_t cz;
ssize_t md;
#endif
...
#if __STD_C
static ssize_t
vcdputcopy(Vcdiff_t *vcd, ssize_t dt, ssize_t cz, ssize_t mt)
#else
static ssize_t vcdputcopy(vcd, dt, cz, mt) Vcdiff_t *vcd;
        ...
        else /* saved ADD */
        {
            if (cz <= 0 || sav->dtsz > siz->add1[md] || cz > siz->copy2[md])
                ...
                if (sav->addr < vcd->vcpa.nsrc)
                    ...
                    data = vcd->vcpa.tar + (sav->addr - vcd->vcpa.nsrc);
...
#if __STD_C
static ssize_t
vcdputadd(Vcdiff_t *vcd, ssize_t dt, ssize_t az)
#else
static ssize_t vcdputadd(vcd, dt, az) Vcdiff_t *vcd;
ssize_t dt;
ssize_t az;
#endif
{
...
#if __STD_C
static ssize_t
vcddiff(Vcodex_t *vc, const Void_t *tar, size_t ntar, Void_t **del)
#else
static ssize_t vcddiff(vc, tar, ntar, del) Vcodex_t *vc;
Void_t *tar;
size_t ntar;
Void_t **del;
#endif
{
    ...
    if (!(output = vcbuffer(vc, NIL(Vcchar_t *), 3 * (ntar + SLOP), 0)))
        return -1;
    vcd->data = &data;
    vcioinit(&data, output + SLOP, ntar);
    vcd->inst = &inst;
    vcioinit(&inst, vcionext(&data) + ntar, ntar + SLOP);
    vcd->addr = &addr;
    vcioinit(&addr, vcionext(&inst) + ntar + SLOP, ntar + SLOP);

    /* initialize address caches and saved COPY instruction */
    vcdkaclear(vcd->cache);
    vcd->save = &save;
    save.dtsz = save.addr = save.mode = 0;
    ...
    ctrl = 0;
    d = vciosize(vcd->data);
    rd = vciodata(vcd->data);
    i = vciosize(vcd->inst);
    ri = vciodata(vcd->inst);
    a = vciosize(vcd->addr);
...
#if __STD_C
static ssize_t
vcdundiff(Vcodex_t *vc, const Void_t *del, size_t ndel, Void_t **out)
#else
static ssize_t vcdundiff(vc, del, ndel, out) Vcodex_t *vc;
Void_t *del;
size_t ndel;
Void_t **out;
#endif
...
#if __STD_C
static int
vcdevent(Vcodex_t *vc, int type, Void_t *init)
#else
static int vcdevent(vc, type, init) Vcodex_t *vc;
int type;
Void_t *init;
#endif
{
...
= { vcddiff,
    ...
    vcdevent,
    "delta",
    "(Delta) Compression using the Vcdiff format.",
