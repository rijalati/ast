...
#if __STD_C
static void
vcdtbl2str(Vcdtable_t *tbl, Void_t *argstr)
#else
static void vcdtbl2str(tbl, argstr) Vcdtable_t *tbl;
Void_t *argstr;
#endif
{
    int i;
...
#if __STD_C
static void
vcdstr2tbl(Vcdtable_t *tbl, Void_t *argstr)
#else
static void vcdstr2tbl(tbl, argstr) Vcdtable_t *tbl;
Void_t *argstr;
#endif
{
    int i;
...
#if __STD_C
ssize_t
vcdputtable(Vcdtable_t *tbl, Void_t *buf, size_t n)
#else
ssize_t vcdputtable(tbl, buf, n) Vcdtable_t *tbl;
Void_t *buf;
size_t n;
#endif
{
    ...
    if (!(vc = vcopen(
          &disc, Vcdelta, NIL(Void_t *), NIL(Vcodex_t *), VC_ENCODE)))
        return -1;
...
#if __STD_C
int
vcdgettable(Vcdtable_t *tbl, Void_t *buf, size_t n)
#else
int vcdgettable(tbl, buf, n) Vcdtable_t *tbl;
Void_t *buf;
size_t n;
#endif
{
    ...
    disc.eventf = NIL(Vcevent_f);
    if (!(vc = vcopen(
          &disc, Vcdelta, NIL(Void_t *), NIL(Vcodex_t *), VC_DECODE)))
        return -1;

    vcioinit(&io, buf, n);
    tbl->s_near = vciogetc(&io);
    tbl->s_same = vciogetc(&io);
    if ((rv = vcapply(vc, vcionext(&io), vciomore(&io), &tarbuf))
        == VCD_TBLSIZE)
