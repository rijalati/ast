...
/* create file name extension mappings, text lines of the form 'pattern :
 * value' */
#if __STD_C
static void
zipalias(char *s)
...
/* map an alias. Arguments are passed onto the first method of the aliased
 * spec */
#if __STD_C
char *
...
#if __STD_C
static int
vcwalklist(Vcalias_t *al, Vcwalk_f walkf, Void_t *disc)
#else
static int vcwalklist(al, walkf, disc) Vcalias_t *al;
Vcwalk_f walkf;
Void_t *disc;
#endif
{
...
#if __STD_C
int
vcwalkalias(Vcwalk_f walkf, Void_t *disc)
#else
int vcwalkalias(walkf, disc) Vcwalk_f walkf;
Void_t *disc;
#endif
{
    if (!Alias)
...
#if __STD_C
int
vcwalkfname(Vcwalk_f walkf, Void_t *disc)
#else
int vcwalkfname(walkf, disc) Vcwalk_f walkf;
Void_t *disc;
#endif
{
    if (!Alias)
