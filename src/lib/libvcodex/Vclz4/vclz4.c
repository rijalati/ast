...
#if __STD_C
static int
_lz4_setmtarg(Vcodex_t *vc, Void_t *params)
#else
static int _lz4_setmtarg(vc, params) Vcodex_t *vc;
Void_t *params;
#endif
{
    Lz4_t *lz4;
...
#if __STD_C
static int
_lz4_opening(Vcodex_t *vc, Void_t *params)
#else
static int _lz4_opening(vc, params) Vcodex_t *vc;
Void_t *params;
#endif
{
    Lz4_t *lz4;
...
#if __STD_C
static int
_lz4_closing(Vcodex_t *vc, Void_t *params)
#else
static int _lz4_opening(vc, params) Vcodex_t *vc;
Void_t *params;
#endif
...
#if __STD_C
static ssize_t
vclz4(Vcodex_t *vc, const Void_t *data, size_t dtsz, Void_t **out)
#else
static ssize_t vclz4(vc, data, dtsz, out) Vcodex_t *vc;
Void_t *data;
size_t dtsz;
Void_t **out;
#endif
{
    ...
    /*
     * Estimate the compressed data size.  The lz4c application just uses the
     * window size plus some fudge for the output buffer size, which will in
     * general be too large, but I do not think this is a problem for
     * vcbuffer() and memory usage.
     */
    s = dtsz + vcsizeu(dtsz);
    ...
    else
    {
        ...
        /*
         * Get the final size of the output buffer, which is the LZ4
         * compressed size plus the size of the original buffer size number.
         */
        s = vciosize(&io);
...
#if __STD_C
static ssize_t
vcunlz4(Vcodex_t *vc, const Void_t *orig, size_t dtsz, Void_t **out)
#else
static ssize_t vcunlz4(vc, orig, dtsz, out) Vcodex_t *vc;
...
#endif
{
    ...
    /*
     * Prepare the vcio buffer for the data decoded by the lower level
     * transforms
     */
...
#if __STD_C
static int
lz4event(Vcodex_t *vc, int type, Void_t *params)
#else
static int lz4event(vc, type, params) Vcodex_t *vc;
int type;
Void_t *params;
#endif
{
