...
#if __STD_C
static Vchash_t
pfhash(Vcchar_t *data, ssize_t dtsz)
#else
static Vchash_t pfhash(data, dtsz) Vcchar_t *data;
ssize_t dtsz;
#endif
{
    Vchash_t key;
...
#if __STD_C
static Pffile_t *
pfopen(Sfio_t *sf, ssize_t sz, int srcfile)
#else
static Pffile_t *pfopen(sf, sz, srcfile) Sfio_t *sf;
ssize_t sz;
int srcfile;
#endif
{
...
#if __STD_C
static int
pfbuild(Pffile_t *pf, ssize_t blksz)
#else
static int pfbuild(pf, blksz) Pffile_t *pf;
ssize_t blksz;
#endif
{
    Vchash_t k, r, n;
...
#if __STD_C
static ssize_t
pfsegment(Pffile_t *pf, ssize_t ld, ssize_t lm, ssize_t mn, ssize_t blksz)
#else
static ssize_t pfsegment(pf, ld, lm, mn, blksz) Pffile_t *pf;
ssize_t ld;
ssize_t lm;
ssize_t mn;
ssize_t blksz;
#endif
{
    ...
    if ((seg = pf->nseg > 0 ? pf->seg + pf->nseg - 1 : 0)
        && lmt <= (seg->rmt + PFMERGE(blksz))
        && rmt >= (seg->lmt - PFMERGE(blksz))) {
        if (seg->lmt > lmt)
            seg->lmt = lmt;
        if (seg->rmt < rmt)
            seg->rmt = rmt;
...
#if __STD_C
static ssize_t
pfparse(Pffile_t *pf, Vchash_t *dt, ssize_t n, Sfoff_t pos, ssize_t blksz)
#else
static ssize_t pfparse(pf, dt, n, pos, blksz) Pffile_t *pf;
Vchash_t *dt;
ssize_t n;
Sfoff_t pos;
ssize_t blksz;
    ...
    for (d = 0, n -= (blksz - 1); d < n; d = dd + mn) {
        for (mn = 0, mm = -1, dd = d; dd < n; ++dd) {
            if (!BLOOMTEST(pf, dt[dd], i, k))
                continue;
            for (o = pf->tab[pfindex(pf, dt[dd])].list; o;
                 o = o->next) { /* find a stretch of approximate matches */
                k = dd;
                i = o - pf->obj;
                ucnt = 0;
...
#if __STD_C
static int
pfwindow(Pffile_t *pf, Vcwmatch_t *wm, ssize_t blksz)
#else
static int pfwindow(pf, wm, blksz) Pffile_t *pf;
Vcwmatch_t *wm;
ssize_t blksz;
#endif
{
    ...
    if (pf->cseg < pf->nseg)
        ...
        sz = (ssize_t)((pf->dtpos + pf->dtsz) - seg->rdt);
    if (sz > 0) {
        seg->rdt += sz;
...
#if __STD_C
static Vcwmatch_t *
pfmatch(Vcwindow_t *vcw, Void_t *data, size_t dtsz, Sfoff_t here)
#else
...
#endif
{
    ...
    if ((pf = pfx->srcf)
        ...
        if (dtsz < pfx->blksz) /* small data, use matching window */
        {
            wm->wpos = here;
            wm->wsize
            = here + dtsz > pf->sfsz ? (ssize_t)(pf->sfsz - here) : dtsz;
...
target_file: /* returning computed windows in target file */
    ...
    if (here != pfx->here || !pf
        ...
        *(key = pfx->key) = k == PF_HUGE ? (k >> 1) : k;
        ...
        if ((pf ? pfparse(pf, pfx->key, dtsz, here, pfx->blksz) : 0)
            <= 0) { /* fail source, try target */
            if ((pf = pfx->tarf)
...
#if __STD_C
static int
pfevent(Vcwindow_t *vcw, int type)
#else
static int pfevent(vcw, type) Vcwindow_t *vcw;
int type;
#endif
{
