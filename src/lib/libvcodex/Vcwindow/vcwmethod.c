...
= { &_Vcwprefix, &_Vcwmirror, &_Vcwvote, 0 };

#if __STD_C
Vcwmethod_t *
vcwgetmeth(char *name)
#else
Vcwmethod_t *vcwgetmeth(name) char *name;
#endif
{
    int i, k;
    ...
    for (i = 0; _Vcwmethods[i]; ++i)
    {
        for (k = 0; name[k] && _Vcwmethods[i]->name[k]; ++k)
            if (name[k] != _Vcwmethods[i]->name[k])
                break;
        if (name[k] == 0 || name[k] == '.' || name[k] == '='
            || name[k] == '-') /* match a prefix */
...
#if __STD_C
int
vcwwalkmeth(Vcwalk_f walkf, Void_t *disc)
#else
int
vcwwalkmeth(Vcwalk_f walkf, Void_t *disc) Vcwalk_t walkf;
Void_t *disc;
#endif
{
    int i, rv;
