...
#define ACCEPT(sig)                                                          \
    (sig > (1 << 24) ? .05                                                   \
                     : sig > (1 << 20) ? .06 : sig > (1 << 16) ? .07 : .08)

#if __STD_C
static int
sigcmp(Void_t *one, Void_t *two, Void_t *disc)
#else
static int sigcmp(one, two, disc) Void_t *one;
Void_t *two;
Void_t *disc;
#endif
{
...
#if __STD_C
static int
votecmp(Void_t *one, Void_t *two, Void_t *disc)
#else
static int votecmp(one, two, disc) Void_t *one;
Void_t *two;
Void_t *disc;
#endif
{
...
#if __STD_C
static int
idxcmp(Void_t *one, Void_t *two, Void_t *disc)
#else
static int sigcmp(one, two) Void_t *one;
Void_t *two;
Void_t *disc;
#endif
{
...
vcwfopen(Sfio_t *f)
#else
Vcwfile_t *vcwfopen(f) Sfio_t *f; /* file to match against	*/
#endif
{
    Sfoff_t size;
    Vcwfile_t *vcwf;
...
#if __STD_C
static int
vcwfsig(Vcwfile_t *vcwf)
#else
static int vcwfsig(vcwf) Vcwfile_t *vcwf;
#endif
{
    int nsig;
...
#if __STD_C
static void
vcwfvote(Vcwfile_t *vcwf, Cand_t *cand, Grint_t *msig, int v, int maxi)
#else
...
#if __STD_C
int
vcwfsearch(Vcwfile_t *vcwf, Vcchar_t *data, size_t size)
#else
