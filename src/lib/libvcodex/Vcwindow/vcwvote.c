...
#if __STD_C
static double
frinterval(Vcwindow_t *vcw, size_t *dfreq, size_t size, Sfoff_t l, Sfoff_t r)
#else
...
#if __STD_C
static double
frsearch(Vcwindow_t *vcw, size_t *dfreq, size_t size)
#else
...
#if __STD_C
static double
frtarget(Vcwindow_t *vcw, size_t *dfreq, size_t size, Sfoff_t here)
#else
...
#if __STD_C
static Vcwmatch_t *
frmatch(Vcwindow_t *vcw, Void_t *data, size_t dtsz, Sfoff_t here)
#else
...
#endif
{
    ...
    if (wm->type == 0) {
        ...
        wm->type = VCD_SOURCEFILE;
        wm->wpos
        = here + dtsz < fr->srcf->size ? here : fr->srcf->size - dtsz;
        if (wm->wpos < 0)
            wm->wpos = 0;
        wm->wsize = dtsz;
    }
...
#if __STD_C
static int
frevent(Vcwindow_t *vcw, int type)
#else
static int frevent(vcw, type) Vcwindow_t *vcw;
int type;
#endif
{
    Freq_t *fr;
