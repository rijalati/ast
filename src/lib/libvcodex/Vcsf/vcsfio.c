...
#define VCSFERROR(dc, m)                                                     \
    ((dc)->sfdt->errorf ? ((*(dc)->sfdt->errorf)(m), -1) : -1)
...
#ifndef SKIPVCDX /* deal with the ancient VCDX header that uses method       \
                    numbers */
...
static Map_t Map[] = {
    { "delta", 0 }, { "huffman", 1 },  { "huffgroup", 2 }, { "arith", 3 },
    { "bwt", 4 },   { "rle", 5 },      { "mtf", 6 },       { "transpose", 7 },
    { "table", 8 }, { "huffpart", 9 }, { "map", 50 },      { "ama", 100 },
    { "ss7", 101 },
};
...
#    else
static ssize_t
hdrvcdx(datap, dtsz, code, cdsz) Vcchar_t **datap; /* old header data	*/
ssize_t dtsz;
...
#if __STD_C
static int
putheader(Sfdc_t *sfdc)
#else
static int putheader(sfdc) Sfdc_t *sfdc;
#endif
{
    Vcchar_t *code;
    ssize_t sz;
...
#if __STD_C
static int
makebuf(Sfdc_t *sfdc, ssize_t size)
#else
static int makebuf(sfdc, size) Sfdc_t *sfdc;
ssize_t size;
#endif
{
...
#if __STD_C
static ssize_t
fillbuf(Sfdc_t *sfdc, Sfio_t *f, Sfdisc_t *disc)
#else
static ssize_t fillbuf(sfdc, f, disc) Sfdc_t *sfdc;
Sfio_t *f;
Sfdisc_t *disc;
#endif
{
...
#if __STD_C
static ssize_t
ident(Sfdc_t *sfdc, Void_t *data, size_t dtsz)
#else
static ssize_t ident(sfdc, data, dtsz) Sfdc_t *sfdc;
Void_t *data;
size_t dtsz;
#endif
{
...
#if __STD_C
static ssize_t
getheader(Sfdc_t *sfdc, Sfio_t *f, int init, int identify, int optional)
#else
static ssize_t getheader(sfdc, f, init, identify, optional) Sfdc_t *sfdc;
Sfio_t *f;
...
#endif
{
    ...
    if (identify) { /* verify header magic -- ignore if no magic */
        ...
        memcpy(cdbuf, code, 4);
        sfread(f, code, 0);
        if (cdbuf[0] != VC_HEADER0 || cdbuf[1] != VC_HEADER1
            || cdbuf[2] != VC_HEADER2
            || cdbuf[3] != VC_HEADER3 && cdbuf[3] != 0)
            return 0;
    }
    ...
    for (loop = 0;; ++loop) {
        ...
        if (loop > 0 || sz <= 0) {
            if (fillbuf(sfdc, f, sfdc->vc ? &sfdc->disc : NIL(Sfdisc_t *))
                ...
                return identify
                       ? 0
                       : init ? VCSFERROR(sfdc, "Bad header data") : -1;
        }
        ...
        if (vciogetc(&io) != VC_HEADER0 || vciogetc(&io) != VC_HEADER1
...
#ifndef SKIPVCDX /* deal with old headers that use method numbers instead of \
                    names */
...
#if __STD_C
static ssize_t
encode(Sfdc_t *sfdc, Vcchar_t *data, size_t dtsz)
#else
static ssize_t encode(sfdc, data, dtsz) Sfdc_t *sfdc;
Vcchar_t *data;
size_t dtsz;
#endif
{
...
#if __STD_C
static ssize_t
vcsfdcread(Sfio_t *f, Void_t *buf, size_t n, Sfdisc_t *disc)
#else
...
#if __STD_C
static ssize_t
vcsfdcwrite(Sfio_t *f, const Void_t *buf, size_t n, Sfdisc_t *disc)
#else
...
#if __STD_C
static Sfoff_t
vcsfdcseek(Sfio_t *f, Sfoff_t addr, int offset, Sfdisc_t *disc)
#else
static Sfoff_t vcsfdcseek(f, addr, offset, disc) Sfio_t *f;
Sfoff_t addr;
int offset;
Sfdisc_t *disc;
#endif
...
#if __STD_C
static int
vcsfdcexcept(Sfio_t *f, int type, Void_t *data, Sfdisc_t *disc)
#else
static int vcsfdcexcept(f, type, data, disc) Sfio_t *f;
int type;
Void_t *data;
Sfdisc_t *disc;
#endif
...
#    if __STD_C
static Rsrv_t *
vcsfrsrv(Sfio_t *f, ssize_t n)
#    else
static Rsrv_t *vcsfrsrv(f,
                        n) Sfio_t *f; /* stream to to create reserve buffer	*/
...
#    else
Void_t *sfreserve(f, n, type) Sfio_t *f;
ssize_t n;
int type;
#    endif
{
    ...
    if (type == SF_LASTR)
        return (!(r = vcsfrsrv(f, 0)) || r->dtsz <= 0) ? NIL(Void_t *)
                                                       : r->data;
...
#    if __STD_C
char *
sfgetr(Sfio_t *f, int nl, int type)
#    else
char *sfgetr(f, nl, type) Sfio_t *f;
int nl;
int type;
#    endif
{
...
#    if __STD_C
ssize_t
vcsfread(Vcsfio_t *vcf, Void_t *buf, size_t n)
#    else
ssize_t vcsfread(vcf, buf, n) Vcsfio_t *vcf;
Void_t *buf;
size_t n;
#    endif
{
...
#    if __STD_C
ssize_t
vcsfwrite(Vcsfio_t *vcf, const Void_t *buf, size_t n)
#    else
ssize_t vcsfwrite(vcf, buf, n) Vcsfio_t *vcf;
Void_t *buf;
size_t n;
#    endif
{
...
#    if __STD_C
int
vcsfsync(Vcsfio_t *vcf)
#    else
int vcsfsync(vcf) Vcsfio_t *vcf;
#    endif
{
    return vcsfdcexcept(
    (( Sfdc_t * ) vcf)->sf, SF_SYNC, 0, ( Sfdisc_t * ) vcf);
}

#    if __STD_C
int
vcsfclose(Vcsfio_t *vcf)
#    else
int vcsfclose(vcf) Vcsfio_t *vcf;
#    endif
{
