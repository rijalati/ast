/***********************************************************************
 *                                                                      *
 *               This software is part of the ast package               *
 *          Copyright (c) 2003-2013 AT&T Intellectual Property          *
 *                      and is licensed under the                       *
 *                 Eclipse Public License, Version 1.0                  *
 *                    by AT&T Intellectual Property                     *
 *                                                                      *
 *                A copy of the License is available at                 *
 *          http://www.eclipse.org/org/documents/epl-v10.html           *
 *         (with md5 checksum b35adb5213ca9657e911e9befb180842)         *
 *                                                                      *
 *              Information and Software Systems Research               *
 *                            AT&T Research                             *
 *                           Florham Park NJ                            *
 *                                                                      *
 *                     Phong Vo <phongvo@gmail.com>                     *
 *                                                                      *
 ***********************************************************************/
#include "vcxhdr.h"

/* National Institute of Standards and Technology,
** U.S. FIPS PUB 197 (FIPS 197), Advanced Encryption Standard.
**
** The below stream-based encryptor is implemented using the AES
** block-based encryptor and the Merkle-Damgard strategy for
** linking blocks. The same code does both encryption and digestion.
** The last step of digestion encodes the message length into the
** message digest.
**
** Written by Adam Edgar and Kiem-Phong Vo.
*/

#define AES_KEY128 (128 / 8) /* AES_128 key size in bytes	*/
#define AES_KEY192 (192 / 8) /* AES_192 key size in bytes	*/
#define AES_KEY256 (256 / 8) /* AES_256 key size in bytes	*/

#define AES_NROWS 4                    /* number of Rijndael rows	*/
#define AES_NB 4                       /* four 32-bit ints or 128 bits	*/
#define AES_STATE (AES_NROWS * AES_NB) /* size of state	*/
#define AES_EKEY (AES_NB * 64)         /* max expanded key size	*/

#define AES_BLOCK 16 /* encrypt blocks of this size	*/

typedef struct _aes_s /* an instance of AES */
{
    int K;  /* cipher key size in bytes 	*/
    int Nk; /* key size in 32-bit words	*/
    int Nr; /* # of transforming rounds	*/

    Vcchar_t ekey[AES_EKEY];  /* expanded key		*/
    Vcchar_t salt[AES_BLOCK]; /* encryption salt	*/

    Vcchar_t todo[AES_BLOCK]; /* data to be digested	*/
    ssize_t tdsz;             /* size of undigested data	*/

    Vcchar_t *obuf; /* output buffer		*/
    ssize_t obsz;   /* size of output buffer	*/
    ssize_t pdsz;   /* size of processed data	*/
} Aes_t;

static Vcchar_t Sbox[256]
= { /* forward s-box */
    
  };

static Vcchar_t InvSbox[256]
= { /* inverse s-box */
    
  };

static Vcchar_t Xtime2Sbox[256]
= { /* combined Xtimes2[Sbox[]] */
    
  };

static Vcchar_t Xtime3Sbox[256]
= { /* combined Xtimes3[Sbox[]] */
    
  };

/* modular multiplication tables based on:
    Xtime2[x] = (x & 0x80 ? 0x1b : 0) ^ (x + x)
    Xtime3[x] = x^Xtime2[x];
*/
#if 0
static Vcchar_t Xtime2[256] =
{
	
};
#endif

static Vcchar_t Xtime9[256]
= {  };

static Vcchar_t XtimeB[256]
= {  };

static Vcchar_t XtimeD[256]
= {  };

static Vcchar_t XtimeE[256]
= {  };

/* exchanges columns in each of 4 rows
    row0 - unchanged,
    row1 - shifted left 1,
    row2 - shifted left 2, and
    row3 - shifted left 3
*/
static void
ShiftRows(Vcchar_t *state)
{
    Vcchar_t tmp;

    /* just substitute row 0 */
    state[0] = Sbox[state[0]];
    state[4] = Sbox[state[4]];
    state[8] = Sbox[state[8]];
    state[12] = Sbox[state[12]];

    /* rotate row 1 */
    tmp = Sbox[state[1]];
    state[1] = Sbox[state[5]];
    state[5] = Sbox[state[9]];
    state[9] = Sbox[state[13]];
    state[13] = tmp;

    /* rotate row 2 */
    tmp = Sbox[state[2]];
    state[2] = Sbox[state[10]];
    state[10] = tmp;

    tmp = Sbox[state[6]];
    state[6] = Sbox[state[14]];
    state[14] = tmp;

    /* rotate row 3 */
    tmp = Sbox[state[15]];
    state[15] = Sbox[state[11]];
    state[11] = Sbox[state[7]];
    state[7] = Sbox[state[3]];
    state[3] = tmp;
}

/* restores columns in each of 4 rows
    row0 - unchanged,
    row1 - shifted right 1,
    row2 - shifted right 2, and
    row3 - shifted right 3
*/
static void
InvShiftRows(Vcchar_t *state)
{
    Vcchar_t tmp;

    /* restore row 0 */
    state[0] = InvSbox[state[0]];
    state[4] = InvSbox[state[4]];
    state[8] = InvSbox[state[8]];
    state[12] = InvSbox[state[12]];

    /* restore row 1 */
    tmp = InvSbox[state[13]];
    state[13] = InvSbox[state[9]];
    state[9] = InvSbox[state[5]];
    state[5] = InvSbox[state[1]];
    state[1] = tmp;

    /* restore row 2 */
    tmp = InvSbox[state[2]];
    state[2] = InvSbox[state[10]];
    state[10] = tmp;

    tmp = InvSbox[state[6]];
    state[6] = InvSbox[state[14]];
    state[14] = tmp;

    /* restore row 3 */
    tmp = InvSbox[state[3]];
    state[3] = InvSbox[state[7]];
    state[7] = InvSbox[state[11]];
    state[11] = InvSbox[state[15]];
    state[15] = tmp;
}

/* recombine and mix each row in a column */
static void
MixSubColumns(Vcchar_t *state)
{
    int i;
    Vcchar_t tmp[AES_STATE]; /**/
    DEBUG_ASSERT(AES_STATE == 16);

    /* mixing column 0 */
    tmp[0] = Xtime2Sbox[state[0]] ^ Xtime3Sbox[state[5]] ^ Sbox[state[10]]
             ^ Sbox[state[15]];
    tmp[1] = Sbox[state[0]] ^ Xtime2Sbox[state[5]] ^ Xtime3Sbox[state[10]]
             ^ Sbox[state[15]];
    tmp[2] = Sbox[state[0]] ^ Sbox[state[5]] ^ Xtime2Sbox[state[10]]
             ^ Xtime3Sbox[state[15]];
    tmp[3] = Xtime3Sbox[state[0]] ^ Sbox[state[5]] ^ Sbox[state[10]]
             ^ Xtime2Sbox[state[15]];

    /* mixing column 1 */
    tmp[4] = Xtime2Sbox[state[4]] ^ Xtime3Sbox[state[9]] ^ Sbox[state[14]]
             ^ Sbox[state[3]];
    tmp[5] = Sbox[state[4]] ^ Xtime2Sbox[state[9]] ^ Xtime3Sbox[state[14]]
             ^ Sbox[state[3]];
    tmp[6] = Sbox[state[4]] ^ Sbox[state[9]] ^ Xtime2Sbox[state[14]]
             ^ Xtime3Sbox[state[3]];
    tmp[7] = Xtime3Sbox[state[4]] ^ Sbox[state[9]] ^ Sbox[state[14]]
             ^ Xtime2Sbox[state[3]];

    /* mixing column 2 */
    tmp[8] = Xtime2Sbox[state[8]] ^ Xtime3Sbox[state[13]] ^ Sbox[state[2]]
             ^ Sbox[state[7]];
    tmp[9] = Sbox[state[8]] ^ Xtime2Sbox[state[13]] ^ Xtime3Sbox[state[2]]
             ^ Sbox[state[7]];
    tmp[10] = Sbox[state[8]] ^ Sbox[state[13]] ^ Xtime2Sbox[state[2]]
              ^ Xtime3Sbox[state[7]];
    tmp[11] = Xtime3Sbox[state[8]] ^ Sbox[state[13]] ^ Sbox[state[2]]
              ^ Xtime2Sbox[state[7]];

    /* mixing column 3 */
    tmp[12] = Xtime2Sbox[state[12]] ^ Xtime3Sbox[state[1]] ^ Sbox[state[6]]
              ^ Sbox[state[11]];
    tmp[13] = Sbox[state[12]] ^ Xtime2Sbox[state[1]] ^ Xtime3Sbox[state[6]]
              ^ Sbox[state[11]];
    tmp[14] = Sbox[state[12]] ^ Sbox[state[1]] ^ Xtime2Sbox[state[6]]
              ^ Xtime3Sbox[state[11]];
    tmp[15] = Xtime3Sbox[state[12]] ^ Sbox[state[1]] ^ Sbox[state[6]]
              ^ Xtime2Sbox[state[11]];

    for (i = 0; i < AES_STATE; ++i)
        state[i] = tmp[i];
}

/* restore and un-mix each row in a column */
static void
InvMixSubColumns(Vcchar_t *state)
{
    int i;
    Vcchar_t tmp[AES_STATE]; /**/
    DEBUG_ASSERT(AES_STATE == 16);

    /* restore column 0 */
    tmp[0] = XtimeE[state[0]] ^ XtimeB[state[1]] ^ XtimeD[state[2]]
             ^ Xtime9[state[3]];
    tmp[5] = Xtime9[state[0]] ^ XtimeE[state[1]] ^ XtimeB[state[2]]
             ^ XtimeD[state[3]];
    tmp[10] = XtimeD[state[0]] ^ Xtime9[state[1]] ^ XtimeE[state[2]]
              ^ XtimeB[state[3]];
    tmp[15] = XtimeB[state[0]] ^ XtimeD[state[1]] ^ Xtime9[state[2]]
              ^ XtimeE[state[3]];

    /* restore column 1 */
    tmp[4] = XtimeE[state[4]] ^ XtimeB[state[5]] ^ XtimeD[state[6]]
             ^ Xtime9[state[7]];
    tmp[9] = Xtime9[state[4]] ^ XtimeE[state[5]] ^ XtimeB[state[6]]
             ^ XtimeD[state[7]];
    tmp[14] = XtimeD[state[4]] ^ Xtime9[state[5]] ^ XtimeE[state[6]]
              ^ XtimeB[state[7]];
    tmp[3] = XtimeB[state[4]] ^ XtimeD[state[5]] ^ Xtime9[state[6]]
             ^ XtimeE[state[7]];

    /* restore column 2 */
    tmp[8] = XtimeE[state[8]] ^ XtimeB[state[9]] ^ XtimeD[state[10]]
             ^ Xtime9[state[11]];
    tmp[13] = Xtime9[state[8]] ^ XtimeE[state[9]] ^ XtimeB[state[10]]
              ^ XtimeD[state[11]];
    tmp[2] = XtimeD[state[8]] ^ Xtime9[state[9]] ^ XtimeE[state[10]]
             ^ XtimeB[state[11]];
    tmp[7] = XtimeB[state[8]] ^ XtimeD[state[9]] ^ Xtime9[state[10]]
             ^ XtimeE[state[11]];

    /* restore column 3 */
    tmp[12] = XtimeE[state[12]] ^ XtimeB[state[13]] ^ XtimeD[state[14]]
              ^ Xtime9[state[15]];
    tmp[1] = Xtime9[state[12]] ^ XtimeE[state[13]] ^ XtimeB[state[14]]
             ^ XtimeD[state[15]];
    tmp[6] = XtimeD[state[12]] ^ Xtime9[state[13]] ^ XtimeE[state[14]]
             ^ XtimeB[state[15]];
    tmp[11] = XtimeB[state[12]] ^ XtimeD[state[13]] ^ Xtime9[state[14]]
              ^ XtimeE[state[15]];

    for (i = 0; i < AES_STATE; i++)
        state[i] = InvSbox[tmp[i]];
}

/* encrypt/decrypt columns of the key */
static void
AddRoundKey(Vcchar_t *state, const Vcchar_t *key)
{
    int i;

    for (i = 0; i < AES_STATE; i++)
        state[i] ^= key[i];
}

static Vcchar_t Rcon[255]
= { /* exponentiation in Rijndael's finite field */
    
  };

/* produce AES_STATE bytes for each round */
static void
ExpandKey(Aes_t *aes, Vcchar_t *ckey, Vcchar_t *ekey)
{
    int i;
    Vcchar_t tmp0, tmp1, tmp2, tmp3, tmp4;

    /* the initial part of the expanded key is the key */
    for (i = 0; i < aes->K; ++i)
        ekey[i] = ckey[i];

    for (i = aes->Nk; i < AES_NB * (aes->Nr + 1); i++) {
        tmp0 = ekey[4 * i - 4];
        tmp1 = ekey[4 * i - 3];
        tmp2 = ekey[4 * i - 2];
        tmp3 = ekey[4 * i - 1];

        if ((i % aes->Nk) == 0) {
            tmp4 = tmp3;
            tmp3 = Sbox[tmp0];
            tmp0 = Sbox[tmp1] ^ Rcon[i / aes->Nk];
            tmp1 = Sbox[tmp2];
            tmp2 = Sbox[tmp4];
        } else if (aes->Nk > 6 && (i % aes->Nk) == 4) {
            tmp0 = Sbox[tmp0];
            tmp1 = Sbox[tmp1];
            tmp2 = Sbox[tmp2];
            tmp3 = Sbox[tmp3];
        }

        ekey[4 * i + 0] = ekey[4 * i - 4 * aes->Nk + 0] ^ tmp0;
        ekey[4 * i + 1] = ekey[4 * i - 4 * aes->Nk + 1] ^ tmp1;
        ekey[4 * i + 2] = ekey[4 * i - 4 * aes->Nk + 2] ^ tmp2;
        ekey[4 * i + 3] = ekey[4 * i - 4 * aes->Nk + 3] ^ tmp3;
    }
}

/* encrypt one 128-bit block */
static int
AesEncrypt(Aes_t *aes, Vcchar_t *ekey, Vcchar_t *in, Vcchar_t *out)
{
    int r;

    if (!aes || !in || !ekey || !out)
        return -1;

    memcpy(out, in, AES_STATE); /**/
    DEBUG_ASSERT(AES_STATE == 16);

    AddRoundKey(out, ekey);
    for (r = 1; r < aes->Nr; ++r) {
        MixSubColumns(out);
        AddRoundKey(out, ekey + r * AES_STATE);
    }
    ShiftRows(out);
    AddRoundKey(out, ekey + aes->Nr * AES_STATE);

    return 0;
}

/* decrypt one 128-bit block */
static int
AesDecrypt(Aes_t *aes, const Vcchar_t *ekey, const Vcchar_t *in, Vcchar_t *out)
{
    int r;

    if (!aes || !in || !ekey || !out)
        return -1;

    memcpy(out, in, AES_STATE); /**/
    DEBUG_ASSERT(AES_STATE == 16);

    AddRoundKey(out, ekey + aes->Nr * AES_STATE);
    InvShiftRows(out);
    for (r = aes->Nr - 1; r > 0; --r) {
        AddRoundKey(out, ekey + r * AES_STATE);
        InvMixSubColumns(out);
    }
    AddRoundKey(out, ekey);

    return 0;
}

static ssize_t
aes_init(Vcx_t *xx, Vcxmethod_t *meth, Vcchar_t *key, ssize_t keyz)
{
    Aes_t *aes;

    if (!xx)
        return -1;

    if (meth != Vcxaes128 && meth != Vcxaes192 && meth != Vcxaes256
        && meth != Vcxaessum)
        return -1;

    if (!(aes = ( Aes_t * )calloc(1, sizeof(Aes_t))))
        return -1;
    aes->tdsz = aes->pdsz = 0;

    /* set key size and number of encrypt/decrypt rounds */
    if (meth == Vcxaes128) {
        aes->K = AES_KEY128;
        aes->Nk = AES_KEY128 / sizeof(Vcuint32_t);
        aes->Nr = 10;
    } else if (meth == Vcxaes192) {
        aes->K = AES_KEY192;
        aes->Nk = AES_KEY192 / sizeof(Vcuint32_t);
        aes->Nr = 12;
    } else /* if(meth == Vcxaes256 || meth == Vcxaessum) */
    {
        aes->K = AES_KEY256;
        aes->Nk = AES_KEY256 / sizeof(Vcuint32_t);
        aes->Nr = 14;
    }

    if (key) /* construct an internal key */
        xx->keyz = _vcxmakekey(key, keyz, xx->key, aes->K);
    else if (meth == Vcxaessum) /* all-zero key */
    {
        xx->keyz = aes->K;
        memset(xx->key, 0, xx->keyz);
    } else
        xx->keyz = -1; /* an error */

    if (xx->keyz != aes->K) /* AES key must have a specified length */
    {
        free(aes);
        return -1;
    }

    ExpandKey(aes, xx->key, aes->ekey); /* expanded encryption key */
    xx->meth = meth;
    xx->data = ( Void_t * )aes;

    /* initialize the salt using the key itself */
    AesEncrypt(aes, aes->ekey, xx->key, aes->salt);

    return xx->meth == Vcxaessum ? sizeof(aes->salt) : 0;
}

static int
aes_stop(Vcx_t *xx)
{
    Aes_t *aes;

    if (!xx || !(aes = ( Aes_t * )xx->data))
        return -1;
    if (xx->meth != Vcxaes128 && xx->meth != Vcxaes192
        && xx->meth != Vcxaes256 && xx->meth != Vcxaessum)
        return -1;

    if (aes->obuf)
        free(aes->obuf);
    free(aes);

    return 0;
}

static ssize_t
aes_encode(Vcx_t *xx, const Void_t *buf, ssize_t size, Vcchar_t **out)
{
    const Vcchar_t *data = buf;
    ssize_t k, obsz;
    Vcchar_t blk[AES_BLOCK], *salt, *obdt;
    Aes_t *aes;

    if (!xx || !(aes = ( Aes_t * )xx->data))
        return -1;
    if (xx->meth != Vcxaes128 && xx->meth != Vcxaes192
        && xx->meth != Vcxaes256 && xx->meth != Vcxaessum)
        return -1;

    if (data && size > 0) {
        if (aes->tdsz > 0) /* deal with buffered data as appropriate */
        {
            if ((k = AES_BLOCK - aes->tdsz) > size)
                k = size;
            memcpy(aes->todo + aes->tdsz, data, k);
            data += k;
            size -= k;
            if ((aes->tdsz += k) == AES_BLOCK) /* full buffer, process it */
            {
                aes->tdsz = 0;
                if (aes_encode(xx, aes->todo, AES_BLOCK, NIL(Vcchar_t **))
                    < 0)
                    return -1;
            }
        } /**/
        DEBUG_ASSERT(aes->tdsz == 0 || size == 0);

        if (xx->meth != Vcxaessum) /* need memory for encrypted data */
        {
            obsz
            = ((aes->pdsz + size + AES_BLOCK - 1) / AES_BLOCK) * AES_BLOCK;
            if (obsz > aes->obsz) {
                if (!(aes->obuf = ( Vcchar_t * )realloc(aes->obuf, obsz))) {
                    aes->obsz = 0;
                    return -1;
                } else
                    aes->obsz = obsz;
            }
        }

        obdt = salt = aes->salt;   /* for digesting, buffer == salt */
        if (xx->meth != Vcxaessum) /* encrypted buffer != salt */
            obdt = aes->obuf + aes->pdsz;

        for (; size >= AES_BLOCK; size -= AES_BLOCK, data += AES_BLOCK) {
            for (k = 0; k < AES_BLOCK; ++k) /* salt data then encrypt */
                blk[k] = data[k] ^ salt[k];
            AesEncrypt(aes, aes->ekey, blk, obdt);
            aes->pdsz += AES_BLOCK; /* count amount of processed data */

            salt = obdt; /* chain next block with current encrypted data */
            if (xx->meth != Vcxaessum)
                obdt += AES_BLOCK; /* next area for encrypted data */
        }

        if (salt != aes->salt) /* save salt */
            memcpy(aes->salt, salt, AES_BLOCK);

        if (size > 0) /* buffer unprocessed data */
        {             /**/
            DEBUG_ASSERT(aes->tdsz == 0 && size < AES_BLOCK);
            memcpy(aes->todo, data, size);
            aes->tdsz = size;
        }
    }

    if (!out || (aes->pdsz == 0 && aes->tdsz == 0 /* nothing done yet */))
        return 0;

    if (xx->meth == Vcxaessum) /* digestion only */
    {
        obsz = aes->pdsz + aes->tdsz; /* total message length */

        /* make room to encode the message length */
        if (AES_BLOCK - aes->tdsz < 4) {
            for (k = aes->tdsz; k < AES_BLOCK; ++k)
                aes->todo[k] = 0;
            aes->tdsz = 0;
            if (aes_encode(xx, aes->todo, AES_BLOCK, NIL(Vcchar_t **)) < 0)
                return -1;
        }

        /* now encode the message length into data digest */
        for (k = aes->tdsz; k < AES_BLOCK - 4; ++k)
            aes->todo[k] = 0;
        /**/ DEBUG_ASSERT(k == AES_BLOCK - 4);
        aes->todo[k + 0] = (obsz >> 24) & 0xff;
        aes->todo[k + 1] = (obsz >> 16) & 0xff;
        aes->todo[k + 2] = (obsz >> 8) & 0xff;
        aes->todo[k + 3] = (obsz >> 0) & 0xff;
        aes->tdsz = 0;
        if (aes_encode(xx, aes->todo, AES_BLOCK, NIL(Vcchar_t **)) < 0)
            return -1;

        *out = aes->salt;
        obsz = AES_BLOCK;
    } else /* encryption */
    {      /* encode the size of unprocessed data (even if zero) */
        for (k = aes->tdsz; k < AES_BLOCK - 1; ++k)
            aes->todo[k] = 0;
        aes->todo[k] = aes->tdsz; /* keep size of real data */

        aes->tdsz = 0; /* now encrypt this last block */
        if (aes_encode(xx, aes->todo, AES_BLOCK, NIL(Vcchar_t **)) < 0)
            return -1;

        *out = aes->obuf;
        obsz = aes->pdsz; /**/
        DEBUG_ASSERT(obsz <= aes->obsz);
    }

    aes->pdsz = aes->tdsz = 0; /* this data stream is done. */
    AesEncrypt(aes, aes->ekey, xx->key, aes->salt); /* reset the salt */

    return obsz;
}

/* note that decryption must be done for an entire stream */
static ssize_t
aes_decode(Vcx_t *xx, const Void_t *buf, ssize_t size, Vcchar_t **out)
{
    const Vcchar_t *data = buf;
    ssize_t k;
    const Vcchar_t *salt;
    Vcchar_t *obdt;
    Aes_t *aes;

    if (!xx || !(aes = ( Aes_t * )xx->data))
        return -1;
    if (xx->meth != Vcxaes128 && xx->meth != Vcxaes192
        && xx->meth != Vcxaes256)
        return -1;

    if ((size % AES_BLOCK) != 0)
        return -1;

    if (!data || size == 0 || !out)
        return 0;

    if (aes->obuf)
        free(aes->obuf);
    if (!(aes->obuf = ( Vcchar_t * )malloc(size))) {
        aes->obsz = 0;
        return -1;
    }
    aes->obsz = size;

    salt = aes->salt; /* start with internal salt */
    obdt = aes->obuf;
    for (; size > 0; size -= AES_BLOCK, data += AES_BLOCK) {
        AesDecrypt(aes, aes->ekey, data, obdt);
        for (k = 0; k < AES_BLOCK; ++k) /* undo salting */
            obdt[k] ^= salt[k];
        salt = data; /* salt to decrypt next block */
        obdt += AES_BLOCK;
    }

    /* the last byte keeps the data size % AES_BLOCK */
    k = ( ssize_t )aes->obuf[aes->obsz - 1]; /**/
    DEBUG_ASSERT(k < AES_BLOCK);

    *out = aes->obuf;
    return aes->obsz - (AES_BLOCK - k);
}

static Vcxmethod_t _Vcxaes128
= { aes_init, aes_stop, aes_encode, aes_decode };
static Vcxmethod_t _Vcxaes192
= { aes_init, aes_stop, aes_encode, aes_decode };
static Vcxmethod_t _Vcxaes256
= { aes_init, aes_stop, aes_encode, aes_decode };
static Vcxmethod_t _Vcxaessum = { aes_init, aes_stop, aes_encode, 0 };

Vcxmethod_t *Vcxaes128 = &_Vcxaes128;
Vcxmethod_t *Vcxaes192 = &_Vcxaes192;
Vcxmethod_t *Vcxaes256 = &_Vcxaes256;
Vcxmethod_t *Vcxaessum = &_Vcxaessum;
