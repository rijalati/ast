...
static ssize_t
vcencrypt(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
{
    Vcchar_t *dt;
    ssize_t sz, k;
    Crypt_t *cr;
...
static ssize_t
vcdecrypt(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
{
    Vcchar_t *dt;
    ssize_t sz;
    Vcchar_t type;
...
static int
cryptheader(Vcodex_t *vc, Vcmtcode_t *mtcd)
{
    Vcmtarg_t *arg;
    Vcchar_t *dt, buf[128];
    ssize_t sz;
...
static int
cryptevent(Vcodex_t *vc, int type, Void_t *param)
{
    char *data, val[1024];
    Vcmtarg_t *arg;
...
Vcmethod_t _Vccrypt = { vcencrypt,
                        vcdecrypt,
                        cryptevent,
                        "crypt",
                        "Data encryption.",
                        "[-?\n@(#)$Id: vcodex-crypt (AT&T Research) "
                        "2003-01-01 $\n]" USAGE_LICENSE,
                        _Cryptargs,
                        1024 * 1024,
                        0 };
