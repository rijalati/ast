...
sum(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
{
    ...
    {
        if (!(dt
              = vcbuffer(vc, NIL(Vcchar_t *), size, sum->head + sum->dgsize)))
            return -1;
        memcpy(dt, data, size);
    }

    dt -= sum->head + sum->dgsize;
    sz += sum->head + sum->dgsize;
    dt[0] = type;
    if (sum->head > 1)
...
static ssize_t
unsum(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
{
    Vcchar_t *dt, *dig, *sig;
    ssize_t sz;
    Vcchar_t type;
...
static int
sumheader(Vcodex_t *vc, Vcmtcode_t *mtcd)
{
    Sum_t *sum;
    Vcmtarg_t *arg;
    Vcchar_t *dt, buf[64];
...
static int
sumevent(Vcodex_t *vc, int type, Void_t *param)
{
    char *data, val[1024];
    Vcmtarg_t *arg;
    Vcxmethod_t *meth;
