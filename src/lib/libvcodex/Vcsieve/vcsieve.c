...
#define VCSIGET(i)                                                           \
    (((i) & (VCS_ISIZE - 1))) /* VCS_ISIZE must be a power-of-2 */
#define VCSKINIT(si)                                                         \
...
#if __STD_C
static ssize_t
address(Sieve_t *si, ssize_t addr, ssize_t size, ssize_t here, int *inst)
#else
static ssize_t address(si, addr, size, here, inst) Sieve_t *si;
    ...
    if (*inst < 0) /* encoding */
    {
        *inst = VCS_HERE;
        ...
        if ((k = VCSIGET(*inst)) == VCS_HERE)
            ...
            ad = vcintcode(addr, cache[k - VCS_KBEGIN], 0, here, VC_DECODE);
        a = ad;
    }
...
#endif
{
    ...
    while (n > 0) {
        if ((si->type & VCS_DIFF) && n > 1)
            ...
            z = mtch->size; /* coding each matched segment separately */
        if (z < si->mmin)
        ...
        ty = -1;
        sz = (type & VCLZ_REVERSE) ? -z : z;
        if ((addr = address(si, mtch->mpos, sz, mtch->tpos, &ty)) < 0)
            RETURN(-1);

        ty |= ((type & VCLZ_MAP) ? VCS_MAP : 0)
              | ((type & VCLZ_REVERSE) ? VCS_REVERSE : 0);
        if ((si->type & VCS_DIFF) && n > 1)
            ty |= VCS_DIFF;
        vcioputc(si->inst, ty);
        ...
        {                  /* corresponding source and target data */
            if (mtch->mpos >= si->vcpa.nsrc)
                ...
                ms = si->vcpa.src + mtch->mpos;
            ts = si->vcpa.tar + (mtch->tpos - si->vcpa.nsrc);

            endd = (dt = vcionext(si->diff)) + z;
            for (md = (ty & VCS_REVERSE) ? -1 : 1; dt < endd;
                 ++dt, ++ts, ms += md)
...
#if __STD_C
static ssize_t
sieve(Vcodex_t *vc, const Void_t *tar, size_t ntar, Void_t **del)
#else
static ssize_t sieve(vc, tar, ntar, del) Vcodex_t *vc;
Void_t *tar;
size_t ntar;
Void_t **del;
#endif
{
    ...
    Vcchar_t ctrl, *dt, *output, *instdt, *addrdt, *dfszdt, *unszdt, *diffdt,
    *untrdt;
...
#define HEAD 128 /* a sufficiently large header */
    ...
    vcioinit(&untr, output + 0 * ntar, ntar);
    si->untr = &untr; /* untransformed data */
    vcioinit(&diff, output + 1 * ntar, ntar);
    si->diff = &diff; /* diff-ed data */
    vcioinit(&inst, output + 2 * ntar, ntar);
    si->inst = &inst; /* control bytes */
    vcioinit(&unsz, output + 4 * ntar, ntar);
    si->unsz = &unsz; /* size of unmatcheds */
    vcioinit(&dfsz, output + 3 * ntar, ntar);
    si->dfsz = &dfsz; /* size of diffs */
    vcioinit(&addr, output + 5 * ntar, ntar);
    si->addr = &addr; /* addresses of segments */
    ...
    nuntr = vciosize(&untr);
    untrdt = vciodata(&untr); /**/
    DEBUG_PRINT(2, "Raw: untr=%d, ", nuntr);
    ndiff = vciosize(&diff);
    diffdt = vciodata(&diff); /**/
    DEBUG_PRINT(2, "diff=%d, ", ndiff);
    ninst = vciosize(&inst);
    instdt = vciodata(&inst); /**/
    DEBUG_PRINT(2, "inst=%d, ", ninst);
    nunsz = vciosize(&unsz);
    unszdt = vciodata(&unsz); /**/
    DEBUG_PRINT(2, "unsz=%d, ", nunsz);
    ndfsz = vciosize(&dfsz);
    dfszdt = vciodata(&dfsz); /**/
    DEBUG_PRINT(2, "dfsz=%d, ", ndfsz);
    naddr = vciosize(&addr);
    addrdt = vciodata(&addr); /**/
...
#if __STD_C
static ssize_t
unsieve(Vcodex_t *vc, const Void_t *del, size_t ndel, Void_t **out)
#else
static ssize_t unsieve(vc, del, ndel, out) Vcodex_t *vc;
Void_t *del;
size_t ndel;
Void_t **out;
#endif
    ...
    for (k = 0, here = si->vcpa.nsrc, endts = (ts = tar) + ntar;
        ...
        if (ctrl == VCS_UNTR) {
            ...
            if ((df = vciogetu(&addr)) < 0)
                RETURN(-1);
            if ((ad = address(
                 si, df, (ctrl & VCS_REVERSE) ? -dz : dz, here, &ctrl))
                < 0)
                RETURN(-1);
...
#if __STD_C
static ssize_t
sieveextract(Vcodex_t *vc, Vcchar_t **datap)
#else
static ssize_t sieveextract(vc, datap) Vcodex_t *vc;
...
#if __STD_C
static int
sieverestore(Vcmtcode_t *mtcd)
#else
static int sieverestore(mtcd) Vcmtcode_t *mtcd;
#endif
{
    Vcio_t io;
    ssize_t sz, k, n;
...
#if __STD_C
static int
sievent(Vcodex_t *vc, int type, Void_t *init)
#else
static int sievent(vc, type, init) Vcodex_t *vc;
int type;
Void_t *init;
#endif
{
...
Vcmethod_t _Vcsieve = { sieve,
                        unsieve,
                        sievent,
                        "sieve",
                        "(Delta) Compression via sieving.",
                        "[-?\n@(#)$Id: vcodex-sieve (AT&T Research) "
                        "2003-01-01 $\n]" USAGE_LICENSE,
                        _Siargs,
                        1024 * 1024,
                        VC_MTSOURCE };
