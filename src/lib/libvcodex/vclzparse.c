...
#define NEXTPAIR(pr)                                                         \
    ((pr)->size < MAXPAIR - 1 ? ((pr)->mtch + (pr)->size++)                  \
                              : NIL(Vclzmatch_t *))
...
#define DELETE(e, p, ob)                                                     \
    ((p) ? ((p)->next = (ob)->next) : (*(e) = (ob)->next))

#if __STD_C
static Hash_t *
hashtable(ssize_t mmin, ssize_t nsrc, ssize_t ntar)
#else
static Hash_t *hashtable(mmin, nsrc, ntar) ssize_t mmin; /* segment size to be
                                                            hashed	*/
...
#if __STD_C
static Vchash_t
fkhash(Hash_t *hs, Vcchar_t *ks, Vcchar_t *cm)
#else
static Vchash_t fkhash(hs, ks, cm) Hash_t *hs;
...
#define fknextm(cf, ks, ns, ky, cm)                                          \
    (((ky) - (cm)[(ks)[0]] * (cf)) * ALPHA + (cm)[(ns)[0]])

#if __STD_C
static Vchash_t
rkhash(Hash_t *hs, Vcchar_t *ks, Vcchar_t *cm)
#else
static Vchash_t rkhash(hs, ks, cm) Hash_t *hs;
Vcchar_t *ks;
Vcchar_t *cm;
#endif
{
...
#if __STD_C
static ssize_t
extend(Vclzparse_t *vcpa, Pair_t *pr, int type)
#else
static ssize_t extend(vcpa, pr, type) Vclzparse_t *vcpa;
Pair_t *pr;
int type;
#endif
{
    ...
    Vcchar_t *cmap
    = ((vcpa->type & type & VCLZ_MAP) && vcpa->cmap) ? vcpa->cmap : 0;
    ...
    mt = pr->mtch + pr->size - 1;
    return (mt->tpos + mt->size) - pr->mtch->tpos;
}

#if __STD_C
static int
hashparse(Vclzparse_t *vcpa, ssize_t prune)
#else
static int hashparse(vcpa, prune) Vclzparse_t *vcpa;
...
#endif
{
    ...
    Vcchar_t *cmap
    = ((vcpa->type & VCLZ_MAP) && vcpa->cmap) ? vcpa->cmap : NIL(Vcchar_t *);
...
#if __STD_C
static int
sfxparse(Vclzparse_t *vcpa)
#else
static int sfxparse(vcpa) Vclzparse_t *vcpa;
#endif
{
    Vcinx_t p, r, ad;
...
#define SEARCH 4 /* neighborhood to search for an improved match */
    ...
    if (str && str != vcpa->tar && str != vcpa->src)
        free(str);
    if (sfx)
        free(sfx);
    return rv;
...
#if __STD_C
int
vclzparse(Vclzparse_t *vcpa, ssize_t bound)
#else
int vclzparse(vcpa, bound) Vclzparse_t *vcpa;
