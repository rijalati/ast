...
static ssize_t
rcrdcopy(Nflrcrd_t *rc, Vcchar_t *dt, ssize_t sz)
#else
...
#if __STD_C /* check for a whole v9 packet. return size of padding and       \
               packet end */
static ssize_t
...
static void
v9padding(Nflrcrd_t *rc, Nflrcrd_t *rcrd)
#else
static void v9padding(rc, rcrd) Nflrcrd_t *rc; /* record set to be checked for
                                                  padding	*/
...
#endif
{
    ...
    rc->dtsz = cdt - rc->data; /**/
    DEBUG_ASSERT(rc->dtsz % rc->sz == 0);
}

#if __STD_C /* transforming version 9 netflow data */
static ssize_t
v9flow(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t v9flow(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
{
    ...
    for (rc = dtfirst(nfl->rcdt); rc; rc = dtnext(nfl->rcdt, rc)) {
        ...
        } else /* processing by parts */
        {
            ...
            else /* use the table transform since # of columns is known */
            {
                ...
                vcsetmtarg(
                nfl->tbl, "columns", TYPECAST(Void_t *, rc->sz), 2);
        ...
        sz
        += vcsizeu(rc->id) + vcsizeu(rc->sz) + vcsizeu(rc->cdsz) + rc->cdsz;
    }
    ...
    } else /* use the table transform on this 2-D array */
    {
        ...
        if ((rcrd.cdsz = vcapply(nfl->tbl, rcrd.data, rcrd.dtsz, &rcrd.code))
            < 0)
            goto done;
    }
...
static ssize_t
unv9flow(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t unv9flow(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
...
static ssize_t
v1567flow(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t v1567flow(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
...
static ssize_t
unv1567flow(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t unv1567flow(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
...
#if __STD_C
static ssize_t
netflow(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t netflow(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
...
#if __STD_C
static ssize_t
unnetflow(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t unnetflow(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
{
    ...
    else if (id == V1_PACKET || id == V5_PACKET || id == V6_PACKET
             || id == V7_HEADER)
...
#if __STD_C
static int
rcrdcmp(Dt_t *dt, Void_t *one, Void_t *two, Dtdisc_t *disc)
#else
static int rcrdcmp(dt, one, two, disc) Dt_t *dt;
Void_t *one;
Void_t *two;
Dtdisc_t *disc;
#endif
...
#if __STD_C
static Void_t *
rcrdmake(Dt_t *dt, Void_t *arg, Dtdisc_t *disc)
#else
static Void_t *rcrdmake(dt, arg, disc) Dt_t *dt;
Void_t *arg;
Dtdisc_t *disc;
#endif
{
...
#if __STD_C
static void
rcrdfree(Dt_t *dt, Void_t *arg, Dtdisc_t *disc)
#else
static void rcrdfree(dt, arg, disc) Dt_t *dt;
Void_t *arg;
Dtdisc_t *disc;
#endif
{
...
static int
nflevent(Vcodex_t *vc, int type, Void_t *params)
#else
static int nflevent(vc, type, params) Vcodex_t *vc;
int type;
Void_t *params;
#endif
{
