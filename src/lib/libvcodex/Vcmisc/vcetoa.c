...
#if __STD_C
static ssize_t
etoa(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t etoa(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
...
#if __STD_C
static ssize_t
unetoa(Vcodex_t *vc, const Void_t *data, size_t size, Void_t **out)
#else
static ssize_t unetoa(vc, data, size, out) Vcodex_t *vc;
Void_t *data;
size_t size;
Void_t **out;
#endif
{
    ...
    for (idt = ( Vcchar_t * )data + size - 1; idt > ( Vcchar_t * )data;
         idt -= 1)
        if (*idt == '\n')
            break;
...
#if __STD_C
static int
etoaevent(Vcodex_t *vc, int type, Void_t *params)
#else
static int etoaevent(vc, type, params) Vcodex_t *vc;
int type;
Void_t *params;
#endif
{
    ...
    if (type == VC_OPENING)
    {
        ...
        if (!(map = vcopen(
              0, Vcmap, e2a, 0, (vc->flags & (VC_ENCODE | VC_DECODE)))))
            return -1;
...
= { etoa,
    ...
    etoaevent,
    "etoa",
    "Mapping EBCDIC blocks to/from ASCII text lines.",
