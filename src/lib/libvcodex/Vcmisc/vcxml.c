            ...
            if (c < TAG_OFF) {
                if (vc->errorf)
                    (*vc->errorf)(NiL,
                                  vc->disc,
                                  2,
                                  "0x%02x: unknown op -- compression format "
            ...
            if (c >= (p->cur - p->sub)) {
                if (vc->errorf)
                    (*vc->errorf)(NiL,
                                  vc->disc,
                                  2,
                                  "%d: tag index not found -- compression "
                                  "format mismatch or corruption",
                                  c);
                RETURN(-1);
            }
            t = p->sub[c];
...
= { vcxml,
    ...
    xmlevent,
    "xml",
    "Preprocess valid xml data to boost compression.",
