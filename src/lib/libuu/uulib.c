...
uu_decode(Uu_t *uu)
{
            ...
            while ((c = m[sfgetc(uu->ip)]) >= 64)
                if (c != UU_IGN)
                {
                    if (uu->disc->errorf)
                        ...
                        uu->disc,
                        1,
                        "%c: extra input character ignored",
                        c);
                    goto pad;
                }
            n = (n << 6) | c;
    ...
    pad:
        ...
        if (n < 4 && (uu->flags & UU_HEADER) && uu->disc->errorf)
            (*uu->disc->errorf)(
            uu, uu->disc, 1, "input end sequence `%s' omitted", dp->end);
    }
    return 0;
}
...
bx_q_putn(Uu_t *uu, Bx_t *bx, unsigned long v, int n)
{
    ...
    case 3:
        bx_q_putc(uu, bx, (v >> 16) & 0xFF);
    case 2:
        bx_q_putc(uu, bx, (v >> 8) & 0xFF);
    case 1:
        bx_q_putc(uu, bx, (v >> 0) & 0xFF);
    }
    return 0;
}
            ...
            if (bol)
            {
                ...
                if (crc != crx)
                {
                    if (uu->disc->errorf)
                        ...
                        uu->disc,
                        2,
                        "%s format header checksum mismatch",
                        uu->meth.name);
                    return -1;
                }
                return 0;
            ...
            if (*s++ == '*' && *s++ == '*' && *s++ == '*')
            {
                ...
                case 'C':
                    ...
                    case 'O':
                        ...
                        continue;
                    case 'H':
                        if (decode == bx_c_decode
                            || strncmp(s, "CHECKSUM:", 9))
                            continue;
                        crc = bx->crc & 0xFF;
        ...
        if (uu->disc->errorf)
        {
            if (crc == 0x10000)
                ...
                (*uu->disc->errorf)(uu,
                                    uu->disc,
                                    2,
                                    "%s format data checksum mismatch",
                                    uu->meth.name);
        }
        return -1;
    }
...
cat(Uu_t *uu)
{
    return sfmove(uu->ip, uu->op, SF_UNBOUND, -1) >= 0 && sfeof(uu->ip) ? 0
                                                                        : -1;
}
            ...
            for (mp = methods; mp->name; mp++)
                if (*mp->name == c
                    && (!strcasecmp(name, mp->name)
                        || vl && !strncasecmp(name, mp->name, vl))
                    || mp->alias && *mp->alias == c
...
uuop(Uu_t *uu, Uu_f fun)
{
            ...
            || !(uu->ip = sfdcsubstream(NiL, uu->lp = uu->ip, p, uu->count))))
    {
        if (uu->disc->errorf)
            ...
            uu->disc,
            2,
            "cannot initialize substream at %I*d for %I*d bytes",
            sizeof(p),
            p,
            sizeof(uu->count),
            uu->count);
    ...
    if (!uu->meth.encodef)
    {
        if (uu->disc->errorf)
            (*uu->disc->errorf)(uu,
                                uu->disc,
                                2,
                                "%s format encoding not supported",
                                uu->meth.name);
        return -1;
    }
    ...
    if (!uu->meth.decodef)
    {
        if (uu->disc->errorf)
            (*uu->disc->errorf)(uu,
                                uu->disc,
                                2,
                                "%s format decoding not supported",
                                uu->meth.name);
        return -1;
    }
    if (!(uu->ip = ip))
    ...
    else
    {
        ...
        if (data && (data->flags & UU_DEFAULT) && (c = sfgetc(uu->ip)) != EOF)
        {
            ...
            if (mp > methods)
            {
                ...
                if (uu->disc->errorf)
                    (*uu->disc->errorf)(
                    uu, uu->disc, 1, "assuming %s encoding", mp->name);
            }
        }
    }
    if (!uu->op)
    {
        if (!uu->path && headerpath)
        {
            if (uu->disc->errorf)
                ...
                uu->disc,
                2,
                "%s format header has no output file name",
                uu->meth.name);
            return -1;
        }
