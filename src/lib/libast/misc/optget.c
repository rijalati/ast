...
static const List_t help_head[] = {
    { '-', 0, 0 },
    { '+', C("NAME"), C("options available to all \bast\b commands") },
    { '+', C("DESCRIPTION"), C("\b-?\b and \b--?\b* options are the same \
...
static const Help_t styles[] = {
    { C("about"), "-", STYLE_match, Z("List all implementation info.") },
    { C("api"),
      "?api",
      STYLE_api,
      Z("List detailed info in program readable form.") },
    { C("help"), "", -1, Z("List detailed help option info.") },
    { C("html"), "?html", STYLE_html, Z("List detailed info in html.") },
    { C("keys"),
      "?keys",
      STYLE_keys,
      Z("List the usage translation key strings with C style escapes.") },
    { C("long"), "?long", STYLE_long, Z("List long option usage.") },
    { C("man"),
      "?man",
      STYLE_man,
      Z("List detailed info in displayed man page form.") },
    { C("nroff"), "?nroff", STYLE_nroff, Z("List detailed info in nroff.") },
    { C("options"),
      "?options",
      STYLE_options,
      Z("List short and long option details.") },
    { C("posix"), "?posix", STYLE_posix, Z("List posix getopt usage.") },
    { C("short"), "?short", STYLE_short, Z("List short option usage.") },
    { C("usage"),
      "?usage",
      STYLE_usage,
...
static const List_t help_tail[] = {
    ...
    { ':', C("\?\?\aname\a"), C("Equivalent to \b--help=\b\aname\a.") },
    { ':', C("\?\?"), C("Equivalent to \b--\?\?options\b.") },
    { ':', C("\?\?\?\?"), C("Equivalent to \b--\?\?man\b.") },
    { ':', C("\?\?\?\?\?\?"), C("Equivalent to \b--\?\?help\b.") },
    { ':',
      C("\?\?\?\?\?\?\aitem\a"),
...
secname(char *section)
{
    ...
    for (i = 0; i < elementsof(sections); i++)
        if (section[0] == sections[i].section[0]
            && (section[1] == sections[i].section[1]
...
skip(char *s, int t1, int t2, int t3, int n, int b, int past, int version)
{
    ...
    if (version < 1) {
        ...
        while (c = *s++) {
            message((-22,
                     "optget: skip t1=%c t2=%c t3=%c n=%d b=%d `%s'",
                     t1 ? t1 : '@',
                     t2 ? t2 : '@',
                     t3 ? t3 : '@',
                     n,
    ...
    if (*b == '?') {
        if (!*++b || streq(b, "NAME")) {
            ...
            n = 1;
               ...
               || (*opt_info.disc->infof)(&opt_info, ip, b, opt_info.disc)
                  < 0)
        n = 0;
    *p = s;
    if (s = sfstruse(ip))
...
init(char *s, Optpass_t *p)
{
...
#if _BLD_DEBUG
    error(-2,
          "optget debug localized=%lu:%lu xp=%p",
          state.localized,
          ast.locale.serial,
          state.xp);
#endif
                ...
                if (*s++ == '[') {
                    if (*s++ != '-') {
                        ...
                        if (strneq(s - 1, "+NAME?", 6) && (s += 5)
                            || strneq(s - 1, "+LIBRARY?", 9) && (s += 8)
                               && (l = 1)
                            || strneq(s - 1, "+PLUGIN?", 8) && (s += 7)
                               && (l = 1)) {
                            for (; *s == '\a' || *s == '\b' || *s == '\v'
                                   || *s == ' ';
                                 s++)
    ...
    message(
    (-2,
     "version=%d prefix=%d section=%s flags=%04x id=%s catalog=%s oopts=%p",
     p->version,
     p->prefix,
     p->section,
     p->flags,
    ...
    if (version < 1) {
        ...
        for (;;) {
            ...
            sfputc(sp, c);
               ...
               && (*(p = skip(s, 0, 0, 0, 1, level, 1, version)) == ':'
                   || *p == '#')) {
        va = 0;
        if (*++p == '?' || *p == *(p - 1)) {
            p++;
            ...
            case '?':
                if (*s == '?')
                    ...
                    goto restore;
                         ...
                         && (tsp = localize(
                             psp, s, e, 0, 1, ip, version, id, catalog))) {
                    psp = tsp;
                    s = psp->nb;
                    e = psp->ne;
                }
    ...
    if (flags & OPT_functions)
        ...
        for (;;) {
            ...
            if (style >= STYLE_man) {
                ...
                sfprintf(sp,
                         "\t%s%s%s%s[%s%s%s%s%s]",
                         font(FONT_BOLD, style, 1),
                         a,
                         font(FONT_BOLD, style, 0),
                         b,
                         b,
                         font(FONT_ITALIC, style, 1),
                         o,
                         font(FONT_ITALIC, style, 0),
                         b);
            } else if (a)
                sfprintf(sp,
                         "%*.*s%s%s%s[%s%s%s]",
     ...
     int *hflags)
{
    ...
    if (*s == '\n') {
        ...
        if (style == STYLE_html) {
            ...
            sfputr(sp, "<A name=\"", -1);
            if (s[-1] == '-' && s[0] == 'l' && s[1] == 'i' && s[2] == 'c'
                && s[3] == 'e' && s[4] == 'n' && s[5] == 's' && s[6] == 'e'
                && s[7] == '?')
                for (t = s + 8; *t && *t != ']'; t++)
                    if (t[0] == 'p'
                        && (!strncmp(t, "proprietary", 11)
                            || !strncmp(t, "private", 7))
            ...
            label(
            sp, 0, s, about, -1, level, style, -1, ip, version, id, catalog);
            sfputr(sp, "\">", -1);
            label(sp,
                  0,
                  s,
            ...
            if (!level) {
                if (!strncmp(s, C("SYNOPSIS"), strlen(C("SYNOPSIS"))))
                    ...
                    if (!strncmp(s, C("NAME"), strlen(C("NAME")))
                        || !strncmp(s, C("PLUGIN"), strlen(C("PLUGIN"))))
                        *hflags |= HELP_index;
                }
            }
        } else {
            if (!level) {
                if (style >= STYLE_nroff)
                    ...
                    sfputc(sp, '\n');
                else if (style != STYLE_options && style != STYLE_match
                         || *s == '-' || *s == '+')
                    sfputc(sp, '\t');
            }
            label(sp,
              ...
              int line)
{
    ...
    message((-21,
             "opthelp: txt#%d +++ %2d \"%s\" style=%d level=%d bump=%d",
             line,
             ++depth,
             show(p),
             style,
...
again:
    ...
    if (c == '+' || c == '-' && (bump = 3) || c != ' ' && level > 1) {
        ...
        if (c == '-'
            ...
            w = C("version");
            par
            = item(sp, w, about, level, style, ip, version, id, ID, hflags);
            for (;;) {
                while (isspace(*(s + 1)))
                    s++;
                w = s;
            ...
            par = item(
            sp, t, about, level, style, ip, version, id, catalog, hflags);
            c = *s;
        }
        if (!about && level)
            par = 0;
    ...
    if ((c == ']' || c == '?' && *(s + 1) == ']' && *(s + 2) != ']' && s++)
        ...
        if (c == ' ')
            ...
            if (X(catalog)
                && (tsp = localize(
                    psp, s, NiL, 0, 1, ip, version, id, catalog))) {
                psp = tsp;
                s = psp->nb;
            }
            if (style < STYLE_nroff)
        ...
        if (conform) {
            sfprintf(
            sp, "[%-.*s %s] ", conformlen, conform, T(NiL, ID, "conformance"));
            conform = 0;
        }
                ...
                if (*s != ']') {
                    ...
                    for (;;) {
                        if ((*s == '#' || *s == ':') && level > lev) {
                            ...
                            if (*(s = next(s, version)) == '[') {
                                s
                                = skip(s + 1, ':', '?', 0, 1, 0, 0, version);
                                while (*s == ':') {
                                    s = skip(
                                    t = s + 1, ':', '?', 0, 1, 0, 0, version);
                                    m = s - t;
                                    if (*t == '!') {
                                        o = t + 1;
                                        ol = m - 1;
                        ...
                        if (*(s = next(s, version)) == GO) {
                            s = textout(sp,
                                        ...
                                        catalog,
                                        hflags);
                            if (*s && *(s = next(s, version)) == '['
                                && !isalnum(*(s + 1))) {
                                s++;
                                message(
                                (-21, "textout#%d s=%s", __LINE__, show(s)));
                                goto again;
                            }
                        } else if (*s == '[' && level > lev) {
                            s++;
...
again:
    ...
    default:
        if (!state.emphasis) {
            ...
            if ((x = getenv("TERM")) && strmatch(x, "(ansi|vt100|xterm)*")
                && isatty(sffileno(sfstderr)))
                state.emphasis = 1;
        }
        break;
    }
                ...
                if (style <= STYLE_short && !y && !mutex
                    ...
                    if (style >= STYLE_match) {
                        ...
                        if (!head) {
                            head = 1;
                            item(sp_body,
                                 (flags & OPT_functions) ? C("FUNCTIONS")
                                                         : C("OPTIONS"),
                                 0,
                                 0,
                                 style,
                    ...
                    if (style >= STYLE_match) {
                        ...
                        if ((a & OPT_invert) && w && (d || u)) {
                            ...
                            if (f)
                                ...
                                sfprintf(sp_info,
                                         " %s %s\bno%-.*s\b %s.",
                                         T(NiL, ID, "On by default; use"),
                                         "--" + 2 - prefix,
                                         u - w,
                                         w,
                                         T(NiL, ID, "to turn off"));
                        ...
                        if (*p == GO) {
                            p = u ? skip(p + 1, 0, 0, 0, 0, 1, 1, version)
                                  : textout(sp_body,
                                            p,
                                            0,
                                            0,
                        ...
                        if (a & OPT_optional) {
                            if (ov) {
                                ...
                                sfprintf(
                                sp_info, "\b %s.", T(NiL, ID, "is assumed"));
                            } else
                                sfprintf(
                                sp_info,
                                "%s%s",
                                y,
                                T(
                                NiL, ID, "The option value may be omitted."));
                            if (!(t = sfstruse(sp_info)))
                                goto nospace;
                            textout(sp_body,
                                    t,
...
",
    ...
    if (style >= STYLE_man) {
        ...
        if (x)
            item(
            sp, C("SYNOPSIS"), 0, 0, style, sp_info, version, id, ID, &hflags);
    }
    if (x) {
        for (t = x + xl; t > x && (*(t - 1) == '\n' || *(t - 1) == '\r'); t--)
            ;
    ...
    if (sp_body) {
        if (sfstrtell(sp_body)) {
            ...
            if (!(t = sfstruse(sp_body)))
                goto nospace;
            if (style == STYLE_html && !(dflags & HELP_head)
                && (bflags & HELP_head))
                sfputr(sp, "\n</DIV>", '\n');
            sfputr(sp, t, -1);
        }
    ...
    m = strlen(
        (style <= STYLE_long && error_info.id && !strchr(error_info.id, '/'))
        ? error_info.id
        : id)
        + 1;
    ...
    if (!(state.flags & OPT_preformat)) {
        ...
        if (style == STYLE_html) {
            ...
            *t = 0;
            ...
            "HTML//EN\">\n<HTML>\n<HEAD>\n<META name=\"generator\" "
            "content=\"optget (AT&T Research) 2011-11-11\">\n%s<TITLE>%s man "
            "document</TITLE>\n<STYLE type=\"text/css\">\ndiv.SH { "
            "padding-left:2em; text-indent:0em; }\ndiv.SY { "
            "padding-left:4em; text-indent:-2em; }\ndt { float:left; "
            "clear:both; }\ndd { margin-left:3em; "
            "}\n</STYLE>\n</HEAD>\n<BODY bgcolor=white>\n",
            ...
            "align=left>%s&nbsp;(&nbsp;%s&nbsp;)&nbsp;<TH align=center><A "
            "href=\".\" title=\"Index\">%s</A><TH "
            "align=right>%s&nbsp;(&nbsp;%s&nbsp;)</TR></TABLE></H4>\n<HR>\n",
            ud,
            section,
            T(NiL, ID, secname(section)),
            ud,
            section);
            co = 2;
            pt = ptstk;
        ...
        for (d = sfstrbase(mp), t = sfstrseek(mp, 0, SEEK_CUR);
             t > d
             && ((c = *(t - 1)) == '\n' || c == '\r' || c == ' ' || c == '\t');
             t--)
            ;
...
again:
    ...
    for (;;) {
        if (!opt_info.offset) {
            ...
            if (opt_info.index == 1 && opt_info.argv != state.strv) {
                ...
                if (argv[0]
                    && (state.argv[0]
                        = save(argv[0], strlen(argv[0]), 0, 0, 0, 0)))
                ...
                for (c = *s; *s; s++) {
                    if (*s == '=') {
                        if (*(s + 1) == '=')
                            s++;
                        if (!isalnum(*(s - 1))
                            && *(w - 1) == (opt_info.assignment = *(s - 1)))
                            w--;
                        v = ++s;
                        break;
                    }
                    if (w < &opt_info.name[elementsof(opt_info.name) - 1]
                        && *s != ':' && *s != '|' && *s != '[' && *s != ']')
                        *w++ = *s;
                }
                *w = 0;
    ...
    if (c == ':' || c == '#' || c == ' ' || c == '[' || c == ']') {
        ...
        for (;;) {
            ...
            message(
            (-20, "optget: opt %s  c %c  w %s  num %ld", show(s), c, w, num));
            if (*s == c && !w)
                break;
            else if (*s == '[') {
                s = next(s + 1, version);
    ...
    if ((n = (*++s == '#')) || *s == ':'
        ...
        opt_info.offset = 0;
    optarg:
        if (*s == ':' && *(s = skip(s, 0, 0, 0, 1, 0, 1, version)) == GO
            && *(s = next(s + 1, version)) == '[' && isalnum(*(s + 1))) {
            x = 0;
            if (opt_info.arg) {
                do {
                    w = y = opt_info.arg;
...
help:
    if (v && *v == '?' && *(v + 1) == '?' && *(v + 2)) {
        ...
        if (hp = ( Help_t * ) search(
            ...
            pop(psp);
            return '?';
        } else if (match(s, "ESC", -1, ID, NiL)
                   || match(s, "EMPHASIS", -1, ID, NiL))
            state.emphasis = n;
        else if (match(s, "MAN", -1, ID, NiL)) {
            opt_info.arg
            = sfprints("\f%s", secname(*w != '?' ? w : pass->section));
            pop(psp);
            return '?';
...
/*
 * parse long options with 0,1,2 leading '-' or '+' from string and pass to
 *optget() syntax is the unquoted
 *
 *	<length> [-|+|--|++]<name>[[-+:|&=]=<value>\n (or \0 for the last)
 *
 * or the quoted
    ...
    if (s) {
        ...
        if (isdigit(*s) && (v = ( int ) strtol(s, &e, 10)) > 1 && isspace(*e)
            ...
            if ((c = *s) == ':' && *(s + 1) != '=') {
                ...
                s += 2;
                e = opt_info.name;
                while (e < &opt_info.name[sizeof(opt_info.name) - 1]
                       && (*e++ = *s++))
                    ;
                opt_info.arg = 0;
                opt_info.num = ( long ) (opt_info.number = 0);
