...
static Info_t info[] = {
    { "atime", INFO_atime }, { "blocks", INFO_blocks },
    { "ctime", INFO_ctime }, { "fstype", INFO_fstype },
    { "gid", INFO_gid },     { "mode", INFO_mode },
    { "mtime", INFO_mtime }, { "name", INFO_name },
    { "nlink", INFO_nlink }, { "size", INFO_size },
    { "uid", INFO_uid },
};
        ...
        unsigned long off)
{
    ...
    fun:
        ...
        case 'E':
            ...
            q = T(ep->desc);
            t = *q ? q : p;
            if (mp->keep[level]++ && b > cur && b < end && *(b - 1) != ' '
                && *t && *t != ',' && *t != '.' && *t != '\b')
                *b++ = ' ';
        ...
        checkstr:
            for (;;) {
                if (*ep->value.str == '*' && !*(ep->value.str + 1)
                    && isprint(*p))
                    break;
                if ((ep->type == 'm' || ep->type == 'M')
                    ? strmatch(p, ep->value.str)
                    : !memcmp(p, ep->value.str, ep->mask))
                    break;
...
#if _UWIN
        {
            ...
            *ep->mime = 0;
            ...
            if (!*ep->mime) {
                ...
                e = "application/x-ms-";
                ep->mime
                = vmnewof(mp->vm, ep->mime, char, strlen(t), strlen(e));
                e = stpcpy(ep->mime, e);
                while ((c = *t++) && c != '.' && c != ' ')
    ...
    swapped:
        q = T(ep->desc);
        if (mp->keep[level]++ && b > cur && b < end && *(b - 1) != ' ' && *q
            && *q != ',' && *q != '.' && *q != '\b')
            *b++ = ' ';
        if (*q == '\b')
            q++;
    ...
    format:
        if (!str)
            b += sfsprintf(
            b, end - b, q, num, num == 1 ? "" : "s", 0, 0, 0, 0, 0, 0);
        else if (ep->type == 'd' || ep->type == 'D')
            b += sfsprintf(b,
                           end - b,
                           q,
                           fmttime("%?%QL", ( time_t ) num),
                           0,
                           0,
                           0,
                           0,
            ...
            b
            += sfsprintf(b, end - b, q, fmtversion(num), 0, 0, 0, 0, 0, 0, 0);
        else
            b += sfsprintf(b,
                           end - b,
                           q,
...
ckenglish(Magic_t *mp, int pun, int badpun)
{
    ...
    if (2 * mp->count[';'] > mp->count['E'] + mp->count['e'])
        return 0;
    if ((mp->count['>'] + mp->count['<'] + mp->count['/'])
        > mp->count['E'] + mp->count['e'])
        return 0;
    for (s = "aeiou"; *s; s++)
...
cklang(Magic_t *mp, const char *file, char *buf, char *end, struct stat *st)
{
        ...
        if (b[0] == '#' && b[1] == '!') {
            ...
            *b = 0;
            if ((st->st_mode & (S_IXUSR | S_IXGRP | S_IXOTH))
                ...
                sfsprintf(
                mp->sbuf, sizeof(mp->sbuf), T("%s%s script"), s, t1);
                s = mp->sbuf;
                goto qualify;
            }
            *b = c;
    ...
    mp->mime = "text/plain";
    if (flags & CC_utf_8)
        s = (flags & CC_control) ? T("utf-8 text with control characters")
                                 : T("utf-8 text");
    else if (flags & CC_latin)
        s = (flags & CC_control) ? T("latin text with control characters")
                                 : T("latin text");
    else
        s = (flags & CC_control) ? T("text with control characters")
                                 : T("text");
qualify:
    if (!flags && mp->count['\n'] >= mp->count['\r']
...
load(Magic_t *mp, char *file, Sfio_t *fp)
{
    ...
    while (p = sfgetr(fp, '\n', 1)) {
        ...
        case '{':
            if (++lev < MAXNEST)
                ...
                mp->disc,
                1,
                "{ ... } operator nesting too deep -- %d max",
                MAXNEST);
            continue;
        case '}':
            if (!last || lev <= 0) {
            ...
            if (*(p + 1) == '{'
                ...
                if (*p == '{') {
                    ...
                    if (*(p + 1) != ')' && mp->disc->errorf)
                        ...
                        mp->disc,
                        2,
                        "%c: invalid function call argument list",
                        n + 'a');
                    ep->desc = "[CALL]";
                }
                ep->cont = cont;
                ...
                if (ret)
                    ...
                    (*mp->disc->errorf)(
                    mp, mp->disc, 2, "%c: function not defined", n + 'a');
                continue;
            }
            if (!ep->nest)
                ep->nest
                = (lev > 0 && lev != ent) ? ('0' + lev - !!ent) : ' ';
            break;
        }
        ...
        case '|':
            ...
            break;
        default:
            if ((mp->flags & MAGIC_VERBOSE) && !isalpha(*p)
                && mp->disc->errorf)
                (*mp->disc->errorf)(
                mp,
                mp->disc,
                1,
                "`%c': invalid line continuation operator",
                *p);
            /*FALLTHROUGH*/
        case '*':
        case '0':
        ...
        if (ep->op != '*' && !ep->value.num) {
            if (ep->type == 'e') {
                ...
                if (ep->op == '@')
                    ...
                    0, ( char * ) &ep->value.num, sizeof(ep->value.num));
            }
        }
    ...
    if ((mp->flags & MAGIC_VERBOSE) && mp->disc->errorf) {
        ...
        if (ret)
            (*mp->disc->errorf)(
            mp, mp->disc, 2, "%c: function has no return", ret->offset + 'a');
    }
    error_info.file = 0;
    error_info.line = 0;
    return 0;
...
magicopen(Magicdisc_t *disc)
{
    ...
    if (!(mp->tmp = sfstropen())
        || !(mp->infotab = dtnew(mp->vm, &mp->dtdisc, Dtoset)))
        goto bad;
    ...
    if (!st)
        ...
        if (!(mp->flags & (MAGIC_MIME | MAGIC_ALL))) {
            ...
            if (!mp->fp && (st->st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)))
                sfprintf(mp->tmp,
                         ", %s",
                         S_ISDIR(st->st_mode) ? T("searchable")
                                              : T("executable"));
