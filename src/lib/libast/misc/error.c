...
= { "break",   OPT_BREAK,  "catalog", OPT_CATALOG, "core",
    OPT_CORE,  "count",    OPT_COUNT, "debug",     OPT_TRACE,
    "fd",      OPT_FD,     "library", OPT_LIBRARY, "mask",
    OPT_MASK,  "match",    OPT_MATCH, "prefix",    OPT_PREFIX,
    "system",  OPT_SYSTEM, "time",    OPT_TIME,    "trace",
    OPT_TRACE, 0,          0 };
        ...
        if (cp->line > ((cp->flags & ERROR_INTERACTIVE) != 0)) {
            if (cp->file)
                sfprintf(sp,
                         ": \"%s\", %s %d",
                         cp->file,
                         ERROR_translate(NiL, NiL, ast.id, "line"),
                         cp->line);
            else
...
errorv(const char *id, int level, va_list ap)
{
    ...
    if (error_info.write) {
        ...
        case ERROR_WARNING:
            sfprintf(
            stkstd, "%s: ", ERROR_translate(NiL, NiL, ast.id, "warning"));
            break;
        case ERROR_PANIC:
            sfprintf(
