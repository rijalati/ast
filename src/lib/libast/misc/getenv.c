...
: nohide getenv
  ...
  /*
   * NOTE: the "intercepts" definition is here instead of astintercept.c
   *because some static linkers miss lone references to "intercepts" without
   *"astintercept()" ALSO: { 0 } definition required by some dynamic linkers
   *averse to common symbols UWIN: no _ast_getenv macro map to maintain ast54
   *compatibility
   */
...
getenv(const char *name)
{
    ...
    return intercepts.intercept_getenv ? (*intercepts.intercept_getenv)(name)
                                       : (*posix_getenv)(name);
#else
#    undef getenv
    return intercepts.intercept_getenv ? (*intercepts.intercept_getenv)(name)
