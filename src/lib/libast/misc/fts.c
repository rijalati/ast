...
#define drop(p, f)                                                           \
    (((f)->fts_namelen < MINNAME)                                            \
     ? ((f)->fts_link = (p)->free, (p)->free = (f))                          \
     : (free(f), (p)->free))

#define ACCESS(p, f) ((p)->cd == 0 ? (f)->fts_name : (f)->fts_path)
#define PATH(f, p, l)                                                        \
    ((!((f)->flags & FTS_SEEDOTDIR) && (l) > 0 && (p)[0] == '.'              \
      && (p)[1] == '/')                                                      \
     ? ((p) + 2)                                                             \
     : (p))
#define SAME(one, two)                                                       \
    ((one)->st_ino == (two)->st_ino && (one)->st_dev == (two)->st_dev)
...
#    define SKIP(p, f)                                                       \
        ((f)->fts_parent->must == 0                                          \
         && (((f)->type == DT_UNKNOWN)                                       \
             ? SKIPLINK(p, f)                                                \
             : ((f)->type != DT_DIR                                          \
                && ((f)->type != DT_LNK || ((p)->flags & FTS_PHYSICAL)))))
#else
#    undef DT_UNKNOWN
#    define DT_UNKNOWN 0
#    undef DT_LNK
...
popdirs(FTS *fts)
{
    ...
    return (verify && (stat(".", &sb) < 0 || !SAME(&sb, f->fts_statp))) ? -1
                                                                        : 0;
}
...
toplist(FTS *fts, char *const *pathnames)
{
    ...
    metaphysical
    = (fts->flags & (FTS_META | FTS_PHYSICAL)) == (FTS_META | FTS_PHYSICAL);
    top = bot = 0;
    while (path = *pathnames++) {
        ...
        path = f->fts_name;
        if (!physical)
            f->fts_namelen = (fts->flags & FTS_SEEDOTDIR)
                             ? strlen(path)
                             : (pathcanon(path, strlen(path) + 1, 0) - path);
        else if (*path != '.') {
...
fts_read(FTS *fts)
{
    ...
    for (;;)
        ...
        case FTS_preorder:
            ...
            if (f->fts_info == FTS_D) {
                if ((fts->diroot = search(f, fts->diroot, statcmp, 0)) != f
                    || f->fts_level > 0 && (t = f)
                       && statcmp(&t, &f->fts_parent) == 0) {
                    f->fts_info = FTS_DC;
                    f->fts_cycle = fts->diroot;
                } else if (!(fts->flags & FTS_TOP)
                           && (!(fts->flags & FTS_XDEV)
                               || f->statb.st_dev
                                  == f->fts_parent->statb.st_dev)) {
                    /*
                    ...
                    if (!fts->dir
                        && (access(fts->name, R_OK)
                            || !(fts->dir = opendir(fts->name))))
                        f->fts_info = FTS_DNR;
                }
            }
        ...
        case FTS_preorder_resume:
            ...
            fts->cpname
            = fts->cd && !fts->nostat || !fts->children && !fts->comparf;
            fts->dotdot = 0;
                ...
                if (n) {
                    ...
                    if (n == 1)
                        ...
                        if (fts->current->fts_parent->fts_level < 0) {
                            ...
                            f->fts_statp
                            = fts->current->fts_parent->fts_statp;
                    }
                    f->fts_info = FTS_DOT;
                } else if ((fts->nostat || SKIP(fts, f))
                           && (f->fts_info = FTS_NSOK)
                           || info(fts, f, s, &f->statb, fts->flags))
                    f->statb.st_ino = D_FILENO(d);
                if (fts->comparf)
                    fts->root = search(f, fts->root, fts->comparf, 1);
                else if (fts->children || f->fts_info == FTS_D
                         || f->fts_info == FTS_SL) {
                    if (fts->top)
                    ...
                    f->fts_pathlen
                    = fts->endbase - f->fts_path + f->fts_namelen;
                    f->fts_accpath = ACCESS(fts, f);
            ...
            while (fts->todo && f == fts->todo) {
                ...
                if ((f->fts_info & FTS_DP) == FTS_D) {
                    ...
                    if (!(fts->flags & FTS_NOPOSTORDER)
                        ...
                        if (fts->cd < 0)
                            fts->cd
                            = setpdir(fts->home, fts->path, fts->base);
                        fts->curdir = fts->cd ? 0 : t;
                        f->fts_info = FTS_DP;
                        f->fts_path = PATH(fts, fts->path, f->fts_level);
                        f->fts_pathlen
                        = (fts->base - f->fts_path) + f->fts_namelen;
                        f->fts_accpath = ACCESS(fts, f);
        ...
        case FTS_preorder_return:
            ...
            if (f->status == FTS_FOLLOW) {
                f->status = 0;
                if (f->fts_info == FTS_SL || ISTYPE(f, DT_LNK)
                    || f->fts_info == FTS_NSOK) {
...
fts_set(FTS *fts, FTSENT *f, int status)
{
    ...
    error(
    -1, "AHA fts_set %s status %d => %d", f->fts_name, f->status, status);
    f->status = status;
    return 0;
}
...
#else
    return !strgrpmatch(
    fmtfs(ent->fts_statp), "([an]fs|samb)", NiL, 0, STR_LEFT | STR_ICASE);
#endif
}
