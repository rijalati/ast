...
static int
stkexcept(Sfio_t *, int, void *, Sfdisc_t *);
static Sfdisc_t stkdisc = { 0, 0, 0, stkexcept };

Sfio_t _Stak_data
= SFNEW(( char * )0, 0, -1, SF_STATIC | SF_WRITE | SF_STRING, &stkdisc, 0);
...
static int
stkexcept(Sfio_t *stream, int type, void *val, Sfdisc_t *dp)
{
    NoP(dp);
    NoP(val);
    switch (type) {
...
stkopen(int flags)
{
    ...
    if (!sfnew(
        stream, cp, bsize, -1, SF_STRING | SF_WRITE | SF_STATIC | SF_EOF))
        return (( Sfio_t * )0);
    sfdisc(stream, dp);
    return (stream);
}
 ...
 */
Sfio_t *
stkinstall(Sfio_t *stream, _stk_overflow_ oflow)
{
    Sfio_t *old;
    struct stk *sp;
    if (!init) {
 ...
 */
int
stkon(Sfio_t *stream, char *loc)
{
    struct stk *sp = stream2stk(stream);
    struct frame *fp;
    for (fp = ( struct frame * )sp->stkbase; fp;
         fp = ( struct frame * )fp->prev)
 ...
 */
char *
stkset(Sfio_t *stream, char *loc, size_t offset)
{
    struct stk *sp = stream2stk(stream);
    char *cp;
    struct frame *fp;
    ...
    while (1) {
        ...
        if (loc >= cp && loc <= sp->stkend) {
            ...
            stream->_data
            = ( unsigned char * )(cp + roundof(loc - cp, STK_ALIGN));
 ...
 */
char *
stkalloc(Sfio_t *stream, size_t n)
{
    unsigned char *old;
    if (!init)
        stkinit(n);
 ...
 */
char *
_stkseek(Sfio_t *stream, ssize_t n)
{
    if (!init)
        stkinit(n);
    increment(seek);
 ...
 */
char *
stkfreeze(Sfio_t *stream, size_t extra)
{
 ...
 */
char *
stkcopy(Sfio_t *stream, const char *str)
{
...
static char *
stkgrow(Sfio_t *stream, size_t size)
{
    size_t n = size;
    struct stk *sp = stream2stk(stream);
