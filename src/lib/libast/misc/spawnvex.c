...
#    if _lib_fork || _lib_vfork
    pgid
    = inherit && (inherit->flags & SPAWN_SETGROUP) ? inherit->pgroup : -1;
    n = errno;
#        if _real_vfork
    exec_errno = 0;
...
#    ifdef F_DUPFD_CLOEXEC
        vex->debug
        = (flags & SPAWN_DEBUG) ? fcntl(2, F_DUPFD_CLOEXEC, 60) : -1;
#    else
        if ((vex->debug = (flags & SPAWN_DEBUG) ? fcntl(2, F_DUPFD, 60) : -1)
            >= 0)
...
#    endif
    ...
    if (vex->debug > 0)
        error(ERROR_OUTPUT,
              vex->debug,
              "spawnvex add %4d %8d %p %4d %4I*d %4I*d %p %p",
              __LINE__,
              getpid(),
              vex,
              vex->cur,
    ...
    if (!(flags & SPAWN_RESET)) {
        ...
        for (;;) {
            ...
            if (vex->debug >= 0)
                error(ERROR_OUTPUT,
                      vex->debug,
                      "spawnvex app %4d %8d %p %4d %4I*d %4I*d %p %p",
                      __LINE__,
                      getpid(),
                      vex,
                      k,
...
#    endif
                case SPAWN_noexec:
                    ...
                    break;
                case SPAWN_pgrp:
                    /* parent may succeed and cause setpigid() to fail but
                     * that's ok */
...
#    endif
                ...
                if (err
                    || callback && (err = (*callback)(handle, op, arg)) > 0) {
...
#    if _lib_spawn_mode || _lib_spawn && _hdr_spawn && _mem_pgroup_inheritance
    ...
    if (vex) {
        ...
        if (j >= 0) {
            ...
            if ((i = save(i, &ic, m)) < 0)
                goto bad;
            if (spawnvex_add(xev, SPAWN_cwd, i, 0, 0) < 0
                || restore(xev, i, -1, 0) < 0) {
                close(i);
                goto bad;
            }
...
#        if _lib_spawn_mode
    ...
    pid
    = spawnve(vex && vex->pgrp >= 0 ? P_DETACH : P_NOWAIT, path, argv, envv);
#        else
    inherit.flags = 0;
    map = 0;
    if (vex) {
...
#            endif
        if (pid == -1)
            ...
            if (!(flags & SPAWN_FOREGROUND))
                sigcritical(0);
            if (vex
                && (n = spawnvex_apply(vex, 0, SPAWN_FRAME | SPAWN_NOCALL)))
                errno = n;
            else {
                if (vex && vex->debug >= 0)
                    error(ERROR_OUTPUT,
                ...
                if (vex && vex->debug >= 0)
                    error(ERROR_OUTPUT,
                          vex->debug,
                          "spawnvex exe %4d %8d %p %4d \"%s\" FAILED",
                          __LINE__,
                          getpid(),
                          vex,
                          vex->cur,
...
#            if _use_spawn_exec
                    /*
                     * setting SPAWN_EXEC here means that it is more efficient
                     * to exec(interpreter) on script than to fork()
                     * initialize and read script -- highly subjective, based
                     * on some ksh implementaions, and probably won't be set
                     * unless its a noticable win
                     */
...
#            endif
                    nx.msgfd = msg[1];
                    errno
                    = (*vex->op[i + 2].callback)(&nx, SPAWN_noexec, errno);
                }
            }
#            if _real_vfork
            if (nx.flags & SPAWN_VFORK)
...
#            endif
        ...
        if (!(flags & SPAWN_FOREGROUND))
            sigcritical(0);
        if (pid != -1 && vex) {
            if (vex->pgrp >= 0 && setpgid(pid, vex->pgrp) < 0 && vex->pgrp
                && errno == EPERM)
                setpgid(pid, pid);
            VEXINIT(vex);
        }
...
#            endif
            case SPAWN_pgrp:
                if (err = posix_spawnattr_setpgroup(&ax, arg))
                    goto bad;
                if (err
                    = posix_spawnattr_setflags(&ax, POSIX_SPAWN_SETPGROUP))
                    goto bad;
                break;
            case SPAWN_resetids:
...
#            endif
                    ...
                    if (err = posix_spawn_file_actions_adddup2(&fx, fd, op))
                        goto bad;
                           ...
                           = posix_spawn_file_actions_adddup2(&fx, op, arg))
                    goto bad;
                break;
            }
        }
        if (err
            = posix_spawn(&pid, path, &fx, &ax, argv, envv ? envv : environ))
            goto bad;
        posix_spawnattr_destroy(&ax);
        ...
        VEXINIT(vex);
    } else if (err = posix_spawn(
               &pid, path, NiL, NiL, argv, envv ? envv : environ))
        goto nope;
    if (vex && vex->debug >= 0)
        error(ERROR_OUTPUT,
              vex->debug,
              "spawnvex exe %4d %8d %p %4d \"%s\" %8d posix_spawn",
              __LINE__,
              getpid(),
              vex,
              vex->cur,
...
nope:
    ...
    if (vex && vex->debug >= 0)
        error(ERROR_OUTPUT,
              vex->debug,
              "spawnvex exe %4d %8d %p %4d \"%s\" %8d posix_spawn FAILED",
              __LINE__,
              getpid(),
              vex,
              vex->cur,
