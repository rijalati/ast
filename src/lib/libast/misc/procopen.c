...
static const Namval_t options[] = { "debug",
                                    PROC_OPT_VERBOSE,
                                    "environment",
        ...
        case PROC_fd_dup | PROC_FD_PARENT | PROC_FD_CHILD:
            ...
            if (m->arg.fd.child.fd != m->arg.fd.parent.fd
                ...
                if (!(m->op & PROC_FD_PARENT)) {
                    ...
                    fcntl(
                    m->arg.fd.parent.fd, F_SETFD, m->arg.fd.parent.flag);
                }
...
#    endif
...
#if _use_spawnveg
            else if ((proc->pid = spawnveg(path, p, environ, proc->pgrp))
                     != -1)
                goto cleanup;
#endif
            if (errno != ENOEXEC)
        ...
        strcpy(env + 2,
               (flags & PROC_PARANOID) ? astconf("SH", NiL, NiL)
                                       : pathshell());
        if (forked || (flags & PROC_OVERLAY))
                ...
                case PROC_sys_pgrp:
                    if (proc->pgrp < 0)
                        ...
                        if (setpgid(proc->pid, proc->pgrp) < 0
                            && proc->pid != proc->pgrp && errno == EPERM)
