...
#    if __STD_C
int
_vmstat(Vmalloc_t *vm, Vmstat_t *st, size_t extra)
#    else
int vmstat(vm, st) Vmalloc_t *vm;
Vmstat_t *st;
#    endif
{
    Seg_t *seg;
    ...
    if (!vm->meth.meth)
        ...
        st->mesg,
        sizeof(st->mesg),
        "region %p size=%zu segs=%zu packs=%zu busy=%zu%% cache=%zu/%zu",
        vm,
        st->extent,
        st->n_seg,
        st->n_pack,
        (st->s_busy * 100) / st->extent,
        st->s_cache,
        st->n_cache);
...
#    endif
{
    ...
    {
        if (_vmheapinit(Vmheap)
            != Vmheap) /* initialize heap if not done yet */
            return -1;
        vm = Vmregion;
    }
