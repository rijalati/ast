...
#    if __STD_C
Vmalloc_t *
_vmopen(Vmalloc_t *vmo, Vmdisc_t *disc, Vmethod_t *meth, int mode)
#    else
...
#    endif
{
    ...
    else if (_vmheapinit(Vmheap)
             != Vmheap) /* make sure Vmheap is done first */
    ...
    VMPAGESIZE(); /**/
    DEBUG_ASSERT(_Vmpagesize > 0);

    mode
    = (mode & VM_OPENFLAGS) | meth->meth; /* user-settable flags & method */
    ...
    if (disc->exceptf) {
        if ((rv = (*disc->exceptf)(vmp, VM_OPEN, ( Void_t * ) (&addr), disc))
            ...
            if (initheap)
                write(
                9, "vmalloc: panic: heap initialization error #1\n", 45);
            return NIL(Vmalloc_t *);
        ...
        } else if (rv > 0) /* region opened before and now being restored */
        {
                  ...
                  = ( Vmdata_t * ) addr)) /* addr should point to Vmdata_t */
            {
                if (initheap)
                    write(
                    9, "vmalloc: panic: heap initialization error #2\n", 45);
                return NIL(Vmalloc_t *);
            }
    ...
    if (!vd) { /* amount to round to when getting raw memory via discipline */
        incr
        = disc->round <= 0 ? _Vmpagesize : ROUND(disc->round, _Vmpagesize);

        /* size of Vmalloc_t if embedded in the same initial segment */
        vmsz = (mode & VM_MEMORYF) ? ROUND(sizeof(Vmalloc_t), MEM_ALIGN) : 0;

        vdsz
        = 0; /* get actual size of Vmdata_t including method specific data */
        if (!meth->eventf || (*meth->eventf)(vmp, VM_OPEN, &vdsz) < 0
            || vdsz <= 0) {
            if (initheap)
                write(
                9, "vmalloc: panic: heap initialization error #3\n", 45);
            return NIL(Vmalloc_t *);
        }
        vdsz = ROUND(vdsz, MEM_ALIGN);

        sgsz
        = ROUND(sizeof(Seg_t), MEM_ALIGN); /* size of segment structure */

        /* get initial memory segment containing Vmdata_t, Seg_t and some
         * extra */
        ...
        if (!(base = ( Vmuchar_t * ) (*disc->memoryf)(
            ...
            if (initheap)
                write(
                9, "vmalloc: panic: heap initialization error #4\n", 45);
            return NIL(Vmalloc_t *);
        }
        if (_Vmassert & 0x10)
            memset(base, 0, size);
    ...
    if (vmo)
                   ...
                   Vmheap, sizeof(Vmalloc_t)))) /* on the heap */
        {
            ...
            if (initheap)
                write(
                9, "vmalloc: panic: heap initialization error #5\n", 45);
            return NIL(Vmalloc_t *);
        }
        *vm = *vmp;
    }
    ...
    } else if (vm != Vmheap) /* locklessly adding vm to list of regions */
    {
        ...
        for (vh = _Vmhold; vh;
             vh = vh->next) /* try adding to an existing slot */
        ...
        if (!vh) /* need a new slot */
        {
            if (!(vh = vmalloc(Vmheap, sizeof(Vmhold_t)))) {
                ...
                if (initheap)
                    write(
                    9, "vmalloc: panic: heap initialization error #6\n", 45);
                return NIL(Vmalloc_t *);
            } else
                vh->vm = vm;
            for (;;) {
...
#    if __STD_C
Vmalloc_t *
vmopen(Vmdisc_t *disc, Vmethod_t *meth, int mode)
#    else
