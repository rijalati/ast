...
#define VMLONG(addr)                                                         \
    ((Vmulong_t)(VMCAST(Vmuchar_t *, addr) - ( Vmuchar_t * ) 0))

/* Round x up to a multiple of y. ROUND2 does powers-of-2 and ROUNDX does
 * others */
...
#    define INITMEMORY(m, z)                                                 \
        ((m)                                                                 \
         ? (memset((m), 'i', (z) > 2 * MEM_ALIGN ? 2 * MEM_ALIGN : (z)), 0)  \
         : 0)
#    define SETBUSYMEM(m, z)                                                 \
        (memset((( char * ) (m)) + 2 * MEM_ALIGN,                            \
                'b',                                                         \
                (z) <= 2 * MEM_ALIGN ? 0 : MEM_ALIGN))
#    define CHKBUSYMEM(m, z)                                                 \
        (memcmp((( char * ) (m)) + 2 * MEM_ALIGN,                            \
                "bbbbbbbb",                                                  \
                (z) <= 2 * MEM_ALIGN ? 0 : 8)                                \
         == 0                                                                \
         ? 1                                                                 \
...
*/
#define SELF(b)                                                              \
    (( Block_t ** ) ((b)->body.data + BDSZ(b) - sizeof(Block_t *)))
#define PREV(b)                                                              \
    (*(( Block_t ** ) ((( Vmuchar_t * ) (b)) - sizeof(Block_t *))))
...
{
    Block_t *( *vm_seginit )
    _ARG_(( Vmdata_t *, Seg_t *, Vmuchar_t *, ssize_t, int ) );
...
extern Vmalloc_t *
_vmopen _ARG_(( Vmalloc_t *, Vmdisc_t *, Vmethod_t *, int ) );
