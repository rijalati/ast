...
#    define SM_MAKE                                                          \
        4 /* cache preallocation gradually increases up to a limit */
#    define SMmake(z) ((z) <= SM_MAX0 ? 4 * SM_MAKE : SM_MAKE)
#    define SMMAKE(pk, k, z)                                                 \
        ((pk)->small[k].make >= 1024 ? 1024                                  \
                                     : ((pk)->small[k].make += SMmake(z)))
...
#    define APPEND(e, b)                                                     \
        (((LINK(b) = LINK(e)) ? LEFT(LINK(b)) = (b) : NIL(Block_t *)),       \
         (LEFT(b) = (e)),                                                    \
         LINK(e) = (b),                                                      \
         SETLIST(b))
#    define DELETE(b)                                                        \
        ((LEFT(b) ? (LINK(LEFT(b)) = LINK(b)) : NIL(Block_t *)),             \
...
#    define METHOD(bst)                                                      \
        ((bst)->vmdt.mode & VM_METHODS) /* allocation method */

static int
chktree(Pack_t *pack, Block_t *node) /* check the shape of a free tree */
{
    Block_t *a, *b, **s;
...
static int
chkregion(Vmbest_t *best, int local) /* check region integrity */
{
    ssize_t k;
    Pack_t *pack;
    int rv = 0;
            ...
            if (asocasint(&pack->lock, 0, KEY_BEST) == 0) {
                if (pack->root) {
                    if (PACK(pack->root) != pack) { /**/
                        DEBUG_MESSAGE("Free tree with bad root");
                        rv = -1; /**/
                        DEBUG_ASSERT(0);
                    }
...
static int
bestfree(Vmalloc_t *vm, Void_t *data, int local)
{
    size_t sz;
    Block_t *blk, *head;
...
#    endif
    ...
    if (( Vmuchar_t * ) data < vm->data->segmin
        || ( Vmuchar_t * ) data >= vm->data->segmax)
        return -1;

    blk = BLOCK(data); /**/
    DEBUG_ASSERT((SIZE(blk) & BUSY) && (BDSZ(blk) % MEM_ALIGN) == 0);
    pack = PACK(blk); /**/
    DEBUG_ASSERT(pack->best == ( Vmbest_t * ) vm->data);
    ...
    if (!local && _Vmtrace)
        (*_Vmtrace)(
        vm, ( Vmuchar_t * ) data, NIL(Vmuchar_t *), TRUESIZE(sz), 0);
...
static Pack_t *
bestpackget(Vmalloc_t *vm, unsigned int ppos, unsigned int tid)
{
    unsigned int lpos;
    Block_t *blk, *pb, *nb;
    Pack_t *pack;
    ...
    if ((pack = asocasptr(&best->pack[ppos], NIL(Pack_t *), PK_INITIALIZING))
        != NIL(Pack_t *)) {
        asospindecl(); /* pack is initializing -- wait until its done */
        for (asospininit();;
             asospinnext()) { /* if we are inititializing then a signal hit --
                               * skip it or deadlock
                               */
    ...
    DEBUG_ASSERT(pack == NIL(Pack_t *)
                 && best->pack[ppos] == PK_INITIALIZING);
    /* set tid for the deadlock check above (which must happen inside the spin
     * loop!) */
    best->tid[ppos] = tid;

#    define EXTZ (8 * 1024) /* extra on each memory extension */
    if (!(blk = (*_Vmsegalloc)(
          vm, NIL(Block_t *), sizeof(Pack_t) + EXTZ, VM_SEGEXTEND))) {
        best->nomem = 1; /**/
        DEBUG_ASSERT(best->list[0]);
    ...
    if (_Vmassert & VM_debug)
        debug_printf(2,
                     ...
                     BDSZ(nb));
    pack->wild = nb; /**/
    DEBUG_ASSERT(PACKWILD(pack, pack->wild));
    ...
    pack->ppos = ppos; /**/
    DEBUG_ASSERT(best->pack[ppos] == PK_INITIALIZING);
    asocasptr(&best->pack[ppos], PK_INITIALIZING, pack);

    return pack;
}

/* extend pack->pblk to accomodate "size". "wild", if not NULL, is the bottom
 * piece */
static Block_t *
bestpackextend(Vmalloc_t *vm,
               Pack_t *pack,
               Block_t *wild,
               ssize_t size,
               int segtype)
{
    ...
    /**/ DEBUG_ASSERT(
    !wild
    || (PACK(wild) == pack && BDSZ(wild) < size && PACKWILD(pack, wild)));
    blkz = BDSZ(pack->pblk); /**/
    DEBUG_ASSERT(blkz >= _Vmpagesize);
    size += blkz - (wild ? BDSZ(wild) : 0) + EXTRA(pack); /**/
    DEBUG_ASSERT(size % MEM_ALIGN == 0);
    if (_Vmassert & VM_debug)
        debug_printf(
        2,
        "%s:%d: PACK(%p) WILD(%p)=%zd BDSZ(%p)=%zd blkz=%zd size=%zu\n",
        __FILE__,
        __LINE__,
        pack,
        wild,
    ...
    if (BDSZ(pblk) > blkz) /* superblock increased in size */
    {
        ...
        if ((_Vmassert & VM_debug)
            && ( Vmuchar_t * ) wild
               > (( Vmuchar_t * ) ENDB(pblk)
                  - sizeof(Head_t))) /* heisus in _Vmsegalloc */
        {
        ...
        if (_Vmassert & VM_debug)
            debug_printf(2,
                         "%s:%d: PACK(%p) pblk=%p endb=%p wild=%p size=%zu\n",
                         __FILE__,
                         __LINE__,
                         pack,
                         pblk,
        ...
        DEBUG_ASSERT(PACKWILD(pack, wild));
        if (_Vmassert & 1) {
            if (_Vmassert & VM_debug)
                debug_printf(2,
                             "%s:%d: PACK(%p) WILD(%p) => WILD(%p)=%zd\n",
...
static Block_t *
bestpackextract(Pack_t *pack, size_t size)
{
    size_t sz;
    ...
    pack->root = r; /**/
    DEBUG_ASSERT(!r || !(SIZE(r) & (BUSY | PFREE)));
...
static int
bestlistreclaim(Vmalloc_t *vm, Pack_t *pack, Block_t *volatile *listp)
{
    ssize_t size, sz;
    ...
    for (adj = NIL(Block_t *), fp = list;;) { /**/
        ...
        {
            np = PREV(fp); /**/
            DEBUG_ASSERT(!(SIZE(np) & SMALL) && NEXT(np) == fp
                         && PACK(np) == pack);
            REMOVE(pack, np); /**/
            DEBUG_ASSERT((SIZE(np) & BITS) == 0);
            SIZE(np) += (BDSZ(fp) + sizeof(Head_t)) | BUSY | MARK;

            LINK(np) = LINK(fp);
            fp = np; /* replace fp by np in the list */
            if (adj)
                LINK(adj) = fp;
            else
                list = fp;
            ...
            np = NEXT(fp);
            sz = SIZE(np); /* try merging next block */
            if (sz & MARK) {
    ...
    for (fp = list; fp != NIL(Block_t *); fp = adj) {
        ...
        /**/ DEBUG_ASSERT(BDSZ(fp) >= sizeof(Body_t)
                        && BDSZ(fp) % MEM_ALIGN == 0);
        SIZE(fp) &= ~BITS;
        t = NEXT(fp);
        SIZE(t) |= PFREE; /**/
        DEBUG_ASSERT(SIZE(NEXT(fp)) & BUSY);
        s = SELF(fp);
...
static Block_t *
bestpackalloc(Vmalloc_t *vm, Pack_t *pack, size_t size, size_t minz)
{
    ssize_t sz;
    Block_t *tp, *np, *pblk;
    Vmbest_t *best = ( Vmbest_t * ) vm->data;
    ...
    if ((tp = pack->alloc)) /* fast allocation from recent memory */
    {
        ...
        if (SIZE(tp) >= size) {
            if (_Vmassert & VM_debug)
                debug_printf(
                2,
                "%s:%d: PACK(%p) SIZE(%p)=%zd size=%zu minz=%zu\n",
                __FILE__,
                __LINE__,
                pack,
                tp,
            ...
            if (tp) {
                ...
                if (!PACKWILD(pack, tp)) {
                    if (_Vmassert & VM_debug)
                        debug_printf(
                        2,
                        "%s:%d: PACK(%p) WILD(%p)=%zd orphaned wild\n",
                        __FILE__,
                        __LINE__,
                        pack,
                        tp,
    ...
    if (!(pblk = (*_Vmsegalloc)(
          vm, NIL(Block_t *), size + EXTRA(pack), VM_SEGEXTEND))) {
        best->nomem = 1;
        return NIL(Block_t *);
    } else /* got a new superblock */
    {
        ...
        pack->pblk = pblk; /**/
        DEBUG_ASSERT(BDSZ(pblk) > size);
    }

a_lloc: /**/
    DEBUG_ASSERT(tp && BDSZ(tp) >= minz && PACK(tp) == pack);
    if ((sz = ( ssize_t ) BDSZ(tp) - ( ssize_t ) size)
        >= ( ssize_t ) sizeof(Block_t)) {
...
static Block_t *
bestsmallalloc(Vmalloc_t *vm, Pack_t *pack, ssize_t size)
{
    int n;
    ...
    if (asocaschar(&small->lock, 0, 1)
        != 0) /* lock to prevent concurrent allocs */
        return NIL(
        Block_t *); /* list is busy; return to explore a different pack */

    for (asospininit();;
         asospinnext()) /* loop is needed because bestfree() is lock-free */
        if (!(blk = small->free)
            || asocasptr(&small->free, blk, LINK(blk)) == blk)
            break;
    ...
    if (!blk && asocasint(&pack->lock, 0, KEY_BEST) == 0) {
        ...
        if ((blk = bestpackalloc(vm, pack, memz, SM_MAKE * blkz))) {
            memz = SIZE(blk) + sizeof(Head_t); /**/
            DEBUG_ASSERT(memz > SM_MAKE * blkz);
            memz = memz - (n = memz / blkz - 1) * blkz
                   - sizeof(Head_t); /* size of end block */
...
static Void_t *
bestalloc(Vmalloc_t *vm, size_t size, int local)
{
    ...
    if (!(pk = best->pack[ppos]) || pk == PK_INITIALIZING)
        pk = bestpackget(vm, ppos, tid); /**/
    DEBUG_ASSERT(pk != PK_INITIALIZING);
    if (pk)
        lpos = begp = pk->lpos;
    else
        begp = (lpos = 0) + 1;
    ...
    for (asospininit();;) {
        ...
        {
            if (!best->pkcnt
                || (lpos = (lpos + 1) % best->pkcnt)
                   == begp) /* cycling over list[] */
            {
                if (best->nomem) /* nothing more possible to do */
                    return NIL(Void_t *);

                do { /* conflict at ppos so move it forward to reduce
                      * contention
                      */
...
static Void_t *
bestresize(Vmalloc_t *vm, Void_t *data, size_t size, int type, int local)
{
    Block_t *rp, *np;
    ...
    /* quick sanity check */
    if (( Vmuchar_t * ) rsdt < vm->data->segmin
        || ( Vmuchar_t * ) rsdt >= vm->data->segmax)
        return NIL(Void_t *);

    rp = BLOCK(rsdt); /**/
    DEBUG_ASSERT(SIZE(rp) & BUSY);
    pack = PACK(rp); /**/
    DEBUG_ASSERT(pack->best == best);
    oldz = TRUEBDSZ(rp);
    newz = LGROUND(size);
    if (oldz == newz)
        goto done;
    ...
    if (SIZE(rp) & SMALL) /* small blocks cannot be extended */
    {
        if (oldz >= newz)
            ...
            goto a_lloc; /* move to a new area */
    }
    ...
    if (( Block_t * ) PACK(np) == rp && (SIZE(np) & MARK)) {
        ...
        if ((sz = newz - oldz) > 0
            && 8 * sz < oldz) /* add to reduce future work */
        {
    ...
    newz
    += incz; /* add space for tail info and perhaps to reduce future work */
    ...
    a_lloc: /* see if we can move to a new area */
        if (!(type & (VM_RSMOVE | VM_RSCOPY))
            || !(rsdt = KPVALLOC(vm, newz, bestalloc)))
            return NIL(Void_t *);
...
static Void_t *
bestalign(Vmalloc_t *vm, size_t size, size_t align, int local)
{
    Vmuchar_t *data;
    Block_t *tp, *np;
    ...
    if ((remz = (size_t)((VMLONG(data) + extra) % algn))
        != 0) /* shift to alignment */
    {
        data += algn - remz; /**/
        ...
        sz = ( Vmuchar_t * ) np - ( Vmuchar_t * ) tp; /**/
        DEBUG_ASSERT(sz >= sizeof(Block_t));
        PACK(np) = pack;
...
static int
beststat(Vmalloc_t *vm, Vmstat_t *st, int local)
{
    ssize_t k, ty, sz;
    ...
    for (seg = best->vmdt.seg; seg; seg = seg->next)
        for (sgb = ( Block_t * ) SEGDATA(seg); sgb < seg->endb;
             sgb = NEXT(sgb)) { /**/
            ...
            for (endbp = ENDB(sgb); bp < endbp; bp = TRUENEXT(bp)) {
                pack = PACK(bp);
                if (DATA(bp) == ( Void_t * ) pack) /* administrative data for
                                                      the pack */
                    continue;

                sz = TRUEBDSZ(bp); /**/
                DEBUG_ASSERT(sz < BDSZ(sgb));
                if (SIZE(bp) & BUSY) {
                    st->n_busy += 1;
                    st->s_busy += sz;
                } else /* a free block */
...
static int
bestevent(Vmalloc_t *vm, int event, Void_t *arg)
{
    Vmbest_t *best = ( Vmbest_t * ) vm->data;
...
void
_vmchkaddress(Vmalloc_t *vm, Vmuchar_t *addr, int tag)
{
    if (vm && addr && (addr < vm->data->segmin || addr >= vm->data->segmax))
        _vmmessage("Address not belonging to a region",
                   0,
                   tag ? "Tag" : NIL(const char *),
                   tag);
}

void
_vmchkall(int tag) /* check to see if some region may have a bad free list or
                      cache */
{
    Vmhold_t *vh;
    Vmbest_t *best;
    ...
    for (vh = _Vmhold; vh; vh = vh->next) {
        ...
        for (p = 0; p < best->pkcnt; ++p) {
            ...
            for (k = 0; k < SM_SLOT; ++k)
                _vmchkaddress(
                vh->vm, ( Vmuchar_t * ) pack->small[k].free, tag);
        }
    }
}
...
#    define VMHOLD(vm, b)                                                    \
        (( Vmuchar_t * ) (b) >= (vm)->data->segmin                           \
         && ( Vmuchar_t * ) (b) < (vm)->data->segmax)
#    define SGHOLD(sg, b)                                                    \
        (( Vmuchar_t * ) (b) >= (sg)->base                                   \
         && ( Vmuchar_t * ) (b) < (sg)->base + (sg)->size)

#    define CAREFUL                                                          \
        1 /* 1 for any system where different malloc packages may be mixed   \
             together */
#    ifdef CAREFUL /* blk is ascertained to be allocated in a region before  \
                      SEG() is invoked */
    if (VMHOLD(Vmheap, addr))
        for (seg = (vmdt = Vmheap->data)->seg; seg; seg = seg->next)
            if (SGHOLD(seg, blk) && (pack = ( Pack_t * ) PACK(blk))
                && pack->best == ( Vmbest_t * ) vmdt)
                return Vmheap;
...
#    else /* !CAREFUL: blk is (optimistically) assumed to be allocated by    \
             Vmalloc */
    if (VMHOLD(Vmheap, addr))
        if ((pack = ( Pack_t * ) PACK(blk))->best
            == ( Vmbest_t * ) Vmheap->data)
            return Vmheap;
