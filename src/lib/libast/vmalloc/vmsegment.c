...
#    if _ast_sizeof_size_t > 4 /* the address space is greater than 32-bit   \
                                */
#        define VM_SEGSIZE                                                   \
            (4 * 1024 * 1024) /* lots of memory available here	*/
#    else
...
static Block_t *
_vmfreelist(Vmdata_t *vmdt, Void_t *arg, int type)
{
    Block_t *pb, *bp, *blk;
...
int
vmsegwalk(Vmalloc_t *vm, Vmseg_f segf, Void_t *handle)
{
    Vmhold_t *vh;
    Seg_t *seg;
    Vmalloc_t *todo;
    ...
    for (vh = _Vmhold;; vh = vh->next) {
        if ((todo = vm ? vm : vh ? vh->vm : Vmheap) && todo->data)
            for (seg = todo->data->seg; seg; seg = seg->next)
                if ((rv = (*segf)(
                     todo, seg->base, seg->size, todo->disc, handle))
                    < 0)
                    return -1;
        if (vm || !vh)
            break;
...
Void_t *
vmsegfind(Vmalloc_t *vm, Void_t *addr)
{
    Seg_t *seg;
...
/* lock-less seg initialization with containing region vmdt and actual memory
 * [base,size] */
static Block_t *
_vmseginit(Vmdata_t *vmdt,
           Seg_t *seg,
           Vmuchar_t *base,
           ssize_t size,
           int insert)
{
    ...
    do
        seg->next = vmdt->seg;
    while (asocasptr(&vmdt->seg, seg->next, seg) != seg->next);

    return seg->begb;
}

static void
_vmsegmerge(Vmdata_t *vmdt, Block_t *blk)
{
    ssize_t size;
    Block_t *next;
...
static Block_t *
_vmsegalloc(Vmalloc_t *vm, Block_t *blk, ssize_t size, int type)
{
    unsigned int key;
    int heisus;
    ssize_t sz, segsz;
...
#    define RETURN(x)                                                        \
    ...
    if (vmdt->lock == key) {
        ...
        if (blk && (type & VM_SEGEXTEND)) /* no physical extension when he is
                                             us */
        {
        ...
        if (asocasint(&seg->iffy, 0, 1)
            == 0) { /* a longjmp() below will make seg not further extendible
                     */
            ...
            {                                      /**/
                ...
                if ((sz = segsz - seg->size)
                    <= 0) /* wrapped around, not good! */
                {
                    seg->iffy = 0;
                    RETURN(blk = NIL(Block_t *));
                }
...
static char *
_vmstrcpy(char *to, const char *from, int endc)
{
    int n;
...
static ssize_t
_vmgcd(ssize_t one, ssize_t two)
{
    if (one == two)
        return one;
    else if (one == 0 || two == 0)
        ...
        if (two & 1)
            return one > two ? _vmgcd((one - two) / 2, two)
                             : _vmgcd(one, (two - one) / 2);
        else
            return _vmgcd(one, two / 2);
    } else {
        if (two & 1)
            return _vmgcd(one / 2, two);
        else
            return 2 * _vmgcd(one / 2, two / 2);
    }
}

static ssize_t
_vmlcm(ssize_t one, ssize_t two)
{
    if (one == 0 || two == 0)
        return 0;
    else
        return (one / _vmgcd(one, two)) * two;
}
