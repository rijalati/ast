...
#    if __STD_C
static void
vmdbwarn(Vmalloc_t *vm, char *mesg, int n)
#    else
...
#    define SLOP 64 /* enough for a message and an int */
    ...
    if (!data)
    {
        if (where == DB_ALLOC)
            ...
            bufp = (*_Vmstrcpy)(bufp, "locked region", ':');
    }
    else if (type == DB_FREE || type == DB_RESIZE)
    {
        if (where == 0)
            ...
            bufp = (*_Vmstrcpy)(bufp, "not currently allocated", ':');
    }
    else if (type == DB_WATCH)
    {
    ...
    else if (type == DB_CHECK)
    {
        ...
        if ((s = DBFILE(data)) && (bufp + strlen(s) + SLOP) < endbuf)
        {
            ...
            bufp
            = (*_Vmstrcpy)(bufp, (*_Vmitoa)(VMLONG(DBLINE(data)), -1), ':');
        }
    }
...
dbwatch(Vmalloc_t *vm,
        ...
        int type)
#    else
static void dbwatch(vm, data, file, line, func, type) Vmalloc_t *vm;
Void_t *data;
char *file;
int line;
Void_t *func;
...
#    if __STD_C
static void
dbsetinfo(Vmuchar_t *data, size_t size, char *file, int line)
#    else
static void
...
#    if __STD_C
static Void_t *
dballoc(Vmalloc_t *vm, size_t size, int local)
#    else
static Void_t *dballoc(vm, size, local) Vmalloc_t *vm;
size_t size;
int local;
#    endif
{
...
#    if __STD_C
static int
dbfree(Vmalloc_t *vm, Void_t *data, int local)
#    else
static int dbfree(vm, data, local) Vmalloc_t *vm;
Void_t *data;
int local;
#    endif
{
    ...
    { /* prepend to delayed free list -- handled by another free() that gets
         the lock */
        asospindecl();
        item = ( Free_t * )data;
    ...
    for (seg = vm->data->seg; seg; seg = seg->next)
    {
        ...
        for (; sgb < seg->endb; sgb = NEXT(sgb))
        {
            ...
            for (; bp < endbp; bp = NEXT(bp))
            {
                ...
                set_bad:
                    dbwarn(
                    vm, data, begp - data, NIL(char *), 0, 0, DB_CHECK);
                    DBSETBAD(data);
                    rv += 1;
                    break;
                }
...
#    if __STD_C
static Void_t *
dbalign(Vmalloc_t *vm, size_t size, size_t align, int local)
#    else
static Void_t *dbalign(vm, size, align, local) Vmalloc_t *vm;
size_t size;
size_t align;
int local;
#    endif
...
static int
dbstat(Vmalloc_t *vm, Vmstat_t *st, int local)
{
...
#    if __STD_C
ssize_t
vmdbstat(Vmalloc_t *vm)
#    else
ssize_t vmdbstat(vm) Vmalloc_t *vm;
#    endif
{
    ssize_t n;
    Vmstat_t st;
...
= { dballoc, dbresize, dbfree, 0, dbstat, dbevent, dbalign, VM_MTDEBUG };
