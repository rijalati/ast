...
#    define LASTLOCK(lst, lcl)                                               \
        ((lcl) ? 0 : asolock(&(lst)->lock, KEY_LAST, ASO_LOCK))
#    define LASTOPEN(lst, lcl)                                               \
        ((lcl) ? 0 : asolock(&(lst)->lock, KEY_LAST, ASO_UNLOCK))
...
#    if __STD_C
static Void_t *
lastalloc(Vmalloc_t *vm, size_t size, int local)
#    else
static Void_t *lastalloc(vm, size, local) Vmalloc_t *vm;
size_t size;
int local;
#    endif
{
...
#    if __STD_C
static int
lastfree(Vmalloc_t *vm, Void_t *data, int local)
#    else
static int lastfree(vm, data, local) Vmalloc_t *vm;
Void_t *data;
int local;
#    endif
{
...
#    if __STD_C
static Void_t *
lastresize(Vmalloc_t *vm, Void_t *data, size_t size, int type, int local)
#    else
static Void_t *lastresize(vm, data, size, type, local) Vmalloc_t *vm;
Void_t *data;
size_t size;
int type;
int local;
...
#    if __STD_C
static Void_t *
lastalign(Vmalloc_t *vm, size_t size, size_t align, int local)
#    else
static Void_t *lastalign(vm, size, align, local) Vmalloc_t *vm;
size_t size;
size_t align;
int local;
#    endif
...
static int
lastevent(Vmalloc_t *vm, int event, Void_t *arg)
{
    Vmlast_t *last;
