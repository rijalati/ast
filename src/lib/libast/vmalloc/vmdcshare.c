...
static Mmvm_t *
mmfix(Mmvm_t *mmvm, Mmdisc_t *mmdc, int fd)
{
...
#    if __STD_C
static int
mmexcept(Vmalloc_t *vm, int type, Void_t *data, Vmdisc_t *disc)
#    else
...
#    if __STD_C
Vmdisc_t *
vmdcshare(char *name, int proj, ssize_t size, int mode)
#    else
Vmdisc_t *vmdcshare(name,
                    proj,
                    size,
...
#    endif
{
    ...
    mmdc->init = 0;
    mmdc->mode
    = mode > 0 ? 0 : mode == 0 ? MM_DETACH : (MM_DETACH | MM_REMOVE);
    mmdc->proj = proj;
