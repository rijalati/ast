...
#define GETMEMUSE(getmem, disc)                                              \
    if (_Vmassert & VM_verbose)                                              \
        debug_printf(2, "vmalloc: getmemory=%s\n", XTR(getmem));             \
...
#ifdef MADV_HUGEPAGE
#    define ADVISE(v, a, z)                                                  \
        ((a) && ((z) >= 8 * _Vmpagesize) ? madvise((a), (z), MADV_HUGEPAGE)  \
                                         : 0)
#else
#    define ADVISE(v, a, z)
#endif
...
bad:
    ...
    RETURN(vm, (newm && caddr) ? caddr : newm, nsize);
                    ...
                    */
/*
** concurrency-safe memory allocation via mmap-anonymous providing an
*emulation of sbrk()
*/
static Void_t *
safebrkmem(Vmalloc_t *vm,
           Void_t *caddr,
           size_t csize,
           size_t nsize,
           Vmdisc_t *disc)
{
    ...
    if (_Vmmemsbrk && _Vmmemsbrk < _Vmmemmax) {
        if (_Vmchkmem
            && (_Vmassert & VM_check_seg)) /* mmap must use MAP_FIXED (eg,
                                              solaris) */
        {
...
#    endif
    if (fd >= FD_PRIVATE
        || (mmdc->fd = fcntl(fd, F_DUPFD_CLOEXEC, FD_PRIVATE)) < 0)
        mmdc->fd = fd;
    else {
        close(fd);
          ...
          Vmdisc_t *disc)
{
    ...
    static Memdisc_t _Vmdcsystem = { { getmemory,
                                       NIL(Vmexcept_f),
                                       0 * 64 * 1024 * 1024,
                                       sizeof(Memdisc_t) },
                                     FD_INIT,
...
#define VMHEAPMETH Vmbest
    ...
    Vmalloc_t *_vmheapinit(Vmalloc_t * vm)
    {
        ...
        /**/ DEBUG_ASSERT(
        !vm /* called from _vmstart() in malloc.c */ || vm == Vmheap);

        if ((status = asocasint(&Init, 0, HEAPINIT)) == HEAPDONE)
            return Vmheap;
        else if (status
                 == HEAPINIT) /* someone else is doing initialization */
        {
        ...
        if (_Vmassert & VM_verbose)
            debug_printf(2,
                         "vmalloc: pagesize=%zu segsize=%zu assert=0x%08x\n",
                         _Vmpagesize,
                         _Vmsegsize,
                         _Vmassert);
        ...
        if (vm == Vmheap
            && Init
               == HEAPDONE) /* initialize malloc/free/realloc interface */
            free(NIL(Void_t *));
    ...
    {
        return (vm = _vmheapinit(vm)) ? (*vm->meth.allocf)(vm, size, local)
                                      : NIL(Void_t *);
    }
    static Void_t *heapresize(
    Vmalloc_t * vm, Void_t * obj, size_t size, int type, int local)
    {
        return (vm = _vmheapinit(vm))
               ? (*vm->meth.resizef)(vm, obj, size, type, local)
               : NIL(Void_t *);
    }
    static int heapfree(Vmalloc_t * vm, Void_t * obj, int local)
    {
        return (vm = _vmheapinit(vm)) ? (*vm->meth.freef)(vm, obj, local)
                                      : -1;
    }
    static Void_t *heapalign(
    Vmalloc_t * vm, size_t size, size_t align, int local)
    {
        return (vm = _vmheapinit(vm))
               ? (*vm->meth.alignf)(vm, size, align, local)
               : NIL(Void_t *);
    }
