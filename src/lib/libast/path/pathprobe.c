...
pathprobe(char *path,
          ...
          int op)
{
    return pathprobe_20100601(
    lang, tool, proc, op, path, PATH_MAX, attr, PATH_MAX);
}
...
#    if defined(ST_NOSUID)
        if ((vfs.f_flag & ST_NOSUID)
            && (stat(path, &st)
                || st.st_uid != getuid() && st.st_uid != geteuid()))
            return 1;
#    endif
    }
    return 0;
...
char *
pathprobe_20100601(const char *lang,
                   const char *tool,
                   const char *aproc,
                   int op,
                   char *path,
