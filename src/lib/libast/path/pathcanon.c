...
#define NAMED_XATTR                                                          \
    1 /* /dev/file/xattr@...//@// named xattr dir 0:disabled 1:enabled */
...
again:
    ...
    if (path[0] == '/') {
        ...
        } else if (size > 7 && s[1] == 'd' && s[2] == 'e' && s[3] == 'v'
                   && s[4] == '/') {
            NEXT(s, 5);
            if (s[0] == 'f') {
                if (s[1] == 'd' && (s[2] == '/' || s[2] == 0)) {
                ...
                if (*s) {
                    ...
                    if (*t) {
                        dev->port.offset
                        = canon ? (dev->host.offset + dev->host.size + 1)
                                : (t - p);
                        if (s = strchr(t, '/')) {
                            dev->port.size = s - t;
                            NEXT(s, 0);
        ...
        if (x)
            for (t = x; r = strchr(t, '@'); t = r + 1)
                if ((r - t) >= 2 && r[-2] == '/' && r[-1] == '/'
                    ...
                    *r = 0;
                    if ((dev->fd = openat(dfd,
                                          buf,
                                          O_INTERCEPT | O_RDONLY | O_NONBLOCK
                                          | O_CLOEXEC | dev->oflags))
                        < 0)
                        return 0;
                    if ((n = openat(dev->fd,
                                    ".",
                                    O_INTERCEPT | O_RDONLY | O_XATTR
                                    | O_NONBLOCK))
                        < 0) {
                        close(dev->fd);
                        dev->fd = -1;
    ...
    if ((flags & PATH_ABSOLUTE) && dev->fd < 0
        && (x && *x != '/' || *s != '/')
    ...
    while (t <= e)
        ...
        case '/':
            ...
            case 0:
                ...
                if (t == canon)
                    *t++ = '.';
                else if (((flags & PATH_DROP_TAIL_SLASH) || s <= b
                          || *(s - 1) != '/')
                         && t > r + 1 && *(t - 1) == '/')
                    t--;
                *t = 0;
