...
#            define CDTLIB(m)                                                \
                void *cdt_lib(                                               \
                const char *name, Dtdisc_t *disc, const char *type)          \
                {                                                            \
                    int i;                                                   \
                    int n;                                                   \
                    if (!type)                                               \
...
#    define DTSETLOCK(dt)                                                    \
        (((dt)->data->type & DT_SHARE)                                       \
         ? asolock(&(dt)->data->lock, 1, ASO_LOCK)                           \
         : 0)
#    define DTCLRLOCK(dt)                                                    \
        (((dt)->data->type & DT_SHARE)                                       \
...
#    define DTERROR(dt, mesg)                                                \
        (!((dt)->disc && (dt)->disc->eventf)                                 \
         ? 0                                                                 \
         : (*(dt)->disc->eventf)(                                            \
           (dt), DT_ERROR, ( Void_t * )(mesg), (dt)->disc))

/* announce completion of an operation of type (ty) on some object (ob) in
 * dictionary (dt) */
#    define DTANNOUNCE(dt, ob, ty)                                           \
        (((ob) && (( ty )&DT_TOANNOUNCE) && ((dt)->data->type & DT_ANNOUNCE) \
          && (dt)->disc && (dt)->disc->eventf)                               \
...
#    define RRSHIFT(x, t)                                                    \
        ((t) = (x)->_left->_left,                                            \
         (x)->_left->_left = (t)->_rght,                                     \
         (t)->_rght = (x),                                                   \
         (x) = (t))
#    define LLSHIFT(x, t)                                                    \
        ((t) = (x)->_rght->_rght,                                            \
         (x)->_rght->_rght = (t)->_left,                                     \
         (t)->_left = (x),                                                   \
         (x) = (t))
