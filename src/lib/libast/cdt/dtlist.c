...
#ifdef DEBUG
int
dtlistprint(Dt_t *dt, Dtlink_t *here, char *(*objprintf)( Void_t * ))
{
    int k;
...
#if __STD_C
Void_t *
lfirstlast(Dt_t *dt, int type)
#else
Void_t *lfirstlast(dt, type) Dt_t *dt;
int type;
#endif
{
    Dtlink_t *lnk;
...
#if __STD_C
Void_t *
llist(Dt_t *dt, Dtlink_t *lnk, int type)
#else
Void_t *llist(dt, lnk, type) Dt_t *dt;
Dtlink_t *lnk;
int type;
#endif
{
...
#if __STD_C
static Void_t *
listat(Dt_t *dt, Dtstat_t *st)
#else
static Void_t *listat(dt, st) Dt_t *dt;
Dtstat_t *st;
#endif
{
    if (st) {
        ...
        st->space
        = sizeof(Dtlist_t)
          + (dt->disc->link >= 0 ? 0 : dt->data->size * sizeof(Dthold_t));
    }
...
#if __STD_C
static Void_t *
dtlist(Dt_t *dt, Void_t *obj, int type)
#else
static Void_t *dtlist(dt, obj, type) Dt_t *dt;
Void_t *obj;
int type;
#endif
{
...
#if __STD_C
static int
listevent(Dt_t *dt, int event, Void_t *arg)
#else
static int listevent(dt, event, arg) Dt_t *dt;
int event;
Void_t *arg;
#endif
{
    ...
    if (event == DT_OPEN) {
        if (list) /* already initialized */
            return 0;
        if (!(list = ( Dtlist_t * )(*dt->memoryf)(
              dt, 0, sizeof(Dtlist_t), dt->disc))) {
