...
int
dtuserlock(Dt_t *dt, unsigned int key, int type)
{
    if (key == 0)
        return -1;
    else if (type > 0)
...
Void_t *
dtuserdata(Dt_t *dt, Void_t *data, int set)
{
...
int
dtuserevent(Dt_t *dt, int flags, Void_t *data)
{
    if (!dt->disc->eventf)
        return 0;
    else
        return (*dt->disc->eventf)(
        dt, DT_ANNOUNCE | DT_USER | flags, data, dt->disc);
}
