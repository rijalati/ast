...
**
**      Written by Kiem-Phong Vo (phongvo@gmail.com) and Adam Edgar
*(09/06/2010).
*/

/* locking and unlocking a class of objects */
#define HCLSLOCK(dt, hh, ty, sh)                                             \
    ((sh) == 0 ? 0 : hclslock((dt), (hh), (ty), 1))
...
#define HSIZE(hh, lv)                                                        \
    ((lv) >= (hh)->nlev ? (1 << H_BITA) : (hh)->mask[lv] + 1)

/* the base (starting search) position of an element in a given table and
 * #search steps */
#define HBASP(hh, lv, h)                                                     \
    ((lv) >= (hh)->nlev ? 0 : (((h) >> (hh)->shft[lv]) & (hh)->mask[lv]))
#define HSRCH(hh, lv)                                                        \
    ((lv) >= (hh)->nlev                                                      \
     ? (1 << H_BITA)                                                         \
     : (lv) == 0 ? H_SRCH0                                                   \
                 : (lv) == 1 ? H_SRCH1 : (lv) == 2 ? H_SRCH2 : H_SRCHA)

/* the actual address of an element supposed to be at location p */
#define HLNKP(tb, p)                                                         \
    (((tb)->list[p] && HTABLE((tb)->list[p]))                                \
     ? &(( Htbl_t * )(tb)->list[p])->pobj                                    \
     : (tb)->list + (p))
...
static Htbl_t *
htable(Dt_t *dt, ssize_t lev, Htbl_t *ptbl, ssize_t ppos)
#else
...
static void
hdelete(Dt_t *dt, Dtlink_t **lnkp, int type)
#else
static void hdelete(dt, lnkp, type) Dt_t *hash;
Dtlink_t **lnkp;
int type;
#endif
{
...
static Void_t *
hclear(Dt_t *dt, Htbl_t *tbl, ssize_t lev, int zap)
#else
static Void_t *hclear(dt, tbl, lev, zap) Dt_t *dt; /* dictionary structure
                                                    */
...
#if __STD_C /* construct/extract a list of objects, or reconstruct from a    \
               list */
static Void_t *
hlist(Dt_t *dt, Dtlink_t *list, int type)
#else
static Void_t *hlist(dt, list, type) Dt_t *dt;
Dtlink_t *list;
int type;
#endif
{
...
static ssize_t
hsize(Dt_t *dt, Htbl_t *tbl, ssize_t lev, Dtstat_t *st)
#else
static ssize_t hsize(dt, tbl, lev, st) Dt_t *dt;
Htbl_t *tbl;
ssize_t lev;
Dtstat_t *st;
#endif
...
#if __STD_C
static Void_t *
hstat(Dt_t *dt, Dtstat_t *st)
#else
static Void_t *hstat(dt, st) Dt_t *dt;
Dtstat_t *st;
#endif
{
    ssize_t size;
    ...
    else
    {
        ...
        st->space = sizeof(Hash_t)
                    + (dt->disc->link >= 0 ? 0 : size * sizeof(Dthold_t));
        /**/ DEBUG_ASSERT((dt->data->type & DT_SHARE)
                        || size == hash->data.size);
    }
...
static Void_t *
dthashtrie(Dt_t *dt, Void_t *obj, int type)
{
    Void_t *o, *ky, *key;
    uint hsh;
    Dtlink_t *lnk, **lnkp, *t;
           ...
           | DT_STAT | DT_EXTRACT | DT_RESTORE | DT_FLATTEN))
    {
        ...
        {
            if (!(fngr
                  = (*dt->memoryf)(dt, NIL(Void_t *), sizeof(Fngr_t), disc)))
                return NIL(Void_t *);
        ...
        else if (type & DT_STEP) /* take a step forward in a walk */
        {
            ...
            obj = _DTOBJ(disc, lnk);
            if (!hnext(
                dt, fngr, fngr->mtbl, fngr->mlev, fngr->mpos, lnk->_hash, type))
                fngr->here
                = NIL(Dtlink_t *); /* walk will end after this call */
    ...
    if (!share && (type & (DT_NEXT | DT_PREV)) && (lnk = fngr->here)
        && obj == _DTOBJ(disc, lnk) && (tbl = fngr->mtbl)
        && (pos = fngr->mpos) >= 0 && (lev = fngr->mlev) >= 0)
...
do_insert: /**/
    ...
    if (!opnt) /* make a new subtable */
    {
        ...
        opnt->pobj
        = tbl->list[hshp]; /* move slot content to drop down slot */
...
hashevent(Dt_t *dt, int event, Void_t *arg)
{
    ...
    if (event == DT_OPEN)
    {
        ...
        for (k = 1; k < H_NLEV && b < H_NBITS; ++k)
        {
            z = k == 1
                ? H_BIT1
                : k == 2 ? H_BIT2
                         : k >= H_NLEV - 1 ? H_NBITS - b
                                           : /* last level gets all bits */
        ...
        if (!(hash->root
              = htable(dt, 0, NIL(Htbl_t *), -1))) /* make root table */
        {
             ...
             == DT_SHARE) /* turn on/off concurrency - return 1 on success */
    {
        ...
        if (( int )(( Dtuint_t )arg)
            > 0) /* set up structures for share mode */
        {
            if (!hash->lock) /* allocate lock table */
            {
                ...
                hash->lmax
                = z - 1; /* max lock index - usable for modulo 2^n */
            }
