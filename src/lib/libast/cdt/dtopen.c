...
#if __STD_C
Dt_t *
_dtopen(Dtdisc_t *disc, Dtmethod_t *meth, unsigned long version)
#else
...
#if __STD_C
Dt_t *
dtopen(Dtdisc_t *disc, Dtmethod_t *meth)
#else
...
Dtlink_t *
_dtmake(Dt_t *dt, Void_t *obj, int type)
{
    Dthold_t *h;
    Dtdisc_t *disc = dt->disc;

    /* if obj is a prototype, make a real one */
    if (!(type & DT_ATTACH) && disc->makef
...
void
_dtfree(Dt_t *dt, Dtlink_t *l, int type)
{
    Dtdisc_t *disc = dt->disc;
