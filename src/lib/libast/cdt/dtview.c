...
#define DT_NOVIEWPATH                                                        \
    (DT_INSERT | DT_APPEND | DT_DELETE | DT_ATTACH | DT_DETACH | DT_RELINK   \
     | DT_CLEAR | DT_FLATTEN | DT_EXTRACT | DT_RESTORE | DT_STAT)

#if __STD_C
static Void_t *
dtvsearch(Dt_t *dt, reg Void_t *obj, reg int type)
#else
...
#endif
{
    ...
    {
        if (!(type
              & (DT_FIRST | DT_LAST | DT_NEXT | DT_PREV | DT_ATLEAST
                 | DT_ATMOST)))
            return NIL(Void_t *);
    ...
    for (d = dt->walk, obj = (*d->meth->searchf)(d, obj, type);;) {
        ...
        if (!(d = dt->walk = d->view)) /* move on to next dictionary */
            ...
            obj = (*(d->meth->searchf))(d, NIL(Void_t *), DT_LAST);
    }
}

#if __STD_C
Dt_t *
dtview(reg Dt_t *dt, reg Dt_t *view)
#else
