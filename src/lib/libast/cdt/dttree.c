...
#if __STD_C
static Void_t *
tlist(Dt_t *dt, Dtlink_t *list, int type)
#else
...
static ssize_t
tsize(Dtlink_t *root, ssize_t lev, Dtstat_t *st)
#else
...
static Dtlink_t *
tbalance(Dtlink_t *list, ssize_t size)
#else
...
static Dtlink_t *
troot(Dt_t *dt, Dtlink_t *list, Dtlink_t *link, Void_t *obj, int type)
{
    ...
    if (type & (DT_ATMOST | DT_ATLEAST)) /* find the left-most or right-most
                                            element */
    {
    ...
    last = list;
    list->_left = list->_rght = NIL(Dtlink_t *);
    root = NIL(Dtlink_t *);

    while (!root
           && (t = link->_rght)) /* link->_rght is the left subtree <= obj */
    {
    ...
    while (!root
           && (t = link->_left)) /* link->_left is the right subtree >= obj */
    {
...
#if __STD_C
static Void_t *
dttree(Dt_t *dt, Void_t *obj, int type)
#else
...
#endif
{
    ...
    l = r
    = &link; /* link._rght(_left) will be LEFT(RIGHT) tree after splaying */
    if ((root = tree->root)
        && _DTOBJ(disc, root) != obj) /* splay-search for a matching object */
    {
        while (1) {
            o = _DTOBJ(disc, root);
            k = _DTKEY(disc, o);
    ...
    if (root) {
        if (dt->meth->type
            & DT_OBAG) /* may need to reset root to the right object */
        {
            if ((type & (DT_ATLEAST | DT_ATMOST))
                || ((type & (DT_NEXT | DT_PREV | DT_REMOVE))
                    && _DTOBJ(disc, root) != obj))
                root = troot(dt, root, &link, obj, type);
        }

        if (type
            & (DT_START | DT_SEARCH | DT_MATCH | DT_ATMOST | DT_ATLEAST)) {
        ...
        } else if (type & DT_INSTALL) { /* remove old object before insert new
                                           one */
            o = _DTOBJ(disc, root);
            _dtfree(dt, root, DT_DELETE);
