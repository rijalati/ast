...
#    endif
    ...
    if (F_DUPFD_CLOEXEC > f_local)
        f_local = F_DUPFD_CLOEXEC;
#else
#    define NEED_F 1
#endif
#ifdef F_GETFD
...
#endif
#ifdef F_GETLEASE
    if (F_GETLEASE > f_local)
        f_local = F_GETLEASE;
#endif
#ifdef F_GETLK64
    if (F_GETLK64 > f_local)
        f_local = F_GETLK64;
#endif
#ifdef F_GETOWN
    if (F_GETOWN > f_local)
...
#endif
#ifdef F_SETLEASE
    if (F_SETLEASE > f_local)
        f_local = F_SETLEASE;
#endif
#ifdef F_SETLK64
    if (F_SETLK64 > f_local)
        f_local = F_SETLK64;
#endif
#ifdef F_SETLKW64
    if (F_SETLKW64 > f_local)
        f_local = F_SETLKW64;
#endif
#ifdef F_SETOWN
    if (F_SETOWN > f_local)
...
#endif
#ifdef O_BLKSEEK
    if (O_BLKSEEK > o_local)
        o_local = O_BLKSEEK;
#endif
#ifdef O_LARGEFILE
    if (O_LARGEFILE > o_local)
        o_local = O_LARGEFILE;
#endif
#ifdef O_LARGEFILE128
    if (O_LARGEFILE128 > o_local)
        o_local = O_LARGEFILE128;
#endif
#ifdef O_NOATIME
    if (O_NOATIME > o_local)
        o_local = O_NOATIME;
#endif
#ifdef O_NOFOLLOW
    if (O_NOFOLLOW > o_local)
        o_local = O_NOFOLLOW;
#endif
#ifdef O_NOLINKS
    if (O_NOLINKS > o_local)
        o_local = O_NOLINKS;
#endif
#ifdef O_PRIV
    if (O_PRIV > o_local)
...
#endif
#ifdef O_TEMPORARY
    if (O_TEMPORARY > o_local)
        o_local = O_TEMPORARY;
#endif
#ifdef O_TMPFILE
    if (O_TMPFILE > o_local)
        o_local = O_TMPFILE;
#endif
#ifdef O_XATTR
    if (O_XATTR > o_local)
...
#    endif
    ...
    if (O_TTY_INIT > o_local)
         ...
         * in the ast headers if they are implemented by the kernel, even
         * absent native definitions. If you touch this code make sure you get
         * it right.
         *
                ...
                for (i = 0; i < ( int ) (sizeof(o_directory_native)
                    ...
                    if (!close(open(tmp, O_DIRECTORY_native))) {
                        ...
                        if (!close(
                            ...
                            && close(
                               open(tmp, o_directory_native[i] | O_RDONLY))))
                            {
                                o_directory_origin
                                = "kernel bits otherwise undefined";
...
#    endif
    ...
    printf("#define _ast_O_LOCAL		%012o	/* %s up to %012o%s */\n",
           ...
           ORIGIN_EXTENSION,
           o_local,
           o_local_use <= o_local_min
           ? " *** encroaches on native O_* flags ***"
           : "");
    printf("\n");
...
|| !defined(AT_REMOVEDIR) || !defined(AT_SYMLINK_FOLLOW)                     \
|| !defined(AT_EACCESS)
    printf("\n");
#    ifndef AT_FDCWD
