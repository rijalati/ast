...
main()
{
    ...
    if (bits2 == (align2 - 1))
        printf(
        "#define ALIGN_ROUND(x,y)	"
        "ALIGN_POINTER(ALIGN_INTEGER((x)+(y)-1)&~((y)-1))\n");
    else
        printf(
        "#define ALIGN_ROUND(x,y)	"
        "ALIGN_POINTER(ALIGN_INTEGER(ALIGN_ALIGN(x)+(((y)+%d)/"
        "%d)-1)&~((((y)+%d)/%d)-1))\n",
        align0,
        align0,
        align0,
    ...
    if (align1 == align2) {
        ...
        printf("#define ALIGN_ALIGN1(x)		ALIGN_TRUNC1((x)+%d)\n",
               align1 - 1);
        printf(
        "#define ALIGN_TRUNC1(x)		"
        "ALIGN_POINTER(ALIGN_INTEGER((x)+%d)&0x%lx)\n",
        align1 - 1,
        ~(bits0 | bits1));
    }
#if _X86_ || _X64_
    printf("#if _X64_\n");
    printf(
    "#define ALIGN_CLRBIT1(x)	"
    "ALIGN_POINTER(ALIGN_INTEGER(x)&0xfffffffffffffffeULL)\n");
    printf("#else\n");
    printf(
    "#define ALIGN_CLRBIT1(x)	"
    "ALIGN_POINTER(ALIGN_INTEGER(x)&0xfffffffe)\n");
    printf("#endif\n");
#else
    printf(
    ...
    printf(
    "#define ALIGN_TRUNC2(x)		"
    "ALIGN_POINTER(ALIGN_INTEGER(x)&0xfffffff8)\n");
    printf("#endif\n");
#else
    printf("#define ALIGN_BOUND2		%d\n", align2);
    printf("#define ALIGN_ALIGN2(x)		ALIGN_TRUNC2((x)+%d)\n", align2 - 1);
    printf(
    "#define ALIGN_TRUNC2(x)		ALIGN_POINTER(ALIGN_INTEGER(x)&0x%lx)\n",
    ~(bits0 | bits1 | bits2));
#endif
    printf(
