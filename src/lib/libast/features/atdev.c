...
#    if !_lib_faccessat
    ...
    printf(
    "#define _fd_dir_access	%d	/* access(%s/old) %s */\n", r, fmt, res[r]);
#    endif
#    if !_lib_fchmodat
...
#        if _lib_lchmod && _lib_symlink
    ...
    printf(
    "#define _fd_dir_chmod	%d	/* lchmod(%s/old) %s */\n", r, fmt, res[r]);
#        endif
#    endif
#    if !_lib_fchownat
...
#        if _lib_lchown && _lib_symlink
    ...
    printf(
    "#define _fd_dir_lchown	%d	/* lchown(%s/old) %s */\n", r, fmt, res[r]);
#        endif
#    endif
#    if !_lib_fstatat
    r = stat(dev_old, &st) >= 0;
    printf(
    "#define _fd_dir_stat	%d	/* stat(%s/old) %s */\n", r, fmt, res[r]);
#    endif
#    if !_lib_linkat
...
#    if !_lib_mkdirat
    if (r = mkdir(dev_new, S_IRUSR | S_IWUSR | S_IXUSR) >= 0)
        rmdir(dir_new);
    printf(
    "#define _fd_dir_mkdir	%d	/* mkdir(%s/new) %s */\n", r, fmt, res[r]);
#    endif
#    if !_lib_mknodat
#        if _lib_mknod
...
#        endif
    printf(
    "#define _fd_dir_mknod	%d	/* mknod(%s/new) %s */\n", r, fmt, res[r]);
#    endif
#    if !_lib_mkfifoat
#        if _lib_mkfifo
...
#        endif
    printf(
    "#define _fd_dir_mkfifo	%d	/* mkfifo(%s/new) %s */\n", r, fmt, res[r]);
#    endif
#    if !_lib_openat
    if (r = (fd = open(dev_old, O_RDONLY)) >= 0)
        close(fd);
    printf(
    "#define _fd_dir_open	%d	/* open(%s/old) %s */\n", r, fmt, res[r]);
#    endif
#    if !_lib_readlinkat
...
#    if !_lib_unlinkat
    ...
    printf(
    "#define _fd_dir_unlink	%d	/* unlink(%s/old) %s */\n", r, fmt, res[r]);
#    endif
#endif
    return 0;
}
