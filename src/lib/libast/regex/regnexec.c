...
static const char *rexnames[] = { "REX_NULL",
                                  "REX_ALT",
                                  "REX_ALT_CATCH",
                                  "REX_BACK",
                                  "REX_BEG",
...
#define LEADING(e, r, s)                                                     \
    (*(s) == (e)->leading && ((s) == (e)->beg || *(( s )-1) == (r)->explicit))
...
stkpush(Stk_t *sp, size_t size)
{
    ...
    if (!(f = ( Stk_frame_t * )stkalloc(
          sp, sizeof(Stk_frame_t) + sizeof(Stk_frame_t *) + size - 1)))
        return 0;
    f->pos = p;
    stkframe(sp) = f;
    return f->data;
...
better(Env_t *env, Pos_t *os, Pos_t *ns, Pos_t *oend, Pos_t *nend, int level)
{
    ...
    for (;;)
    {
        ...
        {
            sfprintf(
            sfstdout, "   %-*.*sold ", (level + 3) * 4, (level + 3) * 4, "");
            for (oe = os; oe < oend; oe++)
                sfprintf(
                sfstdout, "<%d,%d,%d>", oe->p - env->beg, oe->serial, oe->be);
            sfprintf(
            sfstdout, "\n   %-*.*snew ", (level + 3) * 4, (level + 3) * 4, "");
            for (oe = ns; oe < nend; oe++)
                sfprintf(
                sfstdout, "<%d,%d,%d>", oe->p - env->beg, oe->serial, oe->be);
            sfprintf(sfstdout, "\n");
        },
...
_better(Env_t *env, Pos_t *os, Pos_t *ns, Pos_t *oend, Pos_t *nend, int level)
{
    ...
    DEBUG_CODE(0x0040,
               {
                   sfprintf(sfstdout, "AHA better old ");
                   for (i = 0; i <= env->nsub; i++)
                       showmatch(&env->best[i]);
                   sfprintf(sfstdout, "\n           new ");
                   for (i = 0; i <= env->nsub; i++)
                       showmatch(&env->match[i]);
                   sfprintf(sfstdout, "\n");
               },
               { 0; });
...
#define follow(e, r, c, s)                                                   \
    ((r)->next ? parse(e, (r)->next, c, s) : (c) ? parse(e, c, 0, s) : BEST)

static int
parse(Env_t *, Rex_t *, Rex_t *, unsigned char *);
...
parserep(Env_t *env, Rex_t *rex, Rex_t *cont, unsigned char *s, int n)
{
    ...
    if (n < rex->hi)
    {
        ...
        if (env->stack)
        {
            ...
            0x0004,
            (sfprintf(sfstdout,
                      "AHA#%04d 0x%04x PUSH %d   (%z,%z)(%z,%z)(%z,%z) "
                      "(%z,%z)(%z,%z)(%z,%z)\n",
                      __LINE__,
                      debug_flag,
                      rex->re.group.number,
        ...
        if (env->stack)
        {
            ...
            0x0004,
            (sfprintf(sfstdout,
                      "AHA#%04d 0x%04x POP  %d %d (%z,%z)(%z,%z)(%z,%z) "
                      "(%z,%z)(%z,%z)(%z,%z)\n",
                      __LINE__,
                      debug_flag,
                      rex->re.group.number,
...
parse(Env_t *env, Rex_t *rex, Rex_t *cont, unsigned char *s)
{
    ...
    for (;;)
    {
        ...
        case REX_ALT:
            if (env->stack)
            {
                ...
                if (r < BEST || (rex->flags & REG_MINIMAL))
                {
                    ...
                    n = parse(
                    env, rex->re.group.expr.binary.right, &catcher, s);
                    if (n != NONE)
                        r = n;
                }
                pospop(env);
        ...
        case REX_BACK:
            ...
            break;
        case REX_BEG:
            if ((!(rex->flags & REG_NEWLINE) || s <= env->beg
                 || *(s - 1) != '\n')
        ...
        case REX_COLL_CLASS:
            ...
            else
            {
                ...
                while (i++ <= n)
                {
                    ...
                    if (s >= e || !collmatch(env, rex, s, e, &s))
                        break;
                }
            }
            return r;
        ...
        case REX_DONE:
            ...
            0x0100,
            (sfprintf(sfstdout,
                      "AHA#%04d 0x%04x %s (%z,%z)(%z,%z)(%z,%z)(%z,%z) "
                      "(%z,%z)(%z,%z)\n",
                      __LINE__,
                      debug_flag,
                      rexname(rex),
        ...
        case REX_GROUP_CATCH:
            DEBUG_TEST(0x0200,
                       (sfprintf(sfstdout,
                                 ...
                                 debug_flag,
                                 rexname(rex),
                                 rexname(rex->re.group_catch.cont),
                                 env->end - s,
                                 s)),
                       (0));
            if (env->stack)
        ...
        case REX_GROUP_AHEAD_CATCH:
            return follow(
            env, rex, rex->re.rep_catch.cont, rex->re.rep_catch.beg);
        ...
        case REX_GROUP_BEHIND_CATCH:
            ...
            return follow(
            env, rex, rex->re.behind_catch.cont, rex->re.behind_catch.beg);
                    ...
                    else
                    {
                        ...
                        while (i++ <= n)
                        {
                            ...
                            if (s >= e)
                                break;
                            mbinit(&env->s);
                            if (towupper(mbchar(&w, s, MB_LEN_MAX, &env->s))
                                != c)
                                break;
                        }
                    }
        ...
        case REX_REP_CATCH:
                ...
                && rex->re.rep_catch.n > rex->re.rep_catch.ref->lo)
            {
                ...
                __LINE__,
                rex->re.rep_catch.ref->re.group.expr.rex,
                rex->re.rep_catch.ref->re.group.expr.rex->re.group.back,
                rexname(rex->re.rep_catch.ref->re.group.expr.rex))),
                (0));
                if (!env->stack
                    || s != rex->re.rep_catch.ref->re.rep_catch.beg
                       && !rex->re.rep_catch.ref->re.group.expr.rex->re.group
                           .back)
                    r = NONE;
...
int
regnexec_20120528(const regex_t *p,
                  const char *s,
                  size_t len,
                  size_t nmatch,
                  regmatch_t *match,
                  regflags_t flags)
{
    ...
    if (len < env->min)
    {
        ...
        0x0080,
        (sfprintf(
        sfstdout, "AHA#%04d REG_NOMATCH %d %d\n", __LINE__, len, env->min)),
        (0));
        return REG_NOMATCH;
    }
    env->regex = p;
    ...
    if ((e = env->rex)->type == REX_BM)
    {
        ...
        else if (!(flags & REG_LEFT))
        {
            ...
            DEBUG_TEST(0x0080,
                       (sfprintf(sfstdout,
                                 "AHA#%04d REX_BM len=%d right=%d left=%d "
                                 "size=%d %d %d\n",
                                 __LINE__,
                                 len,
                                 e->re.bm.right,
