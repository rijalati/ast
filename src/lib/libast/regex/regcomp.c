...
#    define DEBUG_INIT()                                                     \
        ...
        {                                                                    \
            ...
            {                                                                \
                debug = 0x80000000;                                          \
                if (t = getenv("_AST_regex_comp_debug"))                     \
...
#if _BLD_DEBUG
#    define node(a, b, c, d, e)                                              \
        node(a, b, c, d, e, const char *file, unsigned int line)
#endif
...
stats(Cenv_t *env, Rex_t *e)
{
    ...
    do
    {
        ...
        case REX_ALT:
            ...
            env->stats.n = 0;
            if (e->re.group.expr.binary.right
                && stats(env, e->re.group.expr.binary.right))
                return 1;
            if (env->stats.m > m)
                env->stats.m = m;
            else
        ...
        case REX_GROUP_COND:
            ...
            if (e->re.group.size > 0 && ++env->stats.b <= 0)
                return 1;
            if (e->re.group.expr.binary.left
                && stats(env, e->re.group.expr.binary.left))
                return 1;
            if (q = e->re.group.expr.binary.right)
            {
                if (q->re.group.expr.binary.left
                    && stats(env, q->re.group.expr.binary.left))
                    return 1;
                if (q->re.group.expr.binary.right
                    && stats(env, q->re.group.expr.binary.right))
                    return 1;
            }
            env->stats.m = m;
            env->stats.n = n;
        ...
        case REX_STRING:
            if (!e->map)
            {
                ...
                if ((env->stats.n += e->re.string.size) < cn)
                    return 1;
                if (!env->stats.x
                    || env->stats.x->re.string.size < e->re.string.size)
                {
                    env->stats.x = e;
                    env->stats.l = cm;
                }
...
magic(Cenv_t *env, int c, int escaped)
{
    ...
    if (mp = state.magic[c])
    {
        ...
        if (c >= T_META)
        {
            ...
            if (env->type >= SRE)
            {
                ...
                else if (env->type == KRE && !env->parnest
                         && (env->flags & REG_SHELL_GROUP))
                {
                    if (c == T_AND)
                        c = '&';
                    else if (c == T_BAR)
    ...
    else if (escaped && !(env->flags & (REG_LENIENT | REG_REGEXP))
             && c != ']')
    {
        env->error = REG_BADESC;
        goto bad;
...
token(Cenv_t *env)
{
    ...
    {
        if (env->type == BRE
            && (env->cursor == env->pattern || posixkludge == 1))
        {
            env->posixkludge = 2;
            return T_CFLX;
        }
...
bra(Cenv_t *env)
{
    ...
    if (*env->cursor == 0 || *(env->cursor + 1) == 0
    ...
    if (complicated && mbcoll())
    {
        ...
        if (!(dt = ( Dt_t * )LCINFO(AST_LC_COLLATE)->data))
        {
            ...
            if ((cc = newof(0, Cchr_t, elementsof(primary), 0))
        ...
        if (dt)
        {
            ...
            if (ic = env->flags & REG_ICASE)
                elements *= 2;
            if (!(e = node(
                  env, REX_COLL_CLASS, 1, 1, (elements + 3) * sizeof(Celt_t))))
                return 0;
            ce = ( Celt_t * )e->re.data;
            ...
            for (;;)
            {
                ...
                else if (c == '[')
                {
                    ...
                    case '=':
                        ...
                        rp = env->cursor + 1;
                        if ((rw = regcollate(( char * )env->cursor,
                                             ( char ** )&env->cursor,
                                             ( char * )pp,
                                             COLL_KEY_MAX,
                                             &wc))
                    ...
                    case '.':
                        if (env->flags & REG_REGEXP)
                            goto complicated_normal;
                        pp = ( unsigned char * )cb[inrange];
                        if ((w = regcollate(( char * )env->cursor,
                                            ( char ** )&env->cursor,
                                            ( char * )pp,
                                            COLL_KEY_MAX,
                                            NiL))
...
rep(Cenv_t *env, Rex_t *e, int number, int last)
{
    ...
    if (m <= n && n)
    {
        for (; e && e->type >= REX_GROUP && e->type <= REX_GROUP_CUT;
...
insert(Cenv_t *env, Rex_t *f, Rex_t *g)
{
    ...
    if (!(t = g->re.trie.root[*s])
        && !(t = g->re.trie.root[*s] = trienode(env, *s)))
        return 1;
    for (len = 1;;)
    {
        if (t->c == *s)
...
trie(Cenv_t *env, Rex_t *e, Rex_t *f)
{
    ...
    {
        if (!(g = node(
              env, REX_TRIE, 0, 0, (UCHAR_MAX + 1) * sizeof(Trie_node_t *))))
            return 0;
...
grp(Cenv_t *env, int parno)
{
        ...
        for (;;)
        {
            ...
            case 'm':
                if (i)
                    ...
                    env->flags &= ~REG_NEWLINE;
                env->explicit
                = (env->flags & (REG_NEWLINE | REG_SPAN)) == REG_NEWLINE
                  ? env->mappednewline
                  : -1;
                break;
            case 'p':
            ...
            case 'X':
                if (typ >= 0
                    || env->type == ERE && (env->flags & REG_CLASS_ESCAPE))
                    break; /* PCRE_EXTRA */
                /*FALLTHROUGH*/
            case 'A':
                env->flags
                &= ~(REG_AUGMENTED | REG_EXTENDED | REG_LITERAL | REG_REGEXP
            ...
            case 'K':
                ...
                &= ~(REG_AUGMENTED | REG_EXTENDED | REG_LITERAL | REG_REGEXP
                     | REG_SHELL | REG_LEFT | REG_RIGHT);
                env->flags
                |= REG_AUGMENTED | REG_SHELL | REG_LEFT | REG_RIGHT;
                typ = KRE;
                break;
            case 'M':
    ...
    case T_PERCENT:
        e
        = node(env, REX_NEST, 0, 0, (UCHAR_MAX + 1) * sizeof(unsigned short));
        ...
        for (;;)
        {
            ...
            nesting:
                if (isalnum(i)
                    || (e->re.nest.type[i]
                        & (REX_NEST_close | REX_NEST_escape | REX_NEST_literal
                           | REX_NEST_quote | REX_NEST_delimiter
                           | REX_NEST_separator | REX_NEST_terminator)))
                    goto invalid;
                e->re.nest.type[i] = REX_NEST_open;
                if ((x = chr(env, &esc)) < 0
                    || (e->re.nest.type[x]
                        & (REX_NEST_close | REX_NEST_escape
    ...
    case '{':
        ...
        if (!env->disc->re_compf)
            ...
            e->re.exec.data = (*env->disc->re_compf)(
            env->regex, e->re.exec.text, e->re.exec.size, env->disc);
        return e;
    case '0':
    case '1':
    case '2':
    ...
    env->parnest--;
    if (c != T_CLOSE
        && (c != ')' || !(env->flags & (REG_LITERAL | REG_SHELL))))
    {
        env->error = REG_EPAREN;
        goto nope;
    }
...
seq(Cenv_t *env)
{
    ...
    {
        s = buf;
        while ((c = token(env)) < T_META
               && s < &buf[sizeof(buf) - env->token.len])
        {
            x = c;
            p = env->cursor;
            if (c >= 0)
            ...
            {
                c = (c == C_ESC) ? env->token.lex
                                 : mbtchar(&w, p, MB_LEN_MAX, &env->q);
                if (env->flags & REG_ICASE)
                    c = towupper(c);
        ...
        if (s > buf)
            ...
            case T_BANG:
                ...
                else
                {
                    ...
                    if (!(e = node(env, REX_STRING, 0, 0, i)))
                        return 0;
                    memcpy(( char * )(e->re.string.base
                                      = ( unsigned char * )e->re.data),
                           ( char * )buf,
                           i);
                    e->re.string.size = i;
                }
                ...
                else
                {
                    if (!(f = node(env, REX_STRING, 0, 0, n)))
                        return 0;
                    memcpy(( char * )(f->re.string.base
                                      = ( unsigned char * )f->re.data),
                           ( char * )p,
                           n);
                    f->re.string.size = n;
                }
            ...
            default:
                ...
                if (!(e = node(env, REX_STRING, 0, 0, c)))
                    return 0;
                memcpy(
                ( char * )(e->re.string.base = ( unsigned char * )e->re.data),
                ( char * )buf,
                c);
                e->re.string.size = c;
        ...
        else if (c > T_BACK)
        {
            ...
            case T_OPEN:
                    ...
                    && !(env->flags & (REG_NULL | REG_REGEXP)))
                {
                    drop(env->disc, e);
                    env->error
                    = (*env->cursor == 0 || *env->cursor == env->delimiter
                       || *env->cursor == env->terminator)
                      ? REG_EPAREN
                      : REG_ENULL;
                    return 0;
...
con(Cenv_t *env)
{
    ...
    if (!(e = seq(env)) || !(env->flags & REG_AUGMENTED)
        || token(env) != T_AND)
        return e;
    eat(env);
    if (!(f = con(env)))
...
bmstr(Cenv_t *env, Rex_t *a, unsigned char *v, int n, Bm_mask_t b)
{
    ...
    for (m = 0; m < n; m++)
    {
        ...
        c = v[m];
        a->re.bm.mask[m][c] |= b;
        if (z == HIT || !a->re.bm.skip[c]
            || a->re.bm.skip[c] > z && a->re.bm.skip[c] < HIT)
            a->re.bm.skip[c] = z;
...
special(Cenv_t *env, regex_t *p)
{
    ...
    if (e = p->env->rex)
    {
        ...
        if (x || t)
        {
            ...
            a->re.bm.size = n;
            a->re.bm.back
            = (y == e || y == e->re.group.expr.rex) ? (m - n) : -1;
            a->re.bm.left = l - 1;
            a->re.bm.right = env->stats.m - l - n;
            a->re.bm.complete
            = (env->stats.e
               || y != e
                  && (e->type != REX_GROUP || y != e->re.group.expr.rex)
            ...
            = (y->next && y->next->type == REX_END) ? HIT
                                                    : (n + a->re.bm.left);
            for (i = 1; i <= n; i++)
            ...
            {
                DEBUG_TEST(
                0x0010, (sfprintf(sfstderr, "BM#0: k=%d j=%d\n", k, j)), (0));
                for (q[j] = k; k <= n; k = q[k])
                {
                    for (m = 0; m <= UCHAR_MAX; m++)
            ...
            if (DEBUG_TEST(0x0020, (1), (0)))
            {
                ...
                a->re.bm.right,
                y->type,
                y->next ? y->next->type : 0,
                e->type,
                e->next ? e->next->type : 0);
                for (m = 0; m < n; m++)
        ...
        case REX_STRING:
            ...
            a->map = e->map;
            f = a->re.string.fail;
            memcpy(( char * )(a->re.string.base = ( unsigned char * )&f[n]),
                   ( char * )s,
                   n);
            s = a->re.string.base;
            a->re.string.size = n;
...
again:
    ...
    if (disc->re_version >= REG_VERSION_MAP && disc->re_map)
    {
        ...
        env.MAP = fold;
    env.type = (env.flags & REG_AUGMENTED)
               ? ARE
               : (env.flags & REG_EXTENDED) ? ERE : BRE;
    env.explicit = -1;
    if (env.flags & REG_SHELL)
    {
        ...
        env.flags &= ~(REG_SHELL_DOT | REG_SHELL_ESCAPED | REG_SHELL_GROUP
                       | REG_SHELL_PATH);
    ...
    if (env.stats.b)
        p->env->hard = p->env->separate = 1;
             ...
             && (env.stats.a || env.stats.c > 1 && env.stats.c != env.stats.s
                 || env.stats.t
                    && (env.stats.t > 1 || env.stats.a || env.stats.c)))
        p->env->hard = 1;
...
regncomp(regex_t *p, const char *pattern, size_t size, regflags_t flags)
{
    ...
    if (!(s = malloc(size + 1)))
        return fatal((flags & REG_DISCIPLINE) ? p->re_disc : &state.disc,
                     REG_ESPACE,
                     pattern);
...
regcomb(regex_t *p, regex_t *q)
{
    ...
    for (h = f; h->next; h = h->next)
        ;
    if (g->next && g->next->type == REX_END && h->next
        && h->next->type == REX_END)
    {
