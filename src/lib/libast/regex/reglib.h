...
#    define MBSIZE(e, p)                                                     \
        (((e)->i = mbtsize(p, MB_LEN_MAX, &(e)->q)) > 0 ? (e)->i : 1)
...
#    define REX_GROUP_BEHIND_NOT_CATCH                                       \
        27                          /* REX_GROUP_BEHIND_NOT catcher          \
                                     */
...
extern void *
alloc(regdisc_t *, void *, size_t);
extern regclass_t
classfun(int);
extern void
drop(regdisc_t *, Rex_t *);
extern int
fatal(regdisc_t *, int, const char *);
