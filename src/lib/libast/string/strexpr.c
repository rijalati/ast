...
expr(Expr_t *ex, int precedence)
{
    ...
    for (;;)
    {
        ...
        case '!':
            if (peekchr(ex) != '=')
                error(ex, "operator syntax error");
            if (precedence > 7)
                goto done;
            getchr(ex);
            x = expr(ex, 8);
        ...
        default:
            ...
            pos = --ex->nextchr;
            if (isdigit(c))
                n = strton(ex->nextchr, &ex->nextchr, NiL, 0);
            else if (ex->convert)
