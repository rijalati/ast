             ...
             int flags)
{
    ...
    if (regnexec(re,
                 ...
                 matchstate.match,
                 reflags
                 & ~(REG_MINIMAL | REG_SHELL_GROUP | REG_LEFT | REG_RIGHT
                     | REG_ICASE)))
        return 0;
    if (!sub || n <= 0)
        return 1;
...
int
strgrpmatch_20120528(const char *b,
                     const char *p,
                     ssize_t *sub,
                     int n,
                     int flags)
...
{
    return strngrpmatch(
    s, strlen(s), p, NiL, 0, STR_MAXIMAL | STR_LEFT | STR_RIGHT);
}
