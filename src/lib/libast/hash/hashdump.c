...
dumpflags(int flags)
{
    if (flags & HASH_ALLOCATE)
        sfprintf(sfstderr, "allocate ");
    if (flags & HASH_BUCKET)
        sfprintf(sfstderr, "bucket ");
    if (flags & HASH_FIXED)
        sfprintf(sfstderr, "fixed ");
    if (flags & HASH_HASHED)
        sfprintf(sfstderr, "hashed ");
    if (flags & HASH_RESIZE)
        sfprintf(sfstderr, "resize ");
    if (flags & HASH_STATIC)
        sfprintf(sfstderr, "static ");
    if (flags & HASH_VALUE)
        sfprintf(sfstderr, "value ");
}
            ...
            for (b = *sp; b; b = b->next)
                    ...
                    && (!(tab->flags & HASH_VALUE) || b->value))
                {
                    ...
                    if (b->hash & HASH_FLAGS)
                    {
                        ...
                        if (b->hash & HASH_HIDDEN)
                            sfprintf(sfstderr, "hidden|");
                        if (b->hash & HASH_KEEP)
                            sfprintf(sfstderr, "keep|");
                        if (b->hash & HASH_OPAQUED)
...
dumptable(Hash_table_t *tab, int flags)
{
    ...
    sfprintf(
    sfstderr, "        bucketsize:  %d\n", tab->bucketsize * sizeof(char *));
    sfprintf(sfstderr, "\n");
    if ((flags | tab->flags) & HASH_BUCKET)
        dumpbucket(tab, flags);
}
...
hashdump(Hash_table_t *tab, int flags)
{
    ...
    if (tab)
        dumproot(tab->root, flags);
    else
        for (root = hash_info.list; root; root = root->next)
            dumproot(root, flags);
    sfsync(sfstderr);
}
