...
#        define DEBUG_ASSERT(p)                                              \
            ((p) ? 0 : (debug_fatal(__FILE__, __LINE__, #p), 0))
...
debug_indent(int);
extern void
debug_fatal(const char *, int, const char *);
#    if 0 /* this prototype in <ast.h> for convenience */
extern ssize_t		debug_printf(int, const char*, ...);
#    endif
extern ssize_t
debug_sprintf(char *, size_t, const char *, ...);
extern ssize_t
debug_vsprintf(char *, size_t, const char *, va_list);
extern ssize_t
debug_vprintf(int, const char *, va_list);
extern void
systrace(const char *);
