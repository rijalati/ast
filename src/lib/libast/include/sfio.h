...
#    define __sf_putm(f, v, m)                                               \
        (_sfputm(_SF_(f), (Sfulong_t)(v), (Sfulong_t)(m)))
...
#    define __sf_maxr(n, s)                                                  \
        ((s) ? ((_Sfi = _Sfmaxr), (_Sfmaxr = (n)), _Sfi) : _Sfmaxr)
...
__INLINE__ int
sfputl(Sfio_t *f, Sflong_t v)
{
    return __sf_putl(f, v);
}
__INLINE__ int
sfputu(Sfio_t *f, Sfulong_t v)
{
    return __sf_putu(f, v);
}
__INLINE__ int
sfputm(Sfio_t *f, Sfulong_t v, Sfulong_t m)
{
...
__INLINE__ ssize_t
sfmaxr(ssize_t n, int s)
{
    return __sf_maxr(n, s);
}
...
#        define sfstropen()                                                  \
            sfnew(0, 0, -1, -1, SF_READ | SF_WRITE | SF_STRING)
