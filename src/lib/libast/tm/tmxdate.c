...
#define FLAGS(f)                                                             \
    (f & EXACT) ? "|EXACT" : "", (f & LAST) ? "|LAST" : "",                  \
    (f & THIS) ? "|THIS" : "", (f & NEXT) ? "|NEXT" : "",                    \
    (f & ORDINAL) ? "|ORDINAL" : "", (f & FINAL) ? "|FINAL" : "",            \
    (f & WORK) ? "|WORK" : ""
/*
...
again:
    for (;;)
    {
        ...
        for (;;)
        {
            if (*s == '.' || *s == '-' || *s == '+')
            {
                if (((set | state) & (MONTH | HOUR | MINUTE | ZONE))
                    == (MONTH | HOUR | MINUTE)
                    && (i = tmgoff(s, &t, TM_LOCALZONE)) != TM_LOCALZONE)
                {
                    zone = i;
                    state |= ZONE;
                    if (!*(s = t))
                    ...
                    for (;;)
                    {
                        ...
                        if (flags & (MDAY | WDAY))
                        {
                            ...
                            if (++j > 28)
                            {
                                ...
                                tm->tm_mday = j;
                                tm
                                = tmxtm(tm, tmxtime(tm, zone), tm->tm_zone);
                                i = tm->tm_mon + 1;
                                j = tm->tm_mday;
                                k = tm->tm_wday;
                            }
        ...
        if (isdigit(*s))
        {
            ...
            if ((*t == 'T' || *t == 't')
                && ((set | state) & (YEAR | MONTH | DAY)) == (YEAR | MONTH)
                && isdigit(*(t + 1)))
                t++;
            u = t + (*t == '-');
            message((-1,
                     "AHA#%d n=%d w=%d u='%c' f=%d t=\"%s\"",
                     __LINE__,
                     n,
                     w,
                     *u,
            ...
            else if (w == 6 || w == 8 && (n / 1000000) > 12)
            {
                ...
                goto save;
                     ...
                     && tmlex(t,
                              &t,
                              tm_info.format + TM_ORDINAL,
                              TM_ORDINALS - TM_ORDINAL,
                              NiL,
                              0)
                        >= 0)
            {
            ...
            else
            {
                ...
                if ((j = tmlex(u,
                               ...
                               tm_info.format,
                               TM_NFORM,
                               tm_info.format + TM_SUFFIXES,
                               TM_PARTS - TM_SUFFIXES))
                    >= 0
                ...
                if (tm_data.lex[j] == TM_PARTS || n < 1000)
                {
                    ...
                    case TM_DAYS:
                        ...
                        while (isspace(*s))
                            s++;
                        message(
                        (-1, "AHA#%d disambiguate LAST s='%s'", __LINE__, s));
                        if ((k = tmlex(s,
                                       &t,
                                       tm_info.format + TM_NEXT,
                                       TM_EXACT - TM_NEXT,
                                       NiL,
                                       0))
                            >= 0
                               ...
                               >= 0)
                        {
                            ...
                            if (state & LAST)
                            {
                                ...
                                set |= FINAL;
                                message(
                                (-1, "AHA#%d LAST => FINAL", __LINE__));
                            }
                            else
                                state &= ~(THIS | NEXT);
                        }
                        message(
                        (-1, "AHA#%d disambiguate LAST k=%d", __LINE__, k));
                        if (state & LAST)
                            n = -n;
                        else if (!(state & NEXT))
                            n--;
                        m = (f > 0) ? f * n : n;
                        message((-1,
                                 "AHA#%d f=%d n=%d i=%d j=%d k=%d l=%d m=%d "
                                 "state=" FFMT,
                                 __LINE__,
                                 f,
                                 n,
                        ...
                        j -= tm->tm_wday;
                        message((-1,
                                 "AHA#%d mday=%d wday=%d day=%d dir=%d f=%d "
                                 "i=%d j=%d l=%d m=%d",
                                 __LINE__,
                                 tm->tm_mday,
                                 tm->tm_wday,
                        ...
                        message(
                        (-1,
                         "AHA#%d day=%d mday=%d f=%d m=%d j=%d state=" FFMT,
                         __LINE__,
                         day,
                         tm->tm_mday,
                         f,
        ...
        else if (*s == '/')
        {
            ...
            else
            {
                ...
                {
                    if ((i = tmlex(
                         s, &t, tm_info.format, TM_DAY_ABBREV, NiL, 0))
                        < 0)
                        break;
                    if (i >= TM_MONTH)
                        i -= TM_MONTH;
    ...
    {
        tmfix(tm);
        tm->tm_mday = tm_data.days[tm->tm_mon]
                      + (tm->tm_mon == 1 && tmisleapyear(tm->tm_year));
    }
    if (state & WORK)
    {
        tm->tm_mday = (set & FINAL)
                      ? (tm_data.days[tm->tm_mon]
                         + (tm->tm_mon == 1 && tmisleapyear(tm->tm_year)))
                      : 1;
        tmfix(tm);
        message((-1,
                 "AHA#%d WORK mday=%d wday=%d",
                 __LINE__,
                 tm->tm_mday,
                 tm->tm_wday));
