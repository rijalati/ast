 ...
 * Tv_t==TV_TOUCH_RETAIN retains path value if it exists, current time
 * otherwise otherwise it is exact time file created if it doesn't exist and
 * (flags&TV_TOUCH_CREATE) symlink not followed if (flags&TV_TOUCH_PHYSICAL)
...
#    endif
    ...
    if (!cv && av == TV_TOUCH_RETAIN && mv == TV_TOUCH_RETAIN
        && !stat(path, &st) && !chmod(path, st.st_mode & S_IPERM))
        return 0;
    ...
    if (errno != ENOSYS)
    {
        ...
        if ((fd = open(path, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, mode))
            < 0)
            return -1;
        close(fd);
        errno = oerrno;
        if ((ts[0].tv_nsec != UTIME_NOW || ts[1].tv_nsec != UTIME_NOW)
            && utimensat(AT_FDCWD,
                         path,
                         ts,
                         (flags & TV_TOUCH_PHYSICAL) ? AT_SYMLINK_NOFOLLOW
                                                     : 0))
            return -1;
        return 0;
    }
