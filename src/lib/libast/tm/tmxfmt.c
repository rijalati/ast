...
#define warped(t, n)                                                         \
    ((t) < (( n ) -tmxsns(6L * 30L * 24L * 60L * 60L, 0))                    \
     || (t) > ((n) + tmxsns(24L * 60L * 60L, 0)))
...
tmxfmt(char *buf, size_t len, const char *format, Time_t t)
{
    ...
    for (;;) {
        ...
        case 'C': /* 2 digit century */
            cp = number(
            cp, ep, ( long ) (1900 + tm->tm_year) / 100, 2, width, pad);
            continue;
        case 'd': /* day of month */
        ...
        case '#':
            ...
            *f++ = '%';
            ...
            f
            += sfsprintf(f, &fmt[sizeof(fmt)] - f, "I%du", sizeof(Tmxsec_t));
            cp += sfsprintf(cp, ep - cp, fmt, tmxsec(now));
            if (parts > 1) {
                n = sfsprintf(
                cp, ep - cp, ".%09I*u", sizeof(Tmxnsec_t), tmxnsec(now));
                if (prec && n >= prec)
                    n = prec + 1;
                cp += n;
            }
        ...
        case 'U': /* week number, Sunday as first day */
            cp
            = number(cp, ep, ( long ) tmweek(tm, 0, -1, -1), 2, width, pad);
            continue;
#if 0
		case 'v':	/* (UNUSED) */
			continue;
        ...
        case 'V': /* ISO week number */
            cp
            = number(cp, ep, ( long ) tmweek(tm, 2, -1, -1), 2, width, pad);
            continue;
        case 'W': /* week number, Monday as first day */
            cp
            = number(cp, ep, ( long ) tmweek(tm, 1, -1, -1), 2, width, pad);
            continue;
        ...
        case 'Z': /* time zone */
            ...
            p = (flags & TM_UTC) ? tm_info.local->standard
                                 : tm->tm_isdst && tm->tm_zone->daylight
                                   ? tm->tm_zone->daylight
                                   : tm->tm_zone->standard;
            goto string;
        ...
        for (;;) {
            ...
            if (c == '+') {
                if (!(flags & n)) {
                    ...
                    tm_info.flags |= n;
                    tm = tmxtm(
                    tm, t, (flags & TM_UTC) ? &tm_data.zone[2] : tm->tm_zone);
                    if (!i)
                        tm_info.flags &= ~n;
                }
            } else if (flags & n) {
