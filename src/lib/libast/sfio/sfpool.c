...
** This is to prevent memory faults in calls such as sfsync(NULL) that walk
*the pool
** link list and during such walks may free up streams&pools. Free pools will
*be
** reused in newpool().
*/
#if __STD_C
static int
delpool(reg Sfpool_t *p)
#else
...
#endif
{
    ...
    f->mode &= ~SF_POOL;
    head->mode |= SF_POOL;
    head->next = head->endr = head->endw
    = head->data; /* clear write buffer */
...
#if __STD_C
static int
_sfpmove(reg Sfio_t *f, reg int type)
#else
...
#if __STD_C
Sfio_t *
sfpool(reg Sfio_t *f, reg Sfio_t *pf, reg int mode)
#else
