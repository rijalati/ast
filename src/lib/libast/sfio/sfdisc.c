...
#if __STD_C
static int
_dccaexcept(Sfio_t *f, int type, Void_t *val, Sfdisc_t *disc)
#else
...
#if __STD_C
static ssize_t
_dccaread(Sfio_t *f, Void_t *buf, size_t size, Sfdisc_t *disc)
#else
...
#endif
{
    ...
    if (!disc && f->disc && f->disc->disc
        && f->disc->disc->readf == _dccaread)
        goto done;
    ...
    if (!(f->flags & SF_STRING))
    {
            ...
            || ((f->mode & SF_READ) && (n = f->endb - f->next) > 0))
        {
            ...
            if (rv == 0 && disc
                && disc->exceptf) /* ask discipline being pushed */
            {
                SFOPEN(f, 0);
