...
#    ifdef sysopenatf
            ...
            while ((fd = sysopenatf(cwd, file, oflags & O_ACCMODE)) < 0
                   && errno == EINTR)
                errno = 0;
#    endif
        if (fd >= 0)
...
#    ifdef sysopenatf
                    ...
                    while ((fd = sysopenatf(cwd, file, oflags & O_ACCMODE))
                           < 0
                           && errno == EINTR)
                        errno = 0;
#    endif
...
#if __STD_C
int
_sftype(reg const char *mode, int *oflagsp, int *fflagsp, int *uflagp)
#else
