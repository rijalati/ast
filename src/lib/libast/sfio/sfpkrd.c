...
#if __STD_C
ssize_t
sfpkrd(int fd, Void_t *argbuf, size_t n, int rc, long tm, int action)
#else
...
#    ifdef __sun
             ...
             * this kludge will have to do until sun^H^H^Horacle fixes
             * I_PEEK/rsh
             */
            ...
            if (stream_peek == 0) /* this will be done just once */
            {
                char *e;
                stream_peek
                = (getenv("LOGNAME") == 0 && getenv("MAIL") == 0
                   && ((e = getenv("LANG")) == 0 || strcmp(e, "C") == 0)
                   && ((e = getenv("PATH")) == 0
                       || strncmp(e, "/usr/bin:", 9) == 0))
                  ? -1
                  : 1;
            }
            if (stream_peek < 0)
