...
#if __STD_C
static char *
_sfsetclass(const char *form, Accept_t *ac, int flags)
#else
...
#    if __STD_C
static int
_sfwaccept(wchar_t wc, Accept_t *ac)
#    else
...
#    if __STD_C
static int
_sfgetwc(Scan_t *sc, wchar_t *wc, int fmt, Accept_t *ac, Void_t *mbs)
#    else
...
#    endif
{
    ...
    if (sc->d > sc->f->data && (n = sc->endd - sc->d) > 0 && n < SFMBMAX) {
        ...
        sc->endd = sc->f->endb;
        if (!mbs)
            sc->f->endb = sc->endd; /* stop cc's "unused mbs" warning */
    }
