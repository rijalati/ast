...
sfsetlinemode(void)
{
    ...
    if (modes < 0) {
        ...
        if ((t = getenv("SFIO_OPTIONS"))
            && t[0]) { /* copy option string to a writable buffer */
...
#endif
{
    ...
    okmmap
    = (buf || (f->flags & SF_STRING) || (f->flags & SF_RDWR) == SF_RDWR) ? 0
                                                                         : 1;
    ...
    if ((init || local)
        && !(f->flags & SF_STRING)) { /* ASSERT(f->file >= 0) */
        st.st_mode = 0;
...
setbuf:
    ...
    if (f->flags & SF_STRING) { /* these fields are used to test actual size -
                                   see sfseek() */
        f->extent
        = (!sf_malloc && ((f->flags & SF_READ) || (f->bits & SF_BOTH))) ? size
                                                                        : 0;
        f->here = 0;
