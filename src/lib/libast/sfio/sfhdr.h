...
#        if !_NO_LARGEFILE64_SOURCE && _typ_off64_t && _lib_lseek64          \
        && _lib_stat64
#            undef _LARGEFILE64_SOURCE
#            undef _LARGEFILE_SOURCE
...
#        endif /*_hdr_wchar && _typ_mbstate_t && _lib_wcrtomb &&             \
                  _lib_mbrtowc*/
...
#            define SFMBDCL(mb)
#            define SFMBLEN(s, mb) mbrtowc(NIL(wchar_t *), (s), SFMBMAX, (mb))
#        endif /*!_has_multibyte && _hdr_wchar && _lib_mbtowc &&             \
                  _lib_wctomb*/
...
#        define SFMTXLOCK(f)                                                 \
            (((f)->flags & SF_MTSAFE) ? sfmutex(f, SFMTX_LOCK) : 0)
#        define SFMTXUNLOCK(f)                                               \
            (((f)->flags & SF_MTSAFE) ? sfmutex(f, SFMTX_UNLOCK) : 0)
...
#    if _lib_vfork && !defined(fork) && !defined(__sparc)                    \
    && !defined(__sparc__)
#        if _hdr_vfork
#            include <vfork.h>
#        endif
...
#        define SFWCGET(f, v)                                                \
            (((v) = (f)->bits & SF_WCFORMAT), ((f)->bits &= ~SF_WCFORMAT))
#    else
#        define SFWCSET(f)
#        define SFWCGET(f, v)
#    endif
                    ...
                    *(thousand) = -1;                                        \
                    if ((lv = localeconv())) {                               \
                        ...
                        if (lv->thousands_sep && *lv->thousands_sep)         \
                            *(thousand)                                      \
                            = *( unsigned char * ) lv->thousands_sep;        \
                    }                                                        \
                }                                                            \
            } while (0)
...
#    define SFFMT_TYPES                                                      \
        (SFFMT_SHORT | SFFMT_SSHORT | SFFMT_LONG | SFFMT_LLONG               \
         | SFFMT_LDOUBLE | SFFMT_IFLAG | SFFMT_JFLAG | SFFMT_TFLAG           \
         | SFFMT_ZFLAG)
...
#        define SFMMSEQON(f, a, s)                                           \
            do {                                                             \
                int oerrno = errno;                                          \
                ( void ) madvise(                                            \
                (caddr_t)(a), (size_t)(s), MADV_SEQUENTIAL);                 \
                errno = oerrno;                                              \
            } while (0)
...
#    define CLOSE(f)                                                         \
        {                                                                    \
            while (sysclosef(f) < 0 && errno == EINTR)                       \
                errno = 0;                                                   \
        }
...
#    define _SFOPENWR(f)                                                     \
        ((f)->endw                                                           \
         = ((f)->flags & (SF_MTSAFE | SF_LINE)) ? (f)->data : (f)->endb)
#    define _SFOPEN(f)                                                       \
        ((f)->mode == SF_READ                                                \
         ? _SFOPENRD(f)                                                      \
...
extern char *
_sfcvt _ARG_(( Void_t *, char *, size_t, int, int *, int *, int *, int ) );
