...
#    if __STD_C
static int
_tmprmfile(Sfio_t *f, int type, Void_t *val, Sfdisc_t *disc)
#    else
static int _tmprmfile(f, type, val, disc) Sfio_t *f;
int type;
Void_t *val;
Sfdisc_t *disc;
#    endif
...
#if __STD_C
static int
_rmtmp(Sfio_t *f, char *file)
#else
static int _rmtmp(f, file) Sfio_t *f;
char *file;
#endif
{
...
#    if __STD_C
char **
_sfgetpath(char *path)
#    else
char **_sfgetpath(path) char *path;
#    endif
{
    reg char *p, **dirs;
    reg int n;
...
#    if _has_oflags
        if ((fd = sysopenf(
             file, O_RDWR | O_CREAT | O_EXCL | O_TEMPORARY, SF_CREATMODE))
            >= 0)
            break;
#    else
...
#if __STD_C
static int
_tmpexcept(Sfio_t *f, int type, Void_t *val, Sfdisc_t *disc)
#else
static int _tmpexcept(f, type, val, disc) Sfio_t *f;
int type;
Void_t *val;
Sfdisc_t *disc;
#endif
{
    ...
    if (type != SF_WRITE && type != SF_SEEK && type != SF_DPUSH
        && type != SF_DPOP && type != SF_DBUFFER)
        return 0;
...
#endif
{
    ...
    f = sfnew(
    NIL(Sfio_t *), NIL(char *), s, -1, SF_STRING | SF_READ | SF_WRITE);
    _Sfnotify = notify;
    if (!f)
        return NIL(Sfio_t *);
