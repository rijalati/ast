...
#define SMnputc(f, c, n)                                                     \
    {                                                                        \
        if ((o = SFNPUTC(f, c, n)) > 0)                                      \
            n_output += 1;                                                   \
        if (o != n) {                                                        \
            SFBUF(f);                                                        \
            goto done;                                                       \
...
loop_fmt:
    ...
    loop_flags: /* LOOP FOR \0, %, FLAGS, WIDTH, PRECISION, BASE, TYPE */
        ...
        case '.':
            dot += 1;
            if (dot
                == 1) { /* so base can be defined without setting precis */
        ...
        dot_set:
            if (dot == 0) {
                if ((width = v) < 0) {
                    width = -width;
                    flags
                    = (flags & ~(SFFMT_CENTER | SFFMT_ZERO)) | SFFMT_LEFT;
                }
            } else if (dot == 1)
                precis = v;
            else if (dot == 2)
        ...
        /* 2012-06-27 I* will be deprecated and POSIX will probably settle on
         * one of L* or z* */
        ...
        case 's':
            ...
            for (sp = *ls;;) { /* v: number of bytes  w: print width of those
                                  v bytes */
#if _has_multibyte
                if (flags & SFFMT_LONG) {
                    v = 0;
...
#    endif
                ...
                if (n < 0 && (flags & SFFMT_CHOP) && width > 0
                    && precis < 0) {
#if _has_multibyte
                    if (flags & SFFMT_LONG) {
                        SFMBCLR(&mbs);
...
#    endif
            ...
            if (flags & SFFMT_ALTER) /* prefix */
            {
                if (fmt == 'o') {
                    ...
                    if (width > 0
                        && (flags & SFFMT_ZERO)) { /* do 0 padding first */
                        if (fmt == 'x' || fmt == 'X')
                            n = 0;
                        else if (dot < 2)
                            n = width;
        ...
        a_format:
            ...
            *--ep = (decpt > 0 || dval == 0.) ? '+' : '-';
            *--ep
            = fmt == 'a' ? 'p' : fmt == 'A' ? 'P' : isupper(fmt) ? 'E' : 'e';
        ...
        else if (!(flags & SFFMT_ZERO)) /* right padding */
        {
            if (flags & SFFMT_LEFT)
                v = -v;
                     ...
                     & SFFMT_PREFIX) /* blank padding, output prefix now */
            {
                *--sp = fmt;
                flags &= ~SFFMT_PREFIX;
            }
    ...
    do_output:
        ...
        if (flags
            & (SFFMT_FLOAT | SFFMT_LEFT)) { /* SFFMT_FLOAT: right padding for
                                               float precision */
            if ((n = precis) > 0)
                SFnputc(f, '0', n);
