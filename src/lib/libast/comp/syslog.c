...
#    if _UWIN
    "/var/log/syslog", 0,
#    endif
    "/dev/log",        0, "var/log/syslog", 0,
    "lib/syslog/log",  0, "/dev/console",   LOG_CONS,
};

const Namval_t log_facility[] = { "default",  0,
                                  "user",     LOG_USER,
                                  "kernel",   LOG_KERN,
                                  "mail",     LOG_MAIL,
                                  "daemon",   LOG_DAEMON,
...
= { "panic", LOG_EMERG, "alert",   LOG_ALERT,   "critical", LOG_CRIT,
...
sendlog(const char *msg)
{
    ...
    for (;;) {
        if (log.fd < 0) {
            ...
            if (*(s = p->name) != '/'
                && !(s = pathpath(
                     buf, s, "", PATH_REGULAR | PATH_READ, sizeof(buf))))
                continue;
