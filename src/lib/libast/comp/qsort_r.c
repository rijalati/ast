...
static char *
med3(char *a, char *b, char *c, Qsortcmp_r_f cmp, void *h)
{
    return cmp(a, b, h) < 0
...
void
qsort_r(void *va, size_t n, size_t es, Qsortcmp_r_f cmp, void *h)
{
    ...
    vecswap(pb, pn - s, s);
    if ((s = pb - pa) > es)
        qsort_r(a, s / es, es, cmp, h);
    if ((s = pd - pc) > es)
        qsort_r(pn - s, s / es, es, cmp, h);
}
