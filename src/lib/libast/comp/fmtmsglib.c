...
const MM_table_t mm_class[] = { "null",
                                0,
                                0,
                                "hard",
                                "HARDWARE",
...
= { "nosev", 0,       MM_NOSEV, "halt", "HALT",    MM_HALT,
    "error", "ERROR", MM_ERROR, "warn", "WARNING", MM_WARNING,
    "info",  "INFO",  MM_INFO,  0,      0,         0 };

const MM_table_t mm_verb[]
= { "all",    0, MM_all,    "action",   0, MM_action,
    "class",  0, MM_class,  "default",  0, MM_default,
    "label",  0, MM_label,  "severity", 0, MM_severity,
    "source", 0, MM_source, "tag",      0, MM_tag,
    "text",   0, MM_text,   0,          0, 0 };
        ...
        if ((mask & MM_action) && action
            ...
            if (mask & (MM_class | MM_source | MM_status)) {
                ...
                if ((mask & MM_source)
                    ...
                    && (s = display(mm_class, m, 1)))
                    sfprintf(sp, " %s", s);
                if ((mask & MM_class)
                    && (m = classification & (MM_HARD | MM_SOFT | MM_FIRM))
                    && (s = display(mm_class, m, 1)))
                    sfprintf(sp, " %s", s);
                if ((mask & MM_status)
                    && (m = classification & (MM_RECOVER | MM_NRECOV))
                    && (s = display(mm_class, m, 1)))
                    sfprintf(sp, " %s", s);
            }
            sfputc(sp, '\n');
