...
/*
 * *at() emulation -- if this code is active for your time critical app then
 * upgrade your system
 */
...
faccessat(int cwd, const char *path, mode_t mode, int flags)
{
    ...
    r = (flags & AT_EACCESS) ? eaccess(ATPATH(cwd, path), mode)
                             : access(ATPATH(cwd, path), mode);
    ATEND();
    return r;
}
...
unlinkat(int cwd, const char *path, int flags)
{
    ...
    r = (flags & AT_REMOVEDIR) ? rmdir(ATPATH(cwd, path))
                               : unlink(ATPATH(cwd, path));
    ATEND();
    return r;
}
