...
_getuid(void);
extern int
_link(const char *, const char *);
extern int
_open(const char *, int, ...);
extern long
_pathconf(const char *, int);
extern ssize_t
...
_unlink(const char *);
extern int
_utime(const char *, const struct utimbuf *);
extern int
_utimes(const char *, const struct timeval *);
extern ssize_t
...
execrate(const char *path, char *buf, int size, int physical)
{
    ...
    if (suffix(path))
        return 0;
    oerrno = errno;
    if (physical || strlen(path) >= size
        || !(s = pathcanon(strcpy(buf, path),
                           size,
                           PATH_PHYSICAL | PATH_DOTDOT | PATH_EXISTS)))
...
#    endif
            n = 2;
        r = (m = sysread(fd, buf, n)) >= 2
            && (buf[1] == 0x5a && (buf[0] == 0x4c || buf[0] == 0x4d)
                || ux && buf[0] == '#' && buf[1] == '!' && (*ux = 1)
                   && !(ux = 0))
            ? 0
            : -1;
...
access(const char *path, int op)
{
    ...
    oerrno = errno;
    if ((r = sysaccess(path, op)) && errno == ENOENT
        && execrate(path, buf, sizeof(buf), 0))
    {
        errno = oerrno;
        r = sysaccess(buf, op);
    }
...
chmod(const char *path, mode_t mode)
{
    ...
    if ((r = syschmod(path, mode)) && errno == ENOENT
        && execrate(path, buf, sizeof(buf), 0))
    {
        errno = oerrno;
        return syschmod(buf, mode);
    }
    if (!(r = syschmod(path, mode)) && (mode & (S_IXUSR | S_IXGRP | S_IXOTH))
        && !suffix(path) && (strlen(path) + 4) < sizeof(buf))
    {
        oerrno = errno;
        if (!magic(path, NiL))
        {
...
getuid(void)
{
    ...
    if (!convertinit++ && (d = getenv(convertvars[0])))
        for (e = environ; s = *e; e++)
            if ((n = convert(d, s))
                && (m = cygwin_win32_to_posix_path_list_buf_size(s + n)) > 0)
            {
                if (!(t = malloc(n + m + 1)))
                    break;
                *e = t;
...
#        if DEBUG
    if (trace == 'a' || trace == 'e')
    {
        sfprintf(sfstderr,
                 "%s %s [",
                 mode == _P_OVERLAY ? "_execve" : "_spawnve",
                 path);
        for (n = 0; argv[n]; n++)
...
link(const char *fp, const char *tp)
{
    ...
    oerrno = errno;
    if ((r = syslink(fp, tp)) && errno == ENOENT
        && execrate(fp, fb, sizeof(fb), 1))
    {
...
rename(const char *fp, const char *tp)
{
    ...
    oerrno = errno;
    if ((r = sysrename(fp, tp)) && errno == ENOENT
        && execrate(fp, fb, sizeof(fb), 1))
    {
        if (execrate(tp, tb, sizeof(tb), 1))
            tp = tb;
...
stat(const char *path, struct stat *st)
{
    ...
    oerrno = errno;
    if ((r = sysstat(path, st)) && errno == ENOENT
        && execrate(path, buf, sizeof(buf), 0))
    {
        errno = oerrno;
        r = sysstat(buf, st);
...
truncate(const char *path, off_t offset)
{
    ...
    oerrno = errno;
    if ((r = systruncate(path, offset)) && errno == ENOENT
        && execrate(path, buf, sizeof(buf), 0))
    {
        errno = oerrno;
...
ctime_now(const char *path)
{
    ...
    if (sysstat(path, &fs) || (fs.st_mode & S_IWUSR)
        || syschmod(path, (fs.st_mode | S_IWUSR) & S_IPERM))
        fs.st_mode = 0;
...
utimes(const char *path, const struct timeval *ut)
{
    ...
    oerrno = errno;
    if ((r = sysutimes(path, ut)) && errno == ENOENT
        && execrate(path, buf, sizeof(buf), 0))
    {
        errno = oerrno;
...
utime(const char *path, const struct utimbuf *ut)
{
    ...
    oerrno = errno;
    if ((r = sysutime(path, ut)) && errno == ENOENT
        && execrate(path, buf, sizeof(buf), 0))
    {
        errno = oerrno;
