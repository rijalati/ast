...
#    if DEBUG_TRACE
    if (error_info.trace < DEBUG_TRACE)
        sfprintf(sfstderr,
                 "%s: debug%d: AHA#%d _win_codeset name=%s\n",
                 error_info.id,
                 error_info.trace,
                 __LINE__,
                 name);
#    endif
    if (name == name_native)
        return CP_ACP;
    if (!strcasecmp(name, "utf") || !strcasecmp(name, "utf8")
        || !strcasecmp(name, "utf-8"))
        return CP_UTF8;
    if (!strcasecmp(name, "ucs") || !strcasecmp(name, "ucs2")
        || !strcasecmp(name, "ucs-2"))
        return CP_UCS2;
...
#    if DEBUG_TRACE
    if (error_info.trace < DEBUG_TRACE)
        sfprintf(sfstderr,
                 "%s: debug%d: AHA#%d _win_iconv_open f=%s t=%s\n",
                 error_info.id,
                 error_info.trace,
                 __LINE__,
                 f,
...
#    if DEBUG_TRACE
    if (error_info.trace <= DEBUG_TRACE)
        sfprintf(sfstderr,
                 "%s: debug%d: AHA#%d _win_iconv_open f=0x%04x t=0x%04x\n",
                 error_info.id,
                 error_info.trace,
                 __LINE__,
                 cc->from.index,
...
#    if DEBUG_TRACE
    if (error_info.trace < DEBUG_TRACE)
        sfprintf(sfstderr,
                 "%s: debug%d: AHA#%d _win_iconv from=0x%04x to=0x%04x\n",
                 error_info.id,
                 error_info.trace,
                 __LINE__,
                 cc->from.index,
    ...
    if (cc->from.index == cc->to.index
        ...
        if (cc->to.index == CP_UCS2) {
            ...
            if (tz = WideCharToMultiByte(
                ...
                for (;;) {
                    while (!(fz = MultiByteToWideChar(cc->from.index,
                        ...
                        if (++bz > *fn)
                            goto nope;
                    if (!(tz = WideCharToMultiByte(
                          cc->to.index, 0, ( LPCWSTR )ub, fz, *tb, 0, 0, 0)))
                        goto nope;
                    if (tz == *tn)
                        break;
                    if (!(pz /= 2)) {
                ...
                if (!(tz = WideCharToMultiByte(
                      cc->to.index, 0, ( LPCWSTR )ub, fz, *tb, tz, 0, 0)))
                    goto nope;
#    if DEBUG_TRACE
                if (error_info.trace < DEBUG_TRACE)
                    sfprintf(sfstderr,
                             "%s: debug%d: AHA#%d _win_iconv *fn=%u "
                             "fz=%u[%u] *tn=%u tz=%u\n",
                             error_info.id,
                             error_info.trace,
                             __LINE__,
...
_ast_iconv_name(const char *m, char *b, size_t n)
{
    ...
    for (;;) {
        ...
        if (error_info.trace < DEBUG_TRACE)
            sfprintf(sfstderr,
                     "%s: debug%d: AHA#%d _ast_iconv_name n=%d bp=%p cp=%p "
                     "ccode=%d name=\"%s\"\n",
                     error_info.id,
                     error_info.trace,
                     __LINE__,
...
_ast_iconv_open(const char *t, const char *f)
{
    ...
    if (error_info.trace < DEBUG_TRACE)
        sfprintf(sfstderr,
                 "%s: debug%d: AHA#%d _ast_iconv_open f=%s t=%s\n",
                 error_info.id,
                 error_info.trace,
                 __LINE__,
                 f,
                 t);
#endif
    if (!t || !*t || *t == '-' && !*(t + 1) || !strcasecmp(t, name_local)
        || !strcasecmp(t, name_native))
        t = name_native;
    if (!f || !*f || *f == '-' && !*(f + 1) || !strcasecmp(t, name_local)
        || !strcasecmp(f, name_native))
        f = name_native;
    ...
    if (error_info.trace <= DEBUG_TRACE)
        sfprintf(
        sfstderr,
        "%s: debug%d: AHA#%d _ast_iconv_open f=%s:%s:%d t=%s:%s:%d\n",
        error_info.id,
        error_info.trace,
        __LINE__,
        f,
...
#endif
#if _UWIN
    else if ((cc->cvt = _win_iconv_open(cc, t, f)) != (_ast_iconv_t)(-1)
    ...
    if (cc) {
        if (cc->from.fun) {
            if (cc->to.fun) {
                ...
                tfb = *fb;
                tfn = *fn;
                if ((*cc->from.fun)(cc->cvt, &tfb, &tfn, &b, &i)
                    == (size_t)(-1))
                    return -1;
                tfn = b - cc->buf;
                tfb = cc->buf;
...
ssize_t
_ast_iconv_write(_ast_iconv_t cd,
                 Sfio_t *op,
                 char **fb,
                 size_t *fn,
                 Iconv_disc_t *disc)
{
    ...
    while (ok && *fn > 0) {
        ...
        if (error_info.trace < DEBUG_TRACE)
            sfprintf(sfstderr,
                     "%s: debug%d: AHA#%d iconv_write ts=%p tn=%d\n",
                     error_info.id,
                     error_info.trace,
                     __LINE__,
                     ts,
...
#endif
        {
            ...
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(sfstderr,
                         "%s: debug%d: AHA#%d iconv_write %d => %d `%-.*s'\n",
                         error_info.id,
                         error_info.trace,
                         __LINE__,
                         *fn,
            ...
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(sfstderr,
                         "%s: debug%d: AHA#%d iconv_write %d => %d [%d]\n",
                         error_info.id,
                         error_info.trace,
                         __LINE__,
                         *fn,
...
ssize_t
_ast_iconv_move(_ast_iconv_t cd,
                Sfio_t *ip,
                Sfio_t *op,
                size_t n,
                Iconv_disc_t *disc)
{
    ...
    for (;;) {
        ...
        if (error_info.trace < DEBUG_TRACE)
            sfprintf(sfstderr,
                     "%s: debug%d: AHA#%d iconv_move ff=%d fn=%I*d fl=%d "
                     "tf=%d tn=%I*d tl=%d\n",
                     error_info.id,
                     error_info.trace,
                     __LINE__,
        ...
        if (ff || !fn) {
            ...
            if ((!(fb = ( char * )sfreserve(
                   ip, n == SF_UNBOUND ? (-fn - 1) : n, locked = SF_LOCKR))
                 || !(fo = sfvalue(ip)))
                && (!(fb = ( char * )sfreserve(
                ...
                if (error_info.trace < DEBUG_TRACE)
                    sfprintf(sfstderr,
                             "%s: debug%d: AHA#%d iconv_move count=%I*d\n",
                             error_info.id,
                             error_info.trace,
                             __LINE__,
                             sizeof(fn),
            ...
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(
                sfstderr,
                "%s: debug%d: AHA#%d iconv_move count=%I*d fn=%I*d fo=%I*d\n",
                error_info.id,
                error_info.trace,
                __LINE__,
                sizeof(fn),
            ...
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(
                sfstderr,
                "%s: debug%d: AHA#%d iconv_move fl=%d fb=%p fn=%I*d\n",
                error_info.id,
                error_info.trace,
                __LINE__,
                fl,
        ...
        if (tf || !tn) {
            ...
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(
                sfstderr,
                "%s: debug%d: AHA#%d iconv_move tl=%d tb=%p tn=%I*d\n",
                error_info.id,
                error_info.trace,
                __LINE__,
                tl,
        ...
        if (error_info.trace < DEBUG_TRACE)
            sfprintf(sfstderr,
                     "%s: debug%d: AHA#%d iconv_move r=%I*d fl=%d fs=%p "
                     "fn=%I*d tl=%d ts=%p tn=%I*d\n",
                     error_info.id,
                     error_info.trace,
                     __LINE__,
                     sizeof(r),
        ...
        if (_ast_iconv(cd, &fs, &fn, &ts, &tn) == (size_t)(-1)) {
            ...
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(sfstderr,
                         "%s: debug%d: AHA#%d iconv_move fl=%d fs=%p fn=%I*d "
                         "tl=%d ts=%p tn=%I*d errno=%d\n",
                         error_info.id,
                         error_info.trace,
                         __LINE__,
                         fl,
    ...
    if (error_info.trace < DEBUG_TRACE)
        sfprintf(sfstderr,
                 "%s: debug%d: AHA#%d iconv_move fb=%p fn=%zu\n",
                 error_info.id,
                 error_info.trace,
                 __LINE__,
                 fb,
