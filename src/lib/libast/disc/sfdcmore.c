...
#if __STD_C
static ssize_t
moreread(Sfio_t *f, void *buf, size_t n, Sfdisc_t *dp)
#else
static ssize_t moreread(f, buf, n, dp) Sfio_t *f;
void *buf;
size_t n;
Sfdisc_t *dp;
#endif
...
#if __STD_C
static int
ttyquery(Sfio_t *rp, Sfio_t *wp, const char *label, Sfdisc_t *dp)
#else
static int ttyquery(rp, wp, label, dp) Sfio_t *rp;
Sfio_t *wp;
char *label;
Sfdisc_t *dp;
#endif
...
#if __STD_C
static ssize_t
morewrite(Sfio_t *f, const Void_t *buf, size_t n, Sfdisc_t *dp)
#else
static ssize_t morewrite(f, buf, n, dp) Sfio_t *f;
Void_t *buf;
size_t n;
Sfdisc_t *dp;
#endif
{
    ...
    match:
        for (r = more->pattern[0];; s++) {
            ...
            if (*s == '\n')
                b = s + 1;
            else if (*s == r && (e - s) >= more->match
                     && !strncmp(s, more->pattern, more->match))
                break;
        }
        s = b;
        w += b - ( char * ) buf;
...
#if __STD_C
static int
moreexcept(Sfio_t *f, int type, Void_t *data, Sfdisc_t *dp)
#else
static int moreexcept(f, type, data, dp) Sfio_t *f;
int type;
Void_t *data;
Sfdisc_t *dp;
#endif
...
#if __STD_C
int
sfdcmore(Sfio_t *f, const char *prompt, int rows, int cols)
#else
int sfdcmore(f, prompt, rows, cols) Sfio_t *f;
char *prompt;
int rows;
int cols;
#endif
{
    ...
    if (!(sfset(f, 0, 0) & SF_WRITE) || !isatty(sffileno(sfstdin))
