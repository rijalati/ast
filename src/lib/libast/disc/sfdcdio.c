...
#if __STD_C
static ssize_t
diordwr(Sfio_t *f, Void_t *buf, size_t n, Direct_t *di, int type)
#else
...
#endif
{
    ...
    if ((P2I(buf) % di->dio.d_mem) == 0 && (f->here % di->dio.d_miniosz) == 0
        && n >= di->dio.d_miniosz) { /* direct IO ok, make sure we're in the
                                        right mode */
        if (!(di->cntl & FDIRECT)) {
            di->cntl |= FDIRECT;
...
#if __STD_C
static ssize_t
dioread(Sfio_t *f, Void_t *buf, size_t n, Sfdisc_t *disc)
#else
...
#if __STD_C
static ssize_t
diowrite(Sfio_t *f, const Void_t *buf, size_t n, Sfdisc_t *disc)
#else
...
#if __STD_C
static int
dioexcept(Sfio_t *f, int type, Void_t *data, Sfdisc_t *disc)
#else
...
#if __STD_C
int
sfdcdio(Sfio_t *f, size_t bufsize)
#else
