...
#define END(b)                                                               \
    (*((b)->nxt >= (b)->end ? ((b)->nxt = (b)->end - 1) : (b)->nxt) = 0,     \
     (b)->nxt - (b)->buf)
...
static const Item_t key[] = { KEY("author"),
                              KEY("class"),
                              KEY("company"),
                              KEY("component"),
                              KEY("contributor"),
...
static const Item_t lic[] = { LIC("none", NONE),
                              LIC("inline", SPECIAL),
                              LIC("test", TEST),
                              LIC("verbose", VERBOSE),
                              LIC("usage", USAGE),
...
lookup(const Item_t *item, const char *name, int size)
{
    ...
    for (i = 0; item[i].data; i++)
        if (c == item[i].data[0] && size == item[i].size
            && !strncmp(name, item[i].data, size))
            return i;
    return -1;
}
           ...
           int cc3)
{
                ...
                if (contributor)
                {
                    for (i = 0; i < notice.ids; i++)
                        if (n == notice.id[i].name.size
                            && !strncmp(x, notice.id[i].name.data, n))
                            break;
                    if (i < IDS)
                    {
    ...
    if (!k)
        return 0;
    if (notice.type == INLINE
        && (!notice.verbose || !notice.item[NOTICE].data))
        return 0;
    if (notice.type != USAGE)
    {
        ...
        {
            copy(
            &tmp, notice.item[PACKAGE].data ? "and" : "This software", -1);
            ...
            comment(&notice, &buf, NiL, 0, 0);
            COMMENT(
            &notice, &buf, "A copy of the License is available at", 0);
            if (notice.item[URL].data)
            {
                ...
                COMMENT(
                &notice, &buf, "http://www.opensource.org/licenses/cpl", 0);
            comment(&notice, &buf, NiL, 0, 0);
        }
        else if (notice.type == OPEN)
        {
            copy(
            &tmp, notice.item[PACKAGE].data ? "and it" : "This software", -1);
            ...
            COMMENT(&notice,
                    ...
                    0);
            COMMENT(
            &notice, &buf, "the license and copyright and are violating", 0);
            if (i >= 0)
                expand(&notice, &tmp, &notice.item[i]);
            copy(&tmp, "'s", -1);
            if (n >= COMLONG)
        ...
        else if (notice.type == GPL)
        {
            ...
            COMMENT(&notice,
                    ...
                    0);
            COMMENT(
            &notice, &buf, "will be useful, but WITHOUT ANY WARRANTY;", 0);
            COMMENT(&notice,
                    &buf,
            ...
            comment(&notice, &buf, NiL, 0, 0);
            COMMENT(
            &notice, &buf, "You should have received a copy of the", 0);
        ...
        else if (notice.type == BSD)
        {
            ...
            if (notice.item[i = PARENT].data
                || notice.item[i = CORPORATION].data
                || notice.item[i = COMPANY].data)
                expand(&notice, &tmp, &notice.item[i]);
            else
            ...
            -1);
            COMMENT(
            &notice, &buf, "      specific prior written permission.", -1);
            comment(&notice, &buf, NiL, 0, 0);
            COMMENT(&notice,
                    &buf,
        ...
        else
        {
            ...
            {
                COMMENT(
                &notice, &buf, "Unpublished & Not for Publication", 0);
                comment(&notice, &buf, NiL, 0, 0);
            }
            if (notice.item[URL].data)
            {
                ...
                if (notice.item[CORPORATION].data
                    || notice.item[COMPANY].data)
                {
                    copy(&tmp, " by", -1);
                    if ((notice.item[PARENT].size
                         + (notice.item[CORPORATION].data
                            ? (notice.item[CORPORATION].size
                               + notice.item[INCORPORATION].size)
                            : notice.item[COMPANY].size))
                        >= (COMLONG - 6))
                        comment(&notice, &buf, BUF(&tmp), USE(&tmp), 0);
                    else
                        PUT(&tmp, ' ');
                    ...
                    if (notice.item[CORPORATION].data)
                    {
                        ...
                        if (notice.item[INCORPORATION].data)
                        {
                            copy(&tmp, " ", -1);
                            expand(
                            &notice, &tmp, &notice.item[INCORPORATION]);
                        }
                    }
                    else if (notice.item[COMPANY].data)
                        expand(&notice, &tmp, &notice.item[COMPANY]);
