...
#    if __CYGWIN__
    if (streq(type, "system") || streq(type, "user")) {
        ...
        if (GetVolumeInformation(
            drive, 0, 0, &vser, &len, &flags, hp->typ, sizeof(hp->typ) - 1))
            hp->mnt.type = hp->typ;
        else
            flags = 0;
        SetErrorMode(mode);
...
#                else
#                    define TYPE(f)                                          \
                        (strchr((f)->f_mntfromname, ':') ? "nfs" : "ufs")
#                endif
#            endif
#        endif
    ...
    if (mp->next < mp->last) {
        ...
        for (i = 0; i < elementsof(options); i++)
            if (flags & options[i].flag)
                n += sfsprintf(
                mp->opt + n, sizeof(mp->opt) - n - 1, ",%s", options[i].name);
        set(&mp->hdr,
...
#            endif
        default:
            sfsprintf(
            t = mp->type, sizeof(mp->type), "aix%+d", mp->next->vmt_gfstype);
            break;
        }
...
#                    define MNTBUFSIZE                                       \
                        (sizeof(struct w_mnth) + 16 * sizeof(struct w_mntent))
...
#                        ifndef mnt_type
            ...
            set(&mp->hdr,
                mp->mnt->mnt_fsname,
                mp->mnt->mnt_dir,
                stat(mp->mnt->mnt_dir, &st)
                ? FS_default
                : strlcpy(typ, fmtfs(&st), sizeof(typ)),
                OPTIONS(mp->mnt));
#                        else
            set(&mp->hdr,
                mp->mnt->mnt_fsname,
