...
#endif
{
    ...
    while (s = fgets(buf, sizeof(buf), stdin)) {
        ...
        case CHARSET:
            ...
            cp->ms = copy(&b, arg[2]);
            if (cp
                != ( Charset_t * ) enter(&state.charset, ( Link_t * ) cp)) {
                fprintf(stderr,
        ...
        case TERRITORY:
            ...
            if (tp
                != ( Territory_t * ) enter(&state.territory,
                                           ( Link_t * ) tp)) {
                fprintf(stderr,
    ...
    fprintf(
    hf, "#define LC_territory_language_max\t%d\n", territory_language_max);
    fprintf(hf, "\nstruct Lc_s;\n");
    ...
    fprintf(hf, "\tunsigned long\t\tindex;\n");
    fprintf(
    hf, "\tconst Lc_attribute_t*\tattributes[LC_language_attribute_max];\n");
    ...
    fprintf(hf, "\tunsigned long\t\tindex;\n");
    fprintf(
    hf, "\tconst Lc_language_t*\tlanguages[LC_territory_language_max];\n");
    ...
    for (lp = ( Language_t * ) state.language.root; lp;
        ...
        if (lp->alternates)
            ...
            fprintf(lf, "0,");
        fprintf(
        lf, "&lc_charsets[%d],0,", lp->charset ? lp->charset->link.index : 0);
    ...
    for (mp = ( Map_t * ) state.map.root; mp;
         mp = ( Map_t * ) mp->link.next) {
        ...
        fprintf(
        lf, "&lc_charsets[%d],", mp->charset ? mp->charset->link.index : 0);
        if (mp->attribute)
            fprintf(lf,
                    "&attribute_%s[%d]",
                    mp->language->link.code,
                    mp->attribute->link.index);
        else
            fprintf(lf, "0");
        fprintf(lf, "},\n");
    }
