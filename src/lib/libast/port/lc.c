...
= { &default_lc, &default_lc, &default_lc, &default_lc, &default_lc,
    &default_lc, &default_lc, &default_lc, &default_lc, &default_lc,
    &default_lc, &default_lc, &default_lc, &default_lc };
...
static size_t
canonical(const Lc_language_t *lp,
          ...
          size_t siz)
{
    ...
    if (!(flags
          & (LC_abbreviated | LC_default | LC_local | LC_qualified
             | LC_verbose)))
    ...
    if (s < e) {
        if (tp && tp != &lc_territories[0]) {
            r = 0;
            if (lp) {
                if ((flags & (LC_abbreviated | LC_default))
                    && streq(lp->code, tp->code))
                    r = s;
                *s++ = '_';
            }
            if (flags & LC_verbose) {
        ...
        if (lp
            && (!(flags & (LC_abbreviated | LC_default)) || cp != lp->charset)
            && s < e) {
            *s++ = '.';
            t = cp->code;
...
lccanon(Lc_t *lc, unsigned long flags, char *buf, size_t siz)
{
    if ((flags & LC_local)
        && (!lc->language
            || !(lc->language->flags & (LC_debug | LC_default)))) {
#if _WINIX
        char lang[64];
        char code[64];
        char ctry[64];
        ...
        if (lc->index
            && GetLocaleInfo(
               lc->index, LOCALE_SENGLANGUAGE, lang, sizeof(lang))
            && GetLocaleInfo(
               lc->index, LOCALE_SENGCOUNTRY, ctry, sizeof(ctry))) {
            if (!GetLocaleInfo(
                lc->index, LOCALE_IDEFAULTANSICODEPAGE, code, sizeof(code)))
                code[0] = 0;
            if (!lc->charset || !lc->charset->ms)
                ...
                return sfsprintf(
                buf, siz, "%s_%s.%s,%s", lang, ctry, code, lc->charset->ms);
        }
#endif
        buf[0] = '-';
        buf[1] = 0;
    ...
    return canonical(
    lc->language, lc->territory, lc->charset, lc->attributes, flags, buf, siz);
}
...
lcmake(const char *name)
{
    ...
    *s = 0;
    ...
    if ((ast.locale.set & AST_LC_debug)
        ...
        sfprintf(
        sfstderr,
        "locale make %s language=%s territory=%s charset=%s attributes=%s\n",
        name,
        language_name,
        territory_name,
        charset_name,
    ...
    if (n == 2)
        ...
        for (lp = lc_languages;
             lp->code && (!lp->alternates || !match(s, lp->alternates, n, 0));
             lp++)
            ;
        if (!lp->code) {
            c = s[2];
    ...
    if (!lp || !lp->code) {
        ...
        if (!lp || !lp->code) {
            if (!territory_name) {
                if (n == 2)
                    for (tp = lc_territories; tp->code && !streq(s, tp->code);
                         tp++)
                        ;
                else {
                    z = 0;
                ...
                for (tp = lc_territories; tp->code; tp++)
                    if (streq(s, tp->code)) {
                        if (lp != &lc_languages[0]) {
                            for (i = 0; i < elementsof(tp->languages)
                                        && lp != tp->languages[i];
                                 i++)
                                ;
    ...
    if (s = attributes_name) {
        do {
            ...
            *s = 0;
            ...
            if (!cp->code) {
                for (i = 0; i < elementsof(lp->attributes)
                            && (ap = lp->attributes[i]);
                     i++)
    ...
    if ((ast.locale.set & AST_LC_debug)
        && !(ast.locale.set & AST_LC_internal))
        sfprintf(sfstderr,
                 "locale make %s charset_name=%s cp=%s ppa=%s lp=%s\n",
                 name,
                 charset_name,
                 cp ? cp->code : 0,
                 ppa,
...
override:
    ...
    if ((ast.locale.set & AST_LC_debug)
        && !(ast.locale.set & AST_LC_internal))
        sfprintf(sfstderr,
                 "locale make %17s %16s %16s %16s language=%s territory=%s "
                 "charset=%s%s\n",
                 "",
                 lc->name,
                 lc->code,
...
lcscan(Lc_t *lc)
{
    ...
    if (++ls->attribute >= elementsof(ls->lc.language->attributes)
        || !(ls->list.attribute
             = ls->lc.language->attributes[ls->attribute])) {
        if (++ls->language >= elementsof(ls->lc.territory->languages)
            || !(ls->lc.language
                 = ls->lc.territory->languages[ls->language])) {
    ...
    if (!ls->lc.language || !ls->lc.language->index)
        ...
        if ((!ls->list.attribute
             || !(ls->lc.index = ls->list.attribute->index))
            && (!ls->lc.territory
                || !(ls->lc.index = ls->lc.territory->indices[ls->language])))
            ls->lc.index = SUBLANG_DEFAULT;
        ls->lc.index = MAKELCID(
        MAKELANGID(ls->lc.language->index, ls->lc.index), SORT_DEFAULT);
    }
#endif
    canonical(ls->lc.language,
              ls->lc.territory,
              ls->lc.charset,
              ls->lc.attributes,
              0,
