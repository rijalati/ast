...
#define STANDARD(v)                                                          \
    (streq(v, "standard") || streq(v, "strict") || streq(v, "posix")         \
     || streq(v, "xopen"))
...
= { "getconf",         "_AST_FEATURES", "CONFORMANCE = standard",
...
/*
 * private ast pathconf() intercept with special /dev/ file checks on path for
 * fpathconf()
 */
...
synthesize(Feature_t *fp, const char *path, const char *value)
{
    ...
    if (fp)
        error(-6,
              "astconf synthesize name=%s path=%s value=%s fp=%p%s",
              fp->name,
              path,
              value,
              fp,
...
#if DEBUG_astconf
    error(-6,
          "astconf initialize name=%s path=%s command=%s succeed=%s fail=%s "
          "fp=%p%s",
          fp->name,
          path,
          command,
...
#if DEBUG_astconf
    error(-6,
          "state.std=%d %s [%s] std=%s ast=%s value=%s ok=%d",
          state.std,
          fp->name,
          ok ? succeed : fail,
          fp->std,
...
#if DEBUG_astconf
    error(-6,
          "astconf format name=%s path=%s value=%s flags=%04x fp=%p%s",
          fp->name,
          path,
          value,
          flags,
...
#    endif
...
#if DEBUG_astconf
        error(-6,
              "state.std=%d %s [%s] std=%s ast=%s value=%s",
              state.std,
              fp->name,
              value,
              fp->std,
    ...
    case OP_fs_3d:
        fp->value = fs3d(value ? value[0] ? FS3D_ON : FS3D_OFF : FS3D_TEST)
                    ? "1"
                    : null;
        break;
...
#if DEBUG_astconf
    error(-6,
          "astconf feature name=%s path=%s value=%s flags=%04x fp=%p%s",
          name,
          path,
          value,
          flags,
...
#if DEBUG_astconf
    error(-6,
          "astconf normal name=%s standard=%d section=%d call=%d flags=%04x "
          "elements=%d",
          look->name,
          look->standard,
          look->section,
...
#if DEBUG_astconf
    error(-6,
          "astconf lookup name=%s standard=%d:%d section=%d:%d call=%d:%d",
          look->name,
          look->standard,
          mid->standard,
          look->section,
      ...
      Error_f conferror)
{
    ...
    (p->flags & CONF_LIMIT_DEF) ? "LIMIT_DEF|"
                                : (p->flags & CONF_LIMIT) ? "LIMIT|" : "",
    (p->flags & CONF_MINMAX_DEF) ? "MINMAX_DEF|"
                                 : (p->flags & CONF_MINMAX) ? "MINMAX|" : "",
    ...
    if (conferror && name) {
        ...
        if (look->standard >= 0
            && (name[0] != '_'
                && ((p->flags & CONF_UNDERSCORE) || look->section <= 1)
        ...
        if (p->flags & CONF_LIMIT_DEF) {
            if (p->limit.string)
                sfprintf(sp,
                         ...
                         ? fmtquote(p->limit.string,
                                    "\"",
                                    "\"",
                                    strlen(p->limit.string),
                                    FMT_SHELL)
                         : p->limit.string);
            else
                sfprintf(
        ...
        if (p->flags & CONF_MINMAX_DEF) {
            if (p->minmax.string)
                sfprintf(sp,
                         "M[%s] ",
                         (listflags & ASTCONF_quote)
                         ? fmtquote(p->minmax.string,
                                    "\"",
                                    "\"",
                                    strlen(p->minmax.string),
                                    FMT_SHELL)
                         : p->minmax.string);
            else
                sfprintf(
                sp, "M[%I*d] ", sizeof(p->minmax.number), p->minmax.number);
        }
        if (flags & CONF_ERROR)
            sfprintf(sp, "error");
        else if (defined) {
            ...
            if (!name) {
                if ((p->flags & (CONF_PREFIXED | CONF_STRING))
                    ...
                    sfprintf(sp,
                             ...
                             ? fmtlower(prefix[p->standard].name)
                             : prefix[p->standard].name);
                    if (p->section > 1)
                ...
                if (!lookup(&look, f, flags)) {
                    if (flags & ASTCONF_table) {
                        ...
                        sfprintf(sp,
                                 ...
                                 sizeof(conf[0].name),
                                 f,
                                 sizeof(prefix[look.standard].name),
                                 prefix[look.standard].name,
                                 look.section,
                                 call,
                                 0,
                                 "N",
            ...
            if (flags & ASTCONF_table) {
                ...
                *f = 0;
                sfprintf(sp,
                         ...
                         sizeof(conf[0].name),
                         fp->name,
                         sizeof(prefix[fp->standard].name),
                         prefix[fp->standard].name,
                         1,
                         call,
                         0,
                         flg,
                ...
                sfprintf(sp,
                         "%s %s - %s\n",
                         state.id,
                         (flags & ASTCONF_lower) ? fmtlower(fp->name)
                                                 : fp->name,
                         fmtquote(s, "\"", "\"", strlen(s), FMT_SHELL));
            else
                sfprintf(sp,
