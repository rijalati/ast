 ...
 *      files if the parameter max_chain_length (described below) is too
 * large.
 *
 *  ACKNOWLEDGEMENTS
 *
...
#ifdef DEBUG
local void
check_match OF((deflate_state * s, IPos start, IPos match, int length));
#endif
...
uInt dictLength;
{
    ...
    if (length > MAX_DIST(s))
    {
        length = MAX_DIST(s);
        dictionary
        += dictLength - length; /* use the tail of the dictionary */
    }
...
uLong sourceLen;
{
    ...
    destLen
    = sourceLen + ((sourceLen + 7) >> 3) + ((sourceLen + 63) >> 6) + 11;
...
int flush;
{
    ...
    if (strm == Z_NULL || strm->state == Z_NULL || flush > Z_FINISH
        || flush < 0)
    {
        return Z_STREAM_ERROR;
    }
            ...
            while (s->gzindex < (s->gzhead->extra_len & 0xffff))
            {
                if (s->pending == s->pending_buf_size)
                {
                    if (s->gzhead->hcrc && s->pending > beg)
                        strm->adler = crc32(
                        strm->adler, s->pending_buf + beg, s->pending - beg);
                    flush_pending(strm);
                    beg = s->pending;
                    if (s->pending == s->pending_buf_size)
                        break;
 ...
 * To simplify the source, this is not supported for 16-bit ZLIB_MSDOS (which
...
z_streamp source;
{
    ...
    /* following zmemcpy do not work for 16-bit ZLIB_MSDOS */
 ...
 */
local uInt longest_match_fast(s, cur_match) deflate_state *s;
IPos cur_match; /* current match */
{
    Bytef *scan = s->window + s->strstart; /* current string */
    Bytef *match; /* matched string */
    ...
    do
    {
        ...
        if (s->strstart >= wsize + MAX_DIST(s))
        {
            ...
            /* Slide the hash table (could be avoided with 32 bit values
               ...
               0 later. (Using level 0 permanently is not an optimal usage of
