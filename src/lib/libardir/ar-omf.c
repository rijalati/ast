...
static Dtdisc_t offdisc = { offsetof(Arfile_t, st.offset),
                            sizeof(off_t),
                            offsetof(Arfile_t, link),
                            0,
                            0,
                            offcomp };

static unsigned char *
readint(unsigned char *cp, int *i, int big)
{
    if (big) {
...
static unsigned char *
readindex(unsigned char *cp, int *size)
{
    if (*cp & 0x80) {
        *size = (*cp & 0xf7) << 8 | cp[1];
        return (cp + 2);
...
static Arfile_t *
ar_getnode(Ardir_t *ar, const char *name)
{
    const char *cp;
    Arfile_t *fp;
...
static size_t
displacement(struct Table *tp, size_t addr)
{
    size_t disp = 0;
    while (tp) {
        disp = tp->disp;
...
Ardirmeth_t ar_omf
= { "omf",     "omf archive", omfopen,  omfnext,    omfchange,
    omfinsert, omfspecial,    omfclose, ar_omf_next };
