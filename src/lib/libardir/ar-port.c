...
portopen(Ardir_t *ar, char *buf, size_t n)
{
    ...
    while (read(ar->fd, ( char * )hdr, sizeof(state->header))
        ...
        if (!state->names && hdr->ar_name[1] == TERM_port
            ...
            if (!(state->names = newof(0, char, size, 0))
                || read(ar->fd, state->names, size) != size)
                goto nope;
            ar->truncate = 0;
...
portnext(Ardir_t *ar)
{
    ...
    if (state->names
        && (*ar->dirent.name == TERM_port || *ar->dirent.name == ' '))
...
Ardirmeth_t ar_port
= { "portable", "portable archive", portopen,    portnext, portchange, 0,
    0,          portclose,          ar_port_next };
