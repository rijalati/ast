...
/*
 * find and load lib plugin/module library name with optional version ver and
 * dlopen() flags at least one dlopen() is called to initialize dlerror() if
 * path!=0 then library path up to size chars copied to path with trailing 0
 * if name contains a directory prefix then library search is limited to the
 * dir and siblings
 */

extern void *
dllplugin(const char *lib,
          const char *name,
          const char *ver,
          unsigned long rel,
          unsigned long *cur,
