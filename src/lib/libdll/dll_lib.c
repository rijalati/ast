...
void *
dll_lib(Dllnames_t *names,
        unsigned long version,
        Dllerror_f dllerrorf,
        void *disc)
{
    void *dll;
    Dll_lib_t *lib;
    ...
    if (!(dll = dllplugin(names->id,
        ...
        if (dllerrorf)
            ...
            errorf("dll",
                   NiL,
                   -1,
                   "dll_lib: %s version %lu library not found",
                   names->name,
                   version);
        return 0;
    }
    ...
    if (!(libf = ( Dll_lib_f ) dlllook(dll, sym))) {
        if (dllerrorf)
            ...
            errorf("dll",
                   NiL,
                   -1,
                   "dll_lib: %s version %lu initialization function %s not "
                   "found in library",
                   names->name,
                   version,
                   sym);
    ...
    if (lib = newof(0,
        ...
        errorf("dll",
               NiL,
               -1,
               "dll_lib: %s version %lu loaded from %s",
               names->name,
               version,
               lib->path);
    }
