 ...
 * if path!=0 then library path up to size chars copied to path with trailing
 * 0 if name contains a directory prefix then library search is limited to the
 * dir and siblings
 */
...
dllfind(const char *lib, const char *ver, int flags, char *path, size_t size)
{
    ...
    if ((id = error_info.id)
        && (dll = dllplug(id, lib, ver, flags, path, size)))
        return dll;
