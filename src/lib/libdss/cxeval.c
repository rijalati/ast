        ...
        *sp = r;
        if (expr->reclaim)
            while (--so > sp)
                if (!so->refs
                    && (f = cxcallout(
                        cx, CX_DEL, so->type, cx->state->type_void, disc))
                    && (*f)(cx, pc, &r, NiL, so, data, disc))
                    break;
        pc += cx->jump;
    }
...
eval(Cx_t *cx, Cxexpr_t *expr, void *data, Cxoperand_t *rv)
{
    ...
    do {
        ...
        if ((t
             = expr->group
               ? eval(cx, expr->group, data, rv)
               : expr->query->sel
                 ? (*expr->query->sel)(cx, expr, data, cx->disc)
                 : expr->query->prog
                   ? execute(cx, expr, expr->query->prog, data, rv, cx->disc)
                   : 1)
            < 0)
            return t;
        if (t) {
            ...
            cx->expr = expr;
            if (expr->query->act
                && (t = (*expr->query->act)(cx, expr, data, cx->disc)) < 0)
                return t;
