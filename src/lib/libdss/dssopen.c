...
= { CXV("file", "string", DSS_MEM_file, "Current record file name.")
    ...
    CXV("selected",
        "number",
        DSS_MEM_selected,
        "Current selected record count."){ 0 } };
...
= { &id[0],    "pseudo-format that treats all files as /dev/null",
    CXH,       dssidentf,
    dssopenf,  dssreadf,
    dsswritef, dssseekf,
    dssclosef };
...
static Dssmeth_t *
dssmethf(const char *, const char *, const char *, Dssdisc_t *, Dssmeth_t *);
...
static Cxquery_t queries[] = {
    QUERY_compress, QUERY_count, QUERY_null,  QUERY_print,
    QUERY_return,   QUERY_scan,  QUERY_write, { 0 },
};

static Dsslib_t dss_library
= { &id[0], "dss method", CXH, 0, &dss_method, 0, 0, 0, 0, &queries[0], 0, 0,
    0 };
     ...
     Dssdisc_t *disc)
{
    ...
    if ((flags & DSS_VERBOSE) && disc && disc->errorf)
        ...
        disc,
        2,
        "%s: %s: initialization function not found in library",
        path,
        buf);
    return 0;
}
...
dsslib(const char *name, Dssflags_t flags, Dssdisc_t *disc)
{
            ...
            if (dtsize(state.cx->libraries) == 1
                ...
                while (dle = dllsread(dls))
                    if (dll = dlopen(dle->path, RTLD_LAZY))
                        ...
                        (*disc->errorf)(
                        NiL, disc, 1, "%s: %s", dle->path, dlerror());
                dllsclose(dls);
            }
        }
    ...
    if ((lib = ( Dsslib_t * )dtmatch(state.cx->libraries, names.base))
        || (lib = ( Dsslib_t * )dll_lib(
            &names,
            DSS_PLUGIN_VERSION,
            (flags & DSS_VERBOSE) ? ( Error_f )disc->errorf : ( Error_f )0,
            disc)))
...
location(Cx_t *cx, void *data, Cxdisc_t *disc)
{
    ...
    if (file->count || file->offset)
        sfsprintf(nxt,
                  ...
                  "%s %I*u, %s %I*d: ",
                  ERROR_translate(NiL, NiL, id, "record"),
                  sizeof(file->count),
                  file->count,
                  ERROR_translate(NiL, NiL, id, "offset"),
                  sizeof(file->offset),
                  file->offset);
    return loc;
}
...
= { dss_mem_get, 0, ( Dt_t * )&dss_mem_struct[0], CX_VIRTUAL };
...
dssopen(Dssflags_t flags, Dssflags_t test, Dssdisc_t *disc, Dssmeth_t *meth)
{
    ...
    if (!meth) {
        ...
        while ((lib = ( Dsslib_t * )dtnext(state.cx->libraries, lib))
               && !lib->meth)
            ;
        if (lib)
            meth = lib->meth;
    ...
    dss->test = test;
    dss->state = &state;
    if (!(dss->cx = cxscope(NiL, meth->cx, flags & DSS_CX_FLAGS, test, disc))
        || disc->map && !loadtags(disc->map, "map", disc, meth))
        goto bad;
    dss->cx->caller = dss;
    if (meth->openf && (*meth->openf)(dss, dss->disc))
        goto bad;
    for (var = ( Cxvariable_t * )dtfirst(dss->cx->variables); var;
         var = ( Cxvariable_t * )dtnext(dss->cx->variables, var))
        if (var->format.map)
    ...
    if (!x->query->head) {
        ...
        if (dss->disc->errorf)
            (*dss->disc->errorf)(dss,
                                 dss->disc,
                                 2,
                                 "%s: expression already has %s head",
                                 head,
                                 x->query->name);
        goto bad;
    } else
