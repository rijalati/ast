...
noreadf(Dssfile_t *file, Dssrecord_t *record, Dssdisc_t *disc)
{
    if (disc->errorf)
        (*disc->errorf)(
        NiL, disc, ERROR_SYSTEM | 2, "%s: not open for read", file->path);
    return -1;
}
...
nowritef(Dssfile_t *file, Dssrecord_t *record, Dssdisc_t *disc)
{
    if (disc->errorf)
        (*disc->errorf)(
        NiL, disc, ERROR_SYSTEM | 2, "%s: not open for write", file->path);
    return -1;
}
        ...
        if (io) {
            ...
            if (sfdisc(io, &top)) {
                ...
                if (n) {
                    ...
                    sfdczip(io,
                            path,
                            dss->meth->compress ? dss->meth->compress
                                                : "gzip",
                            dss->disc->errorf);
                }
            }
        ...
        if (sfsize(file->io)
            ...
            if (format && format != file->format) {
                if (dss->disc->errorf)
                    ...
                    dss->disc,
                    2,
                    "%s: %s file format %s incompatible with %s",
                    file->path,
                    dss->meth->name,
                    file->format->name,
                    format->name);
    ...
    if (!file->format) {
        if (dss->disc->errorf)
            (*dss->disc->errorf)(NiL,
                                 dss->disc,
                                 2,
                                 "%s: %s method did not set file format",
                                 file->path,
                                 dss->meth->name);
        dssfclose(file);
        return 0;
...
{
    return file->seekf ? (*file->seekf)(file, DSS_TELL, file->dss->disc)
                       : file->offset;
}
...
{
    return (file->seekf ? (*file->seekf)(file, offset, file->dss->disc)
                        : sfseek(file->io, offset, SEEK_SET))
           == offset
           ? 0
           : -1;
}
...
{
    return record->file->format->dropf
           ? (*record->file->format->dropf)(
             record->file, record, record->file->dss->disc)
           : -1;
}
...
dss_no_fread(Dssfile_t *file, Dssrecord_t *record, Dssdisc_t *disc)
{
    if (disc->errorf)
        (*disc->errorf)(NiL,
                        disc,
                        2,
                        "%s: %s file format %s read not implemented",
                        file->path,
                        file->dss->meth->name,
                        file->format->name);
    return 0;
...
dss_no_fwrite(Dssfile_t *file, Dssrecord_t *record, Dssdisc_t *disc)
{
    if (disc->errorf)
        (*disc->errorf)(NiL,
                        disc,
                        2,
                        "%s: %s file format %s write not implemented",
                        file->path,
                        file->dss->meth->name,
                        file->format->name);
    return -1;
...
dss_no_fseek(Dssfile_t *file, Sfoff_t offset, Dssdisc_t *disc)
{
    if (disc->errorf)
        (*disc->errorf)(NiL,
                        disc,
                        2,
                        "%s: %s file format %s seek not implemented",
                        file->path,
                        file->dss->meth->name,
                        file->format->name);
    return -1;
