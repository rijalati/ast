...
getfmt(Sfio_t *sp, void *vp, Sffmt_t *dp)
{
    ...
    case DSS_FORMAT_long:
        ...
        if (((ret.value.number >= 0) ? ret.value.number : -ret.value.number)
            < 1)
            value->q = 0;
    ...
    case DSS_FORMAT_string:
        if (ap->fmt
            & (FMT_EXP_CHAR | FMT_EXP_LINE | FMT_EXP_NOCR | FMT_EXP_NONL
               | FMT_EXP_WIDE))
        ...
        if (ap->flags & DSS_FORMAT_quote)
            ...
            = strlen(ret.value.string.data = fmtquote(ret.value.string.data,
                                                      ap->qb,
                                                      ap->qe,
                                                      ret.value.string.size,
                                                      ap->fmt));
            ...
            for (;;) {
                ...
                if (!vp->type->generic
                    ...
                    if (!(*cx->getf)(cx, &x, &r, &a, NiL, record, cx->disc)) {
                        ...
                        if (!cxcast(
                            cx, &r, vp, cx->state->type_string, record, NiL)
                            && r.value.string.size)
                            n += sfprintf(sp,
                                          "%s%-*s%-.*s\n",
                                          vp->name,
        ...
        for (;;) {
            ...
            case 's':
                ...
                set:
                    ...
                    if (ap->variable && !ap->edit
                        && cxisstring(ap->variable->type)
                        && ap->variable->format.map
                        && ap->variable->format.map->part
                        && ap->variable->format.map->part->edit)
