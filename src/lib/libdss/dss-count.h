...
count_end(Cx_t *cx, Cxexpr_t *expr, void *data, Cxdisc_t *disc)
{
    ...
    sfprintf(expr->op,
             ...
             expr == expr->parent->fail
             ? (expr->parent->queried - expr->parent->selected)
             : expr->parent->selected,
             sizeof(expr->parent->queried),
             expr->parent->queried);
    return 0;
}
...
#define QUERY_count                                                          \
    {                                                                        \
        "count", "print parent expression record counts", CXH, count_beg, 0, \
        0, count_end                                                         \
    }
