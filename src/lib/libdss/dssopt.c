...
opttype(Sfio_t *sp, Cxtype_t *tp, int members)
{
    ...
    if (!(tp->header.flags & CX_SCHEMA) && tp->member && optmem(sp, tp))
        return -1;
    else if (tp->base && tp->base->member
             && !(tp->base->header.flags & CX_REFERENCED)
             && opttype(sp, tp->base, 0))
        return -1;
    if (tp->generic)
        for (i = 0; tp->generic[i]; i++)
            if (!(tp->generic[i]->header.flags & CX_REFERENCED)
                && opttype(sp, tp->generic[i], 0))
                return -1;
    return 0;
}
...
dssoptinfo(Opt_t *op, Sfio_t *sp, const char *s, Optdisc_t *dp)
{
    ...
    case 'd':
        ...
        else
        {
            ...
            sfprintf(sp,
                     "%s",
                     state->cx->header ? state->cx->header->description
                                       : "unknown-description");
        }
        return 0;
    case 'i':
        ...
        if (state->cx->header)
            sfprintf(sp,
                     "%s - %s",
                     state->cx->header->name,
                     state->cx->header->description);
        else
    ...
    case 'n':
        ...
        sfprintf(sp,
                 "%s",
                 state->cx->header ? state->cx->header->name
                                   : "unknown-name");
        return 0;
    case 'p':
    ...
    case 'v':
        /* fields|variables */
        all = !state->meth
              || meth->cx && (!meth->cx->fields || !dtsize(meth->cx->fields));
        if (head)
            head = 0;
        else if (meth->description
                 && optout(
                    sp, meth->name, NiL, NiL, NiL, meth->description, NiL))
            return -1;
        if (cx = meth->cx)
        {
            ...
            for (mp = ( Cxmap_t * )dtfirst(cx->maps); mp;
                ...
                if (all || (mp->header.flags & CX_REFERENCED))
                {
                    ...
                    if (optout(
                        sp, mp->name, NiL, NiL, NiL, mp->description, NiL))
                        return -1;
                    if (optmap(sp, mp))
                        return -1;
                }
...
dssoptlib(Sfio_t *sp, Dsslib_t *lib, const char *usage, Dssdisc_t *disc)
{
    ...
    if (lib->meth)
    {
        ...
        if (!usage || strncmp(usage, "[+DESCRIPTION?", 13))
            sfprintf(sp,
                     "[+DESCRIPTION?The %s method handles %s.]\n",
                     lib->meth->name,
                     lib->meth->description);
        sfprintf(sp,
    ...
    if (lib->callouts)
    {
        ...
        for (i = 0; lib->callouts[i].callout; i++)
            if (optout(sp,
                       cxopname(lib->callouts[i].op.code,
                                lib->callouts[i].op.type1,
                                lib->callouts[i].op.type2),
                       NiL,
                       NiL,
                       NiL,
                       lib->callouts[i].description,
                       NiL))
                return -1;
        sfprintf(sp, "}\n");
    }
    if (lib->recodes)
    {
        ...
        for (i = 0; lib->recodes[i].recode; i++)
            if (optout(sp,
                       cxopname(lib->recodes[i].op.code,
                                lib->recodes[i].op.type1,
                                lib->recodes[i].op.type2),
                       NiL,
                       NiL,
                       NiL,
                       lib->recodes[i].description,
                       NiL))
                return -1;
        sfprintf(sp, "}\n");
    }
    ...
    if (lib->constraints)
    {
        ...
        for (i = 0; lib->constraints[i].name; i++)
            if (optout(sp,
                       lib->constraints[i].name,
                       NiL,
                       NiL,
                       NiL,
                       lib->constraints[i].description,
                       NiL))
                return -1;
        sfprintf(sp, "}\n");
    }
