                  ...
                  Cxdisc_t *disc)
{
    ...
    if (!cxisstring(pt)) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: match requires %s pattern", st->name, st->name);
        return 0;
    }
                ...
                Cxdisc_t *disc)
{
    ...
    if (CXDETAILS(details, format, type, 0))
        ...
        if (format) {
            if (format->width) {
                if (format->fill > 0)
                    *f++ = format->fill;
                f
                += sfsprintf(f, sizeof(fmt) - (f - fmt), "%d", format->width);
            }
            *f++ = 'l';
            *f++ = 'l';
        ...
        *f = 0;
        ...
        n = sfsprintf(
        buf, size, "%#.*I*g", w, sizeof(value->number), value->number);
        if (n != w) {
            w += w - n;
            n = sfsprintf(
            buf, size, "%#.*I*g", w, sizeof(value->number), value->number);
        ...
        n = sfsprintf(
        buf, size, "%1.15I*g", sizeof(value->number), value->number);
    if (n < 0)
        return -1;
    if (n > size)
                ...
                Cxdisc_t *disc)
{
    if (format && format->width) {
        ...
        if (format->width <= value->string.size)
            ...
            memset(buf + value->string.size,
                   format->fill >= 0 ? format->fill : 0,
                   format->width - value->string.size);
        }
        return format->width;
    } else {
                ...
                Cxdisc_t *disc)
{
    ...
    case 'm':
        return base64encode(
        value->buffer.data, value->buffer.size, NiL, buf, size, NiL);
    case 'h':
    case 'x':
        z = value->buffer.size;
    ...
    if (cx->disc->errorf)
        (*cx->disc->errorf)(cx,
                            cx->disc,
                            ERROR_SYSTEM | 2,
                            "%s: unknown buffer representation details",
                            details);
    return -1;
}
...
#define BT(r, n, s, d, e, i, m)                                              \
    {                                                                        \
        s,                                                                   \
        { n, d, CXH, 0, 0, e, i, r, 0, 0, 0, { 0 }, m },                     \
    },
...
= { BT(CX_void,
    ...
    BT(CX_string,
       &name_string[0],
       &state.type_string,
       "A string. The format details string specifies quoting and C style "
       "character escape processing: \bquote\b[=\achar\a] quotes string with "
       "\achar\a (\b\"\b) as the begin and end quote; \bendquote\b=\achar\a "
       "changes the \bquote\b end to \achar\a; \bshell\b[=\abeg\a] quotes "
       "using shell conventions and \abeg\a (\b$'\b) as the quote begin; "
       "\bopt\b performs \bquote\b and \bshell\b quoting only when "
       "necessary; \bescape\b assumes that escape processing has already "
       "been performed; \bwide\b does not escape characters with the bit 8 "
       "set; \bexpand\b=\amask\a expands escaped characters according to "
       "\amask\a which may be a \b,\b or \b|\b separated list of \ball\b: "
       "expand all escaped chars, \bchar\b expands 7 bit character escapes, "
       "\bline\b expands \b\\r\b and \b\\n\b escapes, \bwide\b expands wide "
       "character escapes, \bnocr\b eliminates \b\\r\b, and \bnonl\b "
       "eliminates \b\\n\b.",
       string_external,
       string_internal,
       &match_string) BT(CX_reference,
                         &name_reference[0],
                         &state.type_reference,
                         "A referenced type.",
                         0,
                         0,
                         0)
    BT(CX_buffer,
       &name_buffer[0],
       &state.type_buffer,
       "A separately allocated sized buffer. The external representation is "
       "a newline separated base64 mime encoding.",
       buffer_external,
       buffer_internal,
       0) BT(CX_bool,
             &name_bool[0],
             &state.type_bool,
             "An boolean value: 0==false, 1==true.",
             bool_external,
             bool_internal,
             0) BT(CX_type,
                   &name_type[0],
...
op_call_V(Cx_t *cx,
          ...
          Cxdisc_t *disc)
{
    return (*pc->data.variable->function)(
    cx, pc->data.variable, r, b + (pc->pp + 1), -pc->pp, data, disc);
}
         ...
         Cxdisc_t *disc)
{
    return (r->value.variable
            = cxvariable(cx, b->value.string.data, a->type, disc))
           ? 0
           : -1;
}
...
{
    r->value.number
    = b->value.string.size > 0 && b->value.string.data[0] != 0;
    return 0;
}
        ...
        Cxdisc_t *disc)
{
    ...
    r->value.number
    = (c = strncmp(a->value.string.data,
                   b->value.string.data,
                   CXMIN(a->value.string.size, b->value.string.size)))
      < 0
      || c == 0 && a->value.string.size < b->value.string.size;
    return 0;
}
...
op_eq_S(Cx_t *cx,
        ...
        Cxdisc_t *disc)
{
    r->value.number = a->value.string.size == b->value.string.size
                      && strncmp(a->value.string.data,
                                 b->value.string.data,
                                 a->value.string.size)
                         == 0;
    return 0;
        ...
        Cxdisc_t *disc)
{
    ...
    r->value.number
    = (c = memcmp(a->value.buffer.data,
                  b->value.buffer.data,
                  CXMIN(a->value.buffer.size, b->value.buffer.size)))
      < 0
      || c == 0 && a->value.buffer.size < b->value.buffer.size;
    return 0;
}
...
op_eq_B(Cx_t *cx,
        ...
        Cxdisc_t *disc)
{
    r->value.number = a->value.buffer.size == b->value.buffer.size
                      && memcmp(a->value.buffer.data,
                                b->value.buffer.data,
                                a->value.buffer.size)
                         == 0;
    return 0;
           ...
           Cxdisc_t *disc)
{
    ...
    if (*e && cx->disc->errorf)
        (*cx->disc->errorf)(
        cx, cx->disc, 2, "%s: invalid number", b->value.string.data);
    return 0;
}
...
{
    r->value.number
    = b->value.string.data && b->value.string.size && *b->value.string.data;
    return 0;
}
         ...
         Cxdisc_t *disc)
{
    ...
    if ((i = (*a->type->match->execf)(
         cx, pc->data.pointer, a->type, &a->value, disc))
        < 0)
        return i;
    r->value.number = i == 1;
    return 0;
...
{
    if (!(x->data.pointer
          = (*a->type->match->compf)(cx, a->type, b->type, &b->data, disc)))
        return -1;
    if (a->type->match->freef
        && cxatfree(cx, expr, a->type->match->freef, x->data.pointer)) {
...
cxopen(Cxflags_t flags, Cxflags_t test, Cxdisc_t *disc)
{
    ...
    if (!(vm = vmopen(Vmdcheap, Vmbest, 0))
        || !(em = vmopen(Vmdcheap, Vmlast, 0))
        || !(rm = vmopen(Vmdcheap, Vmlast, 0))) {
        if (vm) {
            vmclose(vm);
            if (em)
                vmclose(em);
    ...
    if (!(cx = vmnewof(vm, 0, Cx_t, 1, 0))
        || !(cx->cvtbuf = vmnewof(vm, 0, char, cx->cvtsiz = CX_CVT, 0))) {
        if (disc->errorf)
            (*disc->errorf)(NiL, disc, ERROR_SYSTEM | 2, "out of space");
        goto bad;
    }
    ...
    if (!(flags & CX_SCOPE)) {
        cx->op = sfstdout;
        if (!(cx->fields = dtnew(cx->vm, &state.listdisc, Dtqueue))
            || !(cx->buf = sfstropen()) || !(cx->tp = sfstropen())) {
            cxclose(cx);
            return 0;
        }
...
cxscope(Cx_t *top, Cx_t *bot, Cxflags_t flags, Cxflags_t test, Cxdisc_t *disc)
{
    ...
    if (top->scoped != 1) {
        if (top->disc->errorf)
            (*top->disc->errorf)(
            NiL, top->disc, 2, "cannot change active scope");
        return 0;
    }
    if (bot) {
        /*
...
cxaddtype(Cx_t *cx, Cxtype_t *type, Cxdisc_t *disc)
{
    ...
    if (dtsearch(dict, type)) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: type already defined", type->name);
        return -1;
    }
    dtinsert(dict, type);
    if (!(type->header.flags & CX_NORMALIZED)) {
        ...
        if ((base = ( char * )type->fundamental)
            ...
            if (disc->errorf)
                (*disc->errorf)(
                NiL, disc, 2, "%s: unknown fundamental type", base);
            return -1;
        }
        if (type->member) {
            if (!type->member->getf) {
            ...
            if (type->header.flags & CX_SCHEMA)
                type->header.flags |= CX_REFERENCED;
                       ...
                       = cx ? dtnew(cx->vm, &state.namedisc, Dtoset)
                            : dtopen(&state.namedisc, Dtoset))) {
                if (disc->errorf)
                    (*disc->errorf)(
                ...
                for (i = 0; member->name; member++) {
                    ...
                    if (!(v->header.flags & CX_NORMALIZED)) {
                        ...
                        if ((base = ( char * )v->type)
                            ...
                            if (disc->errorf)
                                (*disc->errorf)(
                                NiL, disc, 2, "%s: unknown type", base);
                            return -1;
                        }
                        v->member = type;
                    }
...
cxaddcallout(Cx_t *cx, Cxcallout_t *callout, Cxdisc_t *disc)
{
    ...
    if (cx) {
        ...
        *copy = *callout;
        callout = copy;
    } else if (callout->op.code == CX_GET || callout->op.code == CX_SET
               || callout->op.code == CX_DEL || callout->op.code == CX_RET) {
        if (disc->errorf)
            (*disc->errorf)(NiL,
                            disc,
                            2,
                            "%s: callout must be local",
                            cxcodename(callout->op.code));
        return -1;
    } else
        dict = state.callouts;
...
cxaddquery(Cx_t *cx, Cxquery_t *query, Cxdisc_t *disc)
{
    ...
    if (dtsearch(dict, query)) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: query already defined", query->name);
        return -1;
    }
    dtinsert(dict, query);
    return 0;
...
cxaddmap(Cx_t *cx, Cxmap_t *map, Cxdisc_t *disc)
{
    ...
    if (dtsearch(dict, map)) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: map already defined", map->name);
        return -1;
    }
...
cxaddrecode(Cx_t *cx, Cxrecode_t *recode, Cxdisc_t *disc)
{
    ...
    if (cx) {
        ...
        *copy = *recode;
        recode = copy;
    } else if (recode->op.code == CX_GET || recode->op.code == CX_SET
               || recode->op.code == CX_DEL || recode->op.code == CX_RET) {
        if (disc->errorf)
            (*disc->errorf)(NiL,
                            disc,
                            2,
                            "%s: recode must be local",
                            cxcodename(recode->op.code));
        return -1;
    } else
        dict = state.recodes;
...
cxaddedit(Cx_t *cx, Cxedit_t *edit, Cxdisc_t *disc)
{
    ...
    if (dtsearch(dict, edit)) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: edit already defined", edit->name);
        return -1;
    }
    dtinsert(dict, edit);
...
cxedit(Cx_t *cx, const char *data, Cxdisc_t *disc)
{
    ...
    if (isalpha(*data)) {
        ...
        o = e;
        if (!(e = cx ? vmnewof(cx->vm, 0, Cxedit_t, 1, 0)
                     : newof(0, Cxedit_t, 1, 0))) {
            if (disc->errorf)
                (*disc->errorf)(NiL, disc, ERROR_SYSTEM | 2, "out of space");
            return 0;
        }
        ...
        if (!(e = cx ? vmnewof(cx->vm, 0, Cxedit_t, 1, strlen(data) + 1)
                     : newof(0, Cxedit_t, 1, strlen(data) + 1))) {
            if (disc->errorf)
                (*disc->errorf)(NiL, disc, ERROR_SYSTEM | 2, "out of space");
            return 0;
        }
...
cxaddconstraint(Cx_t *cx, Cxconstraint_t *constraint, Cxdisc_t *disc)
{
    ...
    if (dtsearch(dict, constraint)) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: constraint already defined", constraint->name);
        return -1;
    }
    if (!(constraint->header.flags & CX_INITIALIZED)) {
        constraint->header.flags |= CX_INITIALIZED;
        if (constraint->initf
            && !(constraint->data = (*constraint->initf)(constraint, disc)))
            return -1;
    }
...
cxaddvariable(Cx_t *cx, Cxvariable_t *variable, Cxdisc_t *disc)
{
    ...
    if (dtsearch(dict, variable)) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: variable already defined", variable->name);
        return -1;
    }
    if (!(variable->header.flags & CX_NORMALIZED)) {
        ...
        if ((name = ( char * )variable->type)
            ...
            if (disc->errorf)
                (*disc->errorf)(
                NiL, disc, 2, "%s: %s: unknown type", variable->name, name);
            return -1;
        }
    }
    ...
    if (!m || !m->member) {
        if (!(v = ( Cxvariable_t * )dtmatch(cx->variables, name))) {
            if (!(t = strchr(name, '.'))) {
                if (disc->errorf)
                    (*disc->errorf)(
                    NiL, disc, 2, "%s: undefined variable", name);
                return 0;
            }
            ...
            for (;;) {
                ...
                if (!v) {
                    if (disc->errorf) {
                        if (m)
                            ...
                            (*disc->errorf)(
                            NiL, disc, 2, "%s: undefined variable", s);
                    }
                    return 0;
                }
                ...
                if (!(s = t))
                    break;
                if ((!(m = v->type) || !m->member
                     || !(dict = m->member->members))
                    ...
                    if (disc->errorf)
                        ...
                        disc,
                        2,
                        "%s: struct or union variable expected",
                        v->name);
                    return 0;
                }
                t = strchr(t, '.');
       ...
       const char *format)
{
    ...
    if (x.data.variable = var) {
        ...
        if (data) {
            if (!cx->getf
                ...
                if (cx->disc->errorf)
                    ...
                    cx->disc,
                    3,
                    "%s: cx CX_GET callout must be defined",
                    var->name);
                return -1;
            }
            ...
            if (ref = var->reference) {
                ...
                if (var->name != var->type->name
                    && (*cx->getf)(cx, &x, ret, &a, &b, data, cx->disc))
                    return -1;
                while (ref = ref->next) {
                    b.type = x.data.variable->type;
                    x.data.variable = ref->variable;
                    ret->type = x.data.variable->type;
                    if ((*ref->member->getf)(cx,
                                             &x,
                                             ret,
                                             &a,
                                             &b,
        ...
        from = 0;
    if (var && var->format.map && var->format.map->part
        && var->format.map->part->edit)
        cxsuball(cx, var->format.map->part, ret);
    if ((var && (c = var->format.code) || (c = type->format.code))
        && c != CC_NATIVE
        && (CCCONVERT(c)
            || !(type->format.flags & CX_BINARY) && (c = CCOP(c, CC_NATIVE)))
        ...
        if (cxisstring(type)) {
            if (!var || !cxisnumber(from) || !var->format.map || format
                ...
                if (e) {
                    if (!(cvt = cx->cvt)) {
                        if (!cx->top) {
                            if (!(cx->top = vmnewof(cx->vm, 0, Cxbuf_t, 1, 0))
                                || !(cx->top->data
                                     = vmoldof(cx->vm, 0, char, CX_CVT, 0))) {
                                if (cx->disc->errorf)
                                    (*cx->disc->errorf)(NiL,
                                                        cx->disc,
                                                        ERROR_SYSTEM | 2,
                    ...
                    while ((n = (*e)(cx,
                        ...
                        if (!(cx->cvt->data
                              = vmoldof(cx->vm, cx->cvt->data, char, n, 0))) {
                            if (cx->disc->errorf)
                                (*cx->disc->errorf)(NiL,
                                                    cx->disc,
                                                    ERROR_SYSTEM | 2,
                    ...
                    ret->value.string.size
                    = strlen(ret->value.string.data
                             = sfprints("%Lg", val.value.number));
            } else
                ret->value.string.size = strlen(ret->value.string.data = s);
        } else if (var && data)
            goto bad;
        else if (cxisnumber(type) && var && cxisstring(from)
                 && var->format.map
                 && !cxstr2num(cx,
                               &var->format,
    ...
    return 0;
bad:
    if (((x.callout = cxcallout(cx, CX_CAST, ret->type, type, cx->disc))
         || ret->type->fundamental
            && (x.callout = cxcallout(
                cx, CX_CAST, ret->type->fundamental, type, cx->disc)))
        && !(*x.callout)(cx, &x, ret, NiL, &val, data, cx->disc))
        return 0;
    if (cx->disc->errorf)
        (*cx->disc->errorf)(
        cx, cx->disc, 2, "cannot cast %s to %s", ret->type->name, type->name);
    return -1;
}

/*
 * generic struct type externalf that lists defined members in ksh93u compound
 * notation
 */
...
initialize(Cxdisc_t *disc)
{
    ...
    if (!state.initialized++) {
        ...
        table.comparison[CX_LOG] = table.comparison[CX_LT]
        = table.comparison[CX_LE] = table.comparison[CX_EQ]
        = table.comparison[CX_NE] = table.comparison[CX_MATCH]
        = table.comparison[CX_NOMATCH] = table.comparison[CX_GE]
        = table.comparison[CX_GT] = 1;

        table.logical[CX_ANDAND] = table.logical[CX_LOG]
        = table.logical[CX_OROR] = table.logical[CX_NOT] = 1;

        table.precedence[CX_INV] = table.precedence[CX_LOG]
        = table.precedence[CX_NOT] = table.precedence[CX_UMINUS]
        = table.precedence[CX_UPLUS] = 15;
        table.precedence[CX_DIV] = table.precedence[CX_MOD]
        = table.precedence[CX_MPY] = 14;
        table.precedence[CX_ADD] = table.precedence[CX_SUB] = 13;
        table.precedence[CX_LSH] = table.precedence[CX_RSH] = 12;
        table.precedence[CX_GE] = table.precedence[CX_GT]
        = table.precedence[CX_LE] = table.precedence[CX_LT] = 11;
        table.precedence[CX_EQ] = table.precedence[CX_NE]
        = table.precedence[CX_MATCH] = table.precedence[CX_NOMATCH] = 10;
        table.precedence[CX_AND] = 9;
        table.precedence[CX_XOR] = 8;
        table.precedence[CX_AND] = table.precedence[CX_OR] = 7;
...
cxlocation(Cx_t *cx, void *data)
{
    ...
    return cx->disc->locationf
           && (s = (*cx->disc->locationf)(cx, data, cx->disc))
           ? s
           : "";
}
...
cxcvt(Cx_t *cx, const char *s, size_t n)
{
    if (cx->cvtsiz <= n || !cx->cvtbuf) {
        ...
        if (!(cx->cvtbuf
              = vmoldof(cx->vm, cx->cvtbuf, char, cx->cvtsiz, 0))) {
            if (cx->disc->errorf)
                (*cx->disc->errorf)(
                NiL, cx->disc, ERROR_SYSTEM | 2, "out of space");
            return ( char * )s;
