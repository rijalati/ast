                   ...
                   Tagdisc_t *disc)
{
    ...
    if (fp->prev->data) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: only one <NAME> expected", tagcontext(tag, fp));
        return -1;
    }
    if (!(fp->prev->data
          = cxvariable(state->meth->cx, data, NiL, state->disc)))
        return -1;
    return 0;
}

static int
dss_field_map_dat(Tag_t *tag,
                  Tagframe_t *fp,
                  const char *data,
                  Tagdisc_t *disc)
{
    ...
    if (!fp->prev->data) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: <NAME> expected", tagcontext(tag, fp));
        return -1;
    }
...
= { "NAME",
    ...
    0,
    "METHOD",
    "Method name; the same value as for the command"
    " line --method.",
    0,
    0,
    dss_method_dat,
...
static int
map_part_item_name_dat(Tag_t *tag,
                       Tagframe_t *fp,
                       const char *data,
                       Tagdisc_t *disc)
{
...
static int
map_part_item_mask_dat(Tag_t *tag,
                       Tagframe_t *fp,
                       const char *data,
                       Tagdisc_t *disc)
{
...
static int
map_part_item_value_dat(Tag_t *tag,
                        Tagframe_t *fp,
                        const char *data,
                        Tagdisc_t *disc)
{
...
static int
map_part_item_bit_dat(Tag_t *tag,
                      Tagframe_t *fp,
                      const char *data,
                      Tagdisc_t *disc)
{
...
= { "NAME",
    ...
    0,
    0,
    map_part_item_name_dat,
    0,
    "MASK",
    "A mask applied to this item value.",
    0,
    0,
    map_part_item_mask_dat,
    0,
    "VALUE",
    "Map matching item value after masking.",
    0,
    0,
    map_part_item_value_dat,
    0,
    "BIT",
    "The map item is a bit flag at this bit position;"
    " an alternative to an explicit mask and value.",
    0,
    0,
    map_part_item_bit_dat,
    0,
    "MAP",
    "A map applied to the unshifted, unmasked value;"
    " either a map reference name or a map definition.",
    0,
    dss_map_beg,
    dss_map_dat,
    map_part_item_map_end,
    0 };

static Tags_t *
map_part_item_beg(Tag_t *tag,
                  Tagframe_t *fp,
                  const char *name,
                  Tagdisc_t *disc)
{
                  ...
                  Tagdisc_t *disc)
{
    ...
    if (!cxisnumber(type)) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: map values must be numeric", data);
        return -1;
    }
    if (!type->internalf) {
        if (disc->errorf)
...
= { "TYPE",
    "Value type for this part. The base type must be"
    ...
    0,
    "NUM2STR",
    "A number to string ed(1) style substitute expression"
    ...
    0,
    "EDIT",
    "A string to string ed(1) style substitute expression"
    ...
    if (ref = cxmap(NiL, data, state->disc)) {
        if (map->description || map->shift || map->mask || map->part
            ...
            if (disc->errorf)
                ...
                disc,
                2,
                "<%s> must be the first tag inside a <MAP> definition",
                fp->tag->name);
            return -1;
        }
...
dss_map_end(Tag_t *tag, Tagframe_t *fp, Tagdisc_t *disc)
{
    ...
    fp->data = map;
    return map->name && !(map->header.flags & CX_INITIALIZED)
           ? cxaddmap(NiL, map, state->disc)
           : 0;
}
    ...
    if (con->name) {
        if (disc->errorf)
            (*disc->errorf)(NiL,
                            disc,
                            2,
                            "%s: constraint already has a name",
                            tagcontext(tag, fp));
        return -1;
    }
...
static int
constraint_expression_dat(Tag_t *tag,
                          Tagframe_t *fp,
                          const char *data,
                          Tagdisc_t *disc)
{
...
static int
constraint_match_dat(Tag_t *tag,
                     Tagframe_t *fp,
                     const char *data,
                     Tagdisc_t *disc)
{
...
method_end(Tag_t *tag, Tagframe_t *fp, Tagdisc_t *disc)
{
    ...
    if (!name) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: method name expected", tagcontext(tag, fp));
        return -1;
    }
...
dss_con_dat(Tag_t *tag, Tagframe_t *fp, const char *data, Tagdisc_t *disc)
{
    ...
    if (fp->data) {
        if (disc->errorf)
            (*disc->errorf)(
            NiL, disc, 2, "%s: reference not expected", tagcontext(tag, fp));
        return -1;
    }
...
Tags_t dss_tags[] = { "DSS",
                      ...
                      dss_map_end,
                      "METHOD",
                      "Method name; if different from the current method then"
        ...
        Dssmeth_t *meth)
{
    ...
    state.disc = disc;
    state.meth = meth;
    if (!(tag = tagopen(ip, path, line, dss_tags, &state.tagdisc))
        || tagclose(tag))
        state.meth = 0;
