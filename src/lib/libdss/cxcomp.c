...
#else
            if ((c = *cx->include->next++) != '\\'
                || cx->include->next >= cx->include->last)
#endif
                return c;
            if (*cx->include->next == '\r'
                && ++cx->include->next >= cx->include->last)
                break;
            if (*cx->include->next != '\n')
                return c;
...
cxcodename(int code)
{
    ...
    if (i >= (elementsof(name) - 1))
        ...
        case 'S':
            s = stpcpy(
            s, code == CX_CAST ? "CAST" : (code & CX_X2) ? "==" : "SET");
            break;
        case 'e':
            s = stpcpy(s, "END");
            break;
            ...
            Cxoperand_t *sp)
{
    ...
    if (bp) {
        if ((sp - 1)->type == cx->state->type_string)
            ...
            sfsprintf(
            a, sizeof(a), "  [%d]%p", (sp - 1) - bp, (sp - 1)->value.pointer);
        else
            a[0] = 0;
        if (sp->type == cx->state->type_string)
            sfsprintf(
            b, sizeof(b), "  [%d]\"%-.6s\"", sp - bp, sp->value.string.data);
        else if (sp->type == cx->state->type_number)
            sfsprintf(
            b, sizeof(b), "  [%d]%8.4Lf", sp - bp, sp->value.number);
...
/*
 * pop the top file off the include stack until and including matching
 * cxpush() pop pop==0 pops everything 1 returned if there are more items on
 * the stack after the pop 0 returned if the stack is empty after the pop -1
 * returned if the stack is empty before the pop
 */
     ...
     Cxvariable_t *ref)
{
    ...
    x.op = op;
    x.type = cx->table->logical[op]
             ? cx->state->type_bool
             : cx->table->comparison[op] ? cx->state->type_number : type1;
    x.pp = pp;
    if ((cc->pp += pp) > cc->depth)
        cc->depth = cc->pp;
    if (!pointer)
        x.data.number = number;
    else if (op == CX_GET || op == CX_SET || op == CX_REF || op == CX_CALL
             || !cxisstring(type1))
    ...
    if (type1 != type2) {
        ...
        if (type1->fundamental == type2->fundamental
            && (x.callout = cxcallout(
    ...
    if (x.callout = cxcallout(
        cx, op, cx->state->type_void, cx->state->type_void, cx->disc))
        goto done;
    if (cxisbool(type1) && cxisnumber(type2)
        && (x.callout = cxcallout(
            cx, op, cx->state->type_bool, cx->state->type_bool, cx->disc)))
        goto done;
    if (cx->disc->errorf)
        (*cx->disc->errorf)(
        cx,
        cx->disc,
        2,
        "%s %s not supported [%d:%d] %p",
        cxcontext(cx),
        cxopname(op, type1, type2),
        cxrepresentation(type1),
        cxrepresentation(type2),
        cxcallout(
        cx, op, cx->state->type_string, cx->state->type_string, cx->disc));
    return 0;
done:
    if ((cx->flags & CX_DEBUG) && sfstrtell(cc->xp))
        cxcodetrace(
            ...
            if (t = cxtype(cx, buf, cx->disc)) {
                ...
                if (cx->disc->errorf)
                    (*cx->disc->errorf)(cx,
                                        cx->disc,
                                        2,
                                        "%s %s: unknown prototype type name",
                                        cxcontext(cx),
                                        buf);
                r = -1;
            }
                ...
                if (cx->disc->errorf) {
                    if (o & CX_UNARY)
                        (*cx->disc->errorf)(cx,
                                            cx->disc,
                                            2,
                                            "%s operand expected [o=%04x]",
                                            cxcontext(cx),
                                            o);
                    else
                        (*cx->disc->errorf)(
                        cx, cx->disc, 2, "%s unknown operator", cxcontext(cx));
                }
                goto bad;
            }
            if (o & CX_UNARY) {
            ...
            if (o != CX_SET
                && ((o & CX_UNARY)
                    ? !code(cx,
                            ...
                            0,
                            NiL)
                    : !code(cx, cc, expr, o, -1, t, cc->type, NiL, 0, h)))
                goto bad;
            ...
            if (v->function) {
                ...
                if (c != p && c != q) {
                    ...
                    if (c != p && c != q) {
                        if (cx->disc->errorf)
                            ...
                            cx->disc,
                            2,
                            "%s missing %s in formal argument list",
                            cxcontext(cx),
                ...
                if (r > 0 && !o) {
                    if (cx->disc->errorf)
                        ...
                        cx->disc,
                        2,
                        "%s not enough arguments for %s(%s)",
                        cxcontext(cx),
                        v->name,
                        v->prototype);
                    goto bad;
...
keep:
    ...
    if (x == 2) {
        ...
        if (cxisvoid(v->type))
            goto undefined;
        if (!code(
            cx, cc, expr, CX_GET, 1, v->type, cx->state->type_void, v, 0, NiL))
            goto bad;
...
undefined:
    if (cx->disc->errorf)
        (*cx->disc->errorf)(
        cx, cx->disc, 2, "%s undefined variable", cxcontext(cx));
    goto bad;
operator_expected:
    if (cc->collecting)
        goto done;
    if (cx->disc->errorf)
        (*cx->disc->errorf)(
        cx, cx->disc, 2, "%s operator expected", cxcontext(cx));
    goto bad;
operand_expected:
    if (cx->disc->errorf)
        (*cx->disc->errorf)(
        cx, cx->disc, 2, "%s operand expected", cxcontext(cx));
bad:
    cx->error = cxtell(cx);
    if (precedence)
        back(cx);
    ...
    if (cx->disc->errorf) {
        if (c)
            ...
            (*cx->disc->errorf)(cx,
                                cx->disc,
                                2,
                                "syntax error: unterminated expression: %s",
                                cxcontext(cx));
    }
    return 0;
}
        ...
        if (expr->group) {
            ...
            while (pc->op != CX_END) {
                sfprintf(sp,
                         ...
                         cxcodename(pc->op),
                         pc->type->name,
                         pc->pp > 0 ? pc->pp : pc->pp - 1);
                if (pc->op == CX_GET || pc->op == CX_SET || pc->op == CX_REF
                    || pc->op == CX_CALL)
                    sfprintf(sp, "%s", pc->data.variable->name);
                else
                    sfprintf(
                    sp, "%I*g", sizeof(pc->data.number), pc->data.number);
                sfputc(sp, ';');
                pc++;
            }
            sfputc(sp, ')');
...
cxcomp(Cx_t *cx)
{
    ...
    error(-1,
          "cxcomp push include=%p file=%s eof=%d",
          cx->include,
          cx->include ? cx->include->file : 0,
          cx->eof);
    if (cx->eof || !cx->include && (cx->eof = 1))
        return 0;
    expr = 0;
    if (!(cc = vmnewof(cx->vm, 0, Cxcompile_t, 1, 0))
        || !(cc->tp = sfstropen()) || !(cc->xp = sfstropen()))
        goto nospace;
    cx->error = 0;
    cc->reclaim
    = !!(cx->deletef = cxcallout(
         cx, CX_DEL, cx->state->type_void, cx->state->type_void, cx->disc));
    cx->returnf = cxcallout(
...
nospace:
    ...
    error(-1,
          "cxcomp done include=%p file=%s eof=%d",
          cx->include,
          cx->include ? cx->include->file : 0,
          cx->eof);
    return expr;
