...
#define _TAG_FRAME_PRIVATE_ Tagframe_t *next;
...
typedef int (
*Builtin_f)(Tag_t *, Tagframe_t *, Tags_t *, Tags_t *, Tagdisc_t *);
        ...
        do {
            ...
            for (p = col; p && *s; p = p->next) {
                ...
                if (*s)
                    *s++ = 0;
                if (p->tag->begf
                    && !(*p->tag->begf)(tag, &frame, tp->name, disc))
                    goto done;
                if (*b && p->tag->datf
                    && (*p->tag->datf)(tag, &frame, b, disc))
                    goto done;
