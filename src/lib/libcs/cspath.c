...
#endif
            if (*entry->d_name == '.'
                || D_NAMLEN(entry) + (base - path) + 1 > size)
                continue;
    ...
    else if (!ioctl(fd, TCPGETADDR, &tcp))
    {
        if (tcp.raddr)
            sfsprintf(state->path,
                      sizeof(state->path),
                      "/dev/tcp/%s/%d.%d",
                      (flags & CS_PATH_NAME) ? csname(state, tcp.raddr)
        ...
        else
        {
            ...
            sfsprintf(state->path,
                      sizeof(state->path),
                      "/dev/tcp/%s/%d",
                      (flags & CS_PATH_NAME) ? csname(state, tcp.laddr)
                                             : csntoa(state, tcp.laddr),
                      ntohs(tcp.lport));
        }
    }
#endif
