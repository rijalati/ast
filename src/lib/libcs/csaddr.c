...
realaddr(Cs_t *state, const char *name)
{
    ...
    if (streq(s, CS_HOST_LOCAL)) {
        messagef((state->id,
                  NiL,
                  -8,
                  "realaddr(%s) = %s, flags = |%s%s%s",
                  name,
                  csntoa(state, local),
                  (state->flags & CS_ADDR_LOCAL) ? "LOCAL|" : "",
                  (state->flags & CS_ADDR_REMOTE) ? "REMOTE|" : "",
                  (state->flags & CS_ADDR_SHARE) ? "SHARE|" : ""));
        return local;
    }
...
ok:
    ...
    if (addr == local) {
        ...
        if (!(flags & CS_ADDR_REMOTE) && !streq(name, CS_HOST_PROXY)
            && !csattr(state, name, "*"))
            flags |= CS_ADDR_REMOTE;
    }
    ...
    if (sp)
        sfclose(sp);
    state->flags
    &= ~(CS_ADDR_LOCAL | CS_ADDR_NOW | CS_ADDR_REMOTE | CS_ADDR_SHARE
         | CS_DAEMON_SLAVE | CS_ADDR_TEST | CS_ADDR_TRUST);
    state->flags |= flags;
    messagef((state->id,
              NiL,
              -8,
              "addr(%s) = %s, flags = |%s%s%s",
              aname,
              csntoa(state, addr),
              (state->flags & CS_ADDR_LOCAL) ? "LOCAL|" : "",
              (state->flags & CS_ADDR_REMOTE) ? "REMOTE|" : "",
