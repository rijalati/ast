         ...
         unsigned int flags)
{
    ...
    if (flags & CS_CLIENT_RAW)
    {
        ...
        state.new_term.c_iflag
        &= ~(BRKINT | IGNPAR | PARMRK | INLCR | IGNCR | ICRNL);
    ...
    if (promptlen = (!argv && prompt && isatty(fds[0].fd) && isatty(1))
                    ? strlen(prompt)
                    : 0)
        ...
        for (i = 0; i < elementsof(fds); i++)
            if (fds[i].status & (CS_POLL_READ | CS_POLL_WRITE))
            {
                ...
                if (write(sdf[i], s, n) != n)
                    error(
                    ERROR_SYSTEM | 3, "/dev/fd/%d: write error", sdf[i]);
