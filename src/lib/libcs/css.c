...
cssopen(const char *path, Cssdisc_t *disc)
{
    ...
    if (path && strchr(path, '/')) {
            ...
            if (css->disc->errorf) {
                if (errno == EEXIST)
                    ...
                    (*css->disc->errorf)(css,
                                         css->disc,
                                         ERROR_SYSTEM | 3,
                                         "%s: cannot create connect stream",
                                         path);
            }
            goto bad;
        }
        ...
        if ((css->disc->flags & CSS_AUTHENTICATE)
            ...
            if (css->disc->errorf)
                ...
                css->disc,
                ERROR_SYSTEM | 3,
                "%s: cannot create authentication directory",
                css->service);
            goto bad;
        }
    }
        ...
        if (stat(css->mount, &st)) {
            if (css->disc->errorf)
                (*css->disc->errorf)(css,
                                     css->disc,
                                     3,
                                     "%s: cannot stat service directory",
                                     css->mount);
            goto bad;
        }
        if ((css->disc->flags & CSS_DAEMON) && chdir(css->mount)) {
            if (css->disc->errorf)
                (*css->disc->errorf)(css,
                                     css->disc,
                                     3,
                                     "%s: cannot set service directory",
                                     css->mount);
            goto bad;
        }
        *s = '/';
        ...
        *s = 0;
        if (lstat(css->mount, &st)) {
            if (css->disc->errorf)
                ...
                css->disc,
                3,
                "%s: invalid global authentication directory",
                css->mount);
            goto bad;
        }
        *s = '/';
        ...
        if (lstat(s, &st)
            ...
            if (css->disc->errorf)
                ...
                css->disc,
                3,
                "%s: invalid local authentication directory",
                s);
            goto bad;
        }
            ...
            *css->control = CS_MNT_AUTH;
            ...
            if (close(
                ...
                if (css->disc->errorf)
                    ...
                    css->disc,
                    3,
                    "%s: cannot create service authentication file",
                    css->mount);
                goto bad;
            }
    ...
    cssfd(css,
          css->fd,
          type == 'u' ? CS_POLL_READ
                      : (CS_POLL_AUTH | CS_POLL_CONNECT | CS_POLL_READ));
    CSTIME();
    return css;
bad:
    cssclose(css);
...
cssclose(Css_t *css)
{
    ...
    static char mnt[]
    = { CS_MNT_AUTH, CS_MNT_LOCK, CS_MNT_PROCESS, CS_MNT_STREAM };
        ...
        for (n = 0;; n++) {
            ...
            show:
                if (error_info.trace <= -9) {
                    ...
                    NiL,
                    -9,
                    "cssfd: pending=%d polling=%d next=%d fd=%d op=0x%08lx",
                    state.fdpending,
                    state.fdpolling,
                    state.fdnext,
                    ofd,
                    ...
                    for (n = 0; n < state.fdpolling; n++)
                        errorf(state.main->id,
                               NiL,
                               -9,
                               "cssfd: index=%d fd=%d events=%06o actionf=%p",
                               n,
                               state.fdpoll[n].fd,
                               state.fdpoll[n].events,
                               state.fdinfo[state.fdpoll[n].fd].actionf);
                }
#endif
                return ip;
            }
    ...
    for (;;) {
                ...
                if (css->disc->exceptf) {
                    ...
                    if (css->disc->wakeup) {
                        messagef((state.main->id,
                                  NiL,
                                  -4,
                                  "csspoll: pending=%d polling=%d z=%I*d "
                                  "wakeup=%I*d remain=%I*d",
                                  state.fdpending,
                                  state.fdpolling,
                                  sizeof(z),
                                  z,
                                  sizeof(css->disc->wakeup),
                                  css->disc->wakeup,
        ...
        do {
            ...
            (state.main->id,
             NiL,
             -9,
             "csspoll: pending=%d polling=%d next=%d fd=%d status=%06o",
             state.fdpending,
             state.fdpolling,
             state.fdnext,
             fd,
            ...
            case CS_POLL_CONNECT | CS_POLL_READ:
                if (csrecv(css->state, fd, &id, &fdnew, 1) == 1
                    ...
                    && css->disc->acceptf
                    && (fdnew
                        = (*css->disc->acceptf)(css, ip, &id, NiL, css->disc))
                       != ip->fd)
                    cssfd(css,
                          ip->fd,
                          fdnew >= 0 ? CS_POLL_CMD(fdnew, CS_POLL_MOVE)
                                     : CS_POLL_CLOSE);
                break;
            case CS_POLL_AUTH | CS_POLL_READ:
                ...
                case 2:
                    ...
                    *s = '/';
                    ...
                    if (lstat(css->buf, &st)) {
                        ...
                        *s = 0;
                        close(
                        open(css->buf, O_WRONLY | O_CREAT | O_TRUNC, 0));
                        remove(css->buf);
                        *s = '.';
                        if (lstat(css->buf, &st))
                            goto reject;
                ...
                ok:
                    ...
                    if (css->disc->acceptf) {
                        ...
                        ap->id.gid = st.st_gid;
                        vp = va;
                        while ((vp < &va[elementsof(va) - 1]) && (*vp++ = t)
                               && (t = strchr(t, ' ')))
                            *t++ = 0;
                        *vp = 0;
                        if ((fdnew
                            ...
                            cssfd(css,
                                  ip->fd,
                                  fdnew >= 0
                                  ? CS_POLL_CMD(fdnew, CS_POLL_MOVE)
                                  : CS_POLL_CLOSE);
                    }
                    break;
                default:
            ...
            case CS_POLL_READ:
                ...
                messagef((state.main->id,
                          NiL,
                          -9,
                          "csspoll: status=%06o fd=%d actionf=%p",
                          status,
                          fd,
                          ip->actionf));
                if (!ip->actionf
