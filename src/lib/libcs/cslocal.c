...
initiate(Cs_t *state, const char *svc, char *cmd)
{
    ...
    children = 0;
    if ((fun = signal(SIGCHLD, child)) == SIG_DFL)
        signal(SIGCHLD, fun);
    else if (children)
        children++;
#endif
...
cslocal(Cs_t *state, const char *path)
{
    ...
    for (;;) {
        ...
        if (!p) {
            messagef((state->id,
                      NiL,
                      -1,
                      "local: %s: %s: cannot locate service on ../lib/cs/fdp",
                      path,
                      cmd));
            goto sorry;
        }
    ...
    *t = 0;
    ...
    for (n = 0; (fd = open(tmp, O_RDWR)) < 0; n++)
        if (n || errno == EACCES) {
            ...
            messagef((state->id,
                      NiL,
                      -1,
                      "local: %s: %s: cannot initiate service %s",
                      path,
                      tmp,
                      exe));
            return -1;
    ...
    messagef((state->id,
              NiL,
              -8,
              "local(%s) fd=%d server=%s stream=%s",
              path,
              fd,
              exe,
              tmp));
...
#        if CS_LIB_SOCKET_RIGHTS
                if (read(fd, cmd, 1) != 1)
                    messagef((state->id,
                              NiL,
                              -1,
                              "local: %s: connect ack read error",
                              path));
                else if (cssend(state, fd, NiL, 0))
                    messagef((state->id,
                              NiL,
                              -1,
                              "local: %s: connect authentication send error",
                              path));
                else
#        endif
                    return fd;
