        ...
        int (*to)(void *))
{
    ...
    server->disc.flags = CSS_DAEMON | CSS_LOG | CSS_CLOSE | CSS_ERROR
                         | CSS_INTERRUPT | CSS_TIMEOUT | CSS_WAKEUP;
    ...
    state->control
    = state->mount + (server->css->control - server->css->mount);
