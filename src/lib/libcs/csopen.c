    ...
    if (eaccess(state->mount, R_OK | W_OK | X_OK)) {
        ...
        if (eaccess(state->mount, X_OK)) {
            ...
            *endhost = 0;
            if (eaccess(state->mount, X_OK)) {
                ...
                *endtype = 0;
                if (eaccess(state->mount, X_OK)
                    && (mkdir(state->mount, S_IRWXU | S_IRWXG | S_IRWXO)
                        || chmod(state->mount, S_IRWXU | S_IRWXG | S_IRWXO)))
                    goto bad;
                *endtype = '/';
                if (mkdir(state->mount, S_IRWXU | S_IRWXG | S_IRWXO)
                    || chmod(state->mount, S_IRWXU | S_IRWXG | S_IRWXO))
                    goto bad;
            }
            *endhost = '/';
        ...
        *endserv = '/';
        ...
        if (mode != (S_IRWXU | S_IRWXG | S_IRWXO)
            && (uid >= 0 || gid >= 0 && (mode |= S_ISGID))
            && (chown(state->mount, uid, gid) || chmod(state->mount, mode))) {
            rmdir(state->mount);
            goto bad;
        }
    }
...
bad:
    ...
    *(state->control - 1) = '/';
    messagef((state->id,
              NiL,
              -1,
              "mkmount: %s: cannot access physical mount directory",
              state->mount));
    return -1;
}
    ...
    while (s = sfgetr(sp, '\n', 1))
        if (tokscan(s, NiL, " %s ", &on) == 1) {
            if (streq(on, CS_HOST_LOCAL) || streq(on, local)) {
                ...
                av[1] = path;
                av[2] = 0;
                return procclose(procopen(
                       av[0], av, NiL, NiL, PROC_PRIVELEGED | PROC_ZOMBIE))
                       < 0
                       ? -1
                       : 0;
                ...
                ov[2] = 0;
                if (proc = procopen(
                    av[0], av, NiL, ov, PROC_PRIVELEGED | PROC_ZOMBIE)) {
                    n = 1;
                    for (;;) {
                        if (!lstat(state->mount, &st)
                            && st.st_ctime != otime) {
                            if (!procclose(proc)) {
                                sfclose(sp);
                                return 0;
...
agent(Cs_t *state, const char *host, const char *user, const char *path)
{
    ...
    if (!(proc = procopen(
          av[0], av, NiL, NiL, PROC_READ | PROC_WRITE | PROC_PRIVELEGED)))
        goto sorry;
    ...
    if (m) {
        ...
        if (*num != '\n') {
            ...
            n = sfsprintf(tmp,
                          ...
                          "%s %s\n",
                          num,
                          (s = strchr(buf, ' ')) ? (s + 1) : buf);
            if (cswrite(state, proc->wfd, tmp, n) != n)
                goto sorry;
            if ((m = csread(state, proc->rfd, buf, sizeof(buf), CS_LINE))
                <= 0)
                goto sorry;
            if (cswrite(state, fd, buf, m) != m)
                goto sorry;
         ...
         char *b)
{
    ...
    if (op & CS_OPEN_CREATE) {
        ...
        if (pipe(fds)) {
            messagef(
            (state->id, NiL, -1, "open: %s: pipe error", state->path, path));
            return -1;
        }

#    if CS_LIB_V10

        if ((n = ioctl(fds[1], FIOPUSHLD, &conn_ld))
            || fmount(3, fds[1], path, 0))
        ...
        for (;;) {
            ...
            if (initiate(state, user, opath, tmp, serv)) {
                messagef((state->id,
                          NiL,
                          -1,
                          "open: %s: %s: service initiate error",
                          state->path,
                          path));
                return -1;
            }
...
#    if CS_LIB_SOCKET_UN
    ...
    messagef((state->id,
              NiL,
              -8,
              "%s:%d state.path=`%s' state.mount=`%s' path=`%s' opath=`%s' "
              "user=`%s' serv=`%s'",
              __FILE__,
              __LINE__,
              state->path,
...
#        endif
            ...
            if (errno == EACCES)
                ...
                if (pid > 0 && (s = strrchr(state->temp, '/'))
                    && (pid = strtol(s + 1, NiL, 0)) > 0) {
...
#    else
    ...
    messagef(
    (state->id, NiL, -1, "open: %s: %s: not supported", state->path, path));
    fd = -1;
...
csopen(Cs_t *state, const char *apath, int op)
{
    ...
    if (pathgetlink(path, buf, sizeof(buf)) <= 0) {
        ...
        if (tokscan(tmp, NiL, "/dev/%s/%s/%s", &type, NiL, &serv) == 3)
            sfsprintf(
            buf, sizeof(buf), "/dev/%s/%s/%s", type, csntoa(state, 0), serv);
    }
    path = buf;
    pathcanon(path, 0, 0);
    errno = ENOENT;
    ...
    *s++ = 0;
    ...
    if (streq(s, "fdp")) {
        ...
        messagef(
        (state->id, NiL, -1, "open: %s: %s: invalid type", state->path, s));
        return -1;
    }
        ...
        errno = ENODEV;
        messagef(
        (state->id, NiL, -1, "open: %s: %s: not supported", state->path, s));
        return -1;
    }
#endif
    type = s;
    ...
    messagef((state->id,
              NiL,
              -8,
              "open: type=%s host=%s serv=%s qual=%s",
              type,
              host,
              serv,
              qual));
    if (host) {
        ...
        if (!(state->addr = addr = csaddr(state, host))) {
            if (serv && !(op & CS_OPEN_CREATE) && *type == 't'
                && (port = csport(state, type, serv)) >= CS_PORT_MIN
                && port <= CS_PORT_MAX) {
                /*
                ...
                if ((fd
                     = state->proxy.addr
                       ? csbind(
                         state, type, state->proxy.addr, state->proxy.port, 0L)
                       : reopen(state, csvar(state, CS_VAR_PROXY, 0)))
                    >= 0) {
                    ...
                    n = sfsprintf(tmp,
                                  ...
                                  port,
                                  csname(state, 0),
                                  error_info.id ? error_info.id : state->id);
                    if (cswrite(state, fd, tmp, n) == n
                        && (n = csread(state, fd, tmp, sizeof(tmp), CS_LINE))
                           >= 2) {
                        if (tmp[0] == '0' && tmp[1] == '\n')
                            return fd;
        ...
        if ((state->flags & CS_ADDR_REMOTE)
            && (!serv
                || !strneq(serv, CS_SVC_INET, sizeof(CS_SVC_INET) - 1)
                   && (strtol(serv, &t, 0), *t)))
        ...
        if (state->flags & CS_ADDR_SHARE)
            ...
            if (serv && !qual && *type != 'f'
                && (port = csport(state, type, serv)) != CS_PORT_INVALID) {
                if (op & CS_OPEN_CREATE)
                    addr = 0;
    ...
    if (*type == 'f') {
        ...
        if ((op & CS_OPEN_CREATE) && eaccess(path, X_OK)
            && (mkdir(path, S_IRWXU | S_IRWXG | S_IRWXO)
                || chmod(path, S_ISVTX | S_IRWXU | S_IRWXG | S_IRWXO)))
            goto bad;
    } else {
        if (op & CS_OPEN_TRUST) {
            if (!pathaccess(csvar(state, CS_VAR_TRUST, 1),
                            csvar(state, CS_VAR_SHARE, 1),
                            NiL,
                            PATH_EXECUTE,
                            b,
                            sizeof(state->mount) - (b - state->mount)))
                goto bad;
        } else if (!pathpath(csvar(state, CS_VAR_SHARE, 0),
                             "",
                             PATH_EXECUTE,
                             b,
                             sizeof(state->mount) - (b - state->mount)))
    ...
    if (!host) {
        *(state->control = b + 1) = 0;
        ...
        if ((fd = open(path, O_RDONLY)) < 0) {
            mkmount(
            state, S_IRWXU | S_IRWXG | S_IRWXO, -1, -1, NiL, NiL, NiL);
    ...
    sfsprintf(b,
              ...
              serv,
              CS_SVC_SUFFIX);
    if (!pathpath(b, "", PATH_ABSOLUTE | PATH_EXECUTE, tmp, sizeof(tmp))
        || stat(tmp, &st))
        op |= CS_OPEN_TEST;
    else {
        *strrchr(tmp, '/') = 0;
    ...
    *b = CS_MNT_STREAM;
    ...
    messagef(
    (state->id, NiL, -8, "%s:%d %s", __FILE__, __LINE__, state->mount));
    state->control = b;
    ...
    if ((op & CS_OPEN_CREATE)
        && mkmount(state, mode, uid, gid, endserv, endhost, endtype))
        goto bad;
    mode &= S_IRWXU | S_IRWXG | S_IRWXO;
    if (nfd >= 0) {
        close(nfd);
    ...
    if (*type == 'f') {
        ...
        if ((fd = doattach(state, path, op, mode, user, opath, tmp, serv, b))
            < 0)
            return -1;
    } else {
        /*
            ...
            if ((fd = open(path, O_WRONLY | O_CREAT | O_TRUNC, 0)) < 0) {
                ...
                if ((CSTIME() - ( unsigned long ) st.st_ctime) < 2 * 60) {
                    ...
                    messagef((state->id,
                              NiL,
                              -1,
                              "open: %s: %s: another server won the race",
                              state->path,
                              path));
                    goto unblock;
                }
            ...
            *state->control = CS_MNT_STREAM;
            if (fd < 0)
                return -1;
                   ...
                   && ((op & CS_OPEN_TEST)
                       || initiate(state, user, opath, tmp, serv)
                       || (fd = reopen(state, path)) < 0)) {
            messagef((state->id,
                      NiL,
                      -1,
                      "open: %s: %s: reopen/initiate error",
                      state->path,
                      path));
            return -1;
