...
#            if CS_LIB_SOCKET_RIGHTS
            ...
            if (fstat(rfd[0], &st)) {
                messagef((state->id,
                          NiL,
                          -1,
                          "recv: %d: %d: authentication stat error",
                          fd,
                          rfd[0]));
            drop:
                close(rfd[0]);
            ...
            if ((st.st_mode & CS_AUTH_MASK) != CS_AUTH_MODE) {
                messagef((state->id,
                          NiL,
                          -1,
                          "recv: %d: %d: invalid authentication mode %04o",
                          fd,
                          rfd[0],
                          st.st_mode & CS_AUTH_MASK));
                goto drop;
            }
            close(rfd[0]);
#            else
        ...
        for (i = j = 0; i < n; i++) {
            ...
            if (!stat(s, &st)
                && (fds[i] = open(
                    ...
                    ? O_RDWR
                    : (st.st_mode & S_IWUSR) ? O_WRONLY : O_RDONLY))
                   >= 0)
                j++;
        }
        if (id) {
