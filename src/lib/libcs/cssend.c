...
#        endif
            ...
            || chmod(s, S_ISVTX | S_IRWXU | S_IRWXG | S_IRWXO)))
    {
        messagef((state->id,
                  NiL,
                  -1,
                  "send: %d: %s: invalid authentication directory ",
                  fd,
                  s));
        return -1;
    }
        ...
        || chmod(s, CS_AUTH_MODE) < 0)
    {
        messagef((state->id,
                  NiL,
                  -1,
                  "send: %d: %s: authentication file creat error",
                  fd,
                  s));
        return -1;
    }
    if (remove(s))
    {
        messagef((state->id,
                  NiL,
                  -1,
                  "send: %d: %s: authentication file remove error",
                  fd,
                  s));
        close(i);
        errno = EPERM;
...
#        ifdef EMSGSIZE
    n = 0;
    while ((i = sendmsg(fd, &msg, 0) < 0 ? -1 : 0) < 0 && errno == EMSGSIZE
           && n++ < 5)
        sleep(1);
#        else
...
#        endif
    if (i)
        messagef((state->id, NiL, -1, "send: %d: sendmsg error", fd));
    close(ctl.fds[0]);
    return i;
...
#    else
    ...
    if (!access(CS_PROC_FD_TST, F_OK))
    {
        ...
        if (i)
            messagef((state->id, NiL, -1, "send: %d: write error", fd));
        return i;
    }
    errno = EINVAL;
