...
#define delflush()                                                           \
    (write(Dfd, Ddata, Dnext - Ddata) >= 0 ? (Dnext = Ddata, 0) : -1)
...
static Move *
makeAdd(char *beg, char *end, Move *last)
{
    Move *ip;
    ...
    while (last)
    {
        ...
        cost_m
        = NBYTE(last->size) + NBYTE(last->addr) + NBYTE(ip->size) + ip->size;
...
static int
chkMove(long m_size, long m_addr, long a_size)
{
    int cost_m, cost_a;
...
delta(char *src, long n_src, char *tar, long n_tar, int delfd)
{
    ...
    while (n_tar > 0)
    {
        ...
        if (tree)
            ...
            size = mtchstring(src, n_src, tar, n_tar, &match);
        if (size < 0)
            return -1;
        if (size > 0)
            size = chkMove(
