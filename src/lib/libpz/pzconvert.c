        ...
        const char *to)
{
    ...
    if (m >= k) {
        if (pz->disc->errorf)
            ...
            pz->disc,
            2,
            "internal closure error -- %s not found on unwind stack",
            to);
        return 0;
    }
found:
...
chain(Pz_t *pz, Pzconvert_t *tab, int n, const char *f, const char *t)
{
    ...
    if (!j || !(cp = closure(pz, NiL, n, tab, hit, stk, 0, j, t))) {
        ...
        if (pz->disc->errorf) {
            if (!j)
                ...
                pz->disc,
                2,
                "conversion from %s to %s not implemented",
                f,
                t);
        }
        return 0;
    ...
    if (m) {
        ...
        if ((state->readf || state->writef)
            && !(state->buf = vmnewof(pz->vm, 0, unsigned char, m, 0)))
            return 0;
    }
    return cp;
}
...
cvtevent(Pz_t *pz, int op, void *data, size_t size, Pzdisc_t *disc)
{
        ...
        if ((pz->flags & PZ_READ)
            ...
            if (disc->errorf)
                (*disc->errorf)(pz,
                                disc,
                                1,
                                "%s: no checksum -- expected 0x%08I*x",
                                pz->path,
                                sizeof(cvt->checksum),
                                cvt->checksum);
        }
    ...
    case PZ_OPTION:
        ...
        case 'x':
            ...
            if (disc->errorf && (pz->flags & PZ_NOPZIP)
                && (state->flags & CHECKSUM_WARN))
                (*disc->errorf)(
                pz, disc, 1, "%s: enabled for pzip data only", opt_info.name);
            break;
        case 'c':
        case 'f':
        case 't':
            ...
            if (opt_info.arg && *opt_info.arg) {
                for (xp = state->conversions;; xp++) {
                    ...
                    if (streq(opt_info.arg, xp->from->name)
                ...
                if (sfraise(pz->io, SFPZ_HANDLE, &iz) <= 0) {
                    if (disc->errorf)
                        (*disc->errorf)(pz,
                                        disc,
                                        2,
                                        "%s: cannot determine input format",
                                        pz->path);
                    return -1;
                }
