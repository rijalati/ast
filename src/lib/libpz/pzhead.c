...
pzheadread(Pz_t *pz)
{
    ...
    if (pz->disc->errorf)
        ...
        pz->disc,
        -1,
        "%s: pzheadread: f=%08x i=%02x n=%02x partition=%s%s",
        pz->path,
        pz->flags,
        i,
        n,
        ...
        if (!(pz->flags & (PZ_READ | PZ_WRITE | PZ_STAT))
            ...
            if (pz->prefix.terminator >= 0) {
                while (m-- > 0) {
                    if (!sfgetr(pz->io, pz->prefix.terminator, 0)) {
                        if (pz->disc->errorf)
                            ...
                            pz->disc,
                            2,
                            "%s: cannot read %I*u prefix record%s from data",
                            pz->path,
                            sizeof(pz->prefix.count),
                            pz->prefix.count,
                            pz->prefix.count == 1 ? "" : "s");
                        return -1;
                    }
                ...
                if (pz->disc->errorf)
                    ...
                    pz->disc,
                    2,
                    "%s: cannot read %I*u prefix byte%s from data",
                    pz->path,
                    sizeof(pz->prefix.count),
                    pz->prefix.count,
                    pz->prefix.count == 1 ? "" : "s");
...
noway:
    if (pz->disc->errorf)
        ...
        pz->disc,
        2,
        "%s: data %d.%d not supported by implementation %d.%d",
        pz->path,
        pz->major,
        pz->minor,
        PZ_MAJOR,
