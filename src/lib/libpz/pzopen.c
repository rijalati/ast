...
pzopen(Pzdisc_t *disc, const char *path, unsigned long flags)
{
    ...
    else
    {
        ...
        if (!(pz = vmnewof(vm, 0, Pz_t, 1, 0)) || !(pz->tmp = sfstropen())
            || !(pz->str = sfstropen())
            || (flags & PZ_WRITE) && !(pz->det = sfstropen()))
            goto bad;
        memfatal(NiL);
        tvgettime(&pz->start);
        pz->major = PZ_MAJOR;
        ...
        if (disc->initf && !(*disc->initf)(pz, disc))
            goto bad;
        if (disc->options
            && (!(pz->options = vmstrdup(pz->vm, disc->options))
    ...
    if (pz->disc->eventf
        && (*pz->disc->eventf)(
           pz, (flags & PZ_AGAIN) ? PZ_REOPEN : PZ_OPEN, NiL, 0, pz->disc)
           < 0)
        goto bad;
    if (pz->disc->errorf)
        (*pz->disc->errorf)(pz,
                            pz->disc,
                            -1,
                            "%s: pzopen: flags=%08x win=(%I*u:%I*u) "
                            "row=(%I*u:%I*u) buf=%p wrk=%p",
                            pz->path,
                            pz->flags,
                            sizeof(pz->win),
                            pz->win,
        ...
        else if (pz->flags & PZ_POP)
        {
            ...
            if (sferror(pz->io))
            {
                ...
                if (pz->disc->errorf)
                    ...
                    pz->disc,
                    ERROR_SYSTEM | 2,
                    (pz->flags & PZ_WRITE) ? "write error" : "read error");
            }
        }
    }
