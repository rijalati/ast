...
flush(Deflate_t *dp, size_t w, Sfio_t *op)
{
    ...
    if (!use
        && (!(dp->pz->test & 04) || !(file = getenv("_AST_pzip_debug_use"))
            || !(use = strton(file, NiL, NiL, 1))))
        use = 8 * 1024;
    dp->pz->count.windows++;
        ...
        if (n = sfstrtell(ip->sp)) {
            ...
            if (n >= use && (!ip->row || (n / ip->row) > 16)) {
                if (!ip->used++ && !ip->part && !streq(ip->name, "0")
                    ...
                    dp->pz->disc,
                    1,
                    "%s: generate a partition to improve compression",
                    ip->name);
...
deflate(Pz_t *pz, Sfio_t *op)
{
    ...
    if (!(state.buf = sfstropen()) || !(def.xp = sfstropen()))
        goto nospace;
    if (!(def.ids = dtopen(&iddisc, Dtoset))
        || !(def.sqs = dtopen(&sqdisc, Dtoset)))
        goto nospace;
    def.seq = 0;
    ...
    while (rp = (*pz->disc->splitf)(pz, pz->io, pz->disc)) {
        ...
        if (!(ip = ( Id_t * )dtmatch(def.ids, &rp->id))) {
            ...
            if (!(ip->sp = sfstropen())) {
                if (pz->disc->errorf)
                    (*pz->disc->errorf)(pz,
                                        pz->disc,
                                        ERROR_SYSTEM | 2,
                                        "%s: cannot create tmp stream",
                                        ip->name);
                goto bad;
            }
            if ((ip->part = pzpartget(def.pz, ip->name)) && pz->disc->errorf
                && ip->row && ip->part->row != ip->row)
                (*pz->disc->errorf)(pz,
                                    pz->disc,
                                    1,
                                    "%s: partition row %I*u != data row %I*u",
                                    ip->name,
                                    sizeof(ip->part->row),
                                    ip->part->row,
                                    sizeof(ip->row),
            ...
            (*pz->disc->errorf)(pz,
                                pz->disc,
                                1,
                                "%s: size %I*u not a multiple of %I*u",
                                ip->name,
                                sizeof(rp->size),
                                rp->size,
                                sizeof(ip->row),
    ...
    if ((pz->flags & PZ_DUMP) && pz->disc->errorf) {
        ...
        for (ip = ( Id_t * )dtfirst(def.ids); ip;
            ...
            (*pz->disc->errorf)(pz,
                                pz->disc,
                                0,
                                "%8I*u %12s %2u %4I*u %4I*u %12I*u %12I*u%s",
                                sizeof(ip->windows),
                                ip->windows,
                                ip->name,
                                !!ip->part,
                                ...
                                ip->row,
                                sizeof(ip->total),
                                ip->total ? ip->total : ip->modules * ip->row,
                                sizeof(ip->modules),
                                ip->modules,
    ...
    while ((w = sfgetu(pz->io)) && w <= pz->win) {
        ...
        for (i = 0; i < parts; i++) {
            ...
            if (m >= pz->win || ip->row && m % ip->row) {
                if (pz->disc->errorf)
                    ...
                    pz->disc,
                    2,
                    "input corrupted [%s partition table size %I*u]",
                    ip->name,
                    sizeof(m),
                    m);
                goto bad;
        ...
        if (sfread(pz->io, s, u) != u) {
            if (pz->disc->errorf)
                ...
                pz->disc,
                ERROR_SYSTEM | 2,
                "cannot read %I*u byte partition data buffer",
                sizeof(u),
                u);
            goto bad;
        }
        ...
        while (m = sfgetu(pz->io)) {
            if (m > parts) {
                if (pz->disc->errorf)
                    ...
                    pz->disc,
                    2,
                    "input corrupted [partition record index %I*u > %I*u]",
                    sizeof(m),
                    m,
                    sizeof(parts),
                    parts);
    ...
    if (w || sfgetc(pz->io) != EOF) {
        if (pz->disc->errorf)
            (*pz->disc->errorf)(pz,
                                pz->disc,
                                2,
                                "%s: input corrupted [EOF expected at %I*u]",
                                pz->path,
                                sftell(pz->io));
        goto bad;
    }
    ...
    if (!pz->disc->splitf) {
        if (pz->disc->errorf)
            (*pz->disc->errorf)(pz,
                                pz->disc,
                                2,
                                "%s: split discipline library expected",
                                pz->path);
        return -1;
    }
