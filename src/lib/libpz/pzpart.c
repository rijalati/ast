...
pzpartinit(Pz_t *pz, Pzpart_t *pp, const char *name)
{
    ...
    if (pz->win > pz->mwin) {
        if (pz->disc->errorf)
            (*pz->disc->errorf)(pz,
                                pz->disc,
                                -1,
                                "%s: pzpartinit: win=%I*u mwin=%I*u buf=%p",
                                pz->path,
                                sizeof(pz->win),
                                pz->win,
                                sizeof(pz->mwin),
    ...
    if (m && !(pz->flags & PZ_WRITE)
        && !(pz->wrk = vmnewof(pz->vm, pz->wrk, unsigned char, m, 0)))
        return pznospace(pz);
    ...
    if (pz->options && pzoptions(pz, pp, pz->options, 0))
        return -1;
    if (pz->disc->eventf
        && (*pz->disc->eventf)(pz, PZ_PARTITION, pp, 0, pz->disc) < 0)
        return -1;
...
pzoptions(Pz_t *pz, Pzpart_t *pp, char *options, int must)
{
    ...
    for (;;) {
        ...
        default:
            if (pz->disc->errorf)
                (*pz->disc->errorf)(
                pz, pz->disc, -2, "pzoptions: %-.*s", opt_info.offset, s);
            s += opt_info.offset;
            if (skip)
                continue;
            ...
            if (pz->disc->eventf) {
                ...
                pz->part = pp;
                x = (*pz->disc->eventf)(
                pz, PZ_OPTION, opt_info.argv[1] + 2, 0, pz->disc);
                pz->part = opp;
                if (x < 0)
                    return -1;
            } else
...
pzpartition(Pz_t *pz, const char *partition)
{
    ...
    if (s[0] == '/' && s[i = strlen(s) - 1] == '/') {
        ...
        if ((e = strchr(s, '?')) || (e = strchr(s, '#'))) {
            ...
            t[e - s] = 0;
            s = t;
            if (*e == '#' && !(pz->partname = vmstrdup(pz->vm, e + 1))
                || *e == '?' && pzoptions(pz, NiL, e + 1, 1))
                goto bad;
        }
    ...
    if (!(vm = vmopen(Vmdcheap, Vmlast, 0))) {
        if (pz->disc->errorf)
            ...
            pz->disc,
            ERROR_SYSTEM | 2,
            "partition temporary vmalloc region open error");
        goto bad;
    }
    np = 0;
    pp = 0;
    ...
    do {
        ...
        if (*t == '@') {
            ...
            default:
                if (pz->disc->errorf)
                    ...
                    pz->disc,
                    2,
                    "%s: %d: invalid size -- power of 2 from 1..8 expected",
                    s,
                    m);
                goto bad;
            }
        ...
        *cp++ = -1;
        ...
        if (!(pp->ngrp = g))
            pp->ngrp = 1;
        if (!(pp->map
              = vmnewof(pz->vm, 0, size_t, VECTOR(pz, pp, pp->nmap), 0))
            || !(pp->grp
                 = vmnewof(pz->vm, 0, size_t, VECTOR(pz, pp, pp->ngrp), 0)))
            goto nope;
        m = 0;
        cp = cv;
        g = 0;
