...
alias(struct proc *pp, const char *name)
{
    ...
    if (*name != '/' || *pp->fp->rule->name == '/' || !pp->pwd)
        return (0);
...
mamscan(struct mam *mp, const char *file)
{
    ...
    char *ofile;
    int oline;
#if __sparc__ /* strcpy() and strcmp() bug() when '\0' is last byte in last  \
                 page */
    char buf[8096];
#endif
        ...
        else if (streq(op, "info"))
        {
            ...
            else if (streq(arg, "start"))
            {
                ...
                else if (pp->parent = getproc(mp, n))
                {
                    ...
                    if (!ppp->child)
                        ppp->child = pp;
                    else if (!ppp->child->sibling)
                        ppp->child->sibling = ppp->child->stail = pp;
                    else
