...
int Tksh_LindexCmd(clientData, interp, argc, argv) ClientData clientData;
Tcl_Interp *interp;
int argc;
char *argv[];
{
    ...
    if ((index >= 0) && (index < listArgc)) {
        Tcl_SetResult(interp, listArgv[index], TCL_VOLATILE);
    }
    free(( char * ) listArgv);
    return TCL_OK;
}
...
char **argv;        /* Argument strings. */
{
    ...
    if ((namval = nv_open(argv[2], sh.fun_tree, NV_NOADD))
        && namval->nvalue) {
        Tcl_AppendResult(interp,
                         "can't rename to \"",
                         argv[2],
    ...
    if (!(namval = nv_open(argv[1], sh.fun_tree, NV_NOADD))
        || !namval->nvalue) {
        Tcl_AppendResult(interp,
                         "can't rename \"",
                         argv[1],
    ...
    sh_addbuiltin(
    argv[2], ( Shbltin_f ) namval->nvalue, ( void * ) commandData);
    nv_close(namval);
...
int b_TkshSetlistCmd(argc, argv, context) int argc;
char *argv[];
Shbltin_t *context;
{
...
int b_TkshInfoCmd(argc, argv, context) int argc;
char *argv[];
Shbltin_t *context;
{
    char c;
...
static void tksh_appendBltn(nv, data) Namval_t *nv;
void *data;
{
    char *name = nv_name(nv);
    if ((name[0] == 't') && (name[1] == 'c') && (name[2] == 'l')
        && (name[3] == '_'))
        name += 4;
...
static void tksh_appendLocal(nv, data) Namval_t *nv;
void *data;
{
...
static void tksh_appendProc(nv, data) Namval_t *nv;
void *data;
{
    TkshCommandData *commandData;
...
char **argv;        /* Argument strings. */
{
    ...
    if ((c == 'a') && (strncmp(argv[1], "args", length)) == 0) {
        ...
        infoNoSuchProc:
            Tcl_AppendResult(
            interp, "\"", argv[2], "\" isn't a procedure", ( char * ) NULL);
            return TCL_ERROR;
        }
        ...
        for (argPtr = procPtr->argPtr;; argPtr = argPtr->nextPtr) {
            ...
            if (strcmp(argv[3], argPtr->name) == 0) {
                if (argPtr->defValue != NULL) {
                    if (Tcl_SetVar(
                        ( Tcl_Interp * ) iPtr, argv[4], argPtr->defValue, 0)
                        == NULL) {
                    defStoreError:
                        Tcl_AppendResult(
                        interp,
        ...
        if (p == NULL) {
            ...
            if (iPtr->varFramePtr == NULL) {
                ...
                hPtr
                = Tcl_FindHashEntry(&iPtr->varFramePtr->varTable, argv[2]);
            }
            if (hPtr == NULL) {
                goto noVar;
            }
        ...
        if (argc == 2) {
            ...
            if (level <= 0) {
                ...
                levelError:
                    Tcl_AppendResult(
                    interp, "bad level \"", argv[2], "\"", ( char * ) NULL);
                    return TCL_ERROR;
                }
        ...
        return TCL_OK;
               ...
               && (strncmp(argv[1], "nameofexecutable", length) == 0)) {
        if (argc != 2) {
            Tcl_AppendResult(interp,
                             "wrong # args: should be \"",
                             argv[0],
        ...
        value = Tcl_GetVar(
        interp, "tcl_patchLevel", TCL_GLOBAL_ONLY | TCL_LEAVE_ERR_MSG);
        if (value == NULL) {
            return TCL_ERROR;
        }
        interp->result = value;
        ...
        return TCL_OK;
               ...
               && (strncmp(argv[1], "sharedlibextension", length) == 0)
               && (length >= 2)) {
        if (argc != 2) {
            Tcl_AppendResult(interp,
                             "wrong # args: should be \"",
        ...
        value = Tcl_GetVar(
        interp, "tcl_version", TCL_GLOBAL_ONLY | TCL_LEAVE_ERR_MSG);
        if (value == NULL) {
            return TCL_ERROR;
        }
        interp->result = value;
...
#else
                         "exists, globals, hostname, level, library, loaded, "
                         "locals, ",
