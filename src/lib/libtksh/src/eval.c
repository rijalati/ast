...
tksh_command(int argc, char *argv[], Shbltin_t *context)
{
    ...
    result = commandData->info.proc(
    commandData->info.clientData, commandData->interp, argc, argv);
...
#else
            if (commandData->interp->result && (*commandData->interp->result)
...
int
Tcl_SetCommandInfo(Tcl_Interp *interp, char *cmdName, Tcl_CmdInfo *infoPtr)
{
    Namval_t *namval;
    TkshCommandData *commandData;
...
int
Tksh_SetCommandType(Tcl_Interp *interp, char *cmdName, int tp)
{
    Namval_t *namval;
    TkshCommandData *commandData;
...
#ifndef NEWKSH
int
Tcl_GlobalEval(Tcl_Interp *interp, char *cmd)
{
    int result, jmpval;
...
int
Tksh_GlobalEval(Tcl_Interp *interp, char *cmd, int interpType)
{
    int result;
    static Shscope_t *globalframe;
    Shscope_t *oldframe;
...
int
Tcl_RecordAndEval(Tcl_Interp *interp, char *script, int flags)
{
    int hist_state;
    int result = TCL_OK;
