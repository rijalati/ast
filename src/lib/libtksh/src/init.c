...
extern int
b_TkshSetlistCmd(int argc, char *argv[], Shbltin_t *context);
extern int
b_TkshInfoCmd(int argc, char *argv[], Shbltin_t *context);
...
static CmdMap commandMap[] = { { NULL, "tcl_do" },
                               { NULL, "tcl_readonly" },
                               { NULL, "tcl_history" },
                               { ".", "tcl_dot" },
                               { NULL, NULL } };
...
int
TestCommand(ClientData data, Tcl_Interp *interp, int artc, char *argv[])
{
    fprintf(stderr, "Test\n");
    return 0;
}
...
void Tcl_SourceRCFile(interp) Tcl_Interp *interp; /* Interpreter to source rc
                                                     file into. */
{
    Tcl_DString temp;
    char *fileName;
