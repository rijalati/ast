...
static ArraySearch *ParseSearchId
_ANSI_ARGS_((Tcl_Interp * interp, Var *varPtr, char *varName, char *string));

static ArraySearch *
MakeSearchId _ANSI_ARGS_((Tcl_Interp * interp, Namval_t *nv, char *varName));
...
char **argv; /* Argument strings. */
{
    ...
    {
        result = Tcl_SetVar(
        interp, argv[1], argv[i], TCL_APPEND_VALUE | TCL_LEAVE_ERR_MSG);
        if (result == NULL)
        {
            return TCL_ERROR;
        }
    ...
    for (; argc > 0; argc -= 2, argv += 2)
    {
        for (p = argv[0]; *p != 0; p++)
        {
            if (*p == '(')
            {
                ...
                *p = '\0';
                result = MakeUpvar(
                iPtr, framePtr, argv[0], openParen + 1, argv[1], 0);
                *openParen = '(';
                *p = ')';
                goto checkResult;
            }
...
static ArraySearch *
ParseSearchId(interp, varPtr, varName, string) Tcl_Interp *interp;
Var *varPtr;
char *varName;
char *string;
{
