...
char *
GetOpenMode _ANSI_ARGS_((Tcl_Interp * interp, char *string, int *modePtr));
...
static char *
SplitUnixPath(char *path, Tcl_DString *bufPtr);
...
static char *SplitUnixPath(path, bufPtr) char *path; /* Pointer to string
                                                        containing a path. */
...
extern int
Tksh_CreatePipeline(Tcl_Interp *interp,
                    int argc,
                    char *argv[],
                    Tcl_DString *buffer);
...
char **argv; /* Argument strings. */
{
    ...
    if (Tksh_CreatePipeline(interp, argc - 1, argv + 1, &buffer) == TCL_OK)
    {
        ...
        Tcl_SetResult(
        interp,
        Tcl_GetVar2(interp, ".sh.result", NULL, TCL_GLOBAL_ONLY),
        TCL_VOLATILE);
    }
...
errorSpec:
    Tcl_AppendResult(
    interp, "can't specify \"", argv[i], "\" as last word in command", NULL);
error:
    return -1;
}
...
int
TclCreatePipeline(Tcl_Interp *interp,
                  int argc,
                  char *argv[],
                  int **pidPtr,
                  Tcl_File *inPipePtr,
...
char *Tcl_JoinPath(argc, argv, resultPtr) int argc;
char **argv;
