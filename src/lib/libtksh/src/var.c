...
static void
TkshClearTraces(Namval_t *nv);

static char *
tksh_getval(Namval_t *nv, Namfun_t *nf);
static void
tksh_putval(Namval_t *nv, const char *val, int flags, Namfun_t *nf);
static void
tksh_arrputval(Namval_t *nv, const char *val, int flags, Namfun_t *nf);
static char *
tksh_arrgetval(Namval_t *nv, Namfun_t *nf);
static void
tksh_blockput(Namval_t *nv, const char *val, int flags, Namfun_t *nf);
static char *
tksh_blockget(Namval_t *nv, Namfun_t *nf);
            ...
            char *msg)
{
    Namval_t *namval;
    Hashtab_t *tree = NULL;
    char *errmsg, *openParen, *closeParen, *p, *part1 = *name1,
                                               *part2 = *name2;
    ...
    if (flags & PART1_NOT_PARSED)
        part1
        = nv_name(nv_open(part1, tree, nvflags | NV_NOASSIGN | NV_NOREF));
...
cleanup:
    ...
    if (openParen)
        *openParen = TKSH_ARRAY_OPEN;
    if (closeParen)
        *closeParen = TKSH_ARRAY_CLOSE;
    return namval;
...
scalar: /* ov_return goes to here */
    ...
    nv_close(namval);
    namval = NULL;
    goto cleanup; /* XX Might be able to do return NULL; */
}

Namval_t *
TkshAccessVar(Tcl_Interp *interp, char *part1, char *part2)
{
            ...
            int flags)
{
    ...
    if (flags & TCL_APPEND_VALUE)
    {
        ...
        if (oldval)
            sfputr(stkstd,
                   oldval,
                   ((flags & TCL_LIST_ELEMENT)
                    && TclNeedSpace(oldval, oldval + strlen(oldval)))
                   ? ' '
                   : -1);
...
char *
Tcl_GetVar2(Tcl_Interp *interp, char *part1, char *part2, int flags)
{
    Namval_t *namval;
    ...
    namval = TkshOpenVar(interp,
                         &part1,
                         &part2,
                         flags,
                         OVAR_ARRAY | OVAR_GETDISC | OVAR_NOADD,
                         "read");
    ...
    if ((result = nv_getval(namval)) == NULL) /* Error in trace */
    {
        if (flags & TCL_LEAVE_ERR_MSG)
            VarErrMsg(
            interp, part1, part2, "read", get_result ? get_result : noSuchVar);
        goto scalar;
    }
...
int
Tcl_UnsetVar2(Tcl_Interp *interp, char *part1, char *part2, int flags)
{
    ...
    namval = TkshOpenVar(
    interp, &part1, &part2, flags, OVAR_NOADD | OVAR_UNSETDISC, "unset");
    if (!namval)
        return TCL_ERROR;
...
tksh_putval(Namval_t *nv, const char *val, int flags, Namfun_t *nf)
{
    ...
    if (val) /* Set */
    {
        nv_onattr(nv, NV_NODISC);
        nv_stopdisc(nv);
        nv_putval(nv, val, flags);
        traceinfo->nv = NULL;
        traceinfo->clientData = &trace;
        result = trace.proc(
        trace.clientData, trace.interp, arrayname, subname, traceFlags);
        if (!(trace.flags & TCL_TRACE_DESTROYED))
        {
            val = nv_getval(nv);
            nv_offattr(nv, NV_NODISC);
            nv_resumedisc(nv);
...
tksh_getval(Namval_t *nv, Namfun_t *nf)
{
    ...
    traceinfo->nv = NULL;
    traceinfo->clientData = &trace;
    result = traceinfo->proc(
    trace.clientData, trace.interp, part1, part2, traceFlags);
    fcrestore(&save);
...
int
Tcl_TraceVar2(Tcl_Interp *interp,
              char *part1,
              char *part2,
              int flags,
              Tcl_VarTraceProc *proc,
                ...
                ClientData clientData)
{
    ...
    namval
    = TkshOpenVar(interp, &name1, &name2, flags, OVAR_NOADD, "untrace");
    if (!namval)
        goto scalar;
...
ClientData
Tcl_VarTraceInfo2(Tcl_Interp *interp,
                  char *name1,
                  char *name2,
                  int flags,
                  Tcl_VarTraceProc *proc,
                  ClientData clientData)
{
    ...
    if (!(namval
          = TkshOpenVar(interp, &name1, &name2, flags, OVAR_NOADD, NULL)))
        return NULL;
...
{
    return Tcl_SetVar2(
    interp, varName, NULL, newValue, flags | PART1_NOT_PARSED);
}
char *
Tcl_GetVar(Tcl_Interp *interp, char *varName, int flags)
{
    return Tcl_GetVar2(interp, varName, NULL, flags | PART1_NOT_PARSED);
}
int
Tcl_TraceVar(Tcl_Interp *interp,
             char *varName,
             int flags,
             Tcl_VarTraceProc *proc,
             ClientData clientData)
{
    ...
    interp, varName, NULL, flags | PART1_NOT_PARSED, proc, clientData);
}
int
Tcl_UnsetVar(Tcl_Interp *interp, char *varName, int flags)
{
...
ClientData
Tcl_VarTraceInfo(Tcl_Interp *interp,
                 char *varName,
                 int flags,
                 Tcl_VarTraceProc *proc,
                 ClientData prevClientData)
{
    ...
    interp, varName, NULL, flags | PART1_NOT_PARSED, proc, prevClientData);
}
