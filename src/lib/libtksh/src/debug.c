...
int
__dprintfOK(char *file, int line, int level)
{
    if (!debugInitialized)
        dinit();
    if (debugLevel < level)
...
void
__dprintfNum(char *before, int num, char *after)
{
...
printState(char *file, int line)
{
    sfprintf(outFile,
             "[%s]%s:%d: ",
             Tksh_InterpString(debugInterp->interpType),
             file,
             line);
}
