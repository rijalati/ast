...
static int InterpProc _ANSI_ARGS_(
...
char **argv;        /* Argument strings. */
{
    ...
    for (i = 0; i < argCount; i++)
    {
        ...
        result
        = Tcl_SplitList(interp, argArray[i], &fieldCount, &fieldValues);
        if (result != TCL_OK)
        {
            goto procError;
        }
...
static void
clearlocals(Namval_t *nv, void *data)
{
...
InterpProcBody(void *data)
{
    ...
    for (argPtr = procPtr->argPtr, args = argv + 1, argc -= 1; argPtr != NULL;
...
static void CleanupProc(procPtr) Proc *procPtr; /* Procedure to be deleted. */
{
    Arg *argPtr;
