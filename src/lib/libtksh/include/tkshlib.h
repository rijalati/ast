...
#            define TCL_PATCH_LEVEL                                          \
                "7.4" /* The only ver that wouldn't already set it */
#        endif
#    endif
...
extern void
TclExpandParseValue(ParseValue *pvPtr, int needed);

extern int
TclNeedSpace(char *start, char *end);

extern void
TclSetupEnv(Tcl_Interp *interp);

extern void
TclWinFlushEvents(void);
...
extern char *
TkshMapName(char *name);
extern char *
TkshMapKeyword(char *name);
extern char *
TkshLibDir(void);
...
extern int
Tcl_TclEval(Tcl_Interp *interp, char *cmd);
extern int
...
EXTERN Tcl_Channel
TclGetDefaultStdChannel(int type);
EXTERN Tcl_Channel
TclFindFileChannel(Tcl_File inFil, Tcl_File outFile, int *fileUsedPtr);
EXTERN int
TclGetLoadedPackages(Tcl_Interp *interp, char *targetName);
EXTERN int
TclInterpInit(Tcl_Interp *interp);
EXTERN void
TclPlatformInit(Tcl_Interp *interp);
EXTERN void
TclFreePackageInfo(Interp *iPtr);
EXTERN int
TclChdir(Tcl_Interp *interp, char *dirName);
EXTERN char *
TclGetCwd(Tcl_Interp *interp);
EXTERN int
TclPreventAliasLoop(Tcl_Interp *interp,
                    Tcl_Interp *cmdInterp,
                    char *cmdName,
                    Tcl_CmdProc *proc,
                    ClientData clientData);
EXTERN int
TclFindElement(Tcl_Interp *interp,
               char *list,
               char **elementPtr,
               char **nextPtr,
               int *sizePtr,
               int *bracePtr);
EXTERN void
TclCopyAndCollapse(int count, char *src, char *dst);
EXTERN int
TclGetListIndex(Tcl_Interp *interp, char *string, int *indexPtr);
EXTERN unsigned long
TclGetSeconds(void);
EXTERN void
TclGetTime(Tcl_Time *time);
EXTERN int
TclGetTimeZ(unsigned long time);
EXTERN int
TclGetDate(char *p, unsigned long now, long zone, unsigned long *timePtr);
EXTERN int
TclGuessPackageName(char *fileName, Tcl_DString *bufPtr);
EXTERN int
TclLoadFile(Tcl_Interp *interp,
            char *fileName,
            char *sym1,
            char *sym2,
...
EXTERN char *
TclGetExtension(char *name);
EXTERN int
TclGetOpenMode(Tcl_Interp *interp, char *string, int *seekFlagPtr);
EXTERN unsigned long
TclGetClicks(void);
EXTERN int
TclIdlePending(void);
...
TclServiceIdle(void);
EXTERN int
TclWaitForFile(Tcl_File file, int mask, int timeout);
EXTERN int
TclParseBraces(Tcl_Interp *interp,
               char *string,
               char **termPtr,
               ParseValue *pvPtr);
EXTERN int
TclParseNestedCmd(Tcl_Interp *interp,
                  char *string,
                  int flags,
                  char **termPtr,
                  ParseValue *pvPtr);
EXTERN int
TclParseQuotes(Tcl_Interp *interp,
               char *string,
               int termChar,
               int flags,
               char **termPtr,
               ParseValue *pvPtr);
EXTERN int
TclParseWords(Tcl_Interp *interp,
              char *string,
              int flags,
              int maxWords,
              char **termPtr,
...
extern char *
TclPrecTraceProc(ClientData clientData,
                 Tcl_Interp *interp,
                 char *name1,
                 char *name2,
                 int flags);
