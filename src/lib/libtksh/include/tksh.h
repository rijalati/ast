...
#    endif
    void * /* CallFrame */ xxxframePtr; /* Points to top-most in stack of all
                                         * nested procedure invocations.  NULL
                                         * means there are no active
                                         * procedures. */
    void * /* CallFrame */ xxxvarFramePtr; /* Points to the call frame whose
                                            * variables are currently in use
                                            * (same as framePtr unless an
                                            * "uplevel" command is being
                                            * executed).  NULL means no
                                            * procedure is active or "uplevel
                                            * 0" is being exec'ed. */
...
extern void
TkshDeleteSearches(TkshArrayInfo *);
extern TkshArrayInfo *
TkshArrayData(Namval_t *namval);
extern void
TkshTracesOff(void);

extern int
tksh_waitevent(int, long, int);
extern int
nv_getlevel(void);

extern int
Tksh_Eval(Tcl_Interp *interp, char *cmd, int flag);
extern int
Tksh_SetCommandType(Tcl_Interp *, char *, int tp);
extern int
Tksh_Init(Tcl_Interp *interp);
extern char *
Tksh_ConvertList(Tcl_Interp *interp, char *lst, int to);
extern void
TkshCreateInterp(Tcl_Interp *interp, void *data);
extern void
TkshSubShell(void);
extern int
Tksh_CreatePipeline(Tcl_Interp *, int, char **, Tcl_DString *);

extern int
b_print(int argc, char *argv[], Shbltin_t *);
extern int
b_tclinit(int argc, char *argv[], Shbltin_t *);
extern int
b_tkinit(int argc, char *argv[], Shbltin_t *);
