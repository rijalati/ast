...
static int AliasCmd _ANSI_ARGS_(
(ClientData dummy, Tcl_Interp *currentInterp, int argc, char **argv));
static void AliasCmdDeleteProc _ANSI_ARGS_((ClientData clientData));
static int AliasHelper _ANSI_ARGS_((Tcl_Interp * curInterp,
                                    ...
                                    char **argv));
static int
CreateInterpObject _ANSI_ARGS_((Tcl_Interp * interp, int argc, char **argv));
static Tcl_Interp *
CreateSlave _ANSI_ARGS_((Tcl_Interp * interp, char *slavePath, int safe));
...
static int DescribeAlias _ANSI_ARGS_((Tcl_Interp * interp,
                                      ...
                                      char *aliasName));
static int
DeleteInterpObject _ANSI_ARGS_((Tcl_Interp * interp, int argc, char **argv));
static int DeleteOneInterpObject _ANSI_ARGS_((Tcl_Interp * interp,
                                              char *path));
static Tcl_Interp *GetInterp _ANSI_ARGS_(
(Tcl_Interp * interp, Master *masterPtr, char *path, Master **masterPtrPtr));
static int
GetTarget _ANSI_ARGS_((Tcl_Interp * interp, char *path, char *aliasName));
static void MasterRecordDeleteProc _ANSI_ARGS_((ClientData clientData,
                                                Tcl_Interp *interp));
static int MakeSafe _ANSI_ARGS_((Tcl_Interp * interp));
static int
SlaveAliasHelper _ANSI_ARGS_((Tcl_Interp * interp, int argc, char **argv));
static int SlaveObjectCmd
_ANSI_ARGS_((ClientData dummy, Tcl_Interp *interp, int argc, char **argv));
static void SlaveObjectDeleteProc _ANSI_ARGS_((ClientData clientData));
static void SlaveRecordDeleteProc _ANSI_ARGS_((ClientData clientData,
                                               Tcl_Interp *interp));
...
= { "after",     "append",  "array",   "break",        "case",
    "catch",     "clock",   "close",   "concat",       "continue",
    "eof",       "error",   "eval",    "expr",         "fblocked",
    "fileevent", "flush",   "for",     "foreach",      "format",
    "gets",      "global",  "history", "if",           "incr",
    "info",      "interp",  "join",    "lappend",      "lindex",
    "linsert",   "list",    "llength", "lower",        "lrange",
    "lreplace",  "lsearch", "lsort",   "package",      "pid",
    "proc",      "puts",    "read",    "regexp",       "regsub",
    "rename",    "return",  "scan",    "seek",         "set",
    "split",     "string",  "subst",   "switch",       "tell",
    "time",      "trace",   "unset",   "unsupported0", "update",
    "uplevel",   "upvar",   "vwait",   "while",        NULL };
                                                   ...
                                                   safe. */
{
    ...
    ckfree(( char * ) argv);
    masterPtr
    = ( Master * ) Tcl_GetAssocData(interp, "tclMasterRecord", NULL);
...
Master **masterPtrPtr; /* (Return) its master record. */
{
    ...
    for (searchInterp = interp, i = 0; i < argc; i++) {
        ...
        searchInterp, "tclMasterRecord", NULL);
        if (masterPtrPtr != ( Master ** ) NULL)
            *masterPtrPtr = masterPtr;
        if (masterPtr == ( Master * ) NULL) {
            ckfree(( char * ) argv);
...
int safe;           /* Should we make it "safe"? */
{
    ...
    slavePtr->interpCmd = Tcl_CreateCommand(masterInterp,
                                            slavePath,
                                            SlaveObjectCmd,
                                            ( ClientData ) slaveInterp,
                                            SlaveObjectDeleteProc);
    Tcl_InitHashTable(&(slavePtr->aliasTable), TCL_STRING_KEYS);
    ( void ) Tcl_SetAssocData(slaveInterp,
                              "tclSlaveRecord",
    ...
    if (((safe) && (MakeSafe(slaveInterp) == TCL_ERROR))
        ...
        interp,
        Tcl_GetVar2(
        slaveInterp, "errorInfo", ( char * ) NULL, TCL_GLOBAL_ONLY));
        Tcl_SetVar2(
        interp,
        "errorCode",
        ( char * ) NULL,
        Tcl_GetVar2(
        slaveInterp, "errorCode", ( char * ) NULL, TCL_GLOBAL_ONLY),
        TCL_GLOBAL_ONLY);
...
static int AliasHelper(curInterp,
                       slaveInterp,
                       masterInterp,
                       masterPtr,
                       aliasName,
...
char **argv;              /* with alias. */
{
    ...
    aliasPtr->aliasName
    = ( char * ) ckalloc(( unsigned ) strlen(aliasName) + 1);
    aliasPtr->targetName
    = ( char * ) ckalloc(( unsigned ) strlen(targetName) + 1);
    ...
    aliasPtr->slaveCmd = Tcl_CreateCommand(slaveInterp,
                                           aliasName,
                                           AliasCmd,
                                           ( ClientData ) aliasPtr,
                                           AliasCmdDeleteProc);
...
int Tcl_GetInterpPath(askingInterp, targetInterp)
...
Tcl_Interp *targetInterp; /* Interpreter to find. */
{
    ...
    Tcl_AppendElement(
    askingInterp,
    Tcl_GetHashKey(&(masterPtr->slaveTable), slavePtr->slaveEntry));
    return TCL_OK;
}
...
char **argv;           /* Argument strings. */
{
    ...
    if (cmdName[0] == 'e') {
        if ((strncmp(cmdName, "eval", len) == 0) && (len > 1)) {
             ...
             * have to be careful because the slave interpreter and the
             * current interpreter can be the same - do not destroy the
             * result.. This can happen if an interpreter contains an alias
            ...
            if (interp != slaveInterp) {
                if (result == TCL_ERROR) {
                     ...
                     * since Tcl_AddErrorInfo will store the interp's result
                     * in errorInfo before appending slaveInterp's $errorInfo;
        ...
        if ((strncmp(cmdName, "share", len) == 0) && (len > 1)) {
            if (argc != 5) {
                Tcl_AppendResult(interp,
                                 "wrong # args: should be \"",
                                 argv[0],
                                 " share srcPath channelId destPath\"",
                                 ( char * ) NULL);
                return TCL_ERROR;
            }
    ...
    "issafe, share, slaves, target or transfer",
    ( char * ) NULL);
    return TCL_ERROR;
}
...
char **argv;           /* Argument strings. */
{
    ...
    if (cmdName[0] == 'e') {
        if ((len > 1) && (strncmp(cmdName, "eval", len) == 0)) {
             ...
             * can happen if an interpreter contains an alias which is
            ...
            if (interp != slaveInterp) {
                if (result == TCL_ERROR) {
                     ...
                     * Clear interp's result before calling Tcl_AddErrorInfo,
                     * since Tcl_AddErrorInfo stores the interp's result in
    ...
    Tcl_AppendResult(interp,
                     "bad option \"",
                     argv[1],
                     "\": should be alias, aliases, eval, or issafe",
                     ( char * ) NULL);
    return TCL_ERROR;
}
...
Tcl_Interp *interp;    /* Interpreter being deleted. */
{
    ...
    if (slavePtr->interpCmd != ( Tcl_Command ) NULL) {
         ...
         *
         * Probably Tcl_DeleteInterp() was called on this interpreter
         * directly, rather than via "interp delete", or equivalent (deletion
         * of the command in the master).
         *
        ...
        Tcl_DeleteCommand(
        slavePtr->masterInterp,
        Tcl_GetCommandName(slavePtr->masterInterp, slavePtr->interpCmd));
    }
...
int TclInterpInit(interp) Tcl_Interp *interp; /* Interpreter to initialize. */
{
    Master *masterPtr; /* Its Master record. */
...
int Tcl_IsSafe(interp) Tcl_Interp *interp; /* Is this interpreter "safe" ? */
{
    Master *masterPtr; /* Its master record. */
...
Tcl_Interp *Tcl_GetMaster(interp) Tcl_Interp *interp; /* Get the master of
                                                         this interpreter. */
{
