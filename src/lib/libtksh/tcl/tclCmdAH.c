...
char **argv;        /* Argument strings. */
{
    ...
    if (argc == 3) {
        if (Tcl_SetVar(interp, argv[2], interp->result, 0) == NULL) {
            Tcl_SetResult(
            interp, "couldn't save command result in variable", TCL_STATIC);
            return TCL_ERROR;
        }
    }
...
static char *GetTypeFromMode(mode) int mode;
{
    if (S_ISREG(mode)) {
        return "file";
...
#define STATIC_SIZE 4
    ...
    for (i = 0; i < numLists; i++) {
        result
        = Tcl_SplitList(interp, argv[1 + i * 2], &varcList[i], &varvList[i]);
        if (result != TCL_OK) {
            goto errorReturn;
        }
        result
        = Tcl_SplitList(interp, argv[2 + i * 2], &argcList[i], &argvList[i]);
        if (result != TCL_OK) {
            goto errorReturn;
        }
