...
static int FormatClock _ANSI_ARGS_(
(Tcl_Interp * interp, unsigned long clockVal, int useGMT, char *format));
...
static int ParseTime(interp, string, timePtr) Tcl_Interp *interp;
char *string;
unsigned long *timePtr;
{
    char *end, *p;
