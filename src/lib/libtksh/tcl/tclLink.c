...
void Tcl_UnlinkVar(interp,
                   varName) Tcl_Interp *interp; /* Interpreter containing
                                                   variable to unlink. */
...
char *varName;      /* Name of global variable that is linked. */
{
    ...
    Tcl_SetVar(
    interp, linkPtr->varName, StringValue(linkPtr, buffer), TCL_GLOBAL_ONLY);
...
static char *StringValue(linkPtr,
                         buffer) Link *linkPtr; /* Structure describing linked
                                                   variable. */
