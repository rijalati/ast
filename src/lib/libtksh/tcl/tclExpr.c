...
static int ExprDoubleFunc _ANSI_ARGS_((ClientData clientData,
                                       ...
                                       Tcl_Value *args,
                                       Tcl_Value *resultPtr));
static int ExprGetValue _ANSI_ARGS_(
(Tcl_Interp * interp, ExprInfo *infoPtr, int prec, Value *valuePtr));
                    ...
                    * correctly. */
{
    ...
    case '"':
        infoPtr->token = VALUE;
        result = TclParseQuotes(
        interp, infoPtr->expr, '"', 0, &infoPtr->expr, &valuePtr->pv);
        if (result != TCL_OK) {
            return result;
        }
        Tcl_ResetResult(interp);
    ...
    case '{':
        infoPtr->token = VALUE;
        result = TclParseBraces(
        interp, infoPtr->expr, &infoPtr->expr, &valuePtr->pv);
        if (result != TCL_OK) {
            return result;
        }
        Tcl_ResetResult(interp);
                     ...
                     * initialized pv field. */
{
    ...
    if (infoPtr->token == OPEN_PAREN) {
        ...
        if (infoPtr->token >= UNARY_MINUS) {
            ...
            result = ExprGetValue(
            interp, infoPtr, precTable[infoPtr->token], valuePtr);
            if (result != TCL_OK) {
                goto done;
            }
            if (!iPtr->noEval) {
    ...
    if ((operator== AND) || (operator== OR) || (operator== QUESTY)) {
        ...
        continue;
            ...
            if (valuePtr->intValue != 0) {
                valuePtr->pv.next = valuePtr->pv.buffer;
                result = ExprGetValue(
                interp, infoPtr, precTable[QUESTY] - 1, valuePtr);
                if (result != TCL_OK) {
                    goto done;
                }
                ...
                iPtr->noEval++;
                result = ExprGetValue(
                interp, infoPtr, precTable[QUESTY] - 1, &value2);
                iPtr->noEval--;
            } else {
                iPtr->noEval++;
                result = ExprGetValue(
    ...
    case PLUS:
    case MINUS:
        if ((valuePtr->type == TYPE_STRING) || (value2.type == TYPE_STRING)) {
            badType = TYPE_STRING;
            goto illegalType;
        }
        if (valuePtr->type == TYPE_DOUBLE) {
...
static void ExprMakeString(interp,
                           valuePtr) Tcl_Interp *interp; /* Interpreter to use
                                                          * for precision
                                                          * information. */
                     ...
                     * not be initialized by caller. */
{
    ...
    if ((valuePtr->type == TYPE_DOUBLE)
        && (IS_NAN(valuePtr->doubleValue) || IS_INF(valuePtr->doubleValue))) {
        /*
...
int Tcl_ExprLong(interp, string, ptr) Tcl_Interp *interp; /* Context in which
                                                           * to evaluate the
                                                           * expression. */
char *string; /* Expression to evaluate. */
                     ...
                     * correctly. */
{
    ...
    *p = savedChar;
    ...
    tcl_MathInProgress++;
    result = (*mathFuncPtr->proc)(
    mathFuncPtr->clientData, interp, args, &funcResult);
    tcl_MathInProgress--;
    if (result != TCL_OK) {
        return result;
    }
...
void TclExprFloatError(interp, value) Tcl_Interp *interp; /* Where to store
                                                             error message. */
...
static int ExprUnaryFunc(clientData, interp, args, resultPtr)
...
static int ExprBinaryFunc(clientData, interp, args, resultPtr)
ClientData clientData; /* Contains address of procedure that
...
Tcl_Value *resultPtr;
{
    ...
    = (*func)(args[0].doubleValue, args[1].doubleValue);
    if (errno != 0) {
        TclExprFloatError(interp, resultPtr->doubleValue);
        return TCL_ERROR;
    }
...
/* ARGSUSED */
static int
ExprAbsFunc(clientData, interp, args, resultPtr) ClientData clientData;
Tcl_Interp *interp;
Tcl_Value *args;
Tcl_Value *resultPtr;
{
...
/* ARGSUSED */
static int
ExprDoubleFunc(clientData, interp, args, resultPtr) ClientData clientData;
Tcl_Interp *interp;
Tcl_Value *args;
Tcl_Value *resultPtr;
{
...
/* ARGSUSED */
static int
ExprIntFunc(clientData, interp, args, resultPtr) ClientData clientData;
Tcl_Interp *interp;
Tcl_Value *args;
Tcl_Value *resultPtr;
{
...
/* ARGSUSED */
static int
ExprRoundFunc(clientData, interp, args, resultPtr) ClientData clientData;
Tcl_Interp *interp;
Tcl_Value *args;
Tcl_Value *resultPtr;
{
...
static int ExprLooksLikeInt(p) char *p; /* Pointer to string. */
{
    while (isspace(UCHAR(*p))) {
        p++;
    }
