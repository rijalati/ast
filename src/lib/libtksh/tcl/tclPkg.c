...
static int CheckVersion _ANSI_ARGS_((Tcl_Interp * interp, char *string));
static int ComparePkgVersions _ANSI_ARGS_((char *v1, char *v2, int *satPtr));
static Package *FindPackage _ANSI_ARGS_((Tcl_Interp * interp, char *name));
...
char *version;      /* Version string for package. */
{
    ...
    Tcl_AppendResult(interp,
                     "conflicting versions provided for package \"",
                     name,
                     "\": ",
                     pkgPtr->version,
                     ", then ",
                     ...
                     * use the latest compatible version. */
{
    ...
    for (pass = 1;; pass++) {
        ...
        for (availPtr = pkgPtr->availPtr; availPtr != NULL;
            ...
            if (version != NULL) {
                result
                = ComparePkgVersions(availPtr->version, version, &satisfies);
                if ((result != 0) && exact) {
                    continue;
                }
                if (!satisfies) {
        ...
        if (script != NULL) {
            ...
            Tcl_DStringAppend(
            &command, (version != NULL) ? version : "{}", -1);
            if (exact) {
...
void TclFreePackageInfo(iPtr) Interp *iPtr; /* Interpereter that is being
                                               deleted. */
{
    Package *pkgPtr;
    Tcl_HashSearch search;
...
static int CheckVersion(interp, string) Tcl_Interp *interp; /* Used for error
                                                               reporting. */
