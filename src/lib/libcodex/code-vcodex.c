...
#define VCSFERROR(s, m)                                                      \
    ((s)->codex->disc->errorf                                                \
     ? ((*(s)->codex->disc->errorf)(NiL, (s)->codex->disc, 2, m), -1)        \
     : -1)
...
fillbuf(State_t *state, Sfio_t *f, Sfdisc_t *disc)
{
    ...
    for (; sz < state->bssz; sz += n, state->endb += n) {
        if (!disc) /* plain read if no discipline set yet */
            ...
            n = sfrd(f, state->endb, state->bssz - sz, disc);
        if (n <= 0)
            break;
    }
    ...
    for (loop = 0;; ++loop) {
        ...
        if (loop > 0 || sz <= 0) {
            if (fillbuf(state, f, state->vc ? disc : NIL(Sfdisc_t *)) <= 0
                ...
                return identify
                       ? 0
                       : init ? VCSFERROR(state, "corrupt header data") : -1;
        }
...
encode(State_t *state, Vcchar_t *data, size_t dtsz)
{
    ...
    state->cdsz = vciosize(&io);

    return size; /* return amount processed */
}

static int
vcodex_ident(Codexmeth_t *meth,
             const void *head,
             size_t headsize,
             char *name,
             size_t namesize)
              ...
              char **source)
{
    ...
    badarg:
        if (p->disc->errorf)
            (*p->disc->errorf)(
            NiL, p->disc, 2, "%s: %s: unknown option", s, e);
        return -1;
    }
    return 0;
}
...
#if CODEX_VERSION >= 20130501L
    if ((s = p->disc->window)
        && vcodex_option(p, s, &wmeth, &wsize, &vflags, &source))
        return -1;
#endif
    i = 2;
    while (s = args[i++])
...
= { "vcodex",    "vcodex compression.",
    0,           CODEX_DECODE | CODEX_ENCODE | CODEX_COMPRESS | CODEX_VCODEX,
    0,           vcodex_ident,
    vcodex_open, 0,
    vcodex_init, vcodex_done,
    vcodex_read, vcodex_write,
    vcodex_sync, 0,
    0,           0,
    0,           CODEXNEXT(vcodex) };
