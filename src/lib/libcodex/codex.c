...
trace_read(Sfio_t *f, void *buf, size_t n, Sfdisc_t *disc)
{
    ...
    sfprintf(sfstderr,
             "codex: %d: %s: read(%I*u,%s)=%I*d\n",
             CODEX(disc)->index,
             meth->name,
             sizeof(n),
             n,
             !r ? "\"\""
                : (CODEX(disc)->flags & CODEX_VERBOSE)
                  ? fmtquote(buf, "\"", "\"", r, 0)
                  : "''",
             sizeof(r),
             r,
...
trace_except(Sfio_t *f, int op, void *data, Sfdisc_t *disc)
{
    ...
    sfprintf(sfstderr,
             "codex: %d: %s: except(%s,%p%s%s)=%d\n",
             CODEX(disc)->index,
             meth->name,
             event,
             data,
             (CODEX(disc)->flags & CODEX_ACTIVE) ? "|ACTIVE" : "",
             (CODEX(disc)->flags & CODEX_CACHED) ? "|CACHED" : "",
             r);
    return r;
}
...
trace_data(Codex_t *code, Codexdata_t *data)
{
    ...
    if (r >= 0)
    {
        if (data->buf)
            ...
            sfprintf(
            sfstderr, "%0*I*x", data->size * 2, sizeof(data->num), data->num);
    }
    sfprintf(sfstderr, "\n");
    return r;
}
...
static Codexmeth_t codex_trace = {
    "trace",    "Debug trace wrapper.",
    0,          CODEX_DECODE | CODEX_ENCODE,
    0,          0,
    0,          trace_close,
    trace_init, trace_done,
    trace_read, trace_write,
    trace_sync, trace_seek,
    trace_data,
};
...
= { "trace", OPT_TRACE, "verbose", OPT_VERBOSE, 0, 0 };
     ...
     Codexnum_t flags)
{
    ...
    strncopy(cache->name,
             name,
             namelen < sizeof(cache->name) ? (namelen + 1)
                                           : sizeof(cache->name));
     ...
     Codexmeth_t *meth)
{
    ...
    while (c = *b++)
        if (c == q)
            ...
            break;
    namelen = b - s - 1;
    for (cp = codexstate.cache;
         cp < &codexstate.cache[elementsof(codexstate.cache)];
         cp++)
        if (!cp->code)
            (cache = cp)->cached = 0;
    ...
    if (!code)
    {
        ...
        *a = 0;
        ...
        if ((flags & (CODEX_TRACE | CODEX_VERBOSE))
            && (trace = memdup(&codex_trace, sizeof(codex_trace))))
            sfprintf(sfstderr,
                     "codex: %d: %s: open(\"%s\",%s,%s)\n",
                     code ? code->index : 0,
                     meth->name,
                     arg[0],
                     (sp->_flags & SF_WRITE) ? "WRITE" : "READ",
      ...
      Codexmeth_t *meth)
{
    ...
    case CODEX_DECODE | CODEX_ENCODE:
        if (disc->errorf)
            ...
            disc,
            2,
            "CODEX_DECODE and CODEX_ENCODE are mutually exclusive");
        return -1;
        break;
    case CODEX_DECODE:
    ...
    do
    {
        ...
        *(s - 1) = 0;
        if (codexcmp(t, "copy"))
        {
            ...
            /*
             * '.' arg separator differentiates vcodex method from codex
             * method by same name
             */
            ...
            if (v)
            {
                ...
                if (p == e)
                {
                    if (disc->errorf)
                        ...
                        disc,
                        2,
                        "%s: too many method components -- %d max",
                        name,
                        elementsof(part));
                    free(m);
                    return -1;
    ...
    /*
     * verify that the method for each part supports CODEX_DECODE or
     * CODEX_ENCODE
     */
        ...
        if (!((p->flags & (CODEX_DECODE | CODEX_ENCODE)) & p->meth->flags))
        {
            if (disc->errorf)
            {
                if (e != &part[0])
                    ...
                    ? ERROR_translate(NiL, NiL, id, "decode")
                    : ERROR_translate(NiL, NiL, id, "encode"));
                else
                    (*disc->errorf)(
                    NiL,
                    disc,
...
codexpop(Sfio_t *sp, int serial)
{
    ...
    if (serial >= 0)
        while (sp->disc
               && (sp->disc->exceptf == codex_except
                   || sp->disc->exceptf == trace_except)
               && (!serial || CODEX(sp->disc)->serial == serial)
               && sfdisc(sp, SF_POPDISC))
            pop++;
    return pop;
}
