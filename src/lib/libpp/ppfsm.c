...
#define copy(t, f)                                                           \
    (memcpy(&fsm[t][1], &fsm[f][1], (MAX + 1) * sizeof(short)),              \
     fsm[TERMINAL][(t) + 1] = fsm[TERMINAL][(f) + 1])
...
static struct fsminit fsminit[] = {
    ...
    /* proto
       {do,else,extern,for,if,inline,return,static,typedef,va_start,void,while,NoN}
     */
    {
    RES1,
    { C_XXX },
    ...
    case FSM_QUOTADD:
        while (c = *s++)
            if (fsm[TOKEN][c] == ~S_HUH)
                ...
                error(2, "%c: cannot add to quote set", c);
        break;
...
refill(int c)
{
    if (pp.in->flags & IN_eof) {
        ...
        *((pp.in->nextchr = pp.in->buffer + PPBAKSIZ) - 1) = c;
        c =
#    if PROTOTYPE
        (pp.in->flags & IN_prototype)
        ? pppread(pp.in->nextchr)
        :
#    endif
