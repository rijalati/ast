...
#    define DOSTRIP()                                                        \
        ((st & STRIP)                                                        \
         || pp.level == 1 && (st & (COMPILE | JOINING)) == COMPILE           \
            && !(pp.option & PRESERVE))
            ...
            case IN_MULTILINE:
                ...
                if (prv && !INMACRO(rp))
#endif
                    {
                        ...
                        POP();
#if CPP
                        if (!(st & COMPATIBILITY) && ppisidig(*(op - 1))
                            && ppisidig(*ip))
                            UNGETCHR(' ');
#endif
                        bp = ip;
...
#    endif
    ...
    case S_LITEND:
        ...
        if (c != quot) {
            ...
            if (c == '\n' && quot == '\'' && (pp.option & STRINGSPAN))
                n = 0;
            else
#if COMPATIBLE && !CPP
            if ((st & (COMPATIBILITY | DEFINITION))
                != (COMPATIBILITY | DEFINITION))
#endif
            {
                switch (quot) {
...
#    if CPP
        if (c == '"'
            && !(st & (COLLECTING | NOTEXT | PASSTHROUGH | SKIPCONTROL))
            && (pp.mode & CATLITERAL))
#    else
        if (c == '"' && pp.level == 1
            && !(st & (COLLECTING | JOINING | NOTEXT | SKIPCONTROL))
            && (pp.mode & CATLITERAL))
#    endif
        {
            char *pptoken;
...
#    endif
...
#endif
#if CPP
        if (n && !(st & (PASSTHROUGH | SKIPCONTROL | NOTEXT)) && c == '\''
            && (op - tp) <= 2 && !(pp.mode & (HOSTED | RELAX)))
    ...
    case S_LITESC:
        if (st & (COLLECTING | DIRECTIVE | QUOTE | SQUOTE)) {
            ...
            case '9':
                ...
                if (st & STRICT)
                    error(
                    1, "%c: invalid character in octal character escape", c);
                    /*FALLTHROUGH*/
#endif
            case '0':
            case '1':
                ...
                for (m = 0; m < 2; m++) {
                    ...
                    case '9':
                        ...
                        if (st & STRICT)
                            error(
                            1,
                            "%c: invalid character in octal character escape",
                            c);
                            /*FALLTHROUGH*/
#endif
                    case '0':
            ...
            case 'x':
                ...
                for (m = 0; m < 3; m++) {
                    ...
                    default:
                        if (!m)
                            error(1,
                                  "\\x%c: invalid character in hexadecimal "
                                  "character constant",
                                  c);
                        UNGETCHR(c);
                        break;
                ...
                if (n & ~0777)
                    error(1, "hexadecimal character constant too large");
            octal:
                PUTCHR(((n >> 6) & 07) + '0');
                PUTCHR(((n >> 3) & 07) + '0');
                PUTCHR((n & 07) + '0');
    ...
    case S_MACRO:
        ...
        case HITN:
            bp = tp;
            tp = op
                 - ((pp.truncate && pp.truncate < (HITN - HIT0))
                    ? (pp.truncate - 1)
                    : (HITN - HIT0));
            while (tp > bp && ppisidig(*(tp - 1)))
                tp--;
            break;
        default:
            bp = tp;
            if ((tp = op - (state - HIT0)) > bp && *(tp - 1) == 'L')
                tp--;
            break;
        }
    ...
    default:
        if (state & SPLICE) {
            ...
            case MARK:
                ...
                case IN_INIT:
#    if CATSTRINGS
                    if ((st & JOINING) && (!INQUOTE(rp) || quot != '"')
                        || pp.level > 1 && (rp == fsm[START] || INQUOTE(rp)))
#    else
...
#    endif
                ...
                default:
                    ...
                    case 'A':
                        if (!(st & (DEFINITION | DISABLE))) {
                            ...
                            if (pp.macp->arg[c - ARGOFFSET][-1])
                                PUSH_EXPAND(pp.macp->arg[c - ARGOFFSET],
                                            pp.macp->line);
                            else
                                PUSH_COPY(pp.macp->arg[c - ARGOFFSET],
                                          pp.macp->line);
                            CACHEIN();
                            bp = ip;
                            goto fsm_get;
                    ...
                    case 'L':
                        ...
                        debug(
                        (-6, "actual sync: line = %d", error_info.line));
                        bp = ip = s + 1;
                        goto fsm_get;
                    case 'Q':
                        c = GETCHR();
                        SYNCIN();
                        PUSH_QUOTE(pp.macp->arg[c - ARGOFFSET],
                                   pp.macp->line);
                        CACHEIN();
                        *(bp = ip - 1) = '"';
                        if (st & (COLLECTING | EOF2NL | JOINING))
                            rp = fsm[START];
                        if (state = rp[c = '"'])
                            goto fsm_next;
                        goto fsm_get;
                    case 'S':
                        c = GETCHR();
                        SYNCIN();
                        PUSH_SQUOTE(pp.macp->arg[c - ARGOFFSET],
                                    pp.macp->line);
                        CACHEIN();
                        bp = ip - 1;
                        if (st & COLLECTING)
