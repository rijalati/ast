...
#define SYNC()                                                               \
    do {                                                                     \
        ...
        proto->flags                                                         \
        |= flags & (EXTERN | INIT | OTHER | VARIADIC | VARIADIC2);           \
        proto->call = call;                                                  \
    } while (0)
...
#endif
        if (*proto->tp == '_' && !strncmp(proto->tp, "__STDPP__directive", 6))
            c = '#';
        else
            c = T_ID;
    ...
    fsm_newline:
        ...
        if (flags & DIRECTIVE) {
...
#endif
            {
                ...
                flags &= ~(DEFINE | DIRECTIVE | IDID | INDIRECT | MATCH
                           | OTHER | SHARP | SKIP | TOKENS | TYPEDEF);
            }
            call = 0;
            group = 0;
            paren = 0;
    ...
    case S_RESERVED:
        ...
        if (!(flags & DECLARE))
            ...
            case RESERVED('t', 'f', 7):
                ...
                break;
            case RESERVED('v', 't', 8):
                if (*ip == '(' && !strncmp(proto->tp, "va_start", 8))
                    c = T_VA_START;
                break;
            case RESERVED('v', 'd', 4):
        ...
        if (!(flags & DIRECTIVE))
            ...
            case '#':
                ...
                if (proto->brace == 0 && paren == 0 && last != '='
                    && (flags
                        & (CLASSIC | DECLARE | DIRECTIVE | MATCH | PLUSONLY
                           | SKIP | TOKENS))
                       == (MATCH | TOKENS)
...
#    endif
...
#if PROTOMAIN
            case '[':
                if ((flags & CLASSIC) && paren == 0 && group <= 2)
                    flags |= SKIP;
                    /*FALLTHROUGH*/
#endif
            default:
        ...
        last = c;
#if PROTOMAIN
        if ((flags & (EXTERN | MATCH)) == (EXTERN | MATCH)
            && ((flags & (DIRECTIVE | SKIP)) || proto->brace
                || c != '(' && c != ')' && c != '*' && c != T_ID))
            CACHEOUT();
        else
#endif
        ...
        int flags)
{
    ...
    *(proto->ip + n) = 0;
    ...
    if (flags & PROTO_PLUSPLUS)
        proto->flags |= PLUSPLUS;
    if (flags & PROTO_TEST)
        proto->test = 1;
    if (flags & PROTO_EXTERNALIZE)
        proto->options |= EXTERNALIZE;
#if PROTOMAIN
    if (flags & PROTO_CLASSIC)
        pragma = -pragma;
    if (flags & PROTO_DISABLE)
        pragma = 0;
    if (flags & PROTO_LINESYNC)
        proto->flags |= LINESYNC;
    if (!(proto->flags & YACC) && file && (m = strlen(file)) > 2
        && file[--m] == 'y' && file[--m] == '.')
        proto->flags |= YACC;
#endif
    if (pragma <= 0) {
