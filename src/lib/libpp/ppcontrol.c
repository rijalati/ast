...
#define RESTORE                                                              \
    (COLLECTING | CONDITIONAL | DEFINITION | DIRECTIVE | DISABLE | EOF2NL    \
     | HEADER | NOSPACE | NOVERTICAL | PASSEOF | STRIP)
...
assert(int op, char *pred, char *args)
{
    ...
    mark:
        if ((pp.state & COMPILE) && pp.truncate)
            return;
        ...
        do {
            ...
            if (*t)
                *t++ = 0;
                ...
                t = 0;
            if (*s)
                ppop(op, (flags & TOKOP_DUP) ? strdup(s) : s, n);
        } while (s = t);
}
    ...
    if (sym->flags & (SYM_ACTIVE | SYM_READONLY)) {
        if (!(pp.option & ALLPOSSIBLE))
            error(2,
                  "%s: macro is %s",
                  sym->name,
                  (sym->flags & SYM_READONLY) ? "readonly" : "active");
        return 0;
    }
    if (!sym->macro)
        sym->macro = newof(0, struct ppmacro, 1, 0);
    return sym;
}
...
ppcontrol(void)
{
...
#if COMPATIBLE
    if ((pp.state & (COMPATIBILITY | STRICT)) == COMPATIBILITY
        || (pp.mode & HOSTED))
        pp.state &= ~NOVERTICAL;
#else
    if (pp.mode & HOSTED)
    ...
    case T_OCTAL:
        if ((pp.state & STRICT) && !(pp.mode & (HOSTED | RELAX)))
            error(
            1, "# <line> [ \"<file>\" [ <type> ] ]: non-standard directive");
        directive = INCLUDE;
        goto linesync;
    case T_ID:
        ...
        case INCLUDE:
            ...
            header:
                ...
                if (*pp.token == '/' && !(pp.mode & (HOSTED | RELAX)))
                    error(1,
                          "#%s: reference to %s is not portable",
                          dirname(INCLUDE),
                          pp.token);
        ...
        pass:
            if (!(*pp.control & SKIP) && pp.pragma && !(pp.state & NOTEXT)
                ...
                *p0 = 0;
                ...
                (*pp.pragma)(pp.valbuf + 1,
                             p1,
                             p3,
                             p5,
                             (pp.state & COMPILE) || (pp.mode & INIT) != 0);
                emitted = 1;
            }
            goto donedirective;
        ...
        case ENDMAC:
            c = pplex();
            error(
            2, "no matching #%s for #%s", dirname(MACDEF), dirname(ENDMAC));
            goto enddirective;
#endif
#if MACDEF
        case MACDEF:
        ...
        case DEFINE:
            ...
            old = *mac;
            i0 = sym->flags;
            sym->flags &= ~(SYM_BUILTIN | SYM_EMPTY | SYM_FINAL | SYM_FUNCTION
                            | SYM_INIT | SYM_MULTILINE | SYM_NOEXPAND
                    ...
                    for (;;) {
                        if (c == T_VARIADIC) {
                            if (sym->flags & SYM_VARIADIC)
                                error(
                                2,
                                "%s: %s: duplicate macro formal argument",
                                sym->name,
                                pp.token);
                            ...
                            if (sym->flags & SYM_VARIADIC)
                                error(2,
                                      "%s: %s: macro formal argument cannot "
                                      "follow ...",
                                      sym->name,
                                      v);
            ...
            for (;;) {
                ...
                case T_ID:
...
#endif
                                if (var.type != TOK_TOKCAT && p > mac->value
                                    && *(p - 1) != ' '
                                    && !(pp.option & PRESERVE))
                                    *p++ = ' ';
                            *p++ = MARK;
#if COMPATIBLE
                            if ((pp.state & (COMPATIBILITY | TRANSITION))
                                == COMPATIBILITY)
                                *p++ = 'C';
                            else
#endif
                                *p++
                                = (n1 || var.type == TOK_TOKCAT) ? 'C' : 'A';
                            *p++ = c + ARGOFFSET;
                            if ((pp.state & WARN)
                                && !(pp.mode & (HOSTED | RELAX))
                                && var.type != TOK_TOKCAT
                                ...
                                if (o != '.' && o != T_PTRMEM) {
                                    ...
                                    if (!((o == 0 || o == '(' || o == ')'
                                        ...
                                        && !(o == '*' && c == ')'))
                                        error(1,
                                              "%s: %s: formal should be "
                                              "parenthesized in macro value "
                                              "(t=%x o=%#c c=%#c)",
                                              sym->name,
                                              pp.token,
                                              var.type,
                    ...
                    if (var.type == TOK_BUILTIN)
                        ...
                        for (var.inp = pp.in;
                            ...
                            ;
                        p += sfsprintf(
                        p, MAXTOKEN, "_%d_%s_hIDe", var.inp->hide, pp.token);
                        var.type = TOK_ID;
                        goto checkvalue;
                    }
                    var.type = TOK_ID;
                ...
                case '#':
                    var.type = 0;
#if MACDEF
                    if (!(sym->flags & (SYM_FUNCTION | SYM_MULTILINE)))
                        break;
#else
                ...
                case T_TOKCAT:
                    ...
                    if (c == '\n')
                        error(
                        2, "%s rhs operand omitted", pptokchr(T_TOKCAT));
                    var.type = TOK_TOKCAT;
                    continue;
                case '(':
                    if (*pp.token == '#') {
                ...
                case '\n':
                    ...
                    if (sym->flags & SYM_MULTILINE) {
                        ...
                        pp.state |= EOF2NL;
                        error(
                        2, "%s: missing #%s", sym->name, dirname(ENDMAC));
                    }
#endif
                    goto gotdefinition;
                case 0:
        ...
        gotdefinition:
            ...
            *p = 0;
            ...
            if ((pp.option & (DEFINITIONS | PREDEFINITIONS | REGUARD))
        ...
        benign:
            if (pp.mode & BUILTIN)
                sym->flags |= SYM_BUILTIN;
            if (pp.option & FINAL)
                sym->flags |= SYM_FINAL;
            if (pp.mode & INIT)
                sym->flags |= SYM_INIT;
            if ((pp.mode & HOSTED) || (pp.option & INITIAL))
                sym->flags |= SYM_INITIAL;
            else
                sym->flags &= ~SYM_INITIAL;
            if (pp.state & NOEXPAND)
                sym->flags |= SYM_NOEXPAND;
            if (pp.option & PREDEFINED)
                sym->flags |= SYM_PREDEFINED;
            if (pp.mode & READONLY)
                sym->flags |= SYM_READONLY;
            if (pp.macref)
                (*pp.macref)(sym,
                             error_info.file,
                             n2,
        ...
        case ERROR:
            ...
            *p = 0;
            p = *pp.tmpbuf
                ? pp.tmpbuf
                : ((directive == WARNING) ? "user warning" : "user error");
        ...
        case LET:
            ...
            sym->flags &= ~(SYM_BUILTIN | SYM_FUNCTION | SYM_MULTILINE
        ...
        linesync:
            ...
            if (error_info.line == 0 && directive == LINE
                && (pp.state & STRICT) && !(pp.mode & HOSTED))
            ...
            case T_STRING:
                ...
                case T_DECIMAL:
                case T_OCTAL:
                    if (directive == LINE && (pp.state & STRICT)
                        && !(pp.mode & (HOSTED | RELAX)))
                        error(
                        1,
                        "#%s: integer file type argument is non-standard",
                        dirname(LINE));
                    break;
                default:
                    error(1,
                          "#%s: integer file type argument expected",
                          dirname(LINE));
                    break;
                }
                if (directive == LINE)
                    ...
                    if (error_info.file != s) {
                        ...
                        case PP_sync_pop:
                            if (pp.insert)
                                ...
                                (*pp.incref)(
                                s, error_info.file, n - 1, PP_SYNC_POP);
                            break;
                        case PP_sync_ignore:
                            if (pp.insert)
                                (*pp.incref)(
        ...
        case PRAGMA:
            ...
            *p++ = '#';
            ...
            if (!p1 && p3 && (p4 - p3) == 4 && strneq(p3, "STDC", 4))
                goto pass;
            if ((pp.state & WARN)
                && (pp.mode & (HOSTED | RELAX | PEDANTIC)) == PEDANTIC)
            ...
            *p4 = 0;
            ...
            case X_CDIR:
                tokop(
                PP_CDIR, p3, p, i0, TOKOP_UNSET | TOKOP_STRING | TOKOP_DUP);
                break;
            case X_CHECKPOINT:
#if CHECKPOINT
                ppload(p);
#else
                error(3,
                      "%s: preprocessor not compiled with checkpoint enabled",
                      p3);
#endif
                break;
            case X_CHOP:
                        ...
                        if (i1 == X_NOTE) {
                            ...
                            if (!sym->hidden->level++) {
                                ...
                                if (sym->macro
                                    ...
                                    sym->flags
                                    &= ~(SYM_BUILTIN | SYM_FUNCTION | SYM_INIT
            ...
            case X_HOSTDIR:
                tokop(
                PP_HOSTDIR, p3, p, i0, TOKOP_UNSET | TOKOP_STRING | TOKOP_DUP);
                break;
            case X_HOSTED:
                setmode(HOSTED, i0);
                break;
                ...
                if (!p) {
                    ...
                    if (pp.macref && (s = strchr(p, ' '))) {
                        *s++ = 0;
                        ...
                        (*pp.macref)(
                        pprefmac(p, REF_CREATE),
                        error_info.file,
                        error_info.line - (c == REF_NORMAL ? 2 : 1),
                        c,
                        (s = strchr(s, ' ')) ? strtol(s, NiL, 0) : 0L);
            ...
            case X_MAP:
                ...
                while ((c = pplex()) == T_STRING) {
                    ...
                    if (!(i0 = regcomp(&edit->re,
                                       s,
                                       REG_AUGMENTED | REG_DELIMITED
                                       | REG_LENIENT | REG_NULL))
                        && !(i0 = regsubcomp(
                             &edit->re, s += edit->re.re_npat, NiL, 0, 0)))
                        s += edit->re.re_npat;
                    if (i0)
                        regfatal(&edit->re, 4, i0);
                    if (*s) {
...
#else
                error(1,
                      "preprocessor not compiled with prototype conversion "
                      "enabled");
#endif
                break;
            case X_PROTO:
            ...
            case X_STANDARD:
                tokop(PP_STANDARD,
                      ...
                      p,
                      i0,
                      TOKOP_UNSET | TOKOP_STRING | TOKOP_DUP);
                break;
            case X_STRICT:
                ppop(PP_STRICT, i0);
                break;
            ...
            case X_TRUNCATE:
                ppop(PP_TRUNCATE,
                     i0 ? (p ? strtol(p, NiL, 0) : TRUNCLENGTH) : 0);
                break;
            case X_VENDOR:
                tokop(
                PP_VENDOR, p3, p, i0, TOKOP_UNSET | TOKOP_STRING | TOKOP_DUP);
                break;
            case X_VERSION:
            ...
            default:
                error(PANIC,
                      "%s: option recognized but not implemented",
                      pp.valbuf);
                break;
#endif
            }
        ...
        case RENAME:
            ...
            if (sym->flags & (SYM_ACTIVE | SYM_READONLY)) {
                if (!(pp.option & ALLPOSSIBLE))
                    error(2,
                          "%s: macro is %s",
                          sym->name,
                          (sym->flags & SYM_READONLY) ? "readonly"
                                                      : "active");
                goto eatdirective;
            }
            if ((c = pplex()) != T_ID) {
                ...
                if (var.symbol->flags & (SYM_ACTIVE | SYM_READONLY)) {
                    if (!(pp.option & ALLPOSSIBLE))
                        error(2,
                              "%s: macro is %s",
                              var.symbol->name,
                              (var.symbol->flags & SYM_READONLY) ? "readonly"
                                                                 : "active");
                    goto eatdirective;
                }
            ...
            sym->flags
            &= ~(SYM_BUILTIN | SYM_FUNCTION | SYM_INIT | SYM_MULTILINE
                 | SYM_PREDEFINED | SYM_REDEFINE | SYM_VARIADIC);
...
enddirective:
...
#endif
    {
        ...
        if ((c = pplex()) != '\n'
            ...
            error(1,
                  "%s: invalid characters after directive",
                  pptokstr(pp.token, 0));
    }
eatdirective:
    if (c != '\n') {
