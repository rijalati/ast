...
#define lex(c)                                                               \
    ((((c) = peektoken) >= 0 ? (peektoken = (-1)) : ((c) = pplex())), (c))
...
predicate(int warn)
{
    ...
    case X_NOTICED:
        if (type != T_ID)
            error(1, "%s: identifier argument expected", pred);
        else if (((sym = pprefmac(args, REF_IF))
                  || (sym = ppsymref(pp.symtab, args)))
