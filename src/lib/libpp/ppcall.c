...
ppcall(struct ppsymbol *sym, int tok)
{
    ...
    if (mac = sym->macro) {
        ...
        if ((sym->flags & SYM_PREDICATE)
            ...
            error(1,
                  "%s: macro definition overrides assertion: use #%s ...",
                  sym->name,
                  sym->name);
        if (sym->flags & SYM_DISABLED)
#if COMPATIBLE
            if ((pp.state & (COMPATIBILITY | TRANSITION)) != COMPATIBILITY
                || !mac->arity)
#endif
            {
                pp.mode |= MARKMACRO;
                ...
                if (old_next)
                    ...
                    if (c) {
                        ...
                        if ((pp.state & (COMPATIBILITY | STRICT))
                            == (COMPATIBILITY | STRICT))
                            error(
                            1, "%s: macro arguments omitted", sym->name);
#endif
        ...
        keyarg:
            if (pp.option & KEYARGS) {
                ...
                for (c = 0; c < mac->arity; c++)
                    if (streq(pp.token, mac->args.key[c].name))
                        break;
                if (c >= mac->arity)
                        ...
                        *p++ = 0;
...
#endif
                        {
                            if ((pp.state & STRICT) && p == mp->arg[c])
                                error(1,
                                      "%s: macro call argument %d is null",
                                      sym->name,
                                      c + 1);
                            if (c < mac->arity)
                                c++;
        ...
        endactuals:
            ...
            if (pp.in != old_in) {
                ...
                if (kp)
                    error(2,
                          "%s: macro call starts and ends in different files",
                          sym->name);
            }
...
#endif
            {
                ...
                if (c != (n = mac->arity) && (c > 0 || n > 1)
                    ...
                    if (!(sym->flags & SYM_VARIADIC))
                        ...
                        error(1,
                              "%s: at least %d actual argument%s expected",
                              sym->name,
                              n,
                              n == 1 ? "" : "s");
#if COMPATIBLE
                    if (!c
                        && (pp.state & (COMPATIBILITY | STRICT))
                           == (COMPATIBILITY | STRICT))
                        goto disable;
#endif
                }
