...
#define ppiskey(t, v, p)                                                     \
    (p = t, v >= p->value && value <= (p + elementsof(t) - 2)->value)
...
ppset(long *p, long op, int val)
{
    ...
    r = p == &pp.state ? &pp.ro_state
                       : p == &pp.mode ? &pp.ro_mode : &pp.ro_option;
    if ((pp.mode & INIT) && pp.in->type == IN_FILE && (*r & op)) {
        debug((-7,
               "set %s %s skipped -- readonly",
               p == &pp.state ? "state" : p == &pp.mode ? "mode" : "option",
               p == &pp.state
               ? ppstatestr(*r)
               : p == &pp.mode ? ppmodestr(*r) : ppoptionstr(*r)));
        return 0;
    }
    if (!pp.initialized && (!(pp.mode & INIT) || !(pp.mode & BUILTIN))
        && (p != &pp.mode || !(op & BUILTIN))
        && (p != &pp.option || !(op & PREDEFINED))) {
        *r |= op;
        debug((-7,
               "set %s %s readonly",
               p == &pp.state ? "state" : p == &pp.mode ? "mode" : "option",
...
ppkeyname(int value, int dir)
{
    ...
    if (dir && ppiskey(directives, value, p)
        || !dir
           && (ppiskey(options, value, p) || ppiskey(predicates, value, p)
               || ppiskey(variables, value, p))) {
...
ppmapinclude(char *file, char *s)
{
    ...
    if (s)
        ...
        if (*file == '-') {
            if (!error_info.file) {
                error(1,
                      "%s: input file name required for %s ignore",
                      file,
                      dirname(INCLUDE));
                return;
            }
...
#if CATSTRINGS
    pp.state
    |= (COMPILE | FILEPOP | HEADER | JOINING | STRIP | NOSPACE | PASSEOF);
#else
        ...
        case T_HEADER:
            if (fp) {
                ...
                for (dp = pp.firstdir->next; dp; dp = dp->next)
                    if (dp->name && (c = strlen(dp->name))
                        && !strncmp(dp->name, fp->name, c)
                        && fp->name[c] == '/') {
...
identical(char *file, int fd)
{
    ...
    return !stat(file, &a) && !fstat(fd, &b) && a.st_dev == b.st_dev
           && a.st_ino == b.st_ino;
}
...
{
    return !strchr(b, ' ') && !strneq(b, "__STD", 5)
           ? strncmp(a, b, pp.truncate)
           : strcmp(a, b);
}
...
trunchash(char *a)
{
    ...
    return memhash(a,
                   (n = strlen(a)) > pp.truncate && !strchr(a, ' ')
                   && !strneq(a, "__STD", 5)
                   ? pp.truncate
                   : n);
}
...
ppok(int op)
{
    ...
    if ((pp.mode & INIT) && pp.in->type == IN_FILE && (*r & n)) {
        ...
        return 0;
    } else if (!pp.initialized
               && (!(pp.mode & INIT) || !(pp.mode & BUILTIN))) {
        *r |= n;
...
ppop(int op, ...)
{
    ...
    case PP_COMPATIBILITY:
        if (ppset(&pp.state, COMPATIBILITY, va_arg(ap, int))) {
            ...
            if (pp.state & COMPATIBILITY)
                error(3,
                      "preprocessor not compiled with compatibility dialect "
                      "enabled [COMPATIBLE]");
#endif
            if (pp.state & COMPATIBILITY)
    ...
    case PP_DONE:
        ...
        if (pp.state & STANDALONE) {
            if ((pp.state & (NOTEXT | HIDDEN)) == HIDDEN
                && pplastout() != '\n')
                ppputchar('\n');
            ppflushout();
        }
    ...
    case PP_DUMP:
        ...
        if (pp.mode & DUMP)
            error(
            3,
            "preprocessor not compiled with checkpoint enabled [CHECKPOINT]");
#endif
        break;
    case PP_FILEDEPS:
        if (n = va_arg(ap, int))
                ...
                if (pp.ppdefault && *pp.ppdefault) {
                    ...
                    if (ppsearch(pp.ppdefault, T_STRING, SEARCH_EXISTS) < 0) {
                        ...
                        if (!(pp.ppdefault
                            ...
                            error(
                            1,
                            "cannot determine default definitions for %s",
                            pp.probe ? pp.probe : PPPROBE);
                    }
                    if (pp.probe)
...
",
                ...
                if (error_info.trace)
                    for (dp = pp.firstdir; dp; dp = dp->next)
                        message((-1,
                                 "include directory %s%s%s%s",
                                 dp->name,
                                 (dp->type & TYPE_VENDOR) ? " [VENDOR]" : "",
                                 (dp->type & TYPE_HOSTED) ? " [HOSTED]" : "",
                                 dp->c ? " [C]" : ""));
#if DEBUG
            }
            if (pp.test & TEST_nonoise)
...
#include <preroot.h>
            ...
            if (pp.ignoresrc) {
                if (pp.ignoresrc > 1 && pp.stddirs != pp.firstdir)
                    error(1,
                          "directories up to and including %s are for "
                          "\"...\" include files only",
                          pp.stddirs->name);
        ...
        if (pp.reset.on) {
            ...
            pp.symtab = hashalloc(NiL,
                                  HASH_name,
                                  "symbols",
                                  HASH_free,
                                  undefine,
                                  HASH_set,
                                  HASH_ALLOCATE | HASH_BUCKET,
                                  0);
        ...
        if (pp.mode & DUMP) {
            if (!pp.pragma)
                error(
                3, "#%s must be enabled for checkpoints", dirname(PRAGMA));
            (*pp.pragma)(
            dirname(PRAGMA), pp.pass, keyname(X_CHECKPOINT), pp.checkpoint, 1);
        }
#endif
...
#    endif
...
#else
            error(3,
                  "preprocessor not compiled with macro keyword arguments "
                  "enabled [MACKEYARGS]");
#endif
        break;
    case PP_LINE:
    ...
    case PP_PLUSCOMMENT:
        ...
        break;
    case PP_PLUSPLUS:
        if (ppset(&pp.option, PLUSPLUS, va_arg(ap, int))
...
#else
            error(3,
                  "preprocessor not compiled with input pool enabled [POOL]");
#endif
        }
        break;
    case PP_PRAGMA:
        ...
        if ((pp.state & COMPILE) && (p = va_arg(ap, char *))) {
            ...
            if (*t == '_')
                *t = 0;
                ...
                t = 0;
            op
            = ((key = ppkeyref(pp.symtab, s)) && (key->sym.flags & SYM_LEX))
              ? key->lex
              : T_NOISE;
            if (pp.test & 0x0400)
    ...
    before:
        error(3,
              "ppop(%d): preprocessor operation must be done before PP_INIT",
              op);
        break;
    default:
