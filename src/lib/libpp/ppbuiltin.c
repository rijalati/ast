...
ppbuiltin(void)
{
    ...
    *(a = pp.args) = 0;
    ...
    default:
        ...
        case V_STDC:
            p = pp.valbuf;
            p[0] = ((pp.state & (COMPATIBILITY | TRANSITION))
                    || (pp.mode & (HOSTED | HOSTEDTRANSITION))
                       == (HOSTED | HOSTEDTRANSITION))
                   ? '0'
                   : '1';
            p[1] = 0;
            break;
...
#define BACK(a, p)                                                           \
    ((a > p) ? *--a                                                          \
             : (number++ ? 0                                                 \
                         : ((p = pp.outbuf + PPBUFSIZ),                      \
                            (a = pp.outbuf + 2 * PPBUFSIZ),                  \
                            *--a)))
#define PEEK(a, p)                                                           \
    ((a > p) ? *(a - 1) : (number ? 0 : *(pp.outbuf + 2 * PPBUFSIZ - 1)))
