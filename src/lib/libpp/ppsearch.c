...
ppmultiple(struct ppfile *fp, struct ppsymbol *test)
{
    ...
    status = fp->guard;
    message((-3,
             "search: %s: status=%s%s test=%s",
             fp->name,
             status == INC_CLEAR
             ? "[CLEAR]"
             : status == INC_TEST
               ? "[ONCE]"
               : status == INC_IGNORE ? "[IGNORE]" : status->name,
        ...
        if (status != INC_CLEAR) {
            if (status == INC_TEST || status->macro) {
                if ((pp.mode & (ALLMULTIPLE | LOADING)) == LOADING)
                    fp->guard = INC_IGNORE;
                if ((pp.state & WARN)
                    && (pp.mode & (HOSTED | MARKHOSTED | RELAX | PEDANTIC))
                       == PEDANTIC)
                    error(1, "%s: ignored -- already included", fp->name);
                else
                    message(
                    (-3, "%s: ignored -- already included", fp->name));
                return 0;
            }
            return 1;
        }
...
search(struct ppfile *fp, struct ppdirs *dp, int type, int flags)
{
    ...
    message(
    (-3,
     "search: %s %s%s%s%s%s%s type=%s prefix=%s flags=|%s%s%s%s%s%s "
     "start=%s=\"%s\" pre=%s lcl=%s vnd=%s std=%s cur=%s",
     fp->name,
     ...
     dp ? (dp->index == INC_PREFIX
           ? "pre"
           : dp->index == INC_LOCAL ? "lcl"
                                    : dp->index == INC_VENDOR ? "vnd" : "std")
        : NiL,
     dp ? dp->name : NiL,
     !(fp->flags & INC_MEMBER(INC_PREFIX)) && fp->bound[INC_PREFIX]
     ? fp->bound[INC_PREFIX]->name
     : NiL,
     !(fp->flags & INC_MEMBER(INC_LOCAL)) && fp->bound[INC_LOCAL]
     ? fp->bound[INC_LOCAL]->name
     : NiL,
     !(fp->flags & INC_MEMBER(INC_VENDOR)) && fp->bound[INC_VENDOR]
     ? fp->bound[INC_VENDOR]->name
     : NiL,
     !(fp->flags & INC_MEMBER(INC_STANDARD)) && (xp = fp->bound[INC_STANDARD])
     ? xp->name
     : NiL,
     error_info.file));
...
#    else
                            error(1,
                                  "preprocessor not compiled with checkpoint "
                                  "enabled");
                            goto notvdb;
#    endif
                        case X_HIDE:
                            ...
                            if (t)
                                error(
                                1, "%s: %s: archive option value ignored", e);
                        ...
                        case X_MAP:
                            if (!t)
                                error(
                                1, "%s: archive option value expected", e);
                            else
                                dp->name = strdup(t);
                            break;
                        default:
                    ...
                    notvdb:
                        sfclose(sp);
                        error(
                        1, "%s: ignored -- cannot load archive", dp->name);
                        dp->type = 0;
                        continue;
                    }
                    if (variant = *s != 0)
                    ...
                    off
                    = strtol(s + VDB_OFFSET, NiL, 10) - sizeof(VDB_DIRECTORY);
...
#    endif
            ...
            if (pp.test & 0x0040)
                error(1,
                      "SEARCH#%d dir=%s%s%s%s%s file=%s%s path=%s index=%d",
                      __LINE__,
                      dp->name,
                      (dp->type & TYPE_ARCHIVE) ? " ARCHIVE" : "",
                      (dp->type & TYPE_BUFFER) ? " BUFFER" : "",
                      (dp->type & TYPE_CHECKPOINT) ? " CHECKPOINT" : "",
                      (dp->type & TYPE_DIRECTORY) ? " DIRECTORY" : "",
                      fp->name,
                      (fp->flags & INC_MEMBER(index)) ? " MEMBER" : "",
                      pp.path,
                      index);
            ...
            if (errno == EMFILE)
                ...
                error(ERROR_SYSTEM | 1,
                      "%s: cannot open file for reading",
                      pp.path);
            if (*fp->name == '/')
                break;
        }
