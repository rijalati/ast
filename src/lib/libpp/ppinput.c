...
#    endif
                p = (cur->buffer
                     = oldof(0, char, 0, pp.member->size + PPBAKSIZ + 1))
                    + PPBAKSIZ;
                if (sfseek(
                    pp.member->archive->info.sp, pp.member->offset, SEEK_SET)
                    != pp.member->offset)
                    error(
                    3, "%s: archive seek error", pp.member->archive->name);
                if (sfread(pp.member->archive->info.sp, p, pp.member->size)
                    != pp.member->size)
                    error(
                    3, "%s: archive read error", pp.member->archive->name);
                pp.member = 0;
                break;
            case TYPE_BUFFER:
#    if CHECKPOINT
...
#    endif
                p = cur->buffer
                = pp.member->archive->info.buffer + pp.member->offset;
                cur->flags |= IN_static;
                pp.member = 0;
                break;
#    if CHECKPOINT
...
#    endif
...
#endif
        {
            ...
            if (!(pp.option & NOPROTO) && !(pp.test & TEST_noproto)
                ...
                && (cur->buffer = pppopen(
                    ...
                    NiL,
                    NiL,
                    (PROTO_HEADER | PROTO_RETAIN)
                    | (((pp.mode & EXTERNALIZE) || (pp.option & PROTOTYPED))
                       ? PROTO_FORCE
                       : PROTO_PASS)
        ...
        if (pp.incref && !(pp.mode & INIT))
            (*pp.incref)(
            error_info.file, s, error_info.line - 1, PP_SYNC_PUSH);
    ...
    default:
        error(PANIC,
              "use PUSH_<%d>(...) instead of pppush(IN_<%d>, ...)",
              cur->type,
              cur->type);
        break;
#endif
    ...
    debug(
    (-7, "PUSH in=%s next=%s", ppinstr(pp.in), pptokchr(*pp.in->nextchr)));
}
...
#    endif
    {
        ...
        if (pp.in->fd >= 0) {
            ...
            if (!(sp = sfnew(NiL,
                             pp.in->buffer
                             + ((pp.in->flags & IN_static) ? 0 : PPBAKSIZ),
                             file_size,
                             -1,
                             SF_READ | SF_STRING)))
                error(3, "checkpoint read error");
    ...
    while (*b) {
        ...
        if (pp.test & 0x0200)
            error(2,
                  "%s: %s p=%lu next=<%lu,%lu> keep=<%lu,%lu> skip=<-,%lu> "
                  "guard=%s",
    ...
    if (pp.test & 0x0100)
        error(2, "%s: loop", keyname(X_CHECKPOINT));
