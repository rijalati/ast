...
= "[-?\n@(#)$Id: cpp (AT&T Research) 2013-08-11 $\n]" USAGE_LICENSE
  "[+NAME?cpp - C language preprocessor]"
  "[+DESCRIPTION?\bcpp\b is the preprocessor for all C language dialects. It "
  "is"
  ...
  "[+?If \aoutput\a is omitted then the standard output is written; if "
  "\ainput\a"
        ...
        for (; c = optget(argv, usage); last = 0)
            ...
            case 'D':
                ...
                case '_':
                    ...
                    else if (strmatch(s, "__(ANSI|STDC|STRICT)__*")
                             || !(pp.arg_style & STYLE_gnu)
                                && strmatch(s, "__STRICT_ANSI__*"))
                        ppop(PP_STRICT, 1);
            ...
            case 'A':
                if (isalpha(opt_info.arg[0]) || opt_info.arg[0] == '_'
                    || opt_info.arg[0] == '$')
            ...
            case 'V':
                ...
                break;
            case 'X':
                pp.arg_mode = (*(opt_info.arg + 1)
                               || pp.arg_mode && pp.arg_mode != *opt_info.arg)
                              ? '-'
                              : *opt_info.arg;
                break;
            case 'Y':
...
#pragma pp:allmultiple\n\
                ...
                s += opt_info.offset - 1;
                if (strmatch(s, "i*.h"))
                    ppop((pp.arg_style & STYLE_gnu) || s[1] == '/' ? PP_READ
                                                                   : PP_TEXT,
                         s + 1);
            ...
            ignore:
                while (argv[opt_info.index][opt_info.offset])
                    opt_info.offset++;
                break;
            }
        if (!(s = argv[opt_info.index]))
