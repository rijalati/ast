...
long_print(Sum_t *p, Sfio_t *sp, int flags, size_t scale)
{
    ...
    if (flags & SUM_TOTAL)
        sfprintf(sp,
                 " %*I*u",
                 (flags & SUM_LEGACY) ? 6 : 0,
                 sizeof(x->total_count),
                 x->total_count);
    return 0;
}
...
#define METHOD(x)                                                            \
    x##_match, x##_description, x##_options, x##_open, x##_init, x##_block,  \
    x##_data, x##_print, x##_done, x##_scale, x##_flags

static const Method_t methods[] = {
    METHOD(att),    METHOD(ast4), METHOD(bsd), METHOD(crc), METHOD(prng),
...
static const Map_t maps[] = {
    ...
    { "zip", "The \bzip\b(1) crc.", "crc-0xedb88320-init-done" },
    { "fddi", "The FDDI crc.", "crc-0xedb88320-size=0xcc55cc55" },
    { "fnv|fnv1",
